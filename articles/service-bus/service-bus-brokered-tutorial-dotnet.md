...
1. サービス名前空間を作成する」の手順に従います [How To: 作成または Service Bus Service 名前空間を変更する](https://msdn.microsoft.com/library/azure/hh690931.aspx)します。
...
<br />
1. ソリューション エクスプローラーで、Program.cs ファイルをダブルクリックして、Visual Studio エディターでそのファイルを開きます。 既定の名前から名前空間の名前を変更する `QueueSample` に `Microsoft.ServiceBus.Samples`します。
...
1. 変更、 `を使用して` ステートメントの次のコードに示すようにします。
...
1. 手順 6 で作成した Data.csv ファイルの場所に移動します。 ファイルをクリックし、**[追加]** をクリックします。 いることを確認 **のすべてのファイル (*. *)** ファイルの種類の一覧で選択されています。
...
1. 外部 `Main()`, 、定義、 `読み込む ParseCSV()` で Data.csv でメッセージの一覧を解析し、メッセージにするメソッド、 [DataTable](https://msdn.microsoft.com/library/azure/system.data.datatable.aspx) テーブル、次のようにします。 このメソッドは、**DataTable** オブジェクトを返します。
...
1. `Main()` メソッドを呼び出すステートメントを追加、 `ParseCSVFile()` メソッド。
...
1. 外部 `Main()`, 、定義、 `読み込む GenerateMessages()` を受け取るメソッド、 **DataTable** によって返されるオブジェクト `ParseCSVFile()` して厳密に型指定された仲介型メッセージの一覧にテーブルを読み込みます。 次の例で示すように、このメソッドは **List** オブジェクトを返します。
...
1. `Main()`, への呼び出しのすぐ下 `ParseCSVFile()`, を呼び出すステートメントを追加、 `読み込む GenerateMessages()` からの戻り値を持つメソッド `ParseCSVFile()` を引数として。
...
1. 次に、サービス名前空間と SAS キーを受け取って格納する関数を作成します。 外側には、このメソッドを追加 `Main()`します。 次に例を示します。
...
1. `Main()`, への呼び出しのすぐ下 `読み込む GenerateMessages()`, を呼び出すステートメントを追加、 `CollectUserInput()` メソッド。
...
1. わかりやすくするため、このチュートリアルではすべてのキュー操作を独立したメソッドに配置します。 作成、 `Queue()` メソッドで、 `プログラム` クラス以下、 `Main()` メソッドです。 次に例を示します。
...
1. 次の手順が使用して、SAS 資格情報を作成するには、 [TokenProvider](https://msdn.microsoft.com/library/azure/microsoft.servicebus.tokenprovider.aspx) オブジェクトです。 作成メソッドは SAS キーの名前と値の取得に、 `CollectUserInput()` メソッドです。 次のコードを追加、 `Queue()` メソッド。
...
1. 最初に、キューを作成します。 たとえば、「 `myQueue`, 、し、最後の手順で追加した管理操作の直後で宣言します。
...
1. `Queue()` メソッドを引数として新しく作成したサービス バス URI を持つ、メッセージング ファクトリ オブジェクトを作成します。 1 つ前の手順で追加した管理操作の直後に次のコードを追加します。
...
1. 次に、前に作成した仲介型メッセージのリストをループして各メッセージをキューに送信するコードを追加します。 次のコードを追加直接後、 `CreateQueueClient()` 前の手順でステートメント。
...
呼び出すステートメントを追加する最後の手順では、 `Queue()` メソッドから `Main()`します。 Main() の最後に、次の強調表示されているコード行を追加します。
...
1. ブラウザーを開きに移動、 [Azure クラシック ポータルの [][]します。
...
このチュートリアルでは、Service Bus の仲介型メッセージング機能を使用して、Service Bus クライアント アプリケーションとサービスを構築する方法を紹介しました。 Service Bus を使用する類似のチュートリアル [リレー型メッセージング](service-bus-messaging-overview.md/#Relayed-messaging), を参照してください、 [Service Bus リレー メッセージのチュートリアル](service-bus-relay-tutorial.md)します。
