...
Service Bus 名前空間は、スケール ユニットにマップされます。 Service Bus エンティティのすべての種類を処理するスケールの単位: リレーと仲介型のメッセージング エンティティ (キュー、トピック、サブスクリプション)。 Service Bus スケール ユニットは、次のコンポーネントで構成されています。
...
- [Service Bus キューを使用してキューに置かれたメッセージング ソリューション](service-bus-dotnet-multi-tier-app-using-service-bus-queues.md)
