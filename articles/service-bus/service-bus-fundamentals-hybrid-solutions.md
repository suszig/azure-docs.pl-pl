...
もう 1 つのオプションである *PeekLock* は、この問題に対処できます。 ような **ReceiveAndDelete**, 、 **PeekLock** 読み取りは、キューからメッセージを削除します。 ただし、実際に削除する前に、 メッセージをロックして他の受信者が認識できないようにして、次の 3 つのイベントのいずれかの発生を待ちます。
...
- サブスクライバー 2 は、*Seller="Ruby"* というプロパティを含むか、*Amount* プロパティの値が 100,000 を超えるメッセージを受信します。 Ruby はセールス マネージャーで、自分の売り上げと、担当者を問わずすべての大きな売り上げを確認するようにします。
...
- を使用する方法 [Service Bus キュー](service-bus-dotnet-how-to-use-queues.md)
- を使用する方法 [Service Bus トピック](service-bus-dotnet-how-to-use-topics-subscriptions.md)
- を使用する方法 [Service Bus リレー](service-bus-dotnet-how-to-use-relay.md)
- [Service Bus のサンプル](service-bus-samples.md)
