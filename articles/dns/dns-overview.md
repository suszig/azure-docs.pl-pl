...
このサービスは、Azure リソース マネージャー (ARM) に基づいています。ドメインとレコードは、Azure リソース マネージャー REST API、.NET SDK、PowerShell コマンドレット、コマンド ライン インターフェイスを使用して管理できます。Azure DNS は、現在はプレビュー段階にあり、Azure 管理ポータルではまだサポートされていません。<BR><BR>
Azure DNS では、現在、ドメイン名の購入はサポートされていません。 ドメインを購入するには、サード パーティのドメイン名レジストラーを使用してください。通常、これには、少額の年間料金がかかります。 DNS レコードの管理用 Azure DNS でこれらのドメインがホストし、ことができます-を参照してください [Azure DNS へのドメインの委任](dns-domain-delegation.md) 詳細です。


## 次のステップ

[DNS ゾーン作成の概要します。](dns-getstarted-create-dnszone.md)
