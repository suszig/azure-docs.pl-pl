...
ドメイン "contoso.com" には、"mail.contoso.com" (メール サーバー用) や "www.contoso.com" (Web サイト用) など、多数の DNS レコードが含まれている場合があります。DNS ゾーンは、特定のドメインの DNS レコードをホストするために使用します。<BR><BR>
ドメインのホストを開始するには、まず DNS ゾーンを作成する必要があります。DNS レコードは、特定のドメイン用に作成されたドメインの DNS ゾーン内に含まれます。<BR><BR>
...
Azure DNS では、Azure リソース マネージャー (ARM) を使用します。Azure PowerShell 1.0.0 以降の場合は次の手順を実行します。詳細については、「 [リソース マネージャーで Windows Powershell を使用して](powershell-azure-resource-manager.md)します。<BR><BR>
...
DNS ゾーンは、New-AzureRmDnsZone コマンドレットを使用して作成します。次の例では、"contoso.com"というリソース グループに"myresourcegroup"DNS ゾーンを作成します。<BR>

    PS C:\> New-AzureRmDnsZone -Name contoso.com -ResourceGroupName MyAzureResourceGroup

>[AZURE.NOTE] Azure DNS では、ゾーン名を指定する際に末尾に '.' を付けないでください。たとえば、として"contoso.com"ではなく"contoso.com"です。<BR>
