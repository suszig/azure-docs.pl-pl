...
>[AZURE.NOTE] 特定のドメイン名の所有者やドメインの購入方法の詳細について詳細を調べるを参照してください。 [Azure AD でのインターネット ドメイン管理](https://msdn.microsoft.com/library/azure/hh969248.aspx)します。
...
- A _再帰_ DNS サーバーが DNS ゾーンをホストしません。  このサーバーは、権限のある DNS サーバーを呼び出して必要なデータを収集することで、すべての DNS クエリに応答します。
...
>[AZURE.NOTE] Azure DNS では、そのドメイン名で DNS ゾーンを作成するために、ドメインを所有する必要はありません。  ただし、レジストラーを使用して Azure DNS への委任を設定するには、ドメインを所有する必要があります。
