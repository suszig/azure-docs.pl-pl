...
_Fully qualified_ 名には、ゾーンの名前が含まれますが、 _相対_ 名がないです。 たとえば、"contoso.com" ゾーン内の相対レコード名 "www" によって、完全修飾レコード名 "www.contoso.com" が示されます。
...
Azure DNS では、一般的な DNS レコードの種類である A、AAAA、CNAME、MX、NS、SOA、SRV、TXT をすべてサポートしています。 (そのに注意してください [SPF レコードは TXT レコードの種類を使用して作成する必要があります](http://tools.ietf.org/html/rfc7208#section-3.1).)
...
次の例では、レコード セットとレコードの作成方法を示します。 その他のレコードの種類がを参照してください、'A' の DNS レコードの種類を使用します [DNS レコードを管理する方法](dns-operations-recordsets-cli.md)
...
[DNS ゾーンを管理する方法](dns-operations-dnszones-cli.md)

[DNS レコードを管理する方法](dns-operations-recordsets-cli.md)<BR>

[.NET SDK を使用した Azure の操作を自動化します。](dns-sdk.md)
