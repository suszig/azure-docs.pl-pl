...
Interfejsów API obsługi komunikatów generowania wyjątków, które można podzielić na następujące kategorie, wraz ze skojarzonej akcji, które należy wykonać, aby je rozwiązać. Należy pamiętać, że znaczenie i przyczyny wyjątek może się różnić w zależności od typu jednostki obsługi komunikatów (kolejki/tematy lub Event Hubs):
...
3. Wyjątki przejściowy ([Microsoft.ServiceBus.Messaging.MessagingException](/dotnet/api/microsoft.servicebus.messaging.messagingexception), [Microsoft.ServiceBus.Messaging.ServerBusyException](/dotnet/api/microsoft.azure.servicebus.serverbusyexception), [Microsoft.ServiceBus.Messaging.MessagingCommunicationException](/dotnet/api/microsoft.servicebus.messaging.messagingcommunicationexception)). Akcja ogólne: ponów operację lub powiadomić użytkowników.
