...
Smartfony i tablety mają możliwość „powiadamiania” użytkowników o wystąpieniu zdarzenia. Te powiadomienia mogą mieć wiele form.
...
Powiadomienia wypychane są dostarczane przy użyciu infrastruktur dla poszczególnych platform nazywanych _systemami powiadomień platformy_ (PNS, Platform Notification System). System powiadomień platformy zapewnia podstawowe funkcje (tj. nie obsługuje emisji i personalizacji) i nie ma wspólnego interfejsu. Aby na przykład wysłać powiadomienie do aplikacji ze Sklepu Windows, deweloper musi skontaktować się z usługą WNS (Windows Notification Service), a aby wysłać powiadomienie do urządzenia z systemem iOS, ten sam deweloper musi skontaktować się z usługą APNs (Apple Push Notification Service) i wysłać wiadomość po raz drugi. Usługa Azure Notification Hubs pomaga, zapewniając wspólny interfejs oraz inne funkcje do obsługi powiadomień wypychanych na każdej platformie.
...
W celu umożliwienia bezproblemowej i jednorodnej obsługi we wszystkich usługach Azure usługa [App Service Mobile Apps] ma wbudowaną obsługę powiadomień wypychanych przy użyciu usługi Notification Hubs. Usługa [App Service Mobile Apps] oferuje wysoce skalowalną, globalnie dostępną platformę tworzenia aplikacji mobilnych dla deweloperów w przedsiębiorstwach i integratorów systemów. Platforma ta oferuje bogaty zestaw funkcji dla deweloperów aplikacji mobilnych.
...
- **Zintegrowane środowisko w [Portalu Azure].** Wypychanie jako możliwość ma wizualną reprezentację w usłudze Mobile Apps, a deweloperzy mogą z łatwością pracować przy użyciu skojarzonego centrum powiadomień za pomocą usługi Mobile Apps.
