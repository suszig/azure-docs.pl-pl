...
| .(prop, n)| 上記と同じですが、テキストが省略されているので、テキストに 3 つのドットのサフィックスが付いています。省略された文字列とサフィックスの合計サイズは、n 個の文字を超えていません。.(タイトル、20) で入力プロパティが"This is the title line"の結果の **title... これは**|
| %(prop)| 出力が URI エンコードされる点を除き、$(name) と同様です。|
| #(prop)| (たとえば、iOS や Android テンプレート) は、JSON テンプレートで使用します。<br><br>この関数は指定されていた、JSON テンプレート (Apple テンプレートなど) で使用する場合を除く $ (prop) とまったく同じです。この場合、この関数がで囲まれておらず「{', '}」(たとえば、'myJsonProperty': '#(name)')、として評価される Javascript の書式による数値など regexp: (0 と #124 文字です ((& a) #91; 1 ~ 9 & #93; #91、0 ~ 9 および #93; *)) (\。 (& a) #91; 0 ~ 9 & #93; +) ですか?。((e & #124 文字です。E) (+ (& a) #124;-) ですか? (& a) #91; 0 ~ 9 & #93; +) ですか?、出力 JSON は数字です。<br><br>など ' バッジ: '#(name)' が 'badge': 40 (および '40' ではありません)。|
