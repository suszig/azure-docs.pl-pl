...
4. App.xaml.cs プロジェクト ファイルで、次のプロパティを **App** クラスに追加します。置き換える、 `< ハブの名前 >` と `< listen アクセス権限を持つ接続文字列 >` プレース ホルダーを通知ハブの名前との接続文字列 *DefaultListenSharedAccessSignature* 既に取得してあります。
