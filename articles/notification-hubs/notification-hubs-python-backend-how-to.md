...
> [AZURE.NOTE] これは Python で通知の送信を実装するためのサンプル参照実装はであり、正式にサポートされている通知ハブの Python SDK ではありません。
...
> [AZURE.NOTE] このサンプルの範囲は通知を送信するのみ制限し、登録の管理は行いません。
...
次のメソッドを追加する必要が、 **NotificationHub** トークンを作成するクラスの現在の要求と接続文字列から抽出した資格情報の URI に基づいています。
...
最近追加されましたこのプロパティと呼ばれる [通知ハブの TestSend プロパティ。](http://msdn.microsoft.com/library/microsoft.servicebus.notifications.notificationhubclient.enabletestsend.aspx)
