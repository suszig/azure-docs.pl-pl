...
8. PushToUserAppDelegate.m の **didFinishLaunchingWithOptions** メソッドで、次のコードを追加します。
 ...
 最初の行では、**DeviceInfo** シングルトンが初期化されます。 2 行目は、プッシュ通知で、既に提示は、[通知ハブの「チュートリアルを既に終えている場合に、登録を開始します。
...
9. PushToUserAppDelegate.m で、AppDelegate に **didRegisterForRemoteNotificationsWithDeviceToken** メソッドを実装し、次のコードを追加します。
    ...
    > [AZURE.NOTE] この時点では、このメソッドに他のコードは存在しません。 呼び出しが既にある場合、 **registerNativeWithDeviceToken** メソッドを実行したときに追加された、 [Notification Hubs を使ってみる](/manage/services/notification-hubs/get-started-notification-hubs-ios/) チュートリアル, する必要がありますコメント アウトまたはその呼び出しを削除します。
