...
通知の一般的な送信フローからメッセージを送信、 **アプリケーション バックエンド** に **Azure 通知ハブ (NH)** をさらに処理を行い、すべての登録が構成されているタグとタグ式「目標」の決定を考慮に入れてに対してつまりすべての登録にプッシュ通知を受け取る必要があります。 これらの登録は、iOS、Google、Windows、Windows Phone、Kindle、Baidu for China Android など、サポートされているプラットフォームのいずれかまたはすべてにわたる可能性があります。 NH は、プッシュ通知をデバイス プラットフォーム固有の登録の複数のバッチに分割されたターゲットが確立されると、 **プッシュ通知サービス (PNS)** -例: Apple の APNS、Google の GCM などです。Notification Hubs は、Azure クラシック ポータルの [Notification Hubs の構成] ページで設定した資格情報に基づいて、各 PNS への認証を実行します。 PNS が通知を各を転送し、 **クライアント デバイス**します。 これは、プッシュ通知を配信するためにプラットフォームで推奨されている方法です。通知の配信の最終段階はプラットフォームの PNS とデバイスの間で実行される点に注目してください。 次の 4 つの主要なコンポーネントがあるため *クライアント*, 、*アプリケーション バックエンド*, 、*Azure Notification Hubs (NH)* と *プッシュ通知サービス (PNS)* 、通知の脱落が次のいずれかが発生します。 このアーキテクチャの詳細については、[利用可能な [Notification Hubs Overview]します。
...
1. **全般**
    ...
    b) クライアントとアプリケーション バックエンドで正しい SAS 構成文字列を使用していることを確認します。 経験上、としてを使用していなければなりません、 **DefaultListenSharedAccessSignature** クライアント上および **DefaultFullSharedAccessSignature** (これにより、NH に通知を送信できる) アプリケーションのバックエンドで
...
対象ユーザーのセグメント化にタグまたはタグ式を使用している場合は、通知を送信するときに、送信呼び出しで指定したタグまたはタグ式に基づくターゲットが見つからない可能性が常にあります。 登録を確認して、通知を送信するときに一致するタグがあることを確認し、それらが登録されているクライアントから受信した通知のみを検証することをお勧めします。 例:  タグ"Politics"と答えるとすべての登録 NH とで、"Sports"タグを持つ通知送信する場合は任意のデバイスに送信されません。 複雑なケースでは、「Tag A」OR「Tag B」というタグ式のみを登録したにもかかわらず、「Tag A && Tag B」を対象として通知を送信するといった例が挙げられます。 以下の「自己診断のヒント」のセクションでは、登録およびそれに含まれるタグを確認できる方法について説明しています。 

2) **テンプレートの問題**

ガイドラインが記載されている場合は、テンプレートを使用するをフォローしていることを確認 [Template guidance]します。 
...
ソース - [APNS guidance] (& a) [GCM guidance]
...
1. **PNS 開発者ポータル**
    ...
    各 PNS 開発者ポータル (APNS、GCM、WNS など) を使用して、確認して、 [Getting Started Tutorials]します。
...
1. **Visual Studio**
    ...
    > [AZURE.NOTE] Visual Studio の機能の登録を編集するのには、登録数が限られた開発/テスト中にのみ使用する必要があります。 登録を一括で修正する必要がある場合は、エクスポート/インポートを使用することを検討してください - ここで説明されている登録機能 [Export/Import Registrations] (https://msdn.microsoft.com/library/dn790624.aspx)

2. **Service Bus エクスプローラー**

    多くの顧客がここに説明を使用して [ServiceBus Explorer] の表示と、通知ハブを管理します。 これは、オープン ソース プロジェクト code.microsoft.com- [ServiceBus Explorer code]
...
PNS のエラーを把握と呼ばれるプロパティが導入されています [EnableTestSend feature]します。 このプロパティは、ポータルまたは Visual Studio クライアントからテスト メッセージを送信するときに自動的に有効にされ、詳細なデバッグ情報を表示することができます。 これは、現在 .NET SDK などの API で利用可能で、将来的にすべてのクライアント SDK に追加される予定です。 これを REST 呼び出しで使用するには、送信呼び出しの最後に「test」というクエリ文字列パラメーターを追加します。以下がその例です。 
...
`result.State`result.State では、実行の最後に単に `Enqueued` が返され、プッシュに何が起こったのかを確認することはできません。 
...
> [AZURE.NOTE] このプロパティの使用が制限される頻度の高い、ためする必要がありますのみ、使用して登録の限られた開発/テスト環境でことに注意してください。 デバッグ通知は 10 のデバイスのみに送信されます。 また、1 分あたりに処理できるデバッグ送信は 10 件に制限されています。 
...
1. **Azure クラシック ポータルの使用**
    ...
    確認作業の開始 c) する必要があります、 **受信メッセージ**, 、**登録操作**, 、**正常通知** とプラットフォームのタブを PNS 固有のエラーを確認しに移動します。 
...
> [AZURE.NOTE] いくつかのテレメトリ関連機能 **登録のエクスポート/インポート**, 、**Api を使用してテレメトリ アクセス** など、標準的なレベルで利用できるのみです。 Free または Basic レベルでこれらの機能を使用しようとすると、SDK を使用している場合はこの効果に対する例外メッセージが表示され、REST API から直接使用している場合は HTTP 403 (Forbidden) が表示されます。 Azure クラシック ポータルから Standard レベルに移行してください。  
