...
4. App.xaml.cs プロジェクト ファイルで、次のプロパティを **App** クラスに追加します。
    ...
    上記のコードで置き換えます、 `< ハブの名前 >` と `< listen アクセス権限を持つ接続文字列 >` プレース ホルダーを通知ハブの名前との接続文字列 *DefaultListenSharedAccessSignature* 既に取得してあります。
...
1. 開き、App.xaml.cs ファイルの更新、 **InitNotificationsAsync** メソッドを使用する、 `通知` クラス購読するカテゴリに基づいています。
 ...
 これにより、アプリケーションが起動するたびに、ローカル ストレージからカテゴリを取得し、これらのカテゴリの登録を要求するようになります。  **InitNotificationsAsync** メソッドは、の一部として作成された、 [[入門] 通知ハブを使ってみる][get-started] チュートリアルです。
