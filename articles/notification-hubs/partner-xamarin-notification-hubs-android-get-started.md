<properties
    pageTitle="Wprowadzenie do usługi Notification Hubs dla aplikacji platformy Xamarin.Android | Microsoft Azure"
...
2. Otwórz właściwości projektu, klikając utworzony projekt prawym przyciskiem myszy w widoku Solution (Rozwiązanie) i wybierając pozycję **Options** (Opcje). Wybierz pozycję **Android Application** (Aplikacja dla systemu Android) w sekcji **Build** (Kompilacja).

    Upewnij się, że pierwsza litera w polu **Package name** (Nazwa pakietu) jest mała.

    > [AZURE.IMPORTANT] Nazwa pakietu musi zaczynać się od małej litery. W przeciwnym razie podczas rejestrowania elementów **BroadcastReceiver** i **IntentFilter** dla powiadomień wypychanych w kolejnym etapie zostaną zwrócone błędy manifestu aplikacji.

    ![](./media/partner-xamarin-notification-hubs-android-get-started/notification-hub--xamarin-android-app-options.png)


3. Opcjonalnie możesz ustawić wartość w polu **Minimum Android version** (Minimalna wersja systemu Android) na inny poziom interfejsu API.

4. Opcjonalnie możesz ustawić wartość w polu **Target Android version** (Docelowa wersja systemu Android) na inną docelową wersję interfejsu API (wymagany poziom 8 lub wyższy).

Kliknij przycisk **OK** i zamknij okno dialogowe Project Options (Opcje projektu).


###Dodawanie wymaganych składników do projektu
...
1. Zbierz następujące informacje dotyczące aplikacji dla systemu Android i centrum powiadomień:
    ...
    - **Listen connection string**: na pulpicie nawigacyjnym w [klasycznym portalu Azure] kliknij pozycję **Wyświetl parametry połączeń**. Skopiuj parametr połączenia *DefaultListenSharedAccessSignature* dla tej wartości.
    - **Hub name**: nazwa centrum wprowadzona w [klasycznym portalu Azure]. Na przykład *moje_centrum_powiadomien_2*.
...
3. Utwórz następującą metodę w klasie **MainActivity**:
...
4. W ramach metody `OnCreate` w pliku **MainActivity.cs** zainicjuj zmienną `instance` i dodaj wywołanie `RegisterWithGCM`:
...
5. W pliku **MyBroadcastReceiver.cs** dodaj następujące żądania uprawnień pomiędzy instrukcjami **using** a deklaracją **namespace**:
...
6. W pliku **MyBroadcastReceiver.cs** zmień klasę **MyBroadcastReceiver** zgodnie z poniższym kodem:
...
7. Dodaj w pliku **MyBroadcastReceiver.cs** kolejną klasę o nazwie **PushHandlerService** pochodzącą od klasy **GcmServiceBase**. Pamiętaj o zastosowaniu do klasy atrybutu **Service**:
...
8. Klasa **GcmServiceBase** implementuje metody **OnRegistered()**, **OnUnRegistered()**, **OnMessage()**, **OnRecoverableError()** oraz **OnError()**. Klasa implementacji **PushHandlerService** musi zastępować te metody, które będą uruchamiane w odpowiedzi na interakcję z centrum powiadomień.


9. Zastąp metodę **OnRegistered()** w klasie **PushHandlerService**, używając następującego kodu:
    ...
    > [AZURE.NOTE] W ramach powyższego kodu metody **OnRegistered()** zwróć uwagę na możliwość określenia tagów do rejestracji dla określonych kanałów komunikacji.

10. Zastąp metodę **OnMessage** w klasie **PushHandlerService**, używając następującego kodu:
...
12. Zastąp abstrakcyjne elementy członkowskie **OnUnRegistered()**, **OnRecoverableError()** i **OnError()**, aby umożliwić skompilowanie kodu:
...
> [AZURE.IMPORTANT] Aby otrzymywać powiadomienia wypychane, należy skonfigurować konto Google na urządzeniu AVD. W tym celu w emulatorze przejdź do pozycji **Settings** (Ustawienia) i kliknij polecenie **Add Account** (Dodaj konto). Upewnij się też, że emulator ma połączenie z Internetem.

>[AZURE.NOTE] Projekt powiadomień w systemie Android w wersji 5.0 i nowszych znacząco różni się od tego używanego we wcześniejszych wersjach. Aby uzyskać więcej informacji, zobacz [Android Notifications](http://go.microsoft.com/fwlink/?LinkId=615880) (Powiadomienia w systemie Android).
...
##Wysyłanie powiadomień z poziomu zaplecza


Odbieranie powiadomień w aplikacji możesz przetestować, wysyłając powiadomienia w [klasycznym portalu Azure] za pomocą karty Debugowanie w centrum powiadomień, jak pokazano na poniższym zrzucie ekranu.
...
- Zestaw SDK Java usługi Azure Notification Hubs: zobacz [Jak używać usługi Notification Hubs za pomocą języka Java](notification-hubs-java-backend-how-to.md), aby zapoznać się ze sposobem wysyłania powiadomień za pomocą języka Java. To rozwiązanie przetestowano w programie Eclipse do tworzenia aplikacji dla systemu Android.
- PHP: zobacz [Jak używać usługi Notification Hubs za pomocą języka PHP](notification-hubs-php-backend-how-to.md).
...
####(Opcjonalnie) Wysyłanie powiadomień za pomocą aplikacji .NET
...
5. Dodaj następującą metodę w klasie `Program`. Zmień tekst zastępczy na parametry połączenia *DefaultFullSharedAccessSignature* i nazwę centrum z [klasycznym portalu Azure].
...
1. Zaloguj się do [klasycznym portalu Azure] i wybierz usługę mobilną.
...
6. Kliknij pozycję **Uruchom raz** na dolnym pasku. W aplikacji powinno zostać odebrane wyskakujące powiadomienie.
