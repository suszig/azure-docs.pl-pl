...
このトピックでは、使用する方法、 [テンプレート](notification-hubs-templates.md) を言語およびデバイスごとにローカライズしたニュース速報通知をブロードキャストする、Azure Notification Hubs の機能です。 このチュートリアルでは、[通知ハブを使用したニュース速報の送信] で作成した iOS アプリケーションから始めます。 完了すると、興味のあるニュース速報カテゴリに登録して受信する通知の言語を指定し、選択したカテゴリのその言語のプッシュ通知だけを受信できるようになります。
...
Visual Studio にアクセスできない場合でも、ローカライズされたテンプレート通知をデバイス上のアプリから直接送信することをテストできます。 単純なことができます、ローカライズ版のテンプレート パラメーターを追加、 `SendNotificationRESTAPI` 前のチュートリアルで定義されているメソッド。
...
- [通知ハブによるユーザーへの通知: Mobile Services]
