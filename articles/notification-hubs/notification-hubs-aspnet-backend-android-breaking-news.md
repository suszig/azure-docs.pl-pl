...
3. クラスを作成 **通知** と同じパッケージに、 **MainActivity** クラスです。
...
4.  **MainActivity** クラスのプライベート フィールドを削除する **NotificationHub** と **GoogleCloudMessaging**, 、フィールドを追加して **通知**:

        // private GoogleCloudMessaging gcm;
        // private NotificationHub hub;
        private Notifications notifications;

5. 次に、 **onCreate** メソッドの初期化を削除、 **ハブ** フィールドおよび **registerWithNotificationHubs** メソッドです。 インスタンスを初期化する次の行を追加、 **通知** クラスです。 
...
> [AZURE.NOTE] Google Cloud Messaging (GCM) によって割り当てられた registrationId はいつでも変更できる、ので通知エラーを回避するには、頻繁に通知を登録する必要があります。 この例では、アプリケーションが起動するたびに通知を登録します。 頻繁に実行されるアプリケーションの場合 (1 日に複数回など)、帯域幅を節約するため、前回の登録から 1 日経過していない場合は登録をスキップできます。


1. 末尾に次のコードを追加、 **onCreate** メソッドで、 **MainActivity** クラス。
...
4. .NET コンソール アプリケーションを実行して、新しい通知を送信します。  通知ハブの [デバッグ] タブを使用して、タグ付きテンプレート通知を送信する代わりに、 [Azure Classic Portal]します。
