...
4. という新しいクラスを作成 **RegisterClient** と同じパッケージに、 `MainActivity` クラスです。 新しいクラス ファイルに次のコードを使用します。
 ...
 プッシュ通知を登録するために、このコンポーネントはアプリケーション バックエンドにアクセスするのに必要な REST 呼び出しを実装します。 これもローカルに格納されている、 *registrationIds* で詳述するよう通知ハブによって作成された [アプリ バックエンドから登録](notification-hubs-registration-management.md#registration-management-from-a-backend)します。 **[ログイン]** ボタンをクリックすると、ローカル ストレージに格納した認証トークンが使用されることに注意してください。

5. `MainActivity` クラスを削除またはコメント アウト、プライベート フィールドを `NotificationHub`, 、フィールドを追加して、 `RegisterClient` クラスと、ASP.NET バックエンドのエンドポイントの文字列です。必ずに置き換えて `< Enter Your Backend Endpoint >` で、前に取得したバックエンド エンドポイント。たとえば、 `http://mybackend.azurewebsites.net`します。

        //private NotificationHub hub;
        private RegisterClient registerClient;
        private static final String BACKEND_ENDPOINT = "<Enter Your Backend Endpoint>";

6. `MainActivity` クラスで、 `onCreate` メソッド、削除またはの初期化をコメント アウト、 `ハブ` フィールドとへの呼び出し、 `registerWithNotificationHubs` メソッドです。 インスタンスを初期化するためのコードを追加、 `RegisterClient` クラスです。 メソッドには次の行が含まれています。
...
7. `MainActivity` クラス、削除またはコメント アウト全体 `registerWithNotificationHubs` メソッドです。 このチュートリアルで使用しません。

8. 次の追加 `インポート` ステートメントを **MainActivity.java** ファイルです。
...
9. その後、**[ログイン]** ボタン クリック イベントとプッシュ通知の送信を処理する次のメソッドを追加します。
 ...
 `ログイン` のハンドラー、 **ログイン** ボタンには、基本認証が生成されますトークンのユーザー名を入力およびパスワード (認証スキームが使用する任意のトークンを表すこのメモ) を使用して、使用して `RegisterClient` に登録のためのバックエンドを呼び出します。

 `プッシュ` メソッドは、ユーザーのタグに基づいてユーザーをセキュリティで保護された通知を発生させるためのバックエンドを呼び出します。 プラットフォーム通知サービス `プッシュ` ターゲットによって異なります、 `pns` 文字列が渡されました。

10. `MainActivity` クラス、更新、 `sendNotificationButtonOnClick` メソッドを呼び出す、 `プッシュ` 選択したプラットフォーム通知サービスを次のように、ユーザーの持つメソッドです。
