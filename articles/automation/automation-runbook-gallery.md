...
>[AZURE.NOTE] Runbook ギャラリーから取得しのインストールおよび実稼働環境で実行する十分な注意を使用している runbook の内容を検証する必要があります |。
...
5. Runbook が表示されますが、 **Runbook** 、オートメーション アカウント] タブをクリックします。
...
6. 必要に応じて、runbook の名前を変更し、クリックして **OK** 、runbook をインポートします。
5. Runbook が表示されますが、 **Runbook** 、オートメーション アカウント] タブをクリックします。
...
- PowerShell または PowerShell ワークフローのいずれかのコード スニペットを挿入を使用して、 **コード セクションを挿入する** アイコン。 
...
1. Azure プレビュー ポータルで、Automation アカウントを開きます。 
2. をクリックして、 **資産** タイルを開くには、アセットの一覧です。
3. をクリックして、 **モジュール** タイルを開くにはモジュールの一覧です。
...
6. Azure プレビュー ポータルに返される、 **の展開のカスタム** ウィンドウです。  内のモジュールをインストールするかどうかを指定、 **新規または既存の Automation アカウント** と **Automation アカウント名**します。   **オートメーション アカウントの場所** 既存のアカウントを使用する場合は無視されます。 
7. 選択 **リソース グループ** 既存のリソース グループを指定するか、新しいモジュールを作成します。
6. 選択する必要があります **法律条項** ] をクリック **購入**します。  このボタンの名前はこうなっていますが、実際にはモジュールをインストールしても課金されません。
7. クリックして **作成** 、モジュールをインポートします。  各アクティビティを抽出する必要があるため、数分かかる場合があります。  
