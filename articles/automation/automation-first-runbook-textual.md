...
3. Runbook の名前を付けて *MyFirstRunbook ワークフロー*します。
...
![テスト ウィンドウ](media/automation-first-runbook-textual/runbook-edit-toolbar-test-pane.png)
...
3. ウィンドウを表示する権限をスクロールして戻り **MyFirstRunbook ワークフロー**します。  
...
![Runbook を開始します。](media/automation-first-runbook-textual/runbook-toolbar-start.png)
...
7.  Runbook のステータス表示では 1 回 *完了*, 、クリックして **出力**します。  [出力] ペインを開くし、表示、 *Hello World*します。<br>
...
Runbook をテストして発行しましたが、これまでのところ役に立つことは何もしていません。  ここでは、Runbook で Azure リソースを管理します。  認証で示されている資格情報を使用している場合を除きも行うことはできません、 [の前提条件](#prerequisites)します。  そうと、 **Add-azureaccount** コマンドレットです。

1.  クリックして、テキスト エディターを開きます **編集** ワークフロー MyFirstRunbook ウィンドウにします。<br>
![Runbook を編集します。](media/automation-first-runbook-textual/runbook-toolbar-edit.png) 
2.  必要ない、 **Write-output** もはや行、ので進んでおよびそれを削除します。
...
9. Runbook を保存し、 **テスト ペイン** テストできるようにします。
...
![Runbook を開始します。](media/automation-first-runbook-textual/start-runbook-input-params.png) 
