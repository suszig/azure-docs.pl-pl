...
>[AZURE.NOTE] オペレーション インサイトを Operations Management Suite に統合されている処理を行っていますされ、ドキュメントと、ポータルで使用名かを確認して可能性があります。
...
Operations Management Suite ワークスペースがあるない場合を使用して作成」の手順に  [オペレーション インサイト ワークスペースを設定する](../operational-insights/operational-insights-onboard-in-minutes.md)です。 既存のワークスペースがある場合は、それを使用できます。
...
エージェントは、Operations Management Suite に正常に接続してに表示されます、 **接続されているソース** Operations Management Suite のタブ **設定** ウィンドウです。  エージェントが正しくダウンロードしたこと、Automation ソリューションという名前のフォルダーがあることを確認できます **AzureAutomationFiles** C:\Program files \microsoft Monitoring Agent\Agent にします。 

### 4.Runbook 環境をインストールして、Azure Automation に接続する
Operations Management Suite にエージェントを追加すると、Automation ソリューションはプッシュ ダウン、 **HybridRegistration** PowerShell モジュールを含む、 **Add-hybridrunbookworker** コマンドレットです。  コンピューターに Runbook 環境をインストールして、Azure Automation に登録する場合は、このコマンドレットを使用します。
...
- **名前** Hybrid Runbook Worker グループの名前を指定します。 Automation アカウントにこのグループが既に存在する場合は、現在のコンピューターがそれに追加されます。  まだ存在しない場合は、追加されます。
- **エンドポイント** は、 **URL** フィールドに、 **キーの管理** ブレードです。
- **トークン** は、 **プライマリ アクセス キー** で、 **キーの管理** ブレードです。  

使用して、 **-詳細** スイッチと **Add-hybridrunbookworker** のインストールに関する詳細な情報を受信します。
...
使用した内部設置型マシンから実行して、ハイブリッド Runbook Worker を削除することができます **削除 HybridRunbookWorker** そのマシン上のコマンドレットです。  使用して、 **-詳細** 削除処理の詳細なログのスイッチします。  

## Hybrid Runbook Worker での Runbook の開始

[Azure Automation で Runbook を開始](automation-starting-a-runbook.md) runbook を開始するためのさまざまな方法について説明します。  ハイブリッド Runbook Worker の追加、 **RunOn** オプションは、ハイブリッド Runbook Worker グループの名前を指定することができます。  グループが指定されている場合は、Runbook が取得され、そのグループ内のワーカーによって実行されます。  このオプションが指定されていない場合は、Azure Automation で通常どおり実行されます。

Azure プレビュー ポータルで runbook を開始するときに表示されます、 **上で実行** オプションを選択できる **Azure** または **Hybrid Worker**します。  選択した場合 **Hybrid Worker**, 、ドロップダウン リストからグループを選択することができます。
...
[Service Management Automation (SMA)](https://technet.microsoft.com/library/dn469260.aspx) を使用するローカル データ センターで Azure Automation でサポートされているのと同じ runbook を実行すると、Windows Azure パック (WAP) のコンポーネントです。  Azure Automation とは異なり、SMA には Runbook および SMA の構成を保持するためのデータベースと、Windows Azure パックの管理ポータルを含むローカル インストールが必要です。 Azure Automation ではクラウドでこれらのサービスが提供されるため、必要なのはローカル環境での Hybrid Runbook Worker の維持のみです。

既存の SMA ユーザーの場合を変更しなくても Hybrid Runbook Worker と併用する」の説明とリソースに独自の認証を実行することと仮定すると、Azure Automation に runbook を移動できます [Hybrid Runbook Worker の runbook を作成する](#creating-runbooks-for-hybrid-runbook-worker)です。  SMA の Runbook は、Runbook の認証を提供できるワーカー サーバーのサービス アカウントのコンテキストで実行されます。
