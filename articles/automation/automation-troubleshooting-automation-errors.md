...
**Przyczyną tego błędu:** ten błąd występuje, jeśli nazwa zasobu poświadczeń jest nieprawidłowa lub nie są prawidłową nazwę użytkownika i hasło używane do instalacji zasób poświadczenia usługi Automatyzacja.
...
1. Limit pamięci.  Udokumentowanych limity ilość pamięci przydzielona do piaskownicy [ograniczenia usługi automatyzacja](../azure-subscription-service-limits.md#automation-limits) tak zadania może się nie powieść go, gdy używa ponad 400 MB pamięci. 
...
**Porady dotyczące rozwiązywania problemów:** żadnego z następujących rozwiązań rozwiąże ten problem:  
...
* Należy zaktualizować Azure moduły, wykonując kroki [aktualizacji programu Azure PowerShell modułów w automatyzacji Azure](automation-update-azure-modules.md).  
...
Żadnego z następujących trzech rozwiązań będzie rozwiązanie tego problemu:
...
* Upewnij się, że moduł postępuje następujący format:  
...
**Przyczyną tego błędu:** podczas następujące wyrażenie **węzła** — słowo kluczowe w konfiguracji DSC jest $null, a następnie są tworzone nie konfiguracje węzłów.    
...
**Porady dotyczące rozwiązywania problemów:** postępuj zgodnie z instrukcjami [DSC znane problemy i ograniczenia](https://msdn.microsoft.com/powershell/wmf/5.0/limitation_dsc) Aby rozwiązać problem.
...
**Błąd:** zadania kompilacji DSC zostało wstrzymane z powodu błędu: "System.InvalidOperationException błąd przetwarzania właściwości poświadczenia i typ ``<some resource name>``: konwertowanie i przechowywanie zaszyfrowane hasło jako zwykły tekst jest dozwolone tylko wtedy, gdy PSDscAllowPlainTextPassword jest ustawiona na wartość true".
...
* Upewnij się, że przekazywanie właściwego **ConfigurationData** można ustawić **PSDscAllowPlainTextPassword** na wartość true dla każdej konfiguracji węzła wymienionych w konfiguracji. Aby uzyskać więcej informacji, zapoznaj się [zasoby w konfiguracji DSC automatyzacji Azure](automation-dsc-compile.md#assets).

## <a name="next-steps"></a>Następne kroki
Jeśli zostały wykonane powyższe kroki rozwiązywania problemów i nie można znaleźć odpowiedź na pytanie, możesz przejrzeć dodatkowe opcje pomocy technicznej poniżej.
