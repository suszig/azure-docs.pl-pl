...
There are two types of runbook in Azure Automation, *textual* and *graphical*.  Runbook を作成するときに Runbook の種類を定義し、作成した後で種類を変えることはできません。
...
PowerShell スクリプトを PowerShell ワークフローに変換するための第一歩で囲むことです、 **ワークフロー** キーワードです。  ワークフローが始まり、 **ワークフロー** キーワードの後に中かっこで囲まれたスクリプトの本文。 ワークフローの名前が続く、 **ワークフロー** キーワードを次の構文に示すようにします。 
...
ワークフローにパラメーターを追加するには、使用、 **Param** キーワードは、スクリプトと同様です。 
 ...
 **InlineScript** アクティビティは、PowerShell ワークフローではなく、従来の PowerShell スクリプトの 1 つまたは複数のコマンドを実行する必要がある場合に便利です。  While commands in a workflow are sent to Windows Workflow Foundation for processing, commands in an InlineScript block are processed by Windows PowerShell. 
...
InlineScript ブロックには、値を渡すことができますが、使用する必要があります **$Using** スコープ修飾子です。  次の例は前の例と同じですが、サービス名を変数によって渡しています。 
...
使用することができます、 **並列** キーワードを同時に実行される複数のコマンドで、スクリプト ブロックを作成します。 This uses the syntax shown below. この場合、Activity1 と Activity2 は同時に開始されます。 Activity3 は、Activity1 と Activity2 の両方が完了した後にのみ開始されます。
...
使用することができます、 **ForEach-Parallel** コンストラクトを同時に、コレクション内の各項目のコマンドを処理します。 The items in the collection are processed in parallel while the commands in the script block run sequentially. This uses the syntax shown below. この場合、Activity1 は、コレクション内のすべての項目に対して同時に開始されます。 For each item, Activity2 will start after Activity1 is complete. Activity3 は、すべての項目における Activity1 と Activity2 の両方が完了した後にのみ開始されます。
...
> [AZURE.NOTE]  これがわかっている信頼性の低い結果になりますので、子 runbook を並列に実行はお勧めしません。  子 Runbook からの出力が表示されないことがあり、1 つの子 Runbook での設定が並列に実行されている他の子 Runbook に影響を与える可能性があります。 


## チェックポイント

A *チェックポイント* 変数の現在の値と、そのポイントに生成された出力を含むワークフローの現在の状態のスナップショットを示します。 ワークフローがエラーで終了、または [中断](suspending-a-workflow), 、次回の実行、ワークフローの先頭ではなく、最後のチェックポイントから開始されます。  使用してワークフローにチェックポイントを設定して、 **Checkpoint-workflow** アクティビティ。
