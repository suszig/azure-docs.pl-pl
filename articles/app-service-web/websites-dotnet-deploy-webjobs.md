...
Visual Studio を使用して、コンソール アプリケーション プロジェクトでの web アプリをデプロイする方法に関するトピック [App Service](http://go.microsoft.com/fwlink/?LinkId=529714) として、 [Azure web ジョブの](http://go.microsoft.com/fwlink/?LinkId=390226)します。 使用して web ジョブを展開する方法については、 [Azure ポータル](https://portal.azure.com), を参照してください [web ジョブでバック グラウンドの実行タスク](web-sites-create-web-jobs.md)します。
...
## <a id="convert"></a>既存のコンソール アプリケーション プロジェクトの web ジョブのデプロイメントを有効にします。

2 つのオプションがあります。

* [Web プロジェクトを使用した自動デプロイメントを有効にする](#convertlink)します。
...
* [Web プロジェクトなしでデプロイメントを有効にする](#convertnolink)します。
...
### <a id="convertlink"></a> Web プロジェクトと共に、web ジョブの自動デプロイメントを有効にします。
...
### <a id="convertnolink"></a> Web プロジェクトなしで web ジョブ デプロイメントを有効にします。
...
## <a id="create"></a>新しい web ジョブ対応のプロジェクトを作成します。

新しい web ジョブ対応のプロジェクトを作成するコンソール アプリケーション プロジェクト テンプレートを使用して、説明したように web ジョブのデプロイメントを有効にする [前のセクション](#convert)します。 または、次のように Web ジョブの新しいプロジェクト テンプレートを使用できます。

* [独立した web ジョブの web ジョブの新しいプロジェクト テンプレートを使用します。](#createnolink)
...
* [Web プロジェクトにリンクされた web ジョブの web ジョブの新しいプロジェクト テンプレートを使用してください。](#createlink)
...
### <a id="createnolink"></a> 独立した web ジョブの web ジョブの新しいプロジェクト テンプレートを使用します。
...
### <a id="createlink"></a> Web プロジェクトにリンクされた web ジョブの web ジョブの新しいプロジェクト テンプレートを使用してください。
...
このダイアログのフィールドは、Azure ポータルの **[新しいジョブ]** ダイアログのフィールドに対応しています。 詳細については、次を参照してください。 [web ジョブでバック グラウンドの実行タスク](web-sites-create-web-jobs.md)します。
...
>[AZURE.NOTE]
> 
>* のコマンド ライン デプロイメントについては、次を参照してください。 [Enabling Command-line or または継続的な配信の Azure web ジョブ](/blog/2014/08/18/enabling-command-line-or-continuous-delivery-of-azure-webjobs/)します。
>* を構成する場合、 **定期的なジョブ** し、繰り返しの頻度の設定に分数、Azure Scheduler サービスは無料です。 その他の頻度 (時間、日など) では無料です。
>* Web ジョブをデプロイし、web ジョブと再展開の種類を変更するかを決定する場合は、web ジョブ発行 settings.json ファイルを削除する必要があります。 これにより発行オプションが再度表示され、Web ジョブの種類を変更できます。
...
## <a id="publishsettings"></a>web ジョブ発行 settings.json
...
## <a id="deploy"></a>Web ジョブ プロジェクトを配置します。
