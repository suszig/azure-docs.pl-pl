...
一般的なバグの修正と機能強化。 更新プログラムと機能については、最新のリリース ノートを参照してください [Service Bus NuGet](http://www.nuget.org/packages/WindowsAzure.ServiceBus/)します。
...
- 古い **Storm** このバージョンのツールを使用する場合は、プロジェクトをアップグレードする必要があります。 詳細については、次を参照してください。 [このブログ](http://go.microsoft.com/fwlink/?LinkId=619108)します。
...
- Azure App Service のサポートの追加に [クラウド エクスプ ローラー](azure-sdk-dot-net-release-notes-2_7.md#cloud_explorer)
...
概要 **Azure Data Factory ツール** for Visual Studio です。 次に、有効な機能を示します。 参照してください [このブログ](http://go.microsoft.com/fwlink/?LinkId=617530) の詳細。

- **テンプレートに基づく作成**: テンプレート、データ移動のテンプレートまたはエンド ツー エンドのデータ統合ソリューションを配置し、Data Factory を使ってすばやく作業を開始するデータ処理のテンプレート ベースの使用事例を選択します。 
- **ソリューション エクスプ ローラーの作成と配置の Data Factory のエンティティとの統合**: 作成すると、パイプラインと Visual Studio プロジェクトと関連エンティティを展開します。 
- **作成中に視覚的相互作用のダイアグラム ビューとの統合**: パイプラインとデータセットをダイアグラム表示から補助を視覚的に作成します。 
- **サーバー エクスプ ローラーで参照し、既にデプロイされているエンティティとの対話との統合**: 既に展開されている参照データ ファクトリと対応するエンティティをサーバー エクスプ ローラーを活用します。 デプロイ済みの Data Factory や任意のエンティティ (パイプライン、リンクされたサービス、データセット) をプロジェクトにインポートします。 
- **スキーマの検証と豊富な intellisense による JSON 編集**: 効率的に構成し、機能豊富な intellisense およびスキーマ検証の Data Factory エンティティの JSON ドキュメントの編集 
- **マルチ環境の公開**: 発行環境ごとに個別の構成ファイルを作成することで開発、テスト、運用環境にパイプラインを作成します。
- **Pig、Hive、.Net ベースのデータ処理のサポート**: Pig スクリプトとプロジェクトの Data Factory で Hive スクリプトのサポート。 .Net アクティビティを管理するための C# プロジェクトの参照をサポートします。
