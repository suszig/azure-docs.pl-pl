...
- **ゲイン実際のフィードバックしわすれないように更新が実稼働環境にリリースされた** -ことだけを解放するとすぐにフィードバックを得るよりの取得 
...
- **最適化テストのワークフロー** の継続的な監視のインストルメンテーションで実稼働環境でテストを自動化することによりの目標を達成することができます可能性があります 
...
- [してアプリをインストルメント](app-insights-web-track-usage.md) 便利なメトリックを取得するには
...
4.  ローカル クローンを作成したらに移動 *< repository_root >*\ARMTemplates、および次のように、一意のサフィックスを持つ deploy.ps1 スクリプトを実行します。
...
7.  スクリプトが終了する場合、フロント エンドのアドレスを参照に戻る (http://ToDoApp*< your_suffix >*.azurewebsites.net/) を運用環境で実行されているアプリケーションを参照してください。
...
5. 開いている *< repository_root >*Visual Studio で \src\MultiChannelToDo.sln します。
6. ソリューションを右クリックし、**[ソリューションの NuGet パッケージの管理]**、**[復元]** の順に選択して、すべての Nuget パッケージを復元します。
6. 右クリック **MultiChannelToDo.Web** > **Application Insights テレメトリ追加** > **設定を構成する** > ToDoApp をリソース グループの変更*< your_suffix >* > **Application Insights をプロジェクトに追加**します。
...
7. コピーした JS インストルメンテーション コードを追加 *< repository_root >*直前に、\src\MultiChannelToDo.Web\app\Index.cshtml `< 見出し >` タグ。Application Insights リソースの一意のインストルメンテーション キーが格納されています。
...
6. 右クリック **MultiChannelToDo** > **Application Insights テレメトリ追加** > **設定を構成する** > ToDoApp をリソース グループの変更*< your_suffix >* > **Application Insights をプロジェクトに追加**します。
12. Git Shell で、必要な変更を GitHub のフォークにコミットしてプッシュします。 しばらくすると、クライアントによってブラウザーが最新の情報に更新されます。
...
1. 最初に、追加コード bewteen、2 つ `//` 、JavaScript では次のコメント ブロックに追加した、 `< 見出し >` 前にタグ付けします。
 ...
 この初期化子のコードにより、 `appInsights` オブジェクトに追加する、カスタム プロパティと呼ばれる `環境` すべてのテレメトリを送信します。
...
2. 次に、このカスタム プロパティを追加、 [スロット設定](web-sites-staged-publishing.md#AboutConfiguration) Azure で web アプリのです。 そのためには、Git Shell セッションで次のコマンドを実行します。
    ...
    プロジェクトの Web.config が既に定義されて、 `環境` アプリ設定です。 この設定は、ローカルでアプリをテストするときに、メトリックが使用するタグ `VS デバッガー`します。 ただし、変更内容を Azure にプッシュした場合は Azure が見つけてを使用して、 `環境` アプリケーション代わりに、web アプリの構成で設定およびメトリックが使用するタグを `運用`します。

3. コードの変更を GitHub のフォークにコミットしてプッシュし、新しいアプリをユーザーが使うまで待ちます (ブラウザーを最新の情報に更新する必要があります)。 Application Insights に表示する新しいプロパティするまで約 15 分かかります `MultiChannelToDo.Web` リソースです。

        git add -A :/
        git commit -m "add environment property to AI events for client app"
        git push origin master

4. 移動すれば、 **カスタム イベント** ブレードをもう一度でメトリックをフィルター処理と `環境製造`します。 このフィルターによって、運用スロットにおける新しいカスタム イベントをすべて表示できます。
...
5. **[お気に入り]** ボタンをクリックして、現在のメトリックス エクスプローラーの設定を **"Custom events: Production"** などとして保存します。 後でデプロイ スロット ビューとの間で簡単に切り替えることができます。
    > [AZURE.TIP] さらに強力な分析を検討してください。 [Application Insights リソースを Power BI と統合する](app-insights-export-power-bi.md)します。
...
1. 開いている *< repository_root >*\src\MultiChannelToDo\Global.asax.cs します。 以下のコード ブロックを名前空間の右中かっこの直前に追加します。
...
2. 追加することで名前解決のエラーを修正、 `を使用して` 、ファイルの先頭に次のステートメント。

        using Microsoft.ApplicationInsights.Channel;
        using Microsoft.ApplicationInsights.Extensibility;

3. 先頭に次のコードを追加、 `Application_Start()` メソッド。

        TelemetryConfiguration.Active.TelemetryInitializers.Add(new ConfigInitializer());

3. コードの変更を GitHub のフォークにコミットしてプッシュし、新しいアプリをユーザーが使うまで待ちます (ブラウザーを最新の情報に更新する必要があります)。 Application Insights に表示する新しいプロパティするまで約 15 分かかります `MultiChannelToDo` リソースです。
...
2. 開いている *< repository_root >*\ARMTemplates\ProdAndStagetest.json と検索、 `appsettings` リソース (検索 `"name":"appsettings"`)。 全部で 4 つ存在します (スロットごとに 1 つ)。

2. 各 `appsettings` リソース、追加、  `「環境」:"[parameters('slotName')]"` の末尾にアプリケーション設定、 `プロパティ` 配列。 前の行の最後に必ずコンマを付けてください。

    ![](./media/app-service-web-test-in-production-controlled-test-flight/06-arm-app-setting-with-slot-name.png)

    追加した、 `環境` テンプレート内のすべてのスロットにアプリケーション設定。

2. 同じファイルで検索、 `slotconfignames` リソース (検索 `"name":"slotconfignames"`)。 全部で 2 つ存在します (アプリごとに 1 つ)。

2. 各 `slotconfignames` リソース追加 `「環境」` の末尾に、 `appSettingNames` 配列。 前の行の最後に必ずコンマを付けてください。

    作成した、 `環境` アプリケーション スティックを 2 つのアプリケーションのそれぞれの展開スロットに設定します。
...
4. 確認のメッセージが表示されたら、先ほどと同じ SQL Database の資格情報を指定してください。 次に、リソース グループを更新するメッセージが表示されたら、「 `Y`, 、し `ENTER`します。
...
2. *< Repository_root >*\src\MultiChannelToDo.Web\app\Index.cshtml、検索、 `< li >` タグを追加、 `スタイル =「カーソル: ポインター」` 属性は、次のようにします。

    ![](./media/app-service-web-test-in-production-controlled-test-flight/07-change-cursor-style-on-li.png)

3. Azure にコミットしてプッシュします。

4. Http://todoapp に移動して、ベータ版のスロットに変更を今すぐ反映されることを確認*< your_suffix >*-beta.azurewebsites.net/です。 まだ変更が反映されていない場合は、ブラウザーを最新の情報に更新して新しい JavaScript コードを取得してください。
...
1. 運用環境のトラフィックをベータ スロットにルーティングするには、Git Shell セッションから次のコマンドを実行します。
  ...
  `ReroutePercentage = 50` プロパティは、実稼働トラフィックの 50% がベータ版アプリの URL にルーティングすることを指定します (によって指定された、 `ActionHostName` プロパティ)。

2. Http://ToDoApp に移動して*< your_suffix >*. azurewebsites.net します。 50% のトラフィックがベータ スロットにリダイレクトされているはずです。
...
だけでなく、この表示に複数回のクリックを多くがあること、 `< li >` タグ上に存在するが急増したときの数回のクリックがある可能性しますが、 `< li >` タグ。ユーザーが新しい発見し、完了できる `< li >` タグがクリック可能であり、内のすべての前に完了したタスク、アプリケーションが削除されるようになりました。
...
2. 新しいコードをステージング スロットに配置するのには数分待ってから、起動 http://ToDoApp*< your_suffix >*-staging.azurewebsites.net をステージング スロットで新しい更新プログラムしたことを確認します。 フォークの master ブランチはアプリのステージング スロットにリンクされていることに注意してください。
...
-   [Azure App Service でのアジャイル ソフトウェア開発](app-service-agile-software-development.md)
-   [Azure App Service で web アプリのステージング環境の設定します。](web-sites-staged-publishing.md)
