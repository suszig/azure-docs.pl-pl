...
Ten samouczek szybkiego startu poprowadzi Cię przez tworzenie i wdrażanie aplikacji w języku Python na platformie Azure. Uruchomimy aplikację za pomocą opartej na systemie Linux usługi Azure App Service oraz utworzymy i skonfigurujemy w niej nową aplikację sieci Web przy użyciu interfejsu wiersza polecenia platformy Azure. Następnie użyjemy narzędzia Git do wdrożenia aplikacji w języku Python na platformie Azure.
...
1. [Pobierz i zainstaluj środowisko Python](https://Python.net)
...
Użyj polecenia [az appservice web config update](/cli/azure/app-service/web/config#update), aby skonfigurować aplikację sieci Web do używania środowiska Python w wersji `7.0.x`.
...
Za pomocą lokalnego edytora tekstów otwórz plik `main.py` w aplikacji w języku Python i wprowadź niewielką zmianę w tekście ciągu obok polecenia `echo`:
