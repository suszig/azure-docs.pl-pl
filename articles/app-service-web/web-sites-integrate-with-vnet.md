...
このドキュメントは Azure App Service の仮想ネットワークの統合機能について説明し、アプリ用にセットアップする方法を示しています [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714)します。  Azure 仮想ネットワーク (VNET) とは、Azure リソースの多くをインターネット以外のルーティング可能なネットワークに配置できる機能です。配置先のネットワークへのアクセスは制御できます。  これらのネットワークは、さまざまな VPN テクノロジを使用して、オンプレミスのネットワークに接続できます。  ここにある情報で始まる Azure 仮想ネットワークの詳細について学習する: [Azure 仮想ネットワークの概要] [VNETOverview] です。  
...
- VNET 統合でのアプリでのみ動作する **標準** または **Premium** 料金プランです。  この機能を有効にした後で、App Service プランをサポートされていない価格プランに変更すると、使用している VNET にアプリから接続できなくなります。  
...
>[AZURE.NOTE] 新しい仮想ネットワーク統合を構成するには数分かかることができます。  
...
>[AZURE.NOTE] 運用ゲートウェイと完全に提供する新しい VNET には、最大で 30 分かかることができます。  完了すると、UI が更新されます。  
...
VNET への接続と、VNET との接続の切断は、アプリ レベルの機能です。  複数のアプリをまたいで VNET 統合に影響する可能性がある操作は、ASP レベルで行われます。  アプリ レベルで表示される UI から、VNET の詳細を取得できます。  それと同じ情報のほとんどは、ASP レベルでも確認できます。  

![][5]

[ネットワーク機能の状態] ページで、アプリが VNET に接続されているかどうかを確認できます。  何らかの理由で VNET ゲートウェイがダウンしている場合は、[接続されていません] と表示されます。  

アプリケーション レベルの VNET 統合の UI は、ASP から取得する詳細情報と同じであるようになりました情報です。  それらの項目を次に示します。
...
**ルーティング** 
前に説明した、VNET で定義されているルートは、アプリケーションから、VNET にトラフィックの送信で使用されます。  ただし、お客様がさらに別の送信トラフィックをアプリから VNET に送信するケースもあるため、この機能が用意されています。  この操作を行った後のトラフィックの動きは、お客様による VNET の構成方法によって変わります。  

**証明書**
証明書の状態には、VPN 接続を使用していることを証明書が現在も有効なことを検証する App Service によって実行されるチェックが反映されます。  VNET 統合が有効になっている状態で、この ASP で任意のアプリからその VNET と初めて統合する場合は、接続のセキュリティを確保するために、証明書の交換が必要となります。  証明書と共に、DNS の構成やルートなど、ネットワークに関するその他の同様の情報を取得できます。
これらの証明書またはネットワーク情報が変更された場合は、[ネットワークの同期] をクリックする必要があります。  **注**: アプリと、VNET 間の接続で短時間停止が発生するが、「同期ネットワーク」をクリックするとします。  アプリの再起動が行われない場合は、接続が途切れることで、サイトが正常に機能しなくなる可能性があります。  
...
>[AZURE.NOTE] オンプレミスのリソースにアクセスするサイト間 VPN で VNET 統合機能は使用中に、現在は扱っていません同じように、ExpressRoute VPN です。  
...
アプリでこの機能を使用できるようにするには、アプリを Standard または Premium の App Service プランの対象にする必要があります。  これらのコストで詳細を確認できます。 [App Service の料金] [ASPricing] です。 

ポイント対サイト VPN が処理されるしくみにより、VNET 統合接続を経由する送信データには必ず料金が発生します。これは、VNET が同じデータ センターにある場合でも当てはまります。  どのこれらの費用は見てここ: [データ転送料金の詳細] [DataPricing] です。  

最後の項目は、VNET ゲートウェイのコストです。  サイト間 VPN など、他の用途でゲートウェイを使う必要がない場合、VNET 統合機能をサポートするためにゲートウェイの料金が発生します。  これらのコストの詳細は、: [VPN Gateway の料金] [VNETPricing] です。  
...
次のツールでは、ホストとポートを組み合わせたものへの TCP 接続をテストすることができます。  このツールし、呼ばれ tcpping.exe 構文です。
...
ゲートウェイがダウンしている場合は、再起動してください。  証明書が同期されていない場合は、VNET 統合の ASP ビューに移動し、[ネットワークの同期] をクリックします。  されていると、vnet 間の構成に加えられた変更と思われ、同期されていない場合、通常短時間停止間の接続とアプリケーションにこれにより、Asp VNET 統合の [ASP] ビューに移動し、"同期 Network"だけを再確認する際にヒットします。  
...
今すぐ、VNET がホストされている場合は、VM はオンプレミス システムにアクセスできるが、アプリは、理由は、次のいずれかの可能性が高いことはできません。。
- ルートがサイトでのオンプレミス ゲートウェイの IP 範囲、ポイントが構成されていません。
- ポイント サイトの IP 範囲へのアクセスをブロックしているネットワーク セキュリティ グループ
- 内部設置型のファイアウォールは、ポイントからサイトの IP の範囲へのトラフィックをブロックしています。
- ユーザー定義の Route(UDR) ベースのサイト トラフィック、ポイントをにより、内部設置型ネットワーク上に到達できない、VNET 内にある場合
