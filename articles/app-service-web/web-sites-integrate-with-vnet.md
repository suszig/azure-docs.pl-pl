...
アプリでこの機能を使用できるようにするには、アプリを Standard または Premium の App Service プランの対象にする必要があります。  これらのコストで詳細を確認できます。 [App Service の料金][ASPricing]します。 

ポイント対サイト VPN が処理されるしくみにより、VNET 統合接続を経由する送信データには必ず料金が発生します。これは、VNET が同じデータ センターにある場合でも当てはまります。  どのこれらの費用は見てここ:[データ転送の料金詳細][DataPricing]します。  

最後の項目は、VNET ゲートウェイのコストです。  サイト間 VPN など、他の用途でゲートウェイを使う必要がない場合、VNET 統合機能をサポートするためにゲートウェイの料金が発生します。  これらのコストの詳細は、: [VPN Gateway の料金][VNETPricing]します。  
