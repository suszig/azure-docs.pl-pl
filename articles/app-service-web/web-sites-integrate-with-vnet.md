...
- 10.0.0.0/8 - これは 10.0.0.0 と同じ - 10.255.255.255
- 172.16.0.0/12 - これは「172.16.0.0」と同じ - 172.31.255.255
- 192.168.0.0/16 - これは 192.168.0.0 と同じ - 192.168.255.255
...
ポイント対サイト VPN が処理されるしくみにより、VNET 統合接続を経由する送信データには必ず料金が発生します。これは、VNET が同じデータ センターにある場合でも当てはまります。 どのこれらの費用は見てここ:[データ転送料金の詳細 ][datapricing]します。
