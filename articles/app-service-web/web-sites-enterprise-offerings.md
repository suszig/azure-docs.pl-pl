...
別のアプローチとしては、オンプレミスへの既存の投資を活用する方法があります。 シナリオ例の人件費システムでは、データ ストアを社内のインフラストラクチャ内部に保持したいと思うかもしれません。 内部システム (報告、給与、課金など) との統合が考えられます または、IT ガバナンスの要件を満たすためにします。 Web Apps では、オンプレミスのインフラストラクチャへの接続を可能にするために多くの方法が用意されています。
...
-   認証 - Web Apps では、匿名認証が既定でサポートされています。また、アプリケーションで識別されるフォーム認証がサポートされています。 Web Apps が Azure Active Directory および ADFS のみで統合されている場合は、Windows 認証を利用できます。 これは、詳細で説明されている機能 [ここ](http://aka.ms/azurebizapp)
