...
このチュートリアルは、Azure データ管理で提供される Table サービスを使用して格納し、[ノード] のアプリケーションでホストされているデータにアクセスする方法を示します [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714) Web アプリです。 このチュートリアルでは、ノードおよび [Git] を使用したいくつかの経験があることを前提としています。
...
2. ポータルの左下にある **[新規]** アイコンをクリックし、**[データ + ストレージ]**、**[ストレージ]** の順にクリックします。 ストレージ アカウントに一意の名前を付けるされ、新しい作成 [リソース グループ](../resource-group-overview.md) にします。
...
ここでは、新しい Node アプリケーションを作成し、npm を使用してモジュール パッケージを追加します。 このアプリケーションは、[Express] を使用して、[Azure] モジュール。 Express モジュールは node の Model View Controller フレームワークを提供し、Azure モジュールは Table サービスへの接続を提供します。
...
次に、[azure] をインストールするには、次のコマンドを入力して [node-uuid]、[nconf] および [async] モジュール。
...
**PartitionKey** と **RowKey** は、Table サービスでテーブル キーとして使用されます。 詳細については、次を参照してください。 [テーブル サービス データ モデルについて](https://msdn.microsoft.com/library/azure/dd179338.aspx)します。
...
**views** ディレクトリの **layout.jade** ファイルは、他の **.jade** ファイルのグローバル テンプレートです。 この手順で使用するように変更が [Twitter Bootstrap](https://github.com/twbs/bootstrap), 、見栄えのよい、web アプリをデザインしやすくツールキットです。

ファイルをダウンロードして展開 [Twitter Bootstrap](http://getbootstrap.com/)します。 Bootstrap の **css** フォルダーから **bootstrap.min.css** ファイルをアプリケーションの **public/stylesheets** ディレクトリにコピーします。
...
> [AZURE.NOTE] 次の手順を使用して実行することもできます。、 [Azure ポータル](https://portal.azure.com)します。 参照してください [の構築し、Azure App Service での Node.js web アプリのデプロイ] です。
>
> これが初めて作成した Web アプリの場合、Azure ポータルを使用してこのアプリケーションをデプロイする必要があります。

開始するには、コマンドラインから次のコマンドを入力して [Azure CLI] をインストールします。
...
2. 次のコマンドを使用して、新しい Web アプリを作成します。
    ...
    `- Git` パラメータは、この web アプリの Azure で Git リポジトリを作成します。 また、[なし] が存在し、[Git リモート] azure という名前の' ' を Azure にアプリケーションを発行するために使用を追加する場合にも、現在のディレクトリに Git リポジトリを初期化します。 最後に、**web.config** ファイルが作成されます。このファイルには、ノード アプリケーションをホストするために Azure で使用される設定が含まれます。 省略した場合、 `- git` パラメーターでも、ディレクトリに Git リポジトリが含まれています、コマンドは、リモートが作成されます"azure"です。
    ...
    > [AZURE.NOTE] これがサブスクリプションの最初の App Service Web アプリである場合、Azure ポータルを使用して Web アプリを作成するよう指示するメッセージが表示されます。 詳細については、次を参照してください。 [作成と Azure App Service で Node.js web アプリのデプロイ] です。
...
置換 * *<storage account name>** 記憶領域の名前を前に作成したアカウントし、置換 **<storage access key>* * ストレージ アカウントのプライマリ アクセス キーを持つ。(以前に作成した config.json ファイルと同じ値を使用します)。
...
4. プッシュ操作が完了したら、先ほどから返された web アプリの URL を参照、 `azure サイトを作成する` コマンドしてアプリケーションを表示します。


## 次のステップ

この記事の手順では、テーブル サービスを使用して情報を格納する方法を説明しましたが、MongoDB を使用することもできます。 詳細については、[MongoDB を使用する Node.js web アプリ] を参照してください。
