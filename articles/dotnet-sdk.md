...
## <a id="included"></a>Azure sdk for .NET に含まれるもの

Azure SDK for .NET には次の製品が含まれており、インストールされます。

- [Visual Studio Express for Web](#vwd)
- [Microsoft ASP.NET および Web Tools for Visual Studio](#wte)
...
- [Microsoft Azure のモバイル アプリ SDK V1.0](#mobile)
...
### <a id="wte"></a>Microsoft ASP.NET および Web Tools for Visual Studio
...
* [プロジェクト内または web プロジェクトの発行中に新しい web の作成中に Azure web サイトの作成と SQL Database というリソース](web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database.md)します。
* [新規の web サイトの作成中に配置スクリプトを作成する PowerShell](http://msdn.microsoft.com/library/dn642480.aspx)します。
* [管理およびサーバー エクスプ ローラーで Azure web サイトのトラブルシューティングを行う](web-sites-dotnet-troubleshoot-visual-studio.md#sitemanagement)します。
...
* [の作成、オープン、およびクラウド サービス プロジェクトを発行](cloud-services-dotnet-get-started.md)します。
* [クラウドのデプロイメント パッケージをサービス プロジェクトを作成](http://msdn.microsoft.com/library/ff683672.aspx)します。
* [Web プロジェクトを新規作成するときに Azure 仮想マシンを作成](virtual-machines-dotnet-create-visual-studio-powershell.md)します。
* [新しい仮想マシンの作成中に作成する PowerShell スクリプト](http://msdn.microsoft.com/library/dn642480.aspx)します。
* [ビュー ](http://msdn.microsoft.com/library/ee405486.aspx)します。
...
* [Azure リソース グループ デプロイ プロジェクトを使用してリソースのプロビジョニングを自動化します。](https://msdn.microsoft.com/library/dn872471.aspx)
...
## <a id="notincluded"></a>.NET 用 Azure SDK をインストールするときに含まれないもの
...
* [Azure モバイル サービス プロジェクトのテンプレート](mobile-services-dotnet-backend-windows-store-dotnet-leaderboard.md)します。
...
- [Azure 機能の多くは既に Visual Studio でします。 Azure SDK for .NET インストール必要ですか?](#azinvs)
- [クライアント ライブラリをします。 私はための .NET 用 Azure SDK をインストールしますか?](#clientlib)
- [.NET 用 Azure SDK の旧バージョンはどこで入手できますか。](#olderversions)
- [Azure SDK for .NET のバージョンのライフ サイクル ポリシーとは何ですか。](#lifecycle)
- [ゲスト OS バージョンとは、Azure SDK for .NET との互換性](#guestos)
- [.NET 用 Azure SDK をアンインストールする方法](#uninstall)

### <a id="azinvs"></a>多数の Azure 機能は、Visual Studio が既にいます。Azure SDK for .NET をインストールする必要がありますか。
...
### <a id="clientlib"></a>クライアント ライブラリを表示します。Azure SDK for .NET をインストールしないと入手できませんか。
...
### <a id="olderversions"></a>.NET 用 Azure SDK の旧バージョンはどこで入手できますか。

以前のバージョンを参照してください、 [Azure SDK for .NET](/downloads/archive-net-downloads/) ダウンロード ページです。

### <a id="lifecycle"></a>Azure SDK for .NET のバージョンのライフ サイクル ポリシーとは何ですか。

参照してください [Microsoft Azure Cloud Services のサポート ライフ サイクル ポリシー](http://support.microsoft.com/gp/azure-cloud-lifecycle-faq)します。

### <a id="guestos"></a>ゲスト OS バージョンとは、Azure SDK for .NET との互換性

参照してください [Azure ゲスト OS リリースと SDK の互換性対応表](http://msdn.microsoft.com/library/ee924680.aspx)します。

### <a id="uninstall"></a>.NET 用 Azure SDK をアンインストールする方法

この記事の下に示すように各項目 [Azure sdk for .NET に含まれるもの](#included) Windows コントロール パネルの [インストールされているプログラムの一覧の個別のプログラムは、 **プログラムと機能**します。 これらを、グループとしてアンインストールする方法はなく、各プログラムは個別にアンインストールする必要があります。
