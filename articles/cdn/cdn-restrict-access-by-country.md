...
**国のフィルタリング**機能では、国コードを使用して保護されたディレクトリに対して許可またはブロックする要求の国を定義します。 国コードが見つかります [この](cdn-country-codes.md) トピックです。 "EU" (ヨーロッパ) または "AP" (アジア/太平洋) を指定した場合、その地域の国から発信される IP アドレスのサブセットがブロックまたは許可されます。
