...
> [AZURE.NOTE] Standard および Premium CDN 層が同じ圧縮機能を提供しますが、ユーザー インターフェイスが異なります。  Standard および Premium CDN 層の違いの詳細については、次を参照してください。 [Azure CDN の概要](cdn-overview.md)します。
...
5. をクリックして変更を行った後、 **保存** ] ボタンをクリックします。
...
2. ポインターを合わせる、 **HTTP 大きな** ] タブの [し、ポインターを合わせる、 **キャッシュ設定** フライアウトします。  をクリックして **圧縮**します。
    
    圧縮のオプションが表示されます。
    
    ![ファイル圧縮](./media/cdn-file-compression/cdn-compress-files.png)
    
3. クリックしてファイルの種類の一覧を変更した後、 **更新** ] ボタンをクリックします。


## 圧縮プロセスとルール

1. 依頼者は、コンテンツの要求を送信します。
2. エッジ サーバーがあるかどうかをチェック **Accept-encoding** ヘッダー。
