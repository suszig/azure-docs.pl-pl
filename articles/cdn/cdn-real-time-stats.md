<properties 
    ...
    description="Microsoft Azure CDN でのリアルタイム統計情報 リアルタイム統計情報は、クライアントにコンテンツを配信するときの CDN のパフォーマンスに関するリアルタイム データを提供します。" 
    services="cdn" 
...
> [AZURE.NOTE] リアルタイム統計は、Premium CDN 層の機能です。  Standard および Premium CDN 機能の比較では、次を参照してください。 [Azure CDN の概要](cdn-overview.md)します。
...
> [AZURE.NOTE] 上記のグラフのそれぞれは、一定の時間のリアルタイムの統計情報を表示します。 指定時間を過ぎると、データのスライディング ウィンドウが表示されます。 これは新しいデータを表示するために古いデータをグラフから外すことを意味します。 このスライディング ウィンドウの時間の長さは、[グラフの期間] オプションで設定できます。
...
2. ポインターを合わせる、 **Analytics** ] タブの [し、マウス、 **リアルタイム統計** フライアウトします。  をクリックして **HTTP 大きなプラットフォーム**します。
...
> [AZURE.NOTE] 帯域幅使用のレポートで使用されるユニットは、次のいずれか: ビット/秒 (b/秒)、キロビット/秒 (Kb/秒)、メガビット/秒 (Mb/秒)、またはギガビット/秒 (Gb/秒)。
...
> [AZURE.NOTE]  **更新グラフ** オプションは、グラフをオフにします。 その後、選択した状態コードのみが表示されます。
...
> [AZURE.NOTE] グラフのすぐ下の色分けされたオプションは、グラフに表示される内容にのみ影響します。 グラフがその状態コードを追跡記録するかどうかには影響を与えません。
...
> [AZURE.NOTE]  **更新グラフ** オプションは、グラフをオフにします。 その後、選択したキャッシュの状態のみが表示されます。
