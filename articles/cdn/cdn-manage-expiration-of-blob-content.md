...
2.  明示的に設定、 *x-ms-blob のキャッシュ制御* プロパティを **Put Blob**, 、**Put Block List**, 、または **Set Blob Properties** 要求、または Azure マネージ ライブラリを使用して、設定、 [BlobProperties.CacheControl](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.blob.blobproperties.cachecontrol.aspx) プロパティです。 このプロパティ設定の値、 *Cache-control* 、blob のヘッダー。 ヘッダーまたはプロパティの値には、秒単位で適切な値を指定する必要があります。 たとえば、キャッシュの最大期間を 1 年に設定する場合は、要求ヘッダーとして `x-ms-blob-cache-control: public, max-age=31556926` を指定できます。 キャッシュ ヘッダーの設定の詳細については、「、 [http/1.1 仕様](http://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html)します。  

CDN を使用してキャッシュするすべてのコンテンツは、公開されているアクセス可能な BLOB として、Azure ストレージ アカウントに格納する必要があります。 Azure Blob サービスの詳細については、次を参照してください。 **Blob サービスの概念**します。  

BLOB サービスでコンテンツを操作する方法はいくつかあります。  

-   によって提供されるマネージ API を使用して、 **Azure マネージ ライブラリ リファレンス**します。
...
などのツールを使用する場合は、必要に応じて、 **wget** や Fiddler 要求と応答の詳細を確認します。
