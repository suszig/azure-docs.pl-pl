...
> [AZURE.NOTE] 詳細な HTTP レポートは、Premium CDN レベルの機能です。 Standard および Premium CDN 機能の比較では、次を参照してください。 [Azure CDN の概要](cdn-overview.md)します。

## 詳細な HTTP レポートへのアクセス

1. [CDN プロファイル] ブレードで、**[管理]** をクリックします。

    ![[CDN プロファイル] ブレードの ](./media/cdn-advanced-http-reports/cdn-manage-btn.png)
...
* 市区町村または要求の起点となる地域 (都道府県など) を特定できませんだった場合、それが示されます既知ではないという。 国が不明な場合、2 つの質問のマーク (つまり、[概要] タブ) 表示されます。
...
>
>* トラフィック パターンは、ユーザー エージェントの動作が異なる場合に正確に予測できません。 このため、完了したダウンロードの結果が 100% を超える可能性があります。
>* このレポートでの HTTP プログレッシブ ダウンロードを利用する資産は正確に表現できない可能性があります。 これは、ユーザーがビデオの別の場所をシークするためです。
...
>
>* ヒットでは、ステータス コードに関係なく、アセットのすべての要求を表します。
>* エッジ CNAME Url は、それと同等の CDN Url に変換されます。 これにより、資産の要求に使用されるのが CDN URL とエッジ CNAME URL のどちらであるかに関係なく、その資産に関連付けられたすべての統計情報を正確に把握できます。
