...
> [AZURE.NOTE] エッジのパフォーマンス分析は、Premium CDN 層の機能です。  Standard および Premium CDN 機能の比較では、次を参照してください。 [Azure CDN の概要](cdn-overview.md)します。
>
...
> [AZURE.NOTE] エッジのパフォーマンス分析レポートの生成元となる、生データは、少なくとも 90 日間可能となります。
...
2. ポインターを合わせる、 **Analytics** ] タブの [し、ポインターを合わせる、 **エッジ パフォーマンス分析** フライアウトします。  をクリックして **ダッシュ ボード**します。
...
> [AZURE.NOTE] トラフィック ボリューム番号では、比率およびパーセンテージの計算に使用され、のみに大量の顧客の合計のトラフィックの一部を表示するトラフィックを表します。
...
2. ポインターを合わせる、 **Analytics** ] タブの [し、ポインターを合わせる、 **エッジ パフォーマンス分析** フライアウトします。  をクリックして **ラージ オブジェクトの HTTP**します。
...
ファイルの種類 | アカウントの CDN を通じて要求された上位 10 件のファイルの種類を表示する棒グラフが含まれています。 アセットのファイル名拡張子でこのレポートのためには、ファイルの種類が定義されているし、インターネット メディア タイプ (例: .html \[text/html\]、.htm \[text/html\]、.aspx \[text/html\] など。)。 詳細情報は、上位 100 件のファイルの種類のグラフの下に表示されます。
