...
| 名前| 型| 必須| 使用できる値| 説明|
| ---- | ---- | -------- | ---------------- | ----------- |
| type| enum| はい| リソースの: <br />**{名前空間}/{型}/プロバイダー/ロック**<br /><br />のリソース グループの:<br />**Microsoft.Authorization/locks**| 作成するリソースの種類。|
| apiVersion| enum| はい| **2015-01-01**| リソースの作成に使用する API バージョン。|
| name| string| はい| リソースの:<br />**{resouce}/Microsoft.Authorization/{lockname}**<br /><br />のリソース グループの:<br />**{lockname}**<br /><br />最大 64 文字<br />は使用できません <, > %、&、?、いずれかの制御文字またはです。| 両方のリソース ロックおよびロックの名前を指定する値します。|
...
| properties| オブジェクト| はい| (下記参照)| ロックとロックに関する注意事項の種類を識別するオブジェクト。|
...
管理ロックを作成または削除するには、**Microsoft.Authorization/*** または **Microsoft.Authorization/locks/*** アクションにアクセスできる必要があります。 組み込みのロールののみ **所有者** と **ユーザー アクセスの管理者** は 
これらの操作を許可します。 役割に基づいたアクセス制御の詳細については、次を参照してください。 [リソースへのアクセスを管理する](resource-group-rbac.md)します。
