...
| 名前 | 型 | 必須 | 使用できる値 | 説明 |
| ---- | ---- | -------- | ---------------- | ----------- |
| type | enum | あり | リソースの: <br />**{名前空間}/{型}/プロバイダー/ロック**<br /><br />のリソース グループの:<br />**Microsoft.Authorization/locks** | 作成するリソースの種類。 |
| apiVersion | enum | あり | **2015-01-01** | リソースの作成に使用する API バージョン。 |  
| name | string | あり | リソースの:<br />**{resouce}/Microsoft.Authorization/{lockname}**<br /><br />のリソース グループの:<br />**{lockname}**<br /><br />最大 64 文字<br /><>, % を含めることはできません &、?、またはいずれかの制御文字。 | 両方のリソース ロックおよびロックの名前を指定する値します。 |
...
| 名前 | 型 | 必須 | 使用できる値 | 説明 |
| ------- | ---- | ---------------- | -------- | ----------- |
| level   | enum | あり | **CannotDelete** <br /> **読み取り専用**  | スコープに適用するロックの種類。 CanNotDelete は変更はできますが削除はできません。ReadOnly は変更も削除もできません。 |
