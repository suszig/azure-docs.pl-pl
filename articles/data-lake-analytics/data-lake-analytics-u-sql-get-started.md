...
- **Data Lake 分析アカウント**します。  参照してください [Azure Data Lake (含む) Analytics アカウントを作成する](data-lake-analytics-get-started-portal.md#create_adl_analytics_account)です。
- **Data Lake 分析アカウントにサンプル データをアップロード**します。 参照してください [既定データ湖のストレージ アカウントにアップロード SearchLog.tsv](data-lake-analytics-get-started-portal.md#update-data-to-the-default-adl-storage-account)します。
...
5. 次に、 **送信** ] ボタン、Analytics アカウントを指定します。
5.  **ソリューション エクスプ ローラー**, を右クリックして **Script.usql**, 、クリックして **ビルド スクリプト**します。 [出力] ペインで結果を確認します。  スクリプトにエラーがある場合は、ここにエラー出力が示されます。 
6.  **ソリューション エクスプ ローラー**, を右クリックして **Script.usql**, 、クリックして **送信スクリプト**します。
7. 確認、 **Analytics アカウント**, 、] をクリックし、 **送信**します。 送信が完了すると、Data Lake Tools for Visual Studio の [結果] ウィンドウに送信結果とジョブのリンクが示されます。
8. クリックして、 **更新** ] ボタンをクリックして、最新のジョブの状態、画面を更新します。 ジョブが正常に完了するまで待機します。  ジョブが失敗した場合、ソース ファイルがない可能性があります。  ツールの "エラー" タブを使用して、サービスから返されるエラーを表示できます。 このチュートリアルの「前提条件」を参照してください。 追加のトラブルシューティング情報を参照してください。 [モニターし、Azure Data Lake 分析ジョブのトラブルシューティング](data-lake-analytics-monitor-and-troubleshoot-jobs-tutorial.md)します。
...
3.  ダブルクリックして **SearchLog まず u sql.csv**します。
...
このスクリプトに変換手順は含まれていません。 呼ばれるソース ファイルから読み取ります **SearchLog.tsv**, 、schematizes、およびという名前のファイルに、行セットを出力 **adltools.csv からの SearchLog**します。 

Duration フィールドのデータ型の横にある疑問符に注目してください。 これは、Duration フィールドを null にできることを意味します。

スクリプトには以下のいくつかの概念とキーワードがあります。

- **行セット変数**: 行セットを生成する各クエリ式は、変数に割り当てることができます。 U SQL は T-SQL 変数名前付けパターンに従ってなど **@searchlog** スクリプトにします。 
    この割り当ては実行を強制するものではないことに注意してください。 単に式に名前を付け、より複雑な式を構築できるようにするためのものです。
- **抽出** 読み取り時にスキーマを定義することができます。 スキーマは、列名と列ごとの C# 型名のペアで指定できます。 使用して、いわゆる **抽出**, など、 **Extractors.Tsv()** tsv ファイルを抽出します。 カスタムのエクストラクターを開発することができます。
- **出力** 行セットを取得し、シリアル化します。 Outputters.Csv() は、指定した場所にコンマ区切りファイルを出力します。 また、カスタムのアウトプッターを作成することもできます。
...
U SQL では、使い慣れた **ORDER BY**, 、**GROUP BY** および集計します。
...
すべての U-SQL スクリプトが、その既定のコンテキストとして、既定のデータベース (master) と既定のスキーマ (dbo) で実行されます。 独自のデータベースやスキーマを作成することができます。 コンテキストを変更するには、使用、 **を使用して** コンテキストを変更するステートメントです。
...
次のスクリプト作成と呼ばれる TVF *Searchlog()* 既定のデータベースおよびスキーマ内。
...
次のスクリプトというビューを作成する *SearchlogView* 既定のデータベースおよびスキーマ内。
