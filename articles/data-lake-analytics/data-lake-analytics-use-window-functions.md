...
ウィンドウ関数を使用すると呼ばれる行のセット内で計算を行う *windows*します。 Windows は、OVER 句によって定義されます。 ウィンドウ関数は、いくつかの重要なシナリオを非常に効率的な方法で解決します。
...
- [レポートの集計関数](#reporting-aggregation-functions), 、SUM や AVG など
...
- 次の 2 つのチュートリアルを完了します。
    ...
    - [Azure Data Lake 分析ジョブに対して U SQL を使い始める](data-lake-analytics-u-sql-get-started.md)します。
...
また、結果行セットをデータ ファイルに出力するには、次のステートメントも追加する必要があります。
 ...
 ほとんどのサンプルがという名前の変数を使用して **@result** 結果を得るのためです。
...
>[AZURE.NOTE] Windows の機能に慣れていない場合は、出力の数値を記憶することをお勧めします。  
...
**numeric_literal** で、百分位数を計算します。 値は 0.0 ～ 1.0 の範囲で指定してください。

WITHIN GROUP (ORDER BY <identifier> [ASC |DESC]) では、並べ替えるし、百分位数を計算する数値のリストを指定します。 許可される列識別子は 1 つだけです。 式は、数値型に評価される必要があります。 その他のデータ型は許可されていません。 既定の並べ替え順は昇順です。

([PARTITION BY < 識別子 >... 経由[n]) では、入力行セットを指定とパーセン タイル関数を適用するパーティション キーによってパーティションに分割します。 詳細については、このドキュメントの順位付けに関するセクションを参照してください。
注: データ セット内の null はすべて無視されます。

**PERCENTILE_CONT** 列の値の継続的な分布に基づく百分位の数を計算します。 結果には値が補間され、列内の特定の値と一致しない可能性があります。 

**PERCENTILE_DISC** 列の値の離散型分布に基づく百分位数を計算します。 結果は、列内の特定の値と等しくなります。 つまり、PERCENTILE_CONT とは異なり、PERCENTILE_DISC では、常に実際の値 (元の入力値) が返されます。
