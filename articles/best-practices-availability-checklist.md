...
- ローカルおよび地理的な冗長性による**データ レプリケーションを利用します**。 Azure ストレージ内のデータはインフラストラクチャの障害発生時の損失を防ぐために自動的にレプリケートされ、このレプリケーションの一部の要素を構成することができます。 たとえば、データの読み取り専用のコピーを複数の地理的地域にレプリケートすることができます (これは読み取りアクセス地理冗長ストレージまたは RA-GRS と呼ばれます)。 RA-GRS を使用して、追加料金を負担に注意してくださいを参照してください、 [Azure Storage 料金](http://azure.microsoft.com/pricing/details/storage/) 詳細については、Microsoft web サイトのページです。
