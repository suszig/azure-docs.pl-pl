...
Azure アカウントにログインを使用して、 **ログイン AzureRmAccount** コマンドレットです。

    PS C:\> Login-AzureRmAccount

このコマンドレットは、Azure アカウントのログイン資格情報をユーザーに求めます。 ログイン後にアカウント設定がダウンロードされるため、Azure PowerShell で使用できるようになります。 

アカウント設定の有効期限が切れるため、ときどき更新する必要があります。 アカウントの設定を更新するには、実行 **ログイン AzureRmAccount** 再度します。 

>[AZURE.NOTE] リソース マネージャー モジュールには、ログイン AzureRmAccount が必要です。 発行設定ファイルでは不十分です。     
...
各リソースの種類をサポートする場所を取得するには、使用する必要がある、 **Get AzureRmResourceProvider** コマンドレットです。 まず、このコマンドによって表示される情報を見てみましょう。
...
ProviderNamespace は、関連するリソースの種類のコレクションを表します。 通常、これらの名前空間は Azure で作成するサービスに一致します。 としてリストされているリソース プロバイダーを使用する必要があるかどうか **未登録**, 、そのリソース プロバイダーを登録するには、実行されている、 **レジスタ AzureRmResourceProvider** コマンドレットを登録するプロバイダーの名前空間を指定することです。 ほとんどの場合、このチュートリアルで使用するリソース プロバイダーは、サブスクリプションで既に登録されています。  
...
これらのリソースは多くのリージョンで利用可能であることがわかります。 このトピックでは使用 **米国西部**, 、サポートされているリージョンのいずれかを指定することができます。
...
リソース グループを作成するには、使用、 **新規 AzureRmResourceGroup** コマンドレットです。

コマンドを使用して、 **名前** リソース グループの名前を指定するパラメーター、および **場所** の場所を指定するパラメーターです。 前のセクションで発見しましたに基づき、"West US"の使用します。 
場所です。
...
テンプレートです。 一般的に、新しいテンプレートを作成するときは、最新の API バージョンを選択します。 既存のテンプレートの API バージョンを引き続き使用するかどうかを決めることができますがわかっているが変更されない、 
展開の新機能を活用するために最新のバージョンについては、テンプレートを更新するかどうか、またはです。

この手順は一見複雑に見えますが、お使いのリソースで利用可能な API バージョンを検出するのは難しくありません。 もう一度使用する、 **Get AzureRmResourceProvider** コマンドです。
...
**リソース グループ () .location** リソース グループの場所を使用します。
...
リソース グループとテンプレートの作成が終わり、これでテンプレート内で定義されたインフラストラクチャをリソース グループにデプロイする準備が整いました。 使用してリソースを展開する、 **新規 AzureRmResourceGroupDeployment** コマンドレットです。 基本的な構文は次のようになります。

    PS C:\> New-AzureRmResourceGroupDeployment -ResourceGroupName TestRG1 -TemplateFile c:\Azure\Templates\azuredeploy.json

リソース グループとテンプレートの場所を指定します。 テンプレートがローカルでない場合は使用できます、 **- TemplateUri** パラメーターし、テンプレートの URI を指定します。 設定することができます、 
**モード** するか、パラメーター **Incremental** または **完了**します。 既定では、リソース マネージャーは展開時に、増分更新を実行します。したがってに設定する必要はありません **-モード** したい **Incremental**します。 
...
コマンドを入力するときに、不足している必須パラメーターが表示されたら **administratorLoginPassword**します。 パスワードを入力すると、セキュリティ保護された文字列値が隠されます。 この対策によって、プレーンテキストでパスワードを提供することの危険性が解消されます。
...
- サブスクリプション内のすべてのリソース グループを取得する、 **Get AzureRmResourceGroup** コマンドレット。
...
- リソース グループ内のリソースを取得する、 **検索 AzureRmResource** コマンドレットとその **ResourceGroupNameContains** パラメーター。 パラメーターがない場合、Find-AzureRmResource は Azure サブスクリプション内のすべてのリソースを取得します。
...
リソース グループにリソースを追加するに使用することができます、 **新規 AzureRmResource** コマンドレットです。 ただし、この方法でリソースを追加した場合は、その新しいリソースがテンプレート内には存在しないため、将来混乱が生じる可能性があります。 元のテンプレートを再度デプロイすると、不完全なソリューションがデプロイされることになります。 頻繁にデプロイしていれば、新しいリソースをテンプレートに追加して再度デプロイする方が簡単で確実であることがわかります。
...
- リソース グループからリソースの削除を使用して、 **削除 AzureRmResource** コマンドレットです。 このコマンドレットはリソースを削除しますが、リソース グループは削除しません。

    このコマンドは、TestRG リソース グループから TestSite Web サイトを削除します。

        Remove-AzureRmResource -Name TestSite -ResourceGroupName TestRG1 -ResourceType "Microsoft.Web/sites" -ApiVersion 2015-08-01

- リソース グループを削除するには、使用、 **削除 AzureRmResourceGroup** コマンドレットです。 このコマンドレットは、リソース グループとそのリソースを削除します。
