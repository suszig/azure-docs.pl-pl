...
1. Należy zmienić typ bramy do routingu dynamicznego. Obejmujący wiele lokacji sieci VPN wymaga dynamicznej bramy routingu (znanej także jako opartej na trasach). Aby zmienić typ bramy, musisz najpierw usunąć istniejącą bramę, a następnie utwórz nową. Aby uzyskać instrukcje, zobacz [jak zmienić typ routingu sieci VPN dla bramy](vpn-gateway-configure-vpn-gateway-mp.md).  
2. Konfigurowanie nowej bramy i utworzyć tunel sieci VPN. Aby uzyskać instrukcje, zobacz [Brama sieci VPN w klasycznym portalu Azure](vpn-gateway-configure-vpn-gateway-mp.md). Najpierw należy zmienić typ bramy do routingu dynamicznego.

### <a name="if-you-dont-have-a-site-to-site-virtual-network"></a>Jeśli nie masz lokacja-lokacja sieci wirtualnej:
1. Tworzenie sieci wirtualnej lokacja-lokacja, korzystając z tych instrukcji: [tworzenie sieci wirtualnej za pomocą połączenia sieci VPN lokacja-lokacja w klasycznym portalu Azure](vpn-gateway-site-to-site-create.md).  
...
Zaimportuj plik konfiguracji sieci. Podczas importowania tego pliku ze zmianami, zostaną dodane nowe tuneli. Tunele użyje bramy dynamiczne utworzony wcześniej. Możesz użyć klasycznego portalu lub programu PowerShell do zaimportowania pliku.
