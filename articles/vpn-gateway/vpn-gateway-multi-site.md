...
>[AZURE.NOTE] Azure は現在、2 つのデプロイメント モデル (リソース マネージャーおよびクラシック) で使用できることを理解しておくことは重要です。 構成を開始する前に、デプロイ モデルとツールについて理解しておくようにしてください。 配置モデルについては、次を参照してください。 [Azure デプロイ モデル](../azure-classic-rm.md)します。
...
1. **動的ルーティング ゲートウェイでサイト間 VPN を作成します**すでに作成されている場合は、作業の必要はありません。 進む [仮想ネットワークの構成設定のエクスポート](#export)します。 まだ作成していない場合は、以下の作業を行います。

    **サイト間仮想ネットワークが既に存在するが、静的ルーティング ゲートウェイである場合:** **1.**ゲートウェイ タイプを動的ルーティングに変更します。 マルチサイトの VPN は動的ルーティング ゲートウェイを必要とします。 ゲートウェイ タイプを変更するには、最初に既存のゲートウェイを削除し、新規で作成します。 手順については、次を参照してください。 [VPN ゲートウェイ ルーティング タイプ変更](vpn-gateway-configure-vpn-gateway-mp.md/#how-to-change-your-vpn-gateway-type)します。 **2.**新しいゲートウェイを構成し、VPN トンネルを作成します。 手順については、次を参照してください。 [Azure クラシック ポータルで VPN ゲートウェイを構成する](vpn-gateway-configure-vpn-gateway-mp.md)します。

    **サイト間仮想ネットワークが存在しない場合:** **1.**これらの手順に従ってサイト間仮想ネットワークを作成します。 [Azure クラシック ポータルでサイト間 VPN 接続で仮想ネットワークの作成](vpn-gateway-site-to-site-create.md)します。 **2.**これらの手順に従って、動的ルーティング ゲートウェイを構成する: [VPN ゲートウェイを構成する](vpn-gateway-configure-vpn-gateway-mp.md)します。 必ずゲートウェイ タイプに**動的ルーティング**を選択してください。

1. * *<a name="export"></a>エクスポートするには、仮想ネットワーク構成 settings.* *、ネットワーク構成ファイルをエクスポートするを参照してください [ネットワーク設定をエクスポートする](../virtual-network/virtual-networks-using-network-configuration-file.md)します。 エクスポートするファイルは新しいマルチサイト設定の構成に使われます。
...
1. **ネットワーク構成ファイルを保存し、インポートします。**ネットワーク構成ファイルをインポートするを参照してください。 [ネットワーク設定をインポートする](../virtual-network/../virtual-network/virtual-networks-using-network-configuration-file.md#export-and-import-virtual-network-settings-using-the-management-portal)します。 変更をしてこのファイルをインポートすると、新しいトンネルが追加されます。 トンネルは先ほど作成した動的ゲートウェイを使用します。
