...
- クラシック展開モデルを使用して、仮想ネットワークが作成された場合は、次を参照してください。 [VNet に接続を作成](virtual-networks-configure-vnet-to-vnet-connection.md)します。 クラシック デプロイメント モデルでは、異なるサブスクリプションに存在する VNet 間の接続をサポートします。
    
- 作成された仮想ネットワークに接続する場合は、Azure リソース マネージャーを使用して作成された仮想ネットワークにクラシック展開モデルのモデルを参照してください [従来の Vnet を新しい Vnet に接続する](../virtual-network/virtual-networks-arm-asm-s2s.md)です。
...
次に、仮想ネットワークを作成します。 次の例は、という名前の仮想ネットワークを作成 *VNet1* と呼ばれる 2 つのサブネット *GatewaySubnet* と呼ばれる、もう一方と *Subnet1*します。 具体的にはという名前の 1 つのサブネットを作成することが重要 *GatewaySubnet*します。 別の名前にすると、接続の構成は失敗します。 次の例では、ゲートウェイ サブネットで /28 を使用しています。 ゲートウェイ サブネットには最小値 /29 まで使用できます。 機能によっては (ExpressRoute とサイト間 VPN が共存する接続など)、/27 より大きなゲートウェイ サブネットが必要となる場合があるため、将来追加する可能性がある機能に対応できるようにゲートウェイ サブネットを作成してください。
...
この時点では、リソース マネージャーを使用して作成された VPN 接続はプレビュー ポータルに表示されません。 ただしを使用して、接続が成功したことを確認することは *Get AzureRmVirtualNetworkGatewayConnection – デバッグ*します。 将来、これを行うためのコマンドレットと、プレビュー ポータルで接続を表示する機能が提供される予定です。

次のコマンドレットの例を使用できます。 検証する各接続と一致するように、値を変更してください。 メッセージが表示されたら、[ *A* すべてを実行するためにします。 
