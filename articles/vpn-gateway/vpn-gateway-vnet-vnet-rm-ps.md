<properties
   pageTitle="Tworzenie połączenia usługi VPN Gateway między sieciami wirtualnymi przy użyciu usługi Azure Resource Manager i programu PowerShell dla sieci wirtualnych | Microsoft Azure"
...
# Konfigurowanie połączenia sieci wirtualnych przy użyciu usługi Azure Resource Manager i programu PowerShell

> [AZURE.SELECTOR]
- [Klasyczny portal Azure](virtual-networks-configure-vnet-to-vnet-connection.md)
- [PowerShell — model usługi Azure Resource Manager](vpn-gateway-vnet-vnet-rm-ps.md)

W tym artykule opisano kroki tworzenia połączenia między sieciami wirtualnymi w modelu wdrażania przy użyciu usługi Resource Manager i środowiska PowerShell. Sieci wirtualne mogą być zlokalizowane w tych samych lub różnych regionach i mogą funkcjonować w ramach tej samej lub różnych subskrypcji.
...
W tym artykule opisano różne instrukcje, które mają zastosowanie do sieci wirtualnych utworzonych w ramach modelu wdrażania przy użyciu usługi Resource Manager. Kroki konfiguracji połączenia między sieciami wirtualnymi zależą od tego, czy sieci wirtualne należą do tej samej, czy też do różnych subskrypcji. 

![Oba rodzaje połączeń](./media/vpn-gateway-vnet-vnet-rm-ps/differentsubscription.png)


W kontekście czynności konfiguracyjnych kluczowa różnica między wspomnianymi rodzajami połączeń polega na możliwości bądź braku możliwości konfiguracji wszystkich zasobów sieci wirtualnych i bramy w ramach tej samej sesji programu PowerShell. W tym dokumencie opisano kroki dotyczące każdego z wariantów. Powyższa grafika przedstawia połączenia między sieciami wirtualnymi należącymi do tej samej oraz do innych subskrypcji. 


- [Sieci wirtualne w tej samej subskrypcji](#samesub)
- [Sieci wirtualne w różnych subskrypcjach](#difsub)


## <a name="samesub"></a>Łączenie sieci wirtualnych, które należą do tej samej subskrypcji

Ta konfiguracja ma zastosowanie w przypadku sieci wirtualnych, które należą do tej samej subskrypcji — jak pokazano na poniższym diagramie:

![Połączenie między sieciami wirtualnymi w tej samej subskrypcji](./media/vpn-gateway-vnet-vnet-rm-ps/samesubscription.png)
...
Należy określić zakresy, które zostaną użyte do ustawienia konfiguracji sieci. Niezbędne jest upewnienie się, że zakresy sieci wirtualnej ani sieci lokalnej nie zachodzą na siebie w jakikolwiek sposób.

W poniższych krokach utworzymy dwie sieci wirtualne wraz z odpowiednimi konfiguracjami oraz podsieciami bram. Następnie utworzymy połączenie bramy VPN między dwoma sieciami wirtualnymi.

W tym celu należy zastosować następujące wartości dla sieci wirtualnych:
...
2. Połączenie się z subskrypcją 1
...
4. Utworzenie konfiguracji podsieci dla sieci TestVNet1
    ...
    W poniższym przykładzie podsieć bramy wykorzystuje wartość /27. Choć z technicznego punktu widzenia można utworzyć mniejszą podsieć bramy wykorzystującą wartość /29, to nie zaleca się takiego postępowania. Zaleca się wybranie większych rozmiarów i wartości takich jak /27 lub /26 — na wypadek, gdyby przyszłe koncepcje miały wykorzystywać istniejące lub przyszłe konfiguracje, które mogą wymagać większej podsieci bramy. 
...
6. Przesłanie żądania dotyczącego publicznego adresu IP
    ...
    Następnie zostaje przesłane żądanie przydzielenia publicznego adresu IP do bramy, która zostanie utworzona dla sieci wirtualnej. Nie można określić adresu IP, który ma zostać użyty. Adres jest przydzielany do bramy w sposób dynamiczny. Ten adres IP zostanie użyty w następnej sekcji konfiguracji. Skorzystaj z poniższego przykładu. Dla tego adresu należy wybrać dynamiczną metodę alokacji. 
...
8. Utworzenie bramy dla sieci TestVNet1
    ...
    W tym kroku zostaje utworzona brama sieci wirtualnej TestVNet1. Konfiguracje połączeń między sieciami wirtualnymi wymagają zastosowania wartości RouteBased obiektu VpnType. Tworzenie bramy może potrwać co najmniej 30 minut.
...
Po skonfigurowaniu sieci TestVNet1 należy powtórzyć opisane czynności w celu utworzenia sieci TestVNet4. Wykonaj kroki opisane poniżej, w razie potrzeby zastępując podane wartości własnymi. Ten krok można przeprowadzić w tej samej sesji programu PowerShell ze względu na przynależność do tej samej subskrypcji.
...
## <a name="Verify"></a>Sprawdzanie połączenia między sieciami wirtualnymi

Poniższe przykłady pokazują, w jaki sposób można sprawdzić połączenie. Pamiętaj o zmianie wartości pod kątem danego środowiska.

### Sprawdzenie połączenia przy użyciu programu portalu Azure

Połączenie sieci VPN można sprawdzić w portalu Azure, przechodząc do opcji **Bramy sieci wirtualnej** -> ** i klikając nazwę bramy** -> **Ustawienia** -> **Połączenia**. Po wybraniu nazwy połączenia można wyświetlić więcej informacji w bloku **Połączenie**. 


### Sprawdzanie połączenia przy użyciu programu PowerShell

Istnieje również możliwość sprawdzenia, czy połączenie zostało pomyślnie nawiązane, przy użyciu polecenia *Get-AzureRmVirtualNetworkGatewayConnection –Debug*. Można skorzystać z następującego przykładu użycia, zmieniając wartości odpowiednio do własnych potrzeb. Po wyświetleniu monitu wybierz opcję A, aby uruchomić wszystkie elementy.

    Get-AzureRmVirtualNetworkGatewayConnection -Name $Connection1 -ResourceGroupName $RG1 -Debug

Po zakończeniu działania polecenia cmdlet przewiń, aby wyświetlić wartości. W poniższym przykładzie wartości wyjściowych programu PowerShell stan połączenia jest wyświetlany jako *Połączone* i można zobaczyć bajty przychodzące i wychodzące.
...
Poniższe kroki konfiguracji pozwalają dodać dodatkowe połączenie między sieciami wirtualnymi w celu połączenia sieci wirtualnej TestVNet1 z siecią wirtualną TestVNet5, z których obie należą do innych subskrypcji. W tym przykładzie różnica polega na tym, że w kontekście drugiej subskrypcji część czynności konfiguracyjnych należy wykonać w osobnej sesji programu PowerShell — szczególnie w przypadku, gdy obie subskrypcje należą do różnych organizacji. Po wykonaniu poniższych kroków można uzyskać konfigurację przedstawioną na poniższym diagramie:

![Różne subskrypcje w ramach połączenia między sieciami wirtualnymi](./media/vpn-gateway-vnet-vnet-rm-ps/differentsubscription.png)

Poniższe instrukcje stanowią ciąg dalszy poprzedzających je kroków wymienionych powyżej. Należy wykonać [Krok 1](#Step1) i [Krok 2](#Step2), aby utworzyć i skonfigurować sieć TestVNet1 i bramę VPN dla sieci TestVNet1. Chcąc połączyć tylko sieci wirtualne należące do różnych subskrypcji, nie trzeba wykonywać kroków 3 i 4 z poprzedniego ćwiczenia. Zamiast tego można przejść od razu do kroku 5 poniżej. 
...
5. Weryfikowanie połączenia

    Po wykonaniu tych kroków można sprawdzić połączenie korzystając z metod opisanych w temacie [Sprawdzanie połączenia między sieciami wirtualnymi](#Verify).
