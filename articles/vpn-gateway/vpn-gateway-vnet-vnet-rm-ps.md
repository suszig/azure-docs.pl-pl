...
- 作成された仮想ネットワークに接続する場合は、Azure リソース マネージャーを使用して作成された仮想ネットワークにクラシック展開モデルのモデルを参照してください [従来の Vnet を新しい Vnet に接続する](../virtual-network/virtual-networks-arm-asm-s2s.md)します。
...
VNet1 の構成後、前述の手順を繰り返して、VNet2 とそのゲートウェイ構成を構成します。 2 つの Vnet と、それぞれのゲートウェイの構成が完了したら後に、進んで、* * 手順 8 です。 ゲートウェイ * に接続します。
...
ゲートウェイ サブネットが正しく構成されていることを確認した後で続行 * * 手順 4 です。 これを要求すると、パブリック IP アドレス * * と手順を実行します。


## 次のステップ

接続が完成したら、仮想ネットワークに仮想マシンを追加することができます。 参照してください [仮想マシンの作成](../virtual-machines/virtual-machines-windows-tutorial.md) の手順を実行します。
