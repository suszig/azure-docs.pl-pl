...
クラシック デプロイ モデルで作成した VNet をリソース マネージャー モデルで作成した VNet に接続するには、 参照してください [従来の Vnet を新しい Vnet に接続する](../virtual-network/virtual-networks-arm-asm-s2s.md)します。
...
2. [仮想ネットワークを作成します。](#create-your-virtual-networks)
3. [ローカル ネットワークを追加します。](#add-local-networks)
4. [各 VNet の動的ルーティング ゲートウェイを作成します。](#create-the-dynamic-routing-gateways-for-each-vnet)
5. [VPN ゲートウェイを接続します。](#connect-the-vpn-gateways)
...
1. 画面の左下隅で **[新規]** をクリックします。 ナビゲーション ウィンドウで **[Network Services]** をクリックし、**[Virtual Network]** をクリックします。 **[ローカル ネットワークの追加]** をクリックします。
