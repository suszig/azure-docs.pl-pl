...
> さらに、Azure はロール内キャッシュをサポートします。 この機能
  ...
  [どの Azure Cache を利用すればよいですか。](http://msdn.microsoft.com/library/azure/dn766201.aspx) Microsoft web サイトです。
...
- _サーバー側でクエリをルーティングする。_ この方法でクライアント アプリケーションはのいずれかに要求を送信します
...
- _クライアント側でパーティション分割する。_ このモデルではクライアント アプリケーションに (おそらくのロジックが含まれています
...
- _プロキシでパーティション分割する。_ このスキームでは、クライアント アプリケーションは要求を送信します。
...
-  [を Azure のキャッシュ オファリングはよいですか。](http://msdn.microsoft.com/library/azure/dn766201.aspx) Microsoft web サイトのページです。
