...
- [サービスの詳細情報] ボックスの一覧 (csv1)](https://azurepricing.blob.core.windows.net/supplemental/MOSPServices_csv1.xlsx)
- [サービスの詳細情報] ボックスの一覧 (csv2)](https://azurepricing.blob.core.windows.net/supplemental/MOSPServices_csv2.xlsx)

* 注: **csv1** リンクは csv バージョン 1 の列のヘッダー名を参照し、 **csv2** リンクは csv バージョン 2 の新しい列ヘッダー名を参照します。 これらのファイルは更新された monthly.* です。
...
:---------------| :----------------| -----|
 ...
 該当なし| リソース グループ| _**新しい列の追加**。_ リソース グループのデプロイ済みのリソースを実行しているのです。Http://azure.microsoft.com/en-us/documentation/articles/resource-group-overview/を参照してください。
 コンポーネント| インスタンス ID| 実行中のリソースの識別子です。識別子には、リソースの作成時に指定した名前が含まれています。
 該当なし| タグ| _**新しい列の追加**。_ Azure で新しいリソースの種類では、リソースにタグを使用することができます。Http://azure.microsoft.com/en-us/updates/organize-your-azure-resources-with-tags/を参照してください。
...
- **インスタンス ID**: [インスタンス ID] フィールドは、プロビジョニングされたサービスのユーザー指定の識別子を示します。現在、インスタンス ID を表現する書式は 2 つあり、リソース名、または完全修飾リソース ID です。Microsoft Azure サービスは、標準化された完全修飾リソース ID 形式 _ * * でインスタンス ID を表す移行 (/subscriptions/<subscription id>/resourcegroups/<resourcegroupname>/providers/<providername>/<resourcename>) * * _ です。サービスが新しい書式に移行すると、[インスタンス ID] データ フィールドはリソース名からリソース ID に変わります。リソース ID を使用する書式は、 [Azure リソース マネージャー API](https://msdn.microsoft.com/library/azure/dn790567.aspx) 内のリソースを識別するために、サブスクリプションです。
...
- **タグ**: この列には、指定されたユーザーが含まれています。 リソース タグ。 タグは、課金記録のグループ化に使用できます。 たとえば、タグを使用して、サービスを使用する部門ごとにコストを配分することができます。 詳細について [タグを使用して、Azure リソースを整理する](./resource-group-using-tags.md)します。 タグの生成をサポートするサービスは、次のとおりです。
    - 仮想マシン
    - 記憶域
    - ネットワーク サービスをを使用してプロビジョニング [Azure リソース マネージャー API](https://msdn.microsoft.com/library/azure/dn790567.aspx)
...
- [支払い方法を管理します。](https://msdn.microsoft.com/library/azure/dn736054.aspx)

- [既存のクレジット_カードの支払い情報を編集します。](https://msdn.microsoft.com/library/azure/dn736053.aspx)

- [支払方法として使用する新しいクレジット_カードの追加します。](https://msdn.microsoft.com/library/azure/dn736057.aspx)

- [Microsoft Azure アカウントにクレジット_カードを変更します。](https://msdn.microsoft.com/library/azure/dn736050.aspx)

- [Azure サブスクリプションが無効になった場合はどうすればでしょうか。](https://msdn.microsoft.com/library/azure/dn736049.aspx)
