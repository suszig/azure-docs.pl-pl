...
消費量および料金プランの詳細については、次を参照してください。、 [Microsoft Azure の購入オプション」ページ](http://azure.microsoft.com/pricing/purchase-options/)します。
...
*注: **csv1** リンクは csv バージョン 1 の列のヘッダー名を参照し、 **csv2** リンクは csv バージョン 2 の新しい列ヘッダー名を参照します。  これらのファイルは毎月更新されます。*
...
### Microsoft Azure の課金内容の表示またはダウンロード:
 ...
 [使用状況と課金ポータル](https://account.windowsazure.com/subscriptions) 表示現在および過去の課金情報をダウンロードできます。

課金内容を表示またはダウンロードするには:

1. サインイン、 [使用状況と課金ポータル](https://account.windowsazure.com/subscriptions) 、Microsoft アカウント ID か、職場または学校のアカウント ID を使用します。
...
4.  **請求履歴** セクションでは、最後の 6 つの請求期間と、現行の未請求期間の明細書を示します。 現在の期間の明細は、見積もりを作成した時点での料金の見積もりです。 この情報は毎日更新されますが、その日までのすべての累計使用量が含まれていない可能性があります。 その月に実際に支払う料金は、この見積もりとは異なる場合があります。  

    ![概要 - 請求履歴 2](./media/billing-understand-your-bill/ContentViewaBillforMA2.png)

5. をクリックして **[現在の明細** 見積もりを作成した時点での料金の見積もりを表示します。 この情報は毎日更新されますが、その日までのすべての累計使用量が含まれていない可能性があります。 その月に実際に支払う料金は、この見積もりとは異なる場合があります。
...
6. クリックして **請求書のダウンロード** の前、請求書のコピーを表示します。
...
顧客情報] セクションでは、使用状況とプロファイルを関連情報を識別します。
...
### 詳細な使用状況
「詳細な使用状況」の説明のリンクから、Azure の使用量と課金ポータルに移動します。ここで、このサブスクリプションの詳細な使用状況を表示できます。  今すぐダウンロード可能な 2 つのバージョンがある:  **.csv version 1** 古い命名規則と使用状況のフィールドが含まれていて、 **.csv version 2** したサービス内容を知ることは Microsoft Azure で使用するカテゴリの他の各フィールドに、顧客のフレンドリ名が含まれています。
...
ユーザーが Azure の使用状況を管理しやすくなるように、Azure サービスの使用状況とコストについて報告する使用状況ファイルのダウンロード機能を強化しています。  ダウンロード リンクには、使用状況ファイルの 2 つのバージョンが含まれています: **Version 1** 既存の形式を使用して **バージョン 2** 追加情報が含まれています、日ごとの使用量セクション内の列名を更新します。  

使用料金は合計 **毎月** クレジットまたは割引サブスクリプションに対して請求します。 前月の利用量の未払い分が課金されます。  ファイルの最上部セクションには、前月の請求サイクル中に課金されたサービスに関する詳細が表示されます。  次の表に、各バージョンの .csv ファイルの列名を示します。
...
## 日ごとの使用量データの分析
使用量に応じて、日ごとの使用量データが数千行存在する場合があります。 データを分析する場合は、クリックして **使用量のダウンロード** を選択し、該当する請求期間の日ごとの使用状況データを表示するコンマ区切り値 (.csv) バージョンです。  以下の各バージョンのサンプル .csv ファイルをダウンロードすることもできます。
...
該当なし | リソース グループ | _**新しい列の追加。**_デプロイされたリソースが実行されるリソース グループ。 Http://azure.microsoft.com/en-us/documentation/articles/resource-group-overview/を参照してください。
コンポーネント | インスタンス ID | 実行中のリソースの識別子です。 識別子には、リソースの作成時に指定した名前が含まれています。
該当なし | タグ | _**新しい列の追加。**_Azure の新しいリソースの種類では、リソースにタグを付けることができます。 Http://azure.microsoft.com/en-us/updates/organize-your-azure-resources-with-tags/を参照してください。
...
- **インスタンス ID**: インスタンス ID] フィールドは、ユーザーがプロビジョニングされたサービスの識別子を指定します。 現在、インスタンス ID を表現する書式は 2 つあり、リソース名、または完全修飾リソース ID です。 Microsoft Azure サービスは、標準化された完全修飾リソース ID の形式でインスタンス ID を表す移行 _**(/subscriptions/<subscription id>/resourcegroups/<resourcegroupname>/providers/<providername>/<resourcename>)**_します。 サービスが新しい書式に移行すると、[インスタンス ID] データ フィールドはリソース名からリソース ID に変わります。 リソース ID を使用する書式は、 [Azure リソース マネージャー API](https://msdn.microsoft.com/library/azure/dn790567.aspx) 内のリソースを識別するために、サブスクリプションです。

![instanceid](./media/billing-understand-your-bill/instanceid.png)

- **追加情報**: 使用状況の .csv の [追加情報] 列は、サービス固有のメタデータを指定します。 たとえば、VM のイメージの種類です。 現在、サービスからは、[追加情報]、[サービス情報 1]、[サービス情報 2] フィールドという複数の列にサービス固有のメタデータが出力されます。 Microsoft Azure サービスは、[追加情報] 列のみについて、サービス固有のメタデータの出力を標準化します。  標準化された形式のスナップショットを次に示します。

![additionalinfo_csv2](./media/billing-understand-your-bill/AdditionaInfo_csv2.png)

- **タグ**: この列には、指定されたユーザーが含まれています。 リソース タグ。 タグは、課金記録のグループ化に使用できます。 たとえば、タグを使用して、サービスを使用する部門ごとにコストを配分することができます。 詳細について [タグを使用して、Azure リソースを整理する](./resource-group-using-tags.md)です。 タグの生成をサポートするサービスは、次のとおりです。  
    - 仮想マシン
    - 記憶域
    - 使用してプロビジョニングされたネットワーク サービス、 [Azure リソース マネージャー API](https://msdn.microsoft.com/library/azure/dn790567.aspx)

![tags](./media/billing-understand-your-bill/tags.png)


## その他のリソース
セクションに移動 **アカウントの管理、サブスクリプション、および管理者の役割**, [ [サービスの管理](https://msdn.microsoft.com/library/azure/dn578292.aspx) 非常に役立つリンクが掲載します。
