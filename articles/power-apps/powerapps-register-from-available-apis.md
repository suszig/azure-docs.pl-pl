...
ある **マネージ Microsoft** と **IT 管理対象** Api です。 PowerApps Enterprise を有効にすると、Microsoft 管理の API が自動的に使用可能になります。 メモリ、接続性、信頼なども自動的に管理されます。 次の手順では、Twitter アカウントやパスワードなど、任意の特定のユーザー設定を入力します。 
...
使用する、 **マネージ Microsoft** または **IT 管理対象** Api、する必要があります「登録」Api、Azure ポータルで。 登録されると、アプリでこれらの API を使用することができます。 以下のオプションを使用できます。 
...
この記事でに **マネージ Api を事前に構築されたマイクロソフトの登録と、IT 管理 Api**します。 
 ...
 **マネージ Microsoft** Api PowerApps Enterprise に付属しているし、Microsoft によってホストされています。 多くのシナリオでは、Microsoft 管理の API はアプリに最適です。 たとえば、アプリでツイートを送信したり、ファイルを OneDrive にアップロードしたり、Excel ファイルのデータを表示したりする場合、Microsoft 管理の API は適切な選択です。 
...
API | 説明 | 手順のリンク
--- | --- | ---
![][31] | **Dropbox**<br/><br/> 取得、更新、複数のアイテム、および削除できます。 | [**作業の開始**](powerapps-create-api-dropbox.md)
![][32] | **DynamicsCRM オンライン**<br/><br/> 取得、更新、複数のアイテム、および削除できます。 | [**作業の開始**](powerapps-create-api-crmonline.md)
...
![][43] | **Twitter**<br/><br/> ツイート、ツイートの検索、フォロワーの「」を参照、および送信します。 | [**作業の開始**](powerapps-create-api-twitter.md)


## 使用可能な IT 管理の API を表示する
 **IT 管理対象** Api が自分で制御し、管理します。 Microsoft 管理の環境では実行されません。 一部のシナリオでは、独自の IT 管理の環境でこれらの API を使用する方がアプリのニーズに適している場合があります。 たとえば、アプリが Twitter API を使用しており、(Microsoft の Twitter キー) ではなく、組織の Twitter キーを使用する必要があるとします。 このような状況では、Twitter API を IT 管理の API として構成することをお勧めします。 アプリがオンプレミスのデータベースに接続するために SQL Server API を使用するような例もあります。 IT 管理の環境では、オンプレミスへの接続のために仮想ネットワークを設定したり、Express Route を使用したりできます。 何を選択するのかは自由です。
...
- **すべて** 、マイクロソフトのマネージ Api こともできます IT 管理対象です。 たとえば、Office 365 の独自のインスタンスを作成し、このインスタンスを完全に制御することも可能です。 Office 365 の IT 管理 API と、Office 365 の Microsoft 管理 API を同じ環境で使用することができます。 これはアプリのニーズによります。
- オンプレミスのシステムに接続する場合や、Bing 検索を使用する場合、セキュリティ、認証、ライセンスなどを制御できます。


#### IT 管理の API
> [AZURE.NOTE] 思い出してください **すべて** 、マイクロソフトのマネージ Api こともできます IT 管理対象です。 次の API は IT のみで管理され、Microsoft 管理にすることはできません。
