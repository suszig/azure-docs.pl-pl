...
Azure CDN ノードが、同じ都市<sub>。1</sub>  
Azure CDN ノードが隣接する都市、<sub>。2</sub>  
...
- **回復性** CDN が潜在的な単一アプリケーションの障害点です。 CDN の可用性に関する SLA は BLOB ストレージよりも低いので (BLOB ストレージはコンテンツの直接配信に使用できます)、必要に応じて、重要なコンテンツ用にフォールバック メカニズムの実装を検討します。
