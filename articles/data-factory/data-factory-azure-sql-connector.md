...
**注:** を構成する必要がある [Azure SQL Database ファイアウォール](https://msdn.microsoft.com/library/azure/ee621782.aspx#ConnectingFromAzure)します。 データベース サーバーを構成する必要があります。 [Azure サービス、サーバーへのアクセスを許可する](https://msdn.microsoft.com/library/azure/ee621782.aspx#ConnectingFromAzure)します。 また、Azure の外部から (たとえば、Data Factory ゲートウェイのあるオンプレミスのデータ ソースから) Azure SQL にデータをコピーする場合、Azure SQL にデータを送信するマシンに適切な IP アドレス範囲を設定する必要があります。
