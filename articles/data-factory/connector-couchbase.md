...
|:--- |:--- |:--- |
| type | Właściwość type musi mieć ustawioną: **Couchbase** | Tak |
| Parametry połączenia | Parametry połączenia ODBC do nawiązania połączenia Couchbase. Można wybrać opcję Oznacz to pole jako SecureString Zapisz w bezpiecznej lokalizacji w ADF lub przechowywania haseł w usłudze Azure Key Vault i umożliwić działanie kopiowania ściągnięcia stamtąd podczas wykonywania kopii danych — Dowiedz się więcej o [przechowywania poświadczeń w magazynie kluczy](store-credentials-in-key-vault.md). | Tak |
...
| Właściwość | Opis | Wymagane |
|:--- |:--- |:--- |
| type | Musi mieć ustawioną właściwość type źródła działania kopiowania: **CouchbaseSource** | Tak |
