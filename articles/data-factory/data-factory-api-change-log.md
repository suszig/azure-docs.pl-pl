...
この記事では、特定のバージョンの Azure Data Factory SDK の変更に関する情報を提供します。 Azure Data Factory の最新の Nuget パッケージを検索する [ここで](https://www.nuget.org/packages/Microsoft.Azure.Management.DataFactories) 
...
- 次の新しいアクティビティの種類が追加されました: [AzureMLUpdateResourceActivity](https://msdn.microsoft.com/library/microsoft.azure.management.datafactories.models.azuremlupdateresourceactivity.aspx)します。 利用状況に関する詳細については、「 [リソースの更新プログラムのアクティビティを使用して、Azure ML の更新モデル](https://azure.microsoft.com/documentation/articles/data-factory-azure-ml-batch-execution-activity/#updating-azure-ml-models-using-the-update-resource-activity)します。
...
-  **API バージョン** の今回のリリース: **2015年-10-01**します。

-  **リスト** メソッドが今すぐページングされた結果を返します。 空でないが、応答に含まれている場合 **NextLink** プロパティ、クライアント アプリケーションがすべてのページが返されるまでは、次のページを取り出しを続行する必要があります。  たとえば次のようになります。 
...
- **リスト** パイプライン API は、完全な詳細情報ではなく、パイプラインの概要のみを返します。 たとえば、パイプライン概要のアクティビティには、名前と種類だけが含まれます。
