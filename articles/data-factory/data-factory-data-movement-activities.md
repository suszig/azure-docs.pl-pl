...
## サポートされるデータ ストア
コピー アクティビティからのデータのコピー、 **ソース** データ ストア、 **シンク** データ ストアです。 Data factory は、次のデータ ストアをサポートし、 **データできる任意のソースからに書き込める任意のシンク**します。 データ ストアをクリックすると、そのストアとの間でデータをコピーする方法がわかります。 

| ソース| シンク |
|:------- | :---- |
| <ul><li>[Azure Blob](data-factory-azure-blob-connector.md)</li><li>[Azure テーブル](data-factory-azure-table-connector.md)</li><li>[Azure SQL Database](data-factory-azure-sql-connector.md)</li><li>[Azure SQL Data Warehouse](data-factory-azure-sql-data-warehouse-connector.md)</li><li>[Azure DocumentDB](data-factory-azure-documentdb-connector.md)</li><li>[Azure Data Lake ストア](data-factory-azure-datalake-connector.md)</li><li>[SQL Server の内部設置型と Azure の IaaS](data-factory-sqlserver-connector.md)</li><li>[ファイル システムの内部設置型と Azure の IaaS](data-factory-onprem-file-system-connector.md)</li><li>[Oracle データベースの内部設置型と Azure の IaaS](data-factory-onprem-oracle-connector.md)</li><li>[MySQL データベースの内部設置型と Azure の IaaS ](data-factory-onprem-mysql-connector.md)</li><li>[DB2 データベースの内部設置型と Azure の IaaS](data-factory-onprem-db2-connector.md)</li><li>[Teradata データベースの内部設置型と Azure の IaaS ](data-factory-onprem-teradata-connector.md)</li><li>[Sybase データベースの内部設置型と Azure の IaaS](data-factory-onprem-sybase-connector.md)</li><li>[PostgreSQL データベースの内部設置型と Azure の IaaS](data-factory-onprem-postgresql-connector.md)</li></ul> | <ul><li>[Azure Blob](data-factory-azure-blob-connector.md)</li><li>[Azure テーブル](data-factory-azure-table-connector.md)</li><li>[Azure SQL Database](data-factory-azure-sql-connector.md)</li><li>[Azure SQL Data Warehouse](data-factory-azure-sql-data-warehouse-connector.md)</li><li>[Azure DocumentDB](data-factory-azure-documentdb-connector.md)</li><li>[Azure Data Lake ストア](data-factory-azure-datalake-connector.md)</li><li>[SQL Server の内部設置型と Azure の IaaS](data-factory-sqlserver-connector.md)</li><li>[ファイル システムの内部設置型と Azure の IaaS](data-factory-onprem-file-system-connector.md)</li></ul> |
...
## <a name="copyactivity"></a>アクティビティ
コピー アクティビティは、1 つの入力データセット (**ソース**) し、1 つの出力データセット (**シンク**)。 データのコピーは、アクティビティで指定されているスケジュールに従ってバッチ方式で行われます。 詳細については、さまざまな JSON のセクションでは、すべてのアクティビティの使用可能なプロパティなどの高レベルで一般にアクティビティを定義する、次を参照してください。 [理解パイプラインとアクティビティの](data-factory-create-pipelines.md) 記事です。
...
- データをコピーする場合、 **内部設置型のデータ ソース** に **クラウド** またはその逆 (例: 内部設置型 SQL Server には、Azure Blob が]-> [)、データの移動は実際には、 **Data Management Gateway** なしに抑えながらデータ移動のサービスから内部設置型環境内でします。
- コピーする場合、 **クラウド ソース** に、 **クラウド移行先** (例: Azure Blob には Azure SQL]-> [)、 **Data Movement Service** ある展開を選択 **同じ geo にシンクの場所に最も近い** を転送を行うには。 たとえば、東南アジアから西日本にコピーする場合は、東日本の Data Movement Service のデプロイを使用してコピーを実行します。 コピー元とコピー先の両方が同じ地域にあり、その地域に利用可能な Data Movement Service がない場合 (現時点ではオーストラリアなど)、コピー アクティビティは、代替の地域で行われることなく、失敗します。 注: Data Movement Service はオーストラリアにも拡張されます。 
...
(つまり、ファイアウォールの内側) 内部設置型データ ソースとして使用するデータ ソースを扱う必要があります使用する場合でも **ExpressRoute** と **ゲートウェイを使用して** サービスとデータ ソースの間の接続を確立します。 
...
### コピー アクティビティのプロパティ
名前、説明、入力テーブル、出力テーブル、さまざまなポリシーなどのプロパティがあらゆる種類のアクティビティで利用できます。 使用可能なプロパティ、 **typeProperties** 、アクティビティのセクションは、その一方でアクティビティの種類によって異なります。 

コピー アクティビティの場合、 **typeProperties** セクションとソースの種類によって異なりますが、シンクします。 上に示した各データ ストア固有のページで、データ ストアの種類に固有のこれらのプロパティが説明されています。
