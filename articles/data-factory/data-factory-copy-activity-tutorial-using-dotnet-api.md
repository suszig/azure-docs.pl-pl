...
1. Dodaj następujący kod, który **tworzy i aktywuje potok**, do metody **Main**. Ten potok zawiera działanie **CopyActivity**, które pobiera element **BlobSource** jako źródło i **BlobSink** jako ujście.
...
3. Dodaj następujący kod do metody **Main**, aby uzyskać szczegóły uruchomienia dla wycinka danych.
           ...
           Console.ReadKey();
4. Dodaj następującą metodę pomocnika używaną przez metodę **Main** do klasy **Program**.  
...
5. W Eksploratorze rozwiązań rozwiń projekt (**DataFactoryAPITestApp**), kliknij prawym przyciskiem myszy pozycję **Odwołania** i kliknij polecenie **Dodaj odwołanie**. Zaznacz pole wyboru zestawu „**System.Configuration**” i kliknij przycisk **OK**. 
6. Skompiluj aplikację konsolową. Kliknij przycisk **Kompiluj** w menu i kliknij opcję **Kompiluj rozwiązanie**. 
7. Potwierdź, że istnieje co najmniej jeden plik w kontenerze **adftutorial** w magazynie obiektów blob platformy Azure. W przeciwnym razie utwórz w Notatniku plik **Emp.txt** o następującej treści i przekaż go do kontenera adftutorial.
   
       John, Doe
       Jane, Doe
8. Uruchom próbkę, klikając pozycję **Debuguj** -> **Rozpocznij debugowanie** w menu. Po wyświetleniu komunikatu **Pobieranie szczegółów uruchomienia wycinka danych** zaczekaj kilka minut, a następnie naciśnij klawisz **ENTER**. 
9. Użyj witryny Azure Portal, aby upewnić się, że fabryka danych **APITutorialFactory** została utworzona z następującymi artefaktami: 
