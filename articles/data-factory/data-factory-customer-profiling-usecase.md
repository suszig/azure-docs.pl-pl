...
Azure Data Factory は、ソリューション アクセラレータの Cortana Analytics Suite の実装に使用されている数多くあるサービスの 1 つです。 Cortana Analytics の詳細については、次を参照してください [Cortana Analytics Suite](http://www.microsoft.com/cortanaanalytics)します。 このドキュメントでは、Azure Data Factory が一般的な分析の問題を解決する方法を理解するのに役立つ簡単な使用事例を説明します。

アクセスして、この単純なユース ケースを試用するだけで済みますが、 [Azure サブスクリプション](https://azure.microsoft.com/pricing/free-trial/)します。 次に説明されている手順に従って、この使用事例を実装するサンプルを展開する、 [サンプル](data-factory-samples.md) 記事です。
...
上の図は、展開した後、Azure クラシック ポータル UI でデータ パイプラインが表示する方法を示しています。
