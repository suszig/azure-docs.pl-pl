...
Przedsiębiorstwa dysponują danymi różnych typów, znajdującymi się w różnych magazynach lokalnych i w chmurze, ze strukturą pełną i częściową, przychodzącymi w różnych interwałach i w różnym tempie. Pierwszy krok tworzenia systemu uzyskiwania informacji polega na połączeniu wszystkich wymaganych źródeł danych i systemów przetwarzania, takich jak usługi SaaS, bazy danych, udziały plików oraz internetowe usługi FTP, i przeniesieniu danych w miarę potrzeb do centralnej lokalizacji w celu ich dalszego przetwarzania.
...
Potoki danych stały się kluczową częścią analityki biznesowej, w związku z czym dostrzegliśmy, że te krytyczne działania związane z danymi wymagają elastycznych harmonogramów, obsługujących przyrostowe obciążenia danych oraz uruchamianie wyzwalane przez zdarzenia. Co więcej, wraz z wzrostem złożoności tych aranżacji rosną także wymagania wobec usługi, dotyczące obsługi typowych paradygmatów przepływów pracy, w tym gałęzi, pętli i przetwarzania warunkowego.

Wersja 2 umożliwia także migrację istniejących pakietów usług SQL Server Integration Services (SSIS) do chmury w celu przekształcenia SSIS w usługę zarządzaną przez platformę Azure w ramach usługi ADF dzięki nowej funkcji o nazwie „Integration Runtimes” (IR). Wprowadzenie funkcji SSIS IR w wersji 2 umożliwia wykonywanie, monitorowanie i tworzenie pakietów SSIS oraz zarządzanie nimi w chmurze.

### <a name="control-flow-and-scale"></a>Sterowanie przepływem i skalą 
W celu zapewnienia obsługi różnych przepływów i wzorców integracji w nowoczesnym magazynie danych, usługa Data Factory wprowadziła nowy, elastyczny model potoku danych, który nie jest już powiązany z danymi szeregów czasowych. Ta wersja umożliwia modelowanie zdarzeń warunkowych oraz tworzenie rozgałęzień w przepływie sterowania potoku danych, jak również jawne przekazywanie parametrów wewnątrz i pomiędzy tymi przepływami.

Teraz możesz utworzyć dowolny styl przepływu wymagany przez integrację danych oraz uruchamiać go na żądanie i w sposób powtarzalny, zgodnie z harmonogramem. Kilka typowych przepływów dostępnych obecnie, które wcześniej nie były możliwe, to:   

- Przepływ sterowania:
    - Łączenie działań w sekwencje w ramach potoku
    - Rozgałęzianie działań w ramach potoku
    - Parametry
        - Parametry można definiować na poziomie potoku, a argumenty przesyłać w trakcie wywoływania potoku na żądanie lub przy użyciu wyzwalacza
        - Działania mogą wykorzystywać argumenty przekazane potokowi
...
Więcej informacji znajduje się w [samouczku dotyczącym przepływów sterowania](tutorial-control-flow.md).
...
- Reprezentowanie **magazynu danych**, w tym między innymi lokalnego serwera SQL Server, bazy danych Oracle, udziału plików lub konta usługi Azure Blob Storage. Listę obsługiwanych magazynów danych zamieszczono w artykule na temat [działania kopiowania](copy-activity-overview.md).
...
Wyzwalacze reprezentują jednostki przetwarzania, które określają, kiedy należy rozpocząć wykonanie potoku. Dostępne są różne typy wyzwalaczy dla różnych typów zdarzeń. Wersja zapoznawcza obsługuje wyzwalacze harmonogramów zegarowych. 
...
Parametry to pary klucz-wartość w konfiguracji tylko do odczytu.  Parametry są definiowane w potoku, a argumenty dla zdefiniowanych parametrów są przekazywane w trakcie wykonania z kontekstu uruchomienia utworzonego przez wyzwalacz lub potok wykonany ręcznie. Działania w ramach potoku wykorzystują wartości parametrów.
...
Organizacja działań potoku, która obejmuje działania połączone w sekwencję, rozgałęzienia i parametry, które można zdefiniować na poziomie potoku, oraz argumenty przekazywane podczas wywoływania potoku na żądanie lub przy użyciu wyzwalacza. Zawiera również przekazywanie stanów niestandardowych oraz kontenery pętli, to znaczy iteratory For-each.
