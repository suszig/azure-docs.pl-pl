...
| structure| <p>データセットのスキーマ</p><p>を参照してください [データセット構造](#Structure) 詳細については</p>| 番号| 該当なし|
| type| データセットの型| あり| 該当なし|
| typeProperties| <p>選択した種類に対応するプロパティ</p><p>を参照してください [Dataset 型](#Type) サポートされる型とそのプロパティの詳細セクションです。</p>| はい| 該当なし|
| 外部| データセットを Data Factory パイプラインにより明示的に生成するかどうかを指定するブール型のフラグ| いいえ| false|
| availability| <p>処理時間枠またはスライシング モデルを運用環境のデータセットを定義します。</p><p>参照してください [データセットの可用性](#Availability) 詳細についてはトピック</p><p>を参照してください [スケジュールと実行](data-factory-scheduling-and-execution.md) データセット スライシング モデルの詳細については資料</p>| はい| 該当なし
...
| プロパティ| 説明| 必須| 既定値|
| -------- | ----------- | -------- | ------- |
| frequency| データセット スライス生成の時間単位を指定します。<p>**サポートされる頻度**: 分、時間、日、週、月</p>| はい| 該当なし|
| interval| 頻度の乗数を指定<p>"頻度 x 間隔] は、スライスが生成されるどのくらいの頻度を決定します</p>。<p>設定するデータセットを時間単位でスライスする場合は、 **頻度** に **時間**, 、および **間隔** に **1**.</p><p>**注:** 間隔を 15 以上に設定することをお勧め分として頻度を指定する場合</p>| はい| 該当なし|
| style| 間隔の開始/終了にスライスを生成するかどうかを指定します。<ul><li>StartOfInterval</li><li>EndOfInterval</li></ul><p>スライスが 1 か月の最終日に生成される頻度は月に設定されているし、スタイルを EndOfInterval に設定、します。スタイルが StartOfInterval に設定されている場合、スライスは月の最初の日に生成します。</p><p>1 日の過去 1 時間以内に、スライスが生成される頻度が 1 日に設定されているし、スタイルを EndOfInterval に設定、.</p>頻度が 1 時間に設定されているし、スタイルを EndOfInterval に設定、時間の終わりに、スライスが生成されます。たとえば、午後 1 時 ~ 午後 2 時のスライスでは、午後 2 時、スライスが生成されます。</p>| いいえ| EndOfInterval|
...
| retryTimeout| 各再試行のタイムアウト期間。<p>を 10 分に設定すると場合、検証が 10 分以内に完了する必要があります。検証を実行するまで 10 分より長くかかる場合、再試行はタイムアウトします。</p><p>スライスをタイムアウトとしてマークされますが、検証用のすべての試行がタイムアウトした場合。</p>| いいえ| 00:10:00 (10 分)|
