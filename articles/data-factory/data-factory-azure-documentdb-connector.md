...
| writeBatchSize | ドキュメントを作成する DocumentDB サービスを並列要求の数。<p>このプロパティを使用して DocumentDB からデータをコピーするときに問題なくパフォーマンスをチューニングできます。 writeBatchSize を増やすとパフォーマンスが良くなります。DocumentDB に送信される並列要求の数が増えるためです。 調整は回避する必要がありますが、エラー メッセージをスローできます。「Request rate is large"です。</p><p>調整は、さまざまなドキュメント、文書内の用語の数のサイズを含む、インデックス作成ポリシーの対象となるコレクションなどの要因によって決まります。コピー操作の場合、もっとよいコレクションを利用し (S3 など)、最大のスループットを得ることができます (毎秒 2,500 要求単位)。</p> | 整数値 | いいえ |
