<properties 
    pageTitle="Samouczek: tworzenie potoku za pomocą działania kopiowania przy użyciu programu Azure PowerShell" 
    description="Ten samouczek zawiera instrukcje tworzenia potoku usługi Fabryka danych Azure za pomocą działania kopiowania przy użyciu programu Azure PowerShell. " 
...
W przypadku korzystania z programu Azure PowerShell w **wersji starszej niż 1.0** trzeba będzie użyć poleceń cmdlet, które zostały opisane [tutaj][old-cmdlet-reference]. Przed zastosowaniem poleceń cmdlet dla usługi Fabryka danych trzeba będzie również uruchomić następujące polecenia: 
...
1. Uruchom program Azure PowerShell i uruchom następujące polecenia. Nie zamykaj programu Azure PowerShell, zanim nie wykonasz wszystkich instrukcji z tego samouczka. Jeśli go zamkniesz i otworzysz ponownie, musisz uruchomić te polecenia jeszcze raz.
    ...
    3. Uruchom polecenie **Select-AzureSubscription**, aby wybrać subskrypcję, z którą chcesz pracować. Ta subskrypcja powinna być taka sama jak używana w witrynie Azure Portal.
4. Przełącz do trybu AzureResourceManager, gdyż polecenia cmdlet dla usługi Fabryka danych Azure są dostępne w tym trybie: **Switch-AzureMode AzureResourceManager**.
...
Krok | Opis
-----| -----------
[Tworzenie fabryki danych Azure](#create-data-factory) | W tym kroku opisano tworzenie fabryki danych Azure o nazwie **ADFTutorialDataFactoryPSH**. 
...
[Tworzenie wejściowych i wyjściowych zestawów danych](#create-datasets) | W tym kroku zostaną zdefiniowane dwa zestawy danych (**EmpTableFromBlob** i **EmpSQLTable**), które zostaną użyte jako tabele wejściowe i wyjściowe dla **działania kopiowania** w potoku ADFTutorialPipeline, który zostanie utworzony w następnym kroku.
[Tworzenie i uruchamianie potoku](#create-pipeline) | W tym kroku opisano tworzenie potoku o nazwie **ADFTutorialPipeline** w fabryce danych **ADFTutorialDataFactoryPSH**. . Potok będzie obejmował **działanie kopiowania**, które będzie kopiować dane z obiektu blob Azure do tabeli wyjściowej bazy danych Azure.
...
1. Uruchom program Azure PowerShell i uruchom następujące polecenie. Nie zamykaj programu Azure PowerShell, zanim nie wykonasz wszystkich instrukcji z tego samouczka. Jeśli go zamkniesz i otworzysz ponownie, musisz uruchomić te polecenia jeszcze raz.
    ...
    - Uruchom polecenie **Get-AzureSubscription**, aby wyświetlić wszystkie subskrypcje dla tego konta.
    - Uruchom polecenie **Select-AzureSubscription<Name of the subscription>**, aby wybrać subskrypcję, z którą chcesz pracować. Ta subskrypcja powinna być taka sama jak używana w witrynie Azure Portal.
...
- Nazwa fabryki danych Azure musi być globalnie unikatowa. Jeśli wystąpi błąd: **Nazwa fabryki danych „ADFTutorialDataFactoryPSH” jest niedostępna**, zmień nazwę (np. TwojaNazwaADFTutorialDataFactoryPSH). Użyj tej nazwy zamiast ADFTutorialFactoryPSH podczas wykonywania kroków w tym samouczku. Artykuł [Data Factory — Naming Rules](data-factory-naming-rules.md) (Fabryka danych — zasady nazewnictwa) zawiera zasady nazewnictwa artefaktów usługi Fabryka danych.
...
Połączone usługi łączą magazyny danych lub usługi obliczeniowe z fabryką danych Azure. Magazynem danych może być usługa Azure Storage, usługa Azure SQL Database lub lokalna baza danych SQL Server, która zawiera dane wejściowe lub przechowuje dane wyjściowe dla potoku usługi Fabryka danych. Usługą obliczeniową jest usługa, która przetwarza dane wejściowe i generuje dane wyjściowe. 
...
1.  Utwórz plik JSON o nazwie AzureSqlLinkedService.json o następującej zawartości.
    ...
    Zastąp parametry **servername**, **databasename**, **username@servername** oraz **password** nazwą serwera SQL Azure, bazy danych, konta użytkownika i hasłem.
...
2. Użyj narzędzi takich jak [Eksplorator magazynu Azure](https://azurestorageexplorer.codeplex.com/) do utworzenia kontenera **adftutorial** i przekazania pliku **emp.txt** do kontenera.
    ...
    ![Eksplorator magazynu Azure][image-data-factory-get-started-storage-explorer]
3. Poniższy skrypt SQL umożliwia utworzenie tabeli **emp** w bazie danych SQL Azure.  
    ...
    Jeśli masz zainstalowany na komputerze program Visual Studio 2013, w witrynie Azure Portal ([http://portal.azure.com](http://portal.sazure.com)) kliknij centrum **PRZEGLĄDAJ** po lewej stronie, kliknij pozycję **Serwery SQL**, wybierz bazę danych i kliknij przycisk **Otwórz w programie Visual Studio** na pasku narzędzi, aby połączyć się z serwerem SQL Azure i uruchomić skrypt. Jeśli klient nie ma dostępu do serwera SQL Azure, musisz skonfigurować zaporę serwera SQL Azure w taki sposób, aby dostęp z Twojego komputera (adresu IP) był dozwolony. We wcześniejszej części tego artykułu opisano kroki konfigurowania zapory dla serwera SQL Azure.
...
W tej części opisano tworzenie tabeli wyjściowej o nazwie **EmpSQLTable** wskazującej tabelę SQL (**emp**) w bazie danych SQL Azure, która jest reprezentowana przez połączoną usługę **AzureSqlLinkedService**. Potok kopiuje dane z wejściowego obiektu blob do tabeli **emp**. 
...
1.  Utwórz plik JSON o nazwie **ADFTutorialPipeline.json** w folderze **C:\ADFGetStartedPSH** o następującej zawartości: 
    ...
    W powyższym przykładzie będą 24 wycinki danych, gdyż poszczególne wycinki danych są generowane co godzinę.
...
W tym kroku opisano użycie programu Azure PowerShell do monitorowania tego, co dzieje się w fabryce danych Azure.
...
2.  Uruchom polecenie **Get-AzureRmDataFactorySlice**, aby uzyskać szczegółowe informacje na temat wszystkich wycinków elementu **EmpSQLTable**, który stanowi tabelę wyjściową potoku.  
    ...
    Zastąp rok, miesiąc i datę w parametrze **StartDateTime** bieżącym rokiem, miesiącem i datą. Zachowaj zgodność z wartością właściwości **Start** w kodzie JSON potoku. 
...
3.  Uruchom polecenie **Get-AzureRmDataFactoryRun**, aby uzyskać szczegółowe informacje o uruchomieniach działania dla **określonego** wycinka. Zmień wartość parametru **StartDateTime** w taki sposób, aby była zgodna z czasem **rozpoczęcia** wycinka z danych wyjściowych powyżej. Wartość **StartDateTime** musi być w [formacie ISO](http://en.wikipedia.org/wiki/ISO_8601). Na przykład: 2014-03-03T22:00:00Z.
