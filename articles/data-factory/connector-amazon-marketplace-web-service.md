...
|:--- |:--- |:--- |
| type | Właściwość type musi mieć ustawioną: **AmazonMWS** | Tak |
| punkt końcowy | Punkt końcowy serwera Amazon MWS, (czyli mws.amazonservices.com)  | Tak |
| marketplaceID | Identyfikator witryny Marketplace Amazon chcesz pobrać dane. Pobieranie danych z wielu identyfikatorów Marketplace, rozdziel je przecinkami (`,`). (to znaczy A2EUQ1WTGCTBG2)  | Tak |
...
| accessKeyId | Dostęp do klucza identyfikator umożliwiający dostęp do danych.  | Tak |
...
| Właściwość | Opis | Wymagane |
|:--- |:--- |:--- |
| type | Musi mieć ustawioną właściwość type źródła działania kopiowania: **AmazonMWSSource** | Tak |
| query | Użyj niestandardowych zapytania SQL można odczytać danych. Na przykład: `"SELECT * FROM Orders where  Amazon_Order_Id = 'xx'"`. | Tak |
