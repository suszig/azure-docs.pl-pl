...
4.  Azure Blob ストレージのファイルに Pig クエリのコピーは、上記の手順 2. で構成されます。 データをホストするためのリンクされたサービスがこのクエリ ファイルをホストしているものと異なる場合は、別の Azure ストレージのリンクされたサービスを作成し、アクティビティの構成のトークンを参照します。 使用 **scriptPath * * pig スクリプト ファイルへのパスを指定し、* * scriptLinkedService** をスクリプト ファイルを含む Azure のストレージを指定します。
