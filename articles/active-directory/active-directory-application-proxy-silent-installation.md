...
`$SecureToken = $Token |AsPlainText Convertto-securestring-Force`
3. SecureToken が上記で作成したトークンの名前であり、tenantID がテナントの GUID は次の Windows PowerShell コマンドを実行します。 <br>
`RegisterConnector.ps1 - modulePath"C:\Program files \microsoft AAD アプリケーション プロキシ Connector\Modules\"moduleName"AppProxyPSModule"Authenticationmode トークン - トークン $SecureToken TenantId < テナント GUID >`
