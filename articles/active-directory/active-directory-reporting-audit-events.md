...
Azure Active Directory 監査レポートを利用すると、Azure Active Directory で発生した特権アクションを識別できます。 特権アクションとしては、昇格の変更 (例: ロールの作成、パスワードのリセット)、ポリシー構成の変更 (例: パスワード ポリシー)、ディレクトリ構成の変更 (例: ドメインのフェデレーション設定の変更) などがあります。 レポートでは、イベント名、アクションを実行したアクター、変更によって影響を受けた対象リソース、日時 (UTC) に関する監査レコードが提供されます。 お客様は、使用して、Azure Active Directory に対する監査イベントのリストを取得すること、 [Azure 管理ポータル](https://manage.windowsazure.com/), 」の説明に従って、 [アクセスおよび使用状況レポートの表示](active-directory-view-access-usage-reports.md)します。
 ...
 **ユーザー イベント**|
 ...
 ユーザーの更新| ディレクトリでユーザーが更新されました。[以下を参照](#quotupdate-userquot-attributes) 更新可能な属性です。
 ...
 **アプリケーション イベント**|
 ...
 委任エントリの追加| 作成された、 [OAuth2PermissionGrant](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#OAuth2PermissionGrantEntity) ディレクトリにします。
 委任エントリの設定| 更新、 [OAuth2PermissionGrant](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#OAuth2PermissionGrantEntity) ディレクトリにします。
 委任エントリの削除| 削除、 [OAuth2PermissionGrant](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#OAuth2PermissionGrantEntity) ディレクトリにします。
 **ロール イベント**|
 ...
 **ディレクトリ イベント**|
 ...
 会社情報の設定| 会社レベルの情報が更新されました。参照してください、 [Get-msolcompanyinformation](https://msdn.microsoft.com/library/azure/dn194126.aspx) PowerShell コマンドレットの詳細について。



## 監査レポートの保持

Azure AD の監査レポートのイベントは、180 日間保持されます。 レポートの保存に関する詳細については、次を参照してください。 [Azure Active Directory レポートの保持ポリシー](active-directory-reporting-retention.md)します。

監査イベントを長期間保存する目的のユーザーの場合は、Reporting API を使用して監査イベントを定期的に別のデータ ストアにプルできます。 参照してください [Reporting API の概要](active-directory-reporting-api-getting-started.md) 詳細です。
