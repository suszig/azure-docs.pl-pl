...
Azure AD に対してアプリを記述するアプリケーション オブジェクト (マルチ テナント サービス) し、次のいずれかを含めることがあります: (*注*: これは、排他的なリストではありません)。
...
![アプリケーションのオブジェクトおよびサービスの Azure AD インスタンスに既存のプリンシパルを示す図。][apps_service_principals_directory]
...
* 現在、このような情報の中にはプログラムによって公開されていないものがあります。  次の情報は UI でのみ使用できます。
    * 要求変換ルール
    * 属性マッピング (ユーザーのプロビジョニング)
* サービス プリンシパル オブジェクトおよびアプリケーション オブジェクトの詳細については、Azure AD Graph REST API のリファレンス ドキュメントを参照してください。  *ヒント*: Azure AD Graph API のドキュメントでは、スキーマ参照に最も近いものを Azure AD は現在使用できるのです。  
...
* アプリを追加、 [Azure Active Directory アプリ ギャラリー](http://azure.microsoft.com/updates/azure-active-directory-over-1000-apps/)
* サインアップ/Azure Active Directory に統合されているサードパーティのアプリにサインインします (例: [Smartsheet](https://app.smartsheet.com/b/home) または [DocuSign](https://www.docusign.net/member/MemberLogin.aspx))
...
* などの Microsoft オンライン サービスにサインアップ サインイン [Office 365](http://products.office.com/)
...
* Azure 管理ポータルの「」を参照で開発するアプリを追加します https://msdn.microsoft.com/library/azure/dn132599.aspx。
...
* 使用するアプリを追加、 [Azure AD アプリケーション プロキシ](https://msdn.microsoft.com/library/azure/dn768219.aspx)
...
ディレクトリ内のすべてのユーザーは、開発しているアプリケーションを追加する権限を持ち、共有するアプリケーションまたは組織データにアクセスできるアプリケーションを決定できます。  *アプリケーションにサインアップ/サインインにユーザーに注意してくださいされ、サービスのプリンシパルが作成されるアクセス許可を付与するが発生します。*
...
**注:** *数か月もの既定の構成を使用して Microsoft 自体が運用されています。*
...
![統合アプリケーションの設定を構成するための UI のスクリーン ショット][app_settings]
...
* 開発者: [にアプリケーションを AAD と統合する方法について説明](https://msdn.microsoft.com/library/azure/dn151122.aspx)
* 開発者: [Github で Azure Active Directory と統合されたアプリのサンプル コードの確認](https://github.com/AzureADSamples)
* 開発者および IT プロフェッショナル: [ドキュメントは、Azure Active Directory Graph API を REST API 参照](https://msdn.microsoft.com/library/azure/hh974478.aspx)
* IT プロフェッショナル: [アプリ ギャラリーから Azure Active Directory に事前に統合されたアプリケーションを使用する方法について](https://msdn.microsoft.com/library/azure/dn308590.aspx)
* IT プロフェッショナル: [特定の事前統合アプリを構成するためのチュートリアルの検索](https://msdn.microsoft.com/library/azure/dn893637.aspx)
* IT プロフェッショナル: [Azure Active Directory アプリケーション プロキシを使用して、アプリを発行する方法について](https://msdn.microsoft.com/library/azure/dn768219.aspx)
