...
>[AZURE.NOTE] AD FS で利用状況分析を使用するために、AD FS の監査が有効になっていることを確認する必要があります。 詳細については、次を参照してください。 [AD FS の監査を有効に](active-directory-aadconnect-health-operations.md#enable-auditing-for-ad-fs)します。 
...
|  | 認証方法 | このオプションは、認証に使用された認証方法に基づいて、要求の合計をグループ化します。 このグループ化は、認証に使用される共通の認証方法を把握するのに役立ちます。 可能な認証方法を次に示します <ol> <li>Windows 統合認証 (Windows)</li> <li>フォーム ベース認証 (フォーム)</li> <li>SSO (Single Sign On)</li> <li>X509 証明書の認証 (証明書)</li> <br>フェデレーション サーバーが SSO Cookie で要求を受信する場合、要求が SSO (Single Sign On) としてカウントされているに注意してください。 このような場合、Cookie が有効であれば、ユーザーは資格情報の提供を要求されずに、シームレスにアプリケーションにアクセスできます。 これは、フェデレーション サーバーによって保護される証明書利用者が複数ある場合でも共通です。 |
...
| 失敗した要求の合計数: フェデレーション サービスによって処理され、失敗した要求の合計数。 <br> (このメトリックは Windows Server 2012 R2 の AD FS で利用可能なのみ)| エラーの種類 | あらかじめ定義されたエラーの種類に基づいて、エラーの数を表示します。 このグループ化は、一般的なエラーの種類を把握するのに役立ちます。 <ul><li>ユーザー名またはパスワードが正しくありません。 正しくないユーザー名またはパスワードによるエラー。</li> <li>「エクストラネット ロックアウト」: エクストラネットからロックアウトされたユーザーから受信した要求によるエラー </li><li> 「パスワード期限切れ」: 期限切れのパスワードを使用してログオンしているユーザーのため失敗します。</li><li>「アカウントを無効になっています」: 無効なアカウントでログオンしているユーザーのため失敗します。</li><li>[デバイス認証]: ユーザーのデバイス認証を使用して認証が失敗によるエラー。</li><li>[ユーザー証明書の認証]: 証明書が無効のための認証に失敗しているユーザーによるエラー。</li><li>"MFA": 多要素認証を使用して認証に失敗しているユーザーのため失敗します。</li><li>「その他の資格情報」:「発行承認」: 認証の失敗によるエラー。</li><li>「発行委任」: 発行委任エラーにより失敗します。</li><li>「トークンの承認」: サードからトークンを拒否する ADFS によるエラー パーティ Id プロバイダーです。</li><li>"Protocol": プロトコルのエラーによる障害が発生します。</li><li>「不明」とします。 すべてをキャッチします。 その他のエラーがすべて定義済みのカテゴリに分類されません。</li> |
