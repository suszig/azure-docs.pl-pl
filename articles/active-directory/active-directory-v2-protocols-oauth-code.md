<properties
    pageTitle ="アプリ モデル v2.0 OAuth プロトコル |Microsoft Azure]
    description =「web アプリケーションを構築、OAuth 2.0 認証プロトコルの Azure AD の実装を使用します」。
...
承認コード フローは、ユーザーに指示するクライアントで始まり、 `承認/` エンドポイント。 この要求で、クライアントは、ユーザーから取得する必要のあるアクセス許可を指定します。
...
| scope| 必須| スコープのスペース区切りリスト。v2.0 エンドポイントには、リソースとその (要求されているリソースへの) アクセス許可の両方が 1 つのスコープ値で指定されます。スコープは、形式をとる `< アプリケーションの識別子 URI >/< スコープ値 >`します。上記の例では、Azure AD Graph API のアプリケーションの識別子が使用される、 `https://graph.windows.net`, 、2 つの権限が要求される: `directory.read` と `directory.write`します。スコープの詳細についてを参照してください、 [アプリ モデル v2.0 スコープ参照](active-directory-v2-scopes.md)します。|
...
引き換えることができる、authorization_code を取得し、ユーザーが権限を付与されている、 `コード` の `access_token` 送信することで、目的のリソースを `POST` への要求、 `トークン/` エンドポイント。
...
これで正常に取得した、 `access_token`, 、Web Api への要求トークンを使用するには以下のように、 `承認` ヘッダー。
...
アクセス トークンは有効期間が短く、期限が切れた後もリソースにアクセスし続けるためにはトークンを更新する必要があります。 これを行う別の操作を送信することで `POST` への要求、 `トークン/` を提供するこのときは、エンドポイント、 `refresh_token` の代わりに、 `コード`:
