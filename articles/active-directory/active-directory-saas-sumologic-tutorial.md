...
3.  **アプリケーション URL の構成** ] ページで、 **SumoLogic サインイン URL** ] ボックスに、次のパターンを使用して URL を入力"* https://\<tenant-name\>します。SumoLogic.com*"、クリックして **次**します。
...
9.  **[Configure SAML 2.0]** ダイアログで、次の手順に従います。
    ...
    6.  base-64 でエンコードされた証明書をメモ帳で開き、その内容をクリップボードにコピーして、証明書全体を **[X.509 証明書]** テキストボックスに貼り付けます。
