...
Azure AD Connect により、オンプレミスのコンピューターをクラウド内のデバイス オブジェクトとしてプロビジョニングできます。 Azure AD Connect をデプロイするには、Azure AD Connect によるディレクトリのハイブリッド管理の有効化に関するページを参照してください。
 ...
 - 実行している場合、 [Azure AD Connect のカスタム インストール](active-directory-aadconnect-get-started-custom.md) (高速インストールではありません)、手順を実行する必要があります **内部設置型 Active Directory にサービス接続ポイント (SCP) を作成**, 、以下で説明します。
 - (たとえば、Active Directory フェデレーション サービス (AD FS) を展開する前にある) 場合は、Azure AD Connect をインストールする前に Azure AD とフェデレーションの構成がある場合に従う必要は、 **構成 AD FS 要求規則** 以下の手順です。
...
>[AZURE.NOTE]
 置換 [*コネクタのアカウント名*] AD コネクタ アカウントとして使用するドメイン アカウントを使用します。

>[AZURE.NOTE]
Get-credential ポップアップが表示されたら、入力された資格情報のユーザー名の形式にする必要がある *user@example.com*
...
3.  右クリック **グループ ポリシー オブジェクト** 選択 **新規**します。 グループ ポリシー オブジェクトに Automatic Azure AD Join などの名前を付けます。 Click **OK**.
4.  新しいグループ ポリシー オブジェクトを右クリックし、[ **編集**します。
5.  移動 **コンピューターの構成** > **ポリシー** > **管理用テンプレート** > **Windows コンポーネントの** > **ワークプ レース ジョイン**します。
6.  右クリック **workplace join クライアント コンピューターに自動的に** し、[ **編集**します。
7.  選択、 **有効** オプション ボタンをクリックし、 **適用**します。 Click **OK**.
...
>[AZURE.NOTE]
このグループ ポリシー テンプレートの名前は、Windows 10 で変更されました。 Windows 10 コンピューターから、グループ ポリシー ツールを実行している場合は、ポリシーとして表示されます。 <br>
**ドメインに参加しているコンピューターのデバイスとして登録します。**
ポリシーは、次の場所の下には。<br>
