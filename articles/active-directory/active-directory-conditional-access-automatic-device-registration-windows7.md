<properties
    pageTitle="# Windows 7 のドメインに参加して devices| の自動デバイス登録を構成します。Microsoft Azure"
    description="Windows 7 のドメインを構成する手順には、Azure AD に自動的に登録するデバイスが参加しています。 デバイスの登録ソフトウェア パッケージを Windows 7 のドメインを展開する手順には、ソフトウェア配布システムなど、System Center Configuration Manager を使用してデバイスが参加しています。"
...
Windows 7 のデバイスの登録として利用可能な [ダウンロード可能な MSI パッケージ](https://connect.microsoft.com/site1164)します。 このパッケージは、Active Directory ドメインに参加している Windows 7 コンピューターにインストールする必要があります。 System Center Configuration Manager などのソフトウェア配布システムを使用して、パッケージをデプロイしてください。 MSI パッケージは、/quiet パラメーターを使用する、標準のサイレント インストール オプションをサポートしています。
ソフトウェア パッケージがダウンロード、 [Microsoft Connect web サイト](https://connect.microsoft.com/site1164)します。 ここで、Windows 7 用の社内参加を選択し、ダウンロードします。
...
スケジュールされたタスクは、タスク スケジューラ ライブラリにあります **Microsoft** > **ワークプ レース ジョイン**します。
...
1. Azure AD で、デバイス オブジェクトと証明書が作成されます。 このオブジェクトは、user@device を表します。
1. 社内参加の証明書が、コンピューターに格納されます。

## Windows 7 ドメイン参加済みデバイスの登録解除

次の手順を実行して、ドメインに参加して Windows 7 デバイスを登録解除を選択することがあります。
ソフトウェア配布システムなど、System Center Configuration Manager を使用して、Windows 7 のドメインに参加したデバイスから Workplace Join のソフトウェア パッケージをアンインストールします。
