...
対応するリンクをクリックして、 **アクション** と **アクションではない** ロール定義のプロパティです。  **アクション** プロパティは、Azure リソースに対して許可されるアクションを指定します。 アクションの文字列にワイルドカード文字を使用できます。  **アクションではない** 役割定義のプロパティが許可されているアクションから除外しなければならないアクションを指定します。
...
| [Web Plan の共同作業者](#web-plan-contributor) | Web プランを管理できます |
