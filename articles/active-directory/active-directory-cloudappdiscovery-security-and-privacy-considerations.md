...
Cloud App Discovery] ブレードで、エージェントが監視するクラウド アプリの一覧を編集する [Azure ポータル](https://portal.azure.com), [ **設定**]-> [**データの収集**]-> [**アプリ コレクション リスト**します。 詳細については、次を参照してください [で Cloud App Discovery 概要。](http://social.technet.microsoft.com/wiki/contents/articles/30962.getting-started-with-cloud-app-discovery.aspx)
<br>
**情報カテゴリ**: ユーザー情報 <br>
**説明**: <br>
...
> [AZURE.NOTE] 上記の HTTP 情報は、すべての暗号化されていない接続に対して収集されます。
 TLS 接続では、ポータルで「詳細な検査」設定をオンにしているときにのみ、この情報がキャプチャされます。 設定は、既定では「オン」です。
詳細については、以下を参照してくださいと [で Cloud App Discovery 概要](http://social.technet.microsoft.com/wiki/contents/articles/30962.getting-started-with-cloud-app-discovery.aspx)
...
ドライバー コンポーネントでは、暗号化されたトラフィックをインターセプトし、暗号化されたストリームに挿入します。 詳細については、 **暗号化された接続 (詳細な検査) からのデータをインターセプトする** 以下のセクションです。
...
- **データの収集**: 管理者がアプリケーションまたは検出データを取得するアプリケーションのカテゴリを指定できます。

- **詳細な検査**: 管理者は、エージェントが SSL/TLS 接続の HTTP トラフィックを収集するかどうかの指定を選択できます (aka **"詳細な検査"**)。 詳細については、次のセクションで説明します。

- **同意オプション**: 管理者が Cloud App Discovery ポータルを使用して、エージェントがデータ コレクションのユーザーに通知するかどうかを選択して、ユーザー データを収集するエージェントの開始前に同意すると、ユーザーが必要かどうか。

Cloud App Discovery エンドポイント エージェントのみで説明した情報が収集、 **エージェントによって収集されたデータ** 前のセクションです。


### 暗号化された接続からのデータのインターセプト (詳細な検査) 
既に説明したように、管理者は、エージェントを構成して、暗号化された接続のデータを監視することができます ("詳細な検査")。 TLS ([トランスポート層セキュリティ](https://msdn.microsoft.com/library/windows/desktop/aa380516%28v=vs.85%29.aspx)) 現在インターネット上で使用されている最も一般的なプロトコルの 1 つです。 TLS で通信を暗号化することによって、クライアントが Web サーバーとのセキュリティで保護されたプライベート通信チャネルを確立できます。TLS は、認証の資格情報を渡すために不可欠な保護機能を提供するほか、機密情報の漏えいを防ぎます。
...
> [AZURE.NOTE] 詳細については、次を参照してください [で Cloud App Discovery 概要。](http://social.technet.microsoft.com/wiki/contents/articles/30962.getting-started-with-cloud-app-discovery.aspx)
