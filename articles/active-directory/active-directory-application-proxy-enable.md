...
- 経路上にファイアウォールが存在する場合、コネクタからアプリケーション プロキシに送信される HTTPS (TCP) 要求を許可するようにファイアウォールを開放する必要があります。 コネクタは、上位ドメイン msappproxy.net に属しているサブドメインと併せて、これらのポートを使用します。 必ずを開いて **すべて** 、以下のポートを **送信** トラフィック。
...
3. をクリックして **構成**, 、アプリケーション プロキシまでスクロールし、このディレクトリに対してアプリケーション プロキシ サービスを有効にする切り替え **有効**します。

    ![アプリケーション プロキシを有効にする](./media/active-directory-application-proxy-enable/app_proxy_enable.png) <p>
4. クリックして **を今すぐダウンロード** 、画面の下部にあります。 この操作により、ダウンロード ページが表示されます。 およびライセンス条項に同意して] をクリックして **ダウンロード** アプリケーション プロキシ コネクタの Windows インストーラー ファイル (.exe) を保存します。
...
<p>- Azure AD 全体管理者の資格情報を提供します。
<p>- コネクタを登録する管理者がなどで、アプリケーション プロキシ サービスを有効した同じディレクトリに、テナント ドメインが contoso.com の場合は、管理者でなければなりません admin@contoso.com またはそのドメイン内の他の任意のエイリアスを確認します。 また、Azure AD テナントのグローバル管理者である必要があります。 グローバル管理者のテナントは、Microsoft Azure の資格情報とは異なる場合があります。
<p>- IE セキュリティ強化の構成に設定されている場合は、 **に** Azure AD コネクタをインストールするサーバーで、登録画面がブロックされています。 その場合は、エラー メッセージに示された指示に従って、アクセスを許可します。 Internet Explorer セキュリティ強化の構成が無効になっていることを確認します。
<p>- コネクタの登録が成功しなかった場合は、次を参照してください。 [Troubleshoot Application Proxy](active-directory-application-proxy-troubleshoot.md)します。
