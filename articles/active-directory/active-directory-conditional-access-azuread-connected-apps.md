...
既定では、このポリシーは、アプリケーションへのアクセスを持つすべてのユーザーに適用されます。 ただし、指定したセキュリティ グループのメンバーであるユーザーのみにポリシーを制限することもできます。  **グループの追加** ボタンは、アクセス ルールが適用されるグループの選択] ダイアログから 1 つまたは複数のグループの選択に使用します。 このダイアログ ボックスを使用して、選択したグループを削除することもできます。 グループに適用する規則を選択すると、アクセス規則は、指定したセキュリティ グループに属するユーザーにのみ課せられます。
...
* **多要素認証を要求する**: このオプションを使用してアクセス規則が適用されたユーザーにポリシーを適用するアプリケーションにアクセスする前に multi-factor authentication を完了するために必要にあります。

* **作業ではない場合、多要素認証を要求する**: このオプションが信頼されている IP から送信するユーザーは多要素認証を実行する求められますされません。 Multi-Factor Authentication の設定ページでは、信頼できる IP の範囲を構成できます。

* **職場以外のアクセスをブロック**: このオプションを使用して、信頼できる IP から読み込まれていないいるユーザーはブロックされます。 Multi-Factor Authentication の設定ページでは、信頼できる IP の範囲を構成できます。
