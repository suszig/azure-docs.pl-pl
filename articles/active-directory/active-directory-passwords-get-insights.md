...
- [* * パスワード管理レポート * * を表示する方法](#how-to-view-password-management-reports)
- [* * ビュー パスワード リセットでは、組織 * * 登録アクティビティ](#view-password-reset-registration-activity)
- [* * ビュー パスワード リセット アクティビティで、組織 * *](#view-password-reset-activity)
...
パスワード リセット登録アクティビティ レポートでは、組織で実行されたすべてのパスワード リセットの登録が表示されます。 パスワードで認証情報が正常に登録されたすべてのユーザーのリセット登録ポータルに対して、パスワード リセットの登録がこのレポートに表示される ([http://aka.ms/ssprsetup](http://aka.ms/ssprsetup))します。
...
次の表には、各列で使用できるさまざまな値について説明します。
 ...
 分割| 使用できる値とその意味
---|---
 登録済みデータ| **連絡用電子メール** – 使用するユーザーの連絡用電子メールまたは認証用電子メールを認証する<p><p>**会社電話**– の認証に使用するユーザーの会社電話<p>**携帯電話** -使用するユーザーの携帯電話または認証用電話を認証する<p>**セキュリティの質問** – ユーザーの認証にセキュリティの質問を使用する<p>**上の任意の組み合わせ (連絡用電子メール + 携帯電話など)** – 2 ゲート ポリシーが指定されている場合、ユーザーが使用する 2 つの方法が表示、パスワード リセット要求を認証します。
...
次の表には、各列で使用できるさまざまな値について説明します。
 ...
 分割| 使用できる値とその意味
---|---
 使用された方法| **連絡用電子メール** – 使用するユーザーの連絡用電子メールまたは認証用電子メールを認証する<p>**会社電話** – の認証に使用するユーザーの会社電話<p>**携帯電話** – 使用するユーザーの携帯電話または認証用電話を認証する<p>**セキュリティの質問** – ユーザーの認証にセキュリティの質問を使用する<p>**上の任意の組み合わせ (連絡用電子メール + 携帯電話など)** – 2 ゲート ポリシーが指定されている場合、ユーザーが使用する 2 つの方法が表示、パスワード リセット要求を認証します。
 結果| **破棄** – ユーザー パスワード リセットを開始しはしましたが途中で完了しなくても<p>**ブロック** – パスワード リセット ページを使用するパスワードをリセットしようとして原因を使用するユーザーのアカウントがブロックされたまたは 1 つのパスワード リセット ゲートを 24 時間以内に何度も<p>**Cancelled** – ユーザーはパスワードのリセットを開始しましたが、途中のセッションをキャンセルするキャンセル ボタンをクリックし、 <p>**管理者のアクセス** – ユーザー セッション中に解決できない問題があった、ユーザーは完了せずに、[管理者に連絡"リンクをクリックしたため、パスワード リセット フロー<p>**失敗** – ユーザーは、(例: ライセンスのない、見つからない、認証情報を、管理されているパスワードがオンプレミスでが書き戻しがオフ) 機能を使用して、ユーザーが構成されていなかったために、可能性の高いパスワードをリセットできませんでした。<p>**Succeeded** – パスワード リセットは成功しました。
 詳細| 次の表を参照してください。
