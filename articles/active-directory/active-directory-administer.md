...
新しい Azure サブスクリプションは、既存の Office 365 または Microsoft Intune サブスクリプションへのサインインを認証する同じディレクトリに関連付けることができます。 職場または学校アカウントを使用して、Azure 管理ポータルにサインインします。 管理ポータルに、そのアカウントのサブスクリプションが見つからなかったことを示すメッセージが表示されます。 **[Azure にサインアップ]** を選択すると、ディレクトリをポータル内での管理に使用できるようになります。 詳細については、次を参照してください。 [Azure での Office 365 サブスクリプションのディレクトリを管理](active-directory-how-subscriptions-associated-directory.md#manage-the-directory-for-your-office-365-subscription-in-azure)します。
...
ディレクトリの表示名を変更するには、ポータルでディレクトリをクリックし、**[構成]** をクリックします。 このトピックで後述するように、新しいディレクトリを追加したり、不要になったディレクトリを削除したりできます。 サブスクリプションを別のディレクトリに関連付けるには、左側のナビゲーションで **[設定]** 拡張機能をクリックし、**[サブスクリプション]** ページの下部にある **[ディレクトリの編集]** をクリックします。 既定の *.onmicrosoft.com ドメインではなく、登録済みの DNS 名を使用してカスタム ドメインを作成することもできます。これは、SharePoint Online などのサービスを利用している場合にお勧めします。

## ディレクトリ データを管理する方法

1 つ以上の Microsoft クラウド サービス サブスクリプションの管理者は、Azure 管理ポータル、Microsoft Intune アカウント ポータル、または Office 365 管理センターを使用して、組織のディレクトリ データを管理できます。 ダウンロードして実行することも [Microsoft Azure Active Directory 用 Windows PowerShell モジュール](https://msdn.microsoft.com/library/azure/jj151815.aspx) コマンドレットは Azure AD に格納されたデータを管理しやすくします。
...
次の図に示すように、Azure 管理ポータル、Office 365 管理センター、Microsoft Intune アカウント ポータル、Azure AD コマンドレットはすべて、組織のディレクトリに関連付けられている Azure AD の 1 つの共有インスタンスとの間で読み取りと書き込みを行います。 このようにして、ポータル (またはコマンドレット) はディレクトリ データを取得または変更するフロントエンド インターフェイスとして機能します。
...
Azure AD ディレクトリは、Azure 管理ポータルで追加できます。 左側で **[Active Directory]** 拡張機能を選択し、**[追加]** をクリックします。
...
- [Windows PowerShell を使用して Azure AD を管理します。](https://msdn.microsoft.com/library/azure/jj151815.aspx)
- [Azure AD で管理者ロールの割り当てください。](active-directory-assign-admin-roles.md)
