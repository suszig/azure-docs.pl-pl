<properties
    pageTitle="Azure Active Directory ハイブリッド identity 設計に関する考慮事項には、ディレクトリ同期の要件を決定する |Microsoft Azure"
...
会社の同期要件について考えがまとまったら、これらのディレクトリ サービスを使用するアプリケーションを評価する必要があります。 この評価は、これらのアプリケーションをクラウドに統合する技術的な要件を定義する必要があります。 次のチェック項目に回答してください。
...
- 同期するフォレストについて、正しいアクセス許可を持つアカウントはありますか。
 - 会社では、この質問の回答を認識しない場合、は、「パスワード同期のアクセス許可」記事セクションを確認します。 [Azure Active Directory 同期サービスをインストールする](https://msdn.microsoft.com/library/azure/dn757602.aspx#BKMK_CreateAnADAccountForTheSyncService) これらのアクセス許可を持つアカウントが既にある場合、または 1 つを作成する必要がある場合を判断します。
- 複数フォレストの同期がある場合、同期サーバーから各フォレストにアクセスできますか。

>[AZURE.NOTE]
回答をメモし、その背後にある論理的根拠を理解してください。 [インシデント対応要件を決定する](active-directory-hybrid-identity-design-considerations-incident-response-requirements.md) が利用可能なオプションの上に変わります。 チェック項目に答えることで、ビジネス ニーズに最適な選択肢が見つかります。
...
[設計に関する考慮事項の概要](active-directory-hybrid-identity-design-considerations-directory-overview.md)
