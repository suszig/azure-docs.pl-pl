...
これは既定の規則のため、規則を開くと警告が表示されます。 いずれかを行う必要がない [既定のルールへの変更](active-directory-aadconnectsync-best-practices-changing-default-configuration.md) ため、開発者の意図とは何が求められています。 この場合、必要なのは規則の表示のみなので、**[いいえ]** を選択してください。
