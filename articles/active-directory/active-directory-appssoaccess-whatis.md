...
*   **フェデレーション シングル サインオン** アプリケーションが独自のパスワードを要求する代わりにユーザー認証用に Azure AD にリダイレクトすることができます。 これは、SAML 2.0、WS-Federation、OpenID Connect などのプロトコルをサポートするアプリケーションでサポートされていて、シングル サインオンのモードのうちで最も多機能です。

*   **シングル サインオンをパスワードに基づく** および web ブラウザーの拡張機能またはモバイル アプリを使用した再生を利用してアプリケーション パスワードの記憶域をセキュリティで保護します。 この場合、アプリケーションによって提供される既存のサインイン プロセスが利用されますが、管理者がパスワードを管理できる一方で、ユーザーがパスワードを知っている必要はありません。

*   **既存シングル サインオン** すべて既存シングル サインオン アプリケーションに対して設定されているが、Office 365 または Azure AD アクセス パネル ポータルにリンクするこれらのアプリケーションを活用する Azure AD を有効にし、アプリケーションが起動されたとき、Azure AD でのレポートの追加ができます。
...
関連項目: [でのシングル サインオン フェデレーション用の証明書を管理します。](active-directory-sso-certs.md)
...
1.  **管理者資格情報を管理する** – 管理者が作成し、アプリケーションの資格情報を管理してそれらの資格情報をユーザーまたはアプリケーションへのアクセスが必要なグループに割り当てます。 この場合、エンド ユーザーが資格情報を知る必要がない一方で、アクセス パネルまたは指定されたリンクを使用してクリックするだけで、アプリケーションへのシングル サインオン アクセスを実現できます。 これにより、管理者による資格情報のライフ サイクル管理と、エンド ユーザーがアプリケーション固有のパスワードを覚えたり管理したりする必要がないという利便性の両方が実現されます。 自動化されたサインイン プロセスの実行中、資格情報はエンド ユーザーに対して難読化されますが、技術的にはユーザーが Web デバッグ ツールを使用して識別することができます。ユーザーおよび管理者は、資格情報がユーザーによって直接指定された場合と同じく、同じセキュリティ ポリシーに従う必要があります。 管理者が提供する資格情報は、ソーシャル メディアやドキュメント共有アプリケーションなど、多くのユーザー間で共有されるアカウントのアクセスを提供するときに非常に便利です。

2.  **ユーザーが資格情報を管理する** – 管理者がアプリケーションをエンドユーザーまたはグループに割り当てるし、エンドユーザーにできるよう、アクセス パネルで初めてアプリケーションにアクセスするのには、直接自分の資格情報を入力してください。 この場合、アプリケーションにアクセスするたびにアプリケーション固有のパスワードを毎回入力する必要がないため、エンド ユーザーにとっての利便性が得られます。 このユース ケースは、資格情報の管理を実現する足がかりとしても使用できます。管理者は、エンド ユーザーのアプリケーション アクセス エクスペリエンスを変えることなく、将来アプリケーションの新しい資格情報を設定できます。
...
- Internet Explorer 8、9、および 10 - Windows 7 以降 (「」も参照 [IE 拡張機能の展開ガイド](active-directory-saas-ie-group-policy.md))
- Chrome - Windows 7 以降、MacOS X 以降
- Firefox 26.0 以降 - Windows XP SP2 以降、Mac os X 10.6 以降

**注:** SSO のパスワードに基づく拡張機能が使用可能なる Windows 10 でのエッジのエッジのサポートされるブラウザー拡張機能です。
...
詳細については、次を参照してください [と SaaS アプリケーションへのプロビジョニング解除のユーザー プロビジョニングの自動化。](active-directory-saas-app-provisioning.md)
...
###Azure AD アプリケーション ギャラリーの使用
 ...
 [Azure Active Directory アプリケーション ギャラリー](http://azure.microsoft.com/marketplace/active-directory/all/) と Azure Active Directory のシングル サインオンの形式をサポートすることがわかっているアプリケーションの一覧を示します。

![][1]

サポートされている機能によってアプリを検索するためのヒントを次に示します。

*   Azure AD は、自動プロビジョニングおよびプロビジョニング解除のすべての「おすすめ」アプリをサポートしています、 [Azure Active Directory アプリケーション ギャラリー](http://azure.microsoft.com/marketplace/active-directory/all/)します。

*   シングル サインオン SAML、Ws-federation などのプロトコルを使用してフェデレーションをサポートするフェデレーション アプリケーションの一覧について、または OpenID Connect ご覧 [ここ](http://social.technet.microsoft.com/wiki/contents/articles/20235.azure-active-directory-application-gallery-federated-saas-apps.aspx)します。
...
*   **使用している一覧にないアプリを追加** -Azure 管理ポータル内のアプリケーション ギャラリーでカスタム カテゴリを使用して、組織が使用して一覧にないアプリケーションを接続します。 SAML 2.0 をサポートする任意のアプリケーション、または HTML ベースのサインイン ページをパスワード SSO アプリとして持つ任意のアプリケーションをフェデレーション アプリケーションとして追加できます。 詳細については、この記事を参照してください。 [独自のアプリケーションを追加する](active-directory-saas-custom-apps.md)です。


*   **開発して、独自のアプリケーションを追加する** - 自分でアプリケーションを開発した場合は、フェデレーション シングル サインオンを実装する Azure AD 開発者向けドキュメントのガイドラインに従い、Azure AD を使用して graph API またはプロビジョニングします。 詳細については、次のリソースを参照してください。
...
*   **アプリケーションの統合を要求** -を使用して必要なアプリケーションのサポートを要請、 [Azure AD のフィードバック フォーラム](http://feedback.azure.com/forums/169401-azure-active-directory)します。
...
Https://myapps.microsoft.com に、アクセス パネルは、組織のアカウントを表示する Azure Active Directory とする、アクセスが許可された Azure AD 管理者によって起動クラウド ベースのアプリケーションで、エンドユーザーは、web ベースのポータルです。 エンドユーザーである場合 [Azure Active Directory Premium](http://azure.microsoft.com/pricing/details/active-directory/), 、アクセス パネルを介してセルフ サービス グループ管理機能を利用することもできます。

![][3]

アクセス パネルは Azure 管理ポータルから独立していて、ユーザーが Azure サブスクリプションまたは Office 365 サブスクリプションを持っている必要はありません。

Azure AD アクセス パネルの詳細については、次を参照してください。、 [アクセス パネルの概要](active-directory-saas-access-panel-introduction.md)します。

###Office 365 アプリケーション起動プログラム

Office 365 をデプロイしている組織では、Azure AD を介してユーザーに割り当てられているアプリケーションも https://portal.office.com/myapps で Office 365 ポータルに表示されます。 これにより、組織のユーザーは、2 つ目のポータルを使用することなく簡単かつ手軽にアプリを起動できます。これは、Office 365 を使用する組織に対して推奨されるアプリ起動ソリューションです。

![][4]

Office 365 アプリケーション起動プログラムの詳細については、次を参照してください。 [アプリを Office 365 アプリケーション ランチャーに表示](https://msdn.microsoft.com/office/office365/howto/connect-your-app-to-o365-app-launcher)します。
