<properties
    pageTitle ="高度な rules| を作成する属性の使用Microsoft Azure]
    description ="方法-式のルール演算子とパラメーターがサポートされているグループなど、高度なルールを作成するためのです"。
...
**高度なルールを作成するには**
Azure ポータルで、グループの下にある **構成** ] タブで [、 **高度なルール** オプションを選択し、高度なルールを指定されたテキスト ボックスに入力します。 高度なルールの作成には、以下の情報を使用できます。
...
二重引用符 (") を含んだ文字列は、バック クォート文字 (`) でエスケープする必要があります (例: user.department -eq "Sa`"les")。
...
| proxyAddresses| SMTP: alias@domain smtp: alias@domain| (user.proxyAddresses-が含まれています"SMTP: alias@domain")|
...
2. 次の構文を使用してルールを入力します。
直属の部下 *Direct Reports for {userid_of_manager}*
