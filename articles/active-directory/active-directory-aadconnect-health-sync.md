<properties 
    pageTitle ="を使用して Azure AD Connect Health 同期の使用 |Microsoft Azure] 
    description =「Azure AD Connect sync を監視する方法を説明する Azure AD Connect Health のページです」。 
...
次のドキュメントは、Azure AD Connect Health を使用した Azure AD Connect 同期の監視について記述しています。 AD FS を Azure AD Connect Health の監視の詳細については「 [と AD FS を使用して Azure AD Connect Health](active-directory-aadconnect-health-adfs.md)します。
...
この機能は、(インポート、エクスポートなど) の同期操作の待機時間のグラフィカルな傾向を提供します。 コネクタ。 処理の遅延をすばやく簡単に把握できることに加え (変更が大量に発生している場合などに有効)、待機時間が異常に長く調査が必要なケースを検出することができます。
