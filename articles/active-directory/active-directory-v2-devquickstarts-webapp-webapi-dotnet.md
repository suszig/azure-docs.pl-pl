...
- 作成、 **App Secret** の **パスワード** 型、および後にその値をコピー
...
- 開いている、 `web.config` のルートにファイル、 `TodoList-Service` プロジェクト、および置換、 `ida:Audience` では、同じ **アプリケーション Id** 前述と同様です。
...
- もう 1 つ追加して `using` ステートメントを `App_Start\Startup.Auth.cs` ADAL のファイルです。
