...
| Schema | <li>ユーザー (ローミング ユーザー、連絡先 (ユーザー証明書がありません))</li><li>グループ</li><li>リソース (リソース、室、オンライン会議)</li><li>データベース メールに</li><li>サポートされているオブジェクトの属性の動的な検出</li> |
...
**1 つの値を複数値を持つ** – このオプションを選択すると、複数値属性が単一値属性に変換されます。 値が 1 つ以上存在する場合、上にある値 (通常これが最新の値です) が使用されます。
...
この属性に、最新の更新プログラムが **David Alexander**します。 インポート操作のオプションは、1 つの値を複数値に設定されているために、コネクタはのみインポート **David Alexander** コネクタ領域にします。
