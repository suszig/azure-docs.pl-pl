<properties
    pageTitle="チュートリアル: Azure Active Directory と GaggleAMP の統合 | Microsoft Azure"
...
このチュートリアルでは、Azure Active Directory (Azure AD) と GaggleAMP を統合する方法を説明します。<br>Azure AD と統合する GaggleAMP を使うと、次の利点があります。
...
![アプリケーション][4]<br>
6. 検索ボックスに入力 **GaggleAMP**します。<br><br>
![Azure AD テスト ユーザーを作成します。](./media/active-directory-saas-gaggleamp-tutorial/tutorial_gaggleamp_01.png)<br>
7. 結果ウィンドウで [ **GaggleAMP**, 、クリックして **完了** アプリケーションを追加します。
<br><br>
...
このリンク関係は、Azure AD の **[ユーザー名]** の値を、GaggleAMP の **[Username]** の値として割り当てることで確立されます。
...
4. * *[GaggleAMP テスト ユーザーを作成する](#creating-a-GaggleAMP-test-user)* * - 自分の Azure AD の表現にリンクされている GaggleAMP に Britta Simon に対応します。
...
このセクションの目的は、Azure クラシック ポータルで Azure AD のシングル サインオンを有効にすることと、GaggleAMP アプリケーションでシングル サインオンを構成することです。
...
<br><br>![でのシングル サインオンを構成する](./media/active-directory-saas-gaggleamp-tutorial/tutorial_gaggleamp_04.png) <br>

    a. [サインオン URL] ボックスに、次のパターンを使用して、ユーザーが GaggleAMP アプリケーションへのサインオンに使用する URL を入力します。**"https://secure4.gaggleamp.com"**
    > [AZURE.NOTE]お問い合わせください、 [GaggleAMP 販売チーム](mailto:sales@gaggleamp.com) 必要がある場合、 **サインオン URL** アプリケーションです"。

    b. **[次へ]** をクリックします。

4. **GaggleAMP でのシングル サインオンの構成** ] ページで、次の手順を実行します。
<br><br>![でのシングル サインオンを構成する](./media/active-directory-saas-gaggleamp-tutorial/tutorial_gaggleamp_05.png) <br>

    a. **[証明書のダウンロード]** をクリックし、コンピューターにファイルを保存します。 この証明書とメタデータ URL (エンティティ ID、SSO サインイン URL、サインアウト URL) は GaggleAMP 側で SSO をセットアップするために必要です。

    b. **[次へ]** をクリックします。

5. 別のブラウザー インスタンスでは、チームをサポートするなどが用に作成された SAML SSO ページに移動 (例:: *https://accounts.gaggleamp.com/saml_configurations/oXH8sQcP79dOzgFPqrMTyw/edit*)。

6. **SAML SSO** ] ページで、次の手順を実行します。  
   <br><br>![GaggleAMP シングル サイン オン](./media/active-directory-saas-gaggleamp-tutorial/tutorial_gaggleamp_06.png) <br>

    a. Azure クラシック ポータルで、**[発行元 URL]** をコピーし、**[ID プロバイダーの発行元]** ボックスに貼り付けます。

    b. Azure クラシック ポータルで、**[シングル サインオン サービス URL]** をコピーし、**[ID プロバイダーのシングル サインオン URL]** ボックスに貼り付けます。

    c. **[保存]** をクリックします。

    d. ダウンロードした証明書の送信、 [GaggleAMP 販売チーム](mailto:sales@gaggleamp.com)します。
...
このセクションでは、Britta Simon GaggleAMP に自分のアクセスを与えることで Azure シングル サインオンの使用を有効にするとします。
...
2. アプリケーションの一覧で選択 **GaggleAMP**します。
    <br><br>![Azure の一覧](./media/active-directory-saas-gaggleamp-tutorial/tutorial_gaggleamp_50.png)<br>
