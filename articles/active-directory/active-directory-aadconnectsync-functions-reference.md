...
Azure Active Directory 同期の関数は、同期中に属性値を操作に使用されます。 <br>
関数の構文では、次の形式を使用して表されます。 <br>
`< 出力の種類 > FunctionName (< 入力型 >< 位置名 >,..)`
...
[CBool](#cbool) & nbsp; & nbsp; & nbsp; & nbsp; [CDate](#cdate) & nbsp; & nbsp; & nbsp; & nbsp; [CGuid](#cguid) & nbsp; (& a) nbsp; (& a) nbsp; (& a) nbsp; (& a) nbsp; (& a) nbsp; (& a) nbsp; & nbsp; [ConvertFromBase64](#convertfrombase64) & nbsp; & nbsp; & nbsp; & nbsp; [ConvertToBase64](#converttobase64) & nbsp; & nbsp; & nbsp; & nbsp; [ConvertFromUTF8Hex](#convertfromutf8hex) & nbsp; & nbsp; & nbsp; & nbsp; [ConvertToUTF8Hex](#converttoutf8hex) & nbsp; & nbsp; & nbsp; & nbsp; [CNum](#cnum)  & nbsp; & nbsp; & nbsp; & nbsp; [CRef](#cref) & nbsp; & nbsp; & nbsp; & nbsp; [CStr](#cstr)  & nbsp; & nbsp; & nbsp; & nbsp; [StringFromGuid](#StringFromGuid) & nbsp; & nbsp; & nbsp; & nbsp; [StringFromSid](#stringfromsid)
...
[Contains](#contains) & nbsp; & nbsp; & nbsp; & nbsp; [カウント](#count) & nbsp; & nbsp; & nbsp; & nbsp; [項目](#item)   & nbsp; & nbsp; & nbsp; & nbsp; [ItemOrNull](#itemornull) & nbsp; & nbsp; & nbsp; & nbsp; [参加](#join) & nbsp; & nbsp; & nbsp; & nbsp; [RemoveDuplicates](#removeduplicates) & nbsp; & nbsp; & nbsp; & nbsp; [Split](#split)

**プログラム フロー:**

[エラー](#error) & nbsp; & nbsp; & nbsp; & nbsp; [IIF](#iif)  & nbsp; & nbsp; & nbsp; & nbsp; [Switch](#switch)


**テキスト**

[GUID](#guid) & nbsp; & nbsp; & nbsp; & nbsp; [InStr](#instr) & nbsp; & nbsp; & nbsp; & nbsp; [InStrRev](#instrrev) & nbsp; & nbsp; & nbsp; & nbsp; [LCase](#lcase) & nbsp; & nbsp; & nbsp; & nbsp; [左](#left) & nbsp; & nbsp; & nbsp; & nbsp; [Len](#len) & nbsp; & nbsp; & nbsp; & nbsp; [LTrim](#ltrim)  & nbsp; & nbsp; & nbsp; & nbsp; [Mid](#mid)  & nbsp; & nbsp; & nbsp; & nbsp; [PadLeft](#padleft) & nbsp; & nbsp; & nbsp; & nbsp; [PadRight](#padright) & nbsp; & nbsp; & nbsp; & nbsp; [PCase](#pcase)   & nbsp; & nbsp; & nbsp; & nbsp; [置換](#replace) & nbsp; & nbsp; (& a) nbsp; & nbsp; [ReplaceChars](#replacechars) & nbsp; & nbsp; & nbsp; & nbsp; [右](#right) & nbsp; & nbsp; & nbsp; & nbsp; [RTrim](rtrim) & nbsp; & nbsp; & nbsp; & nbsp; [トリミング](#trim) & nbsp; & nbsp; & nbsp; & nbsp; [UCase](#ucase) & nbsp; & nbsp; & nbsp; & nbsp; [Word](#word)
...
**構文:**<br>
`num BitAnd (num value1、value2 のテンキーの)`
...
**構文:** <br>
`num BitOr (num value1、value2 のテンキーの)`
...
`CDate ("2013年-01-10 午後 4時 00分 -8")` <br>
...
`num Contains (mvstring attribute, str search、Casetype 列挙型)`<br>
...
**例:**<br>
`IIF(Contains([proxyAddresses],"SMTP:") > 0 で、[proxyAddresses], エラー ("No primary SMTP address が見つかりました"))。`<br>
...
`str ConvertFromBase64 (str ソース、enum エンコード)`
...
**例:** <br>
`Dn の場合"cn = Joe, ou = Atlanta, ou = GA, ou = u. s.、dc = contoso, dc = com"Dncomponentrev` <br>  `DNComponentRev([dn],1,"DC")` <br>
US 両方を返します。
...
**構文:** <br>
`Error(str ErrorMessage) を無効にします。`

**例:** <br>
`IIF (IsPresent ([accountName]) [accountName] エラー ("AccountName is required"))` <br>
...
**構文:** <br>
`str FormatDateTime (dt value, str 形式)`
...
**構文:** <br>
`var IIF (exp 条件、var valueIfTrue, var valueIfFalse)`
...
**例:** <br>
`IIF ([employeeType] =「インターン」、"t-"& [alias],[alias])` <br>
...
`num InStr (str stringcheck、str stringmatch、テンキーの開始)` <br>
`num InStr (str stringcheck、str stringmatch、テンキーの開始、列挙型の比較)`
...
**例:** <br>
`InStr ("The quick brown fox"、「クイック」)` <br>
評価結果 5 になります
...
`num InstrRev (str stringcheck、str stringmatch、テンキーの開始)` <br>
`num InstrRev (str stringcheck、str stringmatch、テンキーの開始、列挙型の比較)`
...
**構文:** <br>
`bool IsBitSet (num value, num フラグ)`
...
`スイッチ (IsPresent ([directManager]) [directManager] IsPresent ([skiplevelManager]) [skiplevelManager] IsPresent([director]),[director])`
...
**構文:** <br>
`var (mvstr attribute, num インデックス) の項目`
...
**例:** <br>
`Mid (項目 ([proxyAddress] が含まれています ([proxyAddress]"SMTP:")), 6)`  <br>
...
**構文:** <br>
`var ItemOrNull (mvstr attribute, num インデックス)`
...
**構文:** <br>
`num NumFromDate (dt 値)`


**例:** <br>
`NumFromDate (CDate ("2012年-01-01 23時 00分: 00"))` <br>
...
**構文:** <br>
`str PadLeft (str 文字列、テンキーの長さ、str padCharacter)`
...
**構文:** <br>
`str PadRight (str 文字列、テンキーの長さ、str padCharacter)`
...
**構文:** <br>  
`mvstr RemoveDuplicates (mvstr 属性)`
...
**構文:** <br>
`str に置き換えます (str string, str OldValue、str NewValue)`
...
**構文:** <br>
`mvstr 分割 (str value, str 区切り記号)` < br?
`mvstr 分割 (str 値、str 区切り記号、テンキーの制限)`
...
**構文:** <br>
' var スイッチ (exp expr1 var value1 [、exp expr2 var 値] [、exp expr、var valueN])`
...
**例:** <br>
`スイッチ ([city] ="London","English", [city] ="Rome","Italian", [city] ="Paris","French", True, Error ("Unknown city"))` <br>
...
**構文:** <br>
`str Word (str string、num WordNumber, str 区切り記号)`
...
**例:** <br>
`単語 ("The quick brown fox", 3,"")` <br>
"Brown"を返します。

`単語 ("、この文字列! が & 多く改行「, 3,」、! & #")` <br>
"Has"を返します。


## その他のリソース

* [宣言型のプロビジョニングの式をについてください。](active-directory-aadconnectsync-understanding-declarative-provisioning-expressions.md)
