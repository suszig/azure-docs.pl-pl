...
> [AZURE.IMPORTANT] Azure AD Premium および Basic では、Enterprise Mobility Suite サブスクリプションだけでなく、プロビジョニングされたディレクトリまたはテナントに限定されます。 サブスクリプションをディレクトリ間で分割することや、他のディレクトリ内のユーザーに資格を与えるために使用することはできません。 ディレクトリ間でサブスクリプションを移動できますが、直接購入の場合は、サポート チケットの送信、または取り消しと再購入が必要になります。

> [AZURE.IMPORTANT] Azure AD を購入する際にボリューム ライセンス サブスクリプションのアクティブ化によって Enterprise Mobility Suite は、アグリーメントに Office 365 など、他の Microsoft Online サービスが含まれて ときに自動的に実行されますか。
...
グループまたは個々 のユーザーにライセンスを割り当てるライセンスの割り当て、をクリックするプランを選択 **割り当てる** コマンド バーにします。

![有効な評価版のライセンス プラン](./media/active-directory-licensing-what-is/assign_licenses.png)

ユーザーとに追加することを選択する、選択されたプランの割り当て] ダイアログ ボックスで、 **割り当てる** 右側の列です。 ユーザーの一覧のページを移動したり、ユーザー グリッドの右上にある虫眼鏡を使用して特定のユーザーを検索したりできます。 グループを割り当てるから"の Groups"を選択、 **表示** ] メニューの [クリックして表示されている割り当てを更新するには、右に、チェック ボタンをクリックします。

![ライセンスをグループに割り当てる](./media/active-directory-licensing-what-is/assign_licenses_to_groups.png)

これで検索することまたはグループの最後のページに追加して、 **割り当てる** 列と同じ方法でします。 これらの方法を使用して、1 回の操作でユーザーとグループの組み合わせを割り当てることができます。 割り当て処理を完了するには、ページの右下隅にあるチェック マークをクリックします。
...
使用可能な割り当て、および次のサブスクリプション ライフ サイクル イベントを含む割り当てられたライセンスの概要ビューが表示されます、 **ライセンス** ] タブをクリックします。
...
ライセンスの削除は、割り当てと同じくらい簡単です。 選択する場合は、ユーザーが直接割り当てられているか、割り当てられているグループのライセンスの種類を選択して、ライセンスを削除することができます、 **削除**, 、削除リストにユーザーまたはグループを追加し、アクションを確認します。 または、ライセンスの種類を開く、特定のユーザーまたはグループを選択してタップ **削除** コマンド バーにします。 グループからユーザーへのライセンスの継承を終了するには、グループからそのユーザーを削除するだけです。

###試用版の拡張

顧客向けの評価版の拡張機能は、Office 365 ポータルからセルフ サービスで使用できます。 顧客の管理者に移動できる、 [Office ポータル](https://portal.office.com/#Billing) (アクセスは、Office ポータルに対するアクセス許可によって異なります) と、Azure AD Premium 評価版を選択します。 クリックして、 **試用版の拡張** をリンクし、指示に従います。 クレジット カード情報を入力する必要がありますが、課金されません。
