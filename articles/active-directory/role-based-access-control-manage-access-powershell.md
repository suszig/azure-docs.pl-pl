...
カスタム ロールを作成するには、使用、 `新規 AzureRmRoleDefinition` コマンドです。
...
最初にカスタム ロールを変更するには、使用、 `Get AzureRmRoleDefinition` ロールの定義を取得するコマンドです。 必要に応じてロール定義を変更します。 最後に、使用して `セット AzureRmRoleDefinition` 変更ロール定義を保存するコマンドです。
...
カスタム ロールを削除するには、 `削除 AzureRmRoleDefinition` コマンドです。
...
スコープに割り当て可能な役割の一覧を表示する、 `Get AzureRmRoleDefinition` コマンドです。
