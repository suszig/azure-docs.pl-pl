...
従来、管理者ロールへのユーザーの割り当てには、Azure 管理ポータルまたは Windows PowerShell が使用されています。 その結果、そのユーザーが **永続的な管理者**, 、自分の割り当てられたロールに常にアクティブです。 このプレビューのサポートが追加、 **一時的な管理者**, 、これは、割り当てられたロールに対するアクティブ化プロセスを完了する必要があるユーザーです。  アクティブ化プロセスにより、Azure AD でのロールへのユーザーの割り当てが非アクティブからアクティブに変わります。

## ディレクトリの Privileged Identity Management の有効化

アクセスすることによって Azure AD Privileged Identity Management の使用を開始する、 [Microsoft Azure ポータル](https://portal.azure.com/)します。 現在、Azure AD Privileged Identity Management は Microsoft Azure ポータルのみに表示されます。 ディレクトリの Azure AD Privileged Identity Management の有効化は、グローバル管理者が行う必要があります。

![][1]

この拡張機能を初期化した後は自動的に最初の **セキュリティ管理者** ディレクトリのです。 セキュリティ管理者だけが、この拡張機能にアクセスして他の管理者のアクセスを管理できます。  
初期化中に、Azure AD Privileged Identity Management のタイルが、Azure プレビュー ポータルのスタート画面に追加されます。
...
ロールをアクティブ化するには、一時的管理者は、ロールに対する時間制限付き「アクティブ化」を要求する必要があります。 使用して、ライセンス認証を要求できる、 **私の仕事をアクティブ化** Azure AD Privileged Identity management オプション。
