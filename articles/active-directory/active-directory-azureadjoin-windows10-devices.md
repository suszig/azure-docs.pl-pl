...
1. **Azure AD 参加**, 、Windows 10、プロビジョニング プロセスの作業者が、主に Office 365 など、クラウド内のリソースへのアクセスの対象となる、セルフ サービスの作業の新機能です。
1. **ドメインへの参加**, 、Windows 10 の内部設置型アプリケーション、およびリソースの大規模な投資の組織の Azure AD に接続されている場合に適しています。
1. **BYOD のエクスペリエンスを簡素化、新しい** に個人のデバイスで会社のリソースを簡単にアクセスする Windows に仕事用アカウントを追加するユーザーができます。
...
**組織がその他の特典を楽しんだり、クラウド経由で企業リソースにアクセスする場合は、**:
...
Azure AD 参加の詳細については、参照 [クラウド機能を拡張する Azure Active Directory 参加を使用して Windows 10 デバイスへ](active-directory-azureadjoin-overview.md)します。
...
Azure AD 参加の詳細については、次を参照してください。 [Windows 10 が発生したために、Azure AD にドメインに参加しているデバイスを接続](active-directory-azureadjoin-devices-group-policy.md)します。
...
- Windows 10 でドメインへの参加の詳細については < link-to-DJ-in-Win10-deployment-guide > を参照してください。
...
- 組織で Microsoft Passport を有効にするためのポリシーの設定。
- Microsoft Passport と Windows こんにちは Windows 10 での詳細については < link-to-MS-Passport-and-Windows-Hello-document > を参照してください。
...
Windows 8.1 で Workplace Join は BYOD を有効にするために使用されていました。 Windows 10 では、BYOD はこのドキュメントで既に説明している [職場アカウントを追加] を使用して有効になります。 MDM を Azure AD と統合されない組織の場合は、ユーザー、デバイスに登録して手動で行う管理 **設定** > **アカウント** > **アクセスの作業**します。

###ユーザーは Windows 10 のドメイン アカウントに Microsoft アカウント (MSA) を接続できますか。
Windows 10 ではできません。 Windows 8.1 では、ドメインに参加しているデバイスを使用していたユーザーは、Live サービスで SSO などの特定のエクスペリエンスを有効にしたり、Windows ストアおよびユーザー設定のデバイス間でのユーザー設定のローミングを使用したりするために (Hotmail、Live、Outlook、XBox などの) MSA をドメイン アカウントに接続できました。 Windows 10 では、MSA Connect の機能は提供終了しました。 ユーザーは、追加のアカウントとして MSA を 1 つ以上追加して、Windows ストアなどのコンシューマー サービスで SSO を有効にできます。 これに **設定** > **アカウント** > **アカウント**します。
