...
- 詳細については、Windows 10 でドメインへの参加を参照してください <link-to-DJ-in-Win10-deployment-guide>します。
...
- 組織で Microsoft Passport を有効にするためのポリシーの設定。
- Microsoft Passport と Windows こんにちは Windows 10 の詳細については次を参照してください <link-to-MS-Passport-and-Windows-Hello-document>します。
...
Windows 10 ではできません。 Windows 8.1 でドメインに参加したデバイスのユーザーが「接続」、MSA (例: Hotmail、Live、Outlook、XBox など) ドメイン アカウントを有効にするには、Live サービスと SSO のような特定のエクスペリエンスは、Windows ストア アプリとデバイス間でユーザー設定のローミングの使用します。 Windows 10 では、MSA Connect の機能は提供終了しました。 ユーザーは、追加のアカウントとして MSA を 1 つ以上追加して、Windows ストアなどのコンシューマー サービスで SSO を有効にできます。 これは、**[設定]**、**[アカウント]**、**[アカウント]** で実行します。
