...
1. AD Health エージェントをインストールする前に、それぞれのサーバーに Windows PowerShell 4.0 をインストールします。  Windows PowerShell 4.0 をインストールするには:
 ...
 - インストール、 [Windows Management Framework 4.0 です。](https://www.microsoft.com/download/details.aspx?id=40855)
...
8. 選択、 **成功の監査** と **失敗の監査** のチェック ボックスです。
9. Click **OK**.
...
8. 選択、 **成功の監査し、失敗の監査** のチェック ボックスをクリックして **OK**します。
...
2. Windows ログ] に移動し、選択 **セキュリティ**します。
3. をクリックして、右 **現在のログをフィルター**します。
4. [イベント ソース] を選択 **AD FS の監査**します。

![AD FS audit logs](./media/active-directory-aadconnect-health-requirements/adfsaudit.png)

> [AZURE.WARNING] ある場合は、AD FS が Azure AD Connect Health エージェントで監査を無効にすると、グループ ポリシーは情報を収集することはできません。 監査を無効にするグループ ポリシーが設定されていないことを確認してください。
...
>[AZURE.NOTE] プロキシ設定を更新するためのすべての Azure AD Connect Health エージェント サービスを再起動する必要があります。 次のコマンドを実行します。<br>
    サービスの再起動 AdHealth *
...
例: *セット AzureAdConnectHealthProxySettings HttpsProxyAddress myproxyserver:443*
