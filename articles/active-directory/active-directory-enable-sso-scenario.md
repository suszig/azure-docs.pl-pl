...
この最適なことが可能にコントロールの id を使用してクラウド アプリケーションの word で"*どの操作を行うことができるユーザー*"です。

コンピューティングの用語では次のように説明できます。

- *ユーザー* と呼ばれる *identity* -ユーザーとグループで構成されるデータ ストア

- *どのような* と呼ばれる *アクセス管理* : 保護されたリソースへのアクセスの管理

両方のコンポーネント化と呼びます *Id およびアクセス管理 (IAM)*, 、によって定義される、 [Gartner](http://www.gartner.com/it-glossary/identity-and-access-management-iam) グループとして"*により、右側にある適切なリソースにアクセスする権限のあるユーザーのセキュリティ分野、正当な理由の時間*"です。

では、何が問題になるでしょうか。 IAM は場合 *管理されていない* 統合ソリューションを使用して 1 か所で。
...
Azure Active Directory (AD) は Microsoft の包括的な Id アクセス管理、シングル サインオン (SSO)、およびレポートでのクラウド サービスとして IAM を有効化とを提供するサービス (IDaaS) ソリューションをビルド [数千のアプリケーションに統合](https://azure.microsoft.com/marketplace/active-directory/), 、Salesforce、Google Apps、ボックス、Concur、および多数詳細などです。 Azure AD を使用すると、パートナーと顧客 (ビジネスまたはコンシューマー) 向けに発行したアプリケーションに同じ ID およびアクセス管理の機能が備わるため、ユーザーや管理者は主要なビジネスのみに集中できます。
...
- **アプリケーションとサービス プロバイダーによって事前統合** Office 365 のように、Azure は、これらはアプリケーションに Azure AD に直接構築され、包括的な id およびアクセス管理機能に依存します。 これらのアプリケーションへのアクセスは、ディレクトリ情報とトークンの発行を用いて実現されています。

- **アプリケーションは、Microsoft やカスタム アプリケーションが事前統合** これらは独立したクラウド アプリケーションを内部アプリケーション ディレクトリに依存しており、Azure AD とは独立して動作できます。 これらのアプリケーションへのアクセスは、アプリケーション アカウントにマップされたアプリケーション固有の資格情報を発行することで実現されています。 資格情報は、アプリケーションの機能に応じて、フェデレーション トークンか、そのアプリケーションで以前にプロビジョニングされたアカウントのユーザー名とパスワードのどちらかになります。

- **内部設置型アプリケーション** 、主に内部設置型アプリケーションへのアクセスを有効にする Azure AD アプリケーション プロキシ経由のアプリケーションで公開します。 これらのアプリケーションでは、Windows Server Active Directory のような一元管理のオンプレミス ディレクトリが使用されます。 これらのアプリケーションへのアクセスは、オンプレミスのサインオン要件に従いながら、プロキシをトリガーしてアプリケーションのコンテンツをエンド ユーザーに配信することで実現されています。
...
Azure AD では、事前に統合された MFA ときめ細かいアクセス ポリシーを使用して、アプリケーションをセキュリティで保護することができます。 詳細については、Azure MFA を参照してください [Azure MFA](https://azure.microsoft.com/services/multi-factor-authentication/)します。

## 使用の開始

最初にアプリケーションを Azure AD と統合するを見て、 [の統合と Azure Active Directory を取得するアプリケーションのガイド](active-directory-integrating-applications-getting-started.md)します。
