...
Aby wyświetlić zasoby konta emulatora magazynu, rozwiń węzeł **programowanie** węzła. Jeśli emulator magazynu nie została uruchomiona po rozwinięciu **programowanie** węzła, zostanie ona automatycznie rozpoczęta. Może to zająć kilka sekund. Można kontynuować pracę w innych obszarach programu Visual Studio podczas uruchamiania emulatora magazynu.

Aby wyświetlić zasoby na koncie magazynu, rozwiń węzeł konta magazynu w Eksploratorze serwera. Wyświetlane są następujące węzły podrzędne:
...
2. W **Tworzenie kontenera obiektów Blob** okna dialogowego wprowadź nazwę nowego kontenera.  
3. Wybierz opcję Wprowadź na klawiaturze, lub można kliknij lub naciśnij polecenie poza pole nazwy, aby zapisać kontenera obiektów blob.
...
* Usuń obiekt blob.
  ...
  > [!NOTE]
  > Usunięcie pliku z kontenera obiektów blob nie powoduje usunięcia jego pliku. Powoduje tylko usunięcie go z kontenera obiektów blob.
  > 
  > 
* Otwórz obiektu blob.
...
2. W **Przekaż nowy plik** okno dialogowe, wybierz opcję **Przeglądaj** przycisk, aby określić plik, który chcesz przekazać, a następnie wprowadź nazwę folderu w **Folder (opcjonalnie)** pole.
...
3. Kliknij dwukrotnie folder lub wybierz Enter, aby wyświetlić zawartość tego folderu. Podczas pracy w folderze kontenera, możesz wrócić do katalogu głównego kontenera, wybierając **Otwórz katalog nadrzędny** przycisk (strzałka).

### <a name="to-delete-a-container-folder"></a>Aby usunąć folder kontenera
* Usuń wszystkie pliki w folderze.
  
> [!NOTE]
> Ponieważ folderów w kontenerów obiektów blob są wirtualnych folderów, nie można utworzyć pusty folder. Nie można również usunąć folder można usunąć jego zawartość pliku. Musisz usunąć całą zawartość folderu można usunąć folderu, do samej siebie.
> 
> 
...
* Wybierz **przekazania obiektu Blob** przycisku, gdy kontener jest otwarty w celu wyświetlenia w widoku kontenera obiektów blob.
...
Plik jest pobierany do tymczasowej lokalizacji i otworzyć na komputerze lokalnym. Obiekt blob należy przesłać ponownie po wprowadzeniu zmian.
...
2. W **Create Table** okna dialogowego wprowadź nazwę tabeli.
...
2. Otwórz węzeł konta magazynu, które są zainteresowani, a następnie otwórz **tabel** węzeł umożliwia wyświetlenie listy tabel dla konta magazynu.
3. Otwórz menu skrótów dla tabeli, a następnie wybierz **widok tabeli**.
...
2. W **Dodaj jednostki** okna dialogowego wprowadź wartości **PartitionKey** i **RowKey** właściwości.
...
2. Wybierz **konstruktora zapytań** przycisk na pasku narzędzi w widoku tabeli.
...
3. Po zakończeniu tworzenia zapytania, zamknij okno dialogowe. Tekst w formularzu, zapytania jest wyświetlana w polu tekstowym jako filtr usługi danych WCF.
4. Aby uruchomić zapytanie, wybierz ikonę zielonym trójkątem.
...
2. W **Utwórz konto magazynu** oknie dialogowym Wybierz lub wprowadź następujące informacje:
...
Jeśli utworzono konto magazynu przy użyciu **dostęp do odczytu z magazynu geograficznie nadmiarowego** opcji replikacji można wyświetlić jego dodatkowej punkty końcowe:

* Otwórz menu skrótów dla nazwy konta, a następnie wybierz **właściwości**.
...
> [!NOTE]
> Jeśli usuniesz konto magazynu z poziomu Eksploratora serwera nie wpływa na koncie magazynu lub dane, które zawiera. Po prostu usuwa odwołanie z Eksploratora serwera. Aby trwale usunąć konto magazynu, należy użyć [klasycznego portalu Azure](http://go.microsoft.com/fwlink/?LinkID=213885).
> 
> 
