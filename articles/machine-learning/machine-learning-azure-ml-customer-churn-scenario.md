...
##業界の展望とアプローチ
離反への高度な対処は、業界の成熟の表れです。 その典型例として、電気通信業界が挙げられます。この業界では、利用者は頻繁にプロバイダーを乗り換えることが知られています。 この自発的な離反は、最も大きな懸念事項となっています。 プロバイダーがに関する重要な知識を蓄積がさらに、 *ドライバー絶え間なく行われ*, 、切り替える顧客要因であります。
...
モデリングから最終的にわかるのは、既知の離反要因を取り除くだけでは適切な方針を定めることはできないということです。 デシジョン ツリー) などのカテゴリの変数を定量化する従来のモデルをなど、継続的なモデリング戦略は、実際には、 **必須**します。
...
顧客維持のための包括的なアプローチを提供するなら、モデル間の相互作用が鍵となります。 各モデルは必ずしも時間の経過とともに低下します。そのため、アーキテクチャは、暗黙的なループ (CRISP-DM データ マイニング標準によるアーキタイプと同様 [***3***])。  
...
> ただし、パブリックに利用可能なデータを使用して同様のモデル、に関して次のサンプル実験を参照してください、 [Cortana Analytics ギャラリー](http://gallery.azureml.net/):
> [Telco の顧客離れ](http://gallery.azureml.net/Experiment/31c19425ee874f628c847f7e2d93e383)します。
>
...
ただし、離反で最も重要なメトリックには、誤分類率: つまり、分類器によって予測された上位 N 離反のうち実際には **いない** 、離反しておらず、特別な処理でしょうか。 次の図では、すべてのモデルでこの誤分類を比較しています。  
...
###AUC を使用した結果の比較
曲線下面積 (AUC) はの大域尺度を表すメトリック *可分性* 正と負の母集団のスコア分布の間です。 従来の Receiver Operator Characteristic (ROC) グラフと似ているものの、AUC メトリックではしきい値を選択する必要がないという重要な違いがあります。 代わりに、結果がまとめられます、経由で **すべて** プログラムを選択できます。 それとは対照的に、従来の ROC グラフでは縦軸に陽性率、横軸に偽陽性率が示され、分類のしきい値は変動します。   
...
-   4 つの基本的なカテゴリに対応したメトリックを探してください。
    -   **エンティティ (たとえば、サブスクリプション)**します。 離反の主体であるサブスクリプションまたは顧客に関する基本的な情報を準備します。
    -   **アクティビティ**します。 たとえば、ログインの数など、エンティティに関連するすべての可能な使用状況情報を取得します。
    -   **カスタマー サポート**します。 サブスクリプションに問題やカスタマー サポートとのやり取りがあったかどうかを示す情報をカスタマー サポート ログから収集します。
    -   **競争力とビジネス データ**します。 顧客に関して入手可能な情報をすべて入手します (入手できない場合や、トラッキングできない場合もあります)。
-   重要度を特徴選択に利用します。 これは、ブースト デシジョン ツリー モデルが常に有効なアプローチとなることを意味します。  

前の 4 つのカテゴリを使用するように見せるを単純な *決定的* 、カテゴリごとの合理的なファクターによるインデックスに基づいたアプローチにより、離反のリスクがある顧客を識別するために十分です。 これはもっともらしく思えるものの、残念なことに間違った理解です。 というのも、離反というものは一時的な現象であり、離反に影響するファクターは一般に不変ではないためです。 今顧客が離反を検討している理由は、明日には変わる可能性があります。そして 6 か月もすれば確実に変わっています。 したがって、 *確率論的* モデルが必要です。  
...
[フィードバックの送信](mailto:sqlfback@microsoft.com)します。
 
##参照
[1] 「Predictive Analytics: Predictions」W. McKnight、Information Management、2011 年、p.18 ~ 20 の年 7 月/8 月を超える。  

[2] [正確度と精度](http://en.wikipedia.org/wiki/Accuracy_and_precision) Wikipedia の「

[ [3] CRISP-DM 1.0: ステップ バイ ステップのデータ マイニングのガイド](http://www.the-modeling-agency.com/crisp-dm.pdf)   

[4] ビッグ データ マーケティング  

[5] [ビッグ データ マーケティング: より効果的に顧客を対象にして、ドライブの値](http://www.amazon.com/Big-Data-Marketing-Customers-Effectively/dp/1118733894/ref=sr_1_12?ie=UTF8&qid=1387541531&sr=8-12&keywords=customer+churn)
 
##付録
