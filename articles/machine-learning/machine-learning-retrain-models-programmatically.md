...
3. *トレーニング実験を Web サービスとして発行します。*
    トレーニング済みモデルの再トレーニングするには、Web サービスとして上記の手順 1 で作成したトレーニング実験を発行する必要があります。 この Web サービス、Web サービスの出力モジュールに接続されている必要があります、 [モデルのトレーニング][train-model] モジュール、新しいトレーニング済みモデルを生成することになるためです。
...
更新可能なエンドポイント訪問 Azure クラシック ポータルを作成し、エンドポイントの追加] をクリックする (詳細については [ここ](machine-learning-create-endpoint.md))。
