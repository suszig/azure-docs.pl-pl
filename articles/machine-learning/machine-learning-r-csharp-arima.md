...
自動でサービスの利用の複数の方法があります (アプリケーション例が [ここ](http://microsoftazuremachinelearning.azurewebsites.net/ArimaForecasting.aspx))。
...
Azure Machine Learning 内で、新しい空白の実験が作成されました。 入力データのサンプルは、事前定義済みのデータ スキーマにアップロードされました。 [ARIMA の予測され' と 'forecast' 関数を R. を使用してモデルを生成します [R スクリプトの実行] [- r のスクリプトの実行] モジュール データ スキーマにリンク 
