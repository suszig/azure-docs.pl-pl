...
これは、 **メニュー** さまざまな環境でデータの機能を作成する方法を説明するトピックへのリンク。 このタスクは、Cortana Analytics Process (CAP) の 1 ステップです。
...
* テキスト マイニングの分類の例を使用して、 [特徴ハッシュ](https://msdn.microsoft.com/library/azure/c9a82660-2d9c-411d-8122-4d9e0b3ce92a/)
...
Azure Machine Learning の実験では、これら 4 つのトレーニング データセットは、前処理された入力データセットからの 4 つの分岐を使用して形成されます。 左を除くほとんど分岐は、これらの各分岐を含む、 [R スクリプトの実行](https://msdn.microsoft.com/library/azure/30806023-392b-42e0-94d6-6b775a6e0fd5/) モジュールを一連の特徴 (特徴セット B、C、および D) はそれぞれ構築され、インポートされたデータセットに追加されます。 次の図は、左の 2 番目の分岐にある特徴セット B の作成に使用される R スクリプトを示します。
...
Azure Machine Learning では、 [特徴ハッシュ](https://msdn.microsoft.com/library/azure/c9a82660-2d9c-411d-8122-4d9e0b3ce92a/) これらを作成するモジュールの単語や語句の特徴便利なことにします。 このモジュールを使用する例を次に示します。 入力データセットには、1 ～ 5 の書籍の評価と実際のレビュー内容の 2 つ列が含まれています。 この目標 [特徴ハッシュ](https://msdn.microsoft.com/library/azure/c9a82660-2d9c-411d-8122-4d9e0b3ce92a/) モジュールは、一連の新しい特徴取得する対応する単語の出現頻度を示すレビュー内や語句、特定の書籍。 このモジュールを使用するには、次の手順を完了する必要があります。
