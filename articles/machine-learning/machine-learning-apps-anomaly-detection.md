...
サービスは、REST ベースの API を HTTPS 経由で提供しますが、これは Web アプリケーションやモバイル アプリケーション、R、Python、Excel などを含むさまざまな方法で使用できます。ある、 [Azure の web アプリケーション](http://anomalydetection-aml.azurewebsites.net/) 異常検出の実行が web サービスにデータをし、結果を視覚化します。
...
API は時系列データでこれらの検出を実行し、時系列の各ポイントで異常のスコアを返します。 この出力は、のように、シンプルなパーサーを使用して解析できる [https://adresultparser.codeplex.com/](https://adresultparser.codeplex.com/)します。 これは、API に接続し、出力を解析する方法を表すサンプル コードを示します。 生成されるアラートは、ダッシュボード上で視覚化でき、処置を取ることができる人間の専門家に渡すことができます。
