...
1. 型 **列を投影** [プロジェクト列] の [プロジェクト列] モジュールの検索にあるモジュール パレットの上部にある [検索] ボックスにドラッグ実験キャンバスとの出力ポートに接続、 **自動車価格データ (未加工)** データセット。 このモジュールを使用すると、モデルに含める、またはモデルから除外するデータの列を選択できます。

2. [プロジェクト列] の [プロジェクト列] モジュールを選択し、クリックして **列セレクターの起動** で、 **プロパティ** ウィンドウです。

    - 確認 **すべての列** フィルターのドロップダウン リストで選択した **値で始まる**します。 これにより、[プロジェクト列] プロジェクトの列を渡す (私たちはから除外するもの) を除くすべての列はフィルターです。
    ...
    ![プロジェクトの列のプロパティ][screen4]

    > [AZURE.TIP] モジュールをダブルクリックすると、テキストを入力することで、モジュールにコメントを追加できます。 これで、実験でモジュールがどのような処理をするのかがひとめでわかります。 この場合は、[プロジェクト列] の [プロジェクト列] モジュールをダブルクリックして、コメント「正規化された損失を除外する」を入力

3. [見つからないデータのクリーンアップ] [クリーンで見つからないのデータ] モジュールを実験キャンバスにドラッグし、[プロジェクト列] の [プロジェクト列] モジュールに接続します。  **プロパティ** ] ウィンドウの [ **行全体の削除** [ **クリーニング モード** 不足値がある行を削除することで、データのクリーンアップします。 モジュールをダブルクリックして、「値が見つからない行を削除する」というコメントを入力します。

    ![見つからないデータのクリーンアップのプロパティ][screen4a]
...
ここまで、実験で実行したことは、データのクリーンアップだけです。 整理したデータセットを表示する場合は、[見つからないデータのクリーンアップ] [クリーンで見つからないのデータ] モジュール (「整理したデータセット」) の左側の出力ポートをクリックし、選択 **視覚化**します。 注意して、 **正規化された損失** 列が含まれているもうと不足値はありません。
...
ここでは、データセット内の特徴のサブセットを使用するモデルを構築します。 この手順に戻って別の特徴を選択し、もう一度実験を実行して、より優れた結果が得られるかどうかを確認できます。 最初に推測と、[プロジェクト列] の [プロジェクト列] モジュールでは、以下の特徴 (列) を選択します。 注ことのモデルをトレーニングするため含める必要、 *価格* を予測しようとしている値。

    make, body-style, wheel-base, engine-size, horsepower, peak-rpm, highway-mpg, price

1. [プロジェクト列] の [プロジェクト列] の別のモジュールを実験キャンバスにドラッグし、[見つからないデータのクリーンアップ] [クリーンで見つからないのデータ] モジュールの左側の出力ポートに接続します。 モジュールをダブルクリックして、「予測対象の特徴を選択する」と入力します。
...
3. 列セレクターで選択 **の列は** の **値で始まる**, 、し、[ **Include** と **列名** フィルター行でしません。 列名の一覧を入力します。 これにより、モジュールでは、指定する列のみがフィルターされます。
    ...
    > [AZURE.TIP] 実験を実行したので、データに対する列の定義は、[見つからないデータのクリーンアップ] [クリーンで見つからないのデータ] モジュールを介して元のデータセットから渡されました。 [プロジェクト列] の [プロジェクト列] を [見つからないデータのクリーンアップ] [クリーンで見つからないのデータ] に接続するときに、[プロジェクト列] の [プロジェクト列] モジュールは、データ内の列定義の対応になります。 クリックすると、 **列名** ボックスで、列の一覧を表示すると、および一覧に追加する列を選択できます。
...
1. このデータをトレーニング セットとテスト セットに分割すると、トレーニング用とテスト用の両方に使用できます。 選択し、[分割されたデータ] の [分割] モジュールを実験キャンバスにドラッグし、最後の [プロジェクト列] の [プロジェクト列] モジュールの出力に接続します。 設定 **最初の出力データセットにおける列の割合** を 0.75 にします。 このようにして、データの 75% をモデルのトレーニングに使用し、25% をテスト用に保持しておきます。

    > [AZURE.TIP] 変更することで、 **ランダム シード** パラメーター、トレーニング セットとテスト用に異なるランダムなサンプルを生成することができます。 このパラメーターでは、擬似乱数ジェネレーターによる生成を制御できます。

2. 実験を実行します。 これにより、[プロジェクトの列] [プロジェクト列] と [データの分割] [分割] モジュールに列の定義を次に追加するモジュールに渡します。  
...
3. 学習アルゴリズムを選択するには、展開、 **Machine Learning** 、キャンバスの左側にあるモジュール パレットのカテゴリを展開し、 **モデルの初期化**します。 これにより、機械学習アルゴリズムの初期化に使用できるモジュールのカテゴリが複数表示されます。
    ...
    この実験の下にある [線形回帰] [線形回帰] モジュールを選択する、 **回帰** カテゴリ (することもモジュールを見つけ、パレットの検索ボックスに「線形回帰」を入力して)、実験キャンバスにドラッグします。

4. 検索し、[モデルのトレーニング] [モデルのトレーニング] モジュールを実験キャンバスにドラッグします。 [線形回帰] [線形回帰] モジュールの出力には、左側の入力ポートを接続します。 [分割されたデータ] の [分割] モジュールのトレーニング データの出力 (左側のポート) には、右側の入力ポートを接続します。

5. [モデルのトレーニング] [モデルのトレーニング] モジュールを選択して [ **列セレクターの起動** で、 **プロパティ** ] ウィンドウで、クリックして、 **価格** 列です。 これが、作成しているモデルで予測する値です。
...
![機械学習アルゴリズムを適用する][screen8]

## 手順 5: 新しい自動車の価格を予測する

これまでにデータの 75% を使用してモデルをトレーニングしました。ここからは残りの 25% のデータにスコアを付け、モデルの機能の効果を確認します。

1. [モデルのスコア付け] の [モデルのスコア付け] モジュールを実験キャンバスにドラッグし、[モデルのトレーニング] の [モデルのトレーニング] モジュールの出力に左側の入力ポートを接続します。 テスト データの出力 (右側のポート)、[分割されたデータ] の [分割] モジュールの右側の入力ポートを接続します。  

    ![スコア モデル モジュール][screen8a]

2. 実験を実行し、[モデルのスコア付け] の [モデルのスコア付け] モジュールから出力を表示、出力ポートをクリックし、[ **視覚化**します。 出力に、予測された価格の値と、テスト データから既知の値が表示されます。  

3. 最後に、結果の品質をテストしを選択、および [モデルの評価] の [評価モデル] モジュールを実験キャンバスにドラッグし、左を接続するには、[モデルのスコア付け] の [モデルのスコア付け] モジュールの出力ポートを入力します。 (が 2 つの入力ポート [モデルの評価] の [評価モデル] モジュールは、2 つのモデルの比較に使用できるためです。)

4. 実験を実行します。

[モデルの評価] [- モデルの評価] モジュールからの出力を表示する出力ポートをクリックし、[ **視覚化**します。 作成したモデルに対して、以下の統計値が表示されます。
...
![機械学習チュートリアル: 予測モデリング手法を使用した線形回帰実験を実行します]。[screen10]

## 次のステップ

初めての機械学習チュートリアルはこれで終わりです。実験がセットアップされたら、反復によってモデルの改善を試みることができます。 たとえば、予測で使用する特徴を変更できます。 または、[線形回帰] [線形回帰] アルゴリズムのプロパティを変更または別のアルゴリズムを一緒に試すことができます。 でも、学習の実験にアルゴリズムを同時に複数のマシンを追加し、[モデルの評価] の [評価モデル] モジュールを使用して 2 つを比較できます。
