...
- **自動車の信号と診断のデータセットをシミュレートした** と 
- **車両カタログ**

車両テレマティックス シミュレーターは、このソリューションの一部として含まれています。 診断情報と信号を出力し、信号は特定の時点での車両の状態と運転パターンに対応しています。 をクリックして [車両 Telematics シミュレーター](http://go.microsoft.com/fwlink/?LinkId=717075) をダウンロードする、 **車両 Telematics シミュレーター Visual Studio ソリューション** 、要件に基づいてカスタマイズします。 車両カタログには、参照データセットと、モデル マッピングに対する VIN が含まれています。
...
### シミュレートされたデータを生成するには
1.  車両テレマティックス シミュレーター ノードの右上隅にある矢印をクリックして、データ シミュレーター パッケージをダウンロードします。 ローカルのコンピューターにファイルを保存して、展開します。 ![](./media/cortana-analytics-playbook-vehicle-telemetry-deep-dive/fig3-vehicle-telemetry-blueprint.png) *図 3 – 車両テレメトリ分析ソリューションの設計図*

2.  ローカル コンピューター上で、車両テレマティックス シミュレーター パッケージを展開したフォルダーに移動します。 ![](./media/cortana-analytics-playbook-vehicle-telemetry-deep-dive/fig4-vehicle-telematics-simulator-folder.png) *図 4 – 車両 Telematics シミュレーター フォルダー*
...
## 準備
>[AZURE です。警告]、ソリューションにおいてもこの手順は、バッチ処理にのみ適用されます。 

生の半構造化された車両信号と診断データセットは、データの準備の手順で、YEAR/MONTH 形式に分割されます。これは、クエリを効率化し、拡張性の高い長期保持を可能にするためです (つまり、1 つの BLOB アカウントがいっぱいになったら、次のアカウントにフェールオーバーできます)。 出力データ (というラベルが付いた *PartitionedCarEventsTable*) お客様の"Data Lake"内のデータの基本/"rawest"の形式として、長時間にわたって保持するのには。  このパイプラインへの入力データは、出力データが入力に対して完全に忠実であれば、通常は破棄されます。後で使用する場合にだけ、格納 (および分割) されます。
...
1.  **機械学習**: このサブ セクションには、車両のメンテナンスのサービスを必要として安全性の問題のための呼び出しを必要とする手段を予測するこのソリューションで使用した異常検出実験に関する情報が含まれています。
2.  **リアルタイム分析**: このサブ セクションには、Stream Analytics クエリ言語を使用して、カスタム アプリケーションを使用してリアルタイムで machine learning の実験を任せていますのリアルタイム分析に関する情報が含まれています。
3.  **バッチ分析**: このサブ セクションには、情報については、変換して、Azure HDInsight および Azure Machine Learning の Azure Data Factory で計算を使用してデータをバッチの処理が含まれています。

### Machine Learning

ここでの目標は、特定の状態の統計データに基づいて、メンテナンスやリコールを必要とする車両を予測することです。  想定した条件は、以下のとおりです。

- 手段を必要と **サービス メンテナンス** 次の 3 つの条件のいずれかに該当する場合。
    - タイヤ空気圧が低い
    - エンジン オイルのレベルが低い
    - エンジン温度が高い

- 車両場合があります、 **安全上の問題** を必要と **再現率** 、次の条件のいずれかに該当する場合。
    - エンジン温度が高いが、外部温度が低い
    - エンジン温度が低いが、外部温度が高い
...
**メンテナンス検出モデル**
メンテナンスの検出モデルでは、モデルは、それぞれの条件を満たす tire 圧力、エンジン石油またはエンジン温度、3 つのインジケーターのいずれかの場合に異常を報告します。 そのため、モデルの構築では、これらの 3 つの変数だけを考慮すれば済みます。 Azure Machine Learning の実験で使用して、 **プロジェクト列** これら 3 つの変数を抽出するモジュールです。 次に、PCA ベースの異常検出モジュールを使用して、異常検出モデルを構築します。 
...
**異常検出モデルのことを思い出してください。**
再現率の異常検出モデルで同様の方法にプロジェクトの列と PCA ベースの異常検出モジュールを使用します。 具体的には、まず抽出 3 つの変数 - 外温度、および速度エンジン温度、使用して、 **プロジェクト列** モジュールです。 エンジン温度は、通常、速度と関連しているため、速度の変数も含めます。 次に、PCA ベースの異常検出モジュールを使用して、3 次元空間から 2 次元空間にデータを投影します。 エンジン温度と外部温度が大きく負相関している場合、リコール条件が満たされ、車両のリコールが必要になります。 PCA ベースの異常検出アルゴリズムを使用すると、PCA の実行後、異常をキャプチャできます。 
...
Azure Stream Analytics でのすべての 'Windowing' 機能に関する詳細については、クリックして [ウィンドウ (Azure Stream Analytics)](https://msdn.microsoft.com/library/azure/dn835019.aspx)します。
...
1.  公開の Stream Analytics がイベントで Event Hub インスタンスがリッスンするパターンを継続的にします。 ![](./media/cortana-analytics-playbook-vehicle-telemetry-deep-dive/fig16-vehicle-telematics-stream-analytics-query-for-publishing.png)*図 16: 出力に、データを発行するための Stream analytics クエリ Event Hub インスタンス* 
...
をクリックして [RealtimeDashboardApp ダウンロード](http://go.microsoft.com/fwlink/?LinkId=717078) カスタマイズ RealtimeDashboardApp Visual Studio ソリューションをダウンロードします。 

**リアルタイム ダッシュボード アプリケーションを実行するには **

1.  ダイアグラム ビューで PowerBI ノードをクリックし、プロパティ ウィンドウで [Download Real-time Dashboard Application] リンクをクリックします。 ![](./media/cortana-analytics-playbook-vehicle-telemetry-deep-dive/fig17-vehicle-telematics-powerbi-dashboard-setup.png)  *図 17 – PowerBI ダッシュ ボードのセットアップ手順*
2.  展開し、ローカルに保存 ![](./media/cortana-analytics-playbook-vehicle-telemetry-deep-dive/fig18-vehicle-telematics-realtimedashboardapp-folder.png)  *図 18 – RealtimeDashboardApp フォルダー*
...
>[AZURE.NOTE] 注: PowerBI データセットをフラッシュする場合は、'flushdata' パラメーターを持つ RealtimeDashboardApp を実行します。 
...
1.  **動作を促進積極的** モデル、場所、駆動条件、およびマーケティング キャンペーン、個人用の新機能と使用法に基づくの促進に使用する contoso 社のモーターの保険を許可する、積極的な駆動パターンを把握する年の時間の傾向を識別します。
2.  **効率的な駆動動作の燃料** モデル、場所、駆動条件、およびコスト効率に優れた新機能およびドライバの事前対応型のレポートを向上させ、マーケティング キャンペーンのために使用する Contoso モーターを許可する効率的な駆動パターンを燃料と環境のわかりやすい駆動習慣に把握する年の時間の傾向を識別します。 
3.  **モデルの取り消し** 異常検出の machine learning の実験を任せて、リコールを必要とするモデルを識別します。
