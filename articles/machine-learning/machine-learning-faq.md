...
Machine Learning Studio の詳細については、次を参照してください [Machine Learning Studio とは。](machine-learning-what-is-ml-studio.md)
...
少量のデータしかあり、http URL 経由で公開する場合は、[リーダー] [リーダー] モジュールを使用できます。 大量のデータは、まず Azure ストレージに転送し、[リーダー] [リーダー] モジュールを使用して、実験に取り込みます。
<!--
...
[イメージ リーダー] の [イメージ リーダー] 参照 [イメージの入力機能について学習できます。
...
**ようなものを使用することは [PMML](http://en.wikipedia.org/wiki/Predictive_Model_Markup_Language)  モデルを定義するでしょうか。**
...
Machine Learning Studio は、拡張性の高い強化された意思決定ツリー、ベイズの推薦システム、ディープ ニューラル ネットワーク、Microsoft Research で開発された意思決定のジャングルなど、最先端のアルゴリズムを提供します。 Vowpal Wabbit のような拡張性が高いオープン ソース機械学習パッケージも含まれます。 Machine Learning Studio は、複数クラスと二項分類、回帰、クラスタリングで、機械学習アルゴリズムをサポートします。 [機械学習のモジュール] [機械学習モジュール] の一覧を参照してください。
...
**提供されているアルゴリズム用に別の 1 つのアルゴリズムを選ぶ際のガイドラインはありますか。**
...
**How do I AzureML モデルの再トレーニング プログラムを使用して**
再トレーニング API を使用します。 サンプル コードは、使用可能な [ここ](https://azuremlretrain.codeplex.com/)します。

### 作成

**ローカルまたはインターネットに接続していないアプリケーションでモデルをデプロイできますか。**
いいえ、できません。


**すべての Web サービスで予測される基本的な遅延はありますか。**

参照してください、 [Azure サブスクリプションの制限](../azure-subscription-service-limits.md)
 ...
 **Web サービスを作成できるのは Studio で作成されたモデルからのみですか。
 いいえ、できません。 Jupyter Notebook および RStudio から直接 web サービスを作成することもできます。
...
**既定では、実稼働環境でデプロイされた web サービスの http エンドポイントへのアクセスを持つでしょうか。 エンドポイントへのアクセスはどのようにして制限できますか。**
...
**ストレージ アカウントのアクセス キーは同期するとどうなりますか。**
...
参照してください、 [公開と Machine Learning Marketplace でのアプリの使用に関する FAQ](machine-learning-marketplace-faq.md)
...
トレーニングを検索することも [Microsoft Virtual Academy](http://www.microsoftvirtualacademy.com/training-courses/getting-started-with-microsoft-azure-machine-learning)
...
Azure Machine Learning については、MSDN にコミュニティ フォーラムがあり、ここで、Azure ML に関連する質問をすることができます。 このフォーラムは Azure ML チームが監視しています。 参照してください [Azure フォーラム](http://social.msdn.microsoft.com/Forums/windowsazure/home?forum=MachineLearning)します。
