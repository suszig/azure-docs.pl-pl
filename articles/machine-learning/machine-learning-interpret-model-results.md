...
* [モデルのスコア付け][score-model] モジュールは分類と回帰、 
* [クラスターへの割り当て][assign-to-clusters] モジュールはクラスタ リング 
* [スコア マッチ ボックス レコメンダー][score-matchbox-recommender] は推奨システム 
...
この問題を解決するために、図 1 に示すとおり実験が実行されました。 2 クラスのブースト デシジョン ツリー モデルがトレーニングされ、スコア付けされました。 予測結果を視覚化できるようになりました [モデルのスコア付け][score-model] モジュールの出力ポートをクリックして [モデルのスコア付け][score-model] モジュールをクリックすれば、 **視覚化** 表示されたメニューでします。 図 2 に示すように、スコア付けの結果が表示されます。
...
今度は、Web サービスの入力と出力を設定する必要があります。 入力の右側の入力ポートは、当然ながら、 [モデルのスコア付け][score-model], 、これは、あやめの特徴の入力します。 出力の選択は、予測されたクラス (スコア付けラベル)、スコア付け確率、その両方のどれを使用するかによって異なります。 ここでは、両方に関心がなっていると見なされます。 目的の出力列を選択する必要がありますを使用する、 [プロジェクト列][project-columns] モジュールです。 クリックした [プロジェクト列][project-columns] モジュールをクリック **列セレクターの起動** し、右側のパネルで **Scored Labels** と **Scored Probabilities**します。 出力ポートを設定した後に [プロジェクト列][project-columns] モジュールともう一度実行している必要があります] をクリックして、web サービスとしてスコア付け実験を発行する準備が **WEB サービスの発行** 下部にあるボタンをクリックします。 最終的な実験は、図 4 のようになります。
...
結果を視覚化する [モデルのスコア付け][score-model] モジュールの出力ポートを右/左クリックするとして [モデルのスコア付け][score-model] と、 **視覚化**, 、図 7 に示すウィンドウが表示されます。
...
このとき、使用する代わりに [プロジェクト列][project-columns] を web サービスの出力としていくつかの列を選択したい各エントリと、スコア付けされたラベルの確率のスコア付けされたラベルを取得します。 基本的なロジックは、すべてのスコア付け確率の中から最も高い確率を見つけることです。 これを行う必要がありますを使用する [R スクリプトの実行][execute-r-script] モジュールです。 図 8 に R コードを、図 9 に実験を示します。
...
視覚化 [モデルのスコア付け][score-model] モジュール、結果は次の図 12 のようにします。
...
左側の出力ポートをクリックして、結果の最初の部分を視覚化できます [トレーニング クラスタ リング モデル][train-clustering-model] と **視覚化** 後です。 図 16 は、視覚化ウィンドウを示します。
...
Azure Machine Learning の組み込みのいくつかの方法 [マッチ ボックス レコメンダーのトレーニング][train-matchbox-recommender] モジュール。 
...
4 つのオプションから選択することを選択できます、 **Recommender prediction kind** 右側のパネルでメニュー。 ここでは、4 つのすべてのシナリオについて説明します。 レコメンダー システム用の一般的な Azure Machine Learning の実験は、図 20 のようになります。 「」を参照してください。 これらのレコメンダー システム モジュールを使用する方法の詳細については、ヘルプ ページの [マッチ ボックス レコメンダーのトレーニング][train-matchbox-recommender] と [スコア マッチ ボックス レコメンダー][score-matchbox-recommender]します。
...
評価の予測を選択して、 **Recommender prediction kind** ] メニューの [特定のユーザーと項目に対する評価を予測し、レコメンダー システムを願いします。 視覚化、 [スコア マッチ ボックス レコメンダー][score-matchbox-recommender] 出力は次の図 21 のようにします。
...
選択して **項目の推奨** で、 **Recommender prediction kind** ] メニュー、レコメンダー システム、特定のユーザーに対して項目を勧めるようです。 このシナリオでは、Recommended item selection パラメーターも選ぶ必要があります。 オプション **From Rated Items (for model evaluation)** は主に、トレーニング プロセス中のモデル評価します。 この予測ステージを選択します **From All Items**します。 視覚化、 [スコア マッチ ボックス レコメンダー][score-matchbox-recommender] 出力は次の図 22 のようにします。
...
[Recommender prediction kind] メニューで関連するユーザーを選択し、レコメンダー システムで特定のユーザーに関連するユーザーを検索するよう指定します。 関連するユーザーとは、類似する嗜好を持つユーザーのことです。 このシナリオでは、Related user selection パラメーターも選ぶ必要があります。 オプション [From Users That Rated Items (for model evaluation)] は主に、トレーニング プロセス中のモデルの評価で使用します。 この予測ステージでは、[From All Users] を選択します。 視覚化、 [スコア マッチ ボックス レコメンダー][score-matchbox-recommender] 出力は次の図 23 のようにします。
