...
**理解すると、'モデルのスコア付け' の出力を視覚化します。**
このトピックでは、視覚化し、Azure Machine Learning Studio で予測結果を解釈する方法について説明します。 モデルのトレーニングと予測が完了したら (「モデルのスコア付け」)、得られた予測結果を理解し、解釈する必要があります。
...
* [モデルのスコア付け][モデルのスコア付け] モジュールは分類と回帰、 
* [クラスターへの割り当て][割り当てクラスター] モジュールはクラスタ リング 
* [スコア マッチ ボックス レコメンダー][スコア-マッチ ボックス-レコメンダー] は推奨システム 
 
このドキュメントでは、これらのモジュールの各予測結果を解釈する方法について説明します。 この種類のモデルの概要については、次を参照してください。 [Azure Machine Learning でアルゴリズムを最適化するためにパラメーターを選択する方法](machine-learning-algorithm-parameters-optimize.md)します。

このトピックでは、モデルの評価ではなく、予測の解釈について説明します。 モデルを評価する方法の詳細についてを参照してください [Azure Machine Learning でモデルのパフォーマンスを評価する方法](machine-learning-evaluate-model-performance.md)します。

Azure Machine Learning を初めて開始を参照して簡単な実験を作成する方法について説明していて [Azure Machine Learning Studio で簡単な実験の作成](machine-learning-create-experiment.md) Azure Machine Learning Studio でします。 
...
2 クラス分類問題の例では、あやめの分類を使用します。ここでのタスクは、特徴に基づいてあやめを分類することです。 Azure Machine Learning で用意されているあやめのデータセット、一般的なのサブセットであることに注意してください [Iris データセット](http://en.wikipedia.org/wiki/Iris_flower_data_set) 2 つだけの花の品種 (0 ~ 1 クラス) のインスタンスを含みます。 それぞれの花には 4 種類の特徴 (がくの長さ、がくの幅、花弁の長さ、花弁の幅) があります。

![screenshot_of_experiment](./media/machine-learning-interpret-model-results/1.png)

図 1: あやめの 2 クラス分類問題の実験

この問題を解決するために、図 1 に示すとおり実験が実行されました。 2 クラスのブースト デシジョン ツリー モデルがトレーニングされ、スコア付けされました。 [モデルのスコア付け] の [モデルのスコア付け] モジュールの出力ポートをクリックし、をクリックして [モデルのスコア付け] の [モデルのスコア付け] モジュールから予測結果を視覚化できるようになりました **視覚化** 表示されたメニューでします。 図 2 に示すように、スコア付けの結果が表示されます。
...
予測結果を理解し、結果の妥当性が確認できたら、実験を Web サービスとして発行できます。発行すると、さまざまなアプリケーションで展開して呼び出し、新しいあやめにおけるクラスの予測を取得できます。 トレーニング実験をスコア付け実験に変更し、web サービスとして発行する方法の手順については、次を参照してください。 [Azure Machine Learning web サービスを発行](machine-learning-walkthrough-5-publish-web-service.md)します。 この手順に従うと、図 3 で示すスコア付けの実験を実行できます。

![screenshot_of_experiment](./media/machine-learning-interpret-model-results/3.png)

図 3: あやめの 2 クラス分類問題のスコア付け実験

今度は、Web サービスの入力と出力を設定する必要があります。 当然ながら、入力は、あやめの花の機能の入力である [モデルのスコア付け] [スコア付けのモデル] の右側入力ポートです。 出力の選択は、予測されたクラス (スコア付けラベル)、スコア付け確率、その両方のどれを使用するかによって異なります。 ここでは、両方に関心がなっていると見なされます。 目的の出力列を選択するには、[プロジェクト列] の [プロジェクト列] モジュールを使用する必要があります。 [プロジェクト列] の [プロジェクト列] モジュールをクリックして、をクリックして **列セレクターの起動** し、右側のパネルで **Scored Labels** と **Scored Probabilities**します。 [プロジェクト列] の [プロジェクト列] モジュールの出力ポートを設定し、もう一度実行した後する必要があります] をクリックして、web サービスとしてスコア付け実験を発行する準備が **WEB サービスの発行** 下部にあるボタンをクリックします。 最終的な実験は、図 4 のようになります。
...
[モデルのスコア付け] の [モデルのスコア付け] モジュールからの結果を視覚化する右/左クリックして、[モデルのスコア付け] の [モデルのスコア付け] モジュールの出力ポートをクリックすると **視覚化**, 、図 7 に示すウィンドウが表示されます。
...
今回は、[プロジェクト列] [プロジェクト列] web サービスの出力としていくつかの列を選択する代わりに、今回は各エントリと、スコア付けされたラベルの確率のスコア付けラベルを取得します。 基本的なロジックは、すべてのスコア付け確率の中から最も高い確率を見つけることです。 これを行うには、[R スクリプトの実行] の [の r のスクリプトの実行] モジュールを使用する必要があります。 図 8 に R コードを、図 9 に実験を示します。
...
[モデルのスコア付け] モジュールの [モデルのスコア付け] を視覚化する結果に図 12 のようになります。
...
クリックして、[クラスタ リング モデルのトレーニング] [- クラスタ リングのモデルのトレーニング] モジュールの左側の出力ポートをクリックして、結果の最初の部分を視覚化できます **視覚化** 後です。 図 16 は、視覚化ウィンドウを示します。
...
図 19: あやめの 2 クラス分類の Web サービスの結果
...
これにはいくつかの作業を Azure Machine Learning の組み込み [マッチ ボックス レコメンダーのトレーニング] の [トレーニング マッチ ボックス レコメンダー] モジュールを使用して行うことがあります。 
...
4 つのオプションから選択することを選択できます、 **Recommender prediction kind** 右側のパネルでメニュー。 ここでは、4 つのすべてのシナリオについて説明します。 レコメンダー システム用の一般的な Azure Machine Learning の実験は、図 20 のようになります。 「」を参照してください。 これらのレコメンダー システム モジュールを使用する方法の詳細ページの [トレーニングのマッチ ボックス レコメンダー] [トレーニング マッチ ボックス レコメンダー] および [スコア マッチ ボックス レコメンダー] [スコア マッチ ボックス レコメンダー] に役立ちます。
...
評価の予測を選択して、 **Recommender prediction kind** ] メニューの [特定のユーザーと項目に対する評価を予測し、レコメンダー システムを願いします。 [スコア マッチ ボックス レコメンダー] [スコア マッチ ボックス レコメンダー] 出力の視覚化は、図 21 のようになります。
...
選択して **項目の推奨** で、 **Recommender prediction kind** ] メニュー、レコメンダー システム、特定のユーザーに対して項目を勧めるようです。 このシナリオでは、Recommended item selection パラメーターも選ぶ必要があります。 オプション **From Rated Items (for model evaluation)** は主に、トレーニング プロセス中のモデル評価します。 この予測ステージを選択します **From All Items**します。 [スコア マッチ ボックス レコメンダー] [スコア マッチ ボックス レコメンダー] 出力の視覚化は、図 22 のようになります。
...
[Recommender prediction kind] メニューで関連するユーザーを選択し、レコメンダー システムで特定のユーザーに関連するユーザーを検索するよう指定します。 関連するユーザーとは、類似する嗜好を持つユーザーのことです。 このシナリオでは、Related user selection パラメーターも選ぶ必要があります。 オプション [From Users That Rated Items (for model evaluation)] は主に、トレーニング プロセス中のモデルの評価で使用します。 この予測ステージでは、[From All Users] を選択します。 [スコア マッチ ボックス レコメンダー] [スコア マッチ ボックス レコメンダー] 出力の視覚化は、図 23 のようになります。
...
選択して **関連アイテム** で、 **Recommender prediction kind** ] メニューの [お願い、レコメンダー システム指定の項目に関連する項目を検索します。 関連アイテムとは、同じユーザーが満足する可能性が最も高い項目です。 このシナリオでは、Related item selection パラメーターも選ぶ必要があります。 オプション **From Rated Items (for model evaluation)** は主に、トレーニング プロセス中のモデル評価します。 選択した **From All Items** この予測ステージ。 [スコア マッチ ボックス レコメンダー] [スコア マッチ ボックス レコメンダー] 出力の視覚化は、図 24 のようになります。
 
![screenshot_of_experiment](./media/machine-learning-interpret-model-results/24.png)

図 24: レコメンダー システムのスコア結果を視覚化する - 関連項目

6 つの列があります。 最初の列は、入力データによって提供される、関連する項目の検索に使用される特定の項目 ID を表示します。 残りの 5 つの列は、項目に関連することが予測される項目を、関連性が高い順に並べ替えて表示します。 たとえば、最初の行では、項目 135026 に最も関連する項目は 135074 であり、その後に 135035、132875、135055、134992 が続きます。 
Web サービスの発行
