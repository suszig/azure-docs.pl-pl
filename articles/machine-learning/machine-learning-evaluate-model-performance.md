...
[モデルの評価 ][evaluate-model] (または 2 つのモデルのパフォーマンスを比較したい場合は 2 つ) を入力としてスコア付けされたデータセットが必要です。 これを使用して、モデルをトレーニングする必要があることを意味、 [[モデルのトレーニング] モデルのトレーニング][train-model] データセット モジュールと make の予測、 [モデルのスコア付け ][score-model] モジュールで、結果を評価する前にします。 評価に基づいて、スコア付けされたラベル/確率と実際のラベル、これらはすべて、出力で、 [モデルのスコア付け ][score-model] モジュールです。
...
- [[評価モデル] のモデルを評価します。][evaluate-model]


図 1 のように、ポートに接続し、ラベル列の設定、 [[モデルのトレーニング] モデルのトレーニング][train-model] モジュールを *価格*します。
...
実験を実行した後の出力ポートをクリックすることができます、 [モデルの評価 ][evaluate-model] モジュールを選択 *視覚化* 評価結果を確認します。 回帰モデルで使用できる評価メトリックは: *平均絶対誤差*、*二乗平均絶対誤差*、*相対絶対誤差*、*相対二乗誤差*、*決定係数*です。
...
図 5 のように、ポートに接続し、ラベル列の設定、 [[モデルのトレーニング] モデルのトレーニング][train-model] モジュールを *収入*します。
...
実験を実行した後の出力ポートをクリックすることができます、 [モデルの評価 ][evaluate-model] モジュールを選択 *視覚化* (図 7) の評価結果を確認します。 二項分類モデルで使用できる評価メトリックは、*精度*、*正確度*、*再現率*、*F1 スコア*、*AUC* です。 さらに、このモジュールは、真陽性、偽陰性、偽陽性、真陰性の数を示す混同行列と *ROC*、*正確度/再現性*、*リフト*の曲線を出力します。
...
この実験では使用して、一般的な [あやめ](http://archive.ics.uci.edu/ml/datasets/Iris "あやめ") あやめのさまざまな種類 (クラス) 3 つのインスタンスを含むデータセットです。 事例ごとに 4 つの特徴値 (がくの長さ、がくの幅、花弁の長さ、花弁の幅) があります。 前の実験では、同じデータセットを使ってモデルのトレーニングとテストを行いました。 ここを使用して、 [分割 ][split] モジュールをデータの 2 つのサブセットを作成、トレーニングを最初に、スコア付け/評価 2 回目です。 
...
ラベル列のインデックスを設定、 [[モデルのトレーニング] モデルのトレーニング][train-model] モジュール 5 にします。 このデータセットにはヘッダー行がありませんが、クラス ラベルが第 5 列にあります。
