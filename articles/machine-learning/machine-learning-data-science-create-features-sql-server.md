...
> [AZURE.NOTE] 実用的な例を参照してください、 [NYC タクシー データセット](http://www.andresmh.com/nyctaxitrips/) 」というタイトルの IPNB を参照および [NYC データの処理 IPython Notebook と SQL Server を使用して](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/iPythonNotebooks/machine-Learning-data-science-process-sql-walkthrough.ipynb) エンド ツー エンド チュートリアルについては、です。
...
* データは SQL Server に格納されている。 そうでない場合は、次を参照してください。 [Azure Machine Learning for Azure SQL Database にデータを移動](machine-learning-data-science-move-sql-azure.md) がデータを移動する方法についてです。
...
3. [1 つの列から特徴の展開](#sql-featurerollout)
...
### <a name="sql-featurerollout"></a>1 つの列から特徴の展開

このセクションでは、テーブル内の 1 つの列を展開して追加の特徴を生成する方法を示します。 この例は、特徴を生成しようとするテーブルに、緯度や経度の列があることを前提としています。

緯度/経度の位置データの簡単な概要を次に示します (stackoverflow からのリソース `http://gis.stackexchange.com/questions/8650/how-to-measure-the-accuracy-of-latitude-and-longitude`)。 これは、Featurize する前に [場所] フィールドを理解するうえで役立ちます。
...
位置情報の Featurize は、地域、位置、および都市の情報に分けて、次のように実行します。 1 回で使用できる Bing Maps API などの REST エンドポイントを呼び出すこともなお `https://msdn.microsoft.com/library/ff701710.aspx` 地域または地区の情報を取得します。
...
> [AZURE.TIP] お好みのプログラム言語でレコードを挿入できます。 書き込み効率を向上させるためにチャンク単位でデータを挿入する必要があります [をチェック アウトは、ここで pyodbc を使用してこれを行う方法の例では、](https://code.google.com/p/pypyodbc/wiki/A_HelloWorld_sample_to_access_mssql_with_python)します。 


> [AZURE.TIP] 別の方法としては、を使用してデータベースにデータを挿入する [BCP ユーティリティ](https://msdn.microsoft.com/library/ms162802.aspx)
...
## <a name="python"></a>Python などのプログラミング言語を使用します。
