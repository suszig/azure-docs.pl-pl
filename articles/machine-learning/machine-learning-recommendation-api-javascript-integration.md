...
1.(ページ レンダリング フェーズ) 上のサーバーでは、(データ マーケット) 経由で Azure ML Recommendations Server を呼び出し、推奨事項を取得します。 結果には項目 ID の一覧が含まれます。 ご利用のサーバーでは、画像や説明などのメタ データ項目で結果に情報を追加し、作成されたページをブラウザーに送信する必要があります。

![図面 2][2]

2.その他のオプションでは、第 1 フェーズから小さい JavaScript ファイルを使用して、推奨される項目の簡単な一覧を取得します。 ここで受信するデータは最初のオプションよりも少なくなります。
...
2. < DataMarketUser > をエンコードするには: < dataMarketKey > base64 とします。 (これは基本認証で使用され、JS コードで API を呼び出すために使用できます)。
...
2.  次の URL から Recommendation Javascript ライブラリを含める: http://1drv.ms/1Aoa1Zp
...
- itemDescription (文字列、省略可能) – 項目の説明
- itemCategory (文字列、省略可能) – 項目のカテゴリ
- (文字列配列、省略可能) – 推奨クエリの生成のシードにシードを設定します。
- recoList (文字列配列、省略可能) – がクリックされた項目を生成した推奨設定要求の結果。
...
パラメーター:
* event (文字列) –「購入」
* 項目 (を購入した場合) – 各項目を購入したエントリを保持する配列。<br><br>
購入済みの形式:
    * item (文字列) - 項目の一意の識別子。
    * 購入された項目の数 (int または文字列) – 数。
    * price (float 型または文字列) – 省略可能フィールド – 項目の価格です。
...
パラメーター:
* event (文字列) –"userlogin"
* ユーザー (文字列) – ユーザーの一意の id。
        <script>
            場合 (typeof AzureMLRecommendationsEvent = ="undefined") {AzureMLRecommendationsEvent =;}
            AzureMLRecommendationsEvent.push ({イベント:"userlogin", ユーザー:"ABCD10AA"})。
        </script>
...
パラメーター:
* アイテムを (文字列の配列) – 推奨を取得する 1 つまたは複数のアイテムです。 Fbt ビルドを使用するは、1 つの項目をここで設定できます。 場合、
* numberOfResults (int) – 必要な結果の数。
* includeMetadata (ブール値、省略可能) – かどうかは、'true' に設定するを示します結果のメタデータ フィールドを入力してください。
* Processing function-推奨事項を処理する関数が返されます。 データは配列として返されます。
    * 項目 – 項目の一意の id
    * 名前 – 項目の名前 (場合カタログに存在)
    * レーティング – 推奨の評価
