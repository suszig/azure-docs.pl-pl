...
5. Odszukaj plik modelu z pakietu pickle w plikach wyjściowych z poprzedniego przebiegu.
   ...
   Po uruchomieniu skryptu **iris_sklearn.py** plik modelu został zapisany w folderze `outputs` pod nazwą `model.pkl`. Ten folder znajduje się w środowisku wykonywania wybieranym w celu uruchomienia skryptu, a nie w lokalnym folderze projektu. 
...
6. Poczekaj, aż w panelu zadań po prawej stronie okna aplikacji Machine Learning Workbench najnowsze zadanie **iris_score.py** pojawi się z zielonym statusem **Ukończone**. Następnie kliknij hiperlink **iris_score.py [1]** dla najnowszego przebiegu zadania, aby wyświetlić szczegóły przebiegu z przebiegu **iris_score.py**. 

7. Na stronie właściwości przebiegu w sekcji **Dane wyjściowe** wybierz nowo utworzony plik **service_schema.json**. Zapisz plik w folderze głównym projektu.

8. Wróć do karty, w której został otwarty skrypt **iris_score.py**. 
...
10. Przejrzyj poniższe wiersze kodu w wystąpieniach klasy ModelDataCollector utworzonych przez funkcję `init()`:
...
1. Otwórz interfejs wiersza polecenia w aplikacji Azure Machine Learning Workbench — w tym celu w menu Plik kliknij polecenie **Otwórz wiersz polecenia**.
   ...
   Wiersz polecenia otworzy się w bieżącej lokalizacji folderu projektu, czyli w `c:\temp\myIris>`.

2. Upewnij się, że dostawca zasobów platformy Azure `Microsoft.ContainerRegistry` został zarejestrowany w ramach Twojej subskrypcji. Należy zarejestrować tego dostawcę zasobów, aby można było utworzyć środowisko w kroku 3. Można sprawdzić, czy został on już zarejestrowany za pomocą następującego polecenia:
   ``` 
...
3. Utwórz środowisko. Ten krok musi być uruchamiany raz na każde środowisko — na przykład środowisko deweloperskie lub produkcyjne. Dla pierwszego środowiska użyj _trybu lokalnego_. (Aby później skonfigurować środowisko w _trybie klastra_, możesz w poniższym poleceniu wypróbować przełącznik `-c` lub `--cluster`).
...
6. Ustaw środowisko.
Po zakończeniu konfiguracji ustaw zmienne środowiskowe wymagane do obsługi operacji — w tym celu użyj następującego polecenia. Nazwa środowiska to nazwa użyta w kroku 1 powyżej. Nazwa grupy zasobów stanowi dane wyjściowe tego samego procesu i byłaby widoczna w oknie wiersza polecenia po ukończeniu procesu konfiguracji.
   ```azurecli
   az ml env set -n <deployment environment name> -g <existing resource group name>
   ```

   Aby sprawdzić, czy środowisko obsługi operacji zostało poprawnie skonfigurowane na potrzeby lokalnego wdrożenia usługi internetowej, wprowadź poniższe polecenie:
...
## <a name="create-a-real-time-web-service"></a>Tworzenie usługi internetowej czasu rzeczywistego
Użyj poniższego polecenia, aby utworzyć usługę internetową czasu rzeczywistego:
   ...
   Poniżej przedstawiono przełączniki dla polecenia `az ml service create realtime`:
   ...
   * --collect-model-data true: umożliwia zbieranie danych
   ...
   Po uruchomieniu polecenia plik modelu i plik oceniania są ładowane do konta magazynu utworzonego w ramach konfiguracji środowiska powyżej. Proces wdrażania tworzy obraz platformy Docker zawierający model, schemat oraz plik oceniania, a następnie wypycha go do rejestru ACR: `<ACR_name>.azureacr.io/<imagename>:<version>`. Następnie ściąga ten obraz na komputer lokalny i uruchamia kontener platformy Docker oparty na tym obrazie. (Jeśli środowisko jest konfigurowane w trybie klastra, kontener platformy Docker zostaje wdrożony do klastra ACS Kubernetes).
   ...
   Polecenie `docker ps` umożliwia sprawdzenie działającego kontenera platformy Docker:
   ```azurecli
   docker ps
   ```
### <a name="alternative-route"></a>Trasa alternatywna
Zamiast korzystać z polecenia `az ml service create realtime` pokazanego powyżej, można kroki rejestracji modelu, generowania manifestu, budowania obrazu platformy Docker i tworzenia usługi internetowej wykonać osobno. Takie postępowanie zapewnia większą elastyczność na każdym kroku, jednostki generowane w poszczególnych krokach mogą być używane ponownie i możliwe jest ponowne kompilowanie jednostek w razie potrzeby. Wykonaj poniższe instrukcje, aby zobaczyć, jak można to osiągnąć:
...
1. Zarejestruj model, podając nazwę pliku z pakietu pickle.
   ...
   ```
   Spowoduje to wygenerowanie identyfikatora modelu.
...
2. Tworzenie manifestu
   ...
   ```
   Spowoduje to wygenerowanie identyfikatora manifestu.
...
3. Tworzenie obrazu platformy Docker
   ...
   ```
   Spowoduje to wygenerowanie identyfikatora obrazu platformy Docker.
...
4. Tworzenie usługi
   ...
   ```
   Spowoduje to wygenerowanie identyfikatora usługi internetowej.
...
1. Podczas tworzenia usługi internetowej korzystaliśmy z przykładowych danych. Podczas pracy w trybie lokalnym możesz wywołać polecenie `az ml service show realtime`, aby uzyskać przykładowe polecenie uruchamiania, którego można używać do testowania usługi. Dzięki temu można również uzyskać adres URL oceniania, którego można użyć, aby wprowadzić usługę do własnej aplikacji niestandardowej:
...
6. Te dane z obiektów blob platformy Azure można wykorzystać na różne sposoby, używając oprogramowania firmy Microsoft i narzędzi typu open source. 

   Przykład przybliża korzystanie z wyjściowych obiektów blob:
