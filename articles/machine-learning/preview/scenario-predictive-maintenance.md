...
Wpływ urządzeń nieplanowanych przestojów mogą być szkodliwe dla dowolnego rodzaju działalności. Jest krytyczne w związku z tym pola Zachowaj urządzeń uruchomiona w celu zmaksymalizowania wykorzystania i wydajności i minimalizując kosztowne, nieplanowanych przestojów. Wczesne identyfikację problemów mogą pomóc przydzielić zasoby obsługi ograniczony w sposób ekonomiczny i zwiększyć jakość i procesy łańcucha dostaw. 

W tym scenariuszu używamy stosunkowo [danych na dużą skalę](https://github.com/Microsoft/SQL-Server-R-Services-Samples/tree/master/PredictiveMaintanenceModelingGuide/Data) przeprowadzenie użytkownika przez główne kroki z wprowadzanie danych, funkcji zespołu inżynieryjnego, konstruowania modelu i następnie koniec operationalization modelu i wdrożenia. Kod dla całego procesu są zapisywane w PySpark i implementowane za pomocą notesów Jupyter środowiska roboczego uczenia Maszynowego Azure. Najlepsze modelu jest ostatecznie operationalized przy użyciu Azure Machine Learning Model zarządzania do użycia w środowisku produkcyjnym dokonywania prognoz błędu czasu rzeczywistego.   

## <a name="link-to-the-gallery-github-repository"></a>Połącz z repozytorium GitHub galerii

Poniżej znajduje się łącze do publicznego repozytorium GitHub: [https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance)
...
4.  W **szablony projektów wyszukiwania** pole wyszukiwania, wpisz "Konserwacji predykcyjnej" i wybierz szablon
5.  Kliknij przycisk **Utwórz**

## <a name="prepare-the-notebook-server-computation-target"></a>Przygotowanie docelowy obliczeniowym serwera notesu

Uruchamianie na komputerze lokalnym, z AML Workbench `File` menu, wybierz opcję `Open Command Prompt` lub `Open PowerShell CLI`. W ramach interfejsu wiersza polecenia systemu windows, wykonaj następujące polecenia:

`az ml experiment prepare --target docker --run-configuration docker`

Zalecamy uruchomienie maszyny wirtualnej na dane nauki dla systemu Linux (Ubuntu). Po skonfigurowaniu DSVM, uruchom następujące dwa polecenia:
...
Przy użyciu obrazów docker przygotowany, otwórz serwera notesu Jupyter w karcie notesów AML Workbench lub uruchamiania serwera opartego na przeglądarce, uruchom: `az ml notebook start`.

Notesów są przechowywane w `Code` katalogu w środowisku Jupyter. Przeprowadzana notebooki sekwencyjnie, uruchomienie na pierwszy (`Code\1_data_ingestion.ipynb`) notesu. Po otwarciu każdego notesu monit o wybierz jądro obliczeniowe. Wybierz _Template [Project_Name] [Desired_Connection_Name], a następnie kliknij polecenie Ustaw jądra.
...
* [Dane telemetryczne](https://pdmmodelingguide.blob.core.windows.net/pdmdata/telemetry.csv): dane telemetryczne szeregów czasowych składa się z napięcia, obracanie, wykorzystania i wibrację pomiarów czujnika z każdej maszyny w czasie rzeczywistym. Dane są średnio ponad godzinę i przechowywane w dziennikach telemetrii
...
W repozytorium, Brak [Readme](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/README.md) pliku, który zawiera opis procesów, od przygotowania do budowania modeli kilka, a następnie finally operacyjnych jednym z najlepszych modeli danych. Dostępne są cztery notesów Jupyter [kod](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/tree/master/Code) folder w repozytorium.   

Następnie opisano krok po kroku scenariusza przepływ pracy. Scenariusz pełnego są zapisywane w PySpark i jest podzielony na cztery komputery przenośne:

[`Code\1_data_ingestion.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/1_data_ingestion.ipynb): Ten notes pobiera pięć plików wejściowych CSV, niektóre Oczyszczanie wstępne danych i wizualizacji. Notesu konwertuje je na PySpark format i przechowuje wyniki w kontenerze obiektów blob platformy Azure do użycia w zadaniu engineering funkcji.

[`Code\2_feature_engineering.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/2_feature_engineering.ipynb): Za pomocą oczyszczony zestaw danych z poprzedniego kroku, opóźnienia i zagregowane funkcje są tworzone dla zastępcze składnika czujniki telemetrii i liczby błędów, informacje o maszynie są przyłączone do danych telemetrycznych. Zastępcze dotyczące awarii składników są używane do tworzenia etykiety opisujące, których nie powiodła się. Dane funkcji etykietą jest zapisywany w obiektów blob platformy Azure dla modelu, tworzenie zadań.
...
[`Code\4_operationalization.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/4_operationalization.ipynb): Za pomocą modelu ostatni zapisany na dysku lokalnym (jądro notesu Jupyter), ten notes kompilacje składniki operacyjnych modelu do usługi sieci web platformy Azure. Pełnej eksploatacji zasoby są kompaktowanie do `o16n.zip` plików przechowywanych w innym kontenerze obiektów blob platformy Azure. Plik zip zawiera:
...
W tym scenariuszu zestawienie czytnik jak zbudować rozwiązanie do konserwacji predykcyjnej kompleksowe przy użyciu PySpark w środowisku notesu Jupyter w Workbench uczenia Maszynowego Azure. Scenariusz prowadzi również czytnik na jak najlepsze modelu można łatwo operationalized i wdrażane za pomocą środowiska Azure Machine Learning Model zarządzania do użycia w środowisku produkcyjnym dokonywania prognoz błędu czasu rzeczywistego. Następnie czytnik edytować odpowiednich części scenariusza, aby taper go do potrzeb biznesowych.  
