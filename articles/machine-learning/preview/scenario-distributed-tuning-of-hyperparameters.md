...
Popularne metoda dostrajania hyperparameters *wyszukiwania siatki* połączeniu z *krzyżowego sprawdzania poprawności*. Krzyżowe sprawdzanie poprawności to technika, który ocenia, jak model uczenia na zestaw szkoleniowy prognozuje wyznaczonych dla zestawu testów. Ta technika początkowo możemy podzielić zestawu danych na złożeń K i następnie pociągu razy algorytm K, w sposób okrężnego na wszystkie oprócz jednego złożeń, nazywany złożenia przechowywane poza. Średnia wartość metryki modeli K możemy obliczeniowego za pośrednictwem K przechowywane poza złożeń. Wartość średnia, nazywany *oszacowania wydajności krzyżowego sprawdzania poprawności*, zależy od wartości hyperparameters używany podczas tworzenia modeli K. Dostrajanie hyperparameters, możemy wyszukiwać za pośrednictwem obszaru candidate hyperparameter wartości, aby dowiedzieć się, że te, które optymalizacji wydajności krzyżowego sprawdzania poprawności oszacować. Wyszukiwanie siatki jest typowe techniki wyszukiwania, gdzie przestrzeni wartości candidate hyperparameters wielu jest iloczyn wektorowy zestawów wartości candidate hyperparameters poszczególnych. 

Przy użyciu krzyżowego sprawdzania poprawności wyszukiwania siatki może być czasochłonne. Jeśli algorytm ma hyperparameters 5, 5 wartości kandydata i używamy złożeń K = 5, a następnie przeprowadzenie wyszukiwania siatki musimy uczenia 5<sup>6</sup>= 15625 modeli. Na szczęście siatki wyszukiwania przy użyciu krzyżowego sprawdzania poprawności jest embarrassingly równoległych procedury i może być uczony te modele równolegle.
...
* Aby uruchomić ten scenariusz z klastrem Spark, udostępniania klastra usługi HDInsight, postępując [instrukcje](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-hadoop-provision-linux-clusters). Zaleca się o klastrze z co najmniej sześciu węzłów procesu roboczego i co najmniej 8 rdzeni i 28 Gb pamięci w węzłach zarówno nagłówka i proces roboczy. D4 wystąpień maszyn wirtualnych ma takie wydajności. Aby zmaksymalizować wydajność klastra, firma Microsoft zaleca, aby zmienić parametry spark.executor.instances, spark.executor.cores i spark.executor.memory, postępując [instrukcje](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-apache-spark-resource-manager) i edycji definicji w niestandardowych" Platforma Spark jest domyślne"sekcji.
...
* Tworzenie konta magazynu Azure używanego do przechowywania zestawu danych. Wykonaj [instrukcje](https://docs.microsoft.com/en-us/azure/storage/common/storage-create-storage-account) można utworzyć konta magazynu.
...
W następnych krokach możemy nawiązać środowiska wykonawczego konto platformy Azure. Okno Otwórz wiersz polecenia (CLI), klikając menu Plik w lewym górnym rogu AML Workbench i wybierając polecenie "Otwórz okno wiersza polecenia". Następnie uruchom w interfejsu wiersza polecenia
...
i Znajdź subskrypcji subskrypcji identyfikator platformy Azure, która ma konto AML Workbench roboczym. Na koniec uruchom w interfejsu wiersza polecenia
...
Po wykonaniu tej Wybierz kontener zestawu danych z listy i kliknij przycisk przekazywania. Azure portal umożliwia przekazywania wielu plików jednocześnie. W sekcji "Przekazania obiektu blob" kliknij przycisk folderu, wybierz wszystkie pliki z zestawu danych, kliknij przycisk Otwórz, a następnie kliknij przekazywania. Na poniższym zrzucie ekranu przedstawiono następujące kroki:
...
W naszym kodzie używamy [zestawu SDK usługi Magazyn Azure](https://azure-storage.readthedocs.io/en/latest/) pobierania zestawu danych z magazynu obiektów blob do aktualnego środowiska wykonawczego. Pobieranie jest wykonywane w obciążenia\_funkcja funkcji data() z pliku load_data.py. Aby użyć tego kodu, należy zastąpić < nazwa_konta > i < ACCOUNT_KEY > o nazwie i klucz podstawowy konta magazynu, który jest hostem zestawu danych. Nazwa konta jest wyświetlany w lewym górnym rogu strony systemu Azure konta magazynu. Można uzyskać konta typu klucza, wybierz klucze dostępu Azure stronie magazynu konta, (Zobacz pierwszy zrzut ekranu w sekcji wprowadzanie danych), a następnie skopiuj długi ciąg w pierwszym wierszu kolumny klucza:
...
Zwróć uwagę, że nie trzeba ręcznie uruchom plik load_data.py. Później będzie ona wywoływana z innych plików.
...
* Wskaźnik funkcji dla kategorii aplikacji, które były używane przez używany do wygenerowanych zdarzeń (jeden\_hot_category funkcja)
...
* colsample\_by_level
* podpróbki
* cel znajduje się opis tych hyperparameters [tutaj](http://xgboost.readthedocs.io/en/latest/python/python_api.html#module-xgboost.sklearn) i [tutaj](https://github.com/dmlc/xgboost/blob/master/doc/parameter.md). Początkowo będziemy używać zdalnego DSVM i dostrajania hyperparameters z małych siatki candidate wartości:

    tuned_parameters = [{"n_estimators": [300,400] "max_depth": [3,4] 'cel': ['multi:softprob"],"reg_alpha": [1],"reg_lambda": [1],"colsample_bytree": [1],"learning_rate": [0,1]"colsample_bylevel": [0,1,],"podpróbki": [0,5]}]  
...
Możemy także utworzyć plik sweeping_results.txt z krzyżowego sprawdzania poprawności ujemna dziennika strat wszystkich kombinacji wartości hyperparameter w siatce:
...
Domyślnie okno Uruchom historii zawiera wartości i wykresy pierwszego zarejestrowane wartości 1 i 2. Aby zapoznać się z pełną listą wybrane wartości hyperparameters, kliknij ikonę ustawień oznaczonej jako czerwone kółko na poprzednim zrzucie ekranu i wybierz pozycję hyperparameters ma być wyświetlany w tabeli. Ponadto wybierz wykresy, które są wyświetlane w górnej części okna Uruchom historii, kliknij ikonę ustawienia oznaczonej jako niebieskie koło i wybierz wykresów z listy. 
...
W prawym górnym rogu okna właściwości Uruchom jest sekcji plików wyjściowych z listą wszystkich plików, które zostały utworzone w ". \output" folderu w środowisku wykonywania. zmiennymi\_results.txt można pobrać z tego miejsca, wybierając ją i klikając przycisk Pobierz. sweeping_results.txt powinny mieć następujące dane wyjściowe:
...
To polecenie zakończy się w 1 godzina 6 minut po klastra Spark ma 6 węzłów procesu roboczego z 28 Gb pamięci. Wyniki dostrajanie hyperparameters w klastrze Spark, czyli dzienniki, najlepsze wartości pliku hyperparameters i sweeping_results.txt, są dostępne w konsoli usługi Azure Machine Learning Workbench w taki sam sposób jak zdalne wykonywanie DSVM. 

### <a name="architecture-diagram"></a>Diagram architektury

Na poniższym diagramie przedstawiono przepływ pracy na trasie: ![architektury](media/scenario-distributed-tuning-of-hyperparameters/architecture.png) 

## <a name="conclusion"></a>Podsumowanie 

W tym scenariuszu firma Microsoft pokazano, jak używać usługi Azure Machine Learning Workbench do dostrojenie hyperparameter w zdalnej maszyny wirtualnej i zdalnego klastra Spark. Widzieliśmy się, że usługi Azure Machine Learning Workbench udostępnia narzędzia, aby ułatwić konfigurację środowiska wykonania i przełączania między nimi. 
