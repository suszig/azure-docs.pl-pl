...
## クロス検証のフォールドを定義する
データにフォールドをランダムに割り当てるには、[パーティションとサンプル] の [パーティションとサンプル] モジュールを使用できます。 次の図は、5 つのフォールドを定義し、サンプル インスタンスにフォールド番号をランダムに割り当てたモジュール構成の例を示しています。 

![image4](./media/machine-learning-algorithm-parameters-optimize/fig4.png)


## メトリックを定義する:
[スイープ パラメーター] の [パラメーター スイープ] モジュールでは、特定のアルゴリズムおよびデータセットに対してパラメーターの最適なセットを経験的に選択するためのサポートを提供します。 このモジュールのプロパティ ウィンドウには、モデルのトレーニングに関連した情報のほかに、最適なパラメーター セットを決定するために使用するメトリックが含まれています。 また、分類アルゴリズムと回帰アルゴリズムに関する 2 つの異なるドロップダウン リストがあります。 検討しているアルゴリズムが分類アルゴリズムの場合は回帰のメトリックが無視され、その逆も同じです。 選択したこの特定の例では **精度** メトリックとして。   
 
![image5](./media/machine-learning-algorithm-parameters-optimize/fig5.png)

## トレーニング、評価、および比較を行う:  
同じ [スイープ パラメーター] の [パラメーター スイープ] モジュールの電車パラメーターに対応するすべてのモデルは、設定、さまざまなメトリックを評価し、その後、ユーザーが選択したメトリックに基づいて最適なトレーニング済みモデルを出力します。 このモジュールには、次の 2 つの必須の入力があります。
