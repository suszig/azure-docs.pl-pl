...
1.  ログイン、 [Azure Classic Portal](http://manage.windowsazure.com/) 、アカウントを使用します。
    Azure アカウントがいない場合は、[Azure の無料を参照してください。
    試用版サイト」(http://www.windowsazure.com/pricing/free-trial/)。
...
4.  最初の **仮想マシンの構成** ページで指定します
    ...
    -   **[新しいパスワード]** ボックスに、強力なパスワードを入力します。 サブスクリプションの
        についてを参照してください [強力なパスワードの](http://msdn.microsoft.com/library/ms161962.aspx)します。
    ...
    仮想マシンに接続できるデータ ディスクは、[仮想を参照してください。
    マシンのサイズ
    Azure] (http://msdn.microsoft.com/library/azure/dn197896.aspx)。 料金情報については、次を参照してください。 [仮想マシンの料金](http://azure.microsoft.com/pricing/details/virtual-machines/)します。
...
6.  **エンドポイント** セクションで、下にある空のドロップダウンでをクリックして **名**, と
...
7.  また、SQL Server VM を、後の手順で構成される IPython Notebook Server として機能させることもできます。
    IPython Notebook サーバー用に使われるポートを指定するために新しいエンドポイントを追加してください。 名前を入力、 **名** 列で、パブリック ポート用に任意のポート番号を選択して 9999 プライベート ポートを指定します。
...
## <a name="RemoteDesktop"></a>リモート デスクトップと完全なセットアップを使用して仮想マシンを開く
...
4.  最初にログオンしているをこの仮想マシンに複数のプロセス
    ...
    これらが完了するまでの数分の間です。 `SELECT @@SERVERNAME` ことはできません。
...
## <a name="InstallIPython"></a>IPython Notebook とその他のサポート ツールをインストールします。
...
<br>
- アクセスして、フォームの URL を使用してローカルまたはリモート ブラウザーから IPython Notebook を実行することが `https://<virtual_machine_DNS_name>:&lt;port&gt;`, ポートは仮想マシンのプロビジョニング中に選択した IPython パブリック ポートであるという点です。
- IPython Notebook サーバーはバックグラウンド サービスとして実行され、仮想マシンの再起動時に自動的に再起動されます。

## <a name="Optional"></a>必要に応じてデータ ディスクをアタッチします。
...
## <a name="SSMS"></a>SQL Server Management Studio に接続し、混合モード認証を有効にします。
...
## <a name="Logins"></a>SQL Server 認証ログインを作成します。

別のコンピューターからデータベース エンジンに接続するに作成する必要があります。
1 つ以上の SQL Server 認証ログインです。
> [AZURE.TIP] 新しい SQL Server ログインをプログラムによって作成することも、SQL Server Management Studio を使って作成することもできます。SQL 認証の sysadmin ユーザーをプログラムで新規作成するには、**[新規クエリ]** を開始し、次のスクリプトを実行します。置換 <new user name\> と <new password\> をお好みのユーザー名とパスワード。必要に応じてパスワード ポリシーを調整します (サンプル コードではポリシー検査とパスワード期限設定がオフになっています)。SQL Server ログインの詳細については、次を参照してください。 [ログインを作成](http://msdn.microsoft.com/library/aa337562.aspx)します。  
...
5.  **[パスワード]** ボックスに、新しいユーザーのパスワードを入力します。 次に、
...
7.  有効期限に関するパスワード ポリシー オプションを適用するには、次のように選択します * * 適用します。。
...
## <a name="DNS"></a>仮想マシンの DNS 名を特定します。
...
## <a name="cde"></a>別のコンピューターからデータベース エンジンへの接続します。

1.  インターネットに接続されたコンピューターで、SQL Server Management Studio を開きます。

2.  **サーバーへの接続** または **データベース エンジンに接続する**
...
## <a name="amlconnect"></a>Azure Machine Learning からデータベース エンジンへの接続します。

Advanced Analytics Process and Technology の後の段階では使用して、 [Azure Machine Learning Studio](https://studio.azureml.net) をビルドし、機械学習モデルを展開します。 取り込むには SQL Server VM データベースからデータを直接 Azure Machine Learning のトレーニングまたはスコアリングのために、新しいリーダー モジュールを使用して [Azure Machine Learning Studio](https://studio.azureml.net) 実験します。 このトピックの詳細は Advanced Analytics Process and Technology のガイド リンクに記載されています。 概要については、を参照してください [Azure Machine Learning Studio は何ですか?](machine-learning-what-is-ml-studio.md)。.
...
3.  **データベース サーバー名** テキスト ボックスに、入力 `tcp: < 仮想マシンの DNS 名 > 1433年`
...
## <a name="shutdown"></a>シャット ダウンし、未使用時に仮想マシンの割り当てを解除

Azure Virtual Machines の料金は**従量課金制**です。 仮想マシンを使用しないときに課金されないようにするには、**[停止 (割り当て解除)]** 状態にする必要があります。
> [AZURE.NOTE] (Windows 電源オプションを使用して) 内部から仮想マシンをシャット ダウンすると、その VM は停止しますが、割り当てられた状態のままになります。 課金されないしていることを確認するには、常に仮想マシンを停止、 [Azure Classic Portal](http://manage.windowsazure.com/)します。 また、Powershell で VM を停止することもできます。その際、ShutdownRoleOperation 呼び出しで "PostShutdownAction" を "StoppedDeallocated" にしてください。
