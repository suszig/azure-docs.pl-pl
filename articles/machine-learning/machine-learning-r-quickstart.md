...
R スクリプトの場合は、この記事で使用されるデータは、 [こちらからダウンロード][download]します。 このデータは、ウィスコンシン大学 (http://future.aae.wisc.edu/tab/production.html) から入手した情報を元に作成したものです。
...
RStudio の使用に関する追加情報を提供した [付録 A][appendixa]します。  
...
6.  型、 `source()` してコード ウィンドウに、zip ファイルの名前を持つ関数、 [R スクリプトの実行][execute-r-script] モジュールです。 たとえば、「`source("src/SimplePlot.R")`」と入力します。  
...
をクリックして、実験を実行、 **実行** ] ボタンをクリックします。 実行が完了したら、をクリックして、 [R スクリプトの実行][execute-r-script] クリックしてモジュール **出力ログの表示** プロパティ ペインでします。 ブラウザーで新しいページが開き、output.log ファイルの内容が表示されます。 下へスクロールすると、次のことがわかります。
