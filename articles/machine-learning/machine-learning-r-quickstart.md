...
データと R スクリプトは、この記事で使用されて、[こちらからダウンロードできます] [ダウンロード] このデータは、ウィスコンシン大学 (http://future.aae.wisc.edu/tab/production.html) から入手した情報を元に作成したものです。
...
R コードの小規模な開発、テスト、デバッグを行うには、RStudio を使用します。 このコードは、印刷、Machine Learning Studio を実行できる状態で [R スクリプトの実行] [- r のスクリプトの実行] モジュールに貼り付けます。  

###R スクリプトの実行モジュール

Machine Learning Studio 内では、R スクリプトは、[R スクリプトの実行] の [の r のスクリプトの実行] のモジュール内で実行されます。 Machine Learning Studio で [R スクリプトの実行] の [の r のスクリプトの実行] モジュールの例は、図 1 に示します。

 ![R プログラミング言語: Machine Learning Studio で選択された Execute R Script モジュール][1]

*図 1. Machine Learning Studio 環境は、選択された R スクリプトの実行モジュールを表示します。*

図 1 を参照して、[R スクリプトの実行] の [の r のスクリプトの実行] モジュールを操作するための Machine Learning Studio 環境の重要な部分の一部を見てみましょう。

- 実験するモジュールが、中央のウィンドウに表示されています。

- 左側のウィンドウの上部には、R スクリプトを表示し編集するウィンドウがあります。  

- 右側のペインの下の部分は、[R スクリプトの実行] [- r のスクリプトの実行] の一部のプロパティを示しています。 このウィンドウの該当する位置をクリックすると、エラー ログと出力ログを表示できます。

もちろん、について説明する [R スクリプトの実行] [- r のスクリプトの実行] でこのドキュメントの残りの部分で詳しくします。

複雑な R 関数を操作するときには、RStudio 内で編集、テスト、デバッグすることをお勧めします。 どのようなソフトウェア開発でも、コードを徐々に拡張し、小さい単純なテスト ケースでテストします。 切り取りし、[R スクリプトの実行] の [の r のスクリプトの実行] モジュールの R スクリプト ウィンドウに、関数を貼り付けます。 このアプローチでは、RStudio 統合開発環境 (IDE) と Azure Machine Learning の両方の機能を活用できます。  

####R コードの実行

をクリックして、実験を実行するときに、[R スクリプトの実行] [- r のスクリプトの実行] のモジュール内の R コードが実行、 **実行** ] ボタンをクリックします。 実行が完了したら、チェック マークは、[R スクリプトの実行] の [の r のスクリプトの実行] アイコンに表示されます。
...
繰り返しますが、RStudio では、小規模な R コードのテストとデバッグを行うことをお勧めします。 ただし、これには [R スクリプトの実行] [- r のスクリプトの実行] 自体での R コードの問題を追跡する必要がある場合があります。 また、Machine Learning Studio で結果をチェックすることもお勧めします。

Azure Machine Learning プラットフォームでの R コード実行の出力は、主に output.log にあります。 また、いくつかの追加情報が error.log で見られます。  

R コード実行中に Machine Learning Studio でエラーが発生した場合、最初にすべきことは error.log を調べることです。 このファイルには、エラーを把握して修正するために役立つ有用なエラー メッセージが含まれています。 Error.log を表示する] をクリックして **エラー ログの表示** 上、 **のプロパティ] ウィンドウ** [R スクリプトの実行] [- r のスクリプトの実行] のエラーを含みます。

たとえば、次の R コード [R スクリプトの実行] の [の r のスクリプトの実行] モジュールでは、未定義変数 y、実行します。

    x <- 1.0
    z <- x + y

このコードは、次のエラー状態で実行に失敗します。 クリックすると **エラー ログの表示** 上、 **のプロパティ] ウィンドウ** 図 2 に示すように内容が表示されます。

  ![エラー メッセージのポップアップ][2]

*図 2. エラー メッセージのポップアップします。*

この場合、output.log で R のエラー メッセージを見る必要がありそうです。 [R スクリプトの実行] [- r のスクリプトの実行] をクリックし、をクリックして、 **output.log を表示** 項目に、 **のプロパティ] ウィンドウ** 右側にします。 新しいブラウザー ウィンドウが開き、次のことがわかります。
...
Azure Machine Learning には、350 を超える R 言語パッケージがプレインストールされています。 [R スクリプトの実行] の [の r のスクリプトの実行] モジュールで次のコードを使用すると、プレインストールされたパッケージの一覧を取得します。
...
RStudio は、広く使用されている R 向けの IDE です。このクイック スタート ガイドで使用される R コードの一部は、RStudio を使用して編集、テスト、デバッグします。 R コードがテストと準備が完了すると、単に切り取って貼り付けます RStudio エディター Machine Learning Studio [R スクリプトの実行] の [の r のスクリプトの実行] モジュールに。  
...
私は、[appendixa] [付録 A] の RStudio の使用に関する追加情報を提供します。  

##<a id="scriptmodule"></a>R スクリプトの実行モジュールとの間のデータを取得します。

このセクションでは [R スクリプトの実行] [- r のスクリプトの実行] モジュールに出入りするデータを取得する方法について説明します。 型の読み取り [R スクリプトの実行] の [の r のスクリプトの実行] モジュールに出入りするさまざまなデータを処理する方法を確認します。
...
読み込むことでは、まず、 **csdairydata.csv** Azure Machine Learning Studio へのファイルです。

- Azure Machine Learning 環境を起動します。

- をクリックして __+ 新規__ 、画面の左下にある **データセット**します。

- 選択 **ローカル ファイルから**, 、し **参照** ファイルを選択します。

- 選択したかどうかを確認 **ヘッダー (.csv) を持つ汎用 CSV ファイル** データセットの種類として。

- チェック マークをクリックします。

- データセットがアップロードされると、] をクリックして新しいデータセットを確認する必要があります、 **データセット** ] タブをクリックします。  

####実験の作成

いくつかのデータを Machine Learning Studio に読み込んだので、分析をするために実験を作成する必要があります。  

- をクリックして __+ 新規__ 下の左と選択 **実験**, 、し **空白の実験**します。

- 選択すると、および変更するには、実験を名前を **の実験の作成に...** 、ページの上部にタイトルです。 たとえば、変更することを **カリフォルニア酪農分析**します。

- 実験ページの左側には、展開 **保存されたデータセット**, 、し **My Datasets**します。 確認する必要があります、 **cadairydata.csv** 以前にアップロードしています。

- ドラッグ アンド ドロップ、 **csdairydata.csv データセット** を実験にします。

-  **実験項目の検索** 左側のウィンドウの上部にあるボックスに、[R スクリプトの実行] [- r のスクリプトの実行] を入力します。 検索一覧に、モジュールが表示されます。

- ドラッグ アンド ドロップ [R スクリプトの実行] [- r のスクリプトの実行] モジュールをパレットに。  

- 出力を接続、 **csdairydata.csv データセット** 左端の入力に (**Dataset1**)、[R スクリプトの実行] [- r のスクリプトの実行] のです。

- **忘れずに [保存] をクリックします。**  

この時点で、実験は図 3 のように表示されます。

![カリフォルニア酪農分析は、データセットと R スクリプトの実行モジュールで実験][3]

*図 3: カリフォルニア酪農分析は、データセットと R スクリプトの実行モジュールを実験します。*

####データのチェック

実験に読み込んだデータを調べましょう。 出力をクリックして、実験で、 **cadairydata.csv データセット** 選択 **視覚化**します。 図 4 の内容が表示されます。  

![Cadairydata.csv データセットの概要][4]

*図 4: Cadairydata.csv データセットの概要です。*
...
[R スクリプトの実行] の [の r のスクリプトの実行] モジュールへの入力を見てみましょう。 この例では、[R スクリプトの実行] の [の r のスクリプトの実行] モジュールに、カリフォルニア酪農データを読み取ります。  

[R スクリプトの実行] の [の r のスクリプトの実行] モジュールの次の 3 つの可能な入力があります。 アプリケーションに応じて、いずれか 1 つ、またはすべてを使用できます。 また、入力のない R スクリプトを使用することも、まったく問題ありません。  

これらの各入力を左から右に見ていきましょう。 カーソルを入力の上に移動してツール ヒントを読むと、各入力の名前がわかります。  

#### スクリプト バンドル

スクリプト バンドル入力では [R スクリプトの実行] [- r のスクリプトの実行] モジュールに渡される zip ファイルの内容を行うことができます。 次のコマンドのいずれかを使用すると、zip ファイルの内容を R コードに読み込むことができます。

    source("src/yourfile.R") # Reads a zipped R script
    load("src/yourData.rdata") # Reads a zipped R data file

> [AZURE.NOTE] Azure Machine Learning は、src 内にあるかのように、zip 内のファイルを扱います/ディレクトリため、ファイル名は、このディレクトリ名のプレフィックスにする必要があります。  
...
3.  ファイルを追加、 **データセット** Machine Learning Studio で、型と型を指定する **zip**します。 データセットに zip ファイルが表示されるようになります。

4.  ドラッグ アンド ドロップの zip ファイルを **データセット** 上に、 **ML Studio キャンバス**します。

5.  出力を接続、 **データを zip** アイコン、 **スクリプト バンドル** [R スクリプトの実行] [- r のスクリプトの実行] モジュールの入力します。

6.  型、 `source()` [R スクリプトの実行] [- r のスクリプトの実行] モジュールのコード ウィンドウに、zip ファイル名を持つ関数です。 たとえば、「`source("src/SimplePlot.R")`」と入力します。  

7.  クリックするかどうかを確認 **保存**します。

次の手順が完了した後の zip ファイルに、実験の実行時に [R スクリプトの実行] の [の r のスクリプトの実行] モジュールに R スクリプトが実行されます。 この時点で、実験は図 5 のように表示されます。

![実験は zip 形式の R スクリプトを使用][6]

*図 5: 実験は zip 形式の R スクリプトを使用します。*
...
をクリックして、実験を実行、 **実行** ] ボタンをクリックします。 実行が完了したら、[R スクリプトの実行] [- r のスクリプトの実行] モジュールをクリックし、 **出力ログの表示** プロパティ ペインでします。 ブラウザーで新しいページが開き、output.log ファイルの内容が表示されます。 下へスクロールすると、次のことがわかります。
...
> [AZURE.NOTE] 出力された同じ内容が [R スクリプトの実行] の [の r のスクリプトの実行] モジュールの R デバイス出力から簡単に入手できます。 次のセクションで、[R スクリプトの実行] の [の r のスクリプトの実行] モジュールの出力はについて説明します。  
...
実験を実行すると、結果データセット 1 出力ポートをクリックし、をクリックして **視覚化**します。 図 6 の内容が表示されます。

![カリフォルニア酪農データの出力の視覚化][7]

*図 6: カリフォルニア酪農データの出力の視覚化します。*

この出力は、まったく予想どおりに、入力と同一になります。  

### R デバイス出力

[R スクリプトの実行] の [の r のスクリプトの実行] モジュールのデバイス出力には、メッセージとグラフィックスの出力が含まれています。 R からの標準出力と標準エラー メッセージは両方とも、R デバイス出力ポートに送信されます。  

表示するには、R デバイス出力ポートをクリックしで **視覚化**します。 R スクリプトからの標準出力と標準エラーが表示されます (図 7 を参照)。

![標準出力と標準エラー、R デバイス ポートから][8]

*図 7: 標準出力と標準エラー、R デバイス ポートからです。*

下へスクロールすると、R スクリプトからのグラフィックス出力が表示されます (図 8 を参照)。  

![R デバイス ポートからのグラフィックス出力][9]

*図 8: グラフィックス、R デバイス ポートから出力します。*  
...
カリフォルニア酪農データ [R スクリプトの実行] の [の r のスクリプトの実行] モジュールの R コードに読み込むことと、その列のデータが目的の型と形式にすることは必要があります。  
...
散布図マトリックスを作成した行を削除し、"Month" 列を因子に変換する行を追加しました。 実験では、だけを切り取って [R スクリプトの実行] の [の r のスクリプトの実行] モジュールのコード ウィンドウに、R コードを貼り付けます。 また、zip ファイルを更新し、それを Azure Machine Learning Studio にアップロードすることもできますが、いくつかの手順が必要になります。  
...
*図 9: 因子変数を持つデータ フレームのサマリー*

月の型は、表示されるはず今すぐ '**Factor w/14 levels**' です。 1 年に 12 か月しかないため、これは問題です。 理解することを確認することも、入力 **視覚化** ポートは、結果データセットの '**Categorical**' です。
...
*図 10: 正しい数の因子レベルを持つデータ フレームのサマリー*
...
> [AZURE.NOTE] これ以降このセクションでを示すコードを追加する [R スクリプトの実行] の [の r のスクリプトの実行] モジュールに追加します。 新しい行を追加 **する前に** 、 `str()` 関数です。 この関数を使用し、Azure Machine Learning Studio で結果を検証します。

[R スクリプトの実行] の [の r のスクリプトの実行] モジュールの R コードを次の行を追加します。
...
*図 11: 2 つの列を持つデータ フレームのサマリーが削除されます。*
...
次のコードで、新しい関数 `log.transform()` を定義し、それを数値を含む行に適用します。 R `Map()` 関数を使用して、適用、 `log.transform()` データ フレームの選択した列に関数です。 `Map()` ような `apply()` が関数の引数の 1 つ以上のリストに対して使用できます。 乗数のリストは、`log.transform()` 関数に第二の引数を提供することに注意してください。 `na.omit()` 関数は、データフレームに不足している値か、未定義の値がないことを確認するクリーンアップの一部として使用されます。
...
*図 13. データ フレームに変換された値の概要です。*

値が変換されたことが確認できます。 現在、牛乳の生産が他のすべての乳製品の生産を大きく超えていますが、対数スケールで見ていることに注意してください。

この時点でデータはクリーンアップされ、モデリングの準備が整いました。 [R スクリプトの実行] [- r のスクリプトの実行] モジュールのに関する視覚化サマリーを結果データセット出力を見ると、"Month"列が"Categorical"で 12 個の一意な値で希望が表示されます。
...
それでは、例の操作を開始しましょう。 ドラッグ アンド ドロップ、 **新しい** [R スクリプトの実行] [- r のスクリプトの実行] モジュールを実験にします。 既存の [R スクリプトの実行] [- r のスクリプトの実行] モジュールの結果データセット 1 出力ポートを新しい [R スクリプトの実行] の [の r のスクリプトの実行] モジュールのデータセット 1 入力ポートに接続します。
...
*図 14 です。 R スクリプトの実行モジュールにおけるデータ フレームのサマリー*
...
*図 15 です。 時系列オブジェクトを持つデータ フレームのサマリー*
...
防御的プログラミングの例については、既に [値の変換](#valuetransformations)します。 両方の計算ブロックは、`tryCatch()` に含まれています。 一部のエラーについては、オリジナルの入力ベクトルを返すのが有効であり、別のケースでは、ゼロ ベクトルを返します。  
...
*図 17 です。 傾向除去され標準化された時系列のペアワイズ散布図です。*
...
*図 18 です。 ペアワイズ相関関係分析によるオブジェクト ccf のリスト。*
...
1.  '**[**'演算子を引数を'**1**' ccf オブジェクト リストの最初の要素から時間差における相関関係のベクトルを選択します。

2.  `do.call()` 関数は、`lapply()` によるリスト応答の要素に、`rbind()` 関数を適用します。

3.  `data.frame()` 関数は、`do.call()` により生成された結果を、データフレームに変換します。

行名は、データフレームの列内にあることに注意してください。 ので、[R スクリプトの実行] [- r のスクリプトの実行] からの出力にいるときに、同様に、コマンドを実行で、行名が保持されます。

図 19 に示すように、出力を生成するコードを実行するときは **視覚化** 結果データセット ポートの出力をします。 行名は、意図したとおり、最初の列になっています。

![相関関係分析による結果の出力][20]

*図 19 です。 相関関係分析の出力結果です。*
...
最初を追加、 **新しい** [R スクリプトの実行] [- r のスクリプトの実行] モジュールを実験にします。 接続、 **結果データセット** に既存の [R スクリプトの実行] [- r のスクリプトの実行] モジュールの出力、 **Dataset1** 、新しいモジュールの入力します。 結果が、図 20 のように表示されます。

![新しい R スクリプトの実行モジュールが追加された実験][21]

*図 20 です。 新しい R スクリプトの実行モジュールが追加された実験します。*
...
*図 22 です。 カリフォルニア酪農生産と価格データの時系列プロットします。*
...
サニティ テストとして、傾向曲線を示すカリフォルニア酪農生産データの時系列プロットを作成しましょう。 モデルの作成とプロットを Azure Machine Learning [R スクリプトの実行] [・ r のスクリプトの実行] モデル (RStudio ではない) で、次のコードを追加しました。 図 23 に、結果が示されます。
...
*図 23 です。 傾向モデルを示すカリフォルニア牛乳生産データ。*
...
別のカリフォルニア酪農生産データの時系列プロットを作成し、季節モデルがうまく機能することを確認します。 Azure Machine Learning [R スクリプトの実行] [- r のスクリプトの実行]、モデルを作成し、プロットで次のコードを追加しました。
...
*図 24 です。 モデルの季節的影響を含むカリフォルニア牛乳生産します。*
