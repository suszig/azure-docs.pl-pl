...
機械学習で使用されるトレーニング データは、多くの場合、収集された生データから特徴を選択したり抽出したりすることで向上させることができます。 手書き文字の画像を分類する方法を学習のコンテキストでエンジニア リングの機能の例は、生のビット分布データから構築されているビット密度マップです。 このマップは、生の分布よりも効率的に文字のエッジを検索できます。
...
エンジニア リングと選択は、 **データ強化** 機械学習の手順です。 このプロセスの 3 つの要素を次の目的で区別する場合があります。

* **データの前処理**: このプロセスは、収集したデータがクリーンで一貫性のあることを確認しようとしています。 これには、複数のデータセットの統合、欠落しているデータの処理、一貫性のないデータの処理、データ型の変換などのタスクが含まれます。
...
このトピックでは、データ強化プロセスの特徴エンジニアリングと特徴選択についてのみ説明します。 データ前処理のステップの詳細については、次を参照してください。、 [pre-processing Data in Azure ML Studio](http://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/) ビデオです。
...
* 回帰の例 [自転車のレンタル数の予測](../machine-learning-sample-prediction-of-number-of-bike-rentals.md) ターゲット値が既知の場合の教師あり実験で
* [特徴ハッシュ] [特徴ハッシュ] を使用して、テキスト マイニングの分類の例
...
Azure Machine Learning の実験では、これら 4 つのトレーニング データセットは、前処理された入力データセットからの 4 つの分岐を使用して形成されます。 ほとんどの分岐があり、これらの各分岐を含む左側を除き、[R スクリプトの実行] [- r のスクリプトの実行] モジュールのセットが (特徴セット B、C、および D) 機能を派生は構築され、インポートされたデータセットに追加されますそれぞれします。 次の図は、左の 2 番目の分岐にある特徴セット B の作成に使用される R スクリプトを示します。
...
### <a name="example2"></a> 例 2: テキスト マイニングで特徴を作成します。  
...
このタスクを実現するためと呼ばれる手法 **特徴ハッシュ** 特徴を効率的に任意のテキストのインデックスに適用します。 各テキストの特徴 (単語や語句) を特定のインデックスに関連付ける代わりに、ハッシュ関数を特徴に適用し、そのハッシュ値を直接インデックスとして使用することでこのメソッドが機能します。

Azure Machine Learning では、これらを作成する [特徴ハッシュ] [特徴ハッシュ] モジュールの単語や語句の特徴便利なことにします。 このモジュールを使用する例を次に示します。 入力データセットには、1 ～ 5 の書籍の評価と実際のレビュー内容の 2 つ列が含まれています。 この [特徴ハッシュ] [特徴ハッシュ] モジュールの目的は、一連の新しい特徴取得する対応する単語の出現頻度を示すレビュー内や語句、特定の書籍です。 このモジュールを使用するには、次の手順を完了する必要があります。
...
Azure Machine Learning Studio には、特徴選択に提供されるモジュールがあります。 これらのモジュールが、[フィルターに基づく特徴の選択] を含む次の図に示すように、[フィルター ベースの機能の選択] と [Fisher 線形判別分析] [フィッシャーの線形の判別の分析] です。

![特徴選択の例](./media/machine-learning-feature-selection-and-engineering/feature-Selection.png)


たとえば、[フィルターに基づく特徴の選択] の [フィルター ベースの機能の選択] モジュールの使用を検討します。 便宜上、上記で説明したテキスト マイニングの例を引き続き使用します。 [特徴ハッシュ] [特徴ハッシュ] モジュールから 256 の特徴のセットが作成された後、回帰モデルを作成して、応答変数が"Col1"で、1 から 5 まで書籍レビューの評価を表すことを想定しています。 [特徴スコア付けの方法] を [Pearson Correlation]、[フィルターに基づく特徴選択] を [Col1]、[目的の特徴の数] を [50] に設定すると、 モジュール [フィルターに基づく特徴の選択] [フィルター ベースの機能の選択] は、ターゲット属性"Col1"を持つ 50 の特徴を含むデータセットを生成し、します。 次の図に、この実験のフローと説明した入力パラメーターを示します。
...
この [フィルターに基づく機能の選択] を適用することでは、[フィルター ベースの機能の選択] のモジュール、ターゲット属性"Col1"を使用して最も相関関係を持つ、機能しているために、機能が選択されて 256 のうち 50 は、「ピアソンの相関関係」スコアリング メソッドに基づいています。
