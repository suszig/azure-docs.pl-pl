...
2.  新しい実験を作成し、[プロジェクト列] の [プロジェクト列] モジュールを使用して、Excel で使用される同じデータ特徴   
3.  [分割] [分割] モジュールの使用 (と *相対表現* モード) が、Excel で実行されたのと同じトレーニング セットにデータを分割するには  
4.  [線形回帰] [線形回帰] モジュール (既定のオプションのみ) を使用して実験、文書化、および結果、Excel の回帰モデルを比較
...
* [線形回帰] [線形回帰] モジュールを Azure ML を使用すると、2 つの方法が用意されています。
...
### 最適化と今後の実験
これまで、Excel モデルを使用してベースラインを構築し、Azure ML 線形回帰モデルの最適化を進めてきました。  [フィルターに基づく特徴の選択] モジュールを使用して [フィルター-ベース-機能の選択] の初期データ要素とその選択を改善するでは、4.6% のパフォーマンス向上を実現できました。 平均絶対誤差です。   今後のプロジェクトのために、データ属性に反復処理を行う数週間を節約できる可能性があるこの特徴を使用して、モデリングに使用する正しい特徴セットを検索します。  

次に [ベイジアン] [ベイジアン-線形-回帰] などのアルゴリズムや [ブースト デシジョンのツリー] [ブースト-決定-ツリーの回帰] パフォーマンスを比較する実験に含める予定です。    
...
Excel の回帰と Azure Machine Learning の実験を並行して実行することで、多くのことを学びました。 Excel でベースライン モデルを作成し、Azure ML [線形回帰] を使用して、モデルと比較することは、[線形回帰] では、Azure ML の学習できました。 し、データの選択とモデルのパフォーマンスを改善する機会を発見しました。         

[フィルターに基づく特徴の選択] を使用するが賢明である発見 [フィルター ベース-機能の選択] 今後の予測プロジェクトを促進するためにします。  データに特徴選択を適用することで、Azure ML で全体のパフォーマンスがさらに向上したモデルを作成できます。 
