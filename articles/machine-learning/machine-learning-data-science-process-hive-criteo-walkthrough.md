...
## <a name="setup"></a>「HDInsight Hadoop クラスターをセットアップ データ サイエンス用
...
2. [データ サイエンス用 Azure HDInsight Hadoop クラスターをカスタマイズする](machine-learning-data-science-customize-hadoop-cluster.md): この手順では、すべてのノードにインストールされている 64 ビット Anaconda Python 2.7 を使用、Azure HDInsight Hadoop クラスターを作成します。 HDInsight クラスターをカスタマイズするときに実行する、2 つの重要な手順があります (このトピックで説明します)。
...
## <a name="getdata"></a>取得して公開されているソースからデータを使用します。
...
## <a name="login"></a>クラスター ヘッドノードにログインします。

クラスターのヘッドノードにログインを使用して、 [Azure Management](manage.windowsazure.com) ポータル クラスターを検索します。 左側の HDInsight の象のアイコンをクリックし、クラスターの名前をダブルクリックします。 **[構成]** タブに移動し、ページの下部にある [接続] アイコンをダブルクリックし、プロンプトが表示されたら、リモート アクセスの資格情報を入力します。 入力すると、クラスターのヘッドノードに移動します。
...
## <a name="hive-db-tables"></a> Hive データベースとテーブルを作成します。

Criteo データセットの Hive テーブルを作成するには、開く、 *** Hadoop コマンド ライン *** ヘッド ノードのデスクトップでのコマンドを入力して Hive ディレクトリを入力し、

    cd %hive_home%\bin

**重要な注意事項**: * * このチュートリアルでは、上記の Hive bin からすべての Hive コマンドを実行/ディレクトリ プロンプトです。 これは自動的にすべてのパスの問題に対処します。 用語「Hive ディレクトリ プロンプト」を使用して"Hive bin/ディレクトリ プロンプト"、および「Hadoop コマンドライン」interchangeably.* *

**重要なメモ 2**: **次時は、常に Hive クエリを実行する** 
        cd %hive_home%\bin
        hive
...
## <a name="exploration"></a> Hive にデータの探索
...
## <a name="downsample"></a> 下の Azure Machine Learning のデータセットをサンプリングします。
...
**注:** このチュートリアルでは、高次元のカテゴリ特徴のコンパクトな表現を生成するために、カウント テーブルの使用に重点を置いていします。 これは間違いなくカテゴリの特徴をエンコードする唯一の方法その他の方法の詳細については、関心のある方をチェック_アウトできます [1 つのホット-エンコード](http://en.wikipedia.org/wiki/One-hot) と [特徴ハッシュ](http://en.wikipedia.org/wiki/Feature_hashing)します。
...
## <a name="aml"></a> Azure Machine Learning でモデルを構築します。

Azure Machine Learning のモデル構築プロセスは、次の手順を実行します。

1. [Azure Machine Learning に Hive テーブルからデータを取得します。](#step1)
2. [実験を作成する: データを整理、学習者を選択してカウント テーブルを特徴付ける](#step2)
3. [モデルをトレーニングします。](#step3)
4. [テスト データでモデルのスコアを付ける](#step4)
5. [モデルを評価します。](#step5)
6. [使用する web サービスとしてモデルを発行します。](#step6)
...
### <a name="step1"></a> 手順 1: リーダー モジュールを使用して Azure Machine Learning に Hive テーブルからデータを取得し、machine learning の実験用に選択
...
*** 重要な注意事項: *** * * これトレーニングとテスト データセットの両方を行います。 また、この用途で付けたデータベース名とテーブル名を必ず使用してください。 図で使用される値は、単独の図 purposes.* *

### <a name="step2"></a> 手順 2: Azure Machine learning のクリックを予測する簡単な実験を作成する/クリックしません。
...
### <a name="step6"></a> 手順 6: Web サービスとしてモデルを発行します。
...
青色の入力ポートと出力ポートを取得するには、右下にある **[prepare webservice]** をクリックします。 クリックして、web サービスを公開することは、この実験を実行しても、 **WEB サービスの発行** 、右下にあるアイコン次に示します。
