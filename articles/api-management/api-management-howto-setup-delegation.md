...
## <a name="delegate-signin-up"> </a>委任する開発者のサインインおよびサインアップ
...
2. 要求の送信元が Azure API Management であることを確認します (省略できますが、セキュリティ上強く推奨されます)。
    ...
    * に基づいて、文字列の hmac-sha512 ハッシュを計算、 **returnUrl** と **salt** クエリ パラメーター ([コードの例の下])。
     > HMAC(**salt** + '\n' + **returnUrl**)
...
4. サインアップの場合は、API Management に対応するアカウントを作成する必要があります。 [ユーザーの作成] API Management REST API を使用します。 このとき、ユーザー ID をユーザー ストア内のユーザー ID と同じに設定するか、または追跡が可能な ID に設定してください。

5. ユーザーが正常に認証されたら、次の操作を行います。

    * [シングル サインオン (SSO) トークンを要求] API Management REST API を介して
...
## <a name="delegate-product-subscription"> </a>成果物のサブスクリプション
...
4. ユーザー側の成果物を正常にサブスクライブするには、[成果物のサブスクリプション、REST API を呼び出す] で、ユーザーが API Management の成果物をサブスクライブします。

## <a name="delegate-example-code"> </a> のコード例
