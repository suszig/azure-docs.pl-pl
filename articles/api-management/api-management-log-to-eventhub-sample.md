...
ビルド後、 `requestLine` 要求本文の部分を切り捨てるかどうかを確認します。要求本文は 1,024 に切り詰められます。この値は増やすこともできます。ただし、個々のイベント ハブ メッセージは 256 KB に制限されているため、HTTP メッセージ本文によっては 1 つのメッセージに収まらなくなる可能性があります。ログ記録および分析を実施する場合、HTTP 要求行とヘッダーのみからでも膨大な量の情報が生成される可能性があります。また、多くの API 要求ではサイズの小さな本文のみが返されるため、サイズの大きな本文を切り詰めた場合の情報価値の損害は、本文の内容すべてを保持するための転送、処理、保管のコストの削減と比較すると非常に小さなものです。本文の処理に関する最後の注意点を渡す必要がある `true` に<string>() メソッド本文の内容を読み取ることがあるためでもあったバックエンド本文を読み取ることができる API をします。このメソッドに true を渡すことで、本文をもう一度読み取ることができるように本文はバッファーに格納されます。非常に大きなファイルをアップロードする API や長いポーリング時間を使用する API がある場合は、この点に注意することが重要です。このような場合は、本文の読み取りをまったく行わないようにするのが最善です。

### HTTP ヘッダー

HTTP ヘッダーは、単純なキーと値のペアの形式のメッセージ形式に変換できます。 ここでは、資格情報が不必要に漏えいすることのないように、機密情報が格納される特定のフィールドを除去しています。 API キーとその他の資格情報が分析目的で使用される可能性はほとんどありません。 使用しているので、ユーザーと特定の製品を分析する場合から取得することも、 `コンテキスト` オブジェクトし、それをメッセージに追加します。
### メッセージのメタデータ

最初の行がありませんが、event hub に送信する完全なメッセージを作成するときの一部では実際には、 `アプリケーション/http` メッセージです。 最初の行は追加のメタデータで、メッセージが要求メッセージと応答メッセージのどちらであるかを示す値と、要求を応答に関連付けるためのメッセージ ID で構成されています。 メッセージ ID は、次のような別のポリシーを使用して作成されます。
...
`変数の設定` 両方からアクセス可能な値を作成するポリシー、 `ログ-eventhub` ポリシーで、 `< 受信 >` セクションおよび `< 送信 >` セクションです。

## Event Hubs からのイベントの受信

使用して Azure Event Hub からのイベントが受信した、 [AMQP プロトコル](http://www.amqp.org/)します。 Microsoft Service Bus チームは、クライアント ライブラリでコンシューマー側のイベントを簡単に作成できるようにしました。 2 つの異なるアプローチがサポートされている、1 つ、 *ダイレクト コンシューマー* 、もう一方を使用して、 `EventProcessorHost` クラスです。 これら 2 つの方法の例を参照して、 [Event Hub プログラミング ガイド](../event-hubs/event-hubs-programming-guide.md)します。 相違点の短いバージョンは、 `ダイレクト コンシューマー` 全面的に制御し、 `EventProcessorHost` するが、想定してそれらのイベントを処理する方法についてのプラミング作業の一部です。

### EventProcessorHost

このサンプルでは、使用、 `EventProcessorHost` わかりやすいように、ただし場合があるこの特定のシナリオに適していません。 `EventProcessorHost` は困難な作業のスレッド処理の特定のイベント プロセッサ クラス内での問題について心配する必要ないことを確認します。 ただし、このシナリオでは、メッセージを別の形式に変換し、非同期メソッドを使用してそれを別のサービスに渡しているだけです。 共有した状態を更新する必要はないため、スレッドの問題が発生するリスクはありません。 ほとんどのシナリオでは、 `EventProcessorHost` がおそらく最善の選択肢より簡単なオプションでは確実にします。

### IEventProcessor

使用する場合は、中心となる概念 `EventProcessorHost` を作成するには、実装、 `IEventProcessor` メソッドが含まれているインターフェイス `ProcessEventAsync`します。 このメソッドの最も重要な部分を次に示します。

  非同期タスク IEventProcessor.ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
      {
...
`HttpMessage` インスタンスには、3 つデータにはが含まれています。
...
`HttpMessage` インスタンスに含まれる、 `MessageId` GUID を対応する HTTP 応答と、オブジェクトには、HttpRequestMessage と HttpResponseMessage のインスタンスが含まれている場合を示すブール値への HTTP 要求の接続に使用するとします。 HTTP クラスの組み込みを使用して `System.Net.Http`, 、活用することができました、 `アプリケーション/http` に含まれているコードを解析 `System.Net.Http.Formatting`します。

### IHttpMessageProcessor

`HttpMessage` インスタンスの実装に転送 `IHttpMessageProcessor` を受信して Azure Event Hub からイベントを解釈し、その実際の処理を分離するために作成したインターフェイスであります。
...
`IHttpMessageProcessor` 実装は、次のようになります
...
[ソース コード](https://github.com/darrelmiller/ApimEventProcessor) Github のサンプルのテストとします。 必要があります、 [API Management サービス](api-management-get-started.md), 、[接続されているイベント ハブ](api-management-howto-log-event-hubs.md), 、および [ストレージ アカウント](../storage/storage-create-storage-account.md) 自分用のサンプルを実行します。

このサンプルは、Event Hub から送信されたイベントに変換をリッスンする単純なコンソール アプリケーションだけを `HttpRequestMessage` と `HttpResponseMessage` Runscope API に転送し、オブジェクトです。
...
-   API Management と Event Hubs の統合の詳細
    -   [Azure API Management で Azure Event Hubs にイベント ログに記録する方法](api-management-howto-log-event-hubs.md)
