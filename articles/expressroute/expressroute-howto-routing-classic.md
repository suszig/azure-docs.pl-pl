...
# 作成および PowerShell を使用して ExpressRoute 回線のルーティングを変更します。
...
4. **回線用に Azure プライベート ピアリングを構成します。**
    ...
    次のコマンドレットを実行して、回線用に Azure プライベート ピアリングを構成することができます。
        ...
        ServiceKey New-azurebgppeering AccessType プライベート-"*** * ***"- PrimaryPeerSubnet「10.0.0.0/30」- SecondaryPeerSubnet「10.0.0.4/30」PeerAsn 1234-vlanid 100

    MD5 ハッシュを使用する場合は、以下のコマンドレットを使用できます。

        ServiceKey New-azurebgppeering AccessType プライベート-"*** * ***"- PrimaryPeerSubnet「10.0.0.0/30」- SecondaryPeerSubnet「10.0.0.4/30」PeerAsn 1234 VlanId 100-sharedkey"A1B2C3D4"
...
4. **回線用に Azure パブリック ピアリングを構成します。**
    ...
    次のコマンドレットを実行して、回線用に Azure プライベート ピアリングを構成することができます。
        ...
        ServiceKey New-azurebgppeering AccessType パブリック-"*** * ***"- PrimaryPeerSubnet「131.107.0.0/30」- SecondaryPeerSubnet「131.107.0.4/30」PeerAsn 1234-vlanid 200

    MD5 ハッシュを使用する場合は、次のコマンドレットを使用することができます。

        ServiceKey New-azurebgppeering AccessType パブリック-"*** * ***"- PrimaryPeerSubnet「131.107.0.0/30」- SecondaryPeerSubnet「131.107.0.4/30」PeerAsn 1234 VlanId 200-sharedkey"A1B2C3D4"
    >[AZURE.IMPORTANT] 顧客 ASN ではなく、ピアリング ASN として AS 番号を指定するようにしてください。
