...
- アクティブな ExpressRoute 回線が必要です。 
    - 指示に従って、 [ExpressRoute 回線を作成](expressroute-howto-circuit-arm.md) 回線の接続プロバイダーが有効になっているとします。 
    - 回線用に Azure プライベート ピアリングが構成されていることを確認してください。 参照してください、 [ルーティングを構成する](expressroute-howto-routing-arm.md) ルーティング情報の記事です。 
    - Azure プライベート ピアリングを構成する必要があります。また、ネットワークと Microsoft 間の BGP ピアリングを起動して、エンド ツー エンド接続を有効にする必要があります。
    - 仮想ネットワークと仮想ネットワーク ゲートウェイを作成して完全にプロビジョニングする必要があります。 作成する手順については、 [vpn ゲートウェイ](../articles/vpn-gateway-create-site-to-site-rm-powershell.md)

最大 10 個の仮想ネットワークを ExpressRoute 回線に接続できます。 ExpressRoute 回線はすべて同じ地理的リージョンにある必要があります。 ExpressRoute Premium アドオンを有効にした場合は、ExpressRoute 回線に多数の仮想ネットワークをリンクできます。 チェック アウト、 [FAQ](expressroute-faqs.md) premium アドオンの詳細についてです。 
...
>[AZURE.NOTE] 専用回線の接続と帯域幅料金は、ExpressRoute 回線の所有者に適用されます。 すべての仮想ネットワークが同じ帯域幅を共有します。

![サブスクリプション間接続](./media/expressroute-howto-linkvnet-classic/cross-subscription.png)

### 管理

 *回線の所有者は* は、ExpressRoute 回線のリソースの権限のあるパワー ユーザーです。 回線所有者は承認で利用できるを作成できます *回線ユーザー*します。 *ユーザーを回線* (ExpressRoute 回線と同じサブスクリプション内ではありません) を仮想ネットワーク ゲートウェイの所有者します。 *ユーザーを回線* 承認 (仮想ネットワークあたり 1 つの承認を引き換えることができます。

 *回線の所有者は* 変更し、いつでも承認を取り消すに電源が入っています。 承認を取り消すと、アクセスが取り消されたサブスクリプションからすべての接続が削除されます。
