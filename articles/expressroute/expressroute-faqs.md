...
番号 任意の速度の VPN 接続は、サービス プロバイダーから購入できます。 ただし、Azure への接続は、購入した ExpressRoute 回線の帯域幅に制限されます。
...
番号 Azure にレイヤー 2 接続延長をサポートはされていません。
...
番号 同じ ExpressRoute 回線にリンクされているすべての仮想ネットワークは、同じルーティング ドメインの一部であるし、ルーティングの観点から互いから分離されていません。 ルートの分離が必要な場合は、ExpressRoute 回線を別に作成する必要があります。
...
番号 必要な機能を選択することはできません。 ExpressRoute Premium を有効にすると、すべての機能が有効になります。
