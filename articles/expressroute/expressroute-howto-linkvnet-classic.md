 ...
 *回線の所有者は* は、管理者または共同管理者のサブスクリプションが ExpressRoute 回線を作成します。 回線の所有者を承認できます管理者/共同管理者の他のサブスクリプション (と呼ばれる *回線ユーザー* ワークフロー図で)、専用回線を使用します。 組織の ExpressRoute 回線の使用を承認される回線ユーザーは、いったん承認されると、サブスクリプションの VNet を ExpressRoute 回線にリンクできるようになります。
