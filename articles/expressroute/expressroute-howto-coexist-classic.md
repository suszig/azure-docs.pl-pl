...
>[AZURE.IMPORTANT] Azure が現在 2 つの配置モデルで動作するかを把握することが重要: リソース マネージャーとクラシックします。 構成を開始する前に、デプロイ モデルとツールについて理解しておくようにしてください。 配置モデルについては、次を参照してください [Azure デプロイ モデル。](../azure-classic-rm.md)
...
- **転送中のルーティングがサポートされていません:** サイト間 VPN 経由で接続されているローカル ネットワークと ExpressRoute 経由で接続されているローカル ネットワークの間 (Azure) 経由でルーティングすることはできません。
- **ポイント対サイトがサポートされていません:** ExpressRoute に接続されている同じ VNet へのポイント対サイト VPN 接続を有効にすることはできません。 ポイント対サイト VPN と ExpressRoute を同じ VNet に共存させることはできません。
- **サイト間 VPN ゲートウェイを有効にすることはできません強制トンネリング:** することのみ「強制」インターネットにバインドされたトラフィックをすべて ExpressRoute 経由で内部設置型ネットワークに戻る。 
- **のみに標準または高パフォーマンス ゲートウェイ:** ExpressRoute ゲートウェイとサイト間 VPN ゲートウェイの両方に標準または高パフォーマンス ゲートウェイを使用する必要があります。 参照してください [ゲートウェイの Sku](../vpn-gateway/vpn-gateway-about-vpngateways.md) ゲートウェイの Sku についてです。
- **静的ルートの要件:** 、ローカル ネットワークが ExpressRoute とサイト間 VPN の両方に接続されている場合は、静的ルートをサイト間 VPN 接続をパブリック インターネットにルーティングするローカル ネットワークで構成が必要です。
- **ExpressRoute ゲートウェイを最初に構成する必要があります:** サイト間 VPN ゲートウェイを追加する前にまず ExpressRoute ゲートウェイを作成する必要があります。
...
>[AZURE.NOTE] 転送中のルーターとして仮想ネットワークの構成することはできません。
...
- **新しい仮想ネットワークおよび共存する接続を作成する:**
    ...
    仮想ネットワークがまだない場合、この手順では、新しい仮想ネットワークを作成し、新しい ExpressRoute 接続とサイト間 VPN 接続を作成する方法を説明します。 構成、記事のセクションの手順に従います **ExpressRoute とサイト対サイト接続の両方で、新しい仮想ネットワークを作成する**です。

- **共存する接続用に既存の仮想ネットワークを構成する:**

    既存のサイト間 VPN 接続または ExpressRoute 接続を使用して、仮想ネットワークを既に配置している場合があります。  **、既存の仮想ネットワークの共存する接続を構成する** 、ゲートウェイを削除して新しい ExpressRoute 接続とサイト間 VPN 接続を作成するセクションはについて説明します。 新しい接続を作成する場合は、非常に特殊な順序で完了する必要がありますので注意してください。 他の記事の手順を使用してゲートウェイと接続を作成しないでください。
...
2. 仮想ネットワークのスキーマを作成します。 ネットワーク構成ファイルの使用に関する詳細については、次を参照してください。 [ネットワーク構成ファイルを使用して、仮想ネットワークを構成する](../virtual-network/virtual-networks-create-vnet-classic-portal.md)です。 構成スキーマの詳細については、次を参照してください。 [Azure 仮想ネットワークの構成スキーマ](https://msdn.microsoft.com/library/azure/jj157100.aspx)します。
...
4. ExpressRoute ゲートウェイを作成します。 指定することを確認する *標準* または *HighPerformance* し、GatewayType *DynamicRouting*します。
...
6. 次に、サイト間 VPN ゲートウェイを作成します。 Gatewaysku *標準* または *HighPerformance* 、GatewayType をする必要があります *DynamicRouting*します。
...
**構成を開始する前に:** ゲートウェイ サブネットのサイズを増やせるように、仮想ネットワーク内の残りのための十分な IP アドレスがあることを確認します。
...
3. ゲートウェイ サブネットが /27 またはこれより短いプレフィックス (/26 や /25 など) になるように、ネットワーク構成ファイルのスキーマを編集します。 次の例を参照してください。 ネットワーク構成ファイルの使用に関する詳細については、次を参照してください。 [ネットワーク構成ファイルを使用して、仮想ネットワークを構成する](../virtual-network/virtual-networks-create-vnet-classic-portal.md)です。 構成スキーマの詳細については、次を参照してください。 [Azure 仮想ネットワークの構成スキーマ](https://msdn.microsoft.com/library/azure/jj157100.aspx)します。

          <Subnet name="GatewaySubnet">
            <AddressPrefix>10.17.159.224/27</AddressPrefix>
          </Subnet>

4. 以前のゲートウェイが、サイト間 VPN であった場合は、接続の種類をも変更する必要があります **専用**します。
...
5. この時点では、VNet にゲートウェイがありません。 新しいゲートウェイを作成し、接続を完了させる、作業を続行する **手順 4** この記事では、このセクションで [ExpressRoute とサイト対サイト接続の両方で、新しい仮想ネットワークを作成する](#create-a-new-virtual-network-with-both-expressroute-and-site-to-site-connectivity)です。

## 次のステップ

ExpressRoute の詳細については、次を参照してください、 [ExpressRoute の FAQ。](expressroute-faqs.md)
