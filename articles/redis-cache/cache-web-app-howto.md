<properties 
    pageTitle="Tworzenie aplikacji sieci Web za pomocą pamięci podręcznej Redis | Microsoft Azure" 
...
2. W **Eksploratorze rozwiązań** kliknij dwukrotnie plik **web.config**, aby go otworzyć.

    ![Web.config][cache-web-config]

3.  Dodaj poniższe parametry połączenia do sekcji `connectionStrings`. Nazwa parametrów połączenia musi być zgodna z nazwą klasy kontekstu bazy danych platformy Entity Framework, którą jest `TeamContext`.
...
4. W **Eksploratorze rozwiązań** rozwiń folder **Global.asax** i kliknij dwukrotnie plik **Global.asax.cs**, aby go otworzyć.

    ![Global.asax.cs][cache-global-asax]

5. Dodaj dwie poniższe instrukcje using w początkowej części pliku, pod innymi instrukcjami using.
...
6. Dodaj poniższy wiersz kodu na końcu metody `Application_Start`.


        Database.SetInitializer<TeamContext>(new TeamInitializer());


7. W **Eksploratorze rozwiązań** rozwiń folder `App_Start` i kliknij dwukrotnie pozycję `RouteConfig.cs`.

    ![RouteConfig.cs][cache-RouteConfig-cs]

8. Zastąp ciąg `controller = "Home"` w poniższym kodzie w metodzie `RegisterRoutes` ciągiem `controller = "Teams"`, jak pokazano w poniższym przykładzie.
...
3. W sekcji `body` zaktualizuj pierwszą instrukcję `Html.ActionLink` i zastąp parametr `Application name` ciągiem `Contoso Team Stats` oraz parametr `Home` parametrem `Teams`.
...
1. Utwórz na komputerze plik o nazwie `WebAppPlusCacheAppSecrets.config` i umieść go w lokalizacji, która nie zostanie zaewidencjonowana kodem źródłowym przykładowej aplikacji, na wypadek podjęcia decyzji o zaewidencjonowaniu jej w innym miejscu. W tym przykładzie plik `AppSettingsSecrets.config` znajduje się w folderze `C:\AppSecrets\WebAppPlusCacheAppSecrets.config`.
...
3. Dodaj następujący atrybut `file` do elementu `appSettings`. Jeśli wcześniej używana była inna nazwa pliku lub lokalizacja, podstaw te wartości w miejsce wyświetlonych w przykładzie.
    -   Przed: `<appSettings>`
...
>[AZURE.NOTE] Do korzystania z usługi Azure Redis Cache nie jest wymagane przechowywanie statystyk zespołu w wielu formatach w pamięci podręcznej. W tym samouczku używa się wielu formatów, aby przedstawić różne sposoby i różne typy danych, których można użyć do buforowania danych.
...
3. Dodaj następujące trzy metody do klasy `TeamsController`, aby wdrożyć typy akcji `playGames`, `clearCache` i `rebuildDB` z instrukcji switch dodanej w poprzednim fragmencie kodu.
...
4. Dodaj poniższe cztery metody do klasy `TeamsController`, aby wdrożyć różne sposoby pobierania statystyk zespołu z pamięci podręcznej i bazy danych. Każda z tych metod zwraca obiekt `List<Team>`, który jest następnie wyświetlany w widoku.
...
3. Przewiń w dół pliku **Index.cshtml** i dodaj poniższy element `tr` w taki sposób, aby znajdował się w ostatnim wierszu w ostatniej tabeli w pliku.
...
>[AZURE.NOTE] Jeśli nie masz konta platformy Azure, możesz [utworzyć bezpłatne konto](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=redis_cache_hero) w zaledwie kilka minut.
