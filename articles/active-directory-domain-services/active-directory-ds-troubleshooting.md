...
- **アカウントの同期:** 影響を受けているユーザー アカウントが、内部設置型ディレクトリから同期されている場合は、次の手順を実行することを確認します。
...
- **クラウド専用のアカウント**: 影響を受けるユーザー アカウントがクラウド専用のユーザー アカウントの場合は、Azure AD ドメイン サービスを有効にした後、ユーザーがパスワードを変更していることを確認します。 この手順によって、Azure AD ドメイン サービスに必要な資格情報ハッシュが生成されます。


### お問い合わせ
管理対象ドメインに問題がある場合は、このトラブルシューティング ガイドで説明されている手順で問題が解決するかどうかを確認してください。 問題が解消しない場合は、以下の方法でお気軽にお問い合わせください。

-  [Azure Active Directory ユーザーの声チャネル](http://feedback.azure.com/forums/169401-azure-active-directory)します。 プレ保留することを確認言葉で質問 **'AADDS'** 私たちに到達するためにします。
- 電子メールにてする可能性がありますも [Azure AD ドメイン サービスからのフィードバック](mailto:aaddsfb@microsoft.com)
