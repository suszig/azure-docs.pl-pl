...
これは SLA の目的で役立ちます ([SLA の監視](#SLA-monitoring) このガイドで後で詳しく説明)。 __”ダウンタイム” の定義は、サービスによって異なります。 たとえば、Visual Studio Team Services では、ダウンタイムを、顧客がサービスに接続しようしている時間が 120 秒を超えている期間と、その時間内に接続は確立されたがすべての基本的な読み取りと書き込みの操作が失敗した期間と定義しています。
...
> [AZURE.NOTE] SLA の監視はパフォーマンスの監視と密接に関連しますが、パフォーマンスの監視はシステムが__”最適” に機能することの保証に関心があり、SLA の監視は __”最適” が実際に何を意味するかを定義している契約上の義務の管理に関心があります。
...
> [AZURE.NOTE] 商用システムの一部の契約には、すべてのヘルプ デスクへの要求が 5 分以内に応答され、すべての問題の 99% が 1 営業日以内に完全に対処されるといった、カスタマー サポートに関する SLA も含まれることがあります。 効果的な [問題追跡](#issue-tracking) sla を次のようにキーは、(後でこのセクションで説明)。
...
ユーザーが問題を報告するとき、ユーザーは自分の操作に対する直接的な影響のみを認識している場合が多く、ユーザーは自分の経験の結果のみをシステムの管理責任者に報告できます。 これらの経験は、通常は、1 つ以上の根本的な問題の目に見える症状にすぎません。 多くの場合、アナリストは、(このプロセスと呼びます _Root 発生 Analysis_) 問題の根本原因を確立するために基になる操作の一連の履歴を掘り下げて必要があります。
...
- ロギングは拡張可能であり、具体的なターゲットと直接的な依存関係を持っていない必要があります。 たとえば、__System.Diagnostics.Trace を使用して情報を書き込むのではなく、ロギング メソッドを公開し、適切な手段を使用して実装できる抽象インターフェイス (__ILogger など) を定義します。
...
#### _Pulling をインストルメンテーション data_ をプッシュします。

インストルメンテーション データ収集サブシステムことができますか、アクティブにデータの取得インストルメンテーションからさまざまなログとアプリケーション (_pull model_) の各インスタンスの他のソースまたはアプリケーション (_push model_) の各インスタンスを構成するコンポーネントから送信するデータを待つパッシブ受信者として使用できます。
...
_Figure 3 です。
情報の取得し、共有 storage_ への書き込みを監視エージェントの使用
...
_Figure 4 です。
キューを使用して、インストルメンテーション data_ をバッファーに書き込む
...
#### _Consolidating インストルメンテーション data_
...
_Figure 5 です。
統合し、インストルメンテーション data_ クリーンアップを別のサービスを使用します。
...
_Figure 6 です。
ストレージ requirements_ に基づく分析データをパーティション分割
