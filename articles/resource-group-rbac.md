...
Mac 向け Azure CLI があるない場合は、Linux や Windows がインストールされてで構成していない組織のアカウント用に Azure CLI は、「 [のインストールし、Azure CLI の構成](xplat-cli-install.md)します。
...
バージョンを使用する `2014年-10-01-プレビュー` このトピックのです。
...
- お客様のサブスクリプションでのアプリケーションのアクセスを管理するサービス プリンシパルの処理に関する詳細については、次を参照してください。 [Azure リソース マネージャーでサービス プリンシパルの認証](resource-group-authenticate-service-principal.md) と [Azure クラシック ポータルを使用して新しい Azure サービス プリンシパルを作成する](../resource-group-create-service-principal-portal.md)します。
