...
## Jaka jest lista obsługiwanych systemów operacyjnych, z których można tworzyć kopie zapasowe na platformie Azure przy użyciu usługi Azure Backup? <br/>
...
## Gdzie można pobrać najnowszą wersję agenta usługi Azure Backup? <br/>
...
## Która wersja serwera SCDPM jest obsługiwana? <br/>
...
## Podczas konfigurowania agenta usługi Azure Backup pojawia się monit o wprowadzenie poświadczeń magazynu. Czy poświadczenia magazynu wygasają?
...
## Czy istnieje ograniczenie liczby magazynów, które można utworzyć w poszczególnych subskrypcjach platformy Azure? <br/>
Tak. Począwszy od sierpnia 2016 r., można utworzyć 25 magazynów (dla magazynu usługi Backup i magazynu usługi Recovery Services) w ramach jednej subskrypcji. Jeśli potrzebna jest większa liczba magazynów, należy utworzyć nową subskrypcję.

## Czy istnieją jakiekolwiek ograniczenia dotyczące liczby serwerów/maszyn, które można zarejestrować w każdym magazynie? <br/>
...
## Jak można zarejestrować serwer w innym centrum danych?<br/>
...
## Co się stanie, jeśli zostanie zmieniona nazwa serwera Windows, który wykonuje kopie zapasowe danych na platformie Azure?<br/>
...
## Z jakich typów dysków można tworzyć kopie zapasowe plików i folderów? <br/>
...
## Jakie typy plików i folderów z serwera można umieszczać w kopiach zapasowych?<br/>
...
## Jaki jest minimalny wymagany rozmiar folderu pamięci podręcznej? <br/>
...
## Jeśli organizacja ma jeden magazyn, to w jaki sposób można odizolować dane z jednego serwera od danych z innego serwera podczas przywracania danych?<br/>
...
## Czy można „migrować” magazyn lub dane kopii zapasowej między subskrypcjami? <br/>
Nie. Magazyn jest tworzony na poziomie subskrypcji i po utworzeniu nie można go ponownie przypisać do innej subskrypcji.

## Czy agent usługi Azure Backup działa na serwerze, który korzysta z deduplikacji systemu Windows Server 2012? <br/>
...
## Czy w przypadku anulowania uruchomionego zadania tworzenia kopii zapasowej przesyłane dane zostaną usunięte? <br/>
...
## Dlaczego wyświetlany jest komunikat ostrzegawczy „Proces tworzenia kopii zapasowych Azure nie został skonfigurowany dla tego serwera”, mimo że wcześniej zostały zaplanowane regularne kopie zapasowe? <br/>
...
## Jakie reguły zapory należy skonfigurować na potrzeby usługi Azure Backup? <br/>
...
##Czy można zainstalować agenta usługi Azure Backup na maszynie wirtualnej, której kopia zapasowa została już utworzona przez usługę Azure Backup przy użyciu rozszerzenia maszyny wirtualnej? <br/>
...
## Czy można zainstalować agenta usługi Azure Backup na maszynie wirtualnej Azure, aby wykonywać kopie zapasowe plików i folderów znajdujących się w magazynie tymczasowym dostarczanym przez maszynę wirtualną platformy Azure? <br/>
...
## Agent usługi Azure Backup został zainstalowany w celu ochrony plików i folderów. Czy można teraz zainstalować program SCDPM do pracy z agentem usługi Azure Backup, aby chronić obciążenia aplikacji lokalnych i maszyn wirtualnych na platformie Azure? <br/>
...
## Jaka jest długość ścieżki pliku, którą można określić w ramach zasad usługi Azure Backup przy użyciu agenta usługi Azure Backup? <br/>  
...
## Jakie znaki są dozwolone w ścieżce pliku zasad usługi Azure Backup przy użyciu agenta usługi Azure Backup? <br>  
...
## Czy można używać programu Azure Backup Server do tworzenia kopii zapasowej z odzyskiwaniem systemu od zera (BMR) serwera fizycznego? <br/>
Tak.

## Czy można skonfigurować usługę Backup w taki sposób, aby wysyłała wiadomości e-mail, jeśli zadanie tworzenia kopii zapasowej nie powiedzie się? <br/>
...
## Czy istnieje ograniczenie rozmiaru poszczególnych źródeł danych, dla których tworzona jest kopia zapasowa? <br/>
...
## Czy istnieją ograniczenia zaplanowanej częstotliwości wykonywania zadań tworzenia kopii zapasowej?<br/>
...
## Czy istnieje różnica między zasadami planowania w programie DPM i systemie Windows Server (tj. w systemie Windows Server bez programu DPM)? <br/>
...
## Czy istnieje różnica między zasadami przechowywania w programie DPM i systemie Windows Server/kliencie systemu Windows (tj. w systemie Windows Server bez programu DPM)?<br/>
...
## Czy można skonfigurować zasady przechowywania selektywnie, na przykład skonfigurować zasady tygodniowe i dziennie, ale nie roczne i miesięczne?<br/>
...
## Czy można „zaplanować utworzenie kopii zapasowej” o godzinie 18:00 i określić „zasady przechowywania” na inną godzinę?<br/>
...
## Czy przyrostowa kopia jest przesyłana zgodnie z zaplanowanymi zasadami przechowywania? <br/>
...
## Czy w przypadku, gdy kopia zapasowa przechowywana jest przez długi czas, odzyskanie danych z wcześniejszego punktu danych trwa dłużej? <br/>
...
## Skoro każdy punkt odzyskiwania jest traktowany jak pełny punkt, czy ma to wpływ na całkowitą fakturowaną przestrzeń dyskową dla kopii zapasowych?<br/>
...
## Czy istnieje ograniczenie liczby punktów odzyskiwania, które można utworzyć?<br/>
...
## Dlaczego ilość danych przesyłanych w kopii zapasowej nie jest równa ilości danych, których kopia zapasowa jest tworzona?<br/>
...
## Czy istnieje sposób dostosowania wielkości przepustowości, która będzie używana przez usługę Backup?<br/>
...
## Przepustowość sieci Internet jest ograniczona do ilości danych, jakie mają zostać umieszczone w kopii zapasowej. Czy istnieje sposób przeniesienia danych do określonej lokalizacji za pomocą dużego potoku sieciowego i wypchnięcia tych danych do platformy Azure? <br/>
...
## Ile zadań odzyskiwania można wykonać na danych, których kopie zapasowe utworzono na platformie Azure?<br/>
Nie ma ograniczenia liczby operacji odzyskiwania z usługi Azure Backup.

## Czy trzeba płacić za ruch wyjściowy z centrum danych Azure podczas operacji odzyskiwania?<br/>
 Nie. Operacje odzyskiwania są bezpłatne i nie są naliczane opłaty za ruch wyjściowy.

## Czy dane wysyłane do platformy Azure są szyfrowane? <br/>
...
## Czy dane kopii zapasowej na platformie Azure są również szyfrowane?<br/>
...
## Jaka jest minimalna długość klucza szyfrowania używanego do szyfrowania danych w kopii zapasowej? <br/>
 Klucz szyfrowania powinien mieć co najmniej 16 znaków.

## Co się stanie, jeśli klucz szyfrowania zostanie zgubiony przez użytkownika? Czy użytkownik lub firma Microsoft może odzyskać dane? <br/>
...
## W jaki sposób można zmienić lokalizację pamięci podręcznej określoną dla agenta usługi Azure Backup?<br/>
...
## Gdzie można umieścić folder pamięci podręcznej, aby agent usługi Azure Backup działał zgodnie z oczekiwaniami?<br/>
...
## Czy jakiekolwiek atrybuty folderu pamięci podręcznej nie są obsługiwane?<br/>
