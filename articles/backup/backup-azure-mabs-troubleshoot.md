...
## <a name="error-invalid-vault-credentials-provided-the-file-is-either-corrupted-or-does-not-have-the-latest-credentials-associated-with-recovery-service"></a>Błąd: Udostępniono nieprawidłowe poświadczenia magazynu. Plik jest uszkodzony lub jest nie ma najnowszych poświadczeń skojarzonych z usługą odzyskiwania 
...
## <a name="error-the-agent-operation-failed-because-of-a-communication-error-with-the-dpm-agent-coordinator-service-on-server"></a>Błąd: Operacja agenta nie powiodła się z powodu błędu komunikacji z koordynatorem agenta programu DPM na serwerze 

Postępuj zgodnie z następującymi [kroki rozwiązywania problemów] (https://docs.microsoft.com/en-us/azure/backup/backup-azure-mabs-troubleshoot#registration-and-agent-related-issues), aby rozwiązać ten problem.

## <a name="error-setup-could-not-update-registry-metadata"></a>Błąd: Instalator nie może zaktualizować rejestru metadanych
...
| Tworzenie kopii zapasowych | Zadania tworzenia punktu odzyskiwania w trybie online dla maszyny Wirtualnej VMware nie powiedzie się. Program DPM napotkał błąd z VMware, podczas próby pobrania informacji śledzenia zmian. ErrorCode — FileFaultFault (identyfikator 33621) |  1. Resetuj ctk VMware, odpowiednich maszyn wirtualnych <br/> Sprawdź, czy niezależne dysk nie jest stosowane w programie VMWare <br/> Zatrzymaj ochronę odpowiednich maszyn wirtualnych i Wznów ochronę z przycisku Odśwież <br/> Uruchom DW dla odpowiednich maszyn wirtualnych|
...
| Operacja | Szczegóły błędu | Obejście problemu |
| --- | --- | --- |
| Próbujesz skonfigurować powiadomienia e-mail przy użyciu konta usługi Office 365. | Pobieranie Identyfikator błędu: 2013| **Przyczyna:**<br/> Próby użycia konta usługi Office 365 <br/> **Zalecana akcja:**<br/> Pierwszą czynnością, aby upewnić się, jest to "Zezwalaj na anonimowe przekazywania na łącznika odbioru" dla serwera programu DPM Instalatora na serwerze Exchange. Oto łącza na temat sposobu konfigurowania tego: http://technet.microsoft.com/en-us/library/bb232021.aspx <br/> Jeśli nie można używać wewnętrznego przekaźnika SMTP i należy skonfigurować za pomocą serwera usługi Office 365, usług IIS można skonfigurować jako przekaźnik dla tego. <br/> Musisz skonfigurować serwer programu DPM, aby mieć możliwość przekazywania SMTP dla usługi Office 365 za pomocą usług IIS https://technet.microsoft.com/en-us/library/aa995718(v=exchg.65).aspx to setup IIS to relay to O365 <br/> Ważna Uwaga: W kroku 3 -> g -> ii, należy użyć user@domain.com format i nie domena\użytkownik <br/> Punkt programu DPM użyć lokalnego servername SMTP serwera, portu 587 i adres e-mail użytkownika, który wiadomości e-mail powinna pochodzić z. <br/> Nazwa użytkownika i hasło na stronie Instalatora programu DPM SMTP należy konto domeny w domenie, w którym znajduje się program DPM. <br/> Uwaga: Jeśli zmiana adresu serwera SMTP, wprowadzić zmiany, aby nowe ustawienia, zamknij okno Ustawienia i ponownie otwórz aby upewnić się, że odzwierciedla nową wartość.  Po prostu zmieniania i testowania nie zawsze podejmie nowe ustawienia, testowanie w ten sposób jest najlepszym rozwiązaniem. <br/> W dowolnym momencie w trakcie tego procesu można wyczyścić te ustawienia zamknięcie konsoli programu DPM, a następnie edytując następujące klucze rejestru:<br/> HKLM\SOFTWARE\Microsoft\Microsoft Data Protection Manager\Notification\ <br/> Usuń klucze SMTPPassword i SMTPUserName. <br/> Można dodać je ponownie w interfejsie użytkownika po uruchomieniu go ponownie.
