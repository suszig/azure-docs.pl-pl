...
| -------- | ------------- | ------- |
| 暗号化 | Bitlocker または dmcrypt | 暗号化が行われるため、 *異なる* レイヤーの場合に Azure Backup と比較して、かかわらずどのような暗号化ソフトウェアを使用します。 ただし、このエクスペリエンスは Bitlocker と dmcrypt だけで検証されました。<br><br> データを暗号化するには、キーが必要になります。 また、データへの承認されたアクセスを確保するためにも安全にキーを保持する必要があります。  |
| キー管理 | CloudLink SecureVM<br>または Azure key Vault | データを暗号化または復号化するためにキーは不可欠です。 適切なキーがない場合、データは取得できません。 これは *信じられないほど* では重要。<br><li>キー ロール オーバー<li>長期的な保持<br><br>たとえば、7 年前にデータのバックアップを使用するキーがないことが、同じキーを使用今日です。 7 年前のキーがない場合、当時のバックアップから復元されるデータは使用できなくなるということです。|
...
> [AZURE.IMPORTANT]  (キーのロール オーバーを含む) のキーの管理は、Azure Backup の一部ではありません。 この側面については独立して管理する必要がありますが、全体的なバックアップ/復元操作において非常に重要です。
