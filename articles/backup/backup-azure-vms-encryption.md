...
| -------- | ------------- | ------- |
| 暗号化 | Bitlocker または dmcrypt | 暗号化が行われるため、 *異なる* レイヤーの場合に Azure Backup と比較して、かかわらずどのような暗号化ソフトウェアを使用します。 ただし、このエクスペリエンスについては、Bitlocker と dmcrypt でのみ検証されています。<br><br>データを暗号化するには、キーが必要になります。 また、データへの承認されたアクセスを確保するためにも安全にキーを保持する必要があります。  |
