...
2. **週ごとの保有期間ポリシー**: 毎日午前 0 時と土曜日の午後 6 時に作成されるバックアップは 4 週間保存されます
3. **月ごとの保有ポリシー**: 12 の午前 0 時と各月の最後の土曜日の午後 6 時に作成されるバックアップは保持されます
4. **年ごとの保有ポリシー**: 毎年 3 月の最後の土曜日の午前 0 時に作成されるバックアップは 10 年間保存されます
...
> [AZURE.NOTE] Azure のバックアップの回復ポイントの数に制限はありません。

## 詳細な構成
クリックして **変更** 上記の画面では、顧客があるより柔軟に保持スケジュールを指定することをします。
