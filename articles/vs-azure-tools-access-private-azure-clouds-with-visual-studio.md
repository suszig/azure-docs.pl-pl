...
1. プライベート クラウドの管理ポータルで、発行設定ファイルをダウンロードするか、管理者に連絡して発行設定ファイルを入手します。 これをダウンロードするリンクは、Azure のパブリック バージョンは、 [https://manage.windowsazure.com/publishsettings/](https://manage.windowsazure.com/publishsettings/)します。 (ダウンロードしたファイルには .publishsettings という拡張子が付きます)。

1.  **サーバー エクスプ ローラー** Visual Studio で、選択、 **Azure** ノードし、ショートカット メニューを選択、 **サブスクリプションの管理** コマンドです。

    ![Manage subscriptions command](./media/vs-azure-tools-access-private-azure-clouds-with-visual-studio/IC790778.png)

1.  **Microsoft Azure サブスクリプションの管理** ] ダイアログ ボックスで、選択、 **証明書** ] タブをクリックして、 **インポート** ] ボタンをクリックします。

    ![Importing Azure certificates](./media/vs-azure-tools-access-private-azure-clouds-with-visual-studio/IC790779.png)

1.  **Microsoft Azure サブスクリプションのインポート** ダイアログ ボックスで、発行設定ファイルを保存し、ファイルを選択した場所のフォルダーへの参照を選択し、 **インポート** ] ボタンをクリックします。 発行設定ファイル内の証明書が Visual Studio にインポートされます。 これで、プライベート クラウドのリソースを操作できるようになりました。
...
[方法: 発行の設定とサブスクリプション情報のダウンロードとインポート](https://msdn.microsoft.com/library/dn385850(v=nav.70).aspx)
