...
| Właściwość | Opis |
| --- | --- |
| serwer |Wymagane. Akceptuje wartości ciągu reprezentujący adres TCP/IP lub alias w formacie IPv4 lub IPv6, a następnie (-średnikami) przez numer portu TCP/IP. |
| Bazy danych |Wymagane. Akceptuje wartości ciągu reprezentujący DRDA relacyjne nazwę bazy danych (RDBNAM). Informix akceptuje ciąg 128-bajtowych (baza danych jest znany jako nazwa bazy danych programu IBM Informix (dbname)). |
| Uwierzytelnianie |Opcjonalny. Akceptuje wartości elementu listy, Basic lub systemu Windows (kerberos). |
| nazwa użytkownika |Wymagane. Akceptuje wartości ciągu. |
