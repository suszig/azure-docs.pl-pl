...
- [ワンボックス クラスターに対してアクションを実行します。](#run-an-action-against-a-one-box-cluster)
- [Azure クラスターに対してアクションを実行します。](#run-an-action-against-an-azure-cluster)
...
**CompleteMode** - 完了モードには、再起動アクションが実際に成功したかどうかを検証する必要がないことを指定します。完了モードを "Verify" と指定すると、再起動アクションが実際に成功したかどうかの検証が行われます。  
**OperationTimeout** - 操作が終了するまでの時間を設定します。この時間が経過すると TimeoutException 例外がスローされます。
...
- サービスをテストする方法
   - [サービス ワークロード中のエラーをシミュレーションします。](service-fabric-testability-workload-tests.md)
