...
品質をより深く検証するために、さまざまなグレースフル障害とアングレースフル障害エラーを発生させながら、サービスとビジネスのワークロードを実行します。 アングレースフル障害は、何らかのワークフローの途中でサービス プロセスが突然終了するシナリオで使用します。 Service Fabric によってサービスのレプリカが復元されると、復旧パスをテストします。 障害発生後のデータの整合性とサービス状態が正しく維持されているかどうかをテストするために役立ちます。 もう 1 つの障害セットであるグレースフル障害テストは、サービスが Service Fabric によって移動されるレプリカに対して正しく反応することをテストします。 これは、RunAsync メソッドでのキャンセル処理をテストします。 サービスは、設定されるキャンセル トークンをチェックし、その状態を正しく保存し、RunAsync メソッドを終了する必要があります。
...
このチュートリアルでは、PowerShell を使用して Testability アクションを実行する方法を示します。 (別名ローカルに対して Testability アクションを実行する方法を学びますです。 ワンボックス) クラスターまたは Azure のクラスター。 Microsoft.Fabric.Testability.Powershell.dll (Testability PowerShell モジュール) はMicrosoft Service Fabric MSI のインストール時に自動的にインストールされます。モジュールは、PowerShell プロンプトを開いたときに自動的に読み込まれます。
...
ローカル クラスターに対して Testability アクションを実行するには、クラスターに接続した後、管理者モードで PowerShell プロンプトを開く必要があります。 お知らせを見て、 **Restart-servicefabricnode** アクション。

```powershell
Restart-ServiceFabricNode -NodeName Node1 -CompletionMode DoNotVerify
```

ここでは、アクション **Restart-servicefabricnode** が"Node1"という名前のノードで実行され、入力候補モードはいるかどうか、再起動アクション実際が成功した;"Verify"と、再起動アクションが実際に成功したかどうかを確認する入力候補モードを指定する必要がありますいないを検証を指定します。 ノードは、名前で直接指定する代わりに、パーティション キーとレプリカの種類を使用して次のように指定することができます。
...
**Restart-servicefabricnode** クラスターで Service Fabric ノードを再起動するために使用する必要があります。 これは、そのノードでホストされているすべてのシステム サービスとユーザー サービスのレプリカを再起動する Fabric.exe プロセスを終了します。 この API を使用したサービスのテストは、フェールオーバー時の復旧パスに含まれるバグを検出するために役立ちます。 クラスター内のノード障害をシミュレートするために役立ちます。

次のスクリーン ショット、 **Restart-servicefabricnode** Testability コマンドをします。

![](media/service-fabric-testability-actions/Restart-ServiceFabricNode.png)

最初の出力 *Get-servicefabricnode* (ServiceFabric PowerShell モジュールのコマンドレット) は、5 つのノードがローカル クラスターにあることを示しています: Testability アクション (コマンドレット) を実行した後に Node.5; Node.1 **Restart-servicefabricnode** node.4 で、ノードで、そのノードのアップタイムがリセットされたことを参照してください。
...
**CompleteMode** -完了モードでは、その必要がありますを検証しないこと再起動アクションが実際に成功したかどうかを指定します。"Verify"と入力候補モードを指定すると、再起動アクションが実際に成功したかどうかを確認します。  
**OperationTimeout** -、操作の終了を TimeoutException 例外がスローされるまでの時間数を設定します。
**CancellationToken** -保留中の呼び出しをキャンセルできるようにします。

ノードは、名前で直接指定する代わりに、パーティション キーとレプリカの種類を使用して指定することができます。

詳細については、次を参照してください。 [パーティション セレクターとレプリカ セレクター](#partition_replica_selector)します。
...
Guid partitionIdGuid = 新しい Guid("8fb7ebcc-56ee-4862-9cc0-7c6421e68829") です。
PartitionSelector partitionSelector = PartitionSelector.PartitionIdOf (serviceName、partitionIdGuid) です。
長い replicaId = 130559876481875498 です。
