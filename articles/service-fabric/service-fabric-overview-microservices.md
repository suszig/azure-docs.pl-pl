...
*** 各に対する個々 の変更と通信するために合意した標準のサービスの分離されたフェデレーションのマイクロ サービスのデザイン アプローチは、で簡単に言うと、***
...
*** マイクロ サービス アプリケーション構成は小規模とは独立してバージョン管理された、拡張性の高い顧客を重視サービスが適切に定義されたインターフェイス * で標準のプロトコル経由で互いに通信します。**
...
*** 1 つのデータベースと階層の特定の technologies.* モノリシック アプローチの左側に**

*** 右側ののマイクロ サービスのアプローチ グラフ相互接続されたマイクロ サービスの状態は、通常、マイクロ サービスに制限される、さまざまなテクノロジが使用されます。**
...
*** チームがいるため、効率的に利用するインフラストラクチャのリソースがマイクロ サービス アプローチ * を使用してビジネス上の問題を解決できる、Service Fabric の目的は建物の困難な問題を解決して、アップグレードの障害などのサービスを実行します。**
...
ただし、組み込みのプログラミングは簡単なマイクロ サービスを構築する Api は提供は***
...
* Service Fabricセットアップ [開発環境](service-fabric-get-started.md)
