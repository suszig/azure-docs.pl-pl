...
- `OnActivateAsync` (アクターの実装でオーバーライドできる) メソッドが呼び出されます。
- アクターは、アクティブ アクター テーブルに追加されます。

アクターを非アクティブ化する場合:

- アクターは、一定期間使用されていない場合、アクティブ アクター テーブルから削除されます。
- `OnDeactivateAsync` (アクターの実装でオーバーライドできる) メソッドが呼び出されます。 呼び出されると、アクターのタイマーをすべてクリアします。

> [AZURE.TIP] Fabric アクター ランタイムは、いくつか出力 [アクターのアクティブ化と非アクティブ化に関連するイベント](service-fabric-reliable-actors-diagnostics.md#actor-activation-and-deactivation-events)します。 これらは、診断やパフォーマンスの監視に役立ちます。
...
- 呼び出しを受信する
- `IRemindable.ReceiveReminderAsync` (アクターがアラームを使用する場合にのみ適用) 呼び出されるメソッド
...
通常は、これらの既定値を変更する必要はありません。 ただし、必要に応じて、これらの期間はそのアセンブリ内のすべてのアクター型のアセンブリ レベルまたはアクター型レベルを使用して、 `ActorGarbageCollection` 属性です。 次の例は、HelloActor のガベージ コレクション間隔の変更を示しています。
...
既定値を変更する、 `ActorGarbageCollection` アセンブリ レベル属性は、次のスニペットを追加 `AssemblyInfo.cs`します。

```csharp
[assembly: ActorGarbageCollection(IdleTimeoutInSeconds = 10, ScanIntervalInSeconds = 2)]
```

アクティブ アクター テーブル内の各アクターについて、アクター ランタイムはアイドル (つまり、未使用) 状態だった時間を追跡します。 アクター ランタイムは、アクターをそれぞれチェックすべて `ScanIntervalInSeconds` ガベージ コレクトできるアイドル時間経過している場合は、それを収集し、 `IdleTimeoutInSeconds`します。

アクターが使用されるたびに、そのアイドル時間は 0 にリセットされます。 その後、アクターは、ガベージ コレクトできる、再びアイドル状態の場合にのみ `IdleTimeoutInSeconds`します。 アクター インターフェイス メソッドまたはアクター アラーム コールバックが実行された場合、アクターは使用されていると見なされることを思い出してください。 タイマー コールバックが実行された場合、アクターは使用されていると見なされ**ません**。
