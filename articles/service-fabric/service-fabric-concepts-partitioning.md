...
4. [サービス] ダイアログ ボックスの作成でのステートフル サービスを選択し、Alphabet.Processing 次の図に示すように付けます。
![alphabetstateful](./media/service-fabric-concepts-partitioning/alphabetstatefulnew.png)
5. パーティション数を設定します。 AlphabetPartitions プロジェクトの ApplicationManifest.xml を開き、下図のようにパラメーター Processing_PartitionCount を 26 に更新します。
...
リッスンする URL は HttpListener に渡されます。 公開される URL は、Service Fabric Naming Service に公開される URL です。サービスの検出に使用されます。 クライアントは検出サービスを介してこのアドレスを要求します。 接続するには、クライアントが取得するアドレスにノードの実際の IP または FQDN が含まれる必要があります。そのため、上のように '+' をノードの IP または FQDN に置き換えます。
