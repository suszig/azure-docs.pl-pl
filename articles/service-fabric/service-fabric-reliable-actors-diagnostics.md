...
カテゴリの `Service Fabric アクター`, 、カウンター インスタンス名は、次の形式。

`ServiceFabricPartitionID_ActorsRuntimeInternalID`

*ServiceFabricPartitionID* - パフォーマンス カウンター インスタンスが関連付けられている Service Fabric パーティション ID の文字列表現。 パーティション ID は GUID を使用して、文字列形式を生成します [`Guid.ToString`] 書式指定子"D"を持つ (https://msdn.microsoft.com/library/97af8hh4.aspx) メソッドです。
...
上記の例で `2740af29-78aa-44bc-a20b-7e60fb783264` Service Fabric パーティション ID の文字列表現と `635650083799324046` はランタイムの内部用に生成される 64 ビットの ID を使用します。

#### `Service Fabric アクター メソッド` カテゴリ

カテゴリの `Service Fabric アクター メソッド`, 、カウンター インスタンス名は、次の形式。
...
上記の例で `ivoicemailboxactor.leavemessageasync` メソッド名は、 `2` ランタイムの内部で使用、生成された 32 ビットの ID は、 `89383d32-e57e-4a9b-a6ad-57c6792aa521` Service Fabric パーティション ID の文字列表現ですし、 `635650083804480486` はランタイムの内部用に生成された 64 ビットの ID を使用します。
