...
4. **サービス アカウントと RunAs**: Service Fabric 自体は、Windows Service のプロセス (Fabric.exe) として実行されます。Fabric.exe プロセスを実行するセキュリティ アカウントを構成できます。 クラスター内の各ノードで Fabric.exe を実行するプロセス アカウントに加えて、サービスごとにアクティブ化されるサービス ホスト プロセスを保護できます。 参照してください [アプリケーションのセキュリティおよび Runas](service-fabric-application-runas-security.md) 詳細についてはドキュメント
...
```
呼び出す AddCertToKeyVault SubscriptionId <you subscription id> - ResourceGroupName <string> -場所 <region> - VaultName <Name of the Vault> - CertificateName <Name of the Certificate> -パスワード <Certificate password> - UseExistingCertificate ExistingPfxFilePath <Full path to the .pfx file>
```
Here is a filled out script as an example.
```
呼び出す AddCertToKeyVault-SubscriptionId 35389201-c0b3-405e-8a23-9f1450994307 - ResourceGroupName chackdankeyvault4doc-場所 westus - VaultName chackdankeyvault4doc - CertificateName chackdantestcertificate2-パスワード abcd123 UseExistingCertificate ExistingPfxFilePath C:\MyCertificates\ChackdanTestCertificate.pfx
```
...
The script will create a new resource group and/or a vault if they are not already present.
```
呼び出す AddCertToKeyVault SubscriptionId <you subscription id> - ResourceGroupName <string> -場所 <region> - VaultName <Name of the Vault> - CertificateName <Name of the Certificate> -パスワード <Certificate password> - CreateSelfSignedCertificate DnsName <string- see note below.> -outputpath <Full path to the .pfx file>
```
...
Here is a filled out script as an example.
```
呼び出す AddCertToKeyVault-SubscriptionId 35389201-c0b3-405e-8a23-9f1450994307 - ResourceGroupName chackdankeyvault4doc-場所 westus - VaultName chackdankeyvault4doc - CertificateName chackdantestcertificate3-パスワード abcd123 CreateSelfSignedCertificate-DnsName www.chackdan.westus.azure.com-outputpath C:\MyCertificates
```
...
```
インポート PfxCertificate-CertStoreLocation エクスポート可能な証明書: \CurrentUser\TrustedPeople-filepath C:C:\MyCertificates\ChackdanTestCertificate.pfx-パスワード (Read-host-assecurestring の「証明書のパスワードを入力」プロンプト)
```
```
...
一般的に、x509 デジタル証明書は、クライアントとサーバーの認証、暗号化、メッセージのデジタル署名に使用されています。 これらの証明書の詳細については、」に進んでください [http://msdn.microsoft.com/library/ms731899.aspx](http://msdn.microsoft.com/library/ms731899.aspx)
...
- [Service Fabric クラスター アップグレード プロセスおよびを想定する事項](service-fabric-cluster-upgrade.md)
