...
Service Fabric に導入、 [正常性モデル](service-fabric-health-introduction.md) 問題のあるクラスターまたは特定のエンティティでのアプリケーションの条件にフラグを設定するように設計します。 これは、**正常性レポーター** (システム コンポーネントおよびウォッチドッグ) を使用して実現されます。 その目標は、簡単かつ迅速な診断および修復です。 サービスの作成者は、正常性に関して事前に検討する必要があります。 正常性に影響する可能性があるすべての条件は、特にルートに近い問題にフラグを設定するために役立つ場合に、レポートする必要があります。 これにより、クラウド (プライベートまたは Azure) で、サービスが大規模に開始された後に、デバッグと調査が大幅に削減されます。
...
- Service Fabric のクラスター**外**からリソースのプローブを実行している外部ウォッチドッグ。 例: サービスを監視するように Gomez します。

> [AZURE.NOTE] 既定では、クラスターには、システム コンポーネントによって送信される正常性レポートが事前設定されます。 「 [を使用してシステム正常性レポートのトラブルシューティングの](service-fabric-understand-and-troubleshoot-with-system-health-reports.md)します。 ユーザーのレポートを送信する必要が [正常性エンティティ](service-fabric-health-introduction.md#health-entities-and-hierarchy) 、システムによって作成済みです。
...
> [AZURE.NOTE] 正常性レポートは同期的であり、クライアント側の検証作業のみを表します。 正常性クライアントによって、レポートを受け入れることというわけではストア; に適用されます。それが送信、非同期的に可能性があるその他のレポートと共にバッチ処理され、サーバー上の処理が失敗する (例: です。 古いシーケンス番号、レポートを適用する必要がありますエンティティがされた削除済みなど)。
...
> [AZURE.NOTE] 正常性ストアは、状態情報を保持するために使用**しない**でください。 正常性に関連する情報のみを、エンティティの正常性評価に影響を与える正常性情報としてレポートする必要があります。 正常性ストアは、汎用目的のストアとして設計されていません。 これは、すべてのデータを正常性状態に集計するために、正常性評価のロジックを使用します。 (例: 非稼働状態の関連情報を送信します。 正常性状態のステータスを Ok reporting) と、集計正常性状態に影響はありませんが、正常性ストアのパフォーマンスが低下することができます。
...
[Service Fabric の概要状態の監視](service-fabric-health-introduction.md)

[Service Fabric の正常性レポートを表示する方法](service-fabric-view-entities-aggregated-health.md)
