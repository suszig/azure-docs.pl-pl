...
2. Out パラメーターを使用しない: を使用して `ConditionalResult<T>` bool 型の値と出力パラメーターの代わりに値を返します。 `ConditionalResult<T>` ように、 `Nullable<T>` 、構造体にするのには「T は必要ありません。
...
2. [信頼性の高いキュー](https://msdn.microsoft.com/library/azure/dn971527.aspx): レプリケートされた、トランザクション、および非同期厳密な先入れ先出し (FIFO) キューを表します。 `ConcurrentQueue` のように、値には任意の型を使用できます。
...
- **そうしない** 読み取り操作によって返されるカスタム型のオブジェクトを変更 (例: `TryPeekAsync` または `TryGetAsync`)。 Reliable Collection は、同時実行コレクションのように、コピーではなくオブジェクトへの参照を返すからです。
- **** 詳細コピーを変更する前にカスタム型のオブジェクトが返されます。 構造体型と組み込み型は値渡しであるため、詳細コピーを実行する必要はありません。
- **そうしない** を使用して `TimeSpan.MaxValue` のタイムアウト。 タイムアウトはデッドロックの検出に使用してください。
- **そうしない** 別のトランザクション内でトランザクションを作成 `using` ステートメント デッドロックを引き起こす可能性があるためです。
