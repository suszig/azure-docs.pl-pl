...
サービス インスタンスは、アプリケーション名とサービスの種類名の特定のインスタンス (を参照してください [Service Fabric アプリケーションのライフ サイクル](service-fabric-application-lifecycle.md))します。 サービスの作成時にパーティション構成を指定する (を参照してください [Service Fabric のパーティション分割サービス](service-fabric-concepts-partitioning.md)) 使用します。
