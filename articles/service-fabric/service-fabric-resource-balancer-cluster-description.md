<properties
   ...
   description="Service Fabric クラスターを記述する、フォールト ドメインを指定することによって、ドメイン、ノードのプロパティ、およびノード容量をリソース バランサーにアップグレードします。"
...
![アクティビティしきい値][Image6]
リソース バランサーを実行させるのには、同じメトリックを超過する必要があります、アクティビティおよび分散しきい値の両方にも注意してください。 2 つ内いずれかのメトリックをトリガーするだけでは、リソース バランサーは実行されません。
