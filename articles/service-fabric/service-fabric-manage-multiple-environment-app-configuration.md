...
単純な例として、 `InstanceCount` ステートレスなサービスです。 Azure での実行時には、通常、このパラメーターを特殊な値である -1 に設定し、クラスター内のどのノードでもサービスが確実に実行されるようにします。 ただし、この構成はワンボックス クラスターには適していません。理由は、1 台のコンピューター上の同じエンドポイントで複数のプロセスをリッスン状態にしておくことができないからです。 代わりを通常設定、 `InstanceCount` を 1 にします。
...
[Service Fabric アプリケーション モデル](service-fabric-application-model.md) により、サービスを実行時に読み取り可能なカスタムのキーと値のペアを格納している構成パッケージを含めます。 指定して、これらの設定の値が環境で区別できるようにもできる、 `ConfigOverride` アプリケーション マニフェストにします。

Config\Settings.xml の場合は、次の設定があると、 `Stateful1` サービス。
...
1 組の特定のアプリケーション/環境に対して、この値を変更するには、作成、 `ConfigOverride` アプリケーション マニフェスト内のサービス マニフェストをインポートするときにします。
...
`DeployCreate FabricApplication.ps1` PowerShell スクリプトは、パラメーターとして、パラメーター ファイルを受け付けます。
...
このトピックで説明した基本的な概念のいくつかについての詳細についてを参照してください、 [Service Fabric の技術概要](service-fabric-technical-overview.md)します。 Visual Studio で使用できるその他のアプリケーション管理機能については、次を参照してください。 [Visual Studio で Service Fabric アプリケーションの管理](service-fabric-manage-application-in-visual-studio.md)します。
