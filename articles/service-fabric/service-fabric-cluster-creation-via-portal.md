...
2. をクリックして、 **+ 新規** 新しいリソース テンプレートを追加します。 検索が呼び出されます下にあるすべての市場でテンプレートの **サービス ファブリック クラスター**
(に移動できる **すべて** 最上位のカテゴリのマーケットプ レースに分断 > と、「すべて」の下の「ファブリック」を検索し、enter キーを押して自動フィルターが機能しない場合があります、そうすること **enter キーを押して**)  
    ![SearchforServiceFabricClusterTemplate][SearchforServiceFabricClusterTemplate]

3. 一覧から "Service Fabric Cluster" を選択します
4. サービス ファブリック クラスター ブレードに移動し、をクリックして **作成** クラスター上の詳細の指定
...
6. 選択してください、 **サブスクリプション** 特に複数のサブスクリプションがある場合に、展開するようにクラスターをする.

7. 選択、 **場所** (別の場所、else 米国西部に既定でを作成するには) 場合に、ドロップダウン リストから

8. 構成、 **ノード型**します。 ノードの種類は、Cloud Services の「ロール」と同等のものと見なすことができます。 VM のサイズ、VM の数、プロパティが定義されています。  1 つのクラスターで複数のノードの種類を使用できます。 唯一の制約は、少なくとも 1 つのノードの種類 (プライマリ、つまりポータルで定義する最初のノード) を 5 VM 以上にすることです。
    ...
    5. **複数のノード型がある場合、ノード型に関する考慮事項**します。 単一ノード種類のクラスターをデプロイする場合は、次の手順を省略します。
...
9. **アプリケーション ポート**- クラスターにアプリケーションをすぐに展開し、このノード型 (または作成した Nodetype) でアプリケーションを開く場合のポートを追加する予定の場合。 追加できますポート ノードの種類に後で、ロード バランサーに関連付けられた (プローブを追加し、負荷 blanancer ルール、プローブを追加する必要があります) このノード型を変更することで。 今すぐ行うと、ポータルの Automation が必要なプローブとルールを LB に追加するので少し楽です。 
...
10. **省略可能: 配置プロパティ**-その他の構成を追加する必要はありません、"NodeTypeName"の既定の配置プロパティがシステムによって追加します。 アプリケーションで必要な場合はさらに追加できます。 
...
セキュリティの考慮事項およびに文書化する方法の詳細について [サービス ファブリック クラスターのセキュリティ](service-fabric-cluster-security.md)
...
## クラスター作成の完了
クリックして、 **概要** が用意する構成を参照してください。 または、クラスターを展開に使用される ARM テンプレートをダウンロードします。 必須の設定を指定すると、[作成] ボタンが有効になり、クラスター作成プロセスを開始できます。 
...
3. これにより、クラスターのパブリック IP アドレスなどのクラスターに関する詳細が、ダッシュボードに表示されます。 経由でそのカーソルを置くとに注意してください **パブリック IP アドレスをクラスター** 、クリップボードにコピー] をクリックすることができますが表示されます。
...
次のコマンドを実行してアプリケーションをデプロイします。パスは実際のコンピューターのものに置き換えます。
 ...
 - オプション 2 **セキュリティで保護されたクラスターに接続します。**
