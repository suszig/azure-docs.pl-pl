...
1. VM の数を目的の数に調整し、保存します。スライダーの下限は、その NodeType の VM の最小要件を示します。
  >[AZURE.NOTE] 主要なノードの種類では、最低 5 つの VM を維持する必要があります。
    ...
    その展開が完了した後、削除できるようになりました VM 名の通知が取得されます。 これで、VM のリソースに移動して、それを削除する必要があります。
...
- [クラスターのアップグレードについてください。](service-fabric-cluster-upgrade.md)
- [最大小数点以下桁数のステートフル サービスのパーティション分割についてください。](service-fabric-concepts-partitioning.md)
