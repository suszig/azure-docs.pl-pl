...
[Event Tracing for Windows](https://msdn.microsoft.com/library/windows/desktop/bb968803.aspx) (ETW) は、Service Fabric メッセージをトレースするための推奨されるテクノロジです。 その理由は、次のとおりです。
...
1. 呼び出し `ServiceEventSource.Current.ServiceMessage` で、 `RunAsync` メソッドは、アプリケーション コードからのカスタムの ETW トレースの例を示します。
...
2. コードを見つけます `ActorEventSource.Current.ActorMessage (このため、「を行う作業」);` で、 *DoWorkAsync* メソッドです。 これは、アプリケーション コードからのカスタムの ETW トレースの例です。
...
* [Service Fabric アプリケーションのトレース ストアとして ElasticSearch を使用します。](service-fabric-diagnostic-how-to-use-elasticsearch.md)
