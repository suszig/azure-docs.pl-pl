...
1) 高スループット、低待機時間、エラー トレラントな OLTP を構築する機能は、対話型のストア フロント、検索、モ ノのインターネット (IoT) システム、取引システム、クレジット_カード処理、不正検出システムのようなサービスで、個人記録管理などのコードとデータを保持することで、同じコンピューターにウィンドウを閉じる。

2) アプリケーション設計が単純化の追加キューおよび従来から純粋なステートレス アプリケーションの可用性、待機時間の要件に対処する必要があるキャッシュのステートフル マイクロ サービスを削除します。 ステートフル サービスは高可用性と低待機時間を特徴とするため、アプリケーション全体では管理すべき変動要素が少なくなります。 

アプリケーション パターンとデザインの詳細については Service Fabric を使用して参照してください [アプリケーション シナリオ](service-fabric-application-scenarios.md)
...
ほとんどのアプリケーションは、ステートレス マイクロサービスとステートフル マイクロサービスの組み合わせ、および一緒にデプロイされた他の EXE/ランタイムで構成されています。 Service Fabric は、アプリケーションの厳密な型およびパッケージ化されたマイクロサービスを備えており、個別に管理およびアップグレードできる複数のアプリケーション インスタンスのデプロイが可能です。 Service Fabric が展開することは重要なは *、* 実行可能ファイルまたはランタイムし、これらを信頼性の高い。 たとえば、ASP.NET 5、node.js、スクリプト、またはアプリケーションを構成する任意の要素をデプロイすることができます。 
  
アプリケーション ライフ サイクル管理の詳細については「 [アプリケーションのライフ サイクル](service-fabric-application-lifecycle.md)
...
* 詳細: 
    * [なぜアプリケーションを構築するためのマイクロ サービス アプローチ?](service-fabric-overview-microservices.md)します。
    * [技術概要](service-fabric-technical-overview.md)します。
* Service Fabric のセットアップ [開発環境](service-fabric-get-started.md)します。  
