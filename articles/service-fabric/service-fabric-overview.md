...
(例: プロトコル ゲートウェイ、web プロキシなど)、ステートレス マイクロ サービス すべての要求およびサービスから応答の外部で変更可能な状態を維持しません。 ステートレス サービスの例には Azure Cloud Services worker ロールがあります。 ステートフル マイクロ サービス (例: ユーザー アカウント、データベース、デバイス、ショッピング カート、キューなど) 要求および応答変更可能な認証状態を維持します。 今日のインターネット規模のアプリケーションは、ステートレス マイクロサービスとステートフル マイクロサービスの組み合わせで構成されています。
...
詳細については、アプリケーションのパターンと Service Fabric を使用して設計を参照してください [アプリケーション シナリオ](service-fabric-application-scenarios.md)
...
詳細については、アプリケーションのライフ サイクル管理を参照してください [アプリケーションのライフ サイクル](service-fabric-application-lifecycle.md)
...
* 詳細:
    * [ため、アプリケーションを構築するためのマイクロ サービス アプローチ?](service-fabric-overview-microservices.md).
    * [の技術概要](service-fabric-technical-overview.md)します。
