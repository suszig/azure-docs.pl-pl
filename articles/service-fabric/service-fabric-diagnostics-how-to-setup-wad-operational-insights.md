...
* [オペレーション インサイト](https://azure.microsoft.com/services/operational-insights/)
...
1. Service Fabric ログ: プラットフォームから標準の ETW と EventSource チャネルに対して生成されます。 次の種類があります。
  ...
  - [アクターのプログラミング モデルのイベント](https://azure.microsoft.com/documentation/articles/service-fabric-reliable-actors-diagnostics/)
  - [Reliable Services プログラミング モデルのイベント](https://azure.microsoft.com/documentation/articles/service-fabric-reliable-services-diagnostics/)
...
ARM サンプルをダウンロードできますに変更を加えるおよびを使用して、変更したテンプレートを使用してクラスターを作成する別の方法として、 `New-azureresourcegroupdeployment` Azure Powershell ウィンドウでコマンドです。 コマンドで渡す必要があるパラメーターについては、後述します。 さらに、この展開コマンドを呼び出す前にする必要があります、Azure アカウントを追加するなどのセットアップを行う (`Add-azureaccount`)、サブスクリプションを選択する (`Select-azuresubscription`)、ARM モードに切り替える (`Switch-azuremode AzureResourceManager`) まだ行っていない場合は、リソース グループを作成して (`New-azureresourcegroup`)。
...
```
Json の作成後は、上記としてファイルし、変更して、環境の詳細について、Service Fabric クラスターのリソース グループの名前を渡して、このコマンドを呼び出します。 このコマンドの実行に成功すると、WAD はすべての VM にデプロイされ、指定した Azure ストレージ アカウントのテーブルに、クラスターのログがアップロードされるようになります。 さらに、この展開コマンドを呼び出す前にする必要があります、Azure アカウントを追加するなどのセットアップを行う (`Add-azureaccount`)、適切なサブスクリプションを選択する (`Select-azuresubscription`) と ARM モードに切り替え (`Switch-azuremode AzureResourceManager`)。
```powershell
...
[オペレーション インサイト オンボード](https://technet.microsoft.com/library/mt484118.aspx)
...
![Operational Insights の ](./media/service-fabric-diagnostics-how-to-setup-wad-operational-insights/log-search-button-oi.png)
