...
Service Fabric コンポーネントは、追加の設定なしでクラスター内のすべてのエンティティについてレポートします。  [正常性ストア](service-fabric-health-introduction.md#health-store) を作成して、システム レポートに基づいてエンティティを削除し、エンティティの相互作用をキャプチャする階層に編成します。
> [AZURE.NOTE] 詳細について、 [Service Fabric の正常性モデル](service-fabric-health-introduction.md) 関連する概念を正常性を把握します。
...
> [AZURE.NOTE] ウォッチドッグ正常性レポートは、システム コンポーネントでエンティティが作成された**後**にのみ表示できます。 エンティティが削除されると、正常性ストアは関連付けられているすべての正常性レポートを自動的に削除します。 (例: エンティティの新しいインスタンスが作成されるときに同じ。 新しいサービス レプリカのインスタンスが作成されます): 古いインスタンスに関連付けられているすべてのレポートが削除され、ストアからクリーンアップします。

システム コンポーネント レポートはソース別に識別され、*"System"* プレフィックスで始まります。 ウォッチドッグのソースに同じプレフィックスを使用することはできません (レポートが無効なパラメーターで拒否されるため)。
...
System.Federation は、ネットワーク コンピューターの消失を検出すると Error を報告します。 レポートのデータは個々のノードから収集され、ノード ID がプロパティ名に含まれます。 Service Fabric リング全体でネットワーク コンピューターの消失が 1 件あった場合、通常は 2 つのイベントを想定できます (問題の両側が報告されます)。 失われたネットワーク コンピューターが他にもある場合は、イベントの数はもっと多くなります。
レポートは、TTL としてグローバル リース タイムアウトを指定し、条件がアクティブな間、レポートが TTL の半分の間隔で再送信されます。 イベントは期限切れになると自動的に削除されるため、レポート ノードが停止した場合でも、正常性ストアから正しくクリーンアップされます。

- SourceId: System.Federation
- プロパティ: "Neighborhood" で始まり、ノードの情報が含まれています。
- 次のステップ: ネットワーク コンピューターが失われた理由を調査します。 例: クラスター ノード間の通信を確認してください。
...
System.FabricNode は、ノードで使用されている証明書の期限が近づくと Warning を報告します。 ノードごとに、Certificate_cluster、Certificate_server、Certificate_default_client という 3 つの証明書があります。
有効期限まで 2 週間以上ある場合、レポートの種類は OK です。有効期限まで 2 週間以内の場合、レポートの種類は Warning です。 これらのイベントの TTL は無制限で、イベントはノードがクラスターから切り離されると削除されます。
...
- SourceId: System.CM
- プロパティ: State
- 次のステップ: アプリケーションが作成されると、CM 正常性レポートが生成されます。それ以外の場合、クエリを発行して、アプリケーションの状態を確認 (例: Powershell コマンドレット Get-servicefabricapplication-ApplicationName <applicationName>)。
...
System.FM は、パーティションが作成されて正常なときに、Ok を報告します。 サービスが削除されると、正常性ストアからエンティティを削除します。
パーティションが最小限のレプリカ数を下回ると、Error を報告します。
パーティションがない場合、最小限のレプリカ数を下回っていなくても、ターゲット レプリカ数を下回るときは、Warning を報告します。
パーティションがクォーラム損失の状態にあるとき、FM はエラーを報告します。
その他の重要なイベントとして、再構成に予想よりも時間がかかる場合と、ビルドに予想よりも時間がかかる場合に Warning を報告します。 ビルドまたは再構成の予期される時間は、サービスのシナリオに基づいて構成可能です。たとえば、サービスが TB 状態のとき (SQL Azure など)、小容量の状態を持つサービスよりもビルドに長くかかります。
...
この正常性レポートの説明には、API 呼び出しが行われたときの開始時刻 (UTC) が含まれます。
System.RA は、レプリカを開くために、構成されている期間 (既定値: 30 分) よりも長い時間がかかる場合、Warning を報告します。 API がサービスの可用性に影響する場合、レポートは大幅に短時間で発行されます (間隔は構成可能で既定値は 30 秒)。 この時間には、レプリケーターを開く処理とサービスを開く処理にかかる時間が含まれます。 プロパティは、開く処理が完了すると Ok に変わります。
...
- プロパティ: Activation ロールアウトのバージョンが含まれます。
...
- SourceId: System.Hosting
- プロパティ: ダウンロードします。<RolloutVersion>
...
System.Hosting は、各コード パッケージのアクティブ化が成功すると、Ok を報告します。 アクティブ化が失敗した場合は、構成に従って Warning を報告します。CodePackage のアクティブ化が失敗した場合、または、構成されている "CodePackageHealthErrorThreshold" を超えるエラーで中止された場合、Hosting は Error を報告します。
サービス パッケージに複数のコード パッケージがある場合、コード パッケージごとにアクティブ化レポートが生成されます。

- SourceId: System.Hosting
- プロパティ: プレフィックスが CodePackageActivation であり、コード パッケージとエントリ ポイントの名前が含まれます。Codepackageactivation で:<CodePackageName>:<SetupEntryPoint/EntryPoint>します。例:  CodePackageActivation:Code:SetupEntryPoint
