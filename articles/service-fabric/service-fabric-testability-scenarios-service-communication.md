...
この例では設定 `QuorumLossMode` に `PartialQuorumLoss` 読み取り操作にも行うことができるように、すべてのレプリカを停止させることがなくクォーラム損失を発生させることすることを指定します。 パーティション全体が利用可能なシナリオをテストするにこのスイッチを設定できます `FullQuorumLoss`します。

## 次のステップ

[Testability アクションの詳細についてください。](service-fabric-testability-actions.md)

[Testability のシナリオの詳細についてください。](service-fabric-testability-scenarios.md)
