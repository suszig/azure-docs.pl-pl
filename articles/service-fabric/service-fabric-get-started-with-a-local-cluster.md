...
> [AZURE.NOTE] この記事では、あると見なさを既にお持ち [開発環境を設定する](service-fabric-get-started.md)します。
...
4. ワード カウント アプリケーションをダウンロード [ここ](http://aka.ms/servicefabric-wordcountapp) を作成した場所にします。
...
3. 最後に、WordCountService のパーティションの一覧を見てみましょう。
    ...
    > [AZURE.NOTE] Service Fabric エクスプ ローラーの詳細については、を参照してください [Service Fabric エクスプ ローラーでクラスターの視覚化](service-fabric-visualizing-your-cluster.md)。
...
3. アップグレードの進行中は、Service Fabric Explorer でその状態を監視するのが簡単な場合もあります。 ブラウザー ウィンドウを起動しに移動 [http://localhost:19080/エクスプ ローラー](http://localhost:19080/Explorer)します。 左側のツリーで **[アプリケーション]** をクリックし、**[アップグレード進行中]** を選択します。
...
- この記事で実行したアップグレードは、非常に基本的なものです。 参照してください、 [アップグレード](service-fabric-application-upgrade.md) 機能と柔軟性の Service Fabric のアップグレードの詳細について。
