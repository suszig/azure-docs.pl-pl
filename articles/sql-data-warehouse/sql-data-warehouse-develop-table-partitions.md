...
- テーブルの作成時にパーティションを定義します。 パーティション境界点と、境界点を有効にするかどうかを指定するだけ `RANGE RIGHT` または `RANGE LEFT`します。
...
既にデータが含まれるパーティションに分割する最も効率的な方法を使用して、 `CTAS` ステートメントです。 パーティション テーブルがクラスター化列ストアの場合、テーブルのパーティションを分割するには、パーティションを空にしておく必要があります。
...
> [AZURE.NOTE] 統計オブジェクトを作成することによって、テーブルのメタデータをより正確なものにすることができます。 統計の作成を省略した場合は、既定値が使用されます。 に関する統計情報の詳細についてはレビューをご覧ください [統計情報][]します。

行カウントを利用して照会できますし、 `sys.partitions` カタログを表示します。
...
ただし使用できます `CTAS` 、データを保持する新しいテーブルを作成します。
...
使用して新しいパーティション境界にデータを対応させるために残されているすべては `CTAS` と、データをメイン テーブルに戻してスイッチ
...
>[AZURE.NOTE] パーティションの切り替えには、SQL Server と異なる点がいくつかあります。 必ずお読み [移行コード [][] をこのトピックの詳細を参照してください。
