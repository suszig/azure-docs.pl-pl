...
## 同時実行
理解することが重要 SQL Data Warehouse では、によって管理されている 2 つの概念 **同時実行クエリ** と **同時実行スロット**します。 

同時実行クエリは、同時に実行されるクエリの数を指します。 SQL Data Warehouse は、最大 32 をサポートしている **同時実行クエリ**します。 各クエリの実行は、直列クエリ  (単一スレッド) または並列クエリ (マルチスレッド) かどうかに関わらず、1 つのクエリとしてカウントされます。 これは固定された上限数であり、すべてのサービス レベルとクエリに適用されます。 

同時実行スロットはさらに動的な概念であり、データ ウェアハウスの Data Warehouse ユニット (DWU) サービス レベル目標に対して相対的です。 SQL Data Warehouse に割り当てられている DWU 数を増やすと、コンピューティング リソースが追加で割り当てられます。 ただし、DWU を増やすことの数が増加 **同時実行スロット** 使用できます。

原則として、同時実行の各クエリでは、1 つ以上の同時実行スロットが使用されます。 スロットの正確な使用数は次の 3 つの要因によって異なります。

1. SQL Data Warehouse の DWU 設定
2.  **リソース クラス** 、ユーザーが所属するには
3. クエリまたは操作が同時実行スロット モデルによって管理されるかどうか 

> [AZURE.NOTE] すべてのクエリが同時実行スロットのクエリ規則によって制御されることを注目に値します。 ただし、ほとんどのユーザーのクエリに適用されます。 クエリや操作によっては、同時実行スロットをまったく消費しません。 ただし、これらのクエリや操作は同時実行クエリの上限によって制限されているため、両方のルールについて説明しています。 参照してください、 [リソース クラスの例外](#exceptions) 詳細については後述します。 
...
SQL Data Warehouse の形式で 4 つの異なるリソース クラスを公開する **データベース ロール** 、ワークロード管理の実装の一部として。  
...
> [AZURE.NOTE] ワークロード管理グループとの間にユーザーを追加する方が多くの場合、別のログイン/ユーザーより高いリソース クラスに完全に割り当てられているを通じてより負荷の高い操作を開始する方が簡単です。
...
ステートメントと操作の一覧を次に示しますを **は** リソース クラスによって管理されます。
...
> [AZURE.NOTE] 強調表示に値します `SELECT` 動的管理ビューおよびカタログ ビューに対してのみ実行するクエリは、 **いない** リソース クラスによって管理されます。
...
> [AZURE.NOTE] 上記のクエリは、トラブルシューティングを行う場合は、ワークロード グループのアクティブおよび履歴の使用状況を分析するも使用できます。 
...
ユーザーに対して、1 回完全なアクセス許可が付与される必要があります。 付与次の例 `CONTROL` SQL Data Warehouse データベースにします。 `CONTROL` レベルは、データベースの db_owner SQL Server に相当します。
