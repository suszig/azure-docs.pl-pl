...
SQL Data Warehouse では上記のすべてのメソッドを使用できますが、PolyBase が Azure BLOB Storage からの読み込みを透過的に並列処理する機能を使用すると、データの読み込みが最速になります。  チェック アウトする方法の詳細 [PolyBase を使用して、読み込み][]します。  また、ユーザーの多くがオンプレミス ソースから 100 GB 単位から 10 TB 単位の初期読み込みを要求した場合のように、以下のセクションでは、初期データ読み込みについていくつかのガイダンスを提供します。  
...
> [AZURE.NOTE] SQL Server などのシステムからデータを移動する前に確認してください、 [スキーマの移行][] と [コード移行][] ドキュメントの記事です。 

## BCP によるファイルのエクスポート

Azure へのファイルの移動の準備として、ファイルをフラット ファイルにエクスポートする必要があります。  これには BCP コマンドライン ユーティリティを使用すると最適です。  ユーティリティがないまだを使用してダウンロードできるかどうか、 [SQL Server 用の Microsoft コマンド ライン ユーティリティ][]します。  サンプルの BCP コマンドは、次のようになります。
...
次のステップで、AZCopy を使って、オンプレミスから Azure ストレージ アカウントにデータを移動する方法を詳しく説明します。  同じリージョンに Azure ストレージ アカウントを持っていない場合は、次、1 つを作成、 [Azure Storage のドキュメント][]します。  異なるリージョンのストレージ アカウントからもデータをロードできます。しかし、この場合は最適なパフォーマンスとなりません。  

> [AZURE.NOTE] このドキュメントでは、AZCopy コマンド ライン ユーティリティをインストールし、Powershell で実行することが前提としています。  そうでない場合に従ってください、 [AZCopy インストール手順][]します。  
...
+ **Express Route**: 早いルートが有効になっている場合は前述のように、このプロセスをスピードアップにすることができます。  Express Route と構成手順の概要を参照して、 [ExpressRoute に関するドキュメント][]します。 
...
データが Azure ストレージ BLOB に存在するようになったら、SQL Data Warehouse インスタンスへ PolyBase を使用してデータをインポートします。  以下の手順は構成のみであり、手順の多くは SQL Data Warehouse インスタンス、ユーザー、ストレージ アカウントごとに一回だけ完了する必要があります。  次の手順が説明していますより詳細に説明、 [PolyBase を使用して、読み込み][] ドキュメントです。  
