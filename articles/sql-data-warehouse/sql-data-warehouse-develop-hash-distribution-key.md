...
> [AZURE.NOTE] 使用される列 `参加`, 、`GROUP BY`, 、`DISTINCT` と `HAVING` 句のすべての適切なハッシュ列の候補です。 内の手の形の列に、 `場所` 句 **いない** 良好なハッシュ列の候補を作成します。 バランスの取れた実行についてのセクションは、以下を参照してください。

クエリ構文からのデータ移動が発生する可能性も (`個別のカウント` と `経由で` 句は共に優れた例をされている) をハッシュ分散キーを含まない列を使用するとします。
...
1. 使用される `参加`, 、`GROUP BY`, 、`DISTINCT`, 、または `HAVING` 、クエリ内の句。
2. 使用されていない `、` 句
...
> [AZURE.NOTE] SQL Data Warehouse では、データ型の整合性が重要です！ 既存のスキーマが一貫して列に同じ型を使用しているか確認します。 これは、分散キーにとって特に重要です。 配布キーのデータ型が同期されないままテーブルが結合されると、不要なデータ移動が発生します。 これは、テーブルが大きく、スループットの低下とパフォーマンスになる場合コストがかかるです。
