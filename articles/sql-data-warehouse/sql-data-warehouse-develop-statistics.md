...
詳細については、次を参照してください。 [DBCC SHOW_STATISTICS][] MSDN にします。
...
詳細については、次を参照してください。 [統計][] MSDN にします。
...
実装については、 `UPDATE STATISTICS` の手順を参照してください、 [temporary tables] 記事です。 実装方法は前述の `CREATE STATISTICS` プロシージャと若干異なりますが、最終的な結果は同じです。

完全な構文を参照してください。 [Update Statistics][] MSDN にします。
