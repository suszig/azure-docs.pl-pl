...
詳細については、MSDN の「[DBCC SHOW_STATISTICS] [] を参照してください。
...
> [AZURE.NOTE] SQL Server を使用する場合は、SQL Server を作成し、単一列統計を更新すると、必要に応じてに依存する可能性があります。 SQL Data Warehouse はこの点で異なります。 SQL Data Warehouse では、データが分散しているため、すべての分散データにわたる統計が自動的に集計されるわけではありません。 集計された統計が生成されるのは、開発者が統計を作成および更新したときだけです。
...
この質問は時間で答えることができるものではありません。 最新の統計オブジェクトが非常に古い可能性もあります。 ときに行の数または特定の列の値の分布が実質的に変更が *し* 統計を更新する必要があります。
...
> [AZURE.NOTE] [昇順キー] の詳細については、SQL Server 2014 の基数の推定モデルのホワイト ペーパーを参照してください。

詳細については、MSDN の「[基数の推定] [を参照してください。
...
> [AZURE.NOTE] クエリ オプティマイザーが分散クエリ プランを選択したときにフィルター選択された統計の使用を検討するクエリは、統計オブジェクトの定義内に収まる必要があります。 前の例では、クエリの WHERE 句で col1 の値として 2000101 ～ 20001231 の値を指定する必要があります。
...
完全な参照は、MSDN の [統計の作成] のを参照してください。

### F. 複数列統計の作成

複数列統計を作成するには、これまでの例を使用するだけですが、複数の列を指定します。

> [AZURE.NOTE] クエリ結果内の行の数を推定に使用すると、ヒストグラムは、統計オブジェクト定義に示されている最初の列のできるだけです。
...
> [AZURE.NOTE] テーブルのすべての統計を更新するには、SQL Data Warehouse がサンプル統計ごとのテーブルをスキャンできます。 テーブルが大きく、多数の列と統計が含まれている場合は、ニーズに基づいて個々の統計を更新する方が効率的です。

実装については、 `UPDATE STATISTICS` プロシージャは、[一時テーブル] の記事を参照してください。 実装方法は前述の `CREATE STATISTICS` プロシージャと若干異なりますが、最終的な結果は同じです。

完全な構文については、MSDN の [統計の更新] のを参照してください。
