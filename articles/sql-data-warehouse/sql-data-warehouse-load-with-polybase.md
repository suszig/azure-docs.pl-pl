...
PolyBase を使用して、Azure blob ストレージに格納されたデータのクエリを実行して SQL データを読み込む 
次の手順でデータベースをウェアハウスします。
...
> [AZURE.IMPORTANT] PolyBase によってサポートされている Azure ストレージ アカウントの種類は次のとおりです。
> 
...
2. 使用 [CREATE MASTER KEY (TRANSACT-SQL)][] 、データベースのマスター_キーを作成します。 データベースに既にマスター キーがある場合は、マスター キーを余分に作成する必要はありません。 このキーは、次の手順で資格情報 "シークレット" の暗号化に使用されます。
...
1. データベースの資格情報が既に存在するかどうかを確認します。 これを行うには、サーバーの資格情報を表示するだけの sys.credentials ではなく、sys.database_credentials システム ビューを使用します。 
...
3. 使用 [CREATE CREDENTIAL (TRANSACT-SQL)][] スコープにアクセスする各 Azure ストレージ アカウントの資格情報でデータベースを作成します。 次の例では、IDENTITY は資格情報を表すわかりやすい名前になっています。 この名前は Azure ストレージへの認証には影響を及ぼしません。 SECRET は Azure ストレージ アカウント キーです。
...
1. データベース スコープ資格情報を削除する必要がある場合に使用して [DROP CREDENTIAL (TRANSACT-SQL)][]:
...
必要に応じて、PolyBase が外部データ ソースから受信したダーティ レコードをどのように処理するかを決定する拒否オプション (REJECT_TYPE、REJECT_VALUE、REJECT_SAMPLE_VALUE) も指定できます。
...
> [AZURE.NOTE] 外部テーブルを削除する場合は、使用 `DROP EXTERNAL TABLE`します。  **できません** を使用して `DROP TABLE`します。 

リファレンス トピック: [外部テーブルの削除 (TRANSACT-SQL)][]します。
...
> [AZURE.NOTE] 外部テーブルに対するクエリでは、エラーで失敗できる *「クエリが中止されました-外部ソースからの読み取り中に最大却下のしきい値に達しました」*します。 これは、外部のデータが含まれていることを示して *ダーティ* レコードです。 データ レコードは、列の実際のデータの種類/数値が外部テーブルの列定義と一致しない場合、またはデータが指定された外部ファイルの形式に従っていない場合に「ダーティ」であるとみなされます。 これを修正するには、外部テーブルと外部ファイルの形式の定義が正しいこと、および外部データがこれらの定義に従っていることを確認します。 外部データ レコードのサブセットがダーティである場合は、CREATE EXTERNAL TABLE DDL の中で拒否オプションを使用することで、クエリでこれらのレコードを拒否することを選択できます。
