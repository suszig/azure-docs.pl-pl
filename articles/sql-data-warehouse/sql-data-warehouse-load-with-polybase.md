...
> 
> + 標準的なローカル冗長ストレージ (LRS 標準)
> + 標準 Geo 冗長ストレージ (GRS 標準)
> + 標準的な読み取りアクセス Geo 冗長ストレージ (標準 RAGRS)
>
...
外部テーブルを削除する場合を使用して [ドロップ外部データ ソースの][]:
...
> [AZURE.NOTE] 外部テーブルを削除する場合は、使用 `外部テーブルのドロップ`します。  **できません** を使用して `DROP TABLE`します。 

リファレンス トピック: [外部テーブルの削除 (TRANSACT-SQL)][]します。

外部テーブルが、両方に表示されることに注目すべきも `sys.tables` しより具体的には [ `sys.external_tables` カタログ ビューです。
