...
![SQL データ ウェアハウス アーキテクチャ][1]

- **コントロールのノード:** SQL Data Warehouse の開発、読み込み、またはビジネス インテリジェンス ツールを使用すると、管理ノードに接続します。 SQL Data Warehouse の制御ノードは SQL Database です。接続時は、標準の SQL Database のような外観と操作性となります。 ただし、システム内で発生するデータの移動とコンピューティングはすべてバックグラウンドで調整されます。 コマンドが制御ノードに発行されると、コマンドは一連のクエリに分解され、サービスのコンピューティング ノードに渡されます。

- **計算ノード:** SQL データベースを使用して SQL Data Warehouse のコンピューティング ノードは電源管理] ノードのようにします。 コンピューティング ノードの仕事は、サービスのコンピューティング機能としての役割を果たすことです。 バックグラウンドでは、データが SQL Data Warehouse に読み込まれると常に、サービスの当該ノード間に分散されます。 制御ノードはコマンドを受信すると常に、各コンピューティング ノード用に分割し、コンピューティング ノードは対応するデータを処理します。 各コンピューティング ノードは計算を完了すると、担当部分の結果を制御ノードに渡します。制御ノードは結果を集計してから答えを返します。

- **記憶域:** SQL Data Warehouse のすべての記憶域は、標準的な Azure Storage の Blob です。 すなわち、コンピューティング ノードはデータ操作時に、BLOB との間で書き込みと読み取りを直接行います。 透過的に、ほぼ無制限に拡張可能な Azure Storage の機能を使用すると、ストレージを自動的に拡大縮小することができます。これはコンピューティングとは別に行うことができます。 また、Azure Storage では、スケーリングまたは一時停止の間もストレージを保持し、バックアップおよび復元のプロセスを合理化し、安全でよりフォールト トレラントなストレージを使用することができます。

- **データ移動のサービス:** 最後の部分を保持しているすべての同時 SQL Data Warehouse では、データ移動のサービスです。 データ移動サービスにより、制御ノードは、すべてのコンピューティング ノードと通信してデータを渡すことができます。 また、このデータ移動サービスにより、コンピューティング ノードは相互にデータを渡すことができ (他のコンピューティング ノードのデータにアクセス可能)、さらに、結合と集計を完了するのに必要なデータを取得することができます。
...
**スキャン/集計:** このワークロードのメトリックは、標準的なデータ ウェアハウジングの大量の行をスキャンし、複雑な集計を実行するクエリ。 これは IO と CPU を集中的に使用する操作です。

**読み込み:** このメトリックは、サービスにデータをインジェストする機能を測定します。 読み込みは、Azure Storage BLOB から代表的なデータセットを読み込む PolyBase を使用して実行されます。 このメトリックは、サービスのネットワークおよび CPU 要素にストレスをかけるように設計されています。

**CREATE TABLE AS SELECT (CTAS):** CTAS テーブルのコピーを作成する能力が計測されます。 この測定には、ストレージからのデータの読み取り、アプライアンスのノード間でのデータ分散、ストレージへのデータの再書き込みが必要です。 CPU とネットワークを集中的に使用する操作です。
...
> [AZURE.NOTE] アーキテクチャまたは SQL Data Warehouse のデータの量が低いパフォーマンスの向上を期待表示されない原因に注意してください。  正確なパフォーマンス テスト結果を取得するためには、1 TB 以上のデータ ボリュームで始めることをお勧めします。
...
**従来の SQL Server Tools:** SQL Server Analysis Services、Integration Services、Reporting services との完全な統合は SQL Data Warehouse で使用できます。

**クラウド ベースのツール:** SQL Data Warehouse は、多数の Azure では、新しいツールを併用あり、Azure のデータ ファクトリ、Stream Analytics、機械学習および Power BI と密接に統合します。

**サード パーティ製のツール:** 多数のサード パーティ製のツールのプロバイダーが SQL Data Warehouse では、そのツールの統合を認定します。 完全な一覧を参照してください。
...
[データ ウェアハウスのワークロード、についてわかったところで少し SQL Data Warehouse は、[プロビジョニング] してください。 [サンプル データ] の読み込みとします。

>[AZURE.NOTE] この記事をより良くしてみます。 応答の"no"、「この記事は役に立ちましたか?」を選択した場合 質問、何が足りないまたは記事の改善方法に関する簡単な提案を明記してください。 ご協力ありがとうございます。
