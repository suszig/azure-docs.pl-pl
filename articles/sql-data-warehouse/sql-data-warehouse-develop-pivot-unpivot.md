...
ピボット解除は、少し複雑です。 しかし、これを使用する非常に簡単 `ケース`します。 これには、最初に、ピボット解除する列の数を確認する必要があります。 前の例では 4 つの列をピボットしました。 それを引き続き使用しましょう。 ピボット解除を実行するには、データベースを一時的に4 倍拡大する必要があります。 これには、次の 2 つの段階のプロセスが必要です。
