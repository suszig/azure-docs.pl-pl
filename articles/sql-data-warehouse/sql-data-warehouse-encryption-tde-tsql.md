...
TDE は、データベース暗号化キーと呼ばれる対称キーを使用してデータベース全体のストレージを暗号化します。 SQL Database では、データベース暗号化キーは組み込まれているサーバー証明書によって保護されます。 組み込みのサーバー証明書は、SQL Database サーバーごとに一意です。 Microsoft は、少なくとも 90 日ごとにこれらの証明書を自動的にローテーションします。 TDE の一般的な説明を参照してください。 [Transparent Data Encryption (TDE)]します。

##暗号化の有効化

SQL Data Warehouse の TDE を有効にするには、次の手順を実行します。

1. 接続、 *マスター* 、管理者またはのメンバーであるログインを使用してデータベースをホストするサーバー上のデータベース、 **dbmanager** マスター データベースの役割
...
1. 接続、 *マスター* データベース管理者またはのメンバーであるログインを使用して、 **dbmanager** マスター データベースの役割
2. データベースの暗号化するには、次のステートメントを実行します。
...
1. 接続、 *マスター* またはインスタンスのデータベース管理者またはのメンバーであるログインを使用して、 **dbmanager** マスター データベースの役割
2. データベースの暗号化するには、次のステートメントを実行します。
