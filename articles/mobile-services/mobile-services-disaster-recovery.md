...
+ **Azure Mobile Services の SQL Database のデータをバックアップする**
    モバイル サービス アプリケーションのデータは Azure SQL データベースに格納されています。 指示されたとおりバックアップすることをお勧めします [SQL Database business continuity guidance]します。
+ **モバイル サービス スクリプトをバックアップする**
    など、ソース管理システムにモバイル サービス スクリプトを保存することをお勧め [Team Foundation Service] または [GitHub] し、モバイル サービス自体でのコピーのみに依存します。 Mobile Services を使用して、Azure クラシック ポータルを使用してスクリプトをダウンロードできる [source control feature], 、または [using the Azure CLI]します。 Azure クラシック ポータルで "プレビュー" というラベルが付いている機能には、細心の注意を払ってください。それらのスクリプトの復旧は保証されておらず、独自のソース管理の元のスクリプトから復旧することが必要になる場合があります。
...
+ モバイル サービスの状態が表示 **異常** で、 [Azure classic portal]します。
+  **異常** Azure クラシック ポータルで、モバイル サービスのどのタブの上部にバナーが表示され、管理操作は、エラー メッセージを生成します。
+  [Azure Service Dashboard] に可用性の問題を示します。
...
3. 説明に従って、サブスクリプションを使用する Azure コマンド ライン インターフェイス (Azure CLI) を構成 [Automate mobile services with the Azure CLI]します。
...
6. 復旧したサービスが Azure SQL Database と通信していることを確認します。 復旧コマンドでは、モバイル サービスが復旧されますが、元のデータベースへの接続が保持されます。 Azure のプライマリ リージョンの問題がデータベースにも影響を与えている場合は、復旧したサービスが正常に実行されないことがあります。 特定のリージョンのデータベースの状態は、Azure サービス ダッシュボードを使用して確認できます。 元のデータベースが動作していない場合は、次の手順でデータベースを復旧できます。
    + 説明に従って、モバイル サービスを復旧した Azure のリージョンに Azure SQL Database を復旧 [SQL Database business continuity guidance]します。
