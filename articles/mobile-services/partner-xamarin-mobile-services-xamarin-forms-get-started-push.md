 <properties
    pageTitle =「Mobile Services アプリケーション (Xamarin.Forms) - モバイル サービスにプッシュ通知を追加」
...
このチュートリアルを完了すると、いずれかのアプリケーションでユーザーがタスクを追加するたびに、モバイル サービスからプッシュ通知が送信されるようになります。 完全なサンプルを見つけることができます。 [完了 Xamarin.Forms Azure プッシュ通知のサンプル] です。
...
2. [ダウンロード、およびスタート サンプルの構成](#download-starter-sample)
4. [Xamarin.Forms.iOS アプリにプッシュ通知を追加します。](#iOS)
5. [Xamarin.Forms.Android アプリにプッシュ通知を追加します。](#Android)
6. [Xamarin.Forms.Windows アプリにプッシュ通知を追加します。](#Windows)
7. [すべてのアプリにプッシュ通知を送信する Azure テーブルの挿入スクリプトを更新します。](#all-apps)

## <a name="create-service"></a>新しいモバイル サービスを作成します。
...
## <a name="download-starter-sample"></a>ダウンロード、およびスタート サンプルの構成

既存のサンプルにプッシュ通知を追加します。

1. 次のサンプルをダウンロードします。 [Xamarin.Forms Azure プッシュ通知スタート サンプル] です。

2. クリックして、[Azure クラシック ポータル] で、 **Mobile Services**, 、し、[モバイル サービス] をクリックします。 **[ダッシュボード]** タブをクリックし、**[サイトの URL]** の値をメモしておきます。 **[キーの管理]** をクリックし、**[アプリケーション キー]** の値をメモしておきます。 これらの値は、アプリケーション コードからモバイル サービスにアクセスするときに必要になります。
...
## <a name="iOS"></a>Xamarin.Forms.iOS アプリにプッシュ通知を追加します。

Apple Push Notification サービス (APNS) を使用して、iOS アプリにプッシュ通知を追加します。 アクティブな Google アカウントと、[Google Cloud Messaging クライアント コンポーネント] 必要になります。
...
1. Azure クラシック ポータルで、**[データ]** タブ、**TodoItem** テーブルの順にクリックします。
...
Google Cloud Messaging (GCM) サービスを使用して、Android アプリにプッシュ通知を追加します。 アクティブな Google アカウントと、[Google Cloud Messaging クライアント コンポーネント] 必要になります。
...
[Azure クラシック ポータル]、をクリックして、 **データ** ] タブでをクリックし、 **TodoItem** テーブルです。
...
5. ToDoAzure.Droid プロジェクトというプロジェクトに新しいクラスを作成します。 `GcmService`します。
...
7. **GcmService.cs** プロジェクト ファイルで、次のクラスを追加します。
 ...
 上記のコードで置き換える必要があります _`< PROJECT_NUMBER >`_ は Google の開発者ポータルでアプリケーションをプロビジョニングしたときに Google によって割り当てられたプロジェクト番号です。
...
5. アプリケーションで **Hello push** というタイトルの新しいタスクを作成して、すぐに [開始] ボタンまたは [戻る] ボタンを押してアプリケーションを閉じます。
    ...
    >[AZURE.NOTE]アプリケーションが開いている場合は通知を受信しません。 アプリがアクティブな間は、トースト通知を受信するには、処理、 [ShellToastNotificationReceived](http://msdn.microsoft.com/library/windowsphone/develop/microsoft.phone.notification.httpnotificationchannel.shelltoastnotificationreceived(v=vs.105).aspx) イベントです。
