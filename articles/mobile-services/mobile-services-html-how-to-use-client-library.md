...
エディターで、開くまたは JavaScript ファイルを作成およびを定義する次のコードを追加、 `MobileServiceClient` 変数、およびアプリケーションの URL とアプリケーション キーでモバイル サービスを指定、 `MobileServiceClient` コンス トラクターに順にします。
...
テーブルのすべてのアクセスまたは SQL データベースのデータを変更するコード関数を呼び出します、 `MobileServiceTable` オブジェクトです。 呼び出して、テーブルへの参照を取得、 `getTable()` 関数のインスタンスで、 `MobileServiceClient`します。

    var todoItemTable = client.getTable('todoitem');

### <a name="filtering"></a>方法: 返されるデータをフィルター処理

次のコードを含めることによってデータをフィルター処理する方法を示しています、 `、` クエリ内の句。 すべての項目を返す `todoItemTable` 、complete フィールドに等しい `false`します。 `todoItemTable` 以前に作成したモバイル サービス テーブルへの参照します。 where 関数は、行のフィルタリング述語をテーブルに対するクエリに適用します。 この関数は、引数として JSON オブジェクトを受け取るかまたは行フィルターを定義する関数を受け取り、詳細に構成可能なクエリを返します。
...
呼び出して `場所` クエリ オブジェクトおよび、オブジェクトをパラメーターとして渡すことで、Mobile Services を行のみを返すように指示していますが `完了` 列が含まれる、 `false` 値。 また、次の要求 URI を確認し、クエリ文字列自体変更を加えていることを確認します。
...
渡されるオブジェクト、 `、` メソッドは、パラメーターの任意の数を含めることができ、それらすべて解釈されますクエリに AND 句として。 たとえば、次のコードがあるとします。
...
`、` 上記のステートメントと上記の SQL クエリ"medium"難易度は、"david"に割り当てられている未完了の項目が検索されます。

ただし、別の方法で同じクエリを作成することもできます。 A `.where` Query オブジェクトの呼び出しを追加します、 `AND` 式、 `場所` を行で 3 つの記述ができますので、句。
...
この 2 つの方法は等価であり、区別しないで使用できます。 すべての `、` これまで、この呼び出しでは一部のパラメーターを持つオブジェクトを受け取りし、が等しいかどうか、データベースからのデータに対して比較されます。 しかし、オブジェクトの代わりに関数を受け取る、クエリ メソッドの別のオーバーロードがあります。 この関数では、非同等性やその他の関係演算のための演算子を使用して、より複雑な式を書くことができます。 これらの関数では、キーワードで `この` サーバー オブジェクトにバインドします。
...
以降後のすべての引数のパラメーターを持つ関数を渡した場合、 `、` 句は順番に関数パラメーターにバインドします。 関数スコープ外からのすべてのオブジェクトは、パラメーターとして渡す必要があります (関数は、外部変数をキャプチャできません)。 次の 2 つの例では、引数"david"が、パラメーターにバインド `名` 最初の例では、引数"medium"もバインドして、パラメーターに `レベル`します。 1 つの関数がさらで構成する必要があります `返す` ステートメントでサポートされている式では、次のようにします。
...
組み合わせることは `場所` と `orderBy`, 、`を`, 、および `スキップ`します。 詳細については、次のセクションを参照してください。

### <a name="sorting"></a>方法: 返されるデータを並べ替える

次のコードを含めることによってデータを並べ替える方法を示しています、 `orderBy` または `orderByDescending` クエリ内の関数です。 項目を返します `todoItemTable` 昇順に並べ替えられた、 `テキスト` フィールドです。 既定では、サーバーは最初の 50 個の要素のみを返します。
> [AZURE.NOTE] すべての要素が返されるのを防ぐために、サーバー側で設定されたページ サイズが既定で使用されます。 これにより、大きなデータ セットの既定の要求がサービスに悪影響を与えないようにします。
呼び出して返される項目の数を増やすことが `かかる` 、次のセクションに記載されています。 `todoItemTable` 以前に作成したモバイル サービス テーブルへの参照します。
...
クライアントが応答でさらに多くのデータ数を明示的に指定していない場合、Mobile Services は既定で特定の要求に対して 50 行だけ返します。 次のコードを使用して、返されたデータにページングを実装する方法を示しています、 `かかる` と `スキップ` クエリで句。 次のクエリを実行すると、テーブルの最初の上位 3 つの項目が返されます。
...
これは、ハードコーディングされたページング値を渡すことの簡略化されたシナリオ、 `かかる` と `スキップ` 関数です。 実際のアプリケーションでは、ユーザーが前後のページに移動できるように、ページャー コントロールまたは同等の UI と共に上記と同様のクエリを使用することができます。

### <a name="selecting"></a>方法: 特定の列の選択

追加することで、結果に含める一連のプロパティを指定することができます、 `選択` クエリに句。 たとえば、次のコードを返します、 `id`, 、`完了`, 、および `テキスト` プロパティ内の各行から、 `todoItemTable`:。
...
`ルックアップ` 関数のみを受け取り、 `id` 値に設定して、その ID を持つデータベースからオブジェクトを返します 整数型または文字列でデータベースのテーブルが作成された `id` 列です。 文字列 `id` 列は、既定値です。
...
## <a name="odata-query"></a>OData クエリ操作を実行します。

Mobile Services では、REST クエリの作成と実行に OData クエリ URI 慣例を使用します。 組み込まれているクエリ関数ですべての OData クエリ (特にプロパティないのサブストリング検索などの複雑なフィルター操作) が作成できるわけではありません。 このような複雑なクエリには、有効な OData クエリ オプション文字列を渡すことができます、 `読み取り` 関数を次のようにします。
...
>[AZURE.NOTE]未処理の OData クエリ オプション文字列を指定すると、 `読み取り` 関数の場合、同じクエリでクエリ ビルダー メソッドを使用することもできません。 この場合、クエリ全体を OData クエリ文字列として作成する必要があります。 OData システム クエリ オプションの詳細については、[OData システム クエリ オプション リファレンス] を参照してください。
...
次のコードは、テーブルのデータを更新する方法を示しています。 クライアントは、モバイル サービスに PATCH 要求を送信して、データ行を更新することを要求します。 要求の本文には、更新する特定のフィールドが JSON オブジェクトとして含まれます。 テーブル内の既存の項目を更新 `todoItemTable`します。
...
## <a name="binding"></a>方法: ユーザー インターフェイスにデータを表示

このセクションでは、返されたデータ オブジェクトを UI 要素を使用して表示する方法について説明します。 内の項目をクエリに `todoItemTable` 、非常に単純なリストに表示して、次のコード例を実行することができます。 ここでは、どのような種類の選択、フィルター処理、または並べ替えも行われません。
...
カスタム API を呼び出すことによって、クライアントから呼び出す、 [invokeApi](https://github.com/Azure/azure-mobile-services/blob/master/sdk/Javascript/src/MobileServiceClient.js#L337) メソッドを **MobileServiceClient**します。 たとえば、次のコード行はモバイル サービスで **completeAll** API に POST 要求を送信します。
...
Facebook 以外の id プロバイダーを使用している場合に渡される値を変更、 `ログイン` 上、次のいずれかのメソッド: `microsoftaccount`, 、`facebook`, 、`twitter`, 、`google`, 、または `windowsazureactivedirectory`します。
...
この例では、それぞれのプロバイダー SDK で提供されるトークンに保存すること、 `トークン` 変数です。
...
この単純な例では、Live Connect から、[login] 関数の呼び出しにより、モバイル サービスに指定されたトークンを取得します。
...
## <a name="push-notifications"></a>方法: プッシュ通知に登録
...
>[AZURE.NOTE]現在、このプラグインは iOS および Android デバイスのみをサポートします。 Windows デバイスも含むソリューションでは、記事を参照してください。 [通知ハブの統合を使用する PhoneGap アプリへのプッシュ通知](http://blogs.msdn.com/b/azuremobile/archive/2014/06/17/push-notifications-to-phonegap-apps-using-notification-hubs-integration.aspx)します。
...
## <a name="promises"></a>方法: promise を使用

promise は、まだ計算されていない値の操作を完了するスケジュールを設定するためのメカニズムを提供します。 promise は、非同期 API を使用して相互作用を管理するための抽象化です。

`実行` promise に指定された関数が正常に完了した時点またはエラーが発生すると、すぐに実行します。 異なり、 `し` 関数内で処理されないエラーをスローすることが保証され、この関数が戻されているし、エラー状態の promise として、すべてのエラーをスロー ハンドラーの実行が完了した後、約束します。 詳細については、[完了] を参照してください。
...
`し` に期待されると同じ、 `実行` は異なり、promise、 `し、` promise `実行` 、関数の内部で処理されないエラーをスローすることが保証します。 エラー ハンドラーを指定しない場合 `し` と、操作がエラー、例外をスローしないは、エラー状態ではなく promise が返されます。 詳細については、[] を参照してください。
...
promise はいくつかの異なる方法で使用することができます。 呼び出すことにより、promise 操作を連結できます `し` または `実行` 前から返される promise で `し` 関数です。 使用 `し、` 操作の中間ステージ (たとえば `.then().then()`)、および `実行` 操作の最終ステージ (たとえば `.then().then().done()`)。 複数個連結することができます `し` であるため、機能 `し` promise が返されます。 1 つ以上を連結することはできません `実行` undefined を返すため、このメソッドです。 [Then と done の違いの詳細について説明します。]
...
使用してカスタムの要求ヘッダーを送信する、 `withFilter` 関数、フィルター内で送信される要求の任意のプロパティを読み書きします。 このようなカスタム HTTP ヘッダーを追加するケースとしては、サーバー側スクリプトで必要とされる場合や、カスタム HTTP ヘッダーによってサーバー側スクリプトを拡張できる場合があります。
...
## <a name="hostnames"></a>方法: クロス オリジン リソース共有を使用します。

モバイル サービスとの対話やモバイル サービスへの要求の送信を許可する Web サイトを制御するには、ホストする Web サイトのホスト名をクロス オリジン リソース共有 (CORS) ホワイトリストに追加してください。 JavaScript バックエンド モバイル サービスの [構成] タブでホワイト リストを構成することができます、 [Azure クラシック ポータル](https://manage.windowsazure.com)します。 必要に応じて、ワイルドカードを使用できます。 既定では、新しいモバイル サービスはからのアクセスのみを許可するようにブラウザーを指示する `localhost`, 、クロス オリジン リソース共有 (CORS) により、モバイル サービスと対話する、外部ホスト名のブラウザーで実行される JavaScript コードとします。 この構成は、WinJS アプリケーションには必要ありません。
