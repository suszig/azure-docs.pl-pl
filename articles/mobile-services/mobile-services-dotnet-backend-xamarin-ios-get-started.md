...
5. [ **のダウンロードし、クラウドにサービスを発行**, [ **iOS** ] をクリック **ダウンロード**します。
...
2. キーを押して、 **実行** クライアント プロジェクトをビルドし、iPhone エミュレーターでアプリケーションを開始] ボタンをクリックします。
...
>[AZURE.NOTE]クエリを実行して QSTodoService.cs c# ファイルにデータを挿入して、モバイル サービスにアクセスするコードを確認できます。
