...
このトピックでは、Azure Mobile Services のオフライン機能を使用しているときに、データを同期し、競合を処理する方法について説明します。 このチュートリアルは、 [Get Started with Offline Data] チュートリアルです。
...
1.  **QSTodoListViewController.m**, 、編集 **viewDidLoad**します。 呼び出しを置き換える **defaultService** への呼び出しに **defaultServiceWithDelegate** 代わりにします。

        self.todoService = [QSTodoService defaultServiceWithDelegate:self];

2.  **QSTodoListViewController.h**, 、追加 **& lt;MSSyncContextDelegate & gt;** インターフェイスの宣言を実装する、 **MSSyncContextDelegate** プロトコルです。

        @interface QSTodoListViewController : UITableViewController<MSSyncContextDelegate, NSFetchedResultsControllerDelegate>

3. 上部に次の import ステートメントを追加 **QSTodoListViewController.m**:

        #import "QSUIAlertViewWithBlock.h"

4. 最後に、次の 2 つの操作を追加 **QSTodoListViewController.m** このヘルパー クラスを使用し、3 つの方法のいずれかで競合を調整するように求めます。
