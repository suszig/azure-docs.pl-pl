...
- **Sygnał** — sygnały są emitowane przez zasób docelowy i mogą być różne. Ta wersja zapoznawcza obsługuje typy sygnału metryki i dziennika.
...
> Wypalane alertów przedstawianych w alertach (wersja zapoznawcza) są ograniczone do obsługiwanych metryki i dziennika alerty; Azure Monitor pokazuje liczbę alertów fire, włącznie z zawartymi w starszych alerty Azure
 ...
 ![alerty — wersja zapoznawcza — omówienie](./media/monitoring-overview-unified/alerts-preview-overview.png) 
...
| Dzienniki  | Dzienniki aktywności | Nieobsługiwane w środowisku alerty (wersja zapoznawcza). |
| Dzienniki  | Application Insights | Nieobsługiwane w środowisku alerty (wersja zapoznawcza). |
