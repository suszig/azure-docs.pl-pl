...
4. VMM スクリプトを追加する場合は、選択 **VMM スクリプトへのフェイル オーバー**, 、およびで **スクリプトのパス** 共有への相対パスを入力します。 そのため、共有はある \\<VMMServerName>\MSSCVMMLibrary\RPScripts、パスを指定: \RPScripts\RPScript.PS1 です。
