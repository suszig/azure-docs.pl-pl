 ...
 **モビリティ サービス**| <p>各仮想マシンまたは物理サーバーを保護するインストール</p><p>を手動でインストールまたはプッシュされ、サーバーの保護が有効にすると、プロセス サーバーによって自動的にインストールできます。| モビリティ サービスは、初期レプリケーション (再同期) の一環としてプロセス サーバーにデータを送信します。サーバーが保護状態に達すると (再同期が完了すると)、モビリティ サービスは、ディスクへの書き込みのインメモリ キャプチャを実行し、それをプロセス サーバーに送信します。Windows Server のアプリケーションの整合性は、VSS フレームワークを使って実現されます。
 ...
 **レプリケーション メカニズム**| <p>**経由でインターネット**— 保護された内部設置型サーバーとパブリック インターネット接続を介してセキュリティで保護された SSL/TLS 通信チャネルを使用して Azure からデータを通信およびレプリケートします。これは、既定のオプションです。</p><p>**Vpn/expressroute**-内部設置型サーバーと Azure 間のデータ、VPN 接続を介して通信およびレプリケートします。サイト間 VPN を設定する必要がありますか [ExpressRoute](../expressroute-introduction.md) 内部設置型サイトと Azure ネットワーク間の接続</p>。<p>Site Recovery のデプロイの間にレプリケートする方法を選択します。構成が済んだ後では、既に保護されているサーバーに対する保護に影響を与えずにメカニズムを変更することはできません。| <p>どちらのオプションでは、保護されたマシンへの着信ネットワーク ポートを開く必要があります。すべてのネットワーク通信が内部設置型サイトから開始されます。</p>
...
- **ソースごとの 1 日の最大変更率** — ソースごとの推奨される変更率を検討するときは、3 つの要因を考慮する必要があります。 ターゲット ベースの考慮の場合、ソースでの操作ごとにターゲット ディスクでは 2 IOPS が必要です。 これは、ターゲット ディスクでは古いデータが読み取られて、新しいデータが書き込まれるためです。
    ...
    - **ターゲット ディスクでサポートされる最大のスループット** — ソース ディスクあたりの最大変化量は、144 GB/日を超えることはできません (8 K の書き込みサイズの場合)。 さまざまな書き込みサイズでのターゲットのスループットと IOPS については、マスター ターゲット セクションの表を参照してください。 各ソース IOPS によってターゲット ディスクでは 2 IOPS が生成されるので、この値を 2 で割る必要があります。 参照 [のスケーラビリティおよびパフォーマンスのターゲット Premium Storage を使用する場合](../storage/storage-scalability-targets.md#scalability-targets-for-premium-storage-accounts) Premium Storage アカウントのターゲットを構成するときにします。
    - **ストレージ アカウントでサポートされる最大スループット** — 1 つのソースで複数のストレージ アカウントに対応することはできません。 ストレージ アカウントが 1 秒間に最大で 20,000 の要求を受け取り、各ソース IOPS によってマスター ターゲット サーバーでは 2 IOPS が生成されることから、ソースの IOPS 数を 10,000 に維持することをお勧めします。 参照 [のスケーラビリティおよびパフォーマンスのターゲット Premium Storage を使用する場合](../storage/storage-scalability-targets.md#scalability-targets-for-premium-storage-accounts) Premium Storage アカウントのソースを構成するときにします。
...
- 各マスター ターゲット サーバーは、保持ボリュームを 1 つだけ持ちます。 保持ボリュームは、マスター ターゲット サーバーに接続されたディスク間で共有されます。 次に例を示します。
    ...
    - 保持ボリュームでは、これは 120 * 5 = 600 IOPS になり、ボトルネックになる可能性があります。 このシナリオでの適切な戦略は、保持ボリュームにディスクを追加し、RAID ストライプ構成として全体で共有することです。 このようにすると、IOPS が複数のドライブに分散されるため、パフォーマンスが向上します。 保持ボリュームに追加するドライブの数は次のようになります。
        -ソース環境の IOPS の合計 500/
        の (非圧縮) のソース環境から 1 日あたり合計の変更/287 GB。287 GB は、ターゲット ディスクがサポートする 1 日あたりの最大スループットです。 このケースでは 8 K が想定される書き込みサイズなので、このメトリックは 8 K 単位の書き込みサイズによって異なります。 たとえば、書き込みサイズが 4 K の場合、スループットは 287/2 になります。 また、書き込みサイズが 16 K の場合、スループットは 287*2 になります。
 ...
 **Azure Storage**| <p>レプリケートされたデータを格納する Azure ストレージ アカウントが必要があります</p><p>、いずれかのアカウントにする必要があります、 [標準の Geo 冗長ストレージ アカウント](../storage/storage-redundancy.md#geo-redundant-storage) または [Premium Storage アカウント](../storage/storage-premium-storage-preview-portal.md).</p><p>され、同じサブスクリプションに関連付けられている Azure Site Recovery サービスと同じリージョンに必要です</p>。<p>の詳細を読み取る [Microsoft Azure Storage の概要](../storage/storage-introduction.md)</p>
 ...
 **プロセス サーバー**| <p>物理マシンまたは最新の更新プログラムで Windows Server 2012 R2 を実行する仮想マシンにプロセス サーバーを展開することができます。C ドライブにインストール/です。</p><p>保護するマシンと同じネットワークおよびサブネットに、サーバーに配置することをお勧めします</p>。<p>インストール VMware vSphere CLI 5.5.0 をプロセス サーバーです。VMware vSphere CLI コンポーネントが、vCenter サーバーによって管理されるバーチャル マシンまたは ESXi ホストで実行されている仮想マシンを検出するために、プロセス サーバーに必要です。</p><p>インストール パスは英語文字のみでなければなりません</p>。<p>ReFS ファイル システムがサポートされていません。</p>
 **VMware**| <p>VMware vSphere ハイパーバイザーを管理する VMware vCenter サーバー。これが実行されて vCenter バージョン 5.1 または 5.5 最新の更新プログラム。</p><p>を保護する VMware 仮想マシンを含む 1 つまたは複数の vSphere ハイパーバイザー。ハイパーバイザーが最新の更新プログラム Esx/esxi バージョン 5.1 または 5.5 を実行する必要があります。</p><p>VMware バーチャル マシンは VMware ツールがインストールされ、実行をいる必要があります。</p>
 **Windows マシン**| <p>保護された物理サーバーまたは VMware 仮想マシンが Windows を実行する多くの要件があります。</p><p>には、64 ビット オペレーティング システムがサポートされている: **Windows Server 2012 R2**, 、**Windows Server 2012**, 、または **での Windows Server 2008 R2 SP1 以降**.</p><p>ホスト名、マウント ポイント、デバイス名、Windows のシステム パス (例: C:\Windows) のみ英語にする必要があります</p>。<p>オペレーティング システムは C:\ ドライブにインストールする必要があります</p>。<p>ベーシック ディスクのみがサポートされています。ダイナミック ディスクはサポートされません。</p><p><Firewall rules on protected machines should allow them to reach the configuration and master target servers in Azure.p><p>管理者アカウントを指定する必要があります (Windows マシン上のローカルの管理者である必要があります) Windows サーバーにモビリティ サービスのプッシュ インストールします。提供するアカウントがドメイン アカウントでない場合、ローカル マシンでのリモート ユーザーのアクセス制御を無効にする必要があります。そのためには、LocalAccountTokenFilterPolicy DWORD レジストリ エントリ (値は 1) を HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System に追加します。CLI の開いている cmd または powershell からレジストリ エントリを追加して * *`REG ADD \software\microsoft\windows\currentversion\policies\system/v LocalAccountTokenFilterPolicy/t REG_DWORD/d 1`* *。[詳細](https://msdn.microsoft.com/library/aa826699.aspx) アクセス制御の詳細</p>。<p>フェールオーバー後、リモート デスクトップを使用して Azure で Windows 仮想マシンに接続する場合にください、内部設置型マシンのリモート デスクトップが有効になっていることを確認します。VPN 経由で接続しない場合のファイアウォール ルールは、インターネット経由で、リモート デスクトップ接続を許可する必要があります。</p>
 **Linux マシン**| <p> サポートされる 64 ビット オペレーティング システム: **Centos 6.4、6.5、6.6**です。 **Oracle Enterprise Linux 6.4、6.5、Red Hat 互換カーネルまたは Unbreakable Enterprise Kernel リリース 3 (UEK3) を実行している**, 、**SUSE Linux Enterprise Server 11 SP3**.</p><p>保護されたマシン上でファイアウォール規則は Azure の構成とマスター ターゲット サーバーに到達することを許可する必要があります</p>。<p>/etc/保護されたマシン上の hosts ファイルには、ローカル ホスト名をすべての Nic に関連付けられている IP アドレスにマップするエントリが含まれます </p><p>クライアントを使用して、Secure Shell (ssh) フェールオーバー後に、Linux を実行している Azure の仮想マシンに接続するには、保護されたマシンで、Secure Shell サービスが、システムのブートで自動的に起動に設定されていることと、ファイアウォール ルールを使用することを確認する ssh それへの接続を</p><p>ホスト名、マウント ポイント、デバイス名、および Linux システム パスおよびファイル名 (例:/etc/;/usr) は英語でのみ指定できます</p>。<p>次記憶域と内部設置型マシンの保護を有効にすることができます:-<br>ファイル システム: EXT3、ETX4、ReiserFS、XFS<br>マルチパス ソフトウェア デバイス Mapper (multipath)<br>ボリューム manager: LVM2<br>の物理サーバー HP CCISS コント ローラー ストレージではサポートされていません。</p>
 **サードパーティ**| このシナリオでは、正しく機能するためにサード パーティのソフトウェアに依存している、一部のデプロイ コンポーネントがあります。完全な一覧を参照してください [サード パーティ製ソフトウェアの通知と情報](#third-party)
...
5. **[リージョン]** ボックスで、コンテナーのリージョンを選択します。 サポートされているリージョンは、で利用可能な地域を参照してくださいチェック [Azure Site Recovery の料金詳細](pricing/details/site-recovery/)
...
4. **[新しい構成サーバーの詳細]** で以下を指定します。
    ...
    [AZURE です。[メモ] この設定は、デプロイ時に作成し、後から変更することはできません。
...
構成サーバーは、予約済み IP アドレスで自動的に作成される Azure クラウド サービスにデプロイされます。 予約されたアドレスは、クラウド サービスで仮想マシン (構成サーバーを含む) の再起動の間に構成サーバーのクラウド サービスの IP アドレスが変わらないために必要です。 構成サーバーを使用停止するときは予約されたパブリック IP アドレスを手動で予約解除する必要があります。そうしないと、予約されたまま残ります。 サブスクリプションごとに 20 個の予約されたパブリック IP アドレスの既定の制限があります。 [詳細](../virtual-network/virtual-networks-reserved-private-ip.md) アドレスの予約済み IP についてです。
...
7.  Windows を実行している場合は以下のようにします。
    ...
    3. 必要な推定保持ドライブが 1 TB を超える場合は、仮想ディスクとを使用して保持ボリューム (r:) を構成できます [記憶域スペース](http://blogs.technet.com/b/askpfeplat/archive/2013/10/21/storage-spaces-how-to-configure-storage-tiers-with-windows-server-2012-r2.aspx)
...
8. Linux を実行している場合は以下のようにします。
    ...
    2. **[ターゲット (Azure) リソースの準備]** で、**[追加ソフトウェアのダウンロードとインストール (Linux マスター ターゲット サーバーのみ)]** をクリックし、Linux マスター ターゲット サーバー パッケージをダウンロードします。ダウンロードした tar ファイルを、sftp クライアントを使用して仮想マシンにコピーします。デプロイ済みの linux マスター ターゲット サーバーにログインして、使用する別の方法として *wget http://go.microsoft.com/fwlink/?LinkID=529757&clcid=0x409* をダウンロードする、ファイルです。
    ...
    ![Linux のマスター ターゲット サーバー](./media/site-recovery-vmware-to-azure/ASRVMWare_TSLinuxTar.png)
    4. tar ファイルの内容を抽出したディレクトリで操作していることを確認します。
    5. 構成サーバーのパスフレーズをコマンドを使用してローカル ファイルにコピーする * * エコー *`< パスフレーズ >`* > passphrase.txt**
    6. コマンドを実行"* * sudo。 ./install-t 両方-a host-r MasterTarget-d/usr/local/ASR-i *`< 構成サーバー内部 IP アドレス >`*-p 443-s y-c https-p passphrase.txt**"です。

    ![ターゲット サーバーの登録](./media/site-recovery-vmware-to-azure/Linux-MT-install.png)

9. 10 ～ 15 分間待機してから、**[サーバー]**、**[構成サーバー]** ページの順にクリックして、**[サーバーの詳細]** タブにマスター ターゲット サーバーが登録済みとして表示されていることを確認します。 Linux を実行しており、これ以上登録していない場合、ホストの構成ツールを再度実行/usr/local/ASR/Vx/bin/hostconfigcli からです。 root として chmod を実行し、アクセス許可を設定する必要があります。
...
4. 実行、 **Microsoft-ASR_CX_TP_8.4.0.0_Windows*** インストール手順については、処理を実行します。 これでデプロイに必要なサード パーティのコンポーネントがインストールされます。
...
7. **[環境の詳細]** ページで次のようにします。
    ...
    - 物理サーバーを保護するし、VMware vCLI をプロセス サーバーにインストールされている必要があるためです。 **[いいえ]** をクリックして続行します。
...
8. VMware vCLI をインストールするときは、次のことに注意してください。
    ...
    - VSphere CLI 5.5.0からのダウンロード [ここ](https://my.vmware.com/web/vmware/details?downloadGroup=VCLI550&productId=352)。
...
Site Recovery で、更新プログラムを入手できます ** ダッシュ ボード **します。 Linux のインストールの場合は、gzip 圧縮されたインストーラーからファイルを抽出し、コマンド “sudo ./install” を実行して更新プログラムをインストールします。

最新の更新プログラムをダウンロード、 **フェールバック tool(vContinuum)** から [ここ](http://go.microsoft.com/fwlink/?LinkID=533813)
...
1. パスフレーズを CX からサーバーのファイル "C:\connection.passphrase" にコピーして、このコマンドを実行します。 この例では、CX は 104.40.75.37、HTTPS ポートは 62519です。
    ...
    `C:\Microsoft-ASR_UA_8.2.0.0_Windows_PREVIEW_20Mar2015_Release.exe"ip 104.40.75.37-ポート 62519 - モード UA/LOG="C:\stdout.txt"/DIR ="C:\Program Files (x86) \Microsoft Azure Site Recovery"/VERYSILENT/SUPPRESSMSGBOXES/norestart - usesysvolumes/CommunicationMode https/PassphrasePath"C:\connection.passphrase"`

**Linux サーバーでモビリティ サービスを手動でインストールする**:

1. 前記の表に基づいて、適切な tar アーカイブをプロセス サーバーからソース マシンにコピーします。
2. シェル プログラムを開き、実行することによってローカル パスに zip 形式の tar アーカイブを抽出 `tar - xvzf-ASR_UA_8.2.0.0 *`
3. 入力して、tar アーカイブの内容を抽出したローカル ディレクトリに passphrase.txt ファイルを作成 *`エコー < パスフレーズ >> passphrase.txt`* シェルからです。
4. 入力して、モビリティ サービスをインストール *`sudo。 ./install-t 両方-a host-r エージェント-d/usr/local/ASR-i < IP アドレス >-p < ポート >-s y-c https-p passphrase.txt`* します。
5. IP アドレスとポートを指定します。

    - インターネット経由で構成サーバーに接続している場合は、構成サーバーの仮想パブリック IP アドレスとパブリック HTTPS エンドポイントを指定 `< IP アドレス >` と `< ポート >`します。
...
2. **[仮想マシンの選択]** で、保護グループを選択してから、復旧計画に追加するグループ内のマシンを選択します。 [詳細](site-recovery-create-recovery-plans.md) 復旧計画についてです。
...
[詳細](site-recovery-failback-azure-to-vmware.md) フェールオーバーされたマシンが Azure で実行する方法の詳細を内部設置型環境に戻す。
