...
レプリケーションとネットワークのデプロイを完全にテストするには、複製された仮想マシンが予想どおり機能するようにインフラストラクチャを設定する必要があります。 次のように DNS とドメイン コント ローラーとして仮想マシンを設定して、テスト フェールオーバーを実行して、テスト用ネットワークで作成する [Site Recovery を使用して Azure にレプリケートする方法の 1 つです。  [詳細について、慎重](site-recovery-active-directory.md#considerations-for-test-failover) Active Directory のフェールオーバーに関する考慮事項をテストします。
