...
>[AZURE.NOTE] 背面の VMware 仮想マシンと Windows または Linux の物理サーバーを Azure から内部設置型のプライマリ サイトでの VMware バーチャル マシンのみ失敗します。  物理マシンをフェールバックしている場合、Azure へのフェールオーバーによって物理サーバーが Azure VM に変換され、VMware へのフェールバックによって物理サーバーが VMware VM に変換されます。 
...
4.  ウィザードの最初のページで CX サーバーの IP アドレスとポートを指定します。 選択 **HTTPS を使用して**します。
...
6.  構成サーバーの HTTPS パブリック ポートを検索、 **エンドポイント** 構成サーバーで、Azure VM のタブをクリックします。

    ![](./media/site-recovery-failback-azure-to-vmware/image5.png)

7.  **CS パスフレーズ詳細** ] ページでは、構成サーバーを登録したときにメモしたパスフレーズを指定します。 覚えていない場合は、チェックイン **C:\\Program Files (x86) \\InMage Systems\\private\\connection.passphrase** 構成サーバーの VM にします。

    ![](./media/site-recovery-failback-azure-to-vmware/image6.png)

8.   **変換先の場所の選択** ] ページでは、vContinuum サーバーをインストールし、をクリックする場所を指定 **インストール**します。
...
1.   **構成サーバー** Azure でのページで、選択すると、新しいプロセス サーバーを追加します。
...
>[AZURE.NOTE] フェールバック中に登録されているサーバーは Site Recovery で VM のプロパティの下に表示することはできません。 のみで表示されます、 **サーバー** が登録されている構成サーバーのタブをクリックします。 プロセス サーバーがタブに表示されるまで約 10 分から 15 分かかることがあります。
...
1. 仮想マシンをシャットダウンします。
2. 左側のパネルで VM のエントリを右クリックして > **設定の編集**します。
3. クリックして、 **オプション** ] タブをクリックします。 選択 **詳細設定 > [General item** > **構成パラメーター**します。  **構成パラメーター** オプションは、マシンをシャット ダウン時にのみ使用できます。

    ![](./media/site-recovery-failback-azure-to-vmware/image14.png)

4. 並んでいるかどうかをチェック **ディスクです。EnableUUID** が存在します。 設定されている場合は **False** に設定 * * True **(not case sensitive) します。 場合が存在し、true の場合、クリックに設定されている **キャンセル** およびが起動した後に、ゲスト オペレーティング システム内の SCSI コマンドをテストします。 場合はクリックが存在しない **行の追加。**
5. ディスクを追加します。EnableUUID、 **名前** 列です。 その値を TRUE に設定します。 前述の値を追加する際には、二重引用符で囲まないでください。
...
2. このコマンドを実行して、バイナリを解凍: **- この tar \ < ファイル名] \ >**
...
8. プライマリ ロールを選択するように求められたら **2 (マスター ターゲット)**します。 その他の対話型インストール オプションについては、既定値のままにします。
...
10. フィールドに **IP** 構成サーバーの内部 IP アドレスを入力 (から検索できます、 **ダッシュ ボード** 構成サーバー VM のタブをクリック) し、ENTER キーを押します。  **ポート** 入力 **22** ENTER キーを押します。 
11.  ままにして **HTTPS を使用する** として **はい**, 、ENTER キーを押します。
...
>[AZURE.NOTE] マスター ターゲット サーバーを登録すると、エンドポイントが正しく構成されていないと、その仮想マシンが Azure から削除されていることも、構成エラーが発生した場合は、これはマスター ターゲットが Azure にデプロイされるときに、Azure dndpoints によってマスター ターゲットの構成が検出すると、それだけで、オンプレミスでマスター ターゲット サーバーをオンプレミスの場合は true です。 これはフェールバックには影響しないため、このエラーは無視することができます。 
...
3. クリックして **新しい保護グループの** オペレーティング システムの種類を選択し、 

4.  開いて [新しいウィンドウで、 **OS の種類** > **の詳細の取得** フェールバックする Vm 用です。  **プライマリ サーバーの詳細**, を特定し、保護する仮想マシンを選択します。 VM は、フェールオーバー前の vCenter のホスト名の下に表示されます。
...
10. 仮想マシンが、オンプレミス サイトにフェールオーバーした後に取得するプロパティを構成します。 プロパティを次のテーブルにまとめて示します。
    ...
    **プロパティ** | **詳細**
    ---|---
    ネットワーク構成| 、検出された各ネットワーク アダプターを選択し、クリックして **変更** 、仮想マシンのフェールバック IP アドレスを構成します。 
...
3. 指定されたとおりにオンプレミス仮想マシンを削除しておらず、フェールバック先のデータストアに依然として古い VMDK が残っている場合、フェールバック VM が新しい場所に作成されることを確認する必要があります。 この選択を行うには、 **詳細** 設定でリストア先になる新しい代替フォルダーを指定して **Folder Name Settings**します。 他のオプションは既定の設定のままにします。 フォルダー名設定をすべてのサーバーに適用します。
...
5. 完了するまで待ちます。 すべての VM で完了したら、保護計画の名前を指定できます。 クリックし、 **保護** を開始します。
...
7. ステップの後に **保護計画をアクティブ化** Site Recovery ポータルで VM の保護を監視することが完了するとします。
...
1.  VContinuum を起動し、選択 **計画管理** > **を回復します。**VM のフェールオーバーに使用されているすべての計画の一覧を確認できます。 同じ計画を使用して復旧することができます。

    ![](./media/site-recovery-failback-azure-to-vmware/image37.png)

2. 保護計画と、その中で復旧するすべての VM を選択します。 各 VM を選択すると、ターゲット ESX サーバーおよびソース VM ディスクなどの詳細を確認できます。 クリックして **次** を回復ウィザードを開始し、回復する仮想マシンを選択します。

    ![](./media/site-recovery-failback-azure-to-vmware/image38.png)

3. 複数のオプションに基づいた回復できますが、使用することをお勧め **最新タグ** を選択し、 **、すべての Vm の適用** を仮想マシンからの最新のデータが使用されることを確認します。
4. 実行、 **準備チェックします。**これにより、VM の復旧を有効にするための適切なパラメーターが構成されているかどうかが確認されます。 クリックして **次** すべてのチェックが成功した場合。 成功しない場合は、ログを確認し、エラーを解決します。

    ![](./media/site-recovery-failback-azure-to-vmware/image39.png)

8.   **VM 構成** 回復の設定が正しく設定されていることを確認します。 VM の設定は、必要に応じて変更できます。

    ![](./media/site-recovery-failback-azure-to-vmware/image40.png)

9. 最後に、復旧対象の仮想マシンの一覧を確認し、復旧順序を指定します。 VM はコンピューターのホスト名で表示されることに注意してください。 その方法は
コンピューターのホスト名を仮想マシンにマップできない場合があります。
名前をマップするには、仮想マシンに移動 **ダッシュ ボード** Azure で VM ホスト名を確認します。

    ![](./media/site-recovery-failback-azure-to-vmware/image41.png)

10. プランの名前を指定し、選択 **[Recover later**します。 初期の保護が完了しない可能性があるため、復旧は後で行うことをお勧めします。 
11. をクリックして **回復** 、計画を保存、今すぐおよびいない後で回復するよう選択した場合に、回復をトリガーしたりします。 計画が保存されているかどうかは復旧の状態で確認できます。
...
1.  保存された計画を開始します。 VContinuum で、選択 **モニター** 計画します。 これにより、実行されているすべての計画が一覧表示されます。

    ![](./media/site-recovery-failback-azure-to-vmware/image45.png)

2.  プランを選択 **回復** ] をクリック **開始**します。 復旧を監視することができます。 VM が起動したら、vCenter で VM に接続することができます。
...
[ご覧](site-recovery-vmware-to-azure.md) VMware 仮想マシンを Azure にレプリケート
