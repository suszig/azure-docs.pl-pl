...
2. **Docelowa grupa zasobów:** jest grupa zasobów, aby wszystkie zreplikowanej maszyny wirtualne należą. Domyślnie usługi Azure Site Recovery tworzy nową grupę zasobów w regionie docelowych z nazwą składającą się z sufiksem "asr". W przypadku grupy zasobów utworzonej za pomocą usługi Azure Site Recovery już istnieje, zostanie ono użyte ponownie. Można również dostosować go, jak pokazano w poniższej sekcji.    
3. **Sieć wirtualna docelowa:** Domyślnie usługi Azure Site Recovery tworzy nową sieć wirtualną w regionie docelowych z nazwą składającą się z sufiksem "asr". To zostaną zmapowane do źródłowej sieci i będzie służyć do przyszłych ochrony.

    > [!NOTE]
    > [Sprawdź szczegóły sieci](site-recovery-network-mapping-azure-to-azure.md) Aby dowiedzieć się więcej o mapowania sieci.

4. **Docelowa kont magazynu:** Domyślnie usługi Azure Site Recovery tworzy nowe konto magazynu docelowego mimicking konfigurację magazynu maszyny Wirtualnej źródłowego. W przypadku konta magazynu utworzone za pomocą usługi Azure Site Recovery już istnieje, zostanie ono użyte ponownie.
...
6. **Zestaw dostępności:** Domyślnie usługi Azure Site Recovery tworzy nowy zestawem dostępności w region docelowy z nazwą składającą się z sufiksem "asr". W przypadku, gdy zestaw dostępności utworzone przez usługę Azure Site Recovery już istnieje, zostanie on użyty ponownie.
...
Gdy maszyny wirtualne są chronione, można sprawdzić stan kondycji maszyn wirtualnych w obszarze **zreplikowane elementy**
