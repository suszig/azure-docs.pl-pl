...
---|---|---
 **AlwaysOn 可用性グループ**| <p>SQL Server の複数のスタンドアロン インスタンスでは、複数のノードを持つフェールオーバー クラスターで実行します。</p><p>データベースは、コピーすること (ミラー化された) SQL Server インスタンスで共有記憶域は必要ありませんようにフェールオーバー グループにグループ化することができます。</p><p>プライマリ サイトと 1 つまたは複数のセカンダリ サイトの間での障害復旧を提供します。2 つのノードで設定できます共有同期レプリケーションと自動フェールオーバーによって可用性グループで構成されている SQL Server データベースを持つクラスター何も行われません。</p>| SQL Server 2014/2012 Enterprise エディション
 **フェールオーバー クラスタリング (AlwaysOn FCI)**| <p>SQL Server では、Windows フェールオーバー クラスタ リング、内部設置型 SQL Server のワークロードの高可用性を利用しています。</p><p>共有ディスクを SQL Server のインスタンスを実行しているノードがフェールオーバー クラスターで構成されます。インスタンスがダウンした場合、クラスターは別にフェールオーバーします。</p><p>クラスターは、エラーまたは共有記憶域の障害から保護しません。共有ディスクは、iSCSI、ファイバー チャネルでを実装するまたは共有 Vhdx します。</p>| SQL Server Enterprise エディション</p> <p>(2 つのノードのみに制限されます)、SQL Server Standard edition
...
- オンプレミスの SQL Server では、SQL ユーザー グループにユーザー アカウントを追加する際に、少なくとも次のアクセス権を付与する必要があります。
    ...
    - ALTER DATABASE - [1 を参照して](https://msdn.microsoft.com/library/ff877956.aspx#Security)
...
3.  Azure の SQL Server レプリカ仮想マシンでスクリプトを呼び出す Azure Automation Runbook を作成します。 これを行うには、次のサンプル スクリプトを使用します。 [詳細](site-recovery-runbook-automation.md) に関する復旧計画で automation の runbook を使用します。
