...
> [AZURE.NOTE] Azure PowerShell は、Windows クライアントだけで利用できます。 Linux、Unix、Mac OS X クライアントを使用している場合は、次を参照してください。 [Azure CLI を使用して Linux ベースの HDInsight クラスターを作成する](hdinsight-hadoop-create-linux-cluster-azure-cli.md) については、Azure CLI を使用してクラスターを作成します。
