...
2. Użyj następującego polecenia, aby uruchomić przykładową topologię:
   ...
   > [!NOTE]
   > Część `*` nazwy pliku umożliwia dopasowanie numeru wersji, która zmienia się po zaktualizowaniu usługi HDInsight.
   > 
   > 
   ...
   > [!NOTE]
   > Podczas przesyłania topologii do klastra przed użyciem polecenia `storm` należy skopiować plik JAR zawierający klaster. Można to zrobić za pomocą polecenia `scp` z poziomu klienta, na którym znajduje się plik. Na przykład: `scp FILENAME.jar USERNAME@CLUSTERNAME-ssh.azurehdinsight.net:FILENAME.jar`
   > 
