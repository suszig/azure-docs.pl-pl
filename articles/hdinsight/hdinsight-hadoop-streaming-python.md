...
* Windows クライアントとして PuTTY および PSCP これらのユーティリティはから利用可能な <a href="http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html" target="_blank">PuTTY のダウンロード ページ</a>します。
...
Python では、STDIN からの読み取りに **sys** モジュールを、STDOUT への印刷に **print** を使用して、これらの要件を簡単に処理できます。 残りのタスクが簡単にタブを使用してデータを書式設定 (`\t`) キーと値の間に文字です。
...
> [AZURE.NOTE] SSH アカウントのセキュリティ保護にパスワードを使用している場合は、パスワードの入力が求められます。 SSH キーを使用している場合は、使用する必要があります、 `-i` パラメーターと秘密キーへのパス `scp-i/path/to/private/key mapper.py reducer.py username@clustername-ssh.azurehdinsight.net:`します。
...
1. SSH を使用したクラスターへの接続:
    ...
    > [AZURE.NOTE] SSH アカウントのセキュリティ保護にパスワードを使用している場合は、パスワードの入力が求められます。 SSH キーを使用している場合は、使用する必要があります、 `-i` パラメーターと秘密キーへのパス `ssh-i/path/to/private/key username@clustername-ssh.azurehdinsight.net`します。
