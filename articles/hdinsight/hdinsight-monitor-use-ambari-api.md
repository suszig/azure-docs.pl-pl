...
> [AZURE.NOTE] この記事の情報は、Ambari REST API の読み取り専用バージョンを提供する Windows ベースの HDInsight クラスターを主に対象としたものです。 Linux ベースのクラスターでは、次を参照してください。 [管理 Hadoop クラスターの Ambari を使用した](hdinsight-hadoop-manage-ambari.md)します。

## Ambari とは

[Apache Ambari ambari-ホーム][ambari-home] のプロビジョニング、管理、および Apache Hadoop クラスターを監視するために使用します。 演算子ツールの直観的なコレクションおよび Hadoop の複雑さが見えないようにする信頼性の高い、一連の API が含まれており、クラスターの操作を単純化しています。 Api の詳細については、次を参照してください。 [Ambari API リファレンス ][ambari-api-reference]します。
...
- (省略可能) [cURL][curl]. これをインストールするには、「 [cURL を解放し curl ダウンロードをダウンロード][curl-download]します。
    >[AZURE.NOTE] Windows で cURL コマンドを使用する場合、オプション値には一重引用符の代わりに二重引用符を使用します。

- **Azure HDInsight クラスター**。 クラスターのプロビジョニング方法については、「 [[hdinsight の入門] HDInsight の概要][hdinsight-get-started] または [HDInsight クラスターのプロビジョニング ][hdinsight-provision]します。 このチュートリアルを読み進めるには、次のデータが必要です。
...
- [Azure ポータルの ][hdinsight-admin-portal]
- [Azure PowerShell ][hdinsight-admin-powershell]
- [コマンド ライン インターフェイス ][hdinsight-admin-cli]
- [HDInsight のドキュメントの ][hdinsight-documentation]
