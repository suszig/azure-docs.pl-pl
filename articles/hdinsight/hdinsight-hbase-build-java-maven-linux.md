...
1. 開発環境でコマンド ラインで、プロジェクトを作成する場所にディレクトリを変更 `cd コード/hdinsight`します。
...
1. 次のコマンドを使用して、HDInsight クラスターに jar をアップロードします。 **USERNAME** には、SSH ログイン名を指定します。 **CLUSTERNAME** には、HDInsight クラスター名を指定します。
    ...
    > [AZURE.NOTE] SSH アカウントにパスワードを使用した場合は、そのパスワードの入力を求められます。 アカウントを使用して SSH キーを使用した場合は、使用する必要があります、 `-i` パラメーターをキー ファイルへのパスを指定します。 次の例から秘密キーを読み込む `~/.ssh/id_rsa`:
    >
    > `scp-i ~/.ssh/id_rsa./target/hbaseapp-1.0-SNAPSHOT.jar USERNAME@CLUSTERNAME-ssh.azurehdinsight.net: です。`
