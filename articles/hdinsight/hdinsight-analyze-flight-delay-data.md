...
**注:** 図中の番号は、セクション タイトルに対応します。 **M** はメイン (main) プロセスを表します。 **A** は付録 (appendix) の内容を表します。
...
> [AZURE.NOTE] このドキュメントの手順は、Windows ベースの HDInsight クラスターに固有のものです。 Linux ベースのクラスターで手順については、を参照してください [HDInsight (Linux) で Hive を使用したフライト遅延データの分析](hdinsight-analyze-flight-delay-data-linux.md)。
...
[Research と革新技術庁、運輸統計局または RITA ][rita-website]します。 
...
<tr><td>wasb://flightdelay@hditutorialdata.blob.core.windows.net/flightdelays.hql</td><td>実行する Hive ジョブで使用する HiveQL スクリプト ファイル。このスクリプトは、パブリック アクセス権限の設定された Azure BLOB ストレージ アカウントにアップロード済みです。 <a href="#appendix-b">付録 B</a> 準備と Azure Blob ストレージ アカウントにこのファイルをアップロードする手順についてを持ちます。</td></tr>
<tr><td>wasb://flightdelay@hditutorialdata.blob.core.windows.net/2013Data</td><td>Hive ジョブの入力データ。このデータは、パブリック アクセス権限の設定された Azure BLOB ストレージ アカウントにアップロード済みです。 <a href="#appendix-a">Azure BLOB ストレージ アカウントにデータを取得/アップロードする方法については、</a> 手順については、データの取得と Azure Blob ストレージ アカウントにデータをアップロードしています</td></tr>
...
HDInsight クラスターを作成して、Hive ジョブの実行の詳細については、次を参照してください。 [HDInsight ][hdinsight-provision] と [を使用して HDInsight での Hive ][hdinsight-use-hive]します。
...
## <a id="appendix-a"></a>付録 A: Azure Blob ストレージへのフライト遅延データのアップロード
...
## <a id="appendix-b"></a>付録 B: 作成して、HiveQL スクリプトをアップロード
...
HiveQL コマンドの一覧については、次を参照してください。 [Hive データ定義言語 ][hadoop-hiveql]します。 各 HiveQL コマンドは、セミコロンで終了します。
...
3. 次のスクリプトをスクリプト ウィンドウにコピーして貼り付けます。
 ...
 - **$hqlLocalFileName** - HiveQL スクリプト ファイルは、BLOB ストレージにアップロードされる前に、いったんローカルに保存されます。 その際に、このファイル名が使用されます。 既定値は <u>C:\tutorials\flightdelay\flightdelays.hql</u>します。
...
## <a id="appendix-c"></a>付録 C - Sqoop ジョブ出力用の Azure SQL database を準備します。
...
3. 次のスクリプトをスクリプト ウィンドウにコピーして貼り付けます。
 ...
 - **$fireWallRuleName** - Azure SQL Database サーバーのファイアウォール ルールの名前です。 既定の名前は <u>FlightDelay</u>します。 この名前は必要に応じて変更できます。
