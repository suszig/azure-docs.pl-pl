...
このステートメントは、環境変数を設定 **MDS_RUNNER_CUSTOM_CLUSTER** 値 'true' ともでコンピューター全体にこの変数のスコープが設定されます。 環境変数はコンピューターやユーザーの適切な範囲で設定されていることが重要になることがあります。 環境変数を設定する方法の詳細については [1] [こちら] 参照してください。
