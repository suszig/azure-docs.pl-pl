...
## <a id="run"></a> クエリ コンソールを使用して Hive クエリを実行します。
...
2. ページ上部のリンクから、**[Hive エディター]** を選択します。 HDInsight クラスターで実行する HiveQL ステートメントの入力に使用できるフォームが表示されます。
    ...
    テキストを置き換える `選択 * hivesampletable から` を次の HiveQL ステートメント。
...
3. **[ステータス]** フィールドが **Completed** に変わったら、ジョブの **[詳細の表示]** を選択します。 [詳細] ページ、 **ジョブの出力** を含む `[ERROR] 3`します。 このフィールドの下にある **[ダウンロード]** ボタンを使用して、ジョブの出力を含むファイルをダウンロードします。
