...
> [AZURE.NOTE] このドキュメントの情報は、Windows ベースの Storm を使用して HDInsight クラスター上に基づきます。 HDInsight での Linux ベースの Storm から Azure Event Hub を扱う方法については、次を参照してください [HDInsight で Storm を使用して Azure Event Hubs からイベントを処理する。](hdinsight-storm-develop-java-event-hub-topology.md)
...
* **HDInsight での storm**: Event Hub からデータのリアルタイム処理を提供します。

* **HDInsight での HBase** (省略可能): 永続的な NoSQL データ ストアを提供します。

* **Azure Virtual Network サービス** (省略可能、HBase を使用する場合): HDInsight の Storm と HDInsight クラスターの HBase の間でセキュリティで保護された通信を可能です。

* **ダッシュ ボードの web サイト**: サンプル ダッシュ ボードでリアルタイムにデータをグラフ化します。
    ...
    * [Socket.io](http://socket.io/) Storm トポロジと、web サイト間のリアルタイム通信のために使用します。
        ...
        > [AZURE.NOTE] これは、実装の詳細です。 未加工の Websocket や SignalR など、すべての通信フレームワークを使用できます。

    * [D3.js](http://d3js.org/) 、web サイトに送信されるデータのグラフを使用します。

トポロジを使用して Event Hub からデータを読み取り、 **com.microsoft.eventhubs.spout.EventHubSpout** クラスは、HDInsight クラスター上の Storm に用意されています。 使用して、web サイトとの通信を実現 [socket.io client.java](https://github.com/nkzawa/socket.io-client.java)します。
...
> [AZURE.NOTE] これは、トポロジの非常に単純なビューです。 実行時に、各コンポーネントのインスタンスは、読み取られている Event Hub のパーティションごとに作成されます。 これらのインスタンスはクラスター内のノード間に配布され、次のようにデータはルーティングされます。
>
...
* **ParserBolt.java**: スパウトによって送出されたデータは未加工の JSON と、一度に 1 つ以上ありますイベントが生成されます。 このボルトでは、スパウトによって送出されたデータを読み込み方法とそれを複数のフィールドを含むタプルとして新しいストリームに出力する方法を示します。

* **DashboardBolt.java**: これ Java の Socket.io クライアント ライブラリを使用して、web ダッシュ ボードにリアルタイムでデータを送信する方法を示しています。
...
> [AZURE.NOTE] 最終的に、Event Hub スパウトは Maven から使用できます。
...
2.  **新しいイベント ハブの追加** ] ダイアログ ボックスで、入力、 **イベント ハブ名**, を選択、 **地域** 、ハブを作成して、新しい名前空間を作成するか、既存のものを選択します。 最後に、矢印をクリックして続行します。

2.  **Event Hub の構成** ] ダイアログ ボックスに、入力、 **カウントをパーティション分割** と **メッセージ保有** 値。 この例では、パーティション カウントに 10 を、メッセージ保持に 1 を使用します。

3. Event hub が作成されると、名前空間を選択し、選択 **Event Hubs**します。 最後に、先ほど作成した Event Hub を選択します。
...
4. 選択 **構成**, 、次の情報を使用して 2 つの新しいアクセス ポリシーを作成します。
    ...
    アクセス許可を作成すると、選択、 **保存** ページの下部にあるアイコン。 これにより、このハブへのメッセージの送信や、このハブから受信したメッセージの読み取りに使用される共有アクセス ポリシーが作成されます。

5. ポリシーを保存してから、 **共有アクセス キー生成コンポーネント** のキーを取得するページの下部で、 **デバイス** と **storm** ポリシーです。 後で使用されるため、これらを保存します。

### HDInsight クラスターに Storm を作成する

1. サインイン、 [Azure クラシック ポータル](https://manage.windowsazure.com/)します。

2. クリックして **HDInsight** 左側のウィンドウをクリックし **+ 新規** ページの左下隅にあります。

3. 2 番目の列にある HDInsight アイコンをクリックし、選択 **カスタム**します。

4.  **クラスターの詳細** ] ページで、新しいクラスターの名前を入力して選択 **Storm** の **クラスターの種類**します。 矢印をクリックして続行します。

5. 1 の数を入力 **データ ノード** このクラスターで使用します。

    > [AZURE.NOTE] この記事で使用するクラスターのコストを最小限に削減、 **クラスター サイズ** 1、および使用が完了した後にクラスターを削除します。

6. 管理者を入力 **ユーザー名** と **パスワード**, 、矢印をクリックして続行します。

4.  **ストレージ アカウント**, [ **新しいストレージを作成する** か、既存のストレージ アカウントを選択します。 選択または入力、 **アカウント名** と **既定のコンテナー** を使用します。 左下隅にあるチェック マーク アイコンをクリックして、Storm クラスターを作成します。

## EventHubSpout のダウンロードとインストール

1. ダウンロード、 [HDInsight Storm のサンプル プロジェクト](https://github.com/hdinsight/hdinsight-storm-examples/)します。 ダウンロードしたら、 **lib/eventhubs/eventhubs-storm-spout-0.9-jar-with-dependencies.jar** ファイルです。

2. コマンド プロンプトからインストールする、次のコマンドを使用して、 **eventhubs-storm-spout-0.9-jar-with-dependencies.jar** ファイルをローカルの Maven ストアです。 これにより、後の手順で Storm プロジェクトに参照として容易に追加できます。
...
> [AZURE.NOTE] このドキュメントはいない進むこのサンプルに含まれるコードの完全な詳細情報ただし、コードは完全にコメントされています。

開いている、 **Config.properties** ファイルし、Event Hub を作成するときに使用する以前の情報を追加します。 この情報を追加したら、ファイルを保存します。
...
1. 新しいコマンド プロンプトかターミナルを開き、ディレクトリに移動、 **hdinsight-eventhub-例/ダッシュ ボード**, 、次のコマンドを使用して、web アプリケーションで必要な依存関係をインストールします。
...
> [AZURE.NOTE] このセクションの手順では、Node.js を使用して、任意のプラットフォームで実行できるようにします。 その他の言語の例を参照してください、 **SendEvents** ディレクトリ。


1. 新しいコマンド プロンプトかターミナルを開き、ディレクトリに移動 **hdinsight-eventhub-例/SendEvents/nodejs**, 、次のコマンドを使用して、アプリケーションで必要な依存関係をインストールします。

        npm install

2. 開いている、 **app.js** テキスト エディターでファイルを先に取得した Event Hub の情報を追加します。
...
1. ダッシュ ボードを Azure の web サイトを展開する」の手順に従います [をビルドおよび Node.js web サイトを Azure に配置](../web-sites-nodejs-develop-deploy-mac.md)します。 ようになりますが、web サイトの URL に注意してください **mywebsite.azurewebsites.net**します。

2. Azure クラシック ポータルでサイトにアクセスし、選択して、web サイトの作成時に、 **構成** ] タブをクリックします。 有効にする **Web ソケット**, 、] をクリックし、 **保存** ページの下部にあります。

2. 開いている **hdinsight-eventhub-example\TemperatureMonitor\src\main\java\com\microsoft\examples\bolts\DashboardBolt.java** し、発行されたダッシュ ボードの URL を参照するには、次の行を変更します。

        socket = IO.socket("http://mywebsite.azurewebsites.net");

3. 保存、 **DashboardBolt.java** ファイルです。
...
1. 次のコマンドを使用して、プロジェクトから JAR パッケージを作成します。
    ...
    という名前のファイルが作成されます **TemperatureMonitor 1.0-SNAPSHOT.jar** で、 **ターゲット** プロジェクトのディレクトリ。
...
2. ページの下部にあるをクリックして **+ 新規** > **ネットワーク サービス** > **仮想ネットワーク** > **簡易作成**します。

3. 次の値を入力または選択します。

    - **名前**: 仮想ネットワークの名前。

    - **アドレス空間**: アドレス空間に、クラスター内のすべてのノードのアドレスを提供するのに十分な仮想ネットワークを選択します。 そうでないと、プロビジョニングは失敗します。

    - **最大 VM 数**: 仮想マシンの最大数のいずれかを選択します。

    - **場所**: 場所は作成する HBase クラスターと同じである必要があります。

    - **DNS サーバー**: この記事は Azure によって提供される内部 DNS サーバーを使用して、したがって、選択 **None**します。 カスタム DNS サーバーを使用した、より高度なネットワーク構成もサポートされています。 詳細なガイダンスについては、次を参照してください。 [名前解決 (DNS)](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)します。

4. クリックして **仮想ネットワークの作成**します。 新しい仮想ネットワーク名が一覧に表示されます。 [状態] 列が表示されるまで待機 **Created**します。

5. メイン ウィンドウで、作成した仮想ネットワークをクリックします。

6. クリックして、ページ上部にある **ダッシュ ボード**します。

7.  **概要**, 、書き留めて **仮想ネットワーク ID**します。 この ID は、Storm と HBase クラスターのプロビジョニング時に必要です。

8. ページの上部にあるをクリックして **構成**します。

9. 既定のサブネット名は、ページの下部にある **subnet-1**します。 使用して、 **[サブネットの追加** を追加するボタン **subnet-2**します。 これらのサブネットには、Storm クラスターと HBase クラスターが収容されます。

    > [AZURE.NOTE] この記事でおはでを使用するクラスター ノードを 1 つです。 マルチノード クラスターを作成するかどうか、確認してください、 **CIDR (アドレス数)** クラスターで使用されるサブネットです。 アドレス数は、worker ノード数に 7 (ゲートウェイ: 2、ヘッドノード: 2、Zookeeper: 3) を加えた合計よりも多くする必要があります。 たとえば、10 ノードの HBase クラスターが必要な場合、サブネットのアドレス数は、17 (10+7) を超えている必要があります。 17 以下の場合、デプロイは失敗します。
    >
    > 1 つのクラスターには単一のサブネットを指定することを強くお勧めします。

11. クリックして **保存** ページの下部にあります。
...
5. 1 の数を入力 **データ ノード** このクラスターで使用します。  **リージョン/仮想ネットワーク**, 、先ほど作成した Azure の仮想ネットワークを選択します。  **仮想ネットワーク サブネット**, [ **subnet-1**します。
...
5. 新しいを作成するには、この手順を繰り返します **HBase** クラスターです。 主な違いを次に示します。

    * **クラスターの種類**: 選択 **HBase**

    * **仮想ネットワーク サブネット**: 選択 **サブネット 2**
...
返された JSON データでの検索、 **"host_name"** エントリです。 これには、クラスターのノードの FQDN が含まれています。次に例を示します。

    ...
    "host_name": "wordkernode0.<clustername>.b1.cloudapp.net
    ...

クラスター名で始まるドメイン名の部分は、DNS サフィックスなど **mycluster.b1.cloudapp.net**します。

### HBase ボルトを有効にします。

1. 開いている **hdinsight-eventhub-example\TemperatureMonitor\conf\hbase-site.xml** と置換、 `suffix` 、HBase クラスターの DNS サフィックスを持つ次の行のエントリが以前に入手できます。 これらの変更を行った後は、ファイルを保存します。
...
1. 開いている **hdinsight-eventhub-example\TemperatureMonitor\src\main\java\com\microsoft\examples\bolts\ ** をテキスト エディターで削除することで、次の行をコメント解除します、 `//` 最初からです。 これらの変更を行った後は、ファイルを保存します。
    ...
    > [AZURE.NOTE] いないときにテストしているローカルで Storm クラスターに展開する場合のみ、HBase ボルトを有効にする必要があります。
