...
HDInsight の実装と HBase のスケールアウト アーキテクチャにより、テーブルの自動シャーディング、読み取りと書き込みの強力な一貫性、自動フェールオーバーなどが実現します。 また、メモリ内キャッシュを利用した読み取りと高スループットのストリーミングによる書き込みによって、パフォーマンスも拡張されています。 HDInsight HBase では、仮想ネットワーク プロビジョニングも利用できます。 詳細については、「  [HDInsight クラスターのプロビジョニング Azure Virtual Network ][hbase-provision-vnet]します。


## HDInsight HBase 内のデータの管理方法

使用して HBase にデータを管理できます、 `作成`, 、`取得`, 、`配置`, 、および `スキャン` HBase シェルからコマンドです。 使用して、データベースにデータが書き込まれる `配置` を使用して読み取ると `取得`します。  `スキャン` テーブル内の複数の行からデータを取得するコマンドを使用します。 HBase の C# API を使用してデータを管理することもできます。これにより、HBase REST API 最上層のクライアント ライブラリを利用できます。 HBase データベースは、Hive を使用して照会することもできます。 これらのプログラミング モデルの概要については、次を参照してください。 [[hbase--はじめに] HDInsight での Hadoop での HBase の使用開始][hbase-get-started]します。 コプロセッサも利用でき、データベースをホストするノードでデータが処理できます。
...
- [[Hbase--はじめに] HDInsight での Hadoop で HBase の使用します。][hbase-get-started]
- [Azure Virtual Network ][hbase-provision-vnet]
- [HDInsight での HBase レプリケーションを構成します。](hdinsight-hbase-geo-replication.md)
- [HDInsight ][hbase-twitter-sentiment]
- [Maven を使って HDInsight (Hadoop) ][hbase-build-java-maven]

## <a name="see-also"></a>関連項目

- [Apache HBase](https://hbase.apache.org/)
- [Bigtable: 構造化データ用分散ストレージ システム](http://research.google.com/archive/bigtable.html)
