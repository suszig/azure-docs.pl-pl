...
> [AZURE.NOTE] Windows クライアントからは、この記事の手順を実行する必要があります。 MapReduce、Hive、または Pig を HDInsight の Linux、OS X または Unix クライアントの使用方法の詳細については、次の記事を参照して、いずれかを選択、 **SSH** または **Curl** リンク。
>
...
- **Azure HDInsight クラスター**します。 手順については、次を参照してください。 [HDInsight の概要][hdinsight-get-started] または [作成の Hadoop クラスターの HDInsight][hdinsight-provision]します。
...
参照してください [Hive クエリの実行 PowerShell を使用します。](hdinsight-hadoop-use-hive-powershell.md)

## Visual Studio を使用して Hive ジョブを送信する

参照してください [Visual Studio の HDInsight Hadoop Tools を使い始める][hdinsight-visual-studio-tools]します。

##PowerShell を使用して Sqoop ジョブを送信する

参照してください [HDInsight での Sqoop の使用][hdinsight-use-sqoop]します。
...
5. キーを押して **f5 キーを押して** アプリケーションを実行します。
