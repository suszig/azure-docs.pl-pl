...
詳細については、次を参照してください。 [VNet 間接続を構成する](../virtual-network/virtual-networks-configure-vnet-to-vnet-connection.md)します。

次のビデオを参照してください。

> [AZURE.VIDEO configure-the-vpn-connectivity-between-two-azure-virtual-networks]

このチュートリアルの一部である、 [系列 ][hdinsight-hbase-replication] HBase geo レプリケーションを作成します。

- 2 つの仮想ネットワーク間に VPN 接続を構成します (このチュートリアル)
- [DNS 仮想ネットワークの ][hdinsight-hbase-geo-replication-dns]
- [HBase geo レプリケーション ][hdinsight-hbase-geo-replication]
...
- **Azure PowerShell を実行できるワークステーション**。 参照してください [のインストールおよび使用 Azure PowerShell](http://azure.microsoft.com/documentation/videos/install-and-use-azure-powershell/)します。
...
4.  次のように入力します。
    ...
    - **DNS サーバー**: (空白のままに)
        ...
        仮想ネットワーク内での名前解決には、独自の DNS サーバーが必要になります。 使用する場合の詳細については Azure で提供される名前解決と DNS サーバーを使用する場合を参照してください [名前解決 (DNS)](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)します。 Vnet 間で名前解決を構成する手順については、次を参照してください。 [2 つの Azure 仮想ネットワーク ][hdinsight-hbase-dns]します。
...
- [2 つの Azure 仮想ネットワーク ][hdinsight-hbase-geo-replication-dns]
- [HBase geo レプリケーション ][hdinsight-hbase-geo-replication]
