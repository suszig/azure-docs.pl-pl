...
詳細については、次を参照してください。 [VNet 間接続を構成する](../virtual-network/virtual-networks-configure-vnet-to-vnet-connection.md)です。 

次のビデオを参照してください。

> [AZURE.VIDEO configure-the-vpn-connectivity-between-two-azure-virtual-networks]

このチュートリアルの一部である、 [シリーズ][hdinsight-hbase-replication] HBase geo レプリケーションを作成します。 
...
2.  クリックして **新規**, 、**ネットワーク サービス**, 、**仮想ネットワークの**, 、**カスタム作成**します。
3.  次のように入力します。
...
4.  次のように入力します。
    ...
    - **DNS サーバー**: (空白のままに) 
        ...
        仮想ネットワーク内での名前解決には、独自の DNS サーバーが必要になります。 使用する場合の詳細については Azure で提供される名前解決と DNS サーバーを使用する場合を参照してください [名前解決 (DNS)](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md)します。 Vnet 間で名前解決を構成する手順については、次を参照してください。 [2 つの Azure 仮想ネットワーク間の DNS の構成][hdinsight-hbase-dns]します。
    ...
    - **サイト間 VPN の構成**: (オフ)
        ...
        米国東部で Azure の仮想ネットワークにサイト間 VPN 接続を構成するは データ センターです。
5.  次のように入力します。
    ...
    アドレス空間は、米国と重複しないことができます。 仮想ネットワークです。  
...
1. Azure クラシック ポータルでは、次のようにクリックします。 **新規**, 、**ネットワーク サービス**, 、**仮想ネットワーク**, 、**ローカル ネットワークの追加**します。
3. 次のように入力します。

    - **名前**: Contoso LNet EU
    - **VPN デバイスの IP アドレス**: 192.168.0.1 (このアドレスは後で更新するされます)
...
1.  Azure クラシック ポータルでは、次のようにクリックします **ネットワーク** 左側のウィンドウで。
2.  クリックして **- CONTOSO-VNET-EU**します。
3.  クリックして、 **構成** ] タブをクリックします。
4.  確認 **ローカル ネットワークに接続する**です。
5.   **ローカル ネットワーク**, [ **Contoso LNet US**します。
6.  クリックして **ゲートウェイ サブネットの追加** 仮想ネットワーク アドレス空間セクションです。
7.  クリックして **保存**します。
8.  クリックして **OK** を確認します。
...
1.  Azure クラシック ポータルでは、クリックして、 **ダッシュ ボード** ] タブをクリックします。
4.  クリックして **ゲートウェイの作成** クリックして、ページの下部にある **動的ルーティング**します。
5.  クリックして **はい** を確認します。 ページ上のゲートウェイの図が黄色に変わり、"ゲートウェイを作成しています" と表示されることを確認します。 ゲートウェイが作成されるまでに、通常、15 分ぐらいかかります。
...
6.  コピーを作成、 **ゲートウェイの IP アドレス**です。 次のセクションでは、これを使用して Contoso-VNet-EU の VPN ゲートウェイ IP アドレスを構成します。
...
1.  Azure クラシック ポータルでは、次のようにクリックします。 **ネットワーク** 左側のウィンドウでします。
2.  クリックして **ローカル ネットワーク** 上部にあります。
3.  クリックして **Contoso LNet EU**, 、] をクリックし、 **編集** 下部にあります。
4.  更新プログラム **VPN デバイスの IP アドレス**です。  これは、VContoso-VNET-EU の [ダッシュボード] タブから取得したアドレスです。
5.  右側のボタンをクリックします。
...
1. ワークステーションで、開く **Windows PowerShell ISE** または Windows PowerShell コンソールです。
