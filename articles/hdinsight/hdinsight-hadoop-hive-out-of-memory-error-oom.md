...
顧客に相談 [HDInsight の Hadoop の Hive 最適化クエリ](hdinsight-hadoop-optimize-hive-query.md), 、Tez 実行エンジンを使おうと決意しました。 Tez 設定を有効にして同じクエリを実行すると、クエリの実行時間は 15 分間になり、次のエラーがスローされました。
...
ブログの投稿で説明したよう [HDInsight で Hadoop Yarn のメモリ設定](http://blogs.msdn.com/b/shanyu/archive/2014/07/31/hadoop-yarn-memory-settings-in-hdinsigh.aspx), Tez 実行エンジンは、ヒープを使用する場合、使用される領域は、実際には Tez のコンテナーに属します。 Tez コンテナー メモリについて説明した次の図を参照してください。
