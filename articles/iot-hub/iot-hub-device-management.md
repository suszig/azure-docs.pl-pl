...
* **[デバイスのレジストリおよびデバイス モデル](#device-registry-and-device-models)**: システム、および検証メソッドでデバイスのモデルがデバイスとの関係、ロールのメタデータを使用する構造化を表します。
* **[デバイスのアクセス管理](#device-access-management)**: どのデバイスからアクセスを制御しデバイス リソースへのクラウド サービスです。
* **[リモート制御](#remote-control)**: リモート ユーザーがデバイスにアクセスし、デバイスを変更するように指示する方法です。
...
* **[リモートのファームウェアとソフトウェアの更新](#remote-firmware-and-software-update)**: プロセスが管理者を使用するデバイスのファームウェアとソフトウェア更新できます。
...
用語の使用 *デバイス モデル* を読み取りまたはクラウド サービスによって書き込みができるデバイスのプロパティを含むモデルを参照します。 クラウド サービスからリモートで実行できるデバイス コマンドも含みます。  次のセクションで説明するサービス駆動型モデルでは、そのモデルに関する知識をデバイス側は持ちませんが、クラウド サービスはそれでも、デバイスに関するメタデータを追跡し、利用することができます。
...
リモート監視構成済みのソリューション コード、デバイス オブジェクトとを作成しで定義されているサービスに送信 **Simulator/Simulator.WorkerRole/SimulatorCore/Devices/DeviceBase.cs**します。 自己記述式、つまり自分自身の情報を伝えるデバイス モデルを作成し、Azure IoT Hub に送信する手法が、`SendDeviceInfo` メソッドと `GetDeviceInfo` メソッドに使われています。

クラウド サービスのプロセスのデバイスのモデルの関連するイベントのコードがある **EventProcessor/EventProcessor.WorkerRole/Processors/DeviceManagementProcessor.cs**します。 事前構成済みソリューションのサービス側に送られたデバイス モデル関連メッセージに対して行われるアクションに含まれる処理の大部分は、`ProcessJToken` メソッドで行われます。

デバイス モデルに関連するメッセージが受信されると、メソッド `UpdateDeviceFromSimulatedDeviceInfoPacketAsync` と `UpdateDeviceFromRealDeviceInfoPacketAsync` で **DeviceManagement/Infrastructure/BusinessLogic/DeviceLogic.cs** デバイス レジストリを更新するために行われます。 リモートの監視構成済みのソリューション内のデバイス レジストリ Api を参照して **DeviceManagement/Web/WebApiControllers/DeviceApiController.cs**します。
...
ファームウェア更新の完了後、デバイスは正常な状態を検証によって把握できることが必要となります。 デバイスが正常な状態に移行しなかった場合、デバイス上のソフトウェアは既知の正常な状態にロールバックする処理を開始する必要があります。 既知の良好な状態は、前回正常起動時の状態またはと呼ばれるデバイスのファームウェア イメージを指定できます、 *ゴールデン状態*, 、記憶域のパーティションに格納されています。
