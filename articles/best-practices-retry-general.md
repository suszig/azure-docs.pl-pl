...
このドキュメントでは、一過性の障害に対処するうえでの一般的なガイダンスについて取り上げます。 Microsoft Azure サービスを使用する場合は、一時的な障害を処理については、次を参照してください。 [Azure サービスごとの再試行ガイドライン](best-practices-retry-service-specific.md)します。
...
* **かどうか、操作を再試行するために適した**:
  ...
  * サービスまたはコンポーネントを作成する際、エラー コードおよびエラー メッセージは、失敗した操作を再試行すべきかどうかをクライアントが判断しやすいように実装してください。 具体的には、クライアントが操作を再試行するかどうかを示します (おそらく、返すことによって、 **isTransient** 値) し、次の再試行までの適切な待ち時間を提案します。 Web サービスを構築する場合は、サービス コントラクト内で定義されているカスタム エラーを返すことをお勧めします。 一般的なクライアントにはわかりにくい可能性もありますが、カスタム クライアントを構築する際に役立ちます。
* **適切な再試行回数と間隔を決める。**
  ...
  * 適切な再試行間隔を決めるのは、戦略を練り上げるうえで最も難しい領域です。 一般に、次のような再試行間隔が用いられます。
      * **指数バックオフ**します。 少し間を置いて 1 回目の再試行を行い、その後、再試行を重ねるたびに加速度的にその間隔を増やしていく手法です。 たとえば、1 回目は 3 秒後、2 回目は 12 秒後、3 回目は 30 秒後というように再試行を行います。
      * **段階的間隔**します。 少し間を置いて 1 回目の再試行を行い、その後、再試行を重ねるたびに段階的にその間隔を増やしていく手法です。 たとえば、1 回目は 3 秒後、2 回目は 7 秒後、3 回目は 13 秒後というように再試行を行います。
      * **定期的な間隔**します。 アプリケーションが待機する試行間隔は一定で変化しません。 たとえば、3 秒おきに操作を再試行します。
      * **即時再試行**します。 ネットワーク パケットの衝突、ハードウェア コンポーネントにおける瞬時性電圧上昇など、一過性の障害の中でも、とりわけ短時間で終わる障害もあります。 その場合、アプリケーションが次の要求を組み立てて送信するまでの間に障害が解消されれば操作に成功する可能性があるので、直ちに操作を再試行するのが妥当です。 ただし即時再試行を複数回にわたって行うことは避けてください。即時再試行に失敗した場合は、指数バックオフやフォールバック アクションなど、別の手法に切り替える必要があります。
      * **ランダム化**します。 特定のクライアントの複数のインスタンスが同じタイミングで再試行要求を送信するのを防ぐために、これまでに挙げた再試行方式にはいずれも無作為化を取り入れることができます。 たとえば、あるインスタンスには 3 秒後、11 秒後、28 秒後、別のインスタンスには 4 秒後、12 秒後、26 秒後に操作を再試行させることができます。 無作為化は、他の手法と組み合わせることのできる効果的な手法です。  
...
* **アンチ パターンを避けるため**:
  ...
  *  再試行が際限なく繰り返される設計は確実に避けてください。 リソースやサービスが過負荷状況から回復するのを妨げ、使用量が調整されたり接続が拒否されたりする状況が長引く原因となります。 有限数再試行か、またはなどのパターンを実装 [遮断器](http://msdn.microsoft.com/library/dn589784.aspx) サービスが回復できるようにします。
...
* **再試行の戦略と実装をテストする。**
  * できるだけ幅広い条件下で十分に再試行の戦略をテストしてください。特に、アプリケーションとそこで使用される対象リソース/サービスのどちらにも極度の負荷がかかっている状況でテストすることは重要です。 テスト時の動作確認には、次の方法を用いることができます。
      * 一過性の欠陥と一過性ではない欠陥をサービスに対して意図的に注入する。 たとえば、無効な要求を送信することが考えられます。また、テスト要求を検出してさまざまな種類のエラーを返すコードを追加することもできます。 TestApi を使用する例を次を参照してください。 [TestApi を使った欠陥注入テスト](http://msdn.microsoft.com/magazine/ff898404.aspx) と [TestApi – パート 5 の概要: マネージ コードの欠陥注入 Api](http://blogs.msdn.com/b/ivo_manolov/archive/2009/11/25/9928447.aspx)します。
      ...
      * HTTP ベースの API の場合、自動テストに FiddlerCore ライブラリを使い、ラウンドトリップ時間を追加するか、応答 (HTTP 状態コード、ヘッダー、本文などの要素) に変更を加えることによって、HTTP 要求の結果に変更を加える。 これにより、一過性の障害に限らず、障害の状況をサンプリング的に作り出して決定論的なテストを行うことができます。 詳細については、次を参照してください。 [FiddlerCore](http://www.telerik.com/fiddler/fiddlercore)します。 このライブラリを使用する方法の例については、特に **HttpMangler** クラスを確認してください、 [Azure Storage SDK のソース コード](https://github.com/Azure/azure-storage-net/tree/master/Test)します。
      * 高負荷率テストと同時実行テストを実施し、そうした状況下で再試行の機構と戦略が正しく機能すること、またクライアントの動作に悪影響を及ぼしたり要求間の相互汚染を引き起こさないことを確認する。
* **再試行ポリシーの構成を管理する。**
  * A _再試行ポリシー_ すべての再試行戦略の要素の組み合わせです。 障害が一過性といえるかどうかの基準や使用する間隔の種類 (一定間隔、指数バックオフ、無作為化など)、実際の試行間隔、再試行の回数などを決定する検出の働きを特徴付けるのが、再試行ポリシーであるともいえます。
...
* **絶えず失敗する操作に対処する。**
  * いつ実行しても毎回操作が失敗するという場合、その状況にいかに対処するかを検討する必要があります。
      ...
      * 実装を検討している、継続的に失敗した操作の継続的な再試行を防ぐために、 [サーキット ブレーカーのパターン](http://msdn.microsoft.com/library/dn589784.aspx)します。 このパターンは、指定された時間枠内に生じた失敗の回数がしきい値を超えた場合、障害のあるリソースやサービスへのアクセスを試みずにすぐ、エラーとして呼び出し元に要求を返すものです。
...
* [The Transient Fault Handling Application Block (一時的なエラー処理アプリケーション ブロック)](http://msdn.microsoft.com/library/hh680934.aspx)
