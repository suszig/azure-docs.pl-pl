...
* **再試行ポリシーの構成を管理する。**
  * _Retry policy_ は、すべての再試行戦略の要素の組み合わせです。 障害が一過性といえるかどうかの基準や使用する間隔の種類 (一定間隔、指数バックオフ、無作為化など)、実際の試行間隔、再試行の回数などを決定する検出の働きを特徴付けるのが、再試行ポリシーであるともいえます。
...
* [一時的な障害処理アプリケーション ブロック](http://msdn.microsoft.com/library/hh680934.aspx)
* [サーキット ブレーカーのパターン](http://msdn.microsoft.com/library/dn589784.aspx)
