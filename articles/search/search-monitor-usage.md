...
Azure Search REST API または .NET SDK を使用することにより、プログラムからサービス メトリックにアクセスできます。  使用している場合は、 [インデクサー](https://msdn.microsoft.com/library/azure/dn946891.aspx) 追加 API は必要な数値を取得できる利用可能な Azure SQL Database または DocumentDB からインデックスを読み込めません。 
  ...
  + [Get Index Statistics (Azure Search API) (インデックス統計の取得 (Azure Search API))](https://msdn.microsoft.com/library/azure/dn798942.aspx)
  + [Count Documents (Azure Search) (ドキュメントのカウント (Azure Search))](https://msdn.microsoft.com/library/azure/dn798924.aspx)
  + [インデクサー状態の取得](https://msdn.microsoft.com/library/azure/dn946884.aspx)
