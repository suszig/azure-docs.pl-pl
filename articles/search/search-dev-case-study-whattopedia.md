...
WhatToPedia は新興の情報メディア企業です。 構築した [WhatToPedia.com](http://whattopedia.com/) –、現在、日、2015 年 2 月 2日北ヨーロッパで市場試験。 その顧客ベースは、主に、容易に管理および維持できる手頃な価格のオンライン状態を必要とする実在の店舗です。
...
- [MVC 4 を使用する azure の web サイト](http://azure.microsoft.com/services/websites/)
- [Azure web ジョブのスケジュールされたタスク](../websites-webjobs-resources.md)
...
ポータル サイトは、MVC 4 で実装された、Azure の web サイトと [Twitter Bootstrap](http://en.wikipedia.org/wiki/Bootstrap_%28front-end_framework%29)します。 ASP.NET フォーム ベースの開発よりもはるかにわかりやすい HTML へのアプローチが提供されているので、MVC を選択しました。 複数のデバイス用のアプリを作成して複数のモバイル プラットフォームを維持する必要がないよう、すべてのデバイスおよびプラットフォームをサポートするために Twitter Bootstrap が選択されました。
...
採用したもう 1 つの簡略化として、課金およびアカウントの支払業務を委託しています。 サード パーティに直接ルーティング顧客支払情報 ([SveaWebPay](http://www.sveawebpay.se/)), 、格納することに、リスクを軽減し、データを保護する側の機密データを格納します。
...
インターネット検索テクノロジを提供戻らせて Azure Search を試して、次のビデオ: [Deep Dive at TechEd Europe](http://channel9.msdn.com/events/TechEd/Europe/2014/DBI-B410)
...
2. リセラーを 1 つずつアップロードするには、Azure Search サービス API を呼び出す (を参照してください: http://msdn.microsoft.com/library/azure/dn798930.aspx)。
...
1. 検索からすべてのドキュメントを取得する Azure Search サービスを呼び出す (を参照してください: http://msdn.microsoft.com/library/azure/dn798927.aspx)
2. 使用して json.net http://james.newtonking.com/json) 検索サービスの応答から次の抽出
   - 結果
   - ファセット
   - 結果カウント
...
表示の問題は、ローカライズされたテキストを含む各言語のドキュメントと、その言語によるプロパティを追加することによって解決しました。 ユーザーが検索用語を入力おユーザー `$filter` をユーザーの言語をフィルター処理する式を選択します。
...
Azure Web ジョブを使用して、インデックスに対してデータを更新します。 検索インデクサー ジョブを作成することで、インデックス作成部分をソリューションに非常に簡単に統合できました。 唯一のコード変更を加えましたが追加されたインデクサー ジョブに合わせて、 `インデックス` フィールドをインデックス状態を示すためにデータ モデルにします。 新しいプロファイルを追加または更新されるたびに、 `インデックス` フィールドが false に設定します。 小売業者がポータルを介して自分のプロファイル データを変更した場合も同様です。

ジョブが持つすべての行を探します `インデックス` を false に設定します。 行を検出すると、ドキュメントが Azure Search にポストされてし、 `インデックス化されて` フィールドが設定を true にします。 データの更新と追加は実際には Azure Search サービスによって行われるため、これらの処理を計画する必要はありませんでした。 既に存在するドキュメントを追加する場合、サービスによって自動的に更新が行われます。
...
Azure SQL Database 内のサブスクライバー データは、最も重要な資産です。 このデータはバックアップ組み込みの機能を使用して (を参照してください [Azure SQL データベースのバックアップと復元](http://msdn.microsoft.com/library/azure/jj650016.aspx))します。 バックアップ スケジュールは、週 1 回のデータベースの完全バックアップ、1 日 1 回のデータベースの差分バックアップ、および 5 分ごとのトランザクション ログ バックアップです。 データのサイズを考えれば、このソリューションは、直接のデータ ボリュームおよびプロジェクションされたデータ ボリュームにとって十分です。
...
この問題は、ファセット カウント パラメーターを制限することで解決しました。 カウント パラメーターは、ユーザーに返されるファセットの数にハードリミットを課します。 カウント パラメーターの説明を含むリンクが見つかったら [ここ](search-faceted-navigation.md)します。
...
使用することがわかりました [Azure BLOB ストレージ エクスプ ローラー](https://azurestorageexplorer.codeplex.com/) (codeplex で使用可能) に、サイトへのイメージおよびビデオの更新の管理で役に立ちます。 メイン サイトに含まれるイメージおよびビデオを手動で更新するための開発者ツールとして使用しています。 この方法はポータルに変更を配置するよりも柔軟性が高く、イメージを更新する必要が生じるたびに完全なテスト イテレーションを行う必要もなくなりました。
...
- [Azure Search に専用の MSDN フォーラム](https://social.msdn.microsoft.com/forums/azure/home?forum=azuresearch)
- [StackOverflow にもタグがあります。](http://stackoverflow.com/questions/tagged/azure-search)
