...
ここで説明を使用してクエリを構築する方法、 [Azure Search .NET SDK](https://msdn.microsoft.com/library/azure/dn951165.aspx)します。 次の内容のサブセットである、 [.NET アプリケーションから Azure Search の使用方法](search-howto-dotnet-sdk.md)します。 エンド ツー エンドの手順については、元の記事を参照してください。

クエリを作成するための前提条件を含めるを使用して行う通常の検索サービスとの接続を確立したこと、 `SearchServiceClient`します。
...
最初の呼び出しでは、クエリ語句 "fancy" または "wifi" を含むすべてのドキュメントを検索します。 2 番目の呼び出しでは、検索テキストが "*" に設定されています。これは、「すべてを検索する」ことを意味します。 詳細については、検索クエリ式の構文でを検索する [Azure Search の簡単なクエリ構文](https://msdn.microsoft.com/library/azure/dn798920.aspx)します。

2 番目の呼び出しでは、OData の $filter 式、category eq 'Luxury' を使用しています。 この式は、カテゴリ フィールドが文字列 "Luxury" に完全に一致するドキュメントのみ検索によって返されるように制限します。 OData 構文の詳細を確認できる [Azure Search の OData 式の構文](https://msdn.microsoft.com/library/azure/dn798921.aspx)します。
