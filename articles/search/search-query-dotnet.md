<properties
    pageTitle="Tworzenie zapytań względem indeksu usługi Azure Search przy użyciu zestawu .NET SDK | Microsoft Azure | Hostowana usługa wyszukiwania w chmurze"
...
W tym artykule opisano tworzenie zapytań względem indeksu przy użyciu [zestawu .NET SDK usługi Azure Search](https://msdn.microsoft.com/library/azure/dn951165.aspx).
...
## III. Tworzenie zapytań względem indeksu
...
#### Typy zapytań
Dwa najczęściej używane [typy zapytań](search-query-overview.md#types-of-queries) to `search` i `filter`. Zapytanie `search` umożliwia wyszukanie jednego lub większej liczby terminów we wszystkich polach _z możliwością wyszukiwania_ w indeksie. Zapytanie `filter` ocenia wyrażenie logiczne w odniesieniu do wszystkich pól _z możliwością filtrowania_ w indeksie.
...
## IV. Obsługa wyników wyszukiwania
