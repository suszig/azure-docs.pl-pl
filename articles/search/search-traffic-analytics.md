...
このオプションを選択して、新しいブレードが開きます。 状態を変更 **に**, 、データがコピーされ、コピーするデータを選択して、Azure ストレージ アカウントを選択します。 ログ、メトリック、またはその両方です。 ログとメトリックをコピーすることをお勧めします。

![][2]


> [AZURE.IMPORTANT] ストレージ アカウントは、同じリージョンや検索サービスと同じサブスクリプション内にある必要があります。 
> 
...
各 blob と呼ばれる 1 つのルート オブジェクトを持ちます **レコード** ログ オブジェクトの配列を格納しています。
...
各ファイルと呼ばれる 1 つのルート オブジェクトには **レコード** メトリック オブジェクトの配列を格納しています。
