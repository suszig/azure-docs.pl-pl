...
Azure Search サービスはホスト型のクラウド検索サービスであり、これを利用すれば、優れた検索機能を簡単に提供できます。 検索を可能にするには、先に Azure Search インデックスにデータを入力する必要があります。 データは、Azure SQL データベースが存在する場合、新しい **Azure SQL Database 用 Azure Search インデクサー** (または **Azure SQL インデクサー** 略して) Azure Search でインデックス作成のプロセスを自動化できます。 これは、記述するコードと対処するインフラストラクチャが減ることを意味します。
...
A **データソース** データ、および (新規作成、変更、削除された行) のデータの変更を効率的に識別するために Azure Search を有効にするポリシーにアクセスするために必要な資格情報のインデックス化にデータを指定します。 データ ソースは、複数のインデクサーで使用できるように、独立したリソースとして定義します。
...
接続文字列を取得することができます、 [Azure Classic Portal](https://portal.azure.com); を使用して、 `ADO.NET connection string` オプション。
...
この方法で作成されたインデクサーには、スケジュールがありません。 作成するとすぐに自動的に 1 回実行されます。 使用するどの時間にもう一度実行することができます、 **インデクサーの実行** 要求。
...
インデクサーの状態と実行履歴 (インデックス付けされた項目、エラーなどの数) を監視するには、使用、 **インデクサーの状態** 要求。 
...
詳細については、応答は記載されて [インデクサー状態の取得](http://go.microsoft.com/fwlink/p/?LinkId=528198)

## スケジュールに従ったインデクサーの実行 ##

スケジュールに従って定期的に実行するようにインデクサーを調整することもできます。 これを行うには、追加、 **スケジュール** プロパティまたはインデクサーを更新するを作成するとき。 次の例では、インデクサーを更新する PUT 要求を示します。
 ...
 **間隔** パラメーターは必須です。 interval は、連続する 2 つのインデクサー実行の開始の時間間隔を示します。 許可される最短の間隔は 5 分です。最長は 1 日です。 XSD"dayTimeDuration"値としてフォーマットされている必要があります (の制限されたサブセット、 [ISO 8601 期間](http://www.w3.org/TR/xmlschema11-2/#dayTimeDuration) 値)。 使用されるパターンは、`P(nD)(T(nH)(nM))` です。 たとえば、15 分ごとの場合は `PT15M`、2 分ごとの場合は `PT2H` です。

省略可能な **startTime** かを示すスケジュールされた実行が開始する必要があります。 省略すると、現在の UTC 時刻が使用されます。 この時刻は過去でもかまいません。その場合、最初の実行はインデクサーが startTime から継続的に実行されているかのようにスケジュールされます。  
...
1. 今は午前 2 時で、3 番目の実行が開始する時刻です。 ただし、ため午前 1 から 2 番目の実行 実行している限り、3 番目の実行はスキップされます。 3 番目の実行は午前 3 時に開始します。

追加、変更、またはを使用して、既存のインデクサーのスケジュールを削除することができます、 **PUT インデクサー** 要求します。 
...
たとえば、インデックス **rowversion** 列は高基準列の最適な候補です。 
このポリシーを使用するには、データ ソースを次のように作成または更新します。 
...
なお、 **softDeleteMarkerValue** 文字列である – 実際の値の文字列形式を使用する必要があります。 たとえば、削除された行が値 1 でマークされる整数列がある場合は、`"1"` を使用します。削除された行がブール型の true 値でマークされる BIT 列がある場合は、`"True"` を使用します。 
