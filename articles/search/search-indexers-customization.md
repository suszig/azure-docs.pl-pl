...
- [Azure search インデクサーを使用した Azure SQL データベースを接続します。](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers-2015-02-28.md)
- [DocumentDB と Azure Search インデクサーを使用した接続](../documentdb/documentdb-search-indexer.md)
- [インデクサーのサポートを含む .NET SDK](https://msdn.microsoft.com/library/dn951165.aspx) または
- [インデクサーの REST API リファレンス](https://msdn.microsoft.com/library/azure/dn946891.aspx)

## データソースとターゲット インデックス間のフィールド名を変更する

**フィールド マッピング**は、フィールド定義の違いを調整するプロパティです。 最も一般的な例では、Azure Search の名前付け規則に違反しているフィールド名が記載されています。 ソース テーブルが 1 つまたは複数のフィールド名がアンダー スコアで開始する位置を検討してください (よう `_id`)。 Azure Search ではアンダースコアで始まるフィールド名は許可されません。そのため、このフィールドの名前を変更する必要があります。

次の例は、「名前」を変更するフィールド マッピングを含めるためのインデクサーの更新を示しています。 `_id` にデータ ソースのフィールド `id` ターゲット インデックス フィールド。
...
このような関数、 `jsonArrayToStringCollection`, 、Collection(Edm.String) ターゲット インデックス フィールドに、JSON 配列として書式設定文字列を含むフィールドを解析しています。 特に Azure SQL インデクサーで使用することを意図しています。SQL にはネイティブのコレクション データ型がないためです。 次のように使用できます。

    "fieldMappings" : [ { "sourceFieldName" : "tags", "mappingFunction" : { "name" : "jsonArrayToStringCollection" } } ] 

たとえば、ソース フィールドに文字列が含まれている場合 `["red"、"white"、"blue"]`, 、型のターゲット フィールド `Collection(Edm.String)` は 3 つの値に設定されます `「赤」`, 、`"white"` と `"blue"`します。

注意してください、 `targetFieldName` プロパティです。 省略した場合、out、 `sourceFieldName` 値を使用します。
