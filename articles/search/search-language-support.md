...
> [AZURE.NOTE] ように、フィールドを定義する際はを一から新しいインデックスを作成するときに、または既存のインデックスに新しいフィールドを追加するときに、言語アナライザーを設定できますのみ。 フィールドの作成中に、アナライザーを含むすべての属性を完全に指定するようにしてください。 フィールドが定義されると、属性を編集したり、アナライザーの種類を変更したりできなくなります。

1. サインイン、 [Azure Classic Portal](https://portal.azure.com) search サービスの [サービス] ブレードを開きます。
2. クリックして **、インデックスの追加** を新しいインデックスを開始したり、既存のインデックスに追加する新しいフィールドのアナライザーを設定する既存のインデックスを開いたり、サービス ダッシュ ボードの上部にあります。
...
5. 次のフィールドに進む前に開いて、 **アナライザー** ] タブをクリックします。 
...
特定のフィールドを使用して、検索要求のスコープを設定できますクエリを発行するエージェントの言語がわかっている場合、 **searchFields** クエリ パラメーターです。 次のクエリは、ポーランド語の説明に対してのみ発行されます。
