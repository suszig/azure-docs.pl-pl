...
インデックスを定義するスキーマを作成するには、ポータルを使用するか、.NET SDK または REST API を呼び出すコードを記述します (このトピックの冒頭にあるタブを参照してください)。 データの取り込みの詳細について、インデックスの作成後が「 [Azure Search データをインポート](search-what-is-data-import.md)します。
...
| [サジェスター](https://msdn.microsoft.com/library/azure/mt131377.aspx)| オート コンプリートや先行入力クエリとも呼ばれ、インデックス内にセクションとして定義されます。|
