...
USG 検索インデックスの構築および読み込みには、このテンプレートには含まれていない、その他のコードが使用されています。 サービスは読み取り専用であるために、書き込みアクセスが必要なすべての操作は事前に完了している必要があります。 ご覧の [スキーマのコピー](#schema) この記事の最後にスキーマを構築するために使用します。
...
- アスタリスクを追加することで、ワイルドカード検索を開始 (`*`)。 これによって、インデックス内のドキュメント数、2,262,578 が示されます。
- 次に、「Yellowstone」と入力し、追加「+ center」、「+ building」、および"-ND"段階的に検索結果を絞り、Yellowstone visitor centers のみ、North dakota を除く: `Yellowstone + center +-ND の構築`します。
- 優先順位の演算子と文字列の一致が結合、検索語句を実行してください: `statue+(park+MT)`します。 次のスクリーン ショットのような結果が表示されます。 機能クラスの下に、多くの検索アプリケーションでよく使用されている、自分でフィルタリングを実行できるファセット ナビゲーションが使用された、ファセット カテゴリが表示されることに注意してください。
...
Azure Search には、検索演算子の動作を制御できる構成可能な **searchMode** プロパティがあります。 このプロパティの有効な値は `、` (既定値) または `すべて`します。 参照してください [単純なクエリ構文](https://msdn.microsoft.com/library/dn798920.aspx) 方法のガイダンスについて、これらのオプションを設定します。

- **searchMode.Any** では、一部の検索用語と一致する場合、その項目は検索結果に含められると規定されています。 検索語句が場合 `観光案内所の Yellowstone`, 、これらの用語のいずれかを含むすべてのドキュメントが検索結果に含まれます。 このモードでは、*再呼び出し*が増えます。
...
> [AZURE.NOTE] **searchMode.Any** は、クエリの構造が、演算子は最小限で語句が多い場合に最適です。 一般的に経験則では、ユーザーは、電子商取引サイトなどのコンシューマー アプリケーションでは語句のみで検索をします。コンテンツやデータでは、検索語句に演算子を含める可能性が高いです。 検索では、演算子が含まれる可能性が考えの場合、特に `されません (-)` 演算子で始まる **searchModel.All**します。 コントラストを調整し、その他の任意の **searchMode.Any** は `または` 、 `いない` 演算子とその他の検索用語では、結果ではなく大幅に増えます。 次の例は、違いを理解するのに役立ちます。
...
再構築アプリ ウィンドウなど、前に使用した検索用語を入力します。 `Yellowstone + center +-ND の構築`, 、への変更前と後の結果を比較して **searchMode**します。

かなりの違いが見られます。 検索結果が 7 つではなく 200万以上になっています。

   ![][11]

含めることにより、動作は、 `いない` 演算子 (この場合は、"-ND")、これは *または必要がある* ではなく *AND 必要がある* ときに **searchMode** に設定されている `、`します。

この構成を指定するには、検索結果を含めるヒット数の検索用語 `Yellowstone`, 、`center`, 、および `構築`, は、すべてのドキュメントが `ノースダコタいない`します。 のみドキュメントは 13,081 のフレーズを含むため `ノースダコタ`, 、ほぼすべてのデータセットが返されます。

ようなシナリオは、ここでは、確かの効果を示しています **searchMode** を含む検索語句に、 `いない` 演算子では必要ない場合は、それを変更する方法と、この動作を把握するのに役立つようにします。

このチュートリアルを続けるには、 **searchMode** 元の値に戻す (設定 `すべて` 39 行の)、プログラムの実行、および残りのタスクの再構築したアプリを使用します。
...
- [を定義するサジェスター](https://msdn.microsoft.com/library/mt131377.aspx) オート コンプリートまたは先行入力クエリ候補の追加をユーザーの入力に応答します。
