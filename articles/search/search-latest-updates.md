...
Azure Search は、Microsoft Azure のホスト型クラウド検索サービスです。 一般提供のサポートされる構成についての 99.9% の可用性サービス レベル アグリーメント (SLA) を提供することが、 [API の 2015年-02-28 バージョン](https://msdn.microsoft.com/library/azure/dn798935.aspx)します。
...
このバージョンを新規追加 [Lucene クエリ構文でサポート](https://msdn.microsoft.com/library/azure/mt589323.aspx) に対して使用できる、 [Azure Search サービス REST API のプレビュー バージョン](search-api-2015-02-28-preview.md)します。 新しい構文を使用するには、指定する必要があります、 `queryType` ドキュメントの検索操作でします。

さらに、次の機能の両方のプレビューでは終了移され、MSDN の公式の API の一部であるようになりました。
- [自然言語プロセッサ](search-language-support.md)
- 検索、提案、および参照クエリに投稿します。
...
- [2015年-02-28-preview](search-api-2015-02-28-Preview.md) 機能 (現在、プレビュー専用機能で構成されている Microsoft 自然言語プロセッサと `moreLikeThis`)。
...
- インデクサーは、fieldMapping コンストラクトをサポートするようになりました。これは実際のフィールド名が外部データベースと Azure Search インデックスとの間で異なる場合に、フィールド割り当てを提供します。 参照してください [インデクサー](search-api-indexers-2015-02-28-Preview.md) の `2015年-02-28-preview` インデクサーの資料についてのバージョン.
...
- [moreLikeThis =](search-api-2015-02-28-Preview.md) 排他の検索パラメーターを相互には `検索 =`, 、クエリの代替実行パスをトリガーします。 フルテキスト検索ではなく `検索 =` 検索用語の入力に基づいて `moreLikeThis =` されて、検索可能フィールドを比較することによって指定されたドキュメントに類似したドキュメントを検索します。

## API バージョン 2015-02-28

**リリース日: 2015 年 3 月 5 日**

- [インデクサー](http://go.microsoft.com/fwlink/p/?LinkID=528210) は Azure SQL Database、Azure DocumentDB と Azure Vm 上の SQL Server 上のデータ ソースからのインデックス作成を大幅に簡略化する新機能です。
...
- インデックスとドキュメントの操作のための REST API。 この API バージョンの大半は、2015-02-28 バージョンでも変更されていません。 バージョンのドキュメントを `2014年-07-31-preview` ご覧 [Azure Search Service REST API バージョン 2014年-07-31-preview](search-api-2014-07-31-preview.md)します。
...
- 地理空間サポートはを通じて提供される、最初から提供されて、 `Edm.GeographyPoint` 当初から Azure Search の一部であったデータ型。
