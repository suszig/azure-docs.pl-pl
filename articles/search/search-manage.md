...
ここで検索サービスを管理する方法について説明します [Azure Classic Portal](https://portal.azure.com)します。

または、管理 REST API を使用することができます。 参照してください [Azure Search 管理 REST API を使ってみる](search-get-started-management-api.md) と [Azure Search 管理 REST API リファレンス](http://msdn.microsoft.com/library/azure/dn832684.aspx) 詳細です。
...
+   [Limits and constraints (Azure Search API) (制限と制約 (Azure Search API))](search-limits-quotas-capacity.md)
+   [料金の詳細](http://go.microsoft.com/fwlink/p/?LinkdID=509792)

サインアップする準備ができたらを参照してください。 [ポータルで Search サービスの作成](search-create-service-portal.md)します。
...
2.  クリックして **参照** | **すべて** | **サービスを検索する**です。
3.  目的の検索サービスの名前をクリックして、ダッシュボードを開きます。
4.  クリックして **プロパティ** プロパティ ページを開きます。 ページの上部にサービス URL が表示されています。 後ですばやくアクセスできるように、このページを固定できます。
...
API バージョンはポータル ページに表示されないため、管理者はこの情報を提供できません。 現在および以前の API バージョンについては、次を参照してください。、 [Azure Search REST API](http://go.microsoft.com/fwlink/p/?LinkdID=509922)します。
...
API キーを取得または再生成するには、サービス ダッシュボードを開きます。 クリックして **キー** キー管理ページを開きます。 キーの再生成または作成のコマンドがページ上部にあります。
...
Search Service API を使用して、ドキュメントとインデックスの数を取得できます。 価格レベルに基づいて、これらの数に関連付けられたハードウェアの制限があります。 参照してください [制限および制約](search-limits-quotas-capacity.md) 詳細です。 
...
> [AZURE.NOTE] キャッシング動作が一時的にによりを超える場合に制限します。 たとえば、共有サービスの使用時に、ドキュメント数がハードウェアの制限である 10,000 個を超える場合があります。 この超過は一時的であり、次の制限強制チェックで検出されます。 


<a id="sub-6"></a>
## 拡大または縮小

すべての検索サービスは、最小限の 1 つのレプリカと 1 つのパーティションで開始されます。 サインアップした場合は、標準の料金を使用して専用のリソースのレベルをクリックすることができます、 **スケール** パーティションと、サービスで使用されるレプリカの数を再調整サービス ダッシュ ボードに並べて表示します。
...
将来の計画のため、ストレージをチェックすることがあります (を使用して [Get Index Statistics](http://msdn.microsoft.com/library/dn798942.aspx)) 実際に使用するどの程度を確認します。 
...
Azure は、カスタム管理ツールを使用している場合、ポータルで、または、Azure リソース マネージャー API で管理するすべてのサービスに対して、グローバルなロールベースの承認モデルを用意しています。 所有者、共同作成者、および閲覧者のロールは、各ロールに割り当てる Active Directory のユーザー、グループ、およびセキュリティ プリンシパルに対するサービス管理レベルを設定します。 参照してください [Azure クラシック ポータルでのロールベースのアクセス制御](../role-based-access-control-configure.md) の詳細についてはロールのメンバーシップ。
...
ロールは、サービス エンドポイントへのアクセス権を付与しないことに注意してください。 インデックスの管理、インデックスの作成、検索データのクエリなど、検索サービスの操作は、ロールではなく API キーによって制御されます。 「とデータ操作の管理用の承認」を参照してください [Azure クラシック ポータルでロールベースのアクセス制御](../role-based-access-control-configure.md) の詳細。
