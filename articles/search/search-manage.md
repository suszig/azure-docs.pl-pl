...
+   [制限と制約](search-limits-quotas-capacity.md)
...
+   [インデックス統計を取得します。](http://msdn.microsoft.com/library/dn798942.aspx)
...
Azure Search の観点から、ロールベースのアクセス制御は次の管理タスクを決定します。
 ...
 ロール| タスク
---|---
 所有者| 開始、停止、またはサービスを削除します。<p>管理者キーの生成とビューとクエリ キー<p>。インデックスの数、インデックス名、ドキュメント数、および記憶域のサイズを含むサービスの状態を表示します。<p>ロールのメンバーシップ (所有者のみは、ロールのメンバーシップを管理できます) を追加または削除します<p>。サブスクリプションおよびサービス管理者では、所有者ロールに自動的なメンバーシップがあります。
 共同作成者| ロール管理を除き、アクセス レベルは所有者と同じです。たとえば、共同作成者を表示して再生成 `api キー`, はロール メンバーシップを変更できません。
 閲覧者| サービスの状態とクエリのキーを表示します。このロールのメンバーは、サービスの開始または停止ができず、管理キーの表示もできません。

ロールは、サービス エンドポイントへのアクセス権を付与しないことに注意してください。 インデックスの管理、インデックスの作成、検索データのクエリなど、検索サービスの操作は、ロールではなく API キーによって制御されます。 「とデータ操作の管理用の承認」を参照してください [Azure クラシック ポータルでのロールベースのアクセス制御](../role-based-access-control-configure.md) の詳細。
