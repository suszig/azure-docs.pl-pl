...
Visual Studio を使用して API アプリを作成すると、API 定義エイド ポイントが自動的に正符号の API アプリのベース URL に設定 `/swagger/docs/v1`します。 これは、既定の URL を [Swashbuckle](https://www.nuget.org/packages/Swashbuckle) NuGet パッケージでは、動的に生成された機能を ASP.NET Web API プロジェクトのメタデータの Swagger です。
