...
1. Azure プレビュー ポータルで、CORS を有効にする API アプリのブレードに移動します。 移動後、API アプリの *[ゲートウェイ]* アイコンをクリックします。
    ...
    ![API アプリの ](./media/app-service-api-javascript-client/19-api-app-blade.png)

1. ポータルのブレードで **[ゲートウェイ ホスト]** リンクをクリックします。

    ![API アプリの ](./media/app-service-api-javascript-client/20-gateway-host-blade.png)

1. ポータルのブレードで **[すべての設定]** リンクをクリックします。

    ![ゲートウェイの ](./media/app-service-api-javascript-client/21-gateway-blade-all-settings.png)

1. ポータルのブレードで **[アプリケーション設定]** をクリックします。

    ![ゲートウェイの ](./media/app-service-api-javascript-client/22-gateway-app-settings-blade.png)
...
1. CORS 機能は、 [Microsoft.AspNet.WebApi.Cors](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Cors/) NuGet パッケージ。 これをインストールするには、**Package Manager Console** を開いて、次の PowerShell スクリプトを実行します。
...
1. `Tbody` と `tfoot` 要素は、各を置き換える [を {に] を} します。 (現在、このサイトでは、コード ブロック内で二重の中かっこを使用した式を表示できません)。
...
Index.html に追加したコードでは、1 は、ベース URL にポート番号を置き換えます (`http://localhost:1578`)、API プロジェクトの実際のポート番号。
