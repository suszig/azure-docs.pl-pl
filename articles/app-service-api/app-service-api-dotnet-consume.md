...
- 呼び出す、 **パブリック (匿名)** コンソール アプリケーションから API アプリ
- 呼び出す、 **パブリック (認証済み)** Windows デスクトップ アプリケーションから API アプリ 
...
1. 実行していない場合はに従って、 [API アプリのデプロイ](app-service-dotnet-deploy-api-app.md) 、Azure サブスクリプションに API アプリに ContactsList サンプル プロジェクトを展開します。
    ...
    そのチュートリアルでは、アクセスを設定するように指示 Visual Studio でのレベルを発行するためのダイアログ **だれでも利用できる**, と同じである **パブリック (匿名)** ポータルで。 ただし、した場合、 [API アプリの保護](../app-service-api-dotnet-add-authentication.md) チュートリアルではその後、アクセス レベルに設定されて **パブリック (認証済み)**, 、その場合は、次の手順で指示されたとおりに変更する必要があります。
...
5. ドロップダウン リストから呼び出すには、をクリックする API アプリを選択 **OK**します。 
...
4.  *Form1.cs* ソース コードは、前に、次のコードを追加、 `Form1()` コンス トラクター、GATEWAY_URL の値を前の手順でコピーした値に置換します。  末尾のスラッシュ (/) を含めていることを確認します。 
...
サンプル アプリケーションを示しています [サーバー フロー](../app-service/app-service-authentication-overview.md#server-flow), 、これは、ゲートウェイが id プロバイダーのアクセス トークンを取得します。  [クライアント フロー](../app-service/app-service-authentication-overview.md#client-flow), 、クライアント アプリケーションが id プロバイダーから直接アクセス トークンを取得、ゲートウェイに送信を呼び出します `LoginAsync` なく `SetCurrentUser`です。 
...
この記事では、API アプリ] に設定用の .NET クライアントから API アプリを使用する方法を説明します。 **パブリック (認証済み)** と **パブリック (匿名)** アクセス レベル。 
