<properties
    pageTitle="Wprowadzenie do usługi API Apps i platformy ASP.NET w usłudze App Service | Microsoft Azure"
...
* Program Visual Studio 2015 z [zestawem Azure SDK dla platformy .NET](http://go.microsoft.com/fwlink/?linkid=518003) — jeśli nie masz jeszcze programu Visual Studio 2015, zostanie on automatycznie zainstalowany podczas instalacji tego zestawu SDK.
...
2. Ustaw projekt ToDoListDataAPI (a **nie** projekt ToDoListAPI) jako projekt startowy. 
 
4. Naciśnij klawisz F5 lub kliknij kolejno pozycje **Debuguj > Rozpocznij debugowanie**, aby uruchomić projekt w trybie debugowania.

    W przeglądarce zostanie wyświetlona strona z informacją o błędzie HTTP 403.

12. Na pasku adresu przeglądarki dodaj element `swagger/docs/v1` na końcu adresu i naciśnij klawisz Return. (Adres URL to `http://localhost:45914/swagger/docs/v1`).
...
1. Zamknij przeglądarkę i zatrzymaj debugowanie w programie Visual Studio.

3. W projekcie ToDoListDataAPI w **Eksploratorze rozwiązań** otwórz plik *App_Start\SwaggerConfig.cs*, a następnie przewiń w dół i usuń znaczniki komentarza zawierającego następujący kod.
...
5. Uruchom ponownie projekt.

3. Na pasku adresu przeglądarki dodaj element `swagger` na końcu adresu i naciśnij klawisz Return. (Adres URL to `http://localhost:45914/swagger`).

4. Gdy pojawi się strona interfejsu użytkownika programu Swagger, kliknij pozycję **ToDoList**, aby wyświetlić dostępne metody.

    ![Dostępne metody interfejsu użytkownika programu Swagger](./media/app-service-api-dotnet-get-started/methods.png)

5. Kliknij pierwszy przycisk **Get** na liście.

6. W sekcji **Parameters** (Parametry) wpisz gwiazdkę jako wartość parametru `owner`, a następnie kliknij pozycję **Try it out** (Wypróbuj to).
...
6. Kliknij przycisk **Post**, a następnie kliknij pole w obszarze **schematu modelu**.
...
7. Zmień dane JSON w polu wejściowym parametru `todo`, aby wyglądały one podobnie jak w poniższym przykładzie. Możesz również wpisać własny tekst:
...
10. Kliknij pozycję **Try it out** (Wypróbuj to).

    Interfejs API ToDoList zwraca kod odpowiedzi HTTP 204, co oznacza powodzenie.

11. Kliknij pierwszy przycisk **Get**, a następnie kliknij przycisk **Try it out** (Wypróbuj to) w tej części strony.

    Odpowiedź metody Get zawiera teraz nowe zadanie do wykonania. 

12. Opcjonalnie: wypróbuj również metody Put, Delete i Get by ID.

14. Zamknij przeglądarkę i zatrzymaj debugowanie w programie Visual Studio.
...
3.  W kroku **Profil** kreatora **Publikowanie w sieci Web** kliknij pozycję **Usługa Microsoft Azure App Service**.
...
4. Zaloguj się do konta platformy Azure, jeśli jeszcze tego nie zrobiono, lub odśwież swoje poświadczenia, jeśli wygasły.
...
4. W polu **Nazwa aplikacji interfejsu API** wprowadź nazwę, która jest unikatowa w domenie *azurewebsites.net*. Możesz zaakceptować domyślną nazwę sugerowaną przez program Visual Studio.
...
5. W oknie dialogowym **Konfigurowanie planu usługi App Service** wprowadź ciąg „ToDoListPlan” lub inną wybraną nazwę.
...
11. Dodaj element „swagger” do adresu URL na pasku adresu przeglądarki, a następnie naciśnij klawisz Enter. (Adres URL to `http://{apiappname}.azurewebsites.net/swagger`).
...
12. Otwórz [portal Azure](https://portal.azure.com/).
...
14. Kliknij kolejno pozycje **Przeglądaj > App Services**.

    ![Przeglądanie usług App Services](./media/app-service-api-dotnet-get-started/browseas.png)

15. W bloku **App Services** znajdź swoją nową aplikację interfejsu API i kliknij ją. (W portalu Azure okna, które są wyświetlane z prawej strony, noszą nazwę *bloków*).
...
16. W bloku **Ustawienia** znajdź sekcję **API** i kliknij pozycję **Definicja interfejsu API**. 
...
8. W oknie dialogowym **App Service** rozwiń grupę zasobów używaną na potrzeby tego samouczka i wybierz aplikację interfejsu API, a następnie kliknij przycisk **OK**.
...
5. W projekcie ToDoListAPI otwórz plik *Controllers\ToDoListController.cs*, aby wyświetlić kod, który wywołuje interfejs API za pomocą wygenerowanego klienta. 
...
3.  Na karcie **Profil** kreatora **Publikowanie w sieci Web** kliknij pozycję **Microsoft Azure App Service**.

5. W oknie dialogowym **App Service** kliknij pozycję **Nowy**.

3. Na karcie **Hosting** okna dialogowego **Tworzenie usługi App Service** zaakceptuj domyślną wartość pola **Nazwa aplikacji interfejsu API** lub wpisz nazwę unikatową w domenie *azurewebsites.net*. 
...
4. Z listy rozwijanej **Plan usługi App Service** wybierz wcześniej utworzony plan. Domyślnie zostanie użyta ta wartość. 
...
3.  W kroku **Połączenie** kreatora **Publikowanie w sieci Web** kliknij pozycję **Opublikuj**.
...
4. W bloku **Ustawienia aplikacji** aplikacji interfejsu API przewiń w dół do sekcji **Ustawienia aplikacji** i dodaj następujący klucz oraz wartość:
...
11. W oknie przeglądarki otwórz adres URL nowej aplikacji interfejsu API warstwy środkowej utworzonej przed chwilą dla projektu ToDoListAPI. Aby przejść do tej lokalizacji, możesz kliknąć adres URL w głównym bloku aplikacji interfejsu API w portalu.

13. Dodaj element „swagger” do adresu URL na pasku adresu przeglądarki, a następnie naciśnij klawisz Enter. (Adres URL to `http://{apiappname}.azurewebsites.net/swagger`).
...
12. Wykonaj próbne wywołania metody Get i innych metod, aby sprawdzić, czy wywołanie aplikacji interfejsu API warstwy danych przez aplikację interfejsu API warstwy środkowej przebiega pomyślnie.
