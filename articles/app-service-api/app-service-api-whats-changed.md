...
CORS の構成には従来、**MS_CrossDomainOrigins** のコンマ区切りのアプリ設定が使われていましたが、現在では、そのためのブレードが Azure 管理ポータルに存在します。 また、構成できますリソース マネージャーの Azure PowerShell、CLI などのツールを使用して、または [リソース エクスプ ローラー](https://resources.azure.com/)します。 設定、 **cors** プロパティを **Microsoft.Web/sites/config** のリソースの種類、 **< サイト名 >/web** リソースです。 次に例を示します。
...
Web、Mobile、および API Apps にわたって API 定義ブレードを使用できるようになりました。 管理ポータルでは、相対 URL か、または API の Swagger 2.0 表現をホストするエンドポイントを指す絶対 URL を指定できます。 あるいは、リソース マネージャーのツールを使用して構成することもできます。 設定、 **apiDefinition** プロパティを **Microsoft.Web/sites/config** のリソースの種類、 **< サイト名 >/web** リソースです。 次に例を示します。
...
ほとんどの Web Apps ツールは、同じ基になる **Microsoft.Web/sites** リソース タイプを共有するため、新しい API アプリと連携して動作します。 ただし、Azure Visual Studio ツールは 2.8.1 以降のバージョンにアップグレードする必要があります。API に固有の機能が多数公開されているからです。 SDK をダウンロード、 [Azure ダウンロード ページ](https://azure.microsoft.com/downloads/)します。
...
いくつか [azure.microsoft.com](https://azure.microsoft.com) 以前の API Apps モデル用に作成された記事は、不要になった新しいモデルに適用され、サイトから削除されます。 その Url は、新しいモデルで動作する最も近いにリダイレクトされますが、以前の記事では、引き続き参照できます、 [azure.microsoft.com の GitHub ドキュメント リポジトリ](https://github.com/Azure/azure-content)します。 ほとんどの場合は、記事に記載、 [記事/アプリ-サービスの api](https://github.com/Azure/azure-content/tree/master/articles/app-service-api) フォルダーです。 以前の API アプリをサポートしたり、Marketplace から新しいコネクタ API アプリを作成したりする場合に参考となる可能性の高いページを以下に示します。

* [認証の概要](https://github.com/Azure/azure-content/tree/master/articles/app-service/app-service-authentication-overview.md)
* [API アプリを保護します。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-dotnet-add-authentication.md)
* [内部 API アプリを使用します。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-dotnet-consume-internal.md)
* [クライアント フロー認証を使用して処理します。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-authentication-client-flow.md)
* [展開して、SaaS コネクタ API アプリを構成します。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-connnect-your-app-to-saas-connector.md)
* [新しいゲートウェイを使用する API アプリをプロビジョニングします。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-arm-new-gateway-provision.md)
* [API アプリをデバッグします。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-dotnet-debug.md)
* [SaaS プラットフォームへの接続します。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-dotnet-connect-to-saas.md)
* [ロジック アプリの API アプリを強化します。](https://github.com/Azure/azure-content/tree/master/articles/app-service-api/app-service-api-optimize-for-logic-apps.md)
...
詳細については、記事では、参照、 [API Apps のドキュメント セクション](https://azure.microsoft.com/documentation/services/app-service/api/)します。 これらの記事は更新され、API Apps の新しいモデルを反映しています。 さらに、移行に関する詳細またはガイダンスについては、次のフォーラムを参照してください。
