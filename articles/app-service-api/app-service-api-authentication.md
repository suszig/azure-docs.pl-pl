...
1. 認証済みの要求のみ API アプリへの到達を許可する。
    ...
    事前にわかっている認証プロバイダー (Google、Twitter など) がある場合に機能します。 使用する場合、App Service のログオン プロセスを処理することができます。 または、独自の URL を指定しておき、そこに匿名の要求を App Service からリダイレクトさせる方法もあります。 そのうえで、認証プロバイダーをユーザーが選択できるようにします。
...
.NET API で使用することができます、 `Authorize` 属性があり、きめ細かな承認を簡単に記述できるコード クレームに基づいて .NET クラスでのクレーム情報が設定されるためです。
