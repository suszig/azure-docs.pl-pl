...
1. アプリケーションをローカルで (ctrl + f5 キー) を実行し、追加 `/swagger` ブラウザーのアドレス バーに URL の末尾にします。
...
5. Visual Studio のプロジェクトに戻り、 **ContactsController.cs** ファイルに次のコードを追加します。 このコードを追加、 **投稿** 新規投稿に使用できるメソッドを `連絡先` インスタンスを API です。
