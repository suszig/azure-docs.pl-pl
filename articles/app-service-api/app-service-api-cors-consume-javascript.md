<properties
    pageTitle="Obsługa mechanizmu CORS w usłudze App Service | Microsoft Azure"
...
10. W bloku **Ustawienia**, który pojawi się na prawo od bloku **Aplikacja interfejsu API**, znajdź sekcję **API**, a następnie kliknij pozycję **CORS**.

    ![Wybieranie pozycji CORS w bloku Ustawienia](./media/app-service-api-cors-consume-javascript/clicksettings.png)

11. W polu tekstowym wprowadź adres lub adresy URL, z których mają być dozwolone wywołania języka JavaScript.
...
#### Konfigurowanie mechanizmu CORS za pomocą narzędzi usługi Azure Resource Manager

Mechanizm CORS dla aplikacji interfejsu API można również skonfigurować za pomocą [szablonów usługi Azure Resource Manager](../resource-group-authoring-templates.md) i narzędzi wiersza polecenia, takich jak [program Azure PowerShell](../powershell-install-configure.md) i [interfejs wiersza polecenia Azure](../xplat-cli-install.md). 
...
## Wdrażanie projektu ToDoListAngular w nowej aplikacji sieci Web
...
### Tworzenie nowej aplikacji sieci Web dla projektu ToDoListAngular
...
3. Na karcie **Hosting** okna dialogowego **Tworzenie usługi App Service** wpisz **nazwę aplikacji sieci Web** unikatową w domenie *azurewebsites.net*. 
...
4. Z listy rozwijanej **Plan usługi App Service** wybierz wcześniej utworzony plan. 
...
### Wdrażanie projektu sieci Web ToDoListAngular w nowej aplikacji sieci Web
...
### Testowanie aplikacji bez włączonego mechanizmu CORS 

2. W obszarze narzędzi dla deweloperów w przeglądarce otwórz okno konsoli.

3. W oknie przeglądarki, w którym zostanie wyświetlony interfejs użytkownika klienta AngularJS, kliknij link **listy zadań do wykonania**.
...
## Konfigurowanie mechanizmu CORS dla aplikacji interfejsu API warstwy środkowej
...
12. W polu tekstowym wpisz adres URL aplikacji sieci Web ToDoListAngular (frontonu). Jeśli na przykład projekt ToDoListAngular wdrożono w aplikacji sieci Web o nazwie todolistangular0121, musisz zezwolić na wywołania pochodzące z adresu URL `https://todolistangular0121.azurewebsites.net`.
...
### Testowanie aplikacji z włączonym mechanizmem CORS
...
## Porównanie mechanizmu CORS usługi App Service i mechanizmu CORS interfejsu API sieci Web
...
### Włączanie mechanizmu CORS w kodzie interfejsu API sieci Web
...
1. Dodaj wiersz `config.EnableCors()` w kodzie metody **Register** klasy **WebApiConfig**, jak pokazano w poniższym przykładzie. 
...
1. W kontrolerze interfejsu API sieci Web dodaj instrukcję `using` dla przestrzeni nazw `System.Web.Http.Cors` i dodaj atrybut `EnableCors` do klasy kontrolera lub poszczególnych metod akcji. W poniższym przykładzie obsługa mechanizmu CORS dotyczy całego kontrolera.
...
## Używanie usługi Azure API Management z usługą API Apps
...
* [Azure API Management Overview (Omówienie usługi Azure API Management) (klip wideo, informacje dotyczące mechanizmu CORS są prezentowane od momentu 12:10)](https://azure.microsoft.com/documentation/videos/azure-api-management-overview/)
* [API Management cross domain policies (Zasady usługi API Management obejmujące różne domeny)](https://msdn.microsoft.com/library/azure/dn894084.aspx#CORS)
