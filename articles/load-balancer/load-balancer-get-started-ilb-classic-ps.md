...
[AZURE.INCLUDE [azure-arm-classic-important-include](../../includes/learn-about-deployment-models-classic-include.md)] [リソース マネージャー モデル](load-balancer-get-started-ilb-arm-ps.md)します。
...
このメモ、 [Add-azureendpoint](https://msdn.microsoft.com/library/dn495300.aspx) Windows PowerShell コマンドレットは、DefaultProbe パラメーター セットを使用します。 追加のパラメーター セットの詳細については、次を参照してください。 [Add-azureendpoint](https://msdn.microsoft.com/library/dn495300.aspx)します。
...
トラフィックは負荷分散の内部負荷分散インスタンスの新しい IP アドレス (VIP) を使用するとなるサーバーを構成する必要があります。 このアドレスは、内部負荷分散インスタンスがリッスンしているアドレスです。 多くの場合、必要な操作は、内部負荷分散インスタンスの VIP の DNS レコードの追加または変更のみです。
...
インターネットに接続された web サーバーのセットに負荷分散データベース サービスを提供します。 両方のサーバー セットは、1 つの Azure クラウド サービスでホストされます。 TCP ポート 1433 への Web サーバーのトラフィックは、データベース層の 2 つの仮想マシンに分散される必要があります。 図 1 に、この構成を示します。
...
次のコマンドは、内部負荷分散の新しいインスタンスという名前を構成する **ILBset** し、2 つのデータベース サーバーに対応する仮想マシンにエンドポイントを追加します。
