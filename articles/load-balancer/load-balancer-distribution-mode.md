...
ソース IP アフィニティは Azure Load Balancer と RD ゲートウェイ間の非互換性の問題を解決します。 これで、1 つのクラウド サービス内に RD ゲートウェイ ファームを構築できます。
別の使用シナリオは、実際のデータのアップロードを UDP 経由で行う場合のメディアのアップロードですが、ここではコントロール プレーンが TCP 経由で達成されます。
...
Azure SDK for .NET 2.5 (11 月にリリース予定) を使用してクラウド サービスを更新できます。
