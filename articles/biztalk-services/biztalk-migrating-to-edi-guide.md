...
# BizTalk Services への BizTalk Server EDI ソリューションの移行: テクニカル ガイド

執筆者: Tim Wieman、Nitin Mehrotra

校閲者: Karthik Bharthy

使用して記述します。 Microsoft Azure BizTalk Services – 2014 年 2 月のリリースです。
...
このドキュメントでは、BizTalk Services への BizTalk Server EDI アーティファクトの移行に関連するいくつかの相違点について説明します。 このドキュメントは、BizTalk Server EDI の処理および取引先アグリーメントに関する実用的な知識を前提とします。 BizTalk Server EDI の詳細については、次を参照してください。 [管理を使用して BizTalk Server 取引先](https://msdn.microsoft.com/library/bb259970.aspx)します。

## BizTalk Services に移行できる BizTalk Server EDI アーティファクトのバージョン
...
参照してください **シナリオ/メッセージ フロー** メッセージ フローのパターンは、このトピックにします。
...
Microsoft Azure BizTalk Services も用意されています、 **TPM データ移行** に BizTalk Server の取引先モジュールから BizTalk Services ポータルに取引先およびアグリーメントを移行するツールです。 TPM データ移行ツールは Tools パッケージの一部として、ダウンロードできるから、 [MABS SDK](http://go.microsoft.com/fwlink/p/?LinkId=235057)します。 パッケージには、ツールの使用方法とツールの基本的なトラブルシューティング情報を提供する readme も含まれます。

## スキーマ

BizTalk Services で提供される EDI スキーマは BizTalk Services ソリューションで使用できます。  さらに、BizTalk Server の EDI スキーマも、EDI スキーマのルート ノードが BizTalk Server と BizTalk Services で同じなので、BizTalk Services で使用できます。 そのため、BizTalk Server の EDI スキーマを直接取得し、BizTalk Services を使用して開発する EDI ソリューションで使用できます。 スキーマをダウンロードすることも、 [MABS SDK](http://go.microsoft.com/fwlink/p/?LinkId=235057)します。

## マップ (変換)

BizTalk Server のマップは、BizTalk Services では変換と呼ばれます。 BizTalk Server から BizTalk Services へのマップの移行は、実行するのが比較的複雑なタスクの 1 つです (マップの複雑さに依存します)。 BizTalk Services に使用するマッピング ツールは、BizTalk マッパーとは異なるものです。 マッパーはほとんど同じように見えますが、基になっているマップ形式が異なります。 Functoid (と呼ばれる **マップ操作** BizTalk サービスで) に利用可能なユーザーはも異なります。  実際には、BizTalk Services で BizTalk マップを直接使用することはできません。 また、BizTalk Server で使用できる functoid の中には、BizTalk Services のマップ操作では使用できないものがあります。
...
Microsoft Azure BizTalk Services では、BizTalk Server マップを BizTalk Services 変換に移行するためのツールが提供されています。  **BTMMigrationTool** としての一部では、 **ツール** パッケージは、BizTalk サービス SDK で提供されからダウンロードできます [ここ](http://go.microsoft.com/fwlink/p/?LinkId=235057)します。 このツールの詳細については、次を参照してください。 [BizTalk Services 変換に移行する BizTalk マップを変換](https://msdn.microsoft.com/library/windowsazure/hh949812.aspx)します。

BizTalk Server のマップを BizTalk Services の変換に移行する方法については、BizTalk MVP である Sandro Pereira によるサンプルも参考にしてください。 このサンプルは、使用可能な [ここ](http://social.technet.microsoft.com/wiki/contents/articles/23220.migrating-biztalk-server-maps-to-windows-azure-biztalk-services-wabs-maps.aspx)します。
...
- *WCF ワークフロー サービスを Service Bus キューおよびトピックに統合する方法* Paolo salvatori します。 参照してください [ここ](https://msdn.microsoft.com/library/azure/hh709041.aspx)します。

- *Windows Workflow Foundation と Azure でのアプリの構築* Build 2011 カンファレンスのセッションです。 参照してください [ここ](http://go.microsoft.com/fwlink/p/?LinkId=237314)します。

- *Windows Workflow Foundation デベロッパー センター MSDN*します。 参照してください [ここ](http://go.microsoft.com/fwlink/p/?LinkId=237315)します。

- *MSDN の Windows Workflow Foundation 4 (WF4) ドキュメント*します。 参照してください [ここ](https://msdn.microsoft.com/library/dd489441(v=vs.100).aspx)します。
...
BizTalk Server の EDI 処理には、「フォールバック アグリーメント」の概念があります。  BizTalk services **いない** フォールバック アグリーメントの概念がこれまでにあります。  BizTalk ドキュメントのトピックを参照してください [EDI 処理におけるアグリーメントのロール](http://go.microsoft.com/fwlink/p/?LinkId=237317) と [グローバル構成またはフォールバック アグリーメントのプロパティ](https://msdn.microsoft.com/library/bb245981.aspx) の BizTalk Server でのフォールバック アグリーメントの使用方法に関する情報。
