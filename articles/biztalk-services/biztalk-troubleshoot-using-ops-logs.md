...
> [AZURE.NOTE] この機能では、バックアップにログなど、サービスが開始されたとき、BizTalk サービスに対する管理操作をすることのみ記録されます。 このような操作は、Azure クラシック ポータルから、またはを使用して、実行するかどうかに関係なく、追跡、 [BizTalk サービスの REST Api](http://msdn.microsoft.com/library/azure/dn232347.aspx)します。 管理サービスを使用して追跡される操作の一覧については、次を参照してください。 [サービス追跡される操作を使用して Azure の管理](#bizops)します。<br/><br/>
...
1. Azure クラシック ポータルで、選択 **管理サービス**, 、クリックして、 **操作ログ** ] タブをクリックします。
...
3. チェックマークを選択すると、フィルター処理された一覧が表示されます。 次の図は、testbiztalkservice に関連するアクティビティを示しています。
    ![操作ログの表示][ViewLogs] 
4. 特定の操作についての詳細を表示、行を選択し、をクリックして **詳細** 下部にあるタスク バーにします。
