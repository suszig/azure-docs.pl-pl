...
Azure RemoteApp を使用すると、Windows アプリケーションを、どこでどんなデバイスを使っていても、今すぐ実行できるようなります。 10 年前を記述されたカスタム アプリケーションか、Office アプリであるかどうかをそれらのいくつかのアプリケーション (Windows XP の場合) のような特定のオペレーティング システムに関連付けられている、ユーザーが不要になったがあります。
...
次は、ユーザーの番です。 最初に、ユーザーがインストールして、 [RemoteApp クライアント](remoteapp-clients.md)します。 次に、作成してある Azure ファイル共有にユーザーが各自のアカウントからドライブをマップし、各自の Access ファイルを追加する必要があります。 以下の方法でこれを実行できます。
