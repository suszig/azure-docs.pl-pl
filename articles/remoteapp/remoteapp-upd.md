...
Azure RemoteApp では、セッション間で Outlook の状態 (メールボックス、PST) が保存されます。これを有効にする、PST をユーザー プロファイル データに格納する必要があります (c:\users\<username>)。これはデータの既定の場所なので、この場所を変更しない限り、データはセッション間で保持されます。
...
番号 Upd は、ネットワーク共有として使用できません。 UPD は、ユーザーが Azure RemoteApp にアクティブに接続されている場合にのみ使用できます。
