...
Azure RemoteApp では、デバイスとセッションにわたってユーザー ID とカスタマイズが保存されます。 このユーザー データは、ユーザー プロファイル ディスク (UPD) と呼ばれる、ユーザー/コレクションごとのディスクに格納されます。 ディスクは、ユーザーに依存して、により、ユーザー サインインしている場所に関係なく一貫したエクスペリエンスを持っています。 
保存 
...
>[AZURE.NOTE] UPD を無効にする必要がありますか。 -そのチェック_アウト Pavithra のブログの投稿を行うことができます [を無効にするユーザー プロファイル ディスク (Upd) Azure RemoteApp で](http://blogs.msdn.com/b/rds/archive/2015/11/11/disable-user-profile-disks-upds-in-azure-remoteapp.aspx), 、詳細についてです。


## 管理者がデータにアクセスするには、どうすればよいですか。

必要があります (災害復旧またはユーザーが会社を辞めるかどうか) のユーザーの 1 つのデータにアクセスするに連絡する場合 [Azure RemoteApp](mailto:remoteappforum@microsoft.com) コレクションやユーザー id のサブスクリプション情報を提供します。 Azure RemoteApp チームによって、VHD の URL が提供されます。 VHD をダウンロードし、必要なドキュメントやファイルを取得します。 VHD は 50 GB あるため、ダウンロードに少し時間がかかることに注意してください。
...
チェック アウト [今回](remoteapp-outlook.md) Outlook と Azure RemoteApp を使用する方法についてです。
...
## リダイレクトはいかがですか。
ユーザーがセットアップによってローカル デバイスにアクセスできるように Azure RemoteApp を構成する [リダイレクト](remoteapp-redirection.md)します。 これにより、ローカル デバイスが UPD のデータにアクセスできるようになります。

## UPD をネットワーク共有として使用できますか。
いいえ、できません。 Upd は、ネットワーク共有として使用できません。 UPD は、ユーザーが Azure RemoteApp にアクティブに接続されている場合にのみ使用できます。
...
[はい] に接続する場合は [Azure RemoteApp](mailto:remoteappforum@microsoft.com), を設定するデータにアクセスする URL を使用しています。 約 10 時間後にアクセスの有効期限が切れるので、その前に UPD からデータまたはファイルをダウンロードしてください。
...
参照してください  [を無効にするユーザー プロファイル ディスク (Upd) Azure RemoteApp で](http://blogs.msdn.com/b/rds/archive/2015/11/11/disable-user-profile-disks-upds-in-azure-remoteapp.aspx) の詳細。

## ユーザーによるシステム ドライブへのデータ保存を制限できますか。

はい。ただし、これは、コレクションを作成する前に、テンプレート イメージで設定する必要があります。 システム ドライブへのアクセスをブロックするには、次の手順を実行します。

1. 実行 **gpedit.msc** テンプレート イメージにします。
2. 移動 **ユーザーの構成 > 管理用テンプレート > Windows コンポーネント > Explorer**します。
3. 次のオプションを選択します。
...
スタートアップ スクリプトを実行するには、まず、コレクションに使用するテンプレート イメージで、スケジュールされたタスクを作成します  (この *する前に* sysprep を実行します)。 
...
使用することも [グループ ポリシー ベースのスタートアップ スクリプト](https://technet.microsoft.com/library/cc779329%28v=ws.10%29.aspx)します。 
