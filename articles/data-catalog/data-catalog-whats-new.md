...
> [AZURE.NOTE] [Power BI Desktop で開く] を使用するには、Power BI Desktop アプリケーションの現在のバージョンをインストールしておく必要があります。 問題や、この機能を使用してエラーが発生した場合は、Power BI Desktop からの最新バージョンがあることを確認してください。 [PowerBI.com](https://powerbi.com)します。
...
- **Azure Data Catalog** ポータルでの新しい "概要" データ ソースの検出エクスペリエンス。 ユーザーが、「検出」のページを入力すると、  **Azure Data Catalog** ポータルで、検索語句を入力しなくてが表示されます、最もよく使用されるタグとデータ ソースの種類とオブジェクトの種類の専門家を含むカタログの内容の概要を説明します。
