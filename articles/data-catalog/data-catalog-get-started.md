...
> [AZURE.NOTE] この演習は、省略可能です。 チュートリアルの残りの演習が Adventure Works サンプル データベースを参照する記述されていますが、この手順を省略し、代わりに、独自のデータ ソースを操作する選択することもできます。
...
<table>
            <tr>
              ...
              <td>
                <b>最初の起動時にドメインに参加しているマシンになるように VM をプロビジョニングするには、</b>
              </td>
              <td>
                <b>例</b>
              </td>
            ...
            <tr>
              <td>プロパティ スコープ</td>
              <td>指定したプロパティの検索語句が一致すると、データ ソースを返すのみ</td>
              <td>製品名。</td>
            </tr>
            <tr>
              <td>論理演算子</td>
              <td>範囲を広げるか、このページで、ブール演算子で説明したように、ブール演算を使用して検索を絞り込む</td>
              <td>finance NOT corporate</td>
            </tr>
            <tr>
              <td>かっこを使用したグループ化</td>
              <td>かっこを使用してクエリの一部をグループ化し、論理的に分離します。特にブール演算子と組み合わせて使用します。</td>
              <td>名前: 商品 AND (タグ: 図 OR タグ: フォト)</td>
            </tr>
            <tr>
              <td>比較演算子</td>
              <td>数値データ型および日付データ型を持つプロパティについて、等値演算子以外の比較演算子を使用します。</td>
...
**Azure Data Catalog** では、データ資産の所有権を取得し、共同所有者をデータ資産に追加し、データ資産の可視性を設定できます。
