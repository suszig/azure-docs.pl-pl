...
記述メタデータは、データ資産の目的または意図を説明するメタデータです。 通常、記述メタデータは、カタログ ユーザーが Azure Data Catalog ポータルを使用して追加しますが、登録時にデータ ソースから抽出することもできます。 Azure Data Catalog 登録ツールは SQL Server Analysis Services と SQL Server Reporting Services との間の Description プロパティから記述を抽出、 [ms_description 拡張プロパティ](https://technet.microsoft.com/library/ms190243.aspx) これらのプロパティは、値が設定されている場合、SQL Server データベースにします。
...
> [AZURE.NOTE] データ プロファイルを展開して、大規模なテーブルおよびビューのためのコストのかかる操作にできる、データ ソースを登録に必要な時間が大幅に増加します。
...
> [AZURE.NOTE] 所有権と管理は、標準的な Azure Data Catalog の Edition でのみ使用できます。
