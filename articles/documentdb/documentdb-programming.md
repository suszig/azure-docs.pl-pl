...
-   **アトミック トランザクション:** がアトミックでのデータベース操作で、単一のストアド プロシージャまたはトリガー内で実行される DocumentDB 保証します。 これにより、アプリケーションは、関連する操作を 1 つのバッチに結合できます。その結果は、すべてが成功するか、またはすべてが成功しないかのどちらかになります。 
