...
- JSON プロパティ `{"headquarters":"Belgium"}` プロパティ上の例では、パス/headquarters/belgium に対応します。
- JSON 配列 `{"exports": [{"city":"Moscow"}`, 、`{"city": Athens"}]}` パスに対応する `/exports//市町村/モスクワ` と `/exports//市町村/アテネ`します。
...
スキーマのないにもかかわらずには、DocumentDB の SQL と JavaScript クエリ言語ドキュメント、空間演算、および Udf を JavaScript だけでの呼び出しの間で、リレーショナル プロジェクション、フィルター、階層型ナビゲーションを提供します。 DocumentDB のクエリ ランタイムは、このデータのインデックス ツリー表現に対して直接処理を行えるので、これらのクエリをサポートできます。
...
- DocumentDB インデックスをカスタマイズする方法について説明 [ここ](documentdb-indexing-policies.md)
