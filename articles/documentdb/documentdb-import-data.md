...
3. 複数の代入値を指定することができます。 たとえば、コレクション[0 ~ 1 の ][0-9] ゼロで始まる 20 のコレクション名が生成されます (collection01、..02、.03)。
...
> [AZURE.NOTE] インデックス作成ポリシーを指定しないと、既定のポリシーが適用されます。 DocumentDB インデックス作成ポリシーの詳細を参照 [ここ](documentdb-indexing-policies.md)します。 
...
3. 開始することも、新しいインポートするか (例: 接続文字列情報をソースとターゲットの選択など) は、既存の設定を維持します。 または、すべての値をリセットします。
