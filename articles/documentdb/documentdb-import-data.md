...
この記事は、オープン ソースの DocumentDB データ移行ツールを使用してデータをインポートする方法を示します [Microsoft Azure DocumentDB](http://azure.microsoft.com/services/documentdb/) JSON ファイル、CSV ファイル、SQL、MongoDB、Azure テーブル ストレージ、Amazon DynamoDB、DocumentDB コレクションなど、さまざまなソースからです。
...
- [Microsoft .NET Framework 4.51](https://www.microsoft.com/download/developer-tools.aspx) またはそれ以降。
...
移行ツールのソース コードは github の [このリポジトリ](https://github.com/azure/azure-documentdb-datamigrationtool) 、コンパイル済みのバージョンから [Microsoft ダウンロード センター](http://www.microsoft.com/downloads/details.aspx?FamilyID=cda7703a-2774-4c07-adcc-ad02ddc1a44d)します。  ソリューションをコンパイルすることも、コンパイル済みバージョンをダウンロードして任意のディレクトリに展開することもできます。  その後で、いずれかを実行します。
...
> [AZURE.NOTE] [確認] を使用して、接続文字列] フィールドで指定した MongoDB インスタンスにアクセスできることを確認します。 
...
> [AZURE.NOTE] [確認] を使用して、接続文字列] フィールドで指定された SQL Server インスタンスにアクセスできることを確認します。 
...
*{
  "id":「956」
  "Name":"より細かく Sales and サービス"、
  "Address": {
    "AddressType":「メイン オフィス」
    "AddressLine1":"#500 ~ 75 O'Connor Street"
    "Location": {
      "City":「オタワ」
      「[Stateprovincename]」:「オンタリオ州」
    },
    「郵便番号」:"K4B 1S2"
    "CountryRegionName":"canada に一致"
  }
}*
...
*{
  ...
  "連邦 Agency":「United States の管理用の会議」
  "RedirectInfo": {
    「リダイレクト」:「0」,
...
> [AZURE.NOTE] [確認] を使用して、接続文字列] フィールドで指定した Azure テーブル ストレージ インスタンスにアクセスできることを確認します。 

データのインポート元となる Azure テーブルの名前を入力します。  指定することも、 [フィルター](https://msdn.microsoft.com/library/azure/ff683669.aspx)します。
...
> [AZURE.NOTE] [確認] を使用して、接続文字列] フィールドで指定した Amazon DynamoDB インスタンスにアクセスできることを確認します。 
...
> [AZURE.NOTE] [確認] を使用して、接続文字列] フィールドで指定した DocumentDB インスタンスにアクセスできることを確認します。 
...
> [AZURE.NOTE] コレクション フィールドは、名前持つにはには、正規表現の文字が含まれています。 1 つのコレクションからインポートする場合に、正規表現は後、は、これらの文字を適宜エスケープする必要があります。
...
> [AZURE.NOTE] [確認] を使用して、接続文字列] フィールドで指定した HBase インスタンスにアクセスできることを確認します。 
...
1 つのコレクションにインポートするには、データのインポート先のコレクション名を入力し、[追加] をクリックします。  複数のコレクションにインポートする各コレクション名を個別に入力するか、次の構文を使用して、複数のコレクションを指定します: *collection_prefix*[開始インデックス - 終了インデックス]。  この構文を使用して複数のコレクションを指定する場合、次の点に注意してください。
...
コレクション名を指定したら、コレクションの目的の価格レベルを選択します (S1、S2、または S3)。  インポートのパフォーマンスを最大限に高めるには、S3 を選択してください。  DocumentDB のパフォーマンス レベルの詳細を参照 [ここ](documentdb-performance-levels.md)します。
...
> [AZURE.NOTE] インデックス作成ポリシーを指定しない場合は、既定のポリシーが適用されます。  DocumentDB インデックス作成ポリシーの詳細を参照 [ここ](documentdb-indexing-policies.md)します。 
...
2. ソースとターゲットのオプションがしたらしたら **インポート**します。  インポート処理の進行中に、経過時間、転送済みファイル数、およびエラーに関する情報 (詳細な構成でファイル名を指定しなかった場合) が更新されます。  処理が完了したら、結果をエクスポートできます (インポート エラーに対処する場合など)。 
