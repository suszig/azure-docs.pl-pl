...
---|---|---
 ...
 id| ユーザー設定可能| リソースに対するユーザー定義の一意の名前ユーザーが ID を指定しない場合は、システムによって生成されます。
...
すべてのリソースは URI でアドレス指定されます。値、 **_self** リソースのプロパティは、リソースの相対 URI を表します。URI の形式では、/\<feed\>/{_rid} パス セグメント。
...
UDF は SQL クエリの一部として指定することができ、コアを拡張する手段として使用 [DocumentDB の SQL クエリ言語](https://msdn.microsoft.com/library/azure/dn782250.aspx)します。
...
バイナリの BLOB やメディアは DocumentDB に格納することも、リモート メディア ストアに格納することもできます。 さらに、メディアのメタデータを、添付ファイルと呼ばれる特殊なドキュメントで表すこともできます。 DocumentDB の添付ファイルは、別の場所に格納されたメディアや BLOB を参照する特殊な (JSON) ドキュメントです。 添付ファイルはメタデータ (場所や著者などです。) をキャプチャする特殊なドキュメントにすぎません リモート メディア ストレージに格納されているメディアです。
