...
DocumentDB によって管理されるデータベースのエンティティと呼びます **リソース**します。 各リソースは、論理 URI によって一意に識別されます。 リソースを対話的に扱う際は、標準の HTTP 動詞、要求/応答ヘッダー、状態コードを使用します。 
...
## 階層型リソース モデル
次の図に示した、階層の DocumentDB **リソース モデル** 、データベース アカウントは、それぞれ、不変の論理 URI を使用してアドレス指定可能なリソースのセットで構成されます。 一連のリソースとして参照される、 **フィード** この記事の内容。 

>[AZURE.NOTE] DocumentDB もある RESTful な通信モデルをきわめて効率的な TCP プロトコルが用意されていますから利用できる、 [.NET クライアント SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。

![DocumentDB の階層型リソース モデル][1]  
**階層型リソース モデル**   

リソースの操作を開始するにする必要があります [DocumentDB データベース アカウントの作成](documentdb-create-account.md) 、Azure サブスクリプションを使用します。 データベース アカウントのセットで構成できます **データベース**, 、複数 **コレクション**, 、それぞれを含んでいる順番の **ストアド プロシージャ、トリガー、Udf、ドキュメント** および関連する **添付ファイル** (プレビュー機能)。 データベースが関連付けられて **ユーザー**, 、それぞれのセットを持つ **権限** コレクション、ストアド プロシージャ、トリガー、Udf、ドキュメント、添付ファイルにアクセスします。 データベース、ユーザー、アクセス許可、コレクションが、既知のスキーマを持ったシステム定義のリソースであるのに対し、ドキュメントと添付ファイルは、ユーザーが自由に定義できる JSON コンテンツを格納します。  

|リソース   |説明
|-----------|-----------
|データベース アカウント   |データベース アカウントは一連のデータベースと添付ファイル (プレビュー機能) を格納する固定量の BLOB ストレージと関連付けられます。 データベース アカウントは、Azure サブスクリプションを使用して作成できます。 Standard データベース アカウントにはすべて、最小容量の S1 コレクションが 1 つ割り当てられます。 詳細については、次を参照してください。 当社 [料金のページ](https://azure.microsoft.com/pricing/details/documentdb/)します。
...
|コレクション |コレクションには、JSON ドキュメントのほか、関連する JavaScript アプリケーション ロジックが格納されます。 コレクションは課金対象のエンティティで、そのコストはコレクションに関連付けられたパフォーマンス レベルによって決まります。 パフォーマンス レベル (S1、S2、および S3) では、10 GB のストレージと固定量のスループットが提供されます。 パフォーマンス レベルの詳細については、次を参照してください。 当社 [パフォーマンス ページ](documentdb-performance-levels.md)します。
...
>[AZURE.NOTE] すべてのシステムに対して、リソースのプロパティが生成するには、JSON 表現の中のアンダー スコア (_) が付きます。  
...
### リソースのアドレス指定
すべてのリソースは URI でアドレス指定されます。 値、 **_self** リソースのプロパティは、リソースの相対 URI を表します。 URI の形式では、/\ < フィード \ >/{_rid} パス セグメント。  

|_self の値 |説明
|-------------------|-----------
|/dbs   |データベース アカウント下のデータベースのフィード
|/dbs/{_rid-db} |{_Rid db} の値に一致する id を持つデータベース
|/dbs/{_rid-db}/colls/  |データベース下のコレクションのフィード
|/dbs/{_rid-db}/colls/{_rid-coll}   |{_Rid コレクション} の値に一致する id を持つコレクション
|/dbs/{_rid-db}}/colls/{_rid コレクション}/ドキュメント  |ドキュメントはコレクションのフィード
|/dbs/{_rid-db}}/colls/{_rid コレクション}/docs/{_rid doc}   |{_Rid doc} 値と一致する id を使用して文書化します。
|/dbs/{_rid-db}/users/  |データベース下のユーザーのフィード
|/dbs/{_rid-db}/users/{_rid-user}   |{_Rid ユーザー} の値に一致する id を持つユーザー
|/dbs/{_rid-db}/users/{_rid-user}/permissions   |ユーザーのアクセス許可のフィード
|/dbs/{_rid-db}/users/{_rid-user}/permissions/{_rid-permission} |{_Rid/{_rid-permission} {_rid-permission} の値に一致する id を使用してアクセス許可
...
_Self と _rid プロパティの値は、リソースの代替を持った正規の両方の表現です。  
...
実行できます [を作成し、DocumentDB データベース アカウントを管理する](documentdb-create-account.md) で従来の Azure ポータルを使用して [http://portal.azure.com/](http://portal.azure.com/)します。 データベース アカウントを作成、管理するには管理アクセス権が必要で、Azure サブスクリプションの範囲内でのみ作成、管理ができます。 
...
プロパティ名|説明
---|---
一貫性ポリシー|このプロパティは、データベース アカウントのすべてのコレクションに既定の一貫性レベルを構成するために設定します。 [x-ms-consistency-level] 要求ヘッダーを使用すると、要求ベースごとに一貫性レベルをオーバーライドできます。 <p><p>このプロパティは、ユーザー定義リソースのみに <i>適用されます</i>. システム定義のすべてのリソースは、強力な一貫性の読み取りとクエリがサポートされるように構成されています。
...
注プロビジョニング、構成し Azure クラシック ポータルでは、データベース アカウントを管理できますもプログラムを使用して作成して管理することの DocumentDB データベース アカウントを使用して、 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) だけでなく [クライアント Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。  

## データベース
DocumentDB データベースは、次の図に示すように、1 つ以上のコレクションとユーザーの論理コンテナーです。 プラン上の制約はありますが、DocumentDB データベース アカウントには任意の数のデータベースを作成できます。  

![データベース アカウントとコレクションの階層的モデル][2]  
...
DocumentDB リソース モデル内の他のリソースとデータベースことができますが作成、置換、削除、読み取りまたはいずれかを使用して、簡単に列挙された [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) またはのいずれか、 [クライアント Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。 DocumentDB は、データベース リソースのメタデータの読み取りや照会で強力な一貫性を保証します。 データベースを削除すると、含まれるすべてのコレクションやユーザーに自動的にアクセスできなくなります。   
...
インデックス作成ポリシーは、コレクションで PUT を実行することで変更可能です。 これは、使用するかを達成、 [クライアント SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx), 、 [Azure Classic Portal](https://portal.azure.com) または [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx)します。

### コレクションの照会
コレクション内のドキュメントは任意のスキーマを持つことができ、事前にスキーマやセカンダリ インデックスを提供しなくても、コレクション内でドキュメントを照会することができます。 使用してコレクションを照会する、 [DocumentDB SQL 構文](https://msdn.microsoft.com/library/azure/dn782250.aspx), 、および関係演算子と JavaScript ベースの Udf による拡張性を備えた高度な階層を提供します。 JSON の文法により、ツリー ノードのラベルを付けたツリーとして JSON ドキュメントをモデリングできます。 これは DocumentDB の自動インデックス作成技法と DocumentDB の SQL 方言の両方で利用されています。 DocumentDB クエリ言語の主な機能には以下の 3 つがあります。   
...
DocumentDB クエリ モデルは、機能、効率、簡潔さのバランスを取るように試みます。 DocumentDB のデータベース エンジンは、SQL クエリ ステートメントをネイティブでコンパイルして実行します。 使用してコレクションを照会する、 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) またはのいずれか、 [クライアント Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。 .NET SDK は、LINQ プロバイダーと共に提供されます。

> [AZURE.TIP] DocumentDB を試用して、データセットに対する SQL クエリの実行、 [クエリのプレイ グラウンド](https://www.documentdb.com/sql/demo)します。

### マルチドキュメント トランザクション
データベース トランザクションは、データの同時変更に対応する、安全で予測可能なプログラミング モデルを提供します。 RDBMS でビジネス ロジックを記述する従来の方法が記述されて **ストアド プロシージャ** や **トリガー** トランザクション実行のため、データベース サーバーを送付します。 RDBMS では、アプリケーション プログラマーは、次の 2 種類の異なるプログラミング言語を扱う必要があります。 
...
コレクションの作成後に登録できますストアド プロシージャ、トリガー、Udf を使用してコレクションに、 [Azure DocumentDB REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) またはのいずれか、 [クライアント Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。 登録が終了すると、これらを参照、実行できます。 次に示す全体が JavaScript で記述されたストアド プロシージャを検討してください。以下のコードでは 2 つの引数 (書名と著者名) を使用して、新しいドキュメントを作成し、照会してから更新しますが、すべて暗黙の ACID トランザクション内にあります。 実行中のいずれかの時点で JavaScript 例外がスローされた場合、トランザクション全体が中止されます。
...
クライアントは、HTTP POST 経由でトランザクションを実行するために、前の JavaScript ロジックをデータベースに送信できます。 HTTP メソッドの使用に関する詳細については、次を参照してください。 [DocumentDB リソースと RESTful 操作](https://msdn.microsoft.com/library/azure/mt622086.aspx)します。 
...
### クエリの一部としての UDF の実行
UDF は SQL クエリの一部として指定することができ、コアを拡張する手段として使用 [DocumentDB の SQL クエリ言語](https://msdn.microsoft.com/library/azure/dn782250.aspx)です。
...
上記のスニペットでは登録 (POST)、登録解除 (PUT) を示しましたが、読み取りと一覧表示 (GET)、実行 (POST) を使用して、 [DocumentDB JavaScript SDK](https://github.com/Azure/azure-documentdb-js), 、使用することも、 [REST Api](https://msdn.microsoft.com/library/azure/dn781481.aspx) またはその他の [クライアント Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。 
...
他のすべてのリソースとドキュメントことができますが作成、置換、削除、読み取り、列挙、REST Api またはのいずれかを使用して簡単にクエリを実行、 [クライアント Sdk](https://msdn.microsoft.com/library/azure/dn781482.aspx)します。 ドキュメントを削除すると、入れ子になったすべての添付ファイルに該当するクォータがすぐに解放されます。 ドキュメントの読み取りの一貫性レベルは、データベース アカウントの一貫性ポリシーに従います。 このポリシーは、使用するアプリケーションのデータの一貫性の要件に合わせて、要求ごとにオーバーライドできます。 ドキュメント照会時の読み取りの一貫性は、コレクションに設定されたインデックス作成モードに従います。 "一貫性" に関しては、アカウントの一貫性ポリシーに従います。 

## 添付ファイルとメディア
>[AZURE.NOTE] 添付ファイルとメディア リソースとは、プレビュー機能です。
...
## アクセス許可
アクセス制御の観点、データベース アカウントなどのリソースからデータベース、ユーザー、アクセス許可と見なさ *管理* リソース管理者のアクセス許可が必要なためです。 これに対して、リソースは、コレクションを含む、ドキュメント、添付ファイル、ストアド プロシージャ、トリガー、およびユーザー定義関数が指定のデータベースのスコープし見なさ *アプリケーション リソース*します。 承認モデルを定義で 2 つの種類のリソースと (管理者とユーザー) にアクセスするロールに対応する 2 種類の *アクセス キー*: *マスター_キー* と *リソース キー*します。 マスター キーは、データベース アカウントの一部で、データベース アカウントをプロビジョニングしている開発者 (または管理者) に提供されます。 このマスター キーは管理者セマンティクスを持ち、管理リソースとアプリケーション リソースの両方にアクセスを承認するために使用できます。 一方、リソース キーは細密なアクセス キーにアクセスできる、 *特定* アプリケーション リソースです。 このため、リソース キーは、データベースのユーザーと、そのユーザーが特定のリソース (コレクション、ドキュメント、添付ファイル、ストアド プロシージャ、トリガー、または UDF) に対して所有するアクセス許可との間の関係を捕捉します。   
...
## 次のステップ
HTTP コマンドを使用してリソースの操作に関する詳細については [DocumentDB リソースと RESTful 操作](https://msdn.microsoft.com/library/azure/mt622086.aspx)します。
