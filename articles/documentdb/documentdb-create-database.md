<properties 
    ...
    description="JSON の NoSQL ドキュメント データベースである Azure DocumentDB 用のオンライン サービス ポータルを使用して、データベースを作成する方法について説明します。 無料試用版を今すぐ入手してください。" 
...
2.   **DocumentDB アカウント** ブレードで、DocumentDB NoSQL を追加するアカウントのデータベースを選択します。 表示されているすべてのアカウントを取得していない場合は、する必要があります [DocumentDB アカウントの作成](documentdb-create-account.md)します。

3.  **DocumentDB アカウント** ブレードで、をクリックして **データベースの追加**します。

4.  **データベースの追加** ブレードで、新しいデータベースの ID を入力します。 名前が検証されると、緑色のチェック マークが表示されます、 **ID** ボックス。

5. クリックして **OK** 新しいデータベースを作成するには、画面の下部にあります。 

7. 新しいデータベースに表示されるよう、 **データベース** レンズ、 **DocumentDB アカウント** ブレードです。
