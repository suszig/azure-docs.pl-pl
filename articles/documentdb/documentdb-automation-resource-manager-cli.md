...
- [CLI を使用して DocumentDB アカウントを作成します。](#quick-create-documentdb-account)
- [ARM テンプレートを使用して DocumentDB アカウントを作成します。](#deploy-documentdb-from-a-template)
...
コマンド プロンプトで次のように入力します。 `azure--バージョン` を 0.9.11 のバージョンが既にインストールされているかどうかを確認またはそれ以降。

    azure --version
    0.9.11 (node: 0.12.7)

バージョンが 0.9.11 か、後で、いずれかにする必要があります [Azure CLI をインストール](../xplat-cli-install.md) またはのネイティブ インストーラーを使用して、更新、または **npm** 」と入力して `npm-g azure cli を更新する` を更新または `npm-g の azure cli をインストールする` をインストールします。
...
> [AZURE.NOTE] Azure アカウントを持っていない場合、別の種類のアカウントが必要であることを示すエラー メッセージが表示されます。 現在の Azure アカウントから 1 つを作成するを参照してください。 [Azure Active Directory の職場または学校の id を作成する.](../resource-group-create-work-id-from-personal.md)します。
...
」と入力して、既定のコマンド セットに切り替えることができます `azure config モード asm`します。
...
新しいリソース グループを作成するには、作成する新しいリソース グループの名前とリソース グループを作成するリージョンを指定します。
 ...
 - `< resourcegroupname >` 英数字、ピリオド、アンダー スコアのみ使用できます、'-' 記号とかっこを入力し、ピリオドで終わることはできません。
 - `< resourcegrouplocation >` DocumentDB の一般的に使用できる地域のいずれかを指定する必要があります。現在の地域の一覧が提供される、 [Azure リージョン ページ](https://azure.microsoft.com/regions/#services)します。
...
> [AZURE.TIP] Azure PowerShell または Windows PowerShell でこのコマンドを実行する場合、予想外のコマンドに関するエラーを受け取ります。 代わりに、Windows PowerShell コマンド プロンプトでこのコマンドを実行します。 
 ...
 - `< resourcegroupname >` 英数字、ピリオド、アンダー スコアのみ使用できます、'-' 記号とかっこを入力し、ピリオドで終わることはできません。
 - `< databaseaccountname >` 英小文字、数字にのみ使用できます、'-' 文字、および 3 ~ 50 文字である必要があります。
 - `< databaseaccountlocation >` DocumentDB の一般的に使用できる地域のいずれかを指定する必要があります。現在の地域の一覧が提供される、 [Azure リージョン ページ](https://azure.microsoft.com/regions/#services)します。
...
コマンドが終了すると、アカウントは数分間「**作成中**」の状態になります。その後、「**オンライン**」の状態に変化し、使用する準備ができます。 内のアカウントの状態を確認することができます、 [Azure ポータル](https://portal.azure.com), の **DocumentDB アカウント** ブレードです。
...
Azuredeploy.parameters.json ファイルを使用し、ファイルを保存するデータベースの名前に"samplearmacct"の値を更新します。 `< databaseAccountName >` 英小文字、数字にのみ使用できます、'-' 文字、および 3 ~ 50 文字である必要があります。
...
パラメーター ファイルを使用するには:
 ...
 - `< PathToTemplate >` は手順 1. で作成した azuredeploy.json ファイルのパスです。
 - `< PathToParameterFile >` 手順 1. で作成された azuredeploy.parameters.json ファイルへのパスです。
 - `< resourcegroupname >` DocumentDB データベース アカウントを追加するには、既存のリソース グループの名前を指定します。
 - `< deploymentname >` 展開のオプションの名前を指定します。
...
ようなエラーが発生した場合 `デプロイのプロビジョニング状態が正常に実行されなかった` いくつかがある場合、リソース グループまたはデータベース アカウントを作成するときにトラブルシューティングのオプションです。
> [AZURE.NOTE] データベース アカウント名に正しくない文字を指定するか、DocumentDB を使用できない場所を指定すると、デプロイメント エラーが発生します。 データベース アカウント名に使用できる文字は、英小文字、数字、「-」のみです。文字数は 3 ～ 50 文字にする必要があります。 すべての有効なデータベース アカウントの場所が一覧表示、 [Azure リージョン ページ](https://azure.microsoft.com/regions/#services)します。

- 出力には、次が含まれている場合 `C:\Users\wendy\.azure\azure.err にエラー情報が記録されて`, 、azure.err ファイルにエラー情報を確認します。
...
- Azure ポータルに明記されている [Azure ポータルを使用して DocumentDB データベースを作成する](documentdb-create-database.md)します。
...
-   [DocumentDB リソース モデルと概念](documentdb-resources.md)
