...
- [相関 ID のエラー](#correlation-id-errors)
- [携帯電話紛失または盗まれた場合](#i-have-lost-my-phone-or-it-was-stolen?)
- [電話番号を変更します。](#i-want-to-change-my-phone-number)
- [新しい電話番号があり、電話番号を変更する必要があります。](#i-have-a-new-phone-and-need-to-change-my-phone-number)
- [携帯電話でコードが受信されません。](#i-am-not-receiving-a-code-on-my-phone)
- [アプリ パスワードが機能していません。](#app-passwords-are-not-working)
- [古いデバイスからの Azure Authenticator をクリーンアップして新しいものに移動する方法](#how-do-i-clean-up-azure-authenticator-from-my-old-device-and-move-to-a-new-one)
- [問題に対する回答が見つかりませんでした。](#i-didn't-find-an-answer-to-my-problem)

## 関連付け ID のエラー

トラブルシューティングの手順を試みて、実行中に問題が発生する場合に質問を投稿することができます、 [Azure AD フォーラム](https://social.msdn.microsoft.com/forums/azure/home?forum=WindowsAzureAD), 、[Microsoft サポート技術情報 (KB) を検索](https://www.microsoft.com/en-us/Search/result.aspx?q=azure%20active%20directory%20connect&form=mssupport) または [サポートにお問い合わせ](https://support.microsoft.com/en-us) とが迅速に問題を見ていきましょう。
...
![関連付け ID](./media/multi-factor-authentication-end-user-manage/correlation.png)
 ...
 - **ユーザーに関する情報** – そのユーザーはフェデレーション ユーザー、パスワード ハッシュ同期されたユーザー、クラウド ユーザーでしたか。 そのユーザーには、Azure AD Premium、Enterprise Mobility、または Azure AD Basic のライセンスが割り当てられていましたか。 Office 365 を使用して、ユーザーか。 など
...
多要素認証の使用方法に応じて、電話番号などの設定を変更できる場所がいくつかあります。 下の表からご使用の状況に最も近いものを選択します。
 ...
 多要素認証の使用方法| 説明
:------------- | :------------- |
 [Office 365 で使用します。](#changing-your-settings-with-office-365)| この場合には、Office 365 ポータルを使用して設定を変更します。
 ...
 [Microsoft Azure で使用します。](#changing-your-settings-with-microsoft-azure)| この場合には、Azure ポータルを使用して設定を変更します。
...
最初に、アプリ パスワードが正しく入力されていることを確認します。 これがまだ動作しない場合に、サインインしてくださいと [新しいアプリ パスワードを作成する](multi-factor-authentication-end-user-app-passwords.md)します。 これでうまくいかない場合は、管理者に連絡してもらう [、既存のアプリ パスワードを削除](multi-factor-authentication-manage-users-and-devices.md#delete-users-existing-app-passwords) 新しいものを作成して、それを使用します。

## 古いデバイスから Azure Authenticator をクリーンアップして新しいデバイスに移行する場合

デバイスからアプリをアンインストールしても、デバイスを再フラッシュしても、アクティブ化はバック エンドで削除されません。 」の手順を使用する必要があります [を新しいデバイスに移動します](multi-factor-authentication-azure-authenticator.md#how-to-move-to-the-new-azure-authenticator-app)。.
