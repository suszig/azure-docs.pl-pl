...
1. AD FS フェデレーション サーバーに Azure Multi-Factor Authentication Server をダウンロードしてインストールします。 Azure Multi-factor Authentication server をインストールする方法について、次を参照してください [Azure Multi-factor Authentication Server の概要。](multi-factor-authentication-get-started-server.md)
...
オプション 1 - Web サービス SDK をユーザー名とパスワードを使って構成します。|<ol><li>WebServiceSdkUsername を PhoneFactor Admins セキュリティ グループのメンバーであるアカウントに設定します。  最初の起動時にドメインに参加しているマシンになるように VM をプロビジョニングするには、 <domain>\ < ユーザー名 > の形式です。<li>WebServiceSdkPassword を該当するアカウントのパスワードに設定します。</li></ol>
オプション 2 - Web Service SDK をクライアントの証明書を使って構成します。|<ol><li>Web サービス SDK を実行しているサーバーの証明機関からクライアント証明書を取得します。</li><li>クライアント証明書を Web サービス SDK を実行しているサーバーのローカル コンピューターの個人証明書ストアにインポートします。  注: 証明機関の公開証明書が信頼されたルート証明書を確認します。</li><li>クライアント証明書の公開および秘密キーを .pfx ファイルにエクスポートします。</li><li>Base 64 形式の公開キーを .cer ファイルにエクスポートします。</li><li>サーバー マネージャーでは、Web サーバー (IIS) \Web Server\Security\Client 証明書マッピング認証機能がインストールされていることを確認します。</li><li>インストールされていない場合は、この機能を追加するには、追加の役割と機能を選択します。</li><li>IIS マネージャーでは、Web サービス SDK 仮想ディレクトリが存在する web サイトの構成エディターをダブルクリックします。  注: この非常に仮想ディレクトリ レベルではなく、web サイトのレベルで行う必要があります。</li><li>System.webServer/security/authentication/iisClientCertificateMappingAuthentication セクションに移動します。</li><li>Enabled を true に設定します。</li><li>OneToOneCertificateMappingsEnabled を true に設定します。</li><li>クリックして、... onetoonemappings の隣の横にあるボタンをクリックします。</li><li>[追加] リンクをクリックします。</li><li>前にエクスポートする base-64 .cer ファイルを開きます。  -----BEGIN CERTIFICATE-----、-----END CERTIFICATE-----、および改行を削除します。  結果の文字列をコピーします。</li><li>前の手順でコピーした文字列に証明書を設定します。</li><li>Enabled を true に設定します。</li><li>UserName を PhoneFactor Admins セキュリティ グループのメンバーであるアカウントに設定します。  最初の起動時にドメインに参加しているマシンになるように VM をプロビジョニングするには、 <domain>\ < ユーザー名 > の形式です。</li><li>パスワードを適切なアカウントのパスワードに設定します。</li><li>コレクション エディターを閉じます。</li><li>適用] リンクをクリックします。</li><li>Web サービス SDK 仮想ディレクトリに移動します。</li><li>認証をダブルクリックします。</li><li>ASP.NET の偽装と基本認証が有効になっている他のすべての項目が無効になっていることを確認します。</li><li>再度 Web サービス SDK 仮想ディレクトリに移動します。</li><li>SSL 設定をダブルクリックします。</li><li>クライアント証明書を Accept に設定し、[適用] をクリックします。</li><li>前に、AD FS アダプターを実行するサーバーにエクスポートする .pfx ファイルをコピーします。</li><li>ローカル コンピューターの個人証明書ストアに .pfx ファイルをインポートします。</li><li>右クリック メニューから秘密キーの管理を選択し、Active Directory フェデレーション サービスのサービス アカウントに読み取りアクセス権をログオン時に付与します。</li><li>クライアント証明書を開き、[詳細] タブから拇印をコピーします。</li><li>MultiFactorAuthenticationAdfsAdapter.config ファイルで、前の手順でコピーした文字列を WebServiceSdkCertificateThumbprint に設定します。</li></ol>
-Configurationfilepath Register-multifactorauthenticationadfsadapter.ps1 スクリプトを編集します。 <path> コマンド Register-adfsauthenticationprovider の最後に <path> MultiFactorAuthenticationAdfsAdapter.config ファイルへの完全パスです。|
