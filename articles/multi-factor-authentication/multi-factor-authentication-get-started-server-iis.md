...
4. ユーザー名を検出するには、パスワード、およびドメイン変数は自動的には、[web サイト] ダイアログ ボックス内で、ログイン URL] (https://localhost/contoso/auth/login.aspx など) を入力し、[ok] をクリックします。
...
11.  [詳細設定] をクリックしてください. 詳細設定を確認、カスタム拒否ページファイルを選択する機能などの cookie を使用して、一定の web サイトに成功した認証をキャッシュし、Windows ドメイン、LDAP ディレクトリ、または RADIUS サーバーに対してプライマリ資格情報を認証するかどうかを選択する] ボタンをクリックします。 完了したら、[OK] ボタンをクリックして、[フォームベースの Web サイトの追加] ダイアログ ボックスに戻ります。 詳細設定の詳細については、ヘルプ ファイルを参照してください。
12. [OK] ボタンをクリックします。
...
3. [追加] をクリックしてください. をクリックしてください。
4. ベース URL の追加] ダイアログ ボックスで、ベース URL] フィールドに実行される (http://localhost/owa など) の HTTP 認証が web サイトの URL を入力し、(省略可能) アプリケーション名を入力します。 アプリケーション名は Azure Multi-factor Authentication レポートに表示され、SMS またはモバイル アプリの認証メッセージにも表示される場合があります。
...
8. [OK] ボタンをクリックします。
9. 参照してください、 [Azure 多要素認証サーバー用の IIS を有効にするプラグインを](#enable-iis-plug-ins-for-azure-multi-factor-authentication-server) 直接 IIS 認証の構成を完了する以下のセクションです。
