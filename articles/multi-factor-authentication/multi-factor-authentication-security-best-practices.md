...
ユーザーの MFA を有効にすると、ユーザー アカウントは 3 つの主要な状態 (無効、有効、強制) のいずれかになります。 
...
- アーティクルで使用できるユーザーの通知テンプレートを使用して [クラウドでの Azure Multi-factor Authentication の概要](multi-factor-authentication-get-started-cloud.md) MFA の導入に関して、ユーザーに電子メールを送信します。
...
ただし、一部のシナリオでは MFA を一時的に無効にすることが必要になる場合があります。 これらのシナリオへの対処方法を理解するには、次のガイドラインに従ってください。
...
*既定でインストールされていない **インストールされていますが、既定で有効になっていません
...
- IIS 6 以降が必要です。
- ASP.NET v2.0.507207 をインストールし、登録する必要があります。
...
:------------- | :------------- |
 ...
 [リモート デスクトップ ゲートウェイと Azure Multi-factor Authentication Server が RADIUS を使用します。](multi-factor-authentication-get-started-server-rdg.md)| RADIUS を使用したリモート デスクトップ ゲートウェイによる Azure MFA Server のセットアップと構成に関する情報です。
 ...
 [Azure Multi-factor Authentication Server モバイル アプリ Web サービスを展開します。](multi-factor-authentication-get-started-server-webservice.md)| Azure MFA Server Web サービスのセットアップと構成に関する情報です。
 [Azure 多要素認証での高度な VPN 構成](multi-factor-authentication-advanced-vpn-configurations.md)| Cisco ASA、Citrix Netscaler、Juniper/Pulse Secure などの VPN アプライアンスの LDAP または RADIUS を使用した構成に関する情報です。
...
- [Azure 多要素認証でのレポート](multi-factor-authentication-manage-reports.md)
- [Azure 多要素認証のセットアップ エクスペリエンス](multi-factor-authentication-end-user-first-time.md)
- [Azure 多要素認証に関する FAQ](multi-factor-authentication-faq.md)
