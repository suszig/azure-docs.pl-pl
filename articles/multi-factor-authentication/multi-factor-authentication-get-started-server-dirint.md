...
| Funkcja | Opis |
| ------- | ----------- |
| Użyj usługi Active Directory | Wybierz opcję Użyj usługi Active Directory, aby użyć tej usługi do importowania i synchronizacji.  Jest to ustawienie domyślne. <br>Uwaga: aby integracja usługi Active Directory przebiegła poprawnie, komputer musi być przyłączony do domeny, a użytkownik musi być zalogowany do konta domeny. |
| Uwzględnij domeny zaufane | Zaznacz pole wyboru opcji Uwzględnij domeny zaufane, jeśli chcesz, aby agent podejmował próbę nawiązania połączenia z domenami ustawionymi jako zaufane dla bieżącej domeny, innej domeny w lesie lub domen należących do zaufania lasu.  Jeśli nie zachodzi importowanie ani synchronizowanie użytkowników z żadnej z zaufanych domen, usuń zaznaczenie pola wyboru, aby poprawić wydajność.  Pole jest domyślnie zaznaczone. |
...
| Użyj zapytań dotyczących zakresu atrybutów | Określa, czy należy użyć zapytań dotyczących zakresu atrybutów.  Zapytania dotyczące zakresu atrybutów umożliwiają wydajne wyszukiwanie w katalogu obejmujące kwalifikację rekordów na podstawie wpisów w atrybucie innego rekordu.  Serwer usługi Azure Multi-Factor Authentication wykorzystuje zapytania dotyczące zakresu atrybutów do wydajnego przesyłania zapytań dotyczących użytkowników, którzy są członkami grupy zabezpieczeń.   <br>Uwaga: w określonych przypadkach zapytania dotyczące zakresu atrybutów są obsługiwane, nie powinny jednak być używane.  Na przykład w usłudze Active Directory mogą wystąpić problemy z zapytaniami dotyczącymi zakresu atrybutów, jeśli grupa zabezpieczeń zawiera członków z więcej niż jednej domeny.  W takim przypadku pole wyboru nie powinno być zaznaczone. |

W poniższej tabeli opisano ustawienia konfiguracji LDAP.

| Funkcja | Opis |
| ------- | ----------- |
| Serwer | Wprowadź nazwę hosta lub adres IP serwera z uruchomionym katalogiem LDAP.  Serwer zapasowy można także określić osobno, oddzielając go średnikiem. <br>Uwaga: jeśli typ powiązania to SSL, zwykle wymagana jest w pełni kwalifikowana nazwa hosta. |
...
| Typ powiązania — zapytania | Wybierz odpowiedni typ powiązania do użycia podczas tworzenia powiązań w celu przeprowadzania wyszukiwań w katalogu LDAP.  Ten typ jest wykorzystywany do importowania, synchronizacji oraz rozpoznania nazw użytkowników. <br><br>  Anonimowe — pozwala utworzyć anonimowe powiązanie.  Powiązana nazwa wyróżniająca oraz powiązane hasło nie będą używane.  Użycie tej metody jest możliwe tylko, jeśli katalog LDAP umożliwia tworzenie anonimowych powiązań i jeśli uprawnienia zezwalają na przesyłanie zapytań dotyczących odpowiednich rekordów i atrybutów.  <br><br> Proste — powiązana nazwa wyróżniająca oraz powiązane hasło zostaną przekazane jako zwykły tekst w celu utworzenia powiązania z katalogiem LDAP.  Ta opcja powinna służyć wyłącznie do celów testowych — należy użyć jej do sprawdzenia, czy można połączyć się z serwerem i czy konto powiązania może uzyskać odpowiedni dostęp.  Zamiast korzystania z tej opcji zaleca się użycie protokołu SSL po zainstalowaniu odpowiedniego certyfikatu.  <br><br> SSL — powiązana nazwa wyróżniająca oraz powiązane hasło zostaną zaszyfrowane z użyciem protokołu SSL w celu utworzenia powiązania z katalogiem LDAP.  Ta procedura wymaga lokalnej instalacji certyfikatu ustawionego jako zaufany dla katalogu LDAP.  <br><br> Windows — powiązana nazwa użytkownika i powiązane hasło zostaną użyte do bezpiecznego połączenia z kontrolerem domeny usługi Active Directory lub katalogiem ADAM.  Jeśli pole Powiązana nazwa użytkownika pozostanie puste, do powiązania zostanie użyte konto, do którego zalogował się użytkownik. |
...
| Powiązane hasło | Wprowadź powiązane hasło dla powiązanej nazwy wyróżniającej lub nazwy użytkownika używanej w celu utworzenia powiązania z katalogiem LDAP.  Aby można było skonfigurować hasło usługi Multi-Factor Auth Server AdSync Service, synchronizacja musi być włączona, a usługa musi być uruchomiona na komputerze lokalnym.  Hasło zostanie zapisane w obszarze Nazwy użytkowników i hasła przechowywane w systemie Windows na koncie, w ramach którego jest uruchomiona usługa Multi-Factor Auth Server AdSync Service.  Hasło zostanie także zapisane na koncie, w ramach którego jest uruchomiony interfejs użytkownika usługi Multi-Factor Auth Server i na koncie, w ramach którego jest uruchomiona usługa Multi-Factor Auth Server Service.  <br><br> Uwaga: jako że hasło jest przechowywane wyłącznie w obszarze Nazwy użytkowników i hasła przechowywane w systemie Windows na lokalnym serwerze, ten krok należy wykonać w przypadku każdego serwera usługi Multi-Factor Auth wymagającego dostępu do hasła. |
...
Usługa Azure Multi-Factor Authentication zapewnia dostęp do następujących 3 opcji.

- **Filtr kontenera** — pozwala określić kryteria filtru wykorzystywane do kwalifikowania rekordów kontenera podczas przeprowadzania wyszukiwania w katalogu.  W przypadku katalogów Active Directory i ADAM zwykle wykorzystywane są kryteria filtrowania (|(objectClass=organizationalUnit)(objectClass=container)).  W przypadku innych katalogów LDAP w zależności od schematu katalogu należy użyć kryteriów filtrowania służących do kwalifikowania poszczególnych typów obiektu kontenera.  <br>Uwaga: jeśli pole pozostanie puste, domyślnie użyte zostaną kryteria filtrowania ((objectClass=organizationalUnit)(objectClass=container)).
...
| Funkcja | Opis |
| ------- | ----------- |
| Unikatowy identyfikator | Wprowadź nazwę atrybutu, która pełni rolę unikatowego identyfikatora rekordów kontenera, grupy zabezpieczeń i użytkowników.  W katalogu Active Directory jest to zazwyczaj wartość elementu objectGUID.  W innych wdrożeniach protokołu LDAP może to być wartość entryUUID lub podobna.  Wartość domyślna to objectGUID. |
| Przyciski ... (Wybierz atrybut) | Obok każdego pola atrybutu znajduje się przycisk „...” pozwalający wyświetlić okno dialogowe Wybierz atrybut, umożliwiające wybranie atrybutu z listy. <br><br>Okno dialogowe Wybierz atrybut.<br><br>Uwaga: atrybuty mogą być wprowadzane ręcznie i nie muszą odpowiadać atrybutom na liście. |
| Typ unikatowego identyfikatora | Wybierz typ atrybutu unikatowego identyfikatora.  W katalogu Active Directory atrybut objectGUID ma typ GUID.  W innych wdrożeniach protokołu LDAP może on być powiązany z typem ASCII Byte Array (tablica bajtowa znaków ASCII) lub String (ciąg).  Wartość domyślna to GUID. <br><br>Uwaga: poprawne ustawienie tej opcji ma duże znaczenie, ponieważ odwołania do elementów przeznaczonych do synchronizacji opierają się na ich unikatowych identyfikatorach, a typ unikatowego identyfikatora jest używany do bezpośredniego wyszukiwania obiektu w katalogu.  Ustawienie typu ciągu w przypadku, gdy wartość jest przechowywana w katalogu jako tablica bajtowa znaków ASCII, uniemożliwi prawidłową synchronizację. |
| Nazwa wyróżniająca | Wprowadź nazwę atrybutu, który zawiera nazwę wyróżniającą każdego z rekordów.  W katalogu Active Directory jest to zazwyczaj atrybut distinguishedName.  W innych wdrożeniach protokołu LDAP może to być atrybut entryDN lub podobny.  Wartość domyślna to distinguishedName. <br><br>Uwaga: jeśli nie istnieje atrybut zawierający tylko nazwę wyróżniającą, może zostać użyty atrybut adspath.  Część „LDAP://<server>/” ścieżki zostanie automatycznie usunięta, co pozwoli uzyskać samą nazwę wyróżniającą obiektu. |
| Nazwa kontenera | Wprowadź nazwę atrybutu, który zawiera nazwę w rekordzie kontenera.  Wartość tego atrybutu pojawi się w hierarchii kontenera podczas importowania pozycji z katalogu Active Directory lub dodawania elementów do synchronizacji.  Wartość domyślna to name. <br><br>Uwaga: jeśli różne kontenery używają różnych atrybutów w odniesieniu do swoich nazw, można wskazać wiele atrybutów nazw kontenera, rozdzielając je średnikami.  Pierwszy atrybut nazwy kontenera znaleziony w obiekcie kontenera będzie pełnić rolę jego nazwy wyświetlanej. |
| Nazwa grupy zabezpieczeń | Wprowadź nazwę atrybutu, który zawiera nazwę w rekordzie grupy zabezpieczeń.  Wartość tego atrybutu pojawi się na liście grupy zabezpieczeń podczas importowania pozycji z katalogu Active Directory lub dodawania elementów do synchronizacji.  Wartość domyślna to name. |
| Użytkownicy | Następujące atrybuty są używane do wyszukiwania, wyświetlania, importowania i synchronizowania pochodzących z katalogu informacji o użytkownikach. |
| Nazwa użytkownika | Wprowadź nazwę atrybutu, który zawiera nazwę użytkownika w rekordzie użytkownika.  Wartość tego atrybutu zostanie użyta jako nazwa użytkownika serwera usługi Multi-Factor Auth.  Drugi atrybut może zostać określony jako atrybut zapasowy względem pierwszego.  Drugi atrybut zostanie użyty tylko, jeśli pierwszy atrybut nie zawiera wartości użytkownika.  Wartości domyślne to userPrincipalName i sAMAccountName. |
...
| Adres e-mail | Wprowadź nazwę atrybutu, który zawiera adres e-mail w rekordzie użytkownika.  Adres e-mail zostanie użyty do przesłania użytkownikowi powitalnej wiadomości e-mail oraz w celu przesyłania wiadomości dotyczących aktualizacji.  Wartość domyślna to mail. |
...
| Język połączenia głosowego | Wprowadź nazwę atrybutu, który zawiera skróconą nazwę języka używanego na potrzeby połączeń głosowych użytkownika. |
| Język tekstu wiadomości SMS | Wprowadź nazwę atrybutu, który zawiera skróconą nazwę języka używanego na potrzeby wiadomości SMS użytkownika. |
| Język aplikacji telefonu | Wprowadź nazwę atrybutu, który zawiera skróconą nazwę języka używanego na potrzeby wiadomości tekstowych aplikacji telefonu użytkownika. |
...
| Telefony | Następujące atrybuty są używane do importowania lub synchronizacji numerów telefonów użytkownika.  Jeśli nie określono nazwy atrybutu dla typu telefonu, typ telefonu nie będzie dostępny podczas importowania z usługi Active Directory ani dodawania elementów do synchronizacji. |
| Firmowy | Wprowadź nazwę atrybutu, który zawiera firmowy numer telefonu w rekordzie użytkownika.  Wartość domyślna to telephoneNumber. |
| Domowy | Wprowadź nazwę atrybutu, który zawiera domowy numer telefonu w rekordzie użytkownika.  Wartość domyślna to homePhone. |
| Pager | Wprowadź nazwę atrybutu, który zawiera numer pagera w rekordzie użytkownika.  Wartość domyślna to pager. |
| Komórkowy | Wprowadź nazwę atrybutu, który zawiera numer telefonu komórkowego w rekordzie użytkownika.  Wartość domyślna to mobile. |
...
| Niestandardowy | Wprowadź nazwę atrybutu, który zawiera niestandardowy numer telefonu w |
|  | rekordzie użytkownika.  Pole jest domyślnie puste. |
| Wewnętrzny | Wprowadź nazwę atrybutu, który zawiera numer wewnętrzny w rekordzie użytkownika.  Wartość pola numeru wewnętrznego jest wykorzystywana wyłącznie jako rozszerzenie podstawowego numeru telefonu.  Pole jest domyślnie puste. <br><br>Uwaga: jeśli nie określono atrybutu Wewnętrzny, numer wewnętrzny można uwzględnić w atrybucie numeru telefonu.  Numer wewnętrzny powinien zostać poprzedzony znakiem „x”, co umożliwi jego analizę.  Na przykład wartość 555-123-4567 x890 zostanie zinterpretowana jako numer telefonu 555-123-4567 i numer wewnętrzny 890. |
...
Aby edytować atrybuty, wystarczy kliknąć przycisk edycji na karcie Atrybuty.  Spowoduje to wyświetlenie okna umożliwiającego edycję atrybutów.
...
Usługa Multi-Factor Auth ADSync wykorzystuje rozszerzenie serwera DirSync LDAP dostarczone przez firmę Microsoft w celu wydajnego sondowania pod kątem zmian.  Ten kontrolny obiekt wywołujący narzędzia DirSync musi mieć uprawnienie „directory get changes” i rozszerzone uprawnienie kontroli dostępu DS-Replication-Get-Changes.  Domyślnie te uprawnienia są przypisane do kont Administrator i LocalSystem na kontrolerach domeny.  Usługa Multi-Factor Auth AdSync jest domyślnie skonfigurowana pod kątem uruchomienia w powiązaniu z kontem LocalSystem.  Dlatego też najprościej jest uruchomić usługę na kontrolerze domeny.  Usługa może działać jako konto z bardziej ograniczonymi uprawnieniami, jeśli zostanie skonfigurowania pod kątem każdorazowej pełnej synchronizacji.  Jest to mniej wydajne rozwiązanie, ale wymaga mniej uprawnień konta.

Jeśli zostanie ona skonfigurowana pod kątem użycia protokołu LDAP, a katalog LDAP obsługuje kontroler DirSync, sondowanie w poszukiwaniu zmian w zakresie użytkowników i grupy zabezpieczeń będzie działać tak samo, jak w przypadku katalogu Active Directory.  Jeśli katalog LDAP nie obsługuje kontrolera DirSync, pełna synchronizacja będzie mieć miejsce podczas każdego cyklu.

![Synchronizacja](./media/multi-factor-authentication-get-started-server-dirint/dirint5.png)

Dodatkowe informacje na temat poszczególnych ustawień z karty Synchronizacja można znaleźć w poniższej tabeli.

| Funkcja | Opis |
| ------- | ----------- |
| Włącz synchronizację z usługą Active Directory | W przypadku zaznaczenia tej opcji usługa Multi-Factor Auth Server zostanie uruchomiona w celu okresowego sondowania pod kątem zmian w usłudze Active Directory. <br><br>Uwaga: zanim usługa Multi-Factor Auth Server zacznie przetwarzać zmiany, należy dodać co najmniej jeden element synchronizacji i wybrać opcję Synchronizuj teraz. |
...
>[AZURE.TIP]Po usunięciu elementów synchronizacji należy przeprowadzić pełną synchronizację.  Po określeniu kolejności elementów synchronizacji należy przeprowadzić pełną synchronizację.  Kliknij przycisk Synchronizuj teraz, aby przeprowadzić pełną synchronizację.
