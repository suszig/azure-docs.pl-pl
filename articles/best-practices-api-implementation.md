...
ASP.NET Web API を使用して実装されたサービスでは、各要求は _controller_ クラスのメソッドにルーティングされます。 Web API フレームワークでは、ルーティングを実装する際の主な選択肢として、"規約ベース" のルーティングと "属性ベース" のルーティングの 2 つがあります。____ Web API で要求をルーティングする最良の方法を決定するときには、以下の点を考慮してください。
...
- **クライアント アプリケーションに応答を送信するときは HTTP プロトコルに従う**
    ...
    これらの機能を提供するを使用して、独自の HTTP 応答メッセージを返す、 `IHttpActionResult` インターフェイスです。 次のコード例に示すように、この方法では、HTTP 状態コードと応答メッセージのヘッダーだけでなく、応答メッセージの本文のデータの形式も細かく制御できます。 このバージョンの `CreateNewCustomer` メソッドは、クライアントは、HTTP プロトコルへの期待により厳密に準拠しています。  `Created` のメソッド、 `ApiController` クラスは、指定されたデータから応答メッセージを作成し、結果に、Location ヘッダーを追加します。
...
- **HATEOAS スタイルのナビゲーションとリソースの検出をサポートするリンクを提供する**
    ...
    この例では、顧客データがによって表される、 `顧客` クラスの次のコード スニペットに示すようにします。 HATEOAS リンク内に保持されて、 `リンク` コレクション プロパティ。
    ...
    HTTP GET 操作は、ストレージおよび構成体から顧客データを取得、 `顧客` オブジェクトし、作成、 `リンク` コレクションです。 結果は、JSON 応答メッセージとして書式設定されます。 各リンクは次のフィールドで構成されます。

    - 返されるオブジェクトとリンクで示されているオブジェクトの関係。 リンクは、オブジェクト自体への参照をここで"self"を示します (のような `この` 多くのオブジェクト指向言語でのポインター)、"orders"は関連する注文情報を含むコレクションの名前。

    - ハイパーリンク (`HRef`) の URI の形式でのリンクで示されているオブジェクト。

    - HTTP 要求の種類 (`アクション`) この URI に送信できます。

    - データの形式 (`LinkedResourceMIMETypes`) http 要求またはが返される応答には、要求の種類によっては、提供することです。
...
- **一貫した方法で例外を処理し、エラーに関する情報をログに記録する**
    ...
    一貫した方法で例外を処理するには、Web API 全体にわたるグローバルなエラー処理方法を実装することを検討します。 コント ローラーにはハンドルされない例外がスローしたときに実行される例外フィルターを作成することで、この一部を実現できます、 `HttpResponseException` 例外です。 この方法については説明、 [ASP.NET Web API で例外が処理](http://www.asp.net/web-api/overview/error-handling/exception-handling) 、Microsoft web サイトのページです。
...
- **クライアント側のキャッシュをサポートする**
    ...
    このコードでは、使用するカスタム `IHttpActionResult` という名前のクラス `OkResultWithCaching`します。 このクラスを使用して、コントローラーはキャッシュ ヘッダーの内容を設定できます。
...
- **ETag を提供してクエリ処理を最適化する**
    ...
    次のコード、 `FindOrderByID` メソッドは、[なし]-If-match ヘッダーをサポートするために拡張します。 If-None-Match ヘッダーを省略すると、指定した注文が常に取得されることに注意してください。
    ...
    この例には、追加のカスタムが組み込まれています。 `IHttpActionResult` という名前のクラス `EmptyResultWithCaching`します。 このクラスは、単にラップするラッパーとして機能する `HttpResponseMessage` 応答本文が含まれていないオブジェクト。
...
- **ラージ オブジェクトを伴う要求と応答を最適化する**
    ...
    この例では `ConnectBlobToContainer` Azure Blob ストレージには、指定されたコンテナー (名前は表示されません) に接続するヘルパー メソッドです。 `BlobExists` は指定した名前の blob が blob ストレージ コンテナーに存在するかどうかを示すブール値を返す別のヘルパー メソッドです。

    各製品には、BLOB ストレージに保持されている独自の画像があります。  `FileDownloadResult` クラスは、カスタム `IHttpActionResult` を使用するクラス、 `PushStreamContent` オブジェクトを適切な blob から画像データを読み取り、応答メッセージの内容として非同期に送信します。
    ...
    クライアントがラージ オブジェクトを含む新しいリソースを POST する必要がある場合は、アップロード操作にストリーミングを適用することもできます。 次の例では、Post メソッドを `ProductImages` コント ローラーです。 このメソッドにより、クライアントは新しい製品画像をアップロードできます。
    ...
    このコードを使用して別のカスタム `IHttpActionResult` と呼ばれるクラス `FileUploadResult`します。 このクラスには、データを非同期にアップロードするためのロジックが含まれています。
...
- **クライアント アプリケーションで不要な Continue 状態メッセージを送信しないようにする**
    ...
    .NET Framework を使用してクライアント アプリケーションを構築している場合は、すべての POST メッセージと PUT メッセージで、Expect: 100-Continue ヘッダーを既定で含むメッセージが最初に送信されます。 サーバー側と同様に、このプロセスは .NET Framework によって透過的に処理されます。 ただし、このプロセスでは、要求のサイズが小さい場合でも、各 POST 要求と PUT 要求でサーバーへのラウンド トリップが 2 回発生することになります。 使用してこの機能を無効にできます、アプリケーションでは、大量のデータでの要求を送信しない場合、 `ServicePointManager` クラスを作成する `ServicePoint` クライアント アプリケーション内のオブジェクト。 A `ServicePoint` オブジェクトは、クライアントが行うサーバー上のリソースを識別する uri のスキームおよびホスト フラグメントに基づいて、サーバーへの接続を処理します。 設定できます、 `Expect100Continue` のプロパティ、 `ServicePoint` オブジェクトを false にします。 スキームおよびホスト フラグメントと一致する URI を使用してクライアントによって行われたすべての後続の POST および PUT 要求、 `ServicePoint` Expect なしオブジェクトで送信されます。 100-ヘッダーを続行します。 次のコードを構成する方法を示しています、 `ServicePoint` オブジェクトのスキームで Uri に送信されるすべての要求を構成する `http` とのホスト `www.contoso.com`します。
    ...
    静的なを設定することもできます。 `Expect100Continue` のプロパティ、 `ServicePointManager` クラスを今後作成されるすべてのこのプロパティの既定値を指定する `ServicePoint` オブジェクトです。 詳細については、次を参照してください。、 [ServicePoint クラス](https://msdn.microsoft.com/library/system.net.servicepoint.aspx) 、Microsoft web サイトのページです。
...
- すべてのルートをテストして、正しい操作が呼び出されていることを確認します。 HTTP 状態コード 405 (Method Not Allowed) は、ルートとそのルートにディスパッチできる HTTP メソッド (GET、POST、PUT、DELETE) の不一致を示している場合があるため、この状態コードが予期せず返されたときは特に注意してください。
    ...
    HTTP 要求をサポートしていないルートに要求を送信します。たとえば、POST 要求を特定のリソースに送信します (POST 要求の送信先はリソース コレクションに限られます)。 このような場合、状態コード 405 (Not Allowed) が唯一の有効な応答である必要があります。__
