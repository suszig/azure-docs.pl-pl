...
1. デプロイ プロジェクト内のノードを展開して、作成された内容を確認します。
    ...
    |ファイル名|説明|
    |---|---|
    |Deploy-AzureResourceGroup.ps1|Azure リソース マネージャーへの展開の PowerShell コマンドを呼び出す PowerShell スクリプト<br />**。注** この PowerShell スクリプトは、テンプレートをデプロイする Visual Studio によって使用されます。 このスクリプトに変更を加えると Visual Studio でのデプロイにも影響するため、注意が必要です。|
