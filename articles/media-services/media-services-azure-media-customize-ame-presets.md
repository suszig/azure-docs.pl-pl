...
オーバーレイを表示するタイミング、オーバーレイを表示する時間、およびイメージ/ビデオのオーバーレイでは画面にオーバーレイを表示する位置を指定することができます。 オーバーレイをフェードイン/フェードアウトすることもできます。 オーバーレイするオーディオ/ビデオ ファイルは、複数の資産にも 1 つの資産にも含めることができます。 オーバーレイは、エンコーダーに渡すプリセット XML で制御します。 プリセット スキーマの詳細については、Azure Media Encoder のスキーマを参照してください。 オーバーレイがで指定します <MediaFile> 要素の次のプリセット スニペットに示すようにします。
...
オーディオ オーバーレイは、サポートされているオーディオ ファイル形式のいずれかにすることができます。 サポートされているオーディオ ファイル形式の詳しい一覧については、Media Services Encoder でサポートされる形式を参照してください。 オーディオ オーバーレイもファイルで指定します <MediaFile> 要素の次のプリセット スニペットに示すようにします。
...
>[AZURE.NOTE]このスニペットでは、順番にわかりやすくするためには、各資産を読み込みます。 運用環境では、資産は一括で読み込まれます。 複数のアセットの一括アップロードする方法の詳細を参照してください。 [アセットの Media Services sdk for .NET の一括取り込み](media-services-dotnet-upload-files.md#ingest_in_bulk)します。
...
合成の制御は内で、 <MediaFile> 次のプリセットに示すように要素:
...
結合するには、各ビデオについて、 <Source> 要素を追加します <Sources> 要素。 各 <Source> 要素が含まれています、 <Clips> 要素。 各 <Clips> 要素は、1 つまたは複数含まれています。 <Clip> ビデオの長さを指定する要素は、開始/終了時間を指定することによって、出力アセットに結合します。 この <Source> 要素は、機能するアセットを参照します。 参照の形式は、結合するビデオが別々の資産と 1 つの資産のどちらに含まれているかよって異なります。 ビデオ全体を結合する場合は省略するだけで、 <Clips> 要素。 

###複数の資産からのビデオの結合

MediaFile 属性に、0 から始まるインデックスを使用する複数の資産からのビデオを結合するとき、 <Source> アセットを指定する要素、 <Source> 要素に対応します。 ゼロから始まるインデックスが指定されていない、 <Source> MediaFile 属性が指定されていない要素は、最初の入力アセットを参照します。 その他のすべて <Source> 要素は、ここで n は入力アセットの 0 から始まるインデックスを %n% 構文を使用して参照する入力アセットの 0 から始まるインデックスを指定する必要があります。 1 つ目の前の例 <Source> 要素は、最初の入力アセットを指定しますもう 1 つ。 <Source> 要素は、2 番目の入力アセットとなどを指定します。 参照する入力資産の順序などの要件はありません。
