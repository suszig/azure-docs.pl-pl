...
>[AZURE.IMPORTANT] Azure Media Services 用の Microsoft Hyperlapse では、無料のパブリック プレビューの状態です。  ジョブは、入力資産の最初の 10,000 フレームに制限されます。
...
> [AZURE.NOTE] このコードが動作の名前の「コンテキスト」にスコープ内に、CloudMediaContext を既にあります。  これに関する詳細についてを参照してください、 [コンテンツ管理の記事](media-services-manage-content.md)します。

> [AZURE.NOTE] 上記で説明したように、JSON または XML のいずれかのプリセットに準拠する構成にするのには、文字列引数"hyperConfig"が必要です。
