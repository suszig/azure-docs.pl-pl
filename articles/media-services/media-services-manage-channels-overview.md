...
Azure Media Services で、 **チャネル** ライブ ストリーミング コンテンツを処理するためのパイプラインを表します。 A **チャネル** は次の方法でライブ入力ストリームを受信します。

- 内部設置型のライブ エンコーダーがマルチビット レートを送信 **RTMP** または **スムーズ ストリーミング** (Fragmented MP4) をチャネルにします。 マルチ ビットレートのスムーズ ストリーミングを出力するライブ エンコーダーとして、Elemental、Envivio、Cisco を使用できます。  Adobe Flash Live、Telestream Wirecast、Tricaster トランスコーダーは、RTMP を出力するライブ エンコーダーです。 取り込んだストリームは、通過 **チャネル** 追加の処理なし。 ライブ エンコーダーは、シングル ビットレート ストリームも送信できますが、これはお勧めしません。 Media Services は、要求に応じて、ストリームを顧客に配信します。
...
- **マルチビットレートの RTMP** 
    ...
    ときに、 **RTMP** 取り込みストリーミング プロトコルが選択した場合、チャネルの 2 つのインジェスト (入力) エンドポイントが作成されます。 
    
    **プライマリ URL**: 完全修飾 URL のチャネルのプライマリ RTMP インジェスト エンドポイントを指定します。

    **セカンダリ URL** (省略可能): 完全修飾 URL のチャネルのセカンダリ RTMP インジェスト エンドポイントを指定します。 
...
取り込み URL は、チャネルの作成時に取得できます。 これらの Url を取得する、チャネルが持っていない内にある、 **を実行している** 状態です。 データをチャネルにプッシュする準備ができたら、チャネルにする必要があります、 **を実行している** 状態です。 チャネルがデータの取り込みを開始すると、プレビュー URL 経由でストリームをプレビューできます。
...
プレビュー URL は、チャネルの作成時に取得できます。 URL を取得する、チャネルが持っていない内にある、 **を実行している** 状態です。 
...
- **停止**します。 これは、チャネル作成後の初期状態です。 この状態で、チャネルのプロパティを更新できますが、ストリーミングは許可されていません。
- **開始**します。 チャネルを開始しています。 この状態の場合、更新やストリーミングはできません。 エラーが発生した場合は、チャネルは Stopped 状態に戻ります。
- **実行している**します。 チャネルは、ライブ ストリームを処理できます。
- **停止**します。 チャネルを停止しています。 この状態の場合、更新やストリーミングはできません。
- **削除する**です。 チャネルが削除されました。 この状態の場合、更新やストリーミングはできません。

次の表は、チャネルの状態と課金モードとの対応を示しています。 
 
チャネル state|ポータルの UI Indicators|課金されるか。
---|---|---|---
Starting|Starting|なし (遷移状態)
Running|準備完了 (実行中プログラムなし)<p>または<p>ストリーミング (実行中のプログラムを少なくとも 1 つ) |[はい]
Stopping|Stopping|なし (遷移状態)
Stopped|Stopped|なし
...
- ライブ エンコーダーを再構成するたびに呼び出す、 **リセット** 、チャネル上のメソッドです。 チャネルをリセットする前に、プログラムを停止する必要があります。 チャネルをリセットしたら、プログラムを再起動します。 
...
- チャネルやチャネルに関連付けられたプログラムの実行中は、入力プロトコルを変更できません。 別のプロトコルが必要な場合は、入力プロトコルごとに別のチャネルを作成します。 
- チャネルがの場合のみ課金されます、 **を実行している** 状態です。 詳細についてを参照してください [この](media-services-manage-channels-overview.md#states) セクションです。
...
選択 **ポータル**, 、**.NET**, 、**REST API** を作成し、チャネルとプログラムを管理する方法を確認します。
...
[Azure Media Services を使用したライブ ストリーミング イベントの配信](media-services-live-streaming-workflow.md)
