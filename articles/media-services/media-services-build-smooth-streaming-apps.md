...
Windows 8 用 Smooth Streaming Client SDK を使用すると、オンデマンドおよびライブ スムーズ ストリーミング コンテンツの再生が可能な Windows ストア アプリケーションを作成できます。 スムーズ ストリーミング コンテンツの基本再生機能に加えて、SDK では、Microsoft PlayReady Protection、品質レベル制限、ライブ DVR、オーディオ ストリーム切り替え、ステータス更新 (品質レベルの変化など) のリスニング、エラー イベントなどの豊富な機能が提供されます。 サポートされる機能の詳細については、次を参照してください。、 [リリース ノート](http://www.iis.net/learn/media/smooth-streaming/smooth-streaming-client-sdk-for-windows-8-release-notes)します。 詳細については、次を参照してください。 [Player Framework for Windows 8](http://playerframework.codeplex.com/)します。 
...
- Windows 8 32 ビットまたは 64 ビット。 すれば、 [Windows 8 Enterprise 評価版](http://msdn.microsoft.com/evalcenter/jj554510.aspx) MSDN からです。
- Visual Studio 2012 または Visual Studio Express 2012 (またはそれ以降のバージョン)。 試用版を入手できる [ここ](http://www.microsoft.com/visualstudio/11/downloads)します。
- [Microsoft Smooth Streaming Client SDK を Windows 8 用](http://visualstudiogallery.msdn.microsoft.com/04423d13-3b3e-4741-a01c-1ae29e84fea6?SRC=Homehttp://visualstudiogallery.msdn.microsoft.com/04423d13-3b3e-4741-a01c-1ae29e84fea6?SRC=Home)します。
...
- [レッスン 2](http://code.msdn.microsoft.com/A-simple-Windows-8-Smooth-ee98f63a) - シンプルな Windows 8 スムーズ ストリーミング用のメディア プレーヤーとスライダー バー コントロール 
- [レッスン 3](http://code.msdn.microsoft.com/A-Windows-8-Smooth-883c3b44) - Windows 8 スムーズ ストリーミング用のメディア プレーヤーとストリーム選択  
- [レッスン 4](http://code.msdn.microsoft.com/A-Windows-8-Smooth-aa9e4907)  - Windows 8 スムーズ ストリーミング用のメディア プレーヤーとトラック選択します。
...
Windows ストア アプリケーションの開発の詳細については、次を参照してください。 [Windows 8 用アプリの優れた開発](http://msdn.microsoft.com/windows/apps/br229512.aspx)します。 
...
3.  新しいプロジェクト] ダイアログから入力するか、次の値を選択します。
...
1.  ソリューション エクスプ ローラーで右クリックし **SSPlayer**, 、クリックして **参照の追加**します。
2.  次の値を入力または選択します。
...
1.  ソリューション エクスプ ローラーでダブルクリックして **MainPage.xaml** をデザイン ビューで開きます。
2.  検索、 **& lt;Grid & gt;**  **& lt;/Grid & gt;**  XAML ファイル、および貼り付け、2 つのタグの間は、次のコードにタグを付けます。
...
1.  ソリューション エクスプ ローラーで右クリックし **MainPage.xaml**, 、クリックして **コードの表示**します。
2.  ファイルの先頭に次の using ステートメントを追加します。

        using Windows.Media;

3.  先頭に、 **MainPage** クラスに次のデータ メンバーを追加します。

        private MediaExtensionManager extensions = new MediaExtensionManager();

4.  最後に、 **MainPage** コンス トラクターは、次の 2 つの行を追加します。

        extensions.RegisterByteStreamHandler("Microsoft.Media.AdaptiveStreaming.SmoothByteStreamHandler", ".ism", "text/xml");
        extensions.RegisterByteStreamHandler("Microsoft.Media.AdaptiveStreaming.SmoothByteStreamHandler", ".ism", "application/vnd.ms-sstr+xml");
        
5.  最後に、 **MainPage** クラスは、次のコードを貼り付けます。
...
1.   **ビルド** ] メニューのをクリックして **Configuration Manager**します。
2.  変更 **アクティブ ソリューション プラットフォーム** 、開発プラットフォームに一致するようにします。
3.  キーを押して **F6** 、プロジェクトをコンパイルします。 
4.  キーを押して **f5 キーを押して** アプリケーションを実行します。
5.  アプリケーションの先頭部分で、既定のスムーズ ストリーミング URL を使用するか、別の URL を入力します。 
6.  クリックして **ソース設定**します。  **自動再生** を有効に既定では、メディアは自動的に再生します。  使用してメディアを制御する、 **再生**, 、**一時停止** と **停止** ボタン。  垂直スライダーを使用して、音量を制御できます。  ただし、メディアの進行を制御するための水平スライダーは、まだ完全に実装できていません。 
...
1.  ソリューション エクスプ ローラーで右クリックして **MainPage.xaml**, 、クリックして **コードの表示**します。
2.  ファイルの先頭に次の using ステートメントを追加します。
...
4.  内部、 **MainPage** コンス トラクターは後、は、次のコードを追加、 **これです。初期化 Components();** 行と前のレッスンで記述された登録コード行。
...
5.  内部、 **MainPage** コンス トラクターを追加する 2 つの RegisterByteStreamHandler メソッドを変更する、双方向のパラメーター。
...
1.  ソリューション エクスプ ローラーで右クリックして **MainPage.xaml**, 、クリックして **コードの表示**します。
2.  内部、 **MainPage** クラスに次のデータ メンバーを追加します。

        private AdaptiveSource adaptiveSource = null;

3.  最後に、 **MainPage** クラスに次のイベント ハンドラーを追加します。
...
4.  最後に、 **MainPage** コンス トラクター、アダプティブ ソース オープン イベントをサブスクライブする次の行を追加します。
    
    adaptiveSourceManager.AdaptiveSourceOpenedEvent += 
        new AdaptiveSourceOpenedEventHandler(mediaElement_AdaptiveSourceOpened);

5.  キーを押して **CTRL + S** ファイルを保存します。
...
1.  ソリューション エクスプ ローラーで右クリックして **MainPage.xaml**, 、クリックして **コードの表示**します。
2.  最後に、 **MainPage** クラスに次のイベント ハンドラーを追加します。
...
3.  最後に、 **MainPage** コンス トラクター、イベントをサブスクライブするに次のコードを追加します。
...
3.  内部、 **MainPage** クラスに次のデータ メンバーを追加します。
    
        public static CoreDispatcher _dispatcher;
        private DispatcherTimer sliderPositionUpdateDispatcher;
    
4.  最後に、 **MainPage** コンス トラクターは、次のコードを追加します。

        _dispatcher = Window.Current.Dispatcher;
        PointerEventHandler pointerpressedhandler = new PointerEventHandler(sliderProgress_PointerPressed);
        sliderProgress.AddHandler(Control.PointerPressedEvent, pointerpressedhandler, true);    
    
5.  最後に、 **MainPage** クラスを次のコードを追加します。
    ...
    **注:** CoreDispatcher は、非 UI スレッドから UI スレッドを変更するために使用します。 開発者は、ディスパッチャー スレッドでボトルネックが発生した場合に備えて、更新する UI 要素によって提供されるディスパッチャーの使用を選択できます。  次に例を示します。
...
6.  最後に、 **mediaElement_AdaptiveSourceStatusUpdated** メソッドでは、次のコードを追加します。
    
        setSliderStartTime(args.StartTime);
        setSliderEndTime(args.EndTime);

7.  最後に、 **MediaOpened** メソッドでは、次のコードを追加します。
    
    sliderProgress.StepFrequency = SliderFrequency(mediaElement.NaturalDuration.TimeSpan);
    sliderProgress.Width = mediaElement.Width;
    setupTimer();

8.  キーを押して **CTRL + S** ファイルを保存します。

**アプリケーションのコンパイルとテストを行うには**

1. キーを押して **F6** 、プロジェクトをコンパイルします。 
2.  キーを押して **f5 キーを押して** アプリケーションを実行します。
...
1. ソリューション エクスプ ローラーで右クリックし **MainPage.xaml**, 、クリックして **ビュー デザイナー**します。
...
1. ソリューション エクスプ ローラーで右クリックし **MainPage.xaml**, 、クリックして **ビュー デザイナー**します。
2. 検索、& lt;Grid & gt;名前を持つタグ **gridStreamAndBitrateSelection**, 、タグの末尾に次のコードを追加します。
