...
次 **CreateAssetDeliveryPolicy** メソッドは、作成、 **AssetDeliveryPolicy** 動的共通暗号化を適用するように構成された (**DynamicCommonEncryption**) をスムーズ ストリーミング プロトコル (その他のプロトコルはストリーミングからブロックされます)。 このメソッドには 2 つのパラメーター: **資産** (配信ポリシーを適用するアセット) と **IContentKey** (コンテンツ キーの **CommonEncryption** 型は、詳細についてを参照してください: [コンテンツ キーを作成する](media-services-dotnet-create-contentkey.md#common_contentkey))。
...
>[AZURE.NOTE]Widevine でデータを暗号化するときだけでしょうダッシュを使用して配信すること。 資産配信プロトコルには必ず DASH を指定してください。


##DynamicEnvelopeEncryption 資産の配信ポリシー 

次 **CreateAssetDeliveryPolicy** メソッドは、作成、 **AssetDeliveryPolicy** 動的エンベロープ暗号化を適用するように構成された (**DynamicEnvelopeEncryption**) を HLS と DASH のプロトコルを (その他のプロトコルはストリーミングからブロックされます)。 このメソッドには 2 つのパラメーター: **資産** (配信ポリシーを適用するアセット) と **IContentKey** (コンテンツ キーの **EnvelopeEncryption** 型は、詳細についてを参照してください: [コンテンツ キーを作成する](media-services-dotnet-create-contentkey.md#envelope_contentkey))。
