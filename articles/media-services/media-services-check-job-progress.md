...
ジョブを実行する際には、多くの場合、ジョブの進行状況を追跡する手段が必要になります。 で進行状況をチェックする [StateChanged イベント ハンドラーを定義する](#statechange_event_handler) または [Azure キュー ストレージを使用して Media Services ジョブ通知を監視する](#check_progress_with_queues)します。 このトピックでは両方の方法を説明します。

## <a id="statechange_event_handler"></a>ジョブの進行状況を監視する StateChanged イベント ハンドラーを定義します。
...
## <a id="check_progress_with_queues"></a>Azure キュー ストレージを使用して Media Services ジョブ通知を監視するには
...
>
>または、**IJob.State** プロパティを使用してジョブの状態を確認することもできます。 **IJob** の状態が **[完了]** に設定される前に、ジョブの完了に関する通知メッセージが到着する場合があります。  **IJob.State**  プロパティは、わずかな遅延時間で正確な状態に反映されます。
