...
- **線形** – メイン ビデオを一時停止するフル フレーム広告です。
- **非線形** – メイン ビデオの再生中に表示されるオーバーレイ広告の場合は、通常、ロゴやその他の静的イメージ配置している、プレーヤー内です。
- **コンパニオン** – プレーヤーの外部に表示される広告です。
...
線形広告は、 **<Linear>** 要素。 広告の期間を指定して、イベントを追跡するには、クリックしてスルー、クリック追跡、および多数の **<MediaFile>** 要素。 内で追跡イベントが指定されている、 **<TrackingEvents>** 要素して、広告を表示中に発生するさまざまなイベントを追跡するために ad サーバーです。 この場合、開始、中間、完了、拡大の各イベントが追跡されます。 開始イベントは、広告が表示されたときに発生します。 中間イベントは、広告の時系列が 50% 以上表示されたときに発生します。 完了イベントは、広告が最後まで実行されたときに発生します。 拡大イベントは、ユーザーがビデオ プレーヤーをフル スクリーンに拡大したときに表示されます。 Clickthroughs がで指定された、 **<ClickThrough>** 内の要素、 **<VideoClicks>** 要素をユーザーが広告をクリックしたときに表示されるリソースの URI を指定します。 ClickTracking がで指定された、 **<ClickTracking>** 要素内、 **<VideoClicks>** 要素と、プレーヤーが要求するユーザーが広告をクリックしたときの追跡リソースを指定します。 **<MediaFile>** 要素は、広告の特定のエンコードに関する情報を指定します。 1 つ以上を使用する必要があるときに **<MediaFile>** 要素、ビデオ プレーヤーを選択できます、プラットフォームに応じて最適なエンコードします。 

線形広告は、指定された順序で表示できます。 これを行うには、その他の追加します。 <Ad> VAST への要素は、ファイルし、シーケンス属性を使用して順序を指定します。 次の例を使って説明します。
...
非線形広告で指定します。 <Creative> 同様の要素。 次の例は、 <Creative> 非線形広告を記述する要素。
...
アプリケーション/x-javascript – リソースは HTML に表示されます。 <**スクリプト**> タグです。
...
VMAP ファイルの先頭、 <VMAP> 1 つまたは複数を含む要素 <AdBreak> この要素を使用して、広告ブレークを定義します。 各広告の中断は、中断の種類、中断 ID、時間オフセットを指定します。 BreakType 属性は、中断中に再生できる広告の種類 (線形、非線形、表示) を指定します。 表示広告は、VAST のコンパニオン広告に対応します。 コンマ (スペースなし) 区切りリストで複数の広告の種類を指定できます。 breakID は、広告のオプションの識別子です。 timeOffset は、広告をいつ表示するかを指定します。 これは、次のいずれかの方法で指定できます。
...
AdSchedulerPlugin の詳細については、次を参照してください [を Windows 8 および Windows Phone 8 プレーヤー フレームワークで広告。](http://playerframework.codeplex.com/wikipage?title=Advertising&referringTitle=Windows%208%20Player%20Documentation)
...
このサンプルには、AdSchedulerPlugin を使用して、ミッドロールの線形広告とコンパニオン広告のスケジュールを設定する方法を示しています。 この <RemoteAdSource> 要素は、VAST ファイルの場所を指定します。
...
このサンプルは、AdSchedulerPlugin を使用して、線形広告と非線形広告のスケジュールを指定します。 VAST ファイルの場所を指定します <RemoteAdSource> 要素。
...
このサンプルでは、VmapSchedulerPlugin を使用して、VMAP ファイルを使用する広告のスケジュールを設定します。 VMAP ファイルへの URI の Source 属性で指定します <VmapSchedulerPlugin> 要素。
