...
新しく Azure Media Services アカウントを作成すると、メディア コンテンツの保存に使用する Azure ストレージ アカウントも選択するよう求めるメッセージが表示されます。 できます [1 つ以上のストレージ アカウントを追加](meda-services-managing-multiple-storage-accounts.md) Media Services アカウントにします。
...
>[AZURE.NOTE]複数のストレージ アカウントを使用する場合は、ストレージ アカウントごとには、この手順を実行するとします。
>
...
セカンダリ ストレージ キーの再生成から開始します。 既定では、セカンダリ キーは Media Services で使用されません。  ストレージ キーをローリングする方法の詳細については、次を参照してください。 [方法: 表示、コピーおよび再生成するストレージ アクセス キー](../storage-create-storage-account.md#view-copy-and-regenerate-storage-access-keys)します。
  
##<a id="step2"></a>手順 2: Media Services を更新して新しいセカンダリ ストレージ キーを使用するには
...
>[AZURE.NOTE]保留中のジョブに影響を防ぐために Media Services (たとえば、新しいロケーターの作成) を使用した操作を実行する前に 30 分間待機します。

##手順 3: ロケーターを更新する 

>[AZURE.NOTE]ストレージ アクセス キーをローリングする際は、ストリーミング サービスを中断することがない、既存のロケーターを更新することを確認する必要があります。 
...
>[AZURE.NOTE] OnDemand ロケーターの既存の Url を保持するようにするには、既存のロケーターを削除して、同じ ID で新しいレコードを作成する必要があります。 
...
##手順 5: プライマリ ストレージ アクセス キーを再生成します。

プライマリ ストレージ アクセス キーを再生成します。 ストレージ キーをローリングする方法の詳細については、次を参照してください。 [方法: 表示、コピーおよび再生成するストレージ アクセス キー](../storage-create-storage-account.md#view-copy-and-regenerate-storage-access-keys)します。

##手順 6: Media Services を更新して新しいプライマリ ストレージ キーを使用する
    
」の説明に従って、同じ手順に従います [手順 2.](media-services-roll-storage-access-keys.md#step2) ここは、Media Services アカウントを使用して、新しいプライマリ ストレージ アクセス キーを同期します。
...
」の説明に従って、同じ手順に従います [手順 3.](media-services-roll-storage-access-keys.md#step-3-update-locators)します。
