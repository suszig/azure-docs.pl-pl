...
---|---|---
 ...
 Accept| コンテンツの種類| 次のような応答に対して要求されたコンテンツの種類:<p> --application/json; odata = verbose<p> -アプリケーション フィードおよび atom + xml<p> 応答があります、blob フェッチなど、さまざまなコンテンツの種類、正常な応答が、ペイロードとして blob ストリームに含まれています。
 ...
 client-request-id| String| 指定した要求を識別する、呼び出し元で定義された値。指定されている場合、この値は、要求をマッピングする方法として、応答メッセージに含まれます。<p><p>**重要な**<p>値の上限は 2096b (2 k)。
