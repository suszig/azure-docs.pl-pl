...
## <a id="encoding"></a>エンコーディング \ パッケージ化
...
- [Azure Media Services によるライブ エンコードの実行を有効になっているチャネルを操作](media-services-manage-live-encoder-enabled-channels.md)
...
中間ファイルをアダプティブ ビットレート MP4 セットにエンコードし、セットに変換し、目的の形式を使用して、Media Services を使用の推奨時、 [Dynamic Packaging](media-services-dynamic-packaging-overview.md)します。
