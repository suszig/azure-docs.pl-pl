...
「非ポーリング メソッド」は、次の名前付けパターンを持つ: 送信*OperationName*Operation (たとえば、SendCreateOperation)。送信*OperationName*操作メソッドを返す、 **IOperation** オブジェクト。 返されたオブジェクトには、操作を追跡するために使用できる情報が含まれます。送信*OperationName*OperationAsync メソッドが返す * * タスク<IOperation>* *。
