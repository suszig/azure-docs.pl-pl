...
Media Services の値を使用して、 **IAssetFile.Name** プロパティ、ストリーミング コンテンツ (たとえば、http://{WAMSAccount}.origin.mediaservices.windows.net/{GUID}/{IAssetFile.Name}/streamingParameters) Url を構築する。このため、パーセントエンコーディングは利用できません。Name プロパティの値は、次のいずれかを持つことはできません [パーセント エンコーディング予約文字](http://en.wikipedia.org/wiki/Percent-encoding#Percent-encoding_reserved_characters):! *' ();: @& = + $,/? %#"です。また、ファイル名拡張子で使用できる "." は 1 つのみです。

## Azure サービス管理 REST API でストレージ アカウントをアタッチするには

使用して複数のストレージ アカウントをアタッチする唯一の方法は、現在のところ、 [Azure サービス管理 REST API](http://msdn.microsoft.com/library/azure/dn167014.aspx)します。 サンプル コード、 [方法: Media Services Management REST API](https://msdn.microsoft.com/library/azure/dn167656.aspx) トピックでは定義、 **AttachStorageAccountToMediaServiceAccount** ストレージ アカウントを指定された Media Services アカウントにアタッチする方法です。 同じトピックのコードは、**ListStorageAccountDetails** メソッドを定義し、指定された Media Services アカウントにアタッチされたすべてのストレージ アカウントを表示します。
