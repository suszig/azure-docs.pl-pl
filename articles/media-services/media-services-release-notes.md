...
>[AZURE.NOTE] お客様を聞くし、ユーザーに影響する問題の修正に尽力してみます。 問題の報告または質問に投稿してください、 [Azure Media Services MSDN Forum]します。
...
Azure Storage SDK Version 3.x に含まれる ListBlobs メソッドが失敗する。|Media Services は、に基づいて SAS Url を生成、 [2012年-02-12](http://msdn.microsoft.com/library/azure/dn592123.aspx) バージョンです。 Azure Storage SDK を使用して、BLOB コンテナー内の BLOB を一覧する場合は、Azure Storage SDK Version 2.x の一部である [CloudBlobContainer.ListBlobs](http://msdn.microsoft.com/library/microsoft.windowsazure.storage.blob.cloudblobcontainer.listblobs.aspx) メソッドを使用してください。 Azure Storage SDK Version 3.x に含まれる ListBlobs メソッドは失敗します。
Media Services 調整メカニズムが、サービスに対して過剰な要求を作成するアプリケーションのリソース使用を制限する。 サービスが「サービスを利用できません」(503) HTTP 状態コードを返すことがある。|詳細については、の 503 HTTP 状態コードの説明を参照して、 [Azure Media Services エラー コード](http://msdn.microsoft.com/library/azure/dn168949.aspx) トピックです。
...
Media Services REST API バージョン履歴については、次を参照してください。 [Azure Media Services REST API Reference]します。

##<a id="dec_changes_15"></a>2015 年 12 月のリリース

Azure SDK チームの新しいリリースを公開して、 [Azure SDK for PHP](http://github.com/Azure/azure-sdk-for-php) を Microsoft Azure Media Services の更新と新機能を含むパッケージ。 具体的には、Azure Media Services SDK for PHP サポート最新 [コンテンツ保護](media-services-content-protection-overview.md) 機能: AES と DRM (PlayReady と Widevine) とトークンの制限なしの動的な暗号化します。 拡大/縮小もサポート [エンコード ユニット](media-services-dotnet-encoding-units.md)します。

詳細については、次を参照してください。

-  [Microsoft Azure Media Services SDK for PHP](http://southworks.com/blog/2015/12/09/new-microsoft-azure-media-services-sdk-for-php-release-available-with-new-features-and-samples/) ブログです。
- 次 [コード サンプルは](http://github.com/Azure/azure-sdk-for-php/tree/master/examples/MediaServices) をすぐに開始するために役立ちます。
    - **vodworkflow_aes.php**: これは、PHP ファイル aes-128 動的暗号化とキー配信サービスを使用する方法について説明します。 説明されている .NET のサンプルに基づいて [この](media-services-protect-with-aes128.md) 記事です。
    - **vodworkflow_aes.php**: これは、PHP ファイル PlayReady 動的暗号化とライセンス配信サービスを使用する方法について説明します。 説明されている .NET のサンプルに基づいて [この](media-services-protect-with-drm.md) 記事です。
    - **scale_encoding_units.php**: エンコード予約ユニットの規模の設定方法を示す PHP ファイルです。


##<a id="nov_changes_15"></a>2015 年 11 月のリリース

Azure Media Services は、クラウドで Google Widevine ライセンス配信サービスを提供するようになりました。 詳細についてを参照してください [このお知らせブログ](http://azure.microsoft.com/blog/announcing-google-widevine-license-delivery-services-public-preview-in-azure-media-services/)します。 また、 [このチュートリアル](media-services-protect-with-drm.md) と [GitHub リポジトリ](http://github.com/Azure-Samples/media-services-dotnet-dynamic-encryption-with-drm)します。 

Azure Media Services によって提供される Widevine ライセンス配信サービスはプレビュー期間中であることに注意してください。 詳細については、次を参照してください。 [このブログ](http://azure.microsoft.com/blog/announcing-google-widevine-license-delivery-services-public-preview-in-azure-media-services/)します。

##<a id="oct_changes_15"></a>2015 年 10 月のリリース

Azure Media Services (AMS) は、次のデータ センターではライブでは今すぐ: ブラジル南部、インド西部、インド南部およびインド サーバーの全体です。 Azure クラシック ポータルを使用できます [メディア サービス アカウントを作成](media-services-create-account.md#create-a-media-services-account-using-quick-create) 説明されているさまざまなタスクを実行したり [ここ](https://azure.microsoft.com/documentation/services/media-services/)します。 ただし、これらのデータ センターでは Live Encoding は有効ではありません。 また、これらのデータ センターで、すべての種類のエンコード予約ユニットを使用できるわけではありません。
...
- AMS で、Widevine Modular DRM テクノロジを使用してビデオ オン デマンド (VOD) とライブ ストリーミングの両方を保護できるようになりました。 Widevine ライセンスを配信するため、次の配信サービス パートナーを使用することができます: [Axinom](http://www.axinom.com/press/ibc-axinom-drm-6/), 、[EZDRM](http://ezdrm.com/), 、[castLabs](http://castlabs.com/company/partners/azure/)します。 詳細については、次を参照してください。 [このブログ](http://azure.microsoft.com/blog/azure-media-services-adds-google-widevine-packaging-for-delivering-multi-drm-stream/)します。

    使用する [AMS .NET SDK](https://www.nuget.org/packages/windowsazure.mediaservices/) (バージョン 3.5.1 で始まる) または Widevine を使用する、AssetDeliveryConfiguration を構成する REST API です。  

- AMS に、Apple ProRes ビデオのサポートが追加されました。 Apple ProRes やその他のコーデックを使用する QuickTime ソース ビデオをアップロードできるようになりました。 詳細については、次を参照してください。 [このブログ](http://azure.microsoft.com/blog/announcing-support-for-apple-prores-videos-in-azure-media-services/)します。

- Media Encoder Standard を使用して、サブクリップとライブ アーカイブ抽出を実行できるようになりました。 詳細については、次を参照してください。 [このブログ](http://azure.microsoft.com/blog/sub-clipping-and-live-archive-extraction-with-media-encoder-standard/)します。
...
- Media Encoder Standard の一般公開の発表。 詳細については、次を参照してください。 [このブログの投稿](http://azure.microsoft.com/blog/2015/07/16/announcing-the-general-availability-of-media-encoder-standard/)します。

    説明されているプリセットを使用してメディア エンコーダー スタンダード [この](http://go.microsoft.com/fwlink/?LinkId=618336) セクションです。 4 k エンコードのプリセットを使用している場合は表示、 **Premium** 予約ユニットの種類。 詳細については、次を参照してください。 [スケールのエンコード方法](media-services-portal-encoding-units)します。
- Azure Media Services と Azure Media Player のライブ リアルタイム キャプション。 詳細については、次を参照してください  [このブログの投稿。](https://azure.microsoft.com/blog/2015/07/08/live-real-time-captions-with-azure-media-services-and-player/)
...
- 有効になっている  **メディア エンコーダー プレミアム ワークフロー**します。
...
詳細については、次を参照してください。 [を使用して Json Web キー OpenID Connect discovery 仕様から JWT を使用するトークンを Azure Media Services での認証](http://gtrifonov.com/2015/06/07/using-json-web-keys-from-openid-connect-discovery-spec-to-work-with-jwt-token-authentication-in-azure-media-services/)します。
...
- [Azure Media Player の発表](http://azure.microsoft.com/blog/2015/04/15/announcing-azure-media-player/)します。
- Media Services REST 2.10 以降、RTMP プロトコルをインジェストするように構成されたチャネルは、プライマリとセカンダリのインジェスト URL を使用して作成されます。 詳細については、次を参照してください [チャンネルの取り込みの構成。](media-services-manage-channels-overview.md#channel_input)
- Azure Media Indexer の更新
    - スペイン語のサポート
    - 新しい構成の xml 形式
    
    詳細については、次を参照してください。 [このブログ](http://azure.microsoft.com/blog/2015/04/13/azure-media-indexer-spanish-v1-2/)します。
###Media Services .NET SDK の更新
...
- **互換性に影響する変更**: 変更 **TokenRestrictionTemplate.Issuer** と **TokenRestrictionTemplate.Audience** 文字列型であります。 
- カスタム リトライ ポリシーの作成に関する更新。 
- ファイルのアップロード/ダウンロードに関連するバグの修正。 
-  **MediaServicesCredentials** クラスに対して認証するプライマリとセカンダリのアクセス制御エンドポイントを受け入れるようになりました。
...
- Media Services で、シームレスな Azure CDN 統合が提供されるようになりました。 統合をサポートするために、 **CdnEnabled** にプロパティが追加された **StreamingEndpoint**します。  **CdnEnabled** バージョン 2.9 以降の REST Api で使用できます (詳細については、次を参照してください。 [StreamingEndpoint](https://msdn.microsoft.com/library/azure/dn783468.aspx))。  **CdnEnabled** バージョン 3.1.0.2 以降の .NET SDK で使用できます (詳細については、次を参照してください。 [StreamingEndpoint](https://msdn.microsoft.com/library/azure/microsoft.windowsazure.mediaservices.client.istreamingendpoint(v=azure.10).aspx))。
- アナウンスの **メディア エンコーダー プレミアム ワークフロー**します。 詳細については、次を参照してください。 [Azure Media Services でのプレミアム エンコードの導入に関する](http://azure.microsoft.com/blog/2015/03/05/introducing-premium-encoding-in-azure-media-services)します。
...
Media Services REST API は、現在、バージョン 2.9 です。 このバージョンより、Azure CDN をストリーミング エンドポイントと統合できます。 詳細については、次を参照してください。 [StreamingEndpoint](https://msdn.microsoft.com/library/dn783468.aspx)します。

##<a id="january_changes_15"></a>2015 年 1 月のリリース

### Media Services の全般的な更新

動的な暗号化による Content Protection の General Availability (GA) についてのお知らせ。 詳細については、次を参照してください。 [Azure Media Services は、一般的な可用性の DRM テクノロジによるストリーミング セキュリティを強化](http://azure.microsoft.com/blog/2015/01/29/azure-media-services-enhances-streaming-security-with-general-availability-of-drm-technology/)します。
...
- Azure Indexer メディア プロセッサに、いくつかの更新と新機能が追加されています。 詳細については、次を参照してください。 [Azure Media Indexer Version 1.1.6.7 のリリース ノート](http://azure.microsoft.com/blog/2014/12/03/azure-media-indexer-version-1-1-6-7-release-notes/)します。
- 占有ユニットのエンコードを更新することができる新しい REST API を追加します。 [EncodingReservedUnitType](http://msdn.microsoft.com/library/azure/dn859236.aspx)します。
...
- HLS 自動ターゲット期間が追加されました。 ライブ ストリーミングの実行中、HLS は常に動的にパッケージ化されます。 既定では、Live エンコーダーから送信されるキーフレーム間隔 (KeyFrameInterval) に基づき Media Services が自動的に HLS セグメント パッケージ化率 (FragmentsPerSegment) を計算します。これは GOP (Group of Pictures) と呼ばれることもあります。 詳細については、次を参照してください。 [Working with Azure Media Services Live Streaming]します。
 
###Media Services .NET SDK の更新

- [Azure Media Services .NET SDK](http://www.nuget.org/packages/windowsazure.mediaservices/) が 3.1.0.0 にバージョンアップしました。
- .Net SDK の依存関係が .NET 4.5 Framework にアップグレードされました。
- エンコード予約ユニットを更新できる新しい API が追加されました。 詳細については、次を参照してください。 [占有ユニットの種類の更新とエンコーディング占有の増加 .NET を使用して](http://msdn.microsoft.com/library/azure/jj129582.aspx)します。
- トークン認証に JWT (JSON Web Token) が新たにサポートされています。 詳細については、次を参照してください。 [Azure Media Services と動的暗号化における JWT トークン認証](http://www.gtrifonov.com/2015/01/03/jwt-token-authentication-in-azure-media-services-and-dynamic-encryption/)します。
- PlayReady ライセンス テンプレートに BeginDate と ExpirationDate の相対オフセットが追加されました。


##<a id="november_changes_14"></a>2014 年 11 月のリリース

- ライブ スムーズ ストリーミング (FMP4) コンテンツの SSL 接続での取り込みに Media Services が対応しました。 SSL 経由で取り込むには、取り込み URL を HTTPS に更新する必要があります。  ライブ ストリーミングの詳細については、次を参照してください。 [Working with Azure Media Services Live Streaming]します。
- RTMP ライブ ストリームについては現在、SSL 接続で取り込むことはできません。
- SSL 接続経由でコンテンツのストリーミングもできます。 そのためには、ストリーミング URL の先頭が HTTPS になっていることをご確認ください。
- SSL 経由でのストリーミングを実行できるのは、コンテンツの配信元となるストリーミング エンドポイントが 2014 年 9 月 10 日より後に作成されている場合のみです。 ストリーミング URL の基になるストリーミング エンドポイントの作成日が 9 月 10 日より後である場合、URL に "streaming.mediaservices.windows.net" (新形式) が含まれています。 "origin.mediaservices.windows.net" (旧形式) を含んだストリーミング URL では、SSL がサポートされません。 URL が旧形式で、SSL 経由でストリーミングしたい場合 [新しいストリーミング エンドポイントを作成](media-services-manage-origins.md)します。 SSL でコンテンツをストリーミングするには、新しいストリーミング エンドポイントに基づいて作成された URL を使用する必要があります。
   
##<a id="october_changes_14"></a>2014 年 10 月のリリース

### <a id="new_encoder_release"></a>Media Services Encoder のリリース

Media Services Azure Media Encoder の新規リリースが発表されています。 最新の Azure Media Encoder では、出力 GB 分のみ課金されます。それ以外は、新しいエンコーダーの機能は前のエンコーダーと互換性があります。 詳細については [Media Services Pricing Details])。
...
あるサンプル [Azure Media Services のサンプル GitHub リポジトリ](https://github.com/Azure/Azure-Media-Services-Samples)します。


##<a id="september_changes_14"></a>2014 年 9 月のリリース

Media Services REST メタデータは、現在、バージョン 2.7 です。 最新の REST 更新プログラムの詳細については、次を参照してください。 [Azure Media Services REST API Reference]します。

Media Services SDK for .NET は、現在、バージョン 3.0.0.7 です。
 
### <a id="sept_14_breaking_changes"></a>互換性に影響する変更

* **原点** に名前が変更された [StreamingEndpoint]します。
* 使用する場合の既定の動作の変更、 **Azure Classic Portal** をエンコードし、MP4 ファイルを発行します。
...
* .mp4 をダウンロード (または、漸次的に再生) するための SAS url を作成します。 SAS ロケーターを作成する方法の詳細については、次を参照してください。 [Delivering Content]します。


### <a id="sept_14_GA_changes"></a>GA リリースの一部である新機能/シナリオ

* **インデクサー メディア プロセッサ**します。 詳細については、次を参照してください。 [Indexing Media Files with Azure Media Indexer]します。

*  [StreamingEndpoint] エンティティを今すぐ使用してカスタム ドメイン (ホスト) 名を追加します。
    ...
    * ドメイン名の所有権は、Azure Media Services によって検証する必要があります。 ドメインを検証するには、マップする CName を作成します。 <MediaServicesAccountId>.<parent domain> verifydns. < mediaservices dns ゾーン > です。 
...
* ライブ ストリーミングのプレビュー。 詳細については、次を参照してください。 [Working with Azure Media Services Live Streaming]します。

* キー配信サービス。 詳細については、次を参照してください。 [Using AES-128 Dynamic Encryption and Key Delivery Service]します。

* AES 動的暗号化。 詳細については、次を参照してください。 [Using AES-128 Dynamic Encryption and Key Delivery Service]します。

* PlayReady ライセンス配信サービス。 詳細については、次を参照してください。 [Using PlayReady Dynamic Encryption and License Delivery Service]します。

* PlayReady 動的暗号化。 詳細については、次を参照してください。 [Using PlayReady Dynamic Encryption and License Delivery Service]します。

* Media Services PlayReady ライセンス テンプレート。 詳細については、次を参照してください。 [Media Services PlayReady License Template Overview]します。

* ストリーミング ストレージ暗号化資産。 詳細については、次を参照してください。 [Streaming Storage Encrypted Content]します。

##<a id="august_changes_14"></a>2014 年 8 月のリリース

資産をエンコードすると、エンコード ジョブの完了時に出力資産が生成されます。 このリリースになるまで、Azure Media Services エンコーダーは出力資産に関するメタデータを生成していました。 このリリースから、エンコーダーは入力資産に関するメタデータも生成します。 詳細については、次を参照してください。、 [Input Metadata] と [Output Metadata] トピックです。
...
使用できるように [Dynamic Packaging] HTTP Live Streaming (HLS) v3 のストリーミングにします。 HLS v3 をストリーミングするには、次のフォーマットをオリジン ロケーター パスに追加します。*.ism/manifest(format=m3u8-aapl-v3)。 詳細については、次を参照してください。 [Nick Drouin's Blog]します。

現在、動的パッケージは、PlayReady によるスムーズ ストリーミング静的暗号化に基づく、PlayReady による HLS (v3 および v4) 暗号化の配信もサポートしています。 PlayReady でスムーズ ストリーミングを暗号化する方法については、次を参照してください。 [Protecting Smooth Stream with PlayReady]します。
...
詳細については、次を参照してください。 [Retry Logic in the Media Services SDK for .NET]します。

##<a id="april_changes_14"></a>2014 年 4 月のエンコーダーのリリース

### <a name="april_14_enocer_changes"></a>Media Services Encoder の更新

* Grass Valley HQ/HQX コーデックを使用してビデオをすばやく圧縮する Grass Valley EDIUS 非線形エディターを使用して作成された AVI ファイルの取り込みがサポートされるようになりました。 詳細については、次を参照してください。 [Grass Valley Announces EDIUS 7 Streaming Through the Cloud]します。

* Media Encoder によって生成されたファイルの名前付け規則を指定できるようになりました。 詳細については、次を参照してください。 [Controlling Media Service Encoder Output Filenames]します。

* ビデオ オーバーレイおよびオーディオ オーバーレイのサポートが追加されました。 詳細については、次を参照してください。 [Creating Overlays]します。

* 複数のビデオ セグメントを結合できるようになりました。 詳細については、次を参照してください。 [Stitching Video Segments]します。
...
* テスト ソリューションを分割 [GitHub] ユニット ベースのテストとシナリオ ベースのテストにします。

変更に関する詳細については、次を参照してください。: [Azure Media Services .NET SDK 3.0.0.1 and 3.0.0.2 releases]します。

3.0.0.3 では次の点が変更されました。

* バージョン 3.0.3.0 を使用するように Azure Storage の依存関係がアップグレードされました。 

* 旧バージョンと互換性の問題を修正します。*します。* 解放します。 
...
>[AZURE.NOTE] 3.0.x.x リリースは、2.4.x.x リリースとの下位の互換性ではありません。

現在、Media Services SDK の最新バージョンは 3.0.0.0 です。 Nuget から最新パッケージをダウンロードまたはからビットを取得することができます [GitHub]します。

Media Services SDK バージョン 3.0.0.0 以降では、再利用できる、 [Azure Active Directory Access Control Service (ACS)] トークンです。 詳細については、「アクセス制御サービス トークンの再利用」セクションを参照してください、 [Connecting to Media Services with the Media Services SDK for .NET] トピックです。

### <a name="dec_13_donnet_ext_changes"></a>Azure Media Services .NET SDK Extensions 2.0.0.0

Azure Media Services .NET SDK Extensions は、コードを簡素化し、Azure Media Services による開発を容易にする一連の拡張機能メソッドおよびヘルパー機能です。 最新のバージョンを入手できます [Azure Media Services .NET SDK Extensions]します。
...
次の Media Services PowerShell コマンドレットが含まれるようになりました [azure-sdk-tools]します。
...
* Media Services SDK Version 2.3.0.0 から、複数のストレージ アカウントの Media Services アカウントへのリンクがサポートされています。 次の API がこの機能をサポートしています。
    ...
    詳細については、次を参照してください。 [Managing Media Services Assets across Multiple Storage Accounts]します。

* API 関連の通知。 バージョン 2.2.0.0 から、Azure キュー ストレージ通知をリッスンできます。 詳細については、「 [Handling Media Services Job Notifications]します。
