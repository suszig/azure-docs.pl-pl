...
サービス定義ファイルを次の例と同じにする必要があります、 `< インポート >` 要素を追加します。
...
```
[ServiceConfiguration.cscfg](cloud-services-model-and-package.md#cscfg) ファイルは、次の例と似ています必要がありますに注意してください、 `< ConfigurationSettings >` と `< 証明書 >` 要素。指定された証明書がある必要があります [、クラウド サービスにアップロード](cloud-services-how-to-create-deploy/#how-to-upload-a-certificate-for-a-cloud-service)します。
