...
2.  Web ブラウザーで、**http** ではなく **https** を使用するようにリンクを修正し、ページにアクセスします。
    ...
    >[AZURE.TIP] 運用環境のデプロイではなくステージング環境のデプロイに SSL を使用する場合は、最初に、ステージング環境のデプロイに使用されている URL を確認する必要があります。 クラウド サービスをデプロイすると、ステージング環境への URL によって決定されます、 **展開 ID** 形式で GUID: `https://deployment-id.cloudapp.net/`  
