...
**2014 年 9 月 2 日** Azure ゲスト オペレーティング システム (ゲスト OS) ファミリ 1.x では、Windows Server 2008 オペレーティング システムに基づくが正式に廃止されました。 ファミリ 1 を使用して新しいサービスをデプロイしようとする試みまたは既存のサービスをアップグレードしようとする試みはすべて失敗し、ゲスト OS ファミリ 1 の提供が終了したことを示すエラー メッセージが表示されます。 

**2014 年 11 月 3 日** ゲスト OS ファミリ 1 の延長サポートが終了し、完全に提供が中止されます。 ファミリ 1 を利用しているすべてのサービスは、この影響を受けます。 これらのサービスは、突然停止される可能性があります。 自分で手動でこれらのサービスをアップグレードしない限り、サービスが継続される保証はありません。

その他の質問があればを参照してください、 [Cloud Services フォーラム](http://social.msdn.microsoft.com/Forums/home?forum=windowsazuredevelopment&filter=alltypes&sort=lastpostdesc) または [Azure サポートにお問い合わせ](http://azure.microsoft.com/support/options/)します。
...
どの OS ファミリで実行されているクラウド サービスを検索するには、行うことができます以下のスクリプトを Azure PowerShell の必要があります [Azure PowerShell を設定](../install-configure-powershell.md) 最初です。 追加の詳細については、スクリプトでは、次を参照してください。 [Azure ゲスト OS ファミリ 1 の提供中止: 2014 年 6 月](http://blogs.msdn.com/b/ryberry/archive/2014/04/02/azure-guest-os-family-1-end-of-life-june-2014.aspx)します。 
