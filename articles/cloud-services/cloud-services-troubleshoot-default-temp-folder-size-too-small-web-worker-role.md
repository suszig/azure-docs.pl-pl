...
>[AZURE.NOTE] これは Azure SDK 1.0 ~ SDK 1.4 の Web ロールとワーカー ロールを使用してのみに適用されます。
...
- ローカル ストレージ リソースを構成しを使用せずに直接アクセス **TEMP** または **TMP**します。 アプリケーション内で実行されるコードからローカル ストレージ リソースにアクセスするには、呼び出し、 [RoleEnvironment.GetLocalResource](https://msdn.microsoft.com/library/microsoft.windowsazure.serviceruntime.roleenvironment.getlocalresource.aspx) メソッドです。 ローカル ストレージ リソースの設定の詳細については、次を参照してください。 [ローカル ストレージ リソースの構成](cloud-services-configure-local-storage-resources.md)します。
