...
2. 運用スロットとステージング スロットの両方を削除する - この解決方法では、既存の DNS 名が維持されますが、アプリケーションでダウンタイムが発生します。
    ...
    - 次に、既存のクラウド サービスに新しいデプロイを作成します。 これは、領域内のすべてのクラスターに割り当てを再度試みます。 クラウド サービスがアフィニティ グループに関連付けられていないことを確認します。

3. 予約済み IP は、このソリューションが保存され、既存の IP アドレスしますが、アプリケーションにダウンタイムが発生します。
...
5. 地域仮想ネットワークにある「」を参照する変換 [リージョン Virtual Network (VNet) にアフィニティ グループから移行する方法](https://azure.microsoft.com/documentation/articles/virtual-networks-migrate-to-regional-vnet/)します。

## その他のリソース

### Azure カスタマー サポートへの問い合わせ

Azure の問題を解決するためにこの記事が役立たなかった場合に Azure のフォーラムを参照する [MSDN やスタック オーバーフロー](http://azure.microsoft.com/support/forums/)します。
問題についての Azure サポート インシデントを送信することもできます。 移動して、 [Azure サポート](http://azure.microsoft.com/support/options/) サイトおよびサポートにお問い合わせ] をクリックします。 Azure のサポートを使用する方法の詳細については、読み取り、[ Microsoft Azure サポートに関する FAQ](http://azure.microsoft.com/support/faq/)します。
