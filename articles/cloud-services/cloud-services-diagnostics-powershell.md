...
いずれかを使用して診断拡張機能を有効にした場合 *Set-azureservicediagnosticsextension* または *新規 AzureServiceDiagnosticsExtensionConfig* せず、 *ロール* してパラメーターを使用して、拡張機能を削除できます *Remove-azureservicediagnosticsextension* せず、 *ロール* パラメーター。 場合、 *ロール* 、拡張機能の削除時に、拡張し、それを有効化を使用もする必要がありますパラメーターが使用されました。
...
- Azure 診断やその他の手法に関する問題のトラブルシューティングを使用した追加のガイダンスについては、次を参照してください。 [Azure クラウド サービスおよび仮想マシンで診断を有効にすると](cloud-services-dotnet-diagnostics.md)です。
-  [診断構成スキーマ](https://msdn.microsoft.com/library/azure/dn782207.aspx) 診断拡張機能のさまざまな xml 構成オプションについて説明します。
- 仮想マシンの診断拡張機能を有効にする方法については、次を参照してください [を監視し、Azure リソース マネージャーのテンプレートを使用して診断を Windows 仮想マシンの作成。](virtual-machines-extensions-diagnostics-windows-template.md)  
