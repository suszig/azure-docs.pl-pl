...
## PowerShell からリモート デスクトップを構成する
 ...
 [セット AzureServiceRemoteDesktopExtension](https://msdn.microsoft.com/library/azure/dn495117.aspx) コマンドレットでは、指定された役割またはクラウド サービス デプロイのすべてのロールでリモート デスクトップを有効にすることができます。 コマンドレットでは、を介してリモート デスクトップ ユーザーのユーザー名とパスワードを指定することができます、 *資格情報* パラメーター PSCredential オブジェクトを受け取ります。
...
## ロール インスタンスへのリモート デスクトップ接続
 [Get-azureremotedesktopfile](https://msdn.microsoft.com/library/azure/dn495261.aspx) コマンドレットは、クラウド サービスの特定のロール インスタンスにリモート デスクトップを使用できます。 使用することができます、 *LocalPath* または RDP ファイルをローカルにダウンロードするコマンドレットのパラメーターを使用して、 *起動* パラメーターを直接クラウド サービスのロール インスタンスにアクセスするリモート デスクトップ接続] ダイアログを開きます。
...
>[AZURE.NOTE]  *UninstallConfiguration* パラメーターは、サービスに適用されている拡張機能の構成をアンインストールします。 拡張機能のすべての構成は、デプロイによって拡張機能をアクティブ化するためにサービス構成に関連付けられており、デプロイはその拡張機能の構成に関連付けられている必要があります。 せず削除コマンドレットを呼び出す *UninstallConfiguration* ので、展開から、拡張機能を効果的に削除する拡張機能の構成から展開の関連付けを解除します。 ただし、拡張機能の構成は、サービスに関連付けられたままになります。 拡張機能の構成を完全に削除するには、削除を使用してコマンドレットを呼び出す必要があります、 *UninstallConfiguration* パラメーター。 
