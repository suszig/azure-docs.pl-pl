...
このガイドでは、Python から一般的なサービス管理タスクをプログラムで実行する方法について説明します。  **ServiceManagementService** クラス、 [Azure SDK for Python](../python-how-to-install.md) で提供されるサービス管理に関連する機能の多くにプログラムでアクセスをサポートしている、 [Azure クラシック ポータルの ][management-portal] (など **の作成、更新、およびクラウド サービス、展開、データ管理サービスおよび仮想マシンを削除する**)。 この機能は、サービス管理へのプログラムによるアクセスが必要なアプリケーションをビルドするために役立つ場合があります。
> [AZURE.NOTE] サービス管理 API は、新しいリソース管理 API (現在のところ、プレビュー リリースで使用できます) によって置き換えられます。 参照してください、 [Azure リソース管理ドキュメント](http://azure-sdk-for-python.readthedocs.org/) Python から新しいリソース管理 API の使用の詳細について。


## <a name="WhatIs"> </a>サービス管理とは

サービス管理 API を通じて使用可能なサービス管理の機能の多くにプログラムでアクセスを提供する、 [Azure クラシック ポータルの ][management-portal]します。 Azure SDK for Python を使用すると、クラウド サービスとストレージ アカウントを管理できます。
...
サービス管理エンドポイントに接続するには、Azure サブスクリプション ID、および有効な管理証明書が必要です。 サブスクリプション ID を取得、 [Azure クラシック ポータルの ][management-portal]します。
...
使用する [OpenSSL](http://www.openssl.org/) 管理証明書を作成します。 実際には、サーバーのいずれかの 2 つの証明書を作成する必要があります (、 `.cer` ファイル) と、クライアント用 (、 `.pem` ファイル)。 作成する、 `.pem` ファイルでこれを実行します。

    `openssl req -x509 -nodes -days 365 -newkey rsa:1024 -keyout mycert.pem -out mycert.pem`

作成する、 `.cer` 証明書、これを実行します。
...
これらのファイルを作成した後は、アップロードする必要があります、 `.cer` ファイルを Azure の [設定] タブの [アップロード] アクション、 [Azure クラシック ポータルの ][management-portal], 、保存場所をメモしておいて必要があります、 `.pem` ファイルです。
...
上記の例で `sms` は、 **ServiceManagementService** オブジェクトです。 **ServiceManagementService** クラスは、Azure サービスを管理するときに使用する主要なクラスです。

### Windows での管理証明書 (MakeCert)

自己署名管理証明書を作成するには使用して、マシンで `makecert.exe`します。 **管理者**として **Visual Studio コマンド プロンプト**を開き、次のコマンドを使用して、*AzureCertificate* を、使用する証明書の名前に置き換えます。
...
証明書を作成した後は、アップロードする必要があります、 `.cer` ファイルを Azure の [設定] タブの [アップロード] アクション、 [Azure クラシック ポータルの ][management-portal]します。
...
## <a name="ListAvailableLocations"> </a>方法: 利用可能な場所を一覧表示
...
アプリケーションを作成してそれを Azure で実行して、コードと構成をあわせてと呼ばれる Azure [クラウド サービス] (と呼ばれる、 *ホステッド サービス* 以前の Azure リリース)。  **\_Hosted\_service** メソッドを使用して、ホステッド サービス名 (Azure 内で一意である必要があります)、ラベル (base64 に自動的にエンコードされます)、説明、場所を提供し、新しいホステッド サービスを作成できます。
...
サービスを削除する前に、そのサービスのすべてのデプロイを最初に削除する必要があることに注意してください (を参照してください [方法: デプロイの削除](#DeleteDeployment) 詳細です)。

## <a name="DeleteDeployment"> </a>方法: デプロイの削除

展開の削除を使用して、 **delete \_deployment** メソッドです。 次の例は、という名前のデプロイメントを削除する方法を示しています。 `v1`します。
...
## <a name="ListOperatingSystems"> </a>方法: 利用可能なオペレーティング システムを一覧表示
...
Linux 仮想マシンをキャプチャする方法の詳細については、を参照してください [を Linux 仮想マシンをキャプチャする方法。](../virtual-machines/virtual-machines-linux-capture-image.md)。

Windows 仮想マシンをキャプチャする方法の詳細については、を参照してください [を Windows 仮想マシンをキャプチャする方法。](../virtual-machines/virtual-machines-capture-image-windows-server.md)。
