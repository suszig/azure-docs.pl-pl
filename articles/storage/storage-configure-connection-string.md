...
- Azure クラウド サービスで実行されるアプリケーションでの接続文字列を格納することができます、 [Azure サービス構成スキーマ (.cscfg) ファイル](https://msdn.microsoft.com/library/ee758710.aspx)します。 接続文字列をサービス構成ファイルの **ConfigurationSettings** セクションに追加します。
...
Azure ストレージ アカウントへの接続文字列を作成するには、次の接続文字列の形式を使用します。 HTTP または HTTPS (推奨) でストレージ アカウントに接続するかどうかを示す、それぞれ置き換えます `myAccountName` ストレージ アカウント、および置換の名前で `myAccountKey` をアカウント アクセス キー。
...
2 つのオプションを指定することがある、接続文字列でサービス エンドポイントを明示的に指定するときに `資格情報` 上記の文字列にします。

- アカウント名とキーを指定することができます: `AccountName = myAccountName;AccountKey = myAccountKey`
- 共有アクセス署名を指定することができます: `SharedAccessSignature = base64Signature`
...
エンドポイントには、既定のサービス エンドポイントとカスタム エンドポイントのいずれかを指定できます。  `Base64Signature` 共有アクセス署名の署名部分に対応します。 署名は、有効な署名対象文字列とキーを SHA256 アルゴリズムを使用して計算処理した後、その値を Base64 エンコードした HMAC 値です。

### エンドポイント サフィックスを含む接続文字列を作成する

Azure China、Azure Governance など、別のエンドポイント サフィックスを持つリージョンまたはインスタンスのストレージ サービスの接続文字列を作成するには、次の接続文字列の形式を使用します。 HTTP または HTTPS によるストレージ アカウントに接続し、置換するかどうかを示す `myAccountName` をストレージ アカウントの名前、 `myAccountKey` アカウント アクセス キー、および置換 `mySuffix` を URI サフィックス。
