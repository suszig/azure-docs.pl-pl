...
> [AZURE.NOTE] このガイドで紹介する Azure CLI のコマンドとスクリプトの例は、Azure サービス管理 (ASM) モードで実行されています。 参照してください [Mac、Linux、および Azure リソース管理で Windows 用 Azure CLI の使用](../azure-cli-arm-commands.md#azure-storage-commands-to-manage-your-storage-objects) Azure リソース管理 (ARM) モードでストレージ用の Azure CLI コマンドにします。
...
2. Azure CLI をインストールすると、コマンド ライン インターフェイス (Bash、Terminal、Command プロンプト) から azure コマンドを使用して Azure CLI コマンドにアクセスできるようになります。 型 `azure` コマンドと、次の出力が表示されます。

    ![Azure Command Output][image1]

3. コマンド ライン インターフェイスで次のように入力します。 `azure ストレージ` Azure CLI の機能の第一印象をすると、azure storage のすべてのコマンド一覧が用意されています。 コマンド名を入力することができます **-h** パラメーター (たとえば、 `azure ストレージの共有の作成には、-h`) コマンドの構文の詳細を表示します。
...
6. ここで、構成設定に基づいてスクリプト変数を更新する必要があります。
    ...
    - * *<storage_account_name>* * スクリプトの指定された名前を使用するか、ストレージ アカウントの新しい名前を入力します。**重要:** ストレージ アカウントの名前は、Azure 上で一意である必要があります。また、小文字にする必要もあります。

    - * *<storage_account_key>* * ストレージ アカウントのアクセス キー。

    - * *<container_name>* * スクリプトの指定された名前を使用するか、コンテナーの新しい名前を入力します。
...
Azure Storage のすべての BLOB はコンテナーに格納する必要があります。 使用して、プライベート コンテナーを作成することができます、 `azure ストレージ コンテナーを作成` コマンド。
...
内の blob をコンテナーにアップロードするにを使用して、 `azure ストレージ blob のアップロード`します。 既定では、このコマンドにより、ローカル ファイルがブロック BLOB にアップロードされます。 使用できる blob の種類を指定する、 `--blobtype` パラメーター。
...
この例で実行するのは非同期コピーです。 実行して各コピー操作のステータスを監視することができます、 `azure ストレージ blob のコピーのショー` 操作します。
