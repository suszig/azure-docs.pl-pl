...
Visual Studio を使用してクラウド サービス プロジェクトで Azure ストレージ アカウントを参照または作成した後は、Visual Studio で Azure テーブル ストレージを使用して開始する方法を説明 **[接続済みサービス** ダイアログ。  **[接続済みサービス** 操作が、プロジェクト内の Azure ストレージにアクセスする適切な NuGet パッケージをインストールし、プロジェクトの構成ファイルをストレージ アカウントの接続文字列を追加します。
...
最初に、ストレージ アカウント内にテーブルを作成する必要があります。 コードで Azure テーブルを作成する方法やテーブルとエンティティの基本的な操作について説明します (テーブル エンティティの追加、変更、読み取りなど)。 サンプルは c \# コードで使用する記述、 [.NET 用 Azure ストレージ クライアント ライブラリ](https://msdn.microsoft.com/library/azure/dn261237.aspx)します。

**注:** Azure storage への呼び出しを実行する Api の一部は非同期です。 参照してください [Async および Await における非同期プログラミング](http://msdn.microsoft.com/library/hh191443.aspx) の詳細。 次のコードでは、非同期のプログラミング方法を使用していることを前提としています。

- 参照してください [.NET からテーブル ストレージを使用する方法](storage-dotnet-how-to-use-tables.md) プログラムを使用してテーブルを操作の詳細についてです。
...
> [AZURE.NOTE]  次のサンプルでは、すべてのコードの前に上記のコードを使用します。
...
Azure のテーブルを作成するへの呼び出しを追加 **CreateIfNotExistsAsync** に、入手したら、 **CloudTable** オブジェクトの「コードでテーブルにアクセス」セクションで説明します。
...
エンティティをテーブルに追加するには、エンティティのプロパティを定義するクラスを作成します。 次のコードと呼ばれるエンティティ クラスを定義する **CustomerEntity** パーティション キーと行キーと最後の名前として、顧客の最初の名前を使用しています。
...
エンティティに関連するテーブル操作が実行を使用して、 **CloudTable** 「でテーブルにアクセス コードです」で作成したオブジェクト。  **TableOperation** オブジェクトは、実行する操作を表します。 次のコード例を作成する方法を示しています、 **CloudTable** オブジェクトと **CustomerEntity** オブジェクトです。 操作を準備する、 **TableOperation** テーブルに顧客エンティティを挿入するを作成します。 呼び出して、操作を実行する最後に、 **CloudTable.ExecuteAsync**します。
