...
- 各ファイル共有の実際の使用状況の監視
- 共有のサイズ クォータの調整
- 取得、 `net` コマンドを使用して Windows クライアントからファイル共有をマウントするには
...
置換 `ストレージ アカウント名` と `ストレージ アカウント キー` 、ストレージ アカウント名と次の例でキーを使用します。
...
次に、新しいファイル共有をという名前を作成 `ログ`します。

    # create a new share
    $s = New-AzureStorageShare logs -Context $ctx

これで、File Storage にファイル共有が作成されました。 次に、ディレクトリとファイルを追加します。
> [AZURE.IMPORTANT] ファイル共有の名前はすべて小文字にする必要があります。 ファイル共有とファイルの名前付けに関する完全な詳細については、「 [名前付けおよび参照する共有ディレクトリ ファイル、およびメタデータ](https://msdn.microsoft.com/library/azure/dn167011.aspx)します。

### ファイル共有でのディレクトリの作成

次に、共有内にディレクトリを作成します。 次の例では、ディレクトリの名前は `CustomLogs`します。
...
次に、ディレクトリにローカル ファイルをアップロードします。 次の例は、ファイルからアップロード `C:\temp\Log1.txt`します。 ファイル パスを編集して、ローカル マシン上の有効なファイルを指定してください。
...
クライアントが File Storage にアクセスするときに使用する SMB バージョンは、オペレーティング システムでサポートされている SMB バージョンによって異なります。次の表に、Windows クライアントでのサポートの概要を示します。詳細についてを参照してください << Which version of the SMB protocol blog post>> です。
...
1. 最初に、次の手順に従って新しい Azure 仮想マシンを作成 [Windows Server を実行する仮想マシンを作成する](../virtual-machines-windows-tutorial.md)します。
...
ファイル共有をマウントする前に、ストレージ アカウントの資格情報を仮想マシンにも適用します。そうすることで、仮想マシンが再起動したときに Windows が自動的にファイル共有に再接続します。実行アカウントの資格情報を保持するため、 `cmdkey` 仮想マシンで PowerShell ウィンドウからコマンドです。置換 `< ストレージ アカウント名 >` 、ストレージ アカウントの名前と `< ストレージ アカウント キー >` をストレージ アカウント キー。

    cmdkey /add:<storage-account-name>.file.core.windows.net /user:<storage-account-name> /pass:<storage-account-key>

これで、仮想マシンの再起動時に Windows が自動的にファイル共有に再接続するようになります。 実行して、共有に再接続したことを確認することができます、 `net` PowerShell ウィンドウからコマンドです。

対象のコンテキストでのみ、資格情報が保存されることに注意してください `cmdkey` を実行します。 サービスを実行するアプリケーションを開発している場合、そのコンテキストでも資格情報を永続化する必要があります。

### 適用された資格情報を使用したファイル共有のマウント

仮想マシンへのリモート接続を作成したら、行うことができます、 `net` コマンドで、次の構文を使用して、ファイル共有をマウントします。置換 `< ストレージ アカウント名 >` 、ストレージ アカウントの名前と `< 共有名 >` 実際の File ストレージ共有の名前に置き換えます。
...
前の手順でストレージ アカウントの資格情報を適用したために提供する必要はありません、 `net` コマンドです。 場合は、まだ適用していない、資格情報を指定してに渡されるパラメーター、 `net` コマンドは、次の例に示すようにします。
...
次に、プロジェクトの app.config ファイルに資格情報を保存します。 次の例のように表示されるように app.config ファイルを編集交換 `myaccount` をストレージ アカウント名、および `mykey` をストレージ アカウント キー。
...
いずれかを使用して、app.config ファイルから保存された資格情報を取得することができます、 `Microsoft.WindowsAzure.CloudConfigurationManager` クラス、または `ための System.Configuration.ConfigurationManager `クラスです。 Microsoft Azure Configuration Manager パッケージが含まれて、 `Microsoft.WindowsAzure.CloudConfigurationManager` クラスが [利用可能な [Nuget](https://www.nuget.org/packages/Microsoft.WindowsAzure.ConfigurationManager)します。

次の例を使用して資格情報を取得する方法を示しています、 `CloudConfigurationManager` クラスし、これらをカプセル化、 `CloudStorageAccount` クラスです。 次のコードを追加、 `Main()` program.cs 内のメソッドです。
...
次に、次のコードを追加、 `Main()` メソッド (前に示したコード) の後、接続文字列を取得します。 このコードは、前の手順で作成したファイルへの参照を取得し、その内容をコンソール ウィンドウに出力します。
...
Azure Storage クライアント ライブラリのバージョン 5.x 以降、ファイル共有または個々のファイルの Shared Access Signature (SAS) を生成できます。 また、ファイル共有に共有アクセス ポリシーを作成して、共有アクセス署名を管理することもできます。 共有アクセス ポリシーを作成することをお勧めします。これにより、侵害されそうな場合に SAS を取り消すことができます。
...
作成して、共有アクセス署名の使用に関する詳細については、次を参照してください。 [共有アクセス署名: SAS モデルについて](storage-dotnet-shared-access-signature-part-1.md) と [を使用して、Blob サービスによる SAS の作成と](storage-dotnet-shared-access-signature-part-2.md)します。
...
5. **オンプレミスの仮想マシンから Azure File Storage への接続は、Azure ExpressRoute に依存していますか。**
    ...
    番号 ExpressRoute を取得していないポート 445 (TCP 送信) からのインターネット アクセスがある限り、ファイル共有は、内部設置型からアクセスできます。 ただし、必要であれば File Storage で ExpressRoute を使用できます。
...
10. **共有は入れ子にできますか。つまり共有の下に共有を配置できますか。**
    ...
    番号 ファイル共有は、入れ子になった共有がサポートされていませんので、仮想のドライバーをマウントすることができますですです。
...
12. * * ファイル ストレージにファイルを解凍しようとすると、パフォーマンスは低速でした。 どうすればでしょうか * *。
...
- [Windows および Linux 用 azure ファイル ストレージ: 摩擦のないクラウド SMB のファイル システム](https://azure.microsoft.com/documentation/videos/azurecon-2015-azure-files-storage-a-frictionless-cloud-smb-file-system-for-windows-and-linux/)
...
- [Azure Storage で Azure PowerShell を使用](storage-powershell-guide-full.md)
...
- [参照を .NET 用ストレージ クライアント ライブラリ](https://msdn.microsoft.com/library/azure/dn261237.aspx)
- [ファイル サービス REST API リファレンス](http://msdn.microsoft.com/library/azure/dn167006.aspx)

### ブログ記事

- [Azure File ストレージが利用できるようになりました](http://go.microsoft.com/fwlink/?LinkID=626728&clcid=0x409)
