...
このガイドでは、Azure Table サービスを使用して一般的なシナリオを実行する方法について説明します。 サンプルは Ruby API を使用して記述されています。 紹介するシナリオ **を作成してテーブルを削除して、挿入、およびテーブル内のエンティティを照会する**です。
...
2. 型 **gem のインストール azure** gem と依存関係をインストールするコマンド ウィンドウにします。
...
Azure モジュールでは、環境変数を読み取ります **azure \_storage\_account** と **azure \_storage\_access\_key** については、Azure ストレージ アカウントへの接続に必要です。 これらの環境変数が設定されていない場合は、使用する前にアカウント情報を指定する必要があります **azure::tableservice** を次のコード。
...
エンティティを追加するには、エンティティのプロパティを定義するハッシュ オブジェクトを最初に作成します。 すべてのエンティティにする必要がありますを指定することに注意してください、 **PartitionKey** と **RowKey**します。 これらはエンティティの一意の識別子であり、他のエンティティのプロパティよりはるかに高速に照会できる値です。 Azure ストレージを使用して **PartitionKey** に多くのストレージ ノードにテーブルのエンティティを自動的に配布します。 同じエンティティ **PartitionKey** 同じノードに格納されます。  **RowKey** が属するパーティション内のエンティティの一意の id を指定します。
...
テーブル内のエンティティのセットを照会するには、クエリ ハッシュ オブジェクトを作成しを使用して、 **query \_entities ()** メソッドです。 次の例では、同じエンティティをすべて取得 **PartitionKey**:

    query = { :filter => "PartitionKey eq 'test-partition-key'" }
    result, token = azure_table_service.query_entities("testtable", query)

> [AZURE.NOTE] 結果セットの場合が返すには、継続トークンが返されます後続のページを取得するのにはして単一のクエリに対して大きすぎます。
