...
BLOB のスナップショットには、スナップショットの取得元となるベース BLOB と同じ名前が付いており、スナップショットが取得された時間を示す **DateTime** 値が付加されています。 たとえば、ページ blob の URI の場合は `http://storagesample.core.blob.windows.net/mydrives/myvhd`, 、スナップショットの URI はのようになります `http://storagesample.core.blob.windows.net/mydrives/myvhd?snapshot=2011-03-09T01:42:34.9360000Z`します。 すべての BLOB のスナップショットは URI を共有しており、この追加された **DateTime** 値のみによって識別されます。
