...
既定では、Storage サービスに対してストレージ メトリックは有効になっていません。 いずれかを使用した監視を有効にすることができます、 [Azure Classic Portal](manage.windowsazure.com), 、Windows PowerShell、またはストレージ API にプログラムを作成します。
...
ストレージ メトリックがストレージ サービスの動作の重要な変化を自動的に通知するように、Azure クラシック ポータルの [監視] ページでアラートを設定するようにしてください。ストレージ エクスプローラー ツールを使用してこのメトリック データを区切り形式でダウンロードすると、Microsoft Excel でデータを分析できます。 ブログの投稿を参照してください [Microsoft Azure ストレージ エクスプ ローラー](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) 使用可能記憶域エクスプ ローラー ツールの一覧です。
