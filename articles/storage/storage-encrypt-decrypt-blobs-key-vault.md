...
Azure Key Vault の概要については、次を参照してください。 [Azure Key Vault は何ですか。](key-vault/key-vault-whatis.md)

Azure Storage のクライアント側暗号化の概要については、次を参照してください [Microsoft Azure Storage のクライアント側の暗号化を使ってみる。](storage-client-side-encryption.md)
...
> [AZURE.NOTE] Key Vault オブジェクト モデル
>
...
## BLOB の暗号化とアップロード
次のコードを追加して BLOB を暗号化し、Azure ストレージ アカウントにアップロードします。  **ResolveKeyAsync** ために使用されるメソッドは、IKey を返します。
...
> [AZURE.NOTE] 他の種類の競合回避モジュールやすくするためにキー管理などのいくつかあります: AggregateKeyResolver および CachingKeyResolver です。
