...
Azure Key Vault の概要については、を参照してください [Azure Key Vault は何ですか?](key-vault/key-vault-whatis.md)。

Azure Storage のクライアント側暗号化の概要については、を参照してください [Microsoft Azure Storage のクライアント側の暗号化を使ってみる](storage-client-side-encryption.md)。
...
このチュートリアルを続行するためには、このチュートリアルで説明されている次の手順を行う必要がある  [Azure Key Vault を使ってみる](key-vault/key-vault-get-started.md):
...
次の追加 `を使用して` ステートメントおよび System.Configuration への参照をプロジェクトに追加するようにしてください。
...
次のスクリーン ショットは、 [Azure Classic Portal](manage.windowsazure.com) Key Vault に格納されたキーをクライアント側の暗号化を使用して暗号化されている blob のです。 **KeyId** プロパティは、Key Vault 内のキーに対する URI であり、KEK として機能します。 **EncryptedKey** プロパティには、暗号化されたバージョンの CEK が含まれます。
...
C# で Microsoft Azure Storage の使用に関する詳細については、次を参照してください。 [.NET 用 Microsoft Azure ストレージ クライアント ライブラリ](https://msdn.microsoft.com/library/azure/dn261237.aspx)します。

Blob REST API の詳細については、次を参照してください。 [Blob サービス REST API](https://msdn.microsoft.com/library/azure/dd135733.aspx)します。

Microsoft Azure Storage の最新の情報を参照してください、 [Microsoft Azure Storage チーム ブログ](http://blogs.msdn.com/b/windowsazurestorage/)します。
