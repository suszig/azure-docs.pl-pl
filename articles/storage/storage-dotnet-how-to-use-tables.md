...
プロパティは両方を公開するサポートされている型のパブリック プロパティである必要があります `get` と `set`です。
また、エンティティ型 *必要があります* パラメーターなしのコンス トラクターを公開します。
...
によって表される、 **TableOperation** オブジェクトです。  次のコード例の作成を示しています、 **CloudTable** オブジェクトし、 **CustomerEntity** オブジェクトです。  操作を準備する、 **TableOperation** 顧客エンティティをテーブルに挿入するオブジェクトを作成します。  呼び出して、操作を実行する最後に、 **CloudTable.Execute**します。
 ...
 **TableBatchOperation** を使用して、 **挿入** メソッドです。 次に、 **CloudTable.Execute** を呼び出して、操作を実行します。
...
テーブルに対してパーティション内のすべてのエンティティを照会する、 **TableQuery** オブジェクトです。
...
また **EntityResolver**します。 学びますプロジェクションの詳細について、 [Introducing Upsert とクエリ プロジェクションのブログ投稿][]します。 プロジェクションはローカル ストレージ エミュレーターではサポートされていません。したがって、このコードは Table サービスのアカウントを使用している場合にのみ機能します。
...
- Azure Storage を使用して作業を記述するコードを簡略化する方法を学習、 [Azure web ジョブ SDK](../app-service-web/websites-dotnet-webjobs-sdk-get-started.md)
