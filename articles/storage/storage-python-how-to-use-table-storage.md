...
このガイドでは、Azure Table ストレージ サービスを使用して一般的なシナリオを実行する方法について説明します。 サンプルは Python で記述され使用する、 [Python Azure ストレージ パッケージ][]します。 管理対象のシナリオは、作成および削除します。
...
[AZURE です。注意してください] Python をインストールする必要があるかどうか、または [Python Azure パッケージの][], を参照してください、 [Python インストール ガイド](../python-how-to-install.md)します。


## テーブルの作成

**TableService** オブジェクトを使用して、テーブル サービスを操作できます。 この
...
**PartitionKey** が同じエンティティは同じノードに格納されます。 **RowKey** パーティション内のエンティティの一意の id を指定します。
一意の ID です。
...
[AZURE です。メモ]、次のスニペットはクラウドに対してのみ機能します。
...
-   MSDN リファレンス [Azure ストレージの][]します。
-   参照してください、 [Azure Storage チーム ブログ][]します。
