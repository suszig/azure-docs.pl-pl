...
-   [データ系列パターン](#data-series-pattern) -要求の数を最小限に 1 つのエンティティで完全なデータ系列を格納します。  
...
次の例は両方の通知、 **PartitionKey** と **RowKey** 複合キーです。  **PartitionKey** 部署と従業員の両方の id を使用して複数のパーティションに、ログを配布します。  
...
エンティティの種類がわからなくても削除はできますが、挿入はできません。 ただし、使用することができます **DynamicTableEntity** 、型を把握したり POCO エンティティ クラスを使用せずにエンティティを更新する型。 次のコード サンプルは、単一のエンティティを取得し、チェック、 **EmployeeCount** プロパティを更新する前に存在します。  
