...
Aby utworzyć nową grupę zasobów za pomocą interfejsu wiersza polecenia platformy Azure, użyj polecenia [az group create](/cli/azure/group#create). 
...
Jeśli nie masz pewności, który region należy określić dla parametru `--location`, za pomocą polecenia [az account list-locations](/cli/azure/account#list) możesz pobrać listę obsługiwanych regionów dla swojej subskrypcji.
...
Aby utworzyć konto magazynu ogólnego przeznaczenia w witrynie Azure Portal, wykonaj następujące kroki:
...
3. Pozostaw następujące pola ustawione na ich wartości domyślne: **Model wdrażania**, **Rodzaj konta**, **Wydajność**, **Replikacja** i **Wymagany bezpieczny transfer**.
4. Wybierz subskrypcję, w ramach której chcesz utworzyć konto magazynu.
5. Z listy rozwijanej **Grupa zasobów** wybierz pozycję **Użyj istniejącej**, a następnie wybierz grupę zasobów utworzoną w poprzedniej sekcji.
6. Wybierz lokalizację nowego konta magazynu.
7. Kliknij pozycję **Utwórz**, aby utworzyć konto magazynu.      
...
Aby utworzyć konto magazynu ogólnego przeznaczenia z poziomu programu PowerShell, użyj polecenia [New-AzureRmStorageAccount](/powershell/module/azurerm.storage/New-AzureRmStorageAccount): 
...
Aby utworzyć konto magazynu ogólnego przeznaczenia z poziomu interfejsu wiersza polecenia platformy Azure, użyj polecenia [az storage account create](/cli/azure/storage/account#create).
...
Aby usunąć grupę zasobów i skojarzone z nią zasoby, w tym nowe konto magazynu, użyj polecenia [az group delete](/cli/azure/group#delete).
