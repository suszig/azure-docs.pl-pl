...
同じ時間内に作成されたログについて、重複するレコードが存在する場合があります。 かどうか、レコードが重複しているチェックを指定できます、 **RequestId** と **操作** 数。
...
| 属性         | 説明                                                                                                                                                                                   |
|----------------   |--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   |
| < サービス名 >    | ストレージ サービスの名前。 例: blob、table、または queue                                                                                                                          |
...
ストレージ要求がログに記録されるとき、生成されるログの名前は、要求された操作が完了した時刻が基準となります。 たとえば、GetBlob 要求が午後 6 時 30 分に完了した場合 2011 年 7 月 31 日が書き込まれるログのプレフィックス。 `blob/2011/07/31/1800/`
...
Storage Analytics を使用するには、監視するサービスごとに Storage Analytics を個別に有効にする必要があります。 有効にすることができます、 [Azure ポータル](portal.azure.com)します。 詳細については、「 [ストレージ アカウントの監視方法](../how-to-monitor-a-storage-account.md)します。 また、プログラムから REST API またはクライアント ライブラリを使用して有効にすることもできます。 [使用して Get Blob Service Properties、Get Queue Service Properties](https://msdn.microsoft.com/library/hh452239.aspx), 、および [Get Table Service Properties](https://msdn.microsoft.com/library/hh452238.aspx) 各サービスに対して Storage Analytics を有効にする操作。
...
たとえばを実行する場合、 **GetBlob** 、Blob サービス、Storage Analytics Metrics で操作が要求を記録され、Blob サービスの集計データに含めるだけでなく、 **GetBlob** 操作します。 ただし、ない場合 **GetBlob** 時間内に操作が要求されると、エンティティは書き込まれませんに、 `$MetricsTransactionsBlob` その操作用です。
...
>[AZURE.NOTE] 現時点では、容量メトリックでは、Blob サービスの使用のみです。 Table サービスと Queue サービスに関する容量メトリックは、Storage Analytics の将来のバージョンでの提供を予定しています。
...
- **容量**: バイト単位でのストレージ アカウントの Blob サービスによって使用されるストレージの量。

- **ContainerCount**: ストレージ アカウントの Blob サービス内の blob コンテナーの数。

- **ObjectCount**: ストレージ アカウントの Blob サービスでコミット済みとコミットされていないブロックまたはページ blob の数。
...
データ保持ポリシーを構成した場合、Storage Analytics が古いログ データやメトリック データを削除しますが、その際の削除トランザクションに対する料金は発生しません。 ただし、クライアントからの削除トランザクションは課金対象となります。 保有ポリシーの詳細については、次を参照してください。 [Storage Analytics のデータ保持ポリシーを設定する](https://msdn.microsoft.com/library/azure/hh343263.aspx)です。
