...
||すべてのサービス| Caching|    [繰り返し使用し、ほとんど変更されないデータをアプリケーションでキャッシュしていますか?](#subheading7)
||すべてのサービス| Caching|    [アプリケーションでバッチ更新 (クライアント側でキャッシュし、より大きなセットにアップロードする方法) を実行していますか?](#subheading8)
...
||テーブル|   非正規化|    [データを取得する際に非効率なクエリや複数の読み取り要求が回避されるように、データを非正規化していますか?](#subheading34)
...
||キュー|   一括取得|  [1 回の "Get" 操作で複数のメッセージを取得していますか?](#subheading41)
||キュー|   ポーリング頻度|  [アプリケーションの知覚可能な遅延を減らすために、ポーリング頻度は十分ですか?](#subheading42)
||キュー|   更新メッセージ| [エラー発生時にメッセージ全体の再処理が必要になる事態を回避するために、UpdateMessage を使用してメッセージ処理中の進捗を格納していますか?](#subheading43)
||キュー|   アーキテクチャ|   [キューを使用して、クリティカル パスから長期間のワークロードを外し、アプリケーション全体の拡張性を高めて、その後、個別に規模を設定していますか?](#subheading44)
...
-   アプリケーションがスケーラビリティ ターゲットに達する場合は、再試行に指数関数的バックオフを使用していることを確認して (を参照してください [再試行](#subheading14))。  望ましいのは、前の方法のいずれかを使用して、スケーラビリティ ターゲットに近づけないことですが、そうするとアプリケーションを迅速に再試行できなくなり、調整の質が低下します。  
...
####<a name="subheading4"></a>Location (場所)
...
.NET を使用して、最終更新日を検出する blob のプロパティを取得する方法の例は、次を参照してください。 [プロパティの取得およびメタデータの設定と](storage-properties-metadata.md)です。 条件付きダウンロードの詳細については、次を参照してください。 [Blob のローカル コピーを条件付きで更新](http://msdn.microsoft.com/library/azure/dd179371.aspx)します。  
...
詳細については、記事を参照して [のパフォーマンス向上概要 .NET 4.5 で](http://msdn.microsoft.com/magazine/hh882452.aspx)します。  
...
.NET を使用して blob のメタデータを取得する方法の例は、次を参照してください。 [プロパティの取得およびメタデータの設定と](storage-properties-metadata.md)です。  
...
Azure ストレージには、blob の 2 つの種類がサポートしています: *ページ* blob と *ブロック* blob です。 特定の使用シナリオでは、BLOB の種類の選択がソリューションのパフォーマンスとスケーラビリティに影響を及ぼします。 ブロック blob は大量のデータを効率的にアップロードする場合に適しています。たとえば、クライアント アプリケーションが BLOB ストレージに写真やビデオをアップロードする必要がある場合です。 ページ blob はアプリケーションがランダム書き込みを実行する必要がある場合に適しています。たとえば、Azure VHD がページ blob として格納されている場合です。  
...
使用するテーブル **Upsert** 操作可能な場合です。 2 種類があります **Upsert**, 、どちらも、従来よりも効率的 **挿入** と **更新** 操作。  

-   **InsertOrMerge**: このエンティティのプロパティのサブセットをアップロードするときに使用が所属していないことを確認して、エンティティが既に存在するかどうか。 この呼び出しに含まれるプロパティを更新するエンティティが存在する場合、 **Upsert** 操作、既存のすべてのプロパティは、エンティティが存在しない場合、新しいエンティティを挿入します。 変更するプロパティのアップロードだけが必要とされる点が、クエリにおけるプロジェクションの使用に類似しています。
-   **InsertOrReplace**:、まったく新しいエンティティをアップロードする場合は、既に存在するかどうかがわからない場合に使用します。 この方法は、以前のエンティティを全面的に上書きするため、新たにアップロードするエンティティに誤りが無いことが確実な場合にのみ使用してください。 たとえば、アプリケーションが以前にユーザーの場所データを保存しているかどうかにかかわらず、ユーザーの現在の場所を格納しているエンティティを更新する必要があり、その新しい場所エンティティに問題がなく、かつ以前のすべてのエンティティが持つあらゆる情報を必要としない場合などです。
...
または、アプリケーションは、1 つのエンティティの個々 のプロパティとして 1 時間ごとの CPU 使用率を格納可能性があります: 1 時間ごとに更新するには、アプリケーションが、1 つを使用できます **InsertOrMerge Upsert** 、最新の時間分の値を更新するとき。 データをプロットするアプリケーションのみに必要な 24、ため、非常に効率的なクエリではなく 1 つのエンティティを取得 (についてディスカッション上 [クエリ スコープ](#subheading30))。
...
###< 名前 = subheading44"></a>UpdateMessage
使用する **UpdateMessage** 非常時タイムアウトを長くまたはメッセージの状態情報を更新します。 これは効果的ですが、各記憶 **UpdateMessage** 操作はスケーラビリティ ターゲットにカウントします。 しかし、それでもジョブの手順が完了するごとに、次のキューにジョブを渡すワークフローに比べると、はるかに効率的な方法です。 使用して、 **UpdateMessage** 操作により、アプリケーションをジョブの状態をメッセージに保存し、手順が完了するたびに、次のステップのジョブのメッセージをキューではなく、作業を続行します。  

詳細については、記事を参照してください。 [方法: キューに置かれたメッセージの内容を変更する](../storage-dotnet-how-to-use-queues/#change-contents)です。  

###< 名前 = subheading45"></a>アプリケーションのアーキテクチャ
