...
または、アプリケーションは、単一のエンティティの個々 のプロパティとして 1 時間ごとの CPU 使用率を格納可能性があります: 1 時間ごとに更新するには、アプリケーションが、1 つを使用できます **InsertOrMerge Upsert** 、最新の時間分の値を更新するとき。 データをプロットするアプリケーションのみに必要な 24、ため、非常に効率的なクエリではなく、単一のエンティティを取得 (についてディスカッション上 [クエリ スコープ](#subheading30))。
...
###< 名前 = subheading45"></a>アプリケーション アーキテクチャ
