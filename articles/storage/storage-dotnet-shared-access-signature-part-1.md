...
- **アカウント SAS。** アカウント SAS では、1 つ以上のストレージ サービスのリソースへのアクセスを委任できます。 サービス SAS を使用して実行できるすべての操作は、アカウント SAS でも実行できます。 さらに、SA アカウントでアクセスを委任など、特定のサービスに適用される操作に **サービスのプロパティを取得または設定** と **Get Service Stats**します。 サービス SAS で許可されていない BLOB コンテナー、テーブル、キューおよびファイル共有の読み取り、書き込みおよび削除操作へのアクセスも委任できます。 参照してください [アカウント SAS を構築する](https://msdn.microsoft.com/library/mt584140.aspx) アカウントの SAS トークンを構築する方法の詳細についてです。

- **サービス SAS。** サービス SAS は、1 つのストレージ サービス (BLOB、Queue、Table、または File サービスのいずれか) のリソースへのアクセスを委任します。 参照してください [サービスの SAS を構築する](https://msdn.microsoft.com/library/dn140255.aspx) と [Service SAS 例](https://msdn.microsoft.com/library/dn140256.aspx) サービスの SAS トークンの作成の詳細についてです。
...
- **IP。** IP アドレスまたは Azure の外部での範囲の IP アドレスを指定する省略可能なパラメーター (セクションを参照して [セッション構成の状態をルーティング](../expressroute/expressroute-workflows.md#routing-session-configuration-state) Express Route の) 元となる要求を受け入れるため。 
...
10. **Storage Analytics を使用してアプリケーションを管理します。** SAS プロバイダー サービスが中断したり、保存されているアクセス ポリシーを不注意で削除したりしたために発生する認証失敗の急増を、ログやメトリックを使用して監視できます。 参照してください、 [Azure Storage チーム ブログ](http://blogs.msdn.com/b/windowsazurestorage/archive/2011/08/03/windows-azure-storage-logging-using-logs-to-track-storage-requests.aspx) の詳細。
