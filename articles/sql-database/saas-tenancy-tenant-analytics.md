...
- Skrypty Wingtip SaaS i aplikację [kod źródłowy](https://github.com/Microsoft/WingtipSaaS) są pobierane z usługi GitHub. Zobacz instrukcje pobierania. Upewnij się, *odblokować plik zip* przed wyodrębniania jego zawartość.
...
1. W programie SSMS, nawiąż połączenie z **jobaccount** bazy danych w katalogu -&lt;użytkownika&gt;.
...
1. W programie SSMS, nawiąż połączenie z **jobaccount** bazy danych w katalogu -<User>serwera.
...
### <a name="create-views-that-aggregate-data"></a>Utwórz widoki, które agregowanie danych

Dane w tabeli Schemat gwiazdy zawiera wszystkie biletu sprzedaży dane potrzebne do analizy.  Aby ułatwić Zobacz trendów w danych, musisz agregacji danych. Można zdefiniować widoki, które ułatwiają wysyłać zapytania o dane i udostępniającym szczegółowe, przydatne informacje. Kolejnych kroków utworzyć cztery następujące widoki:
...
Widoki te są używane w kroku wizualizacji danych. Wstępne tworzenie tych widoków pomaga mniej doświadczeni użytkownicy, co ułatwia im scalenia wizualizacje przydatnych danych.

1. W programie SSMS, nawiąż połączenie z **tenantanalytics** przechowywane w katalogu<User>
2. W programie SSMS *...\Learning Modules\Operational Analytics\Tenant Analytics\DailySales.sql*.
3. Naciśnij klawisz **F5** do uruchom odpowiedni skrypt, który tworzy cztery widoki i wysyła zapytanie do ich zawartości.
    - CumulativeDailySalesByEvent składa się z zagregowane sprzedaży dla poszczególnych dni w ciągu poprzednich dwóch miesięcy dla wszystkich zdarzeń.
    - TicketSalesDistribution przedstawia średnią, jak również całkowitej sprzedaży dla wszystkich miejsc.
    - TicketsSoldVersusSaleDay przedstawia sumę sprzedanych w każdym dniu sprzedaży przez 60 dni przed zdarzeniem biletów.
    - TotalSalesPerDay przedstawia łączna sprzedaż na dzień.
...
Wykresy ułatwiają trendy w dużych zestawach danych. W tej sekcji zostanie przedstawiony sposób użycia **usługi Power BI** do manipulowania i wizualizacji danych dzierżawy zostały wyodrębnione i organizowane.
...
4. W oknie nazwy logowania bazy danych, wprowadź nazwę serwera (katalogu -&lt;użytkownika&gt;. database.windows.net). Wybierz **importu** dla **tryb łączności danych**, a następnie kliknij przycisk OK. 
...
6. W **Nawigator** okienku w obszarze bazy danych analizy, wybierz CumulativeDailySalesByEvent, TicketSalesDistribution, TicketsSoldVersusSaleDay, TotalSalesPerDay. Następnie wybierz **obciążenia**. 
...
Aby przejść do szczegółów w danych ponownie, aby sprawdzać, czy ten mad łazienkowych obowiązuje dla wszystkich zdarzeń obsługiwanych przez te miejsc. W poprzednich powierzchni obserwowanych że Hall porozumieniu Contoso sprzedaje partii biletów i że Contoso również ma kolekcji w sprzedaży biletów w określone dni. Wybierz następujące opcje usługi Power BI do wykreślenia biletu zbiorcza sprzedaży Hall porozumieniu firmy Contoso, koncentrujących się na trendy sprzedaży dla każdego z jego zdarzeń. 
