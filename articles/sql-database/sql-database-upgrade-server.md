...
> [AZURE.IMPORTANT] 以降の Azure PowerShell 1.0 のプレビュー リリースでは、Switch-azuremode コマンドレットは、不要になったと、Azure ResourceManger モジュールに含まれていたコマンドレットの名前が変更されています。 この記事の例では、新しい PowerShell 1.0 Preview の名付け規則が使用されています。 詳細については、次を参照してください。 [非推奨の Switch-azuremode Azure PowerShell で](https://github.com/Azure/azure-powershell/wiki/Deprecation-of-Switch-AzureMode-in-Azure-PowerShell)します。
...
**警告:** アップグレードしたデータベースの DTU 消費が特定の高レベルに近づいたときに通知するため、Azure ポータルで 'Alerts' を設定します。 データベース アラートは、Azure ポータルで、DTU、CPU、IO、ログといったさまざまなパフォーマンス指標に対して設定できます。 データベースとを選択するには、「参照」 **アラート ルール** で、 **設定** ブレードです。
