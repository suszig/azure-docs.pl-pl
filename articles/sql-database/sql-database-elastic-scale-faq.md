...
参照してください * *[シャードを elastic database クライアント ライブラリを使用してアプリケーションに追加する](sql-database-elastic-scale-add-a-shard.md)* *。
...
番号  **分割** 操作、ターゲット データベースは、適切なスキーマに存在する必要があり、シャード マップ マネージャーに登録されています。 **結合**操作の場合、シャード マップ マネージャーからシャードを削除してから、データベースを削除する必要があります。
