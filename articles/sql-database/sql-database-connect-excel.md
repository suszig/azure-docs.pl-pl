...
2.  ページの上部にあるメニュー バーで、クリックして **データ**, 、] をクリックして **その他のソース**, 、] をクリックし、 **SQL Server から**します。

    ![データ ソースの選択: SQL データベースに Excel を接続します。](./media/sql-database-connect-excel/excel_data_source.png)

    データ接続ウィザードが開きます。

3.   **データベース サーバーへの接続** ダイアログ ボックスで、種類、 **サーバー名** フォーム内に接続する論理サーバーをホストする **<*servername*>. database.windows.net** します。 たとえば、 **adventureserver.database.windows.net**します。

4.   **ログオン資格情報** ] をクリックして **次のユーザー名とパスワードを使用して**, 、型、 **ユーザー名** と **パスワード** 作成したとき、SQL データベース サーバーを設定し、順にクリックして **次**します。
...
5.  **データベースの選択とテーブル** ダイアログ ボックスで、選択、 **AdventureWorks** プルダウン メニューからデータベースし、選択 **[vGetAllCategories** にテーブルとビューのリストをクリックし、 **次**します。

    ![データベースとテーブルを選択します。][5]

6.  **データ接続ファイルを保存し、[完了]** ダイアログ ボックスで、をクリックして **完了**します。
