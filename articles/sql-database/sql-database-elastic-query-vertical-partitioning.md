...
このドキュメントでは、データベース間クエリ シナリオ用にエラスティック クエリ (列方向のパーティション分割) を設定する方法とクエリを実行する方法について説明します。 垂直方向のパーティション分割のシナリオの定義では、次を参照してください。 [Azure SQL Database エラスティック データベース クエリの概要 (プレビュー)](sql-database-elastic-query-overview.md)します。
...
**注**: とは異なり水平パーティション分割を使用した場合、これらの DDL ステートメントに依存しない、エラスティック データベース クライアント ライブラリを介してシャード マップとデータ層を定義します。 
...
いることを確認、 *< ユーザー名 >* どれも含まない *"@servername"* サフィックスです。 
...
* **スキーマ**: 外部テーブル DDL は、クエリを使用するスキーマを定義します。 外部テーブル定義に指定するスキーマは、実際のデータが格納されているリモート データベース内のテーブルのスキーマに一致する必要があります。 

* **リモートのデータベース参照**: 外部テーブル DDL は、外部データ ソースを参照します。 外部データ ソースは、実際のテーブル データが格納されているリモート データベース論理サーバー名とデータベース名を指定します。 
...
SCHEMA_NAME 句と OBJECT_NAME 句は、外部テーブル定義をリモート データベース上の別のスキーマのテーブルまたは別の名前を持つテーブルにマップする機能をそれぞれ提供します。 これは、リモート データベースのカタログ ビューまたは DMV に対して外部テーブルを定義する場合や、それ以外のリモート テーブル名が既にローカルに取得されている状況に便利です。  

次の DDL ステートメントは、ローカル カタログから既存の外部テーブル定義を削除します。 リモート データベースには影響しません。 

    DROP EXTERNAL TABLE [ database_name . [ schema_name ] . | schema_name. ] table_name[;]  

**CREATE/DROP EXTERNAL TABLE のアクセス許可**: これは、基になるデータ ソースを参照する必要も DDL の外部テーブルの ALTER ANY EXTERNAL DATA SOURCE のアクセス許可が必要です。  
...
**垂直的パーティション分割の例を**: 次のクエリは、顧客の注文および注文明細行の 2 つのローカル テーブルと、リモート テーブルの間で 3 方向結合を実行します。 これは、エラスティック クエリの参照データのユース ケースの例を示します。 
