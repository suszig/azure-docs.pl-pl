...
* シャード セット (を使用して作成 [Elastic Database クライアント ライブラリ](sql-database-elastic-database-client-library.md))します。
...
1.  **エラスティック データベース ジョブ** コンポーネントをインストールします。 詳細については、次を参照してください。 [弾力性データベースのインストール ジョブ](sql-database-elastic-jobs-service-installation.md)します。 インストールが失敗した場合を参照してください。 [をアンインストールする方法](sql-database-elastic-jobs-uninstall.md)します。
...
5.  Azure ポータルを使用してジョブを作成する次の手順: [の作成と弾力性データベース ジョブを管理する](sql-database-elastic-jobs-create-and-manage.md)します。
6.  PowerShell スクリプトを使用または: [作成 PowerShell (プレビュー) を使用して SQL Database エラスティック データベース ジョブの管理と](sql-database-elastic-jobs-powershell.md)します。

## べき等スクリプトの重要性

スクリプトである必要があります [アイデムポ テントな](https://en.wikipedia.org/wiki/Idempotence)します。 簡単に言えば、「べき等」とは、スクリプトが成功し、再度実行されるとき、結果が同じになることを意味します。 一時的なネットワークの問題により、スクリプトが失敗することがあります。 その場合、ジョブは事前に設定した回数に達するまで自動的にスクリプトを再試行します。 べき等スクリプトは、2 回実行して成功した場合、結果が同じになります。
...
* **Azure SQL Database**: このサービスは、**管理データベース**と呼ばれる Azure SQL Database を使用してすべてのジョブ メタデータを保持します。 既定のサービス層は、S0 です。 料金については、次を参照してください。 [SQL Database の料金](http://azure.microsoft.com/pricing/details/sql-database/)します。
* **Azure Service Bus**: Azure Service Bus は、Azure Cloud Service 内の作業を調整します。 参照してください [Bus の料金をサービス](http://azure.microsoft.com/pricing/details/service-bus/)します。
* **Azure Storage**: 問題にさらにデバッグを必要とする診断出力のログを格納する Azure ストレージ アカウントが使用される (一般的な方法 [Azure 診断](cloud-services-dotnet-diagnostics.md))します。 料金については、次を参照してください。 [Azure Storage 料金](http://azure.microsoft.com/pricing/details/storage/)します。
...
[コンポーネントをインストール](sql-database-elastic-jobs-service-installation.md), 、し [作成し、データベースのグループ内の各データベースへのログを追加](sql-database-elastic-jobs-add-logins-to-dbs.md)します。 ジョブの作成と管理の詳細についてを参照してください。 [を作成すると、弾力性データベース ジョブを管理する](sql-database-elastic-jobs-create-and-manage.md)します。
