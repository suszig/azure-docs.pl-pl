...
<a id="bkmk_connection_errors" name="bkmk_connection_errors">& nbsp;</a>
...
- データベース <db_name> サーバーで <Azure_instance> は現在使用できません。後で接続を再試行してください。カスタマー サポートに問い合わせて、問題が解決しない場合、およびのセッション トレース ID を控えてください <session_id>

- データベース <db_name> サーバーで <Azure_instance> は現在使用できません。後で接続を再試行してください。カスタマー サポートに問い合わせて、問題が解決しない場合、およびのセッション トレース ID を控えて <session_id>します。(Microsoft SQL Server、エラー: 40613)
...
- [クライアント開発とクイック スタート コード サンプルを SQL データベース](sql-database-develop-quick-start-client-code-samples.md)

- [SQL データベースの接続エラーと一時的なエラーを解決する操作](sql-database-connectivity-issues.md)
...
| 40197| 17| 要求の処理中にサービスでエラーが発生しました。もう一度実行してください。エラー コード %d。<br/><br/>ソフトウェアまたはハードウェアのアップグレード、ハードウェアの障害、またはその他のフェールオーバーの問題によって、サービスがダウンしている場合、このエラーを受け取ります。障害の種類や発生したフェールオーバーに関する詳細な情報は、エラー 40197 のメッセージに埋め込まれたエラー コード (%d) から得られます。エラー 40197 のメッセージ内でコードが埋め込まれているエラーの例をいくつかは、40020、40143、40166、40540 です。<br/><br/>SQL データベースに再接続するサーバーが自動的に接続するデータベースの正常なコピーをします。アプリケーションでエラー 40197 をキャッチし、メッセージに埋め込まれているエラー コード (%d) をログに記録してトラブルシューティングに備えたうえで、リソースが復旧して接続が再度確立されるまで SQL Database への再接続を試みる必要があります。|
| 40501| 20| サービスは現在ビジー状態です。10 秒後に要求を再試行してください。インシデント ID: %ls。コード: %d。<br/><br/>*注:* 詳細については、を参照してください:<br/>• [Azure SQL Database のリソース制限](sql-database-resource-limits.md)します。
...
| 49920| 16| 要求を処理できません。サブスクリプション用に進行中の操作が多すぎる"% %ld"です。<br/><br/>、サービスがこのサブスクリプションに対する複数の要求の処理でビジー状態です。現在、要求はリソースの最適化のためにブロックされています。クエリ [sys.dm_operation_status](https://msdn.microsoft.com/library/dn270022.aspx) 操作の状態にします。保留中の要求が完了するまで待つか、いずれかの保留中の要求を削除して後で要求を再試行します。|


<a id="bkmk_b_database_copy_errors" name="bkmk_b_database_copy_errors">& nbsp;</a>

## データベース コピー エラー

以下の表は、Azure SQL Database でデータベースをコピーするときに発生する可能性のある各種エラーの一覧です。 詳細については、次を参照してください。 [Azure SQL データベースをコピー](sql-database-copy.md)します。
...
<a id="bkmk_c_resource_gov_errors" name="bkmk_c_resource_gov_errors">& nbsp;</a>
...
| エラー番号| 重大度| 説明|
|---:|---:|:---|
| 10928| 20| リソース ID: %d。データベースの %s 制限の %d に達しました。詳細については、次を参照してください [http://go.microsoft.com/fwlink/?LinkId=267637](http://go.microsoft.com/fwlink/?LinkId=267637)。<br/>。<br/>リソース ID が限度に達したリソースを示します。ワーカー スレッドの場合、リソース ID = 1 となります。セッションのリソース ID = 2 です。<br/><br/>*注:* このエラーおよび問題を解決する方法の詳細については、次を参照してください。:<br/>• [Azure SQL Database のリソース制限](sql-database-resource-limits.md)します。|
| 10929| 20| リソース ID: %d。%S の最低限保証は %d、最大値は %d 、データベースの現在の使用状況は %d です。ただし、サーバーは現在ビジー状態であり、このデータベースの %d を超える要求をサポートできません。詳細については、次を参照してください。 [http://go.microsoft.com/fwlink/?LinkId=267637](http://go.microsoft.com/fwlink/?LinkId=267637)します。それ以外の場合、もう一度やり直してください。<br/><br/>リソース ID が限度に達したリソースを示します。ワーカー スレッドの場合、リソース ID = 1 となります。セッションのリソース ID = 2 です。<br/><br/>*注:* このエラーおよび問題を解決する方法の詳細については、次を参照してください。:<br/>• [Azure SQL Database のリソース制限](sql-database-resource-limits.md)します。|
...
| 40551| 16| 過剰なため、セッションを終了しました `TEMPDB` 使用します。一時テーブルの領域使用率を低減してクエリを変更してください。<br/><br/>*ヒント:* 一時オブジェクトを使用している場合は、内の領域を節約、 `TEMPDB` セッションで不要になったに必要な一時オブジェクトを削除することによってデータベースです。|
| 40552| 16| トランザクション ログの使用領域が多すぎるため、セッションを終了しました。単一のトランザクションで行を減らしてを変更してください。<br/><br/>*ヒント:* 一括挿入を使用して実行する場合、 `bcp.exe` ユーティリティまたは `System.Data.SqlClient.SqlBulkCopy` クラスを使用してください、 `b batchsize` または `BatchSize` 行の数を制限するオプションは、各トランザクションでサーバーにコピーします。インデックスを再構築する場合、 `ALTER INDEX` ステートメントを使用してください、 `オンラインで再構築 = ON` オプション。|
| 40553| 16| メモリの使用量が多すぎるため、セッションを終了しました。少ない行を処理するクエリを変更してください。<br/><br/>*ヒント:* の数を減らす `ORDER BY` と `GROUP BY` TRANSACT-SQL コードでの操作は、クエリのメモリ要件を軽減します。|
...
<a id="bkmk_e_general_errors" name="bkmk_e_general_errors">& nbsp;</a>
...
| 40553| 16| メモリの使用量が多すぎるため、セッションを終了しました。少ない行を処理するクエリを変更してください。<br/><br/>*注:* の数を減らす `ORDER BY` と `GROUP BY` TRANSACT-SQL コードでの操作では、クエリのメモリ要件を減らすことができます。|
...
| 40617| 16| ファイアウォール規則の名前で始まる <rule name> が長すぎます。最大長は 128 です。|
...
| 40638| 16| 無効なサブスクリプション id <subscription-id>します。サブスクリプションが存在しません。|
| 40639| 16| 要求がスキーマに準拠していません: <schema error>します。|
...
| 40645| 16| Servername <servername> 空または null にすることはできません。小文字アルファベットの 'a' ～ 'z'、数字の 0 ～ 9、およびハイフンでのみ構成されている必要があります。名前の先頭や末尾にハイフンを使用することはできません。|
...
| 40650| 16| サブスクリプション <subscription-id> が存在しないか、操作の準備ができていません。|
| 40651| 16| サーバーを作成するために失敗しましたサブスクリプション <subscription-id> は無効になります。|
| 40652| 16| サーバーを移動または作成できません。サブスクリプション <subscription-id> サーバーのクォータを超えます。|
| 40671| 17| ゲートウェイと管理サービスの間で通信エラーが発生しました。後で再試行してください。|
| 40852| 16| データベースを開けません。 ' % です。*サーバーの ' %*。%ls はログインで要求します。データベースへのアクセスは、セキュリティが有効な接続文字列を使用した場合のみ許可されます。このデータベースにアクセスする変更を含めるために接続文字列 'セキュリティで保護された'、サーバーの FQDN の [サーバー名' *servername*.database.windows.net を指定で .net を変更して [サーバー名'.database します。`セキュリティで保護された`. 準備完了です。|
