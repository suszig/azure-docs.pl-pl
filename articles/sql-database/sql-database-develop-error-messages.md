...
|49919|16|要求を処理、作成、更新できません。 サブスクリプション "%ld" に対して実行中の作成または更新の操作が多すぎます。<br/><br/>サービスは、サブスクリプションまたはサーバーに対する複数の作成要求または更新要求の処理でビジー状態です。 現在、要求はリソースの最適化のためにブロックされています。 クエリ [sys.dm_operation_status](https://msdn.microsoft.com/library/dn270022.aspx) の保留中の操作です。 保留中の作成要求または更新要求が完了するまで待つか、いずれかの保留中の要求を削除して後で要求を再試行します。 |
|49920|16|要求を処理できません。 サブスクリプション "%ld" に対して実行中の操作が多すぎます。<br/><br/>サービスは、このサブスクリプションに対する複数の要求の処理でビジー状態です。 現在、要求はリソースの最適化のためにブロックされています。 クエリ [sys.dm_operation_status](https://msdn.microsoft.com/library/dn270022.aspx) 操作の状態にします。 保留中の要求が完了するまで待つか、いずれかの保留中の要求を削除して後で要求を再試行します。 |
...
|エラー番号|重大度|説明|
|---:|---:|:---|
|10928|20|リソース ID: %d。 データベースの %s 制限の %d に達しました。 詳細については、次を参照してください [http://go.microsoft.com/fwlink/?LinkId=267637](http://go.microsoft.com/fwlink/?LinkId=267637)。<br/><br/>。リソース ID では、上限に達したリソースを示します。 ワーカー スレッドの場合、リソース ID = 1 となります。 セッションのリソース ID = 2<br/><br/>*。注:* このエラーおよび問題を解決する方法の詳細については、次を参照してください。:<br/>• [Azure SQL Database のリソース制限](sql-database-resource-limits.md)します。 |
|10929|20|リソース ID: %d。 %S の最低限保証は %d、最大値は %d 、データベースの現在の使用状況は %d です。 ただし、サーバーは現在ビジー状態であり、このデータベースの %d を超える要求をサポートできません。 詳細については、次を参照してください。 [http://go.microsoft.com/fwlink/?LinkId=267637](http://go.microsoft.com/fwlink/?LinkId=267637)します。 それ以外の場合は、後でもう一度お試しください。<br/><br/>リソース ID は、制限に達したリソースを示します。 ワーカー スレッドの場合、リソース ID = 1 となります。 セッションのリソース ID = 2<br/><br/>*。注:* このエラーおよび問題を解決する方法の詳細については、次を参照してください。:<br/>• [Azure SQL Database のリソース制限](sql-database-resource-limits.md)します。|
...
|40551|16|`TEMPDB` の使用領域が多すぎるため、セッションを終了しました。 一時テーブルの領域使用率を低減してクエリを変更してください<br/><br/>*。ヒント:* 一時オブジェクトを使用している場合は、内の領域を節約、 `TEMPDB` セッションで不要になったに必要な一時オブジェクトを削除することによってデータベースです。|
|40552|16|トランザクション ログの使用領域が多すぎるため、セッションを終了しました。 単一のトランザクションで行を減らしてを変更してください<br/><br/>*。ヒント:* 一括挿入を使用して実行する場合、 `bcp.exe` ユーティリティまたは `System.Data.SqlClient.SqlBulkCopy` クラスを使用してください、 `-b batchsize` または `BatchSize` 行の数を制限するオプションは、各トランザクションでサーバーにコピーします。 `ALTER INDEX` ステートメントでインデックスを再構築する場合は、`REBUILD WITH ONLINE = ON` オプションの使用を検討してください。|
