...
**レビュー担当者**: Conor Cunningham、Michael Thomassy
...
>[AZURE.NOTE] 結果はベンチマークではなくが、表示するのには、 **相対的なパフォーマンス**します。 計時結果は、10 回以上のテスト ランの平均値に基づいています。 空のテーブルへの挿入操作を計測対象としています。 これらのテスト測定 V12 より前で、新しい V12 データベースで発生する可能性がスループットに必ずしも対応しない [サービス階層](sql-database-service-tiers.md)します。 それでもバッチ処理手法の相対的なメリットに大きな違いはないと考えられます。
...
実際には、両方の例にトランザクションが使われています。 1 つ目の例では、個々の呼び出しがそれぞれ暗黙的なトランザクションとなっています。 2 つ目の例では、トランザクションを明示的に指定して、その中にすべての呼び出しを投入しています。 については、ドキュメントごと、 [先行書き込みトランザクション ログ](https://msdn.microsoft.com/library/ms186259.aspx), 、トランザクションのコミット時に、ログ レコードがディスクにフラッシュします。 したがって、より多くの呼び出しをトランザクションに含めることで、その分、トランザクションがコミットされるまでの間、トランザクション ログへの書き込みを先送りすることができます。 実質的に、サーバーのトランザクション ログに対する書き込みに関して、バッチ処理を有効にしたことになります。
...
>[AZURE.NOTE] 結果は、ベンチマークではできません。 参照してください、 [ここでタイミングの結果に関する注意事項](#note-about-timing-results-in-this-topic)します。
...
トランザクションの使用は、パフォーマンスを向上させることができます、引き続き [トランザクションおよび接続のベスト プラクティスに従います](https://msdn.microsoft.com/library/ms187484.aspx)します。 トランザクションはできるだけ短く保ち、作業が完了したらデータベース接続を閉じるようにしてください。 前の例のように using ステートメントを使うことで、後続のコード ブロックの完了時に接続が確実にクローズされます。
...
ADO.NET でのトランザクションの詳細については、次を参照してください。 [ADO.NET でローカル トランザクション](https://msdn.microsoft.com/library/vstudio/2k2hy99x.aspx)します。

### テーブル値パラメーター
テーブル値パラメーターは、Transact-SQL のステートメント、ストアド プロシージャ、関数の中で、ユーザー定義テーブル型をパラメーターとして使用する際に使用します。 これはクライアント側のバッチ処理手法で、テーブル値パラメーターに複数行のデータを含めて送信することができます。 テーブル値パラメーターを使用するにはまず、テーブル型を定義します。 次の TRANSACT-SQL ステートメントの作成という名前のテーブル型 **MyTableType**します。
...
作成するコードでは、 **DataTable** まったく同じ名前とテーブル型の型を使用します。 これを渡す **DataTable** テキスト クエリまたはストアド プロシージャのパラメーターでを呼び出します。 この手法を使った例を次に示します。
...
前の例で、 **SqlCommand** オブジェクトは、テーブル値パラメーターから行を挿入 **@TestTvp**します。 以前に作成した **DataTable** でこのパラメーターにオブジェクトが割り当てられている、 **SqlCommand.Parameters.Add** メソッドです。 これらの挿入操作を 1 回の呼び出しでバッチ処理すれば、挿入操作を逐次的に実行した場合と比べ、大幅にパフォーマンスを高めることができます。

先ほどの例をさらに改良するには、テキスト ベースのコマンドをストアド プロシージャに置き換えます。 次の TRANSACT-SQL コマンドを受け取るストアド プロシージャの作成、 **SimpleTestTableType** テーブル値パラメーターです。
...
変更、 **SqlCommand** オブジェクトの次に、上記のコード例で宣言します。
...
テーブル値パラメーターよりも一括コピーの方が好ましいケースもいくつかあります。 テーブル値パラメーターのトピックで BULK INSERT 操作との比較表を参照して [テーブル値パラメーター](https://msdn.microsoft.com/library/bb510489.aspx)します。

次のアドホック テストの結果でバッチ処理のパフォーマンスを表示する **SqlBulkCopy** (ミリ秒単位)。
...
テーブル値パラメーターを使用するには、次のようなバッチ サイズが小さい、 **SqlBulkCopy** クラスです。 ただし、 **SqlBulkCopy** 1,000 行と 10,000 行のテストのテーブル値パラメーターよりも高速に 12 ~ 31% を実行します。 テーブル値パラメーターと同様に **SqlBulkCopy** 挿入のバッチ処理に適切なオプションは、非バッチ操作のパフォーマンスと比較した場合に特にです。

ADO.NET の一括コピーの詳細については、次を参照してください。 [SQL Server でのバルク コピー操作](https://msdn.microsoft.com/library/7ek5da1a.aspx)します。
...
### DataAdapter
 **DataAdapter** クラスでは、変更することができます、 **データセット** オブジェクトし、変更を INSERT、UPDATE、および削除操作として送信します。 使用している場合、 **DataAdapter** この方法では個別の操作ごとに呼び出しが行われることに注意してください。 パフォーマンスを向上させるには、使用、 **UpdateBatchSize** プロパティを一度にバッチ処理する必要があります操作数にします。 詳細については、次を参照してください。 [Dataadapter によるバッチ操作の実行](https://msdn.microsoft.com/library/aadf8fk2.aspx)します。

### Entity framework
Entity Framework では現在、バッチ処理がサポートされていません。 さまざまな開発者コミュニティでは、上書きなどの回避策を試みています、 **SaveChanges** メソッドです。 しかし、その解決策は複雑であることが多く、また、アプリケーションやデータ モデルに合わせてカスタマイズする必要があります。 この機能のリクエストに関して、Entity Framework CodePlex プロジェクトが現在、意見交換のページを開設しています。 このディスカッションを表示するには、次を参照してください。 [設計に関するミーティングのメモ – 2012 年 8 月 2 日](http://entityframework.codeplex.com/wikipage?title=Design%20Meeting%20Notes%20-%20August%202%2c%202012)します。
...
コマンドを使用する場合は、テーブル値パラメーターは、ストアド プロシージャを使用して、 **SET NOCOUNT ON** プロシージャの先頭にします。 通常、プロシージャで処理された行数が返されますが、このステートメントを記述することで、その出力を抑制することができます。 ただし、テストの結果での使用 **SET NOCOUNT ON** 効果がありませんでしたか、パフォーマンスが低下します。 テストのストアド プロシージャは、1 つのシンプルな **挿入** コマンドをテーブル値パラメーターです。 もっと複雑なストアド プロシージャであれば、このステートメントが有利に作用する可能性があります。 追加するとは限らないが、 **SET NOCOUNT ON** 、ストアド プロシージャに自動的に、パフォーマンスが向上します。 影響を理解するには、テスト、ストアド プロシージャとそうでない、 **SET NOCOUNT ON** ステートメントです。
...
次のコード例では [Reactive Extensions - Rx](https://msdn.microsoft.com/data/gg577609) 監視クラスによって発生してバッファー内のイベントを処理します。 バッファーがいっぱいになるか指定した時間が経過したら、テーブル値パラメーターを使って、ユーザー データのバッチをデータベースに送信します。
...
データベースに格納するユーザー ナビゲーション データをバッファリングするのは、NavHistoryDataMonitor クラスの役割です。 させることで応答する RecordUserNavigationEntry、メソッドが含まれている、 **OnAdded** イベントです。 Rx を使用し、このイベントに基づいて、監視可能なコレクションを作成するコンストラクターのロジックを示したのが以下のコードです。 この監視可能なコレクションを Buffer メソッドでサブスクライブします。 このメソッドのオーバーロードを使用して、バッファーへの送信間隔を 20 秒または 1000 エントリに指定しています。
