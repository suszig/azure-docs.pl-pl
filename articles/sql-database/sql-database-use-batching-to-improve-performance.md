 <properties
    pageTitle =「バッチ処理を使用して Azure SQL データベース アプリケーションのパフォーマンスを向上させる方法」
    description ="をバッチ処理のデータベース操作に証拠を提供するトピック imroves 速度が大幅にし、Azure SQL データベース アプリケーションのスケーラビリティです。 でもこれらのバッチ処理手法では、他の SQL Server データベースが動作、記事の主眼は Azure 上です。"
    サービス ="sql データベース]
...
>[AZURE.NOTE] 結果はベンチマークではなく、**相対的なパフォーマンス**を示すことを意図したものです。 計時結果は、10 回以上のテスト ランの平均値に基づいています。 空のテーブルへの挿入操作を計測対象としています。 これらのテスト測定 V12 より前で、新しい V12 データベースで発生する可能性がスループットに必ずしも対応しない [サービス階層](sql-database-service-tiers.md)します。 それでもバッチ処理手法の相対的なメリットに大きな違いはないと考えられます。
...
>[AZURE.NOTE] 結果はベンチマークではありません。 参照してください、 [ここでタイミングの結果に関する注意事項](#note-about-timing-results-in-this-topic)します。
...
このテーブル型とまったく同じ名前と型を持つ **DataTable** をコード内で作成します。 テキスト クエリまたはストアド プロシージャを呼び出す際にパラメーターとして、この **DataTable** を渡します。 次の例は、この方法を示しています。
...
SQL 一括コピーは、ターゲット データベースに大量のデータを挿入するもう 1 つの方法です。 .NET アプリケーションを使用して、 **SqlBulkCopy** 一括を行うにはクラスの挿入操作。 **SqlBulkCopy** の機能は、コマンドライン ツールの **Bcp.exe** や Transact-SQL ステートメントの **BULK INSERT** に似ています。 次のコード例では、**DataTable** テーブルから SQL Server にあるテーブル MyTable に行を一括コピーしています。
...
テーブル値パラメーターよりも一括コピーの方が好ましいケースもいくつかあります。 テーブル値パラメーターのトピックで BULK INSERT 操作との比較表を参照してください [テーブル値パラメーター](https://msdn.microsoft.com/library/bb510489.aspx)します。
...
バッチ サイズが小さいときは、複数行を挿入する包括的なパラメーター化 INSERT ステートメントを作成するのも一つの方法です。 次のコード例では、この手法を実証します。
...
次のアドホック テストの結果は、この種の挿入ステートメントのパフォーマンスをミリ秒単位で表示します。
...
| バッチ サイズ [反復回数]| 2 スレッド (ミリ秒)| 4 つのスレッド (ミリ秒)| 6 つのスレッド (ミリ秒)|
| -------- | --- | --- | --- |
| 1000 [1]| 277| 315| 266|
| 500 [2]| 548| 278| 256|
| 250 [4]| 405| 329| 265|
