...
description: "Opis dodawania do konfiguracji bazy danych SQL skalowalnej puli elastycznej ułatwiającej administrowanie zasobami i ich udostępnianie między bazami danych."
...
---
# <a name="create-a-new-elastic-pool-with-the-azure-portal"></a>Tworzenie nowej puli elastycznej za pomocą witryny Azure Portal
...
> [!NOTE]
> Nową pulę można również utworzyć, wyszukując hasło **SQL elastic pool** w portalu **Marketplace** lub klikając pozycję **+Dodaj** w bloku przeglądania **Pule elastyczne SQL**. Za pośrednictwem przepływu pracy aprowizacji puli będzie można określić nowy lub istniejący serwer.
>
>

1. W witrynie [Azure Portal](http://portal.azure.com/) pod listą z lewej strony kliknij kolejno pozycje **Więcej usług** **>** **Serwery SQL**, a następnie kliknij serwer zawierający bazy danych, które chcesz dodać do puli.
2. Kliknij przycisk **Nowa pula**.
    ...
    **— Lub —**
    ...
    Może zostać wyświetlony komunikat z informacją, że istnieją zalecane pule elastyczne serwera (tylko w wersji 12). Kliknij komunikat, aby wyświetlić pule zalecane na podstawie historycznych danych telemetrycznych użycia baz danych, a następnie kliknij przycisk warstwy, aby zobaczyć więcej szczegółów i dostosować pulę. Zobacz sekcję [Pojęcie zaleceń puli](#understand-pool-recommendations) w dalszej części tego artykułu, aby dowiedzieć się, w jaki sposób tworzone są zalecenia.

    ![zalecana pula](./media/sql-database-elastic-pool-create-portal/recommended-pool.png)

3. Zostanie wyświetlony blok **Pula elastyczna**, w którym należy określić ustawienia puli. Jeśli w poprzednim kroku kliknięto pozycję **Nowa pula**, warstwa cenowa zostanie domyślnie ustawiona na warstwę **Standardowa** i nie będzie jeszcze żadnych wybranych baz danych. Możesz utworzyć pustą pulę lub określić zestaw istniejących baz danych z tego serwera w celu przeniesienia ich do puli. W przypadku tworzenia zalecanej puli zalecana warstwa cenowa, ustawienia wydajności i lista baz danych zostaną wstępnie wypełnione, ale nadal można je zmienić.
