...
- アップグレードする場合、データベース Basic から Premium 午後 10時 00分 午前 1 時 35 分にアップグレードが完了して 次の日には、午前 1 時から始まる Premium 料金で課金されます。 

- 午前 11 時に基本のデータベースを Premium からダウン グレードする場合 午後 2時 15分に完了し、データベースで課金されます Premium 料金午後 3 時までになる基本的な料金で課金されたします。
...
- 午前 11時 18分を 200 Edtu を標準のエラスティック データベース プールを作成する場合、プールに 5 つのデータベースを追加するに対して課金されます 200 Edtu 全体の時間の午前 11 以降 1 日の残りのです。
...
- 第 3 の日付の午前 5 時に 15 の別のデータベースを追加するとします。 1 日中データベースの使用量は増え、午後 8 時 5 分にプールの eDTU を 200 から 400 に増やすことに決めました。 午後 8 時まで 200 eDTU レベルで課金され、残りの 4 時間は 400 eDTU に増やされました。 
...
- 使用履歴からプールよりも Single Database の方が利点があると判断される場合、Azure インテリジェント エンジンはエラスティック データベース プールを推奨します。 参照してください [推奨エラスティック データベース プール](sql-database-elastic-pool-portal.md#recommended-elastic-database-pools) をこの数式の動作を確認します。 自分で計算を行う方法の詳細については、次を参照してください [弾力性データベース プールの価格およびパフォーマンスに関する考慮事項。](sql-database-elastic-pool-guidance.md)
...
データベースのサービス階層の変更やそれのプールからの出し入れでは、バック グラウンド操作でプラットフォームにデータベースをコピーする必要があります。 これは、データベースのサイズに応じて数分から数時間かかることがあります。 いずれの場合も、データベースはオンラインを維持し、移動中も使用できます。 1 つのデータベースを変更する方法の詳細をご覧ください [、データベースのサービス階層を変更する](sql-database-scale-up.md)です。 弾力性データベースは、次を参照してください [弾力性プールのリファレンス。](sql-database-elastic-pool-reference.md#latency-of-elastic-pool-operations) 
...
使用パターンから利益があると判断される場合、Azure インテリジェント エンジンがデータベースにプールを推奨します。 詳細については、「 [推奨エラスティック データベース プール](sql-database-elastic-pool-portal.md#recommended-elastic-database-pools)します。 1 つとエラスティック データベース間の選択の詳細なガイダンスについては、次を参照してください。 [弾力性データベース プールの価格およびパフォーマンスの考慮事項](sql-database-elastic-pool-guidance.md)します。
