...
keywords: pula elastyczna, bazy danych sql
...
Kolejne jednostki eDTU można dodać do istniejącej puli bez przestoju bazy danych z tym zastrzeżeniem, że może zaistnieć konieczność przeniesienia baz danych w celu zapewnienia dodatkowych zasobów obliczeniowych dla nowej rezerwacji jednostek eDTU. Podobnie, jeśli dodatkowe jednostki eDTU nie są już potrzebne, można je usunąć z istniejącej puli w dowolnej chwili.
...
## <a name="edtu-and-storage-limits-for-elastic-pools"></a>Limity liczby jednostek eDTU i magazynu dla pul elastycznych
...
## <a name="elastic-pool-properties"></a>Właściwości puli elastycznej

W poniższej tabeli opisano limity dla pul elastycznych i baz danych w puli.
...
### <a name="limits-for-pooled-databases"></a>Limity dla baz danych w puli
| Właściwość | Opis |
|:--- |:--- |
...
* [Kurs wideo w serwisie Microsoft Virtual Academy poświęcony elastycznym możliwościom usługi Azure SQL Database](https://mva.microsoft.com/en-US/training-courses/elastic-database-capabilities-with-azure-sql-db-16554)
