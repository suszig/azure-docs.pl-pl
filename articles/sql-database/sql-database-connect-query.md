...
- Azure SQL Database サービス上の **AdventureWorksLT** デモンストレーション データベース。
 - [デモ データベースを作成する](sql-database-get-started.md) (分) です。
...
- Visual Studio 2013 Update 4 以降。 Microsoft は、Visual Studio コミュニティで *無料*します。
 ...
 - [多くのオプションを Visual Studio を解放します。](http://www.visualstudio.com/products/free-developer-offers-vs.aspx)
...
2. **[名前]** に「**ConnectAndQuery_Example**」と入力します。 **[OK]** をクリックします。
...
2. 次の App.config コード サンプルに示すように、**&#x3c;configuration&#x3e; &#x3c;/configuration&#x3e;** 要素を追加します。
 - *{your_placeholders}* を実際の値に置き換えます。
...
5. **[出力ディレクトリにコピー]** を **[常にコピーする]** に設定します。
 ...
 - &#x2a;.exe.config ファイルは、C# サンプル プログラムの実行時に読み取られます。
    ...
    ![[出力ディレクトリにコピー] = ][50-vscopytooutputdirectoryproperty]
...
>
> [ここ](sql-database-develop-csharp-retry-windows.md) 再試行ロジックを持つコード サンプルへのリンクです。
