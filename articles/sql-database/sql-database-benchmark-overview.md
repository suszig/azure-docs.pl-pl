...
Microsoft Azure SQL Database は、3 つの [サービス階層](sql-database-service-tiers.md) と複数のパフォーマンス レベル。 パフォーマンス レベルごとに提供されるリソースのセット ("能力") が増加し、スループットが段階的に高くなるように設計されています。
...
[サービス階層とパフォーマンス レベル](sql-database-service-tiers.md)

[1 つのデータベースのパフォーマンス ガイダンス](sql-database-performance-guidance.md)
