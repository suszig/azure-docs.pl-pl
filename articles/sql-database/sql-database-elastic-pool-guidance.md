---
title: "Kiedy należy użyć puli elastycznej?"
description: "Pula elastyczna to zbiór dostępnych zasobów, które są współużytkowane przez grupę elastycznych baz danych. Ten dokument zawiera wskazówki ułatwiające ocenę przydatności korzystania z puli elastycznej dla grupy baz danych."
...
---
# <a name="when-should-an-elastic-pool-be-used"></a>Kiedy należy użyć puli elastycznej?
Na podstawie wzorców użycia baz danych i różnic między cenami puli elastycznej i pojedynczych baz danych oceń, czy korzystanie z puli elastycznej jest opłacalne. Udostępniane są również dodatkowe wskazówki ułatwiające określenie bieżącego rozmiaru puli wymaganego dla istniejącego zestawu baz danych SQL.  

* Omówienie pul zawiera temat [SQL Database elastic pools](sql-database-elastic-pool.md) (Pule elastyczne usługi SQL Database).
...
Aby sprawdzić dostępne rozmiary, zobacz [limity liczby jednostek eDTU i magazynu dla pul elastycznych i elastycznych baz danych](sql-database-elastic-pool.md#edtu-and-storage-limits-for-elastic-pools).
...
Aby uzyskać bardziej elastyczne oceny użycia zasobów umożliwiające szacowanie ad hoc rozmiaru dla serwerów starszych niż wersja 12 oraz szacowanie rozmiaru dla baz danych na różnych serwerach, zobacz [Skrypt programu PowerShell do identyfikowania baz danych odpowiednich dla puli elastycznej](sql-database-elastic-pool-database-assessment-powershell.md).
...
Nie wszystkie pojedyncze bazy danych są optymalnymi kandydatami dla pul. Świetnymi kandydatami są bazy danych, których wzorce użycia charakteryzują się niskim średnim wykorzystaniem i stosunkowo rzadkimi okresami zwiększonego użycia. Wzorce użycia aplikacji są dynamiczne, a więc informacje i narzędzia opisane w niniejszym artykule umożliwiają początkową ocenę, czy pula jest dobrym wyborem dla niektórych lub wszystkich baz danych. Ten artykuł jest jedynie punktem wyjścia ułatwiającym podjęcie decyzji, czy pula elastyczna jest dobrym rozwiązaniem. Trzeba mieć na uwadze, że konieczne jest dalsze ciągłe monitorowanie historycznego użycia zasobów i stałe dokonywanie ponownej oceny poziomów wydajności wszystkich używanych baz danych. Należy pamiętać, że można łatwo przenosić bazy danych do i z elastycznych pul baz danych, a w przypadku dużej liczby baz danych można mieć wiele pul o różnych rozmiarach, między które można podzielić bazy danych.

## <a name="next-steps"></a>Następne kroki
* [Tworzenie puli elastycznej](sql-database-elastic-pool-create-portal.md)
* [Monitorowanie puli elastycznej, zmienianie jej rozmiaru i zarządzanie nią](sql-database-elastic-pool-manage-portal.md)
...
* [Skrypt programu PowerShell do identyfikowania baz danych odpowiednich dla puli elastycznej](sql-database-elastic-pool-database-assessment-powershell.md)
