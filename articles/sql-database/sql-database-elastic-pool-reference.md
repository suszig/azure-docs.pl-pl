...
エラスティック データベース プールは、エラスティック データベース スループット単位 (eDTU) と、複数のデータベースで共有されるストレージ (GB) のコレクションです。 エラスティック データベースは、いつでも、プールに追加したり、プールから削除したりできます。 プール内のエラスティック データベースでは、使用可能なリソースを解放するプールから必要なリソースのみが、それを必要とするアクティブなデータベース用にのみ使用されます。 エラスティック データベース プールのデータベースが役立つかどうかの詳細については、次を参照してください。 [弾力性データベース プールの価格およびパフォーマンスに関する考慮事項](sql-database-elastic-pool-guidance.md)します。



## エラスティック データベース プールを作成および管理するための前提条件

- エラスティック データベース プールは、Azure SQL Database V12 サーバーでのみ使用できます。 V12 へのアップグレードをプールに直接、データベースの移行は、次を参照してください。 [Azure SQL Database V12 にアップグレード](sql-database-upgrade-server-powershell.md)します。
- 作成して、弾力性データベース プールの管理を使用してサポート、 [Azure ポータル](https://portal.azure.com), 、PowerShell、および .NET クライアント ライブラリ (REST Api のラッパー) の Azure リソース マネージャーのみで、 [旧ポータル](https://manage.windowsazure.com/) およびサービス管理コマンドはサポートされていません。
...
- プールごとにデータベースとプール eDTUs の最大数があります。
  ...
  | Premium| 50| 1500|
    ...
    *** * プールごとのデータベースの数とプール Edtu の数の現在の制限が予想される increase.***
...
| [価格とパフォーマンスに関する考慮事項](sql-database-elastic-pool-guidance.md)| エラスティック データベース プールの利用のコスト効率を評価する方法|
| [作成し、Azure ポータルでの SQL Database エラスティック データベース プールの管理](sql-database-elastic-pool-portal.md)| Azure ポータルを使ったエラスティック データベース プールの作成と管理方法|
| [作成し、PowerShell を使用した SQL Database エラスティック データベース プールの管理](sql-database-elastic-pool-powershell.md)| PowerShell コマンドレットを使ったエラスティック データベース プールの作成と管理方法|
| [作成し、.NET 用 Azure SQL Database Library で SQL データベースの管理](sql-database-elastic-pool-powershell.md)| C# を使ったエラスティック データベース プールの作成と管理方法|
...
| [弾力性ジョブ サービスの必要なユーザーを作成します。](sql-database-elastic-jobs-add-logins-to-dbs.md)| エラスティック データベース ジョブのスクリプトを実行するには、プール内のすべてのデータベースに適切なアクセス許可を持つユーザーを追加する必要があります。|
| [エラスティック データベース ジョブ コンポーネントをアンインストールする方法](sql-database-elastic-jobs-uninstall.md)| エラスティック データベース ジョブ サービスをインストールしようとするときに発生するエラーからの回復|
...
- **管理エンドポイント** REST API 呼び出し (リソース マネージャー) の: https://management.azure.com
...
エラスティック プールの作成と管理には、いくつかの PowerShell コマンドレットと REST API コマンドを使用できます。 詳細とコード例では、次を参照してください。 [作成 PowerShell を使用して SQL Database エラスティック データベース プールの管理と](sql-database-elastic-pool-powershell.md), 、と [作成 c# での SQL データベースの管理と](sql-database-client-library.md)します。
...
| :-- | :-- |
| [新しい AzureRmSqlElasticPool](https://msdn.microsoft.com/library/azure/mt619378.aspx)| [エラスティック データベース プールを作成します。](https://msdn.microsoft.com/library/mt163596.aspx)|
| [セット AzureRmSqlElasticPool](https://msdn.microsoft.com/library/azure/mt603511.aspx)| [エラスティック データベース プールのパフォーマンスを設定します。](https://msdn.microsoft.com/library/mt163641.aspx)|
| [削除 AzureRmSqlElasticPool](https://msdn.microsoft.com/library/azure/mt619355.aspx)| [エラスティック データベース プールを削除します。](https://msdn.microsoft.com/library/mt163672.aspx)|
...
| [Get AzureRmSqlElasticPoolActivity](https://msdn.microsoft.com/library/azure/mt603812.aspx)| [エラスティック データベース プール操作のステータスを取得します。](https://msdn.microsoft.com/library/mt163669.aspx)|
| [Get AzureRmSqlElasticPoolDatabase](https://msdn.microsoft.com/library/azure/mt619484.aspx)| [弾力性データベース プール内のデータベースを取得します。](https://msdn.microsoft.com/library/mt163646.aspx)|
| [Get AzureRmSqlElasticPoolDatabaseActivity]()| [プールとの間のデータベースの移動の状態を取得します。](https://msdn.microsoft.com/library/mt163669.aspx)|
...
| タスク| 詳細|
| :-- | :-- |
| 新しいエラスティック データベースを作成する (プール内で直接)| [CREATE DATABASE (Azure SQL データベース)](https://msdn.microsoft.com/library/dn268335.aspx)|
| 既存のデータベースをプールの内外に移動する| [ALTER DATABASE (TRANSACT-SQL)](https://msdn.microsoft.com/library/ms174269.aspx)|
| プールのリソース使用状況の統計を取得する| [sys.elastic_pool_resource_stats (Azure SQL データベース)](https://msdn.microsoft.com/library/mt280062.aspx)|
...
エラスティック プールの eDTU 単価は、同じサービス階層のスタンドアロン データベースの DTU 単価よりも高くなります。 詳細については、「 [SQL Database 料金](http://azure.microsoft.com/pricing/details/sql-database/)します。
...
| 40861| EX_USER| データベース エディション ' % です。*'%.*ls' がである弾力性プールのサービス階層とは異なることはできませんに ' %*。%ls '。| データベースのエディション、エラスティック プールのサービス階層| データベースのエディションの大文字と小文字が、エラスティック プールのサービス階層と異なります。| エラスティック プールのサービス階層と異なるデータベース エディションを指定しないでください。データベースのエディションを指定する必要がないことにご注意ください。|
...
| 40867| EX_USER| データベースあたり最大 DTU がである必要がありますサービス層の少なくとも (%d を参照) ' %。 * ls' です。| データベースあたりの DTU の最大値、エラスティック プールのサービス階層| サポートされている制限を下回るデータベースあたりの DTU 最大値を設定しようとしています。| 目的の設定をサポートするエラスティック プールのサービス階層を使用することをご検討ください。|
...
| 40881| EX_USER| エラスティック プール '%.*ls' のデータベース数が上限に達しました。エラスティック プールのデータベース数上限は、(%d) DTU を含むエラスティック プールの場合、(%d) を超えることはできません。| エラスティック プールの名前弾力性プールのデータベース数上限はリソース プールの Dtu を e。| エラスティック プールのデータベース数が上限に達したときに、データベースを作成またはエラスティック プールにデータベースを追加しようとしています。| 可能であれば、エラスティック プールの DTU を増やして、データベースの上限を上げることを検討するか、エラスティック プールからデータベースを削除してください。|
