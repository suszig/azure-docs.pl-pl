...
状況に応じて、 [ビジネス継続性のためにアプリケーションを設計](sql-database-business-continuity.md), 、実行する訓練のワークフローは異なります。 下記に、Azure SQL Database のコンテキストで障害復旧の訓練を行う際のベスト プラクティスについて説明します。 
...
- 説明に従って、別のサーバーにデータベースの地理的復元を実行 [ここ](sql-database-disaster-recovery.md)します。 
- 復旧したデータベースに接続し、次のアプリケーション構成の変更、 [回復後にデータベースを構成](sql-database-disaster-recovery.md) ガイド、復旧を完了します。
...
- 実行 [計画されたフェールオーバー](sql-database-geo-replication-powershell.md#initiate-a-planned-failover) 新しいプライマリ、セカンダリ データベースを作成するには
- 次の [回復後にデータベースを構成](sql-database-disaster-recovery.md) ガイド、復旧を完了します。
