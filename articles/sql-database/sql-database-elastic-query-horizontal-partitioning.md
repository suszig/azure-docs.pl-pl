...
このドキュメントでは、行方向のパーティション分割シナリオ用にエラスティック クエリを設定する方法と、クエリを実行する方法について説明します。 水平方向のパーティション分割のシナリオの定義を参照してください、 [エラスティック データベース クエリの概要 (プレビュー)](sql-database-elastic-query-overview.md)します。

機能は、Azure SQL の一部 [Database 弾力性データベースの機能セット](sql-database-elastic-scale.md)します。  
 
## データベース オブジェクトの作成

エラスティック データベース クエリは、T-SQL 構文を拡張して、シャーディング (行方向のパーティション分割) を使用して多数のデータベースにデータを分散するデータ層を参照します。 ここでは、シャーディングされたテーブルに対するエラスティック クエリに関係する DDL ステートメントの概要を説明します。 これらのステートメントを使うと、エラスティック クエリ データベース内のシャーディングされたデータのメタデータ表現を作成できます。 これらのステートメントを実行するための前提条件は、エラスティック データベース クライアント ライブラリを使用してシャード マップを作成することです。 詳細については、次を参照してください。 [シャード マップの管理](sql-database-elastic-scale-shard-map-management.md); 次のトピックでサンプルを利用して、1 つを作成する: [エラスティック データベース ツールを使ってみる](sql-database-elastic-scale-get-started.md)します。   
...
**注**    いることを確認、 *< ユーザー名 >* どれも含まない *"@servername"* サフィックスです。 
...
* **スキーマ**: 外部テーブル DDL は、クエリを使用するスキーマを定義します。 外部テーブル定義に指定するスキーマは、実際のデータが格納されているシャード上のテーブルのスキーマに一致する必要があります。 

* **データ分布**: 外部テーブル DDL は、データ層の間でデータを分散するために使用するデータの分布を定義します。 Azure SQL Database では、外部テーブルに定義した分散はシャード上の実際の分散に対して検証されないことに注意してください。 ユーザー側で、シャード上の実際のデータの分散が外部テーブルでの定義に一致することを確認する必要があります。 

* **データ層の参照を**: 外部テーブル DDL は、外部データ ソースを参照します。 外部データ ソースは、データ層のすべてのデータベースを見つけるために必要な情報を外部テーブルに提供するシャード マップを指定します。 
...
DATA\_SOURCE 句は、外部テーブルに対して使用される外部データ ソース (水平的パーティション分割の場合のシャード マップ) を定義します。  

SCHEMA\_NAME および OBJECT_NAME 句に、シャード上の別のスキーマ内のテーブルに外部テーブルの定義をマッピングする機能を提供する、または別の名前を持つテーブルにそれぞれします。 これらを省略した場合、リモート オブジェクトのスキーマは "dbo" と見なされ、その名前は定義されている外部テーブルの名前と同一であると見なされます。  

SCHEMA\_NAME と OBJECT\_NAME 句は、リモート テーブルの名前は既に使用されている場合、データベースの外部テーブルを作成するには特に役立ちます。 この問題の例として、スケールアウトされたデータ層のカタログ ビューまたは DMV の集計ビューを取得する外部テーブルを定義する場合が挙げられます。 カタログ ビューと DMV は既にローカルに存在するため、外部テーブルの定義にその名前を使うことはできません。 代わりに、別の名前を使用して、カタログ ビューまたは DMV の名前を SCHEMA_NAME 句または OBJECT_NAME 句で使用します  (次の例をご覧ください)。 

DISTRIBUTION 句は、このテーブルに使用するデータ分散を指定します。  

* SHARDED は、シャード マップ内のデータベース間でこのテーブルのデータが行方向にパーティション分割されることを意味します。 < Sharding_column_name > パラメーターでは、データ分布のパーティション分割キーがキャプチャされます。  
...
* ROUND\_ROBIN では、水平的パーティション分割を使用してテーブルを配布することを意味します。 ただし、アプリケーションに依存する分散が使用されています。  
...
**CREATE/DROP EXTERNAL TABLE のアクセス許可:** これは、基になるデータ ソースを参照する必要も ALTER ANY EXTERNAL DATA SOURCE のアクセス許可が必要です。  

**セキュリティの考慮事項:** 外部テーブルへのアクセスを持つユーザーは、外部データ ソース定義で指定された資格情報 [基になるリモート テーブルへのアクセスを自動的に取得します。 外部データ ソースの資格情報を介した特権の不要な昇格を回避するために、外部テーブルへのアクセスを慎重に管理する必要があります。 外部テーブルへのアクセスは、通常の SQL 権限を使用して、通常のテーブルの場合と同様に許可または禁止することができます。  

**例**: 次の例は、外部テーブルを作成する方法を示しています。  
...
**水平的パーティション分割の例:** 次のクエリは、倉庫、注文、および注文明細行の間で 3 方向結合を実行し、いくつかの集計と選択的フィルターを使用します。 ここでは、(1) 行方向のパーティション分割 (シャーディング) のほか、(2) 倉庫、注文、および注文明細行がウェアハウスの ID 列でシャーディングされること、エラスティック クエリがシャード上の結合を併置できること、クエリの負荷の高い部分をシャード上で並列に処理できることを想定しています。 
...
### 2.2 ストアド プロシージャ SP_EXECUTE_FANOUT 

エラスティック クエリには、シャードへの直接アクセスを提供するストアド プロシージャも導入されています。 sp_execute_fanout という名前のこのストアド プロシージャは、次のパラメーターを受け取ります。   
...
sp\_execute\_fanout では、[起動パラメーターで提供されるシャード マップ情報を使用して、シャード マップに登録されているすべてのシャードで指定された T-SQL ステートメントを実行します。 結果は、UNION ALL セマンティクスを使用してマージされます。 結果には、シャード名の付いた "仮想" 列も取り込まれます。 
