...
1. [フェデレーション ルートからシャード マップ マネージャーを作成します。](#create-a-shard-map-manager-from-a-federation-root)
2. [既存のアプリケーションを変更します。](#modify-the-existing-application)
3. [既存のフェデレーション メンバーをスイッチ](#switch-out-existing-federation-members)
...
アプリケーションが変更された後でのエラスティック データベース ツール Api、Federations アプリケーションの移行の最後の手順で、 **SWITCH OUT** フェデレーション メンバー (詳細については、の MSDN リファレンスを参照してください [ALTER FEDERATION (Azure SQL Database](http://msdn.microsoft.com/library/dn269988(v=sql.120).aspx))します。 特定のフェデレーション メンバーに対して **SWITCH OUT** を発行すると、すべてのフェデレーション制約とメタデータが削除され、その結果、フェデレーション メンバーは、他の Azure SQL Database と同じ通常の Azure SQL Database となります。
...
- **FILTERING=ON** の使用。 行のフィルター処理には行レベルのセキュリティ (RLS) の使用をお勧めします。 Federations でのフィルター処理と同様に、RLS でもシャード化したテーブル上のすべてのクエリに述語を自動的に追加します。 詳細については、「 [エラスティック データベース ツールと行レベル セキュリティを持つマルチ テナント アプリケーション](sql-database-elastic-tools-multi-tenant-row-level-security.md)します。
