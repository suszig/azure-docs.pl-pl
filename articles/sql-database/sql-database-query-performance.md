...
1. 詳細については、グラフの周囲をクリックします。<br>バーは、選択したクエリから呼び出される CPU % を示して、一番上の行が、データベースの全体的な DTU 割合を示しています。グラフに表示する個別のクエリを選別するには、選択または選択解除します。
...
1. 上位のクエリの一覧で任意のクエリをクリックします。<br>詳細ビューが開き、時間の経過と共に、クエリの CPU 消費量が分割されます。
3. 詳細については、グラフの周囲をクリックします。<br>先頭行は、全体的な DTU % であり、バーは選択したクエリによって使用される CPU % です。
