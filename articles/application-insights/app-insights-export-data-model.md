...
製品利用統計情報の種類名のいずれかのプレース ホルダーを"< telemetryType >"の最初のセクションには。
表示、要求、およびなどです。
...
* 
    ...
    *既定:* R2: remotedepencyType の場合は dependencyType = HTTP このフィールドは必須<br/>        clientperformanceType にこのフィールドは必須です
...
* 
    ...
    *既定:* かどうかは null の場合、ユーザー エージェントがかが検査代理の既知のソース (webcrawlers など) おり、これに基づき、ソースが設定可能性があります。
...
* 
    ...
    *既定:* Null の場合、代理エージェントの既知の一覧に対してユーザー エージェントが確認されます。 一致が見つかった場合、値が設定を true にします。<br/>ユーザー エージェントは null を false に設定されている場合
...
* 
     リモートの依存関係のタイプ。 サポートされる型が SQL では、AZURE リソース (ストレージ、キューなど) HTTP です。
