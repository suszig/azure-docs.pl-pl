...
Visual Studio Application Insights には、Microsoft Azure *リソース*のアプリケーションに関するデータが表示されます。 新しいリソースの作成の一部であるため [[スタート] の新しいアプリケーションを監視する Application Insights を設定する][start]します。 多くの場合、これは IDE によって自動的に行うことができ、可能な場合には常にそうするよう勧められています。 しかし、リソースを手動で作成する場合もあります。
...
![[新規]、](./media/app-insights-create-new-resource/01-new.png)
...
インストルメンテーション キーを使用して構成する [、アプリケーションの ][start]します。

SDK には、コードを記述せずにテレメトリを送信する標準的なモジュールが含まれています。 ユーザーの操作を追跡したり、詳細については、問題を診断 [API の ][api] 、独自のテレメトリを送信します。
...
[検索タイルを表示する] をクリックして [診断検索の ][diagnostic], 、最初のイベントが表示されます。
