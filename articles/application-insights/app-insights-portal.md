...
作成したら [プロジェクトに Application Insights を設定](app-insights-overview.md), 、プロジェクトの Application Insights のリソースにアプリケーションのパフォーマンスと利用状況に関するテレメトリ データが表示されます、 [Azure ポータル](https://portal.azure.com)します。

## テレメトリの検索

サインイン、 [Azure ポータル](https://portal.azure.com) アプリ用に作成した Application Insights リソースを参照します。

![[参照] をクリックし、](./media/app-insights-portal/00-start.png)

[概要] ページには、いくつかの基本的なテレメトリとその他のリンクが示されます。
内容は、アプリの種類によって異なり、カスタマイズすることができます。
...
![編集] をクリックします。 タイルやグラフをドラッグします。 ギャラリーからタイルを追加します。 [Done] をクリックします。](./media/app-insights-portal/020-customize.png)

### Azure ダッシュボードのカスタマイズ

Azure ポータル ダッシュボードは、ポータルに初めてサインインしたときに表示されるホーム ページです。 ダッシュボードでは、複数リソースのタイル (グラフのグループ) をまとめて確認できます。

Application Insights の [概要] ブレードのタイルをポータル ダッシュボードにピン留めするには、タイルのヘッダーを選択して [...] を選択します。

包括的なダッシュ ボードでは、使用 [Power BI](https://azure.microsoft.com/blog/application-insights-content-pack-for-power-bi/) 利用統計情報を表示します。
...
![[概要] ブレードで ](./media/app-insights-portal/16-metrics.png)

### グラフおよびグリッドの編集

新しいグラフをブレードに追加するには:

![メトリックス エクスプローラーで、](./media/app-insights-portal/04-add.png)
...
組み込んだ場合 [カスタム メトリック](app-insights-api-custom-events-metrics.md#track-metric) アプリ (には TrackMetric と TrackEvent の呼び出しにアタッチされているメトリックの呼び出し) に、ここに示されます。

### データの分割

グラフまたはグリッドを選択します。[グループ化] をオンにし、グループ化のために使用するプロパティを選択します。

![[グループ化] をオンにし、](./media/app-insights-portal/15-segment.png)

アプリにカスタム メトリックがコード化された、そのオプションがあります [プロパティ値](app-insights-api-custom-events-metrics.md#properties), 、一覧でプロパティを選択することができます。
...
![概要ブレードで ](./media/app-insights-portal/22-favorite-get.png)
...
> [AZURE.NOTE] (ASP.NET SDK バージョン 2.0.0-beta3 以降を使用している状態で) アプリから大量のテレメトリが生成されると、アダプティブ サンプリング モジュールからイベントの代表的な部分のみが送信され、ポータルに送信されるデータ量が自動的に削減されます。 ただし、同じ要求に関連するイベントはグループ単位で選択または選択解除されるので、関連するイベントごとに操作できます。 [サンプリングについて](app-insights-sampling.md)します。
...
特定のプロパティの値を選択しない場合、すべての値を選択するのと同じ意味になり、そのプロパティに基づくフィルターはオフになります。
> [AZURE.NOTE] アプリから大量のテレメトリが生成されると、アダプティブ サンプリング モジュールからイベントの代表的な部分のみが送信され、ポータルに送信されるデータ量が自動的に削減されます。 同じ操作に含まれるイベントは、グループ単位で選択または選択解除されるので、関連するイベントごとに操作できます。 [サンプリングについて説明します。](app-insights-sampling.md)
...
![[お気に入り] をクリックし、名前を設定して ](./media/app-insights-portal/08-favorite-save.png)
