...
[料金][pricing] の [Visual Studio Application Insights][start] アプリケーションごとのデータ量に基づきます。 真の意味で Free レベルがあり、いくつかの制限はありますが、ほとんどの機能を利用できます。

各 Application Insights リソースは個々のサービスとして課金され、Azure のサブスクリプションの課金内容に加えられます。

[料金プランを参照してください][pricing]します。
...
* [データ転送レート](#data-rate) -アプリからデータを処理できる最大転送率。
* [保有期間](#data-retention) - 表示するための Application Insights ポータルで長い形式のデータを保持します。
...
* 毎月、アプリケーションは、指定された量のテレメトリを Application Insights に送ることができます。 参照してください、 [料金プラン][pricing] の実際の数値。 
* クォータは、選択した価格レベルによって異なります。
* クォータは、毎月 1 日の午前 0 時 UTC からカウントされます。
* データ ポイントのグラフは、今月クォータのどのくらいが使用されたかを示しています。
* クォータの単位で *データ ポイント。*単一のデータ ポイントは、追跡メソッドの 1 つの呼び出しで、コードで明示的に呼び出されるか、標準テレメトリ モジュールのいずれかによって呼び出されます。 データ ポイントには次の項目が含まれます。
...
* *セッション データ* クォータにカウントされません。 これには、ユーザー、セッション、環境、デバイスのデータの数が含まれます。
...
* 追加データ分を支払います。 参照してください、 [料金プラン][pricing] 詳細です。 このオプションは、事前に選択できます。 このオプションは、Free 価格レベルでは選択できません。
* 価格レベルをアップグレードします。
...
月間クォータの他にも、データ速度でのスロットルの上限もあります。 無料の [価格レベル][pricing] 上限は 200 のデータ ポイント数/秒 5 分間の平均し、有料層の 500/秒の平均を求める 1 分を超える。 
...
* 使用 [サンプリング](app-insights-sampling.md)します。 このテクノロジは、メトリックや、検索で関連するアイテム間を移動する機能を損なうことなく、データ レートを削減します。
