...
診断検索] ブレードには [Application Insights][start] を検索し、ページ ビュー、例外などの個々 のテレメトリ項目の探索や web 要求を使用することです。 診断検索を使用すると、作成したログ トレースやイベントを表示できます。
...
* **要求** -ページ、スクリプト、イメージ、スタイル ファイル、データなど、サーバー アプリケーションで受信した HTTP 要求です。 これらのイベントは、要求と応答の概要グラフの作成に使用されます。
* **ページ ビュー** -ページ ビュー レポートの作成に使用される、web クライアントによって送信されたテレメトリ。 
* **カスタム イベント** を行うために TrackEvent() への呼び出しを挿入した場合 - [使用状況を監視][track], 、ここで検索することができます。
* **例外** -TrackException() を使用してログオンしたもののサーバーでの例外をキャッチしませんでした。
...
さらにも表示する他のイベント発生しているこの期間中にチェックする **未定義のプロパティを持つイベントを含める**します。

## Bot と Web テストのトラフィックの除外

フィルターを使用して **または統合された実際のトラフィック** チェックと **実際**します。

フィルターすることもできます。 **人工トラフィックのソース**します。
...
語句| *いない* に一致します。|一致する
---|---|---
HomeController.About|about<br/>ホーム|h\ * について<br/>ホーム *
IsLocal|local<br/>します。<br/>\*local|isl\ *<br/>islocal<br/>i \ * 探して *
New Delay|w d|新しい<br/>delay<br/>n \ * AND/d \ *
...
database??|database01、databaseAB などに一致します<br/>? 検索語句の先頭には許可されません。
...
apple OR banana<br/>apple banana|どちらかの語句を含むイベントを検索します。 なく"OR"を使用して「または」です。 </br/> 短いフォームです。
...
ここでも、検索条件を表示する **概要ブレードに移動** [お気に入り] を開きます。
...
* [コードを記述][track] カスタム イベント、ページ ビュー、および例外を送信します。 

[ログとカスタム テレメトリを Application Insights に送信する方法について説明][trace]します。
