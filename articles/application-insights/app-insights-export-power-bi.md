...
**サンプリング。**アプリケーションが送信するデータ量が多く、Application Insights SDK for ASP.NET バージョン 2.0.0-beta3 以降を使用している場合は、アダプティブ サンプリング機能が動作して、テレメトリの一定の割合のみが送信される可能性があります。 [サンプリングについて説明します。](app-insights-sampling.md)

## Application Insights によるアプリの監視

まだ試していないなら、今からはじめましょう。 Application Insights では、さまざまなプラットフォーム (Windows、iOS、Android、J2EE など) のすべてのデバイスまたは Web アプリを監視できます。 [Get started](app-insights-overview.md).
...
1. サブスクリプションの「昔ながらの」ストレージ アカウントの作成、 [Azure ポータル](https://portal.azure.com)します。
    ...
    ![Azure ポータルで、](./media/app-insights-export-power-bi/030.png)

2. コンテナーを作成する

    ![新しいストレージで、](./media/app-insights-export-power-bi/040.png)

3. ストレージ アクセス キーのコピー

    これは、ストリーム分析サービスへの入力のセットアップのためにすぐに必要になります。

    ![ストレージで、](./media/app-insights-export-power-bi/045.png)
...
1. Azure ポータルで、アプリケーション用に作成した Application Insights リソースを参照します。
    ...
    ![[参照]、](./media/app-insights-export-power-bi/050.png)

2. 連続エクスポートを作成します。

    ![[設定]、](./media/app-insights-export-power-bi/060.png)
...
* `1234...` Application Insights リソースのインストルメンテーション キーは、 **ダッシュ記号を省略すると**します。
* `PageViews` 分析するデータの種類です。 使用可能な種類は、連続エクスポートで設定するフィルターによって異なります。 エクスポートされたデータを他の利用可能な型を参照して調べる、 [データ モデルをエクスポート](app-insights-export-data-model.md)します。
...
![新しいチャネルを選択して、](./media/app-insights-export-power-bi/160.png)
...
![ジョブを選択し、[クエリ] をクリックします。 次のサンプルを貼り付けます。](./media/app-insights-export-power-bi/180.png)
...
![ジョブを選択し、[クエリ] をクリックします。 次のサンプルを貼り付けます。](./media/app-insights-export-power-bi/190.png)
