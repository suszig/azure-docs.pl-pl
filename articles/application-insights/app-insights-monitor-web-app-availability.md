...
する場合は、この手順をスキップ既に [Application Insights リソース ][start] のこのアプリケーションしたい場合、同じ場所で可用性データを表示します。
...
2. .webtest ファイルを開き、記録を開始します。
    ...
    ![.webtest ファイルを開き、](./media/app-insights-monitor-web-app-availability/appinsights-71webtest-multi-vs-start.png)
...
5. 動作を確認するには、Visual Studio でテストを実行します。
    ...
    ![Visual Studio で .webtest ファイルを開き、](./media/app-insights-monitor-web-app-availability/appinsights-71webtest-multi-vs-run.png)
...
1. Application Insights ポータルで、新しい Web テストを作成します。
    ...
    ![Web テスト ブレードで、](./media/app-insights-monitor-web-app-availability/16-another-test.png)
...
2. 各プラグインのプロパティを開きます。 名前を付け、現在の時刻を使用するように設定します。 いずれかのプロパティに、「Add Minutes = -15」を設定します。
    ...
    ![[名前]、](./media/app-insights-monitor-web-app-availability/appinsights-72webtest-plugin-parameters.png)
...
* *Web テストからコードを呼び出すことはできますか。*
    ...
    番号 テストの手順は、.webtest ファイルでなければなりません。 他の Web テストの呼び出しまたはループの使用は許可されていません。 ただし、役に立つさまざまなプラグインがあります。
