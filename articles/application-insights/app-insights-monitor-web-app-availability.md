...
* [URL の ping テスト](#set-up-a-url-ping-test): 簡単なテストには、Azure ポータルで作成することができます。
* [複数手順の web テスト](#multi-step-web-tests): Visual Studio Ultimate または Visual Studio Enterprise で作成し、ポータルにアップロードします。
...
する場合は、この手順をスキップ既に [Application Insights リソースを設定する][start] のこのアプリケーションしたい場合、同じ場所で可用性データを参照してください。

サインアップ [Microsoft Azure](http://azure.com), に移動して、 [Azure ポータル](https://portal.azure.com), 、新しい Application Insights リソースを作成します。

![New > Application Insights](./media/app-insights-monitor-web-app-availability/11-new-app.png)

新しいリソースの概要ブレードが開きます。 いつでも特定するには、 [Azure ポータル](https://portal.azure.com), 、クリックして **参照**します。
...
- **URL** パブリック インターネットからアクセス可能である必要があります。 これにはクエリ文字列を含めることができます。したがって、たとえば限られた範囲でデータベースを実行できます。 URL が解決されてリダイレクトする場合、それに続いて最大で 10 個リダイレクトを使用できます。
- **依存する要求を解析**: 画像、スクリプト、スタイル ファイル、およびページの他のリソースは、テストの一部として要求します。 テスト全体のタイムアウト時間内にこれらすべてのリソースを正常にダウンロードできない場合、テストは失敗します。
- **再試行を有効にする**: テストに失敗したときに、短い間隔の後に再試行します。 エラーは試行が 3 回連続で失敗した場合にのみ報告されます。 その後、後続のテストが通常のテスト間隔で実行されます。 再試行は、次の成功まで一時的に中断されます。 このルールがテスト場所ごとに独立して適用されます。 (この設定をお勧めします。 再試行の際に平均でエラーの約 80% がなくなります)
- **[テスト頻度**: 各テスト場所からテストを実行する頻度を設定します。 間隔が 5 分で、テストの場所が 5 か所の場合、サイトは平均すると毎分テストされることになります。
- **テストの場所** は、サーバーが URL に web 要求を送信元の場所。 Web サイトで発生している問題とネットワークの問題とを区別できるように、複数の場所を選択してください。 最大 16 個の場所を選択できます。

- **成功の条件**:

    **テストのタイムアウト**: 低速な応答に関する警告を表示するこの問題を軽減します。 テストは、この期間内にサイトから応答が返されない場合に、エラーとしてカウントされます。 選択した場合は **依存要求を解析**, 、すべてのイメージ、スタイル ファイル、スクリプト、およびその他の依存リソース必要がありますが内に受け取られてこの期間、します。

    **HTTP 応答**: 返されるステータス コードは、成功としてカウントされます。 200 は、通常の Web ページが返されたことを示すコードです。

    **コンテンツの一致**:「ようこそ!」のように、文字列 すべての返信に表示されるかどうかをテストします。 文字列は、(ワイルドカードを含まない) プレーン文字列である必要があります。 ページ コンテンツが変更された場合は、この文字列も更新する必要がある可能性があることに注意してください。


- **アラート** は、既定で、する場合、エラーが 3 つの場所で 5 分間に送信します。 エラーが 1 つの場所で発生している場合は、サイトでの問題ではなく、ネットワークの問題である可能性が高くなります。 ただし、しきい値は上げ下げでき、電子メールの送信先を変更することもできます。
...
1 ~ 2 分後に次のようにクリックします。 **更新** 可用性/web テスト ブレードでします。 (自動的には更新されません)。
...
*[Ok] に見えますが、エラーとして報告されましたか。*イメージ、スクリプト、スタイル シート、およびページによって読み込まれるその他のファイルすべてをご確認ください。 これらのいずれかにエラーがある場合、メインの html ページの読み込みに問題がない場合でも、テストはエラーとして報告されます。
...
1. 目的の各変数のパラメーター値に対して Web テスト プラグインを追加します。 Web テスト ツールバーで **[Web テスト プラグイン**します。
...
* *Web テストからコードを呼び出すことはできますか。*
    ...
    いいえ、できません。 テストの手順は、.webtest ファイルでなければなりません。 他の Web テストの呼び出しまたはループの使用は許可されていません。 ただし、役に立つさまざまなプラグインがあります。
