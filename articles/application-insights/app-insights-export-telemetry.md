...
1. サブスクリプションの「昔ながらの」ストレージ アカウントの作成、 [Azure ポータル](https://portal.azure.com)します。
    ...
    ![Azure ポータルで、](./media/app-insights-export-telemetry/030.png)

2. コンテナーを作成します。

    ![新しいストレージで、](./media/app-insights-export-telemetry/040.png)

## <a name="setup"></a> 連続エクスポートの設定します。

Application Insights ポータルのアプリケーションの概要ブレードで、[連続エクスポート] を開きます。

![下へスクロールし、](./media/app-insights-export-telemetry/01-export.png)

エクスポートを追加して、 [Azure ストレージ アカウント](../storage-introduction.md) データを配置します。

![[追加]、](./media/app-insights-export-telemetry/02-add.png)
...
* エクスポートを追加または変更するには、所有者、共同作成者、または Application Insights 共同作成者のアクセス権が必要になります。 [[ロール] の役割について理解する][roles]します。

## <a name="analyze"></a> どのようなイベントを取得するか
...
大規模なコード サンプルを参照してください。 [ワーカー ロール ][exportasa]します。



## <a name="delete"></a>古いデータを削除します。
...
* *ストアに格納できるデータの量に制限はありますか。*
    ...
    番号 おを保存データのプッシュ配信エクスポートを削除するまでです。 Blob ストレージの制限に達した場合は配信が停止されますが、その制限には非常に大きな値が設定されています。 使用するストレージの量を管理するのはお客様です。
...
* [Power BI に Application Insights のストリーム](app-insights-export-power-bi.md)
* [ワーカー ロール ][exportcode]
* [Stream Analytics ][exportasa]
