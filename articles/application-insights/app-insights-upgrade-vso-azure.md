...
ApplicationInsights.config を開きます。 ノードがある場合 `ActiveProfile` と `プロファイル`, 以前のバージョンが、およびアップグレードする必要があります。
...
4. SDK: プロジェクトを右クリックし、 [Application Insights の追加 ][greenbrown]します。 これによって SDK がプロジェクトに追加され、さらに Azure で新しい Application Insights リソースが作成されます。
5. ログ: コードに LogEvent() など、以前の API の呼び出しが含まれている場合、それらはソリューションをビルドしようとしたときに見つかります。 ように更新 [[トラック] の新しい API を使用して][track]します。
6. Web ページ: プロジェクトには、web ページが含まれている場合は、スクリプトを置き換えます、 <head> セクションです。一般に Views\Shared\_Layout.cshtml などのマスター ページが 1 つのコピーがあります。[Azure ][usage]します。 
Web ページが logEvent や logPage などの本文にテレメトリの呼び出しを含める場合 [新しい API の ][api]します。
7. サーバー モニター: アプリが IIS で実行されているサービスの場合は、サーバーから Microsoft Monitoring Agent をアンインストールし、 [Application Insights Status Monitor ][redfield]します。
8. Web テスト: web 可用性テストを使用していた場合 [新しいポータルの ][availability], 、そのアラートがあります。
9. アラート: を設定 [[警告] メトリックに対するアラート][alerts] Azure ポータルで。
...
2. Web サービスを再起動します。
3. Microsoft Azure ポータルで [新しい Application Insights リソースを ][java]します。 開発用コンピューターで追加 [Java SDK ][java] 、web プロジェクトにします。
4. スクリプトを置き換えます、 <head> セクションでは、web ページのです。(サーバー側には 1 つのコピーだけが含まれている可能性があります)。[Azure ][usage]します。 
Web ページが logEvent や logPage などの本文にテレメトリの呼び出しを含める場合 [[トラック] の新しい API を使用するように更新][track]します。
8. Web テスト: web 可用性テストを使用していた場合 [新しいポータルの ][availability], 、そのアラートがあります。
9. アラート: を設定 [[警告] メトリックに対するアラート][alerts] Azure ポータルで。
