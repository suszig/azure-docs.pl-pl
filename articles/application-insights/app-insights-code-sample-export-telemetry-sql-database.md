...
この記事では、Application Insights からエクスポートされた JSON データを処理する方法を示します。 たとえばから製品利用統計情報データを移動するためのコードを書く [Visual Studio Application Insights][start] を使用して Azure SQL データベースに [連続エクスポート][export]します。 (これを実現できます [Stream Analytics を使用して、](app-insights-code-sample-export-sql-stream-analytics.md), 、いくつかのコードを表示するが、ここでのねらいです)。 
