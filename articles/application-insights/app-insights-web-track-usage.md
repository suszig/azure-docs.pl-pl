...
* **ユーザー、セッションおよびページ ビュー データ** - ですぐに提供されています。  
* **カスタム テレメトリ** -する [コードを記述][api] 、アプリのユーザー エクスペリエンスを使用してユーザーを追跡します。 
...
サインイン、 [Azure ポータル][portal], 、アプリケーションのリソースを参照し、[使用状況] をクリックします。

![](./media/app-insights-web-track-usage/14-usage.png)

* **ユーザー:** グラフの時間範囲内での別個のアクティブなユーザーの数。 
* **セッション:** アクティブなセッションの数
...
**セッションの期間** は、 [メトリック][metrics] セッションの最初と最後のテレメトリ項目の時間範囲を記録します。 (タイムアウト期間は含まれません)。
...
既定では、ユーザーは、Cookie を配置することによって識別されます。 複数のブラウザーやデバイスを使用しているユーザーは複数回カウントされます。 (表示 [認証されたユーザー](#authenticated-users)


 **ユーザー カウント** この間隔中に記録されたアクティビティと、一定の間隔でメトリックを一意のユーザーの数として定義されます。 その結果、時間範囲を1 時間未満に設定した場合、長いセッションを持つユーザーは複数回カウントされることがあります。

**New Users** counts the users whose first sessions with the app occurred during this interval. Cookie によってユーザーをカウントする既定のメソッドを使用した場合、Cookie をクリアするか、新しいデバイスやブラウザーを使用して最初にアプリにアクセスしているユーザーも含まれます。
![使用状況] ブレードで、新しいユーザーを確認ユーザー] グラフをクリックします。](./media/app-insights-web-track-usage/031-dual.png)
...
ただし、Application Insights の [web テスト][availability], ユーザー id が自動的に設定に基づいて POP の場所、およびセッション id は、テストの実行 id に基づいて設定されます。 既定のレポートでは、合成のトラフィックは既定により、フィルター処理され、これらのユーザーとセッションは除外されます。 ただし、合成トラフィックが含まれる場合は、全体的なユーザーとセッション数はわずかに増加する可能性があります。
...
![イベントの一覧で、イベントを開くし、[...] をクリックし、 その他のプロパティを表示するには](./media/app-insights-web-track-usage/11-details.png)
