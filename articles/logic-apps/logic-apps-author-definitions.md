...
Można utworzyć definicji przepływu pracy dla [Azure Logic Apps](logic-apps-overview.md) prosty, deklaratywny języka JSON. Jeśli nie jest jeszcze, najpierw należy przejrzeć [tworzenie pierwszej aplikacji logiki z projektanta aplikacji logiki](quickstart-create-first-logic-app-workflow.md). Zobacz też [pełne odwołania dla języka definicji przepływu pracy](http://aka.ms/logicappsdocs).
