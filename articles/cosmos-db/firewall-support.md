...
Domyślnie konto bazy danych Azure DB rozwiązania Cosmos jest dostępny z publicznej sieci internet, tak długo, jak długo żądanie towarzyszy token prawidłowego autoryzacji. Aby skonfigurować kontroli dostępu na podstawie zasad IP, użytkownik musi podać zbiór adresów IP i zakresów adresów IP w postaci CIDR do uwzględnienia jako lista dozwolonych adresów IP klienta, konto bazy danych. Po zastosowaniu tej konfiguracji wszystkich żądań wysyłanych z komputerów spoza tej listy dozwolonych jest blokowana przez serwer.  Na poniższym diagramie przedstawiono połączenia przetwarzania przepływu kontroli dostępu na podstawie adresu IP:
...
## <a name="connections-from-azure-paas-service"></a>Połączenia z usługą Azure PaaS 
Na platformie Azure, PaaS usług takich jak Azure Stream analytics — usługi Azure Functions są używane w połączeniu z bazy danych Azure rozwiązania Cosmos. Aby włączyć dostęp do bazy danych Azure rozwiązania Cosmos konto bazy danych z tymi rodzaj usługi, której adres IP nie jest jeszcze dostępna, adres IP 0.0.0.0 należy dodać do listy dozwolonych adresów IP skojarzonych z Twoim kontem bazy danych Azure DB rozwiązania Cosmos przez [Konfigurowanie zasad kontroli dostępu IP](#configure-ip-policy).  Dzięki temu, czy usług Azure PaaS dostęp do konta bazy danych Azure rozwiązania Cosmos, którego ta reguła. 
