...
> [AZURE.NOTE] プログラムによって作成された stream Analytics のジョブでは、監視を既定で有効にすることはありません。  ジョブの監視] ページに移動し、有効にする] ボタンをクリックして、Azure ポータルで監視を有効にできます手動でまたは次にある手順に従って、プログラムで行うことができます [Azure Stream Analytics - Programatically Monitor Stream Analytics のジョブ](stream-analytics-monitor-jobs.md)
...
JSON ファイルの構造と内容の詳細についてを参照してください、 [入力の作成 (Azure Stream Analytics)][msdn-rest-api-create-stream-analytics-input] のセクションで、 [Stream Analytics 管理 REST API リファレンス ライブラリ][stream.analytics.rest.api.reference]します。
...
JSON ファイルの構造と内容の詳細についてを参照してください、 [Stream Analytics ジョブの作成][msdn-rest-api-create-stream-analytics-job] のセクションで、 [Stream Analytics 管理 REST API リファレンス ライブラリ][stream.analytics.rest.api.reference]します。
...
JSON ファイルの構造と内容の詳細についてを参照してください、 [出力の作成 (Azure Stream Analytics)][msdn-rest-api-create-stream-analytics-output] のセクションで、 [Stream Analytics 管理 REST API リファレンス ライブラリ][stream.analytics.rest.api.reference]します。
...
JSON ファイルの構造と内容の詳細についてを参照してください、 [変換の作成 (Azure Stream Analytics)][msdn-rest-api-create-stream-analytics-transformation] のセクションで、 [Stream Analytics 管理 REST API リファレンス ライブラリ][stream.analytics.rest.api.reference]します。
