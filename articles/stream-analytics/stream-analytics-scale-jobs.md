...
分析ジョブを調整し、計算する方法について説明 *ストリーミング ユニット* Stream Analytics の Stream Analytics ジョブを構成することでスケールする方法は、分析クエリ定義のチューニングのパーティションを入力し、ジョブのストリーミング ユニットの設定です。 
...
ジョブにはデータ ストリーミング用に少なくとも 1 つの入力ソースが必要です。 データ ストリームの入力ソースは、Azure Service Bus Event Hub または Azure BLOB ストレージに格納できます。 詳細については、次を参照してください。 [Azure Stream Analytics の概要](stream-analytics-introduction.md), 、[Azure Stream Analytics の使用を開始](stream-analytics-get-started.md), 、および [Azure Stream Analytics 開発者ガイド](../stream-analytics-developer-guide.md)します。
...
特定のジョブに必要な SU の数の選択は、入力のパーティション構成と、ジョブに定義されたクエリによって異なります。  Azure ポータルを使用して、1 つのジョブにクォータまでのストリーミング ユニットを設定できます。 また、1 つの Azure サブスクリプションの既定のクォータでは、ストリーミング ユニットの最大数は特定のリージョン内のすべての分析ジョブを合わせて 50 個です。  サブスクリプションのストリーミング ユニットを増やすには、次にお問い合わせください。 [Microsoft サポート](http://support.microsoft.com)します。
...
- 入力ソースはパーティション分割する。 詳細については、次を参照してください。 [Azure Stream Analytics 開発者ガイド](../stream-analytics-developer-guide.md) と [Event Hub プログラミング ガイド](../event-hubs/event-hubs-programming-guide.md)します。
- クエリの SELECT ステートメントは、パーティション分割された入力ソースから読み取る。
- ステップ内のクエリが必要、 **Partition By** キーワード
...
クエリがパーティション分割されている場合、入力イベントは処理されて個々のパーティション グループに集計されます。 出力イベントは、それぞれのグループに対しても生成されます。 分割予期しない結果になることができるときに、 **Group by** フィールドが入力データ ストリームでパーティション キーではありません。 たとえば、上のサンプル クエリの TollBoothId フィールドは Input1 のパーティション キーではありません。 TollBooth 1 のデータは、複数のパーティションに分散できます。
...
>[AZURE.NOTE] 2 つのストリームを結合する場合は、結合を実行したことと、両方のストリームに同じ数のパーティションがある列のパーティション キーでストリームがパーティション分割されることを確認します。
...
2. クリックして **Stream Analytics** 左側のウィンドウでします。
3. スケールする Stream Analytics ジョブをクリックします。
4. クリックして **スケール** 、ページの上部にあります。
