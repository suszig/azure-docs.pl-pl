...
2.  各 Stream Analytics ジョブが単一の Event Hub コンシューマー グループから読み取るようにするのがベスト プラクティスです。 手順を説明する、以下にコンシューマー グループを作成するプロセスとすることができます [コンシューマー グループの詳細について](https://msdn.microsoft.com/library/azure/dn836025.aspx)します。 コンシューマー グループを作成するには、新しく作成した Event Hub に移動し、をクリックして、 **コンシューマー グループ** ] タブの [ **作成** 、ページの下部にあるし、コンシューマー グループの名前を指定します。
...
6.  移動、 **ダッシュ ボード** ] をクリック **接続情報** 下部] ページで、[コピーして接続情報を保存します。
...
1.  Azure ポータルで、クリックして **新規 > Data Services > Stream Analytics > 簡易作成**します。
2.  次の値を指定し、クリックして **Stream Analytics ジョブの作成**:
    ...
    * **ストレージ アカウント**: を使用してこのリージョン内で実行されているすべての Stream Analytics ジョブの監視データを格納する Azure ストレージ アカウントを選択します。 既存のストレージ アカウントを選択するか、新しいストレージ アカウントを作成することができます。
...
4.  3 ページ目で、次の値を入力または選択します。
    ...
    * **入力のエイリアス**: このジョブの入力などのわかりやすい名前を入力 *CallStream*します。 後でクエリでこの名前を使用します。
...
#### 省略可能: サンプルの入力データ
実際のジョブ データに対するクエリを検証するには、使用することができます、 **サンプル データ** 、ストリームからイベントを抽出し、作成する機能、します。テスト用のイベントの JSON ファイルです。  次の手順は、これを行う方法を説明し、サンプルも提供しています [Telco.json](https://github.com/Azure/azure-stream-analytics/blob/master/Sample%20Data/telco.json) テスト用にファイルです。

1.  Event Hub の入力を選択し、クリックして **サンプル データ** ページの下部にあります。
2.  ダイアログ ボックスが表示されますが、指定、 **開始時刻** からのデータの収集を開始して、 **期間** を使用する追加データの量のです。
3.  入力からのデータのサンプリングを開始するには、チェック ボタンをクリックします。  データ ファイルが生成されるまでに、1 ～ 2 分かかります。  プロセスが完了したら、クリックして **詳細** ダウンロードして保存します。生成される JSON ファイルです。
...
1.  コード エディターでクエリを次のように変更します。
    ...
    このクエリを使用して、 **Timestamp By** 一時的な計算に使用されるペイロードのタイムスタンプ フィールドを指定するキーワードです。 このフィールドが指定されていない場合は、イベントがイベント ハブに到着した時間を使用してウィンドウ化操作が実行されます。 参照してください [「到着時間とアプリケーション時間」Stream analytics クエリ言語リファレンス](https://msdn.microsoft.com/library/azure/dn834998.aspx)します。

    各ウィンドウの最後のタイムスタンプを使用してアクセスできることに注意してください、 **System.Timestamp** プロパティです。
...
1.  既存のストレージ アカウントを使用して、クリックして新しいストレージ アカウントを作成または **新規 > DATA SERVICES > ストレージ > 簡易作成** し、指示に従います。
2.  ストレージ アカウントを選択して [ **コンテナー** クリックして、ページの上部にある **追加**します。
3.  指定、 **名前** コンテナーとセットの **アクセス** パブリック Blob にします。

## ジョブの出力の指定

1.  Stream Analytics ジョブをクリックして **出力** クリックして、ページの上部にある **出力の追加**します。 表示されたダイアログ ボックスには、出力を設定する手順がいくつか表示されます。
2.  選択 **BLOB ストレージ**, 、右側のボタンをクリックします。
3.  3 ページ目で、次の値を入力または選択します。

    * **出力の別名**: このジョブの出力のフレンドリ名を入力します。
    ...
    * **ファイル名のプレフィックス**: blob 出力の書き込み時に使用するファイルのプレフィックスを入力します。
...
2.  表示されるダイアログ ボックスで選択 **ジョブ開始時刻** ] ダイアログ ボックスの下部にあるチェック ボタンをクリックします。 ジョブの状態が変わります **開始** は、すぐに移動して **を実行している**します。
