...
6.  [新しいクエリ] をクリックします。  
![図 19][graphic19]
7.  次のクエリの種類 <login_name> をユーザー名と交換 <enterStrongPasswordHere> 、新しいパスワードで。  
...
12. [新しいクエリ] をクリックします。  
![図 24][graphic24]
13. 次のクエリの種類 <user_name> このデータベースのコンテキストでこのログインを識別する名前の (指定したのと同じ値を指定する <login_name>, など) と交換 <login_name> を新しいユーザー名。  
`< User_name > からのログイン < login_name > [ユーザーの作成`
14. [実行] をクリックします。  
![図 25][graphic25]
...
1.  第 7 部 1 の手順と次のクエリでの型からクエリ ウィンドウに移動して置き換える <previous_login_name> ジョブで使用されていたユーザー名。  
