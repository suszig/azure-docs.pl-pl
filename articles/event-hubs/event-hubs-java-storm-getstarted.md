...
> [AZURE.NOTE] このチュートリアルでは、Storm をローカル モードで開発目的にのみ使用します。 参照してください、 [HDInsight Storm の概要][] と公式 [Apache Storm の][] Storm の開発とパターンの詳細についてはドキュメントです。

## 次のステップ

Event Hubs と Storm を統合するアプリケーションの開発には、次のリソースを使用できます。

- [Storm と HDInsight を使用したセンサー データの分析] は、Hadoop クラスター内のセンサー データを取り込むには、Event Hubs、Storm、および HBase を使用して完全なシナリオ チュートリアルです。
