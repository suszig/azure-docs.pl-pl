...
6. をクリックして、 **構成** ] タブの上部にある、という名前のルールを追加 **SendRule** と *送信* 権限でという別のルールを追加する **ReceiveRule** と *リッスン* 権限、およびクリック **保存**します。

    ![][5]

7. 同じページに対して生成されたキーを書き留めます **ReceiveRule**します。

    ![][6c]

8. クリックして、 **ダッシュ ボード** 上部にあるタブをクリックして **接続情報**します。 2 つの接続文字列をメモします。
