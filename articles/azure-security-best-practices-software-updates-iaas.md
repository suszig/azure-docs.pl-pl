...
モデルでは従来のデータ センター全体のインフラストラクチャが設置は、オペレーティング システム、アプリケーションの更新を管理するためのフルの責任を持つ、ネットワーク デバイス (ルーター、スイッチなど) およびハードウェア (ファームウェア)。 IaaS のシナリオにおいても、オペレーティング システムおよびアプリケーションの更新はユーザーが管理する必要がありますが、オペレーティング システムとアプリケーションの基盤となるインフラストラクチャ全体は Microsoft によって管理されます。 これらすべてのモデルでは、お客様はデータの所有者であり、エンド ポイント レベルでデータを保護する責任を担っています。
...
-   [Azure の仮想マシンをバックアップします。](../backup/backup-azure-vms.md)
