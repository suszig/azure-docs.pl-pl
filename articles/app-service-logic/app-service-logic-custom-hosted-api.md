...
これは、ロジック アプリが Active Directory に対する認証に使用する ID です。 だけ *必要がある* これを行う 1 回、ディレクトリなどを選択できます、ロジック アプリのすべての同じ id を使用したい場合に、ロジック アプリごとの一意のものを作成することも可能性がありますができます。 この操作は UI で、または PowerShell を使用して実行できます。
