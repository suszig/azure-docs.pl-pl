...
2. 「Azure Service Bus コネクタ」を検索して選択し、選択 **作成**します。
...
4. 次のパッケージの設定を入力します。
    ...
    名前 |説明
--- | ---
接続文字列 |Azure Service Bus の接続文字列。 たとえば、入力: *Endpoint=sb://[namespace].servicebus.windows.net です。SharedAccessKeyName [name] を =;SharedAccessKey = [key]*します。
エンティティ名 |キューまたはトピックの名前を入力します。
サブスクリプション名 |メッセージを受信するサブスクリプションの名前を入力します。
