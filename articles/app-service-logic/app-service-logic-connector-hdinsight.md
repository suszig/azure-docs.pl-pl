...
2. 「HDInsight コネクタ」を検索して選択し、選択 **作成**します。
3. 名前、App Service プラン、その他のプロパティを入力します。
4. パッケージ設定で HDInsight クラスターのユーザー名とパスワードを入力します。 選択 **OK**します。
5. 選択 **作成**:  
![][1]  

## 証明書の構成 (省略可能) ##

> [AZURE.NOTE] 管理操作を実行する場合は、この手順は必要な (作成し、クラスターを削除する) ロジック アプリでします。
...
2. [セキュリティ] ブレードで、選択 **証明書のアップロード**します。
3. 次のブレードで証明書ファイルを参照して選択します。
4. 証明書を選択したら、選択 **OK**します。

証明書が正常にアップロードされた証明書の詳細が表示されます。  
![][3]

> [AZURE.NOTE] 証明書を変更する場合は、既存の証明書の代わりとして、別の証明書をアップロードできます。
...
シナリオをテストするには、選択 **を今すぐ実行** ロジック アプリを手動で開始します。
