...
2. ダイアログ ボックスで検索 **クラウド** し、[ **Azure リソース グループ**します。 型 a **名前** ] をクリックし、 **OK**します。
    ![新しいプロジェクトを追加します。](./media/app-service-logic-deploy-from-vs/addnewproject.png)

3. 今すぐ場合を選択する必要があります、 **ロジック アプリ** または **ロジック アプリと API アプリ**します。 選択すると **ロジック アプリ** 既存の Api をポイントする必要があります。 選択した場合 **ロジック アプリと API アプリ** し、同時に、新しい空の API アプリを作成することもできます。
    ![Azure テンプレートを選択します。](./media/app-service-logic-deploy-from-vs/selectazuretemplate.png)

4. 選択した後、 **テンプレート** ヒット **OK**します。
...
使用することをお勧め **パラメーター** 定義全体でします。 これは、開発環境と運用環境の両方にデプロイする場合に役立ちます。 その場合、`.param` ファイルに環境固有のすべての構成と、実際の文字列ではなくパラメーターを配置する必要があります。
...
- 使用 [Azure リソース マネージャーの powershell](../powershell-azure-resource-manager.md),  、特に、 [`Get-AzureResource` コマンド](https://msdn.microsoft.com/library/dn654579.aspx) 定義をダウンロードします。
...
1. ソリューション エクスプ ローラーでデプロイを右クリックしに移動 **展開** > **新しいデプロイ.**
    ![新しい展開](./media/app-service-logic-deploy-from-vs/newdeployment.png)
