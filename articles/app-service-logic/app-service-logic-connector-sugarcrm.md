...
4. SugarCRM コネクタのパッケージ設定を構成します。 次に、コネクタを作成するために入力する必要があるパッケージ設定を示します。
    ...
    名前 |必要な |説明
--- | --- | ---
サイトの URL |[はい |SugarCRM インスタンスの URL を入力します。 たとえば、https://abcde1234.sugarcrm.com を入力します。
クライアント Id |[はい |SugarCRM で OAUTH 2.0 キーのコンシューマー キーを入力します。 
クライアント シークレット |[はい |OAUTH のコンシューマー シークレットを入力します。
ユーザー名 |[はい |SugarCRM ユーザーのユーザー名を入力します。
パスワード |[はい |SugarCRM ユーザーのパスワードを入力します。
モジュール名 |[はい |操作を実行する SugarCRM モジュール (アカウント、連絡先、製品) などを入力してください。<br><br>例: アカウント、潜在顧客、連絡先  
...
9.  [...] をクリックします。 [アカウントの作成アクションの出力から"id"値を取得するレコード Id]: の横にあります。  
![][7]
