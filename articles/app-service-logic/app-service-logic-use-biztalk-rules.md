...
1. 新しく開いたブレードで、次の情報を入力します。
    ...
    1. App Service プラン]-選択するか新しい App Service プランの作成
    1. [価格レベル] - このアプリが属する価格レベルを選択します
    1. [リソース グループ] - このアプリが属するリソース グループを選択します
    2. サブスクリプションで使用するサブスクリプションを選択し
...
1. BizTalk は、[参照] に移動して、ポータルから API アプリをルールの起動] メニューの [API アプリ - ><Your Rules API App>します。これはアクセス ルール API アプリのダッシュ ボードに次のようにします。

   ![Alt text][4]

2.「ボキャブラリ定義」を選択します。 ボキャブラリ作成画面を表示するとこの
3.新しいボキャブラリの定義の追加を開始するには、[追加] を選択します。
...
1.  [追加] をクリックすると、[定義の追加] ブレードが新しく開きます。 次の値を入力します。
  ...
  5. [入力] – ここには定義する値を入力します。 ここに入力する値は選択したデータ型と一致する必要があります。 一連のコンマ、またはキーワードを使用して値の範囲で区切られた値の 1 つの値を入力するか、 *に*します。 たとえば、一意の値を 1 を入力できます。セット 1, 2, 3 です。または 1 ~ 5 の範囲。 番号の範囲のみがサポートされているに注意してください。
  6. *[OK]* を選択します。
...
[一括追加] を選択すると、新しいブレードを見ていきます。 まず、複数の定義を作成するスキーマを選択します。 これを選択すると、既にアップロード済みのスキーマの一覧から選択すると、いずれかにするか、新しくアップロードを許可する、新しいブレードが開きます。
これで XPATHS プロパティのロックが解除されます。 これを選択すると、複数のノードを選択できるスキーマ ビューアーが開きます。
...
開発者が必要なボキャブラリを作成したら、ビジネス アナリストが Azure ポータルを利用してビジネス ポリシーを作成することを想定します。  
    ...
    2.このページでは、ルール アプリがポリシーの一覧を示します。 アナリストは、ポリシー名を入力し、tab キーを 2 回押すだけで、新しいポリシーを追加できます。 1 つのルール API アプリに複数のポリシーを含めることができます。  
    3.作成したポリシーを選択するとかかりますユーザー ポリシーの詳細ページに、ポリシー内のルールを確認できるあります。  
    ![Alt text][8]
    4.新しいルールを追加するには、[追加] を選択します。 新しいブレードが開きます。

## ルールの作成

ルールは条件とアクションのステートメントで構成されます。 条件が true に評価された場合、アクションが実行されます。 [ルールの作成] ブレードでは、ポリシーに対して一意のルール名を指定し、説明をオプションで入力します。
複雑な条件付きステートメントを作成する条件 (IF) ボックスを使用できます。 次のキーワードがサポートされています。
1.  And – 条件演算子
2.  Or – 条件演算子
3.  does\_not\_exist
4.  exists
...
Action(THEN) ボックスには、複数のステートメントを実行するのにはアクションを作成する、行ごとに 1 つを含めることができます。 次のキーワードがサポートされています。
1.  equals
2.  false
3.  true
4.  halt
...
条件とアクションのボックスは、ルールを簡単に作成するための Intellisense を提供します。 この機能は Ctrl キーを押しながら Space キーを押すか、入力を開始することによって起動されます。 入力した文字に一致するキーワードが自動的に下にフィルターされ、表示されます。 Intellisense ウィンドウにはすべてのキーワードとボキャブラリの定義が表示されます。
![Alt text][9]

## 明示的なフォワード チェーン

BizTalk ルールのサポートが明示的な場合は、ユーザーが特定のアクションに応答内のルールを再評価するには、フォワード チェーン、それらをトリガーできますこの特定のキーワードを使用しています。 次のキーワードがサポートされます。
   1.   更新 <vocabulary definition> – このキーワードは、条件内の指定したボキャブラリの定義を使用してすべてのルールを再評価します。
...
ポリシーは作成とテストが完了すると、使用できます。 新しいロジック アプリを作成するには、ポータルのホーム ページの左側から [Logic Apps] を選択します。 ロジック アプリが作成されたら、それを起動し、*[トリガーとアクション]* を選択します。 次に、*[最初から作成]* テンプレートを選択できます。 BizTalk ルールの API アプリをロジック アプリに追加するための手順に従います。 これを完了すると、どのルールの API アプリ (アクション) をターゲットを選択するオプションがあります。 アクションには、実行される一連のポリシーが含まれます。 付与する必要があるポリシーにその後、入力が必要な特定のポリシーを選択します。 ユーザーはルール API アプリのダウンストリームからの出力を、後続の決定に使用できます。

## API 経由でのルールの使用

豊富な Api を使用しても、ルール API アプリを呼び出すことができます。 このユーザーは、ロジック アプリの使用だけに限定されず、REST の呼び出しで任意のアプリケーションでルールを使用できます。 利用できる REST API　を確認するには、ルールのダッシュボードで [API の定義] レンズをクリックします。
