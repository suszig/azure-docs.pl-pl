...
モバイル アプリがプラットフォーム固有の仕様を使用して `MobileServiceClient.LoginAsync` ログイン インターフェイスとキャッシュ データを表示するためにメソッドです。 定義は、Xamarin フォーム プロジェクトでの認証に、 `IAuthenticate` ポータブル クラス ライブラリ内のインターフェイスです。 サポートするプラットフォームごとに、このインターフェイスをプラットフォーム固有のプロジェクト内に実装します。

また、ポータブル クラス ライブラリに定義されているユーザー インターフェイスを更新して、ログイン ボタンを追加します。 ユーザーはアプリの起動後に、このボタンをクリックして認証する必要があります。

1. Visual Studio または Xamarin Studio で、**portable** プロジェクトから App.cs を開きます。 次の追加 `を使用して` ステートメントをファイルにします。

        using System.Threading.Tasks;

2. App.cs の次のコードを追加 `IAuthenticate` インターフェイス定義の直前に、 `アプリ` クラス定義です。
...
4. **portable** プロジェクトから TodoList.xaml.cs を開きます。 次のフラグを追加、 `TodoList` をユーザーが認証されているかどうかを示すクラスです。

        bool authenticated = false;

5. TodoList.xaml.cs 更新プログラムで、 `OnAppearing` メソッド、ユーザーが認証されている場合にのみ、項目が更新できるようにします。
...
6. コンス トラクターの上部にある、TodoList.xaml.cs で、 `TodoList` クラスは、次のようなログイン ボタンを定義し、ハンドラーをクリックしています.
...
3. 次に、MainActivity.cs droid プロジェクトを開き、次の追加 `を使用して` ステートメントです。

        using Microsoft.WindowsAzure.MobileServices;
        using System.Threading.Tasks;

4. 更新プログラム、 `MainActivity` を実装するクラス、 `IAuthenticate` インターフェイスです。

        public class MainActivity : global::Xamarin.Forms.Platform.Android.FormsApplicationActivity, IAuthenticate

5. 更新プログラム、 `MainActivity` クラスを追加することで、 `MobileServiceUser` フィールドおよび `認証` メソッドをサポートするために、次に示す、 `IAuthenticate` インターフェイスです。

 異なるを使用する場合 `MobileServiceAuthenticationProvider` Facebook の代わりにもその変更を行います。
...
6. 更新プログラム、 `OnCreate` のメソッド、 `MainActivity` クラスをアプリケーションの読み込み前に、認証システムを初期化します。
...
3. 次に、iOS のプロジェクトで appdelegate.cs プロジェクトを開き、次を追加 `を使用して` ステートメントです。

        using Microsoft.WindowsAzure.MobileServices;
        using System.Threading.Tasks;

4. 更新プログラム、 `AppDelegate` を実装するクラス、 `IAuthenticate` インターフェイスです。

        public partial class AppDelegate : global::Xamarin.Forms.Platform.iOS.FormsApplicationDelegate, IAuthenticate

5. 更新プログラム、 `AppDelegate` クラスを追加することで、 `MobileServiceUser` フィールドおよび `認証` メソッドをサポートするために、次に示す、 `IAuthenticate` インターフェイスです。
...
6. 更新プログラム、 `FinishedLaunching` のメソッド、 `AppDelegate` クラスをアプリケーションの読み込み前に、認証システムを初期化します。
...
3. 次に、WinApp プロジェクトで MainPage.xaml.cs を開き、次を追加 `を使用して` ステートメントです。置換 <*Your portable class library namespace*> ポータブル クラス ライブラリの名前空間。

        using Microsoft.WindowsAzure.MobileServices;
        using System.Threading.Tasks;
        using <Your portable class library namespace>;

4. 更新プログラム、 `MainPage` を実装するクラス、 `IAuthenticate` インターフェイスです。

        public sealed partial class MainPage : IAuthenticate

5. 更新プログラム、 `MainPage` クラスを追加することで、 `MobileServiceUser` フィールドおよび `認証` メソッドをサポートするために、次に示す、 `IAuthenticate` インターフェイスです。
...
6. コンス トラクターを更新、 `MainPage` クラスをアプリケーションの読み込み前に、認証システムを初期化します。置換 <*Your portable class library namespace*> ポータブル クラス ライブラリの名前空間。
...
3. 次に、WinPhone81 プロジェクトで MainPage.xaml.cs を開き、次を追加 `を使用して` ステートメントです。置換 <*Your portable class library namespace*> ポータブル クラス ライブラリの名前空間。
...
7. Windows Phone では、さらにログインを追加する必要があります。 App.xaml.cs を開き、次の追加 `を使用して` ステートメントとコードを `OnActivated` ハンドラーで、 `アプリ` クラスです。
