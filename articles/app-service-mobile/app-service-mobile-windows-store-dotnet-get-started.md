...
このチュートリアルでは、ユニバーサル Windows アプリにクラウドベースのバックエンド サービスを追加する方法を示します。 詳細については、次を参照してください。 [モバイル アプリとは](app-service-mobile-value-prop.md)です。  
...
1. 戻り、 **開始** 、モバイル アプリ バックエンド用のブレードをクリックして **新しいアプリを作成する** > **ダウンロード**, からローカル コンピューターに圧縮されたプロジェクト ファイルを抽出します。
...
5. アプリケーションで、意味のあるテキストをなど入力 *に関するチュートリアルを完了*, で、 **Insert a TodoItem** テキスト ボックスに入力し **保存**します。
...
6. デバッグの停止を右クリックして、 `<your app name>.WindowsPhone` プロジェクトで、をクリックして **スタートアップ プロジェクトとして設定**, 、f5 キーを押します。
