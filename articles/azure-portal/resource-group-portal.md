...
このトピックでは、Azure ポータル内でリソース グループを使用する方法を概説します。 現時点では、すべてのサービスでポータルまたはリソース マネージャーがサポートされているわけではありません。 これらのサービスを使用する必要がある、 [旧ポータル](https://manage.windowsazure.com)します。 各サービスの状態、を参照してください [サービス、領域、および API のバージョンに対応するリソース マネージャー](resource-manager-supported-services.md)。
