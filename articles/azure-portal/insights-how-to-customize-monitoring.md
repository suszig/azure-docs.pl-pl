...
3. いずれかのグラフをクリックすると、説明、 **メトリック** ブレードです。 ブレードには、グラフに加えて、メトリック (選択した時間範囲の平均、最小値、最大値など) の集計を示すテーブルが表示されます。 次に、リソースのアラート ルールです。
    ![[メトリック] ブレード](./media/insights-how-to-customize-monitoring/Insights_MetricBlade.png)

4. 表示される行をカスタマイズするには、 **編集** 、グラフ上または **によるグラフの編集** コマンド [メトリック] ブレードをします。
...
6. 時間範囲の変更は、異なる範囲を選択し、簡単 (など **過去 1 時間**) をクリックすると、 **保存** ブレードの下部にあります。 選択することもできます **カスタム**, 、過去 2 週間の任意の期間を選択できます。 たとえば、2 週間全体を表示することも、昨日の 1 時間だけを表示することもできます。 別の期間を指定するには、テキスト ボックスに入力します。
...
3. メトリックが表示されない場合は、事前設定されたメトリックのいずれかをいつでも追加でき、 **編集** のグラフにする必要があるメトリックを表示します。 
...
* [インスタンスの数を自動的にスケール変更](insights-how-to-scale.md) 、サービスの可用性と応答性を確認します。
