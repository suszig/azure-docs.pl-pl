<properties 
   pageTitle ="アプリケーション ゲートウェイの作成 Azure リソース マネージャー templates| を使用します。Microsoft Azure]
   description =「このページには Azure リソース マネージャー テンプレートを使用して Azure Application Gateway を作成する方法」
   documentationCenter ="na"
   サービス =「アプリケーション ゲートウェイ」
   authors ="joaoma"
   manager ="jdial"
   editor="tysonn"/>
<tags 
   ms.service=「アプリケーション ゲートウェイ」
   ms.devlang="na"
   ms.topic=「ヒーロー記事」 
   ms.tgt_pltfrm="na"
...
11. ファイルを保存します。Json テンプレートとなどのオンラインの json 検証ツールを使用してパラメーターのテンプレートをテストできる [JSlint.com](http://www.jslint.com/)
...
3. 必要に応じて、新しいリソース グループを使用して、作成 `New-azureresourcegroup` コマンドレットです。次の例では、米国東部の場所に AppgatewayRG と呼ばれる新しいリソース グループを作成します。
...
3. 必要に応じて、次のように **azure group create** で新しいリソース グループを作成します。 コマンドの出力が表示されます。 出力の後に表示される一覧では、使用されたパラメーターについて説明されています。 リソース グループの詳細については、次を参照してください。 [Azure リソース マネージャーの概要](resource-group-overview.md)します。
...
4. 上記でダウンロードして変更したテンプレート ファイルとパラメーター ファイルを使用して、**azure group deployment create** コマンドレットを実行して新しい VNet をデプロイします。 出力の後に表示されるリストは、使用されたパラメーターについての説明です。
