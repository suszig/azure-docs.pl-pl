...
使用して、 `Get AzureRmApplicationGateway` コマンドレット ゲートウェイの状態を確認します。 前の手順で *Start-AzureApplicationGateway* が成功した場合、State は *Running* になります。
...
次の例では、最初の行で `Stop-AzureRmApplicationGateway` コマンドレットを示し、続いてその出力を示します。
