...
2. [ゲートウェイを構成します。](#configure-the-gateway)
3. [ゲートウェイ構成を設定します。](#set-the-gateway-configuration)
4. [ゲートウェイを起動します。](#start-the-gateway)
4. [ゲートウェイを確認します。](#verify-the-gateway-status)
...
- 値、 *名* で指定された要素 *FrontendIPConfiguration* httplistener のために使用する必要があります *FrontendIP* 、FrontendIPConfiguration を参照する要素。
...
次に、アプリケーション ゲートウェイを設定します。 使用することができます、 `セット AzureApplicationGatewayConfig` コマンドレット、構成オブジェクトまたは構成 XML ファイルです。
...
使用して、 `Get AzureApplicationGateway` コマンドレット ゲートウェイの状態を確認します。 前の手順で *Start-AzureApplicationGateway* が成功した場合、State は *Running* になり、Vip と DnsName に有効な値が入力されます。 このサンプルの最初の行はコマンドレットを示し、その後に出力が続きます。 このサンプルでは、ゲートウェイが実行中で、トラフィックを受け取る準備ができています。
