...
Azure では、レイヤー 4 の負荷 (TCP または UDP) トランスポート レベルで作業し、アプリケーション ゲートウェイ サービスに負荷分散されているすべての着信ネットワーク トラフィックを Azure ロード バランサーを使用して分散を提供します。 Application Gateway は、HTTP トラフィックにルーティング ルールを適用して レベル 7 (HTTP) の負荷分散を提供します。 アプリケーション ゲートウェイを作成するときに、エンドポイント (VIP) が関連付けられているし、受信ネットワーク トラフィックのパブリック IP として使用します。
...
>[AZURE.NOTE] これはアプリケーション ゲートウェイ スループットのおおよそのガイダンスです。 実際のスループットは、ページの平均サイズ、バックエンドのインスタンスの場所、サーバーのページへの処理時間などさまざまな環境の詳細に依存します。
