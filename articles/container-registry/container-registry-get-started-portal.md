---
title: "Tworzenie rejestru kontenerów platformy Azure — portal | Microsoft Docs"
description: "Rozpoczynanie pracy z tworzeniem rejestrów kontenerów platformy Azure za pomocą witryny Azure Portal i zarządzaniem nimi"
...
Użyj witryny Azure Portal, aby utworzyć rejestr kontenerów i zarządzać jego ustawieniami. Tworzenie rejestrów kontenerów i zarządzanie nimi jest także możliwe przy użyciu [poleceń interfejsu wiersza polecenia platformy Azure w wersji zapoznawczej 2.0](container-registry-get-started-azure-cli.md) lub programowo przy użyciu interfejsu [API REST](https://go.microsoft.com/fwlink/p/?linkid=834376) usługi Container Registry.
...
5. W bloku usługi **Container Registry** wprowadź następujące informacje. Po zakończeniu kliknij przycisk **Utwórz**.
   ...
   > Oprócz zapewniania dostępu za pośrednictwem konta administratora rejestry kontenerów obsługują uwierzytelnianie wspierane przez nazwy główne usług w usłudze Azure Active Directory. Więcej informacji i zagadnień do rozważenia znajduje się w temacie [Authenticate with a container registry](container-registry-authentication.md) (Uwierzytelnianie za pomocą rejestru kontenerów).
    ...
    e. **Konto magazynu** — użyj ustawienia domyślnego, aby utworzyć [konto magazynu](../storage/storage-introduction.md), lub wybierz istniejące konto magazynu w tej samej lokalizacji. Pamiętaj, że konta magazynu klasycznego i Premium Storage nie są obsługiwane.
