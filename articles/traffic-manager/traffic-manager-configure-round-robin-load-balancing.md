...
1. 左側のウィンドウで、Azure ポータルで、クリックして、 **Traffic Manager** Traffic Manager] ウィンドウを開くにはアイコン。 Traffic Manager プロファイルを作成がない場合は、次を参照してください。 [Traffic Manager プロファイルの管理](traffic-manager-manage-profiles.md) 、基本的な Traffic Manager プロファイルを作成する手順についてです。
