...
| < 任意 >| 無効| 無効| 無効なプロファイルは監視されません。ただし、無効なプロファイル内のエンドポイントの状態は管理できます。|
...
|---|---|---|---|
| 無効| < 任意 > または、プロファイルで定義されているエンドポイントはありません。| 無効| エンドポイントは監視されていません。|
...
>[AZURE.NOTE] 応答メッセージが 200 OK の場合にのみ、Traffic Manager は、エンドポイントがオンラインであると見なします。 200 以外の応答を受信すると、Traffic Manager は、エンドポイントが使用できないと想定し、このチェックを失敗としてカウントします。 トラブルシューティングの詳細については失敗のチェックを参照してください [機能低下状態では、Azure Traffic Manager のトラブルシューティング](traffic-manager-troubleshooting-degraded.md)します。
...
次の表では、入れ子になったプロファイルに対する子と親のプロファイルおよび minChildEndpoints 設定に対する Traffic Manager の監視の動作について説明します。 詳細については、を参照してください [Traffic Manager は何ですか?](traffic-manager-overview.md)。.
...
[機能低下状態では、Azure Traffic Manager のトラブルシューティング](traffic-manager-troubleshooting-degraded.md)
