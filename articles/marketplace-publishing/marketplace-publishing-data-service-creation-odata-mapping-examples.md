...
---
# <a name="examples-of-mapping-an-existing-web-service-to-odata-through-csdls"></a>Przykładowe mapowanie istniejącej usługi sieci web OData za pomocą CSDLs
> [!IMPORTANT]
...
Użyj POST nieprzetworzone dane, aby utworzyć nowy podrzędny i zwraca jego serwerem zdefiniowane przez URL(location) lub zaktualizować część podrzędnym na serwerze adresu URL.  Gdzie podrzędnym jest typu stream, tj. bez struktury, np. plik tekstowy.  Uważaj POST w nie idempotentności bez lokalizacji.
...
Usuń umożliwia usunięcie określonego identyfikatora URI.
...
Użyj POST nieprzetworzone dane, aby utworzyć nowy podrzędny i zwraca jego serwerem zdefiniowane przez URL(location) lub zaktualizować część podrzędnym na serwerze adresu URL.  Gdzie podrzędnym jest strukturą. Uważaj POST nie jest idempotentności bez lokalizacji.
...
Aby utworzyć nowy podrzędny lub zaktualizować cały podrzędnym pod adresem URL zdefiniowanego serwera, należy użyć PUT.  W przypadku, gdy podrzędnym jest strukturą, PUT jest idempotentności tak wielu wystąpień spowodują samym stanie, tj x = 5.  Umieść powinien być używany z pełnej zawartości określonego zasobu.
...
Użyj PUT nieprzetworzone dane, aby utworzyć nowy podrzędny lub zaktualizować cały podrzędnym pod adresem URL zdefiniowanego serwera.  Gdzie podrzędnym jest typu stream, tj. bez struktury, np. plik tekstowy.  PUT jest idempotentności tak wielu wystąpień spowodują samym stanie, tj x = 5.  Umieść powinien być używany z pełnej zawartości określonego zasobu.
...
Użyj UZYSKAĆ pierwotnych danych do zwrócenia podrzędny, która nie ma określonej struktury, tj. tekst.
...
Implementowanie stronicowania RESTful za pośrednictwem danych za pomocą GET.  Domyślne stronicowania wynosi 100 wierszy na stronie danych.
