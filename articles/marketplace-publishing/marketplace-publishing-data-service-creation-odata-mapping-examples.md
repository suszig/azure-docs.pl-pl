...
Użyj POST nieprzetworzone dane, aby utworzyć nowy podrzędny i zwraca jego serwerem zdefiniowane przez URL(location) lub zaktualizować część podrzędnym na serwerze adresu URL.  Gdzie podrzędnym to stream, tzn. bez struktury, np. plik tekstowy.  Uważaj POST w nie idempotentności bez lokalizacji.
...
Użyj PUT nieprzetworzone dane, aby utworzyć nowy podrzędny lub zaktualizować cały podrzędnym pod adresem URL zdefiniowanego serwera.  Gdzie podrzędnym to stream, tzn. bez struktury, np. plik tekstowy.  PUT jest idempotentności tak wielu wystąpień spowodują samym stanie, tj x = 5.  Umieść powinien być używany z pełnej zawartości określonego zasobu.
