...
> [AZURE.NOTE] 場合のみ無料サービスを発行する (または、bring your-所有-ライセンスを提供) 税金と bank の情報は不要です。

### 使用する ID とアカウント
理想的には、これは、配布リストまたはセキュリティ グループ (@ azurepublishing など*partnercompany*.com)。 配布リストまたはセキュリティ グループ **必要があります** Microsoft アカウントとして登録されます。

> [AZURE.TIP] 使用して配布リストまたはセキュリティ グループの個別の依存関係が削除されるためにも使用できますが、個々 のアカウントをお勧めします。
