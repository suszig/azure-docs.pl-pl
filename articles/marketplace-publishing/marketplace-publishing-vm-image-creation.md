...
この記事では、 **手順 2.**, 、Azure Marketplace に展開する仮想ハード_ディスク (Vhd) の準備について説明します。 VHD は SKU の基礎です。 プロセスは、Linux ベースの SKU または Windows ベースの SKU のどちらを提供するかによって異なります。 この記事では、両方のシナリオについて説明します。 このプロセスを並行して実行できます [アカウントの作成と登録][link-acct-creation]します。
...
- Azure プレビュー ポータル: https://portal.azure.com/#gallery/{PublisherNamespace} します。{OfferIdentifier}{SKUIDdentifier}  
...
1. サインイン、 [公開ポータル][link-pubportal] 、販売者のアカウントを使用しています。
2. 選択、 **仮想マシン** 公開ポータルのタブをクリックします。 フィールドにプラン名を入力します。 通常、プラン名は Azure Marketplace で販売する計画の製品またはサービスの名前です。
3. 選択 **作成**します。
...
> [AZURE.NOTE] Marketplace で提供 URL には、オファーと SKU 識別子が表示されます。

2. **SKU の概要の説明を追加します。**概要説明は顧客に表示されるので、読みやすくする必要があります。 この情報は、"ステージングにプッシュ" フェーズまでロックする必要はありません。 それまでは、自由に編集できます。
3. Windows ベースの SKU を使用する場合、表示されたリンクに従って、Windows Server の承認されたバージョンを取得します。

## 2.Azure と互換性のある VHD の作成 (Linux ベース)
このセクションでは、Azure Marketplace 用 Linux ベースの VM イメージを作成するためのベスト プラクティスについて説明します。 ステップ バイ ステップ チュートリアルでは、次のドキュメントを参照してください: [の作成とアップロード Linux オペレーティング システムを含むバーチャル ハード ディスク][link-azure-vm-1]

> [AZURE.TIP] (たとえば、エージェントのインストール、カーネルのブート パラメーター) は、次の手順の多くは既に自動的に行わ Microsoft Azure イメージ ギャラリーから Linux イメージを使用します。 したがって、ベースとしてこれらのイメージのいずれかを使用して始めれば、Azure 対応ではない Linux イメージを構成する場合と比較して、時間を節約できます。

### 2.1 適切な VHD サイズの選択
発行済み SKU (VM イメージ) は、SKU に対してディスクをサポートしたあらゆる VM サイズに対応するよう設計する必要があります。 推奨サイズに関するガイダンスを提供できますが、これらは強制ではなく、推奨事項として取り扱ってください。

1. Linux オペレーティング システム VHD: VM イメージ内の Linux オペレーティング システム VHD は、30 ～ 50 GB の固定形式 VHD として作成する必要があります。 30 GB を下回らないようにしてください。 物理サイズが VHD サイズより小さいと、VHD が疎になります。 ケースバイケースで、Linux VHD を 50 GB より大きくすることも検討します。 使用することができます、異なる形式で VHD を既にがある場合、 [形式を変更する CONVERT-VHD PowerShell コマンドレット][link-technet-1]します。
...
オペレーティング システムの VHD は、次のように準備が整ったときに必ず最新 [Azure Linux エージェント][link-azure-vm-2] がインストールされています。 RPM または Deb パッケージを使用します。 パッケージの名前は通常は walinuxagent または WALinuxAgent ですが、ディストリビューションを確認してください。 エージェントには、Azure で Linux IaaS をデプロイメントするための主要な機能 (VM プロビジョニング、ネットワーク機能など) が用意されています。  
...
手動でエージェントのインストールを選択したかどうか、 [GitHub リポジトリ][link-github-waagent], をまたは sbin に Waagent ファイルをコピーし、ルート ディレクトリで次のコマンドを実行します。
...
1.  [Linux Integration Services][link-intsvc] カーネルで、3.0 以上が有効にする必要があります。 参照してください [Linux カーネルの要件](../virtual-machines/virtual-machines-linux-create-upload-vhd-generic/#linux-kernel-requirements)します。
2. [カーネル パッチ](https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/commit/drivers/scsi/storvsc_drv.c?id=5c1b10ab7f93d24f29b5630286e323d1c5802d5c) Azure I/O の安定性 (おそらく、すべて最新のカーネルには必要ありませんが、その確認する必要があります)
3. [Python][link-python] 2.6 以降
4. Python pyasn1 パッケージ (まだインストールされていない場合)
5. [OpenSSL][link-openssl] (v1.0 大きいを推奨)
...
顧客に対して SSH を有効にすることを強くお勧めします。 SSH サーバーが有効になっている場合は、SSH keep alive を次のオプションを使用して、sshd 構成に追加: **ClientAliveInterval 180**します。 180 が推奨されますが、許可される範囲は 30 ～ 235 です。 すべてのアプリケーションについて、顧客に仮想マシンへのダイレクト SSH を許可する必要はありません。 SSH が明示的にブロックされている場合、 **ClientAliveInterval** オプションを設定する必要はありません。
...
- ネットワーク構成は制御できる、 **から** と **ifdown** スクリプトです。 Linux エージェントは、これらのコマンドを使ってプロビジョニング中にネットワークを再起動することがあります。
- カスタム ネットワーク構成は使用しないでください。 最後の手順として、Resolv.conf ファイルを削除する必要があります。 これは通常プロビジョニング解除の一部として行います (を参照してください、 [Azure Linux エージェント ユーザー ガイド](../virtual-machines/virtual-machines-linux-agent-user-guide/))。 このステップは、次のコマンドを使って手動で実行することもできます。
...
### 2.8 のベスト プラクティスが存在するセキュリティを確保します。
...
- 構成ファイル内の Provisioning.DeleteRootPassword を "y" に設定して、/etc/shadow からルート パスワードを削除します。 構成ファイルの内容に関するドキュメントは、エージェントの Github リポジトリのページ上の README ファイルの「構成」セクションを参照してください ([https://github.com/Azure/WALinuxAgent](https://github.com/Azure/WALinuxAgent) 下にスクロール) します。  
...
最初に、次のイメージのいずれかから VM の作成にある、 [Microsoft Azure ポータル][link-azure-portal]:

- Windows Server ([2012 R2 Datacenter][link-datactr-2012-r2], 、[2012 Datacenter][link-datactr-2012], 、[2008 R2 SP1][link-datactr-2008-r2])
- SQL Server 2014 ([エンタープライズ][link-sql-2014-ent], 、[標準][link-sql-2014-std], 、[Web][link-sql-2014-web])
- SQL Server 2012 SP2 ([エンタープライズ][link-sql-2012-ent], 、[標準][link-sql-2012-std], 、[Web][link-sql-2012-web])

これらのリンクは、SKU ページの発行ポータルにもあります。

> [AZURE.TIP] 現在の Azure ポータルまたは PowerShell を使用している場合は、2014 年 9 月 8 日に公開し、後で、Windows Server のイメージが承認されます。


### 3.2 Windows ベースの VM の作成
Microsoft Azure ポータルから承認された基本イメージに基づいて、簡単な数ステップ実行して VM を作成できます。 大まかな手順を以下に示します。

1. 基本イメージのページ選択 **バーチャル マシンの作成** を送るには、新しい [Microsoft Azure ポータル][link-azure-portal]します。
...
4. デプロイする VM のサイズを選択します。
    ...
    c.  料金についてを参照してください、 **価格レベルの推奨** セレクターは、ポータルに表示されます。 発行元が指定できるサイズとして、3 つのサイズが推奨されています。 (この場合、発行元はマイクロソフト。)

    ![図][img-portal-vm-size]

5. プロパティを設定します。

    a.  [プロパティの既定値のまま、簡単に展開できる **オプションの構成** と **リソース グループ**します。

    b.   **ストレージ アカウント**, 、オペレーティング システムの VHD を格納するストレージ アカウントを選択することもできます。

    c.   **リソース グループ**, 、VM を配置するための論理グループを選択することもできます。
6. 選択、 **場所** の展開。
...
7. クリックして **作成**します。 VM がデプロイを開始します。 数分でデプロイメントが完了し、SKU のイメージの作成を開始できます。

### 3.3 クラウドでの VHD の開発
リモート デスクトップ プロトコル (RDP) を使用してクラウドで VHD を開発することを強くお勧めします。 プロビジョニングではユーザー名とパスワードを指定して RDP に接続します。

> [AZURE.IMPORTANT] 内部設置型 (これは推奨されません) を参照してください、VHD を開発する場合は [内部設置型仮想マシン イメージを作成する](marketplace-publishing-vm-image-creation-on-premise.md)です。 クラウドで開発する場合は、VHD をダウンロードする必要はありません。


**RDP を使用して接続、 [Microsoft Azure ポータル][link-azure-portal]**

1. 選択 **参照** > **Vm**します。
2. [仮想マシン] ブレードが開きます。 接続する VM が実行していることを確認し、デプロイ済み VM の一覧からその VM を選択します。
3. ブレードが開き、選択した VM の詳細が表示されます。 上部にあるをクリックして **接続**します。
4. プロビジョニング時に指定したユーザー名とパスワードを入力するように求められます。

**PowerShell を使用した RDP 経由の接続**

ローカル コンピューターにリモート デスクトップ ファイルをダウンロードするには、使用、 [Get-azureremotedesktopfile コマンドレット][link-technet-2]します。 このコマンドレットを使用するには、サービス名と VM 名が必要です。 VM を作成した場合、 [Microsoft Azure ポータル][link-azure-portal], 、この情報は VM のプロパティを見つけることができます。

1. Microsoft Azure ポータルで [ **参照** > **Vm**します。
...
RDP の詳細については記載されて MSDN の記事 [RDP または SSH で Azure VM への接続](http://msdn.microsoft.com/library/azure/dn535788.aspx)します。

**VM の構成と SKU の作成**

オペレーティング システム VHD がダウンロードされた後、Hyper-V を使って、SKU の作成を開始するように VM を構成します。 詳細な手順は、次の TechNet リンクで入手できます: [HyperV のインストールおよび構成する VM](http://technet.microsoft.com/library/hh846766.aspx)します。
...
- Windows でイメージは"sysprep"、および構成が行われないをサポートしない、 **sysprep** コマンドです。
- ディレクトリ %windir%\System32\Sysprep から次のコマンドを実行できます。

        sysprep.exe /generalize /oobe /sshutdown

  どのように sysprep のオペレーティング システムに追加された次の MSDN 記事の手順のガイダンス: [の作成とアップロード Azure への Windows Server VHD](../virtual-machines/virtual-machines-create-upload-vhd-windows-server/)します。
...
### 4.1 ユーザー VM イメージの作成
複数の Vm の展開を開始する、SKU からユーザーの VM イメージを作成するを使用する必要があります、 [VM イメージの Rest API の作成](http://msdn.microsoft.com/library/azure/dn775054.aspx) VM イメージとして Vhd を登録します。

使用することができます、 **Invoke-webrequest** コマンドレットを PowerShell から VM イメージを作成します。 次の PowerShell スクリプトは、単一のオペレーティング システム ディスクおよびデータ ディスクを使って VM イメージを作成する方法を示しています。 サブスクリプションと PowerShell セッションが既に設定されている必要があることに注意してください。
...
ユーザー VM イメージから VM を展開するには、現在を使用できる [Azure ポータル](https://manage.windowsazure.com) または PowerShell。

**現在の Azure ポータルからの VM のデプロイ**

1. 移動して **新規** > **コンピューティング** > **仮想マシン** > **ギャラリーから**します。

    ![図][img-manage-vm-new]

2. 移動 **マイ イメージ**, 、元の VM をデプロイする VM イメージを選択します。
  1. に、どのイメージを選択すると、特に注意を払って、 **マイ イメージ** ビューには、オペレーティング システム イメージと VM イメージの両方が一覧表示します。
  2. ほとんどの VM イメージに 1 つ以上のディスクが含まれるため、ディスクの数に注目すると、デプロイするイメージの種類を特定する際に役立ちます。 ただし、のみが単一のオペレーティング システム ディスクとし、必要があります VM イメージは存在することは可能は **ディスク数** 1 に設定します。
...
1. 最初に、証明書ツールをダウンロード、 [Microsoft ダウンロード サイト][link-msft-download]します。
2. 認証ツールを開き、クリックして、 **新しいテストの開始** ] ボタンをクリックします。
3.  **テスト情報** 画面で、テストの実行の名前を入力します。
4. Linux VM か Windows VM かを選択します。 選択に応じてを選択すると、それに続く
オプションです。
...
Linux または Windows ベースのバーチャル マシン イメージの適切なオプションを選択した後、選択 **接続のテスト** ことを確認するには
その SSH.Net または PowerShell は、テスト目的で有効な接続を持っています。 接続が確立されると、選択 **次** テストを開始します。
...
自動テストが済むと、アンケート画面で VM イメージへの追加入力を求められます。  クリックして、質問に回答 **次**します。
...
発行プロセスにおいて、SKU として作成した各 VHD にリンクされた Uniform Resource Identifier (URI) を指定します。 マイクロソフトは、認定プロセスでこれらの VHD にアクセスします。 そのため、VHD ごとに Shared Access Signature URI を作成する必要があります。 これは、URI で入力する必要があります、
**イメージ** 公開ポータルでタブをクリックします。
...
共有アクセス署名 URI を作成する」の指示を行うことができる [共有アクセス署名、第 1 部: SAS モデルについて][link-azure-1] と [共有アクセス署名、第 2 部: を作成し、SAS を使用して Azure Blob サービスで][link-azure-2]します。

共有アクセス キーを生成するコードを使用して、代わりにしてもツールを使用して記憶域など [Azure ストレージ エクスプ ローラー][link-azure-codeplex]します。

**Azure Storage エクスプローラーを使用した共有アクセス キーの生成**

1. ダウンロード [Azure ストレージ エクスプ ローラー][link-azure-codeplex] 6 およびそれ以上 CodePlex からです。
2. インストールした後、アプリケーションを開きます。
3. クリックして **アカウントを追加**します。

    ![図][img-azstg-add]

4. ストレージ アカウント名、ストレージ アカウント キー、およびストレージ エンドポイントのドメインを指定します。 選択しない **HTTPS を使用する**です。
...
7. コンテナー内の .vhd ファイルを選択すると、クリックして、 **セキュリティ** ] タブをクリックします。

    ![図][img-azstg-setup-4]

8.  **Blob コンテナーのセキュリティ** ] ダイアログ ボックスで、上、既定値をそのまま、 **アクセス レベル** タブをクリックし、をクリックして、 **共有アクセス署名** ] タブをクリックします。
...
9. 次の手順に従って、.vhd イメージの Shared Access Signature URI を生成します。
    ...
    a.  **アクセスが許可されて**: UTC 時刻のデータを保護するには、現在の日付の前日を選択します。 たとえば、今日が 2014 年 10 月 6 日の場合は、2014 年 10 月 5 を選択します。

    b.  **アクセスが許可されている**: の後に、少なくとも 7 ~ 8 日の日付を選択して、 **のアクセスが許可されて** 日付。

    c.  **許可される操作**: を選択、 **リスト** と **読み取り** アクセス許可。

    d.  .Vhd ファイルを正しく選択した場合に、ファイルが表示 **にアクセスする Blob 名** 拡張子 .vhd にします。

    e.  クリックして **署名の生成**します。

    f.   **生成された Shared Access Signature URI このコンテナーの**, 、上記のとおり、次のチェック。

    -   URL の先頭が "https" でないことを確認します。
    -   イメージ ファイル名および ".vhd" が URI に含まれることを確認します。
    -   署名の末尾に "=rl" があることを確認します。 これは、読み取りおよび一覧アクセスが正常に提供されたことを示します。

    g.  確実に、生成された共有アクセス署名の URI は、次のようにクリックします。 **ブラウザーでテスト**します。 ダウンロード プロセスが開始します。
...
1. 移動して、 [公開ポータル][link-pubportal], 、販売者のアカウントを使用してサインインします。
2. 選択、 **VM イメージ** ] タブをクリックします。
3. ページの上部に一覧表示された ID はプラン ID で、SKU ID ではありません。
4. 下にあるプロパティを入力、 **Sku** セクションです。

    ![図][img-pubportal-vm-skus]

5.  **オペレーティング システム ファミリ**, 、オペレーティング システムの VHD に関連付けられているオペレーティング システムの種類をクリックします。
6.  **オペレーティング システム** ボックスで、オペレーティング システムを説明します。 オペレーティング システム ファミリ、種類、バージョン、更新の形式で記載してください。 たとえば、「Windows Server Datacenter 2014 R2」などと入力します。
7. 推奨された 3 つの仮想マシンのサイズを選択します。 これらは、イメージの購入およびデプロイ時に Azure ポータルの [価格レベル] ブレードで顧客に表示されるものです。

  > [AZURE.NOTE] これらは、推奨事項のみです。 顧客はイメージに指定されたディスクを収容できる任意の VM サイズを選択できます。
...
9.  **OS VHD の URL** ボックスに、オペレーティング システムの VHD の URI が作成される共有アクセス署名を入力します。
10. この SKU に関連付けられたデータ ディスクがある場合は、デプロイメント時のこのデータ ディスクのマウント先である論理ユニット番号 (LUN) を選択します。
11.  **LUN X VHD の URL** ボックスに、最初のデータ VHD URI が作成される共有アクセス署名を入力します。
12. クリックして **アップロード テスト結果**します。
13. クリックして **証明書を要求します。**
...
## 次のステップ
前方に移動することができます認定資格の取得、仮想マシン イメージの Sku を送信した後、 [Azure Marketplace のマーケティング コンテンツ ガイド][link-pushstaging]します。 マーケティング コンテンツ、料金、およびより前のバージョンに必要なその他の情報を提供する発行プロセスの手順、 **手順 3: ステージング環境では、VM をテスト**, 、プランをパブリックな可視性と購入の Azure Marketplace にデプロイする前にさまざまなユース ケースのシナリオをテストします。  
