...
この記事 (**手順 2**) では、Azure Marketplace にデプロイする仮想ハード ディスク (VHD) の準備について説明します。 VHD は SKU の基礎です。 プロセスは、Linux ベースの SKU または Windows ベースの SKU のどちらを提供するかによって異なります。 この記事では、両方のシナリオについて説明します。 このプロセスを並行して実行できます [アカウントの作成と登録 ][link-acct-creation]します。
...
1. サインイン、 [公開ポータルの ][link-pubportal] 、販売者のアカウントを使用しています。
...
このセクションでは、Azure Marketplace 用 Linux ベースの VM イメージを作成するためのベスト プラクティスについて説明します。 ステップ バイ ステップ チュートリアルでは、次のドキュメントを参照してください: [の作成とアップロード Linux オペレーティング システム ][link-azure-vm-1]
...
1. Linux オペレーティング システム VHD: VM イメージ内の Linux オペレーティング システム VHD は、30 ～ 50 GB の固定形式 VHD として作成する必要があります。 30 GB を下回らないようにしてください。 物理サイズが VHD サイズより小さいと、VHD が疎になります。 ケースバイケースで、Linux VHD を 50 GB より大きくすることも検討します。 使用することができます、異なる形式で VHD を既にがある場合、 [CONVERT-VHD PowerShell コマンドレットでは ][link-technet-1]します。
...
オペレーティング システムの VHD は、次のように準備が整ったときに必ず最新 [Azure Linux エージェント ][link-azure-vm-2] がインストールされています。 RPM または Deb パッケージを使用します。 パッケージの名前は通常は walinuxagent または WALinuxAgent ですが、ディストリビューションを確認してください。 エージェントには、Azure で Linux IaaS をデプロイするための主要な機能 (VM プロビジョニング、ネットワーク機能など) が用意されています。
...
3. [Python リンク-python][link-python] 2.6 以降
...
- カスタム ネットワーク構成は使用しないでください。 最後の手順として、Resolv.conf ファイルを削除する必要があります。 これは通常プロビジョニング解除の一部として行います (を参照してください、 [Azure Linux エージェント ユーザー ガイド](../virtual-machines/virtual-machines-linux-agent-user-guide/))します。 このステップは、次のコマンドを使って手動で実行することもできます。
...
最初に、次のイメージのいずれかから VM の作成にある、 [Microsoft Azure ポータルの ][link-azure-portal]:
...
1. 基本イメージのページ選択 **バーチャル マシンの作成** を送るには、新しい [Microsoft Azure ポータルの ][link-azure-portal]します。
...
> [AZURE.IMPORTANT] 内部設置型 (これは推奨されません) を参照してください、VHD を開発する場合は [内部設置型仮想マシン イメージを作成する](marketplace-publishing-vm-image-creation-on-premise.md)します。 クラウドで開発する場合は、VHD をダウンロードする必要はありません。


* * RDP を使用して接続する、 [Microsoft Azure ポータルの ][link-azure-portal]* *
...
ローカル コンピューターにリモート デスクトップ ファイルをダウンロードするには、使用、 [Get-azureremotedesktopfile コマンドレット ][link-technet-2]します。 このコマンドレットを使用するには、サービス名と VM 名が必要です。 VM を作成した場合、 [Microsoft Azure ポータルの ][link-azure-portal], 、この情報は VM のプロパティを見つけることができます。
...
- [VM イメージ PowerShell 方法](https://azure.microsoft.com/blog/vm-image-powershell-how-to-blog-post/)
- [Azure の VM イメージについて](https://msdn.microsoft.com/library/azure/dn790290.aspx)
...
その SSH.Net または PowerShell は、テスト目的で有効な接続を持っています。 接続が確立された後、**[次へ]** をクリックしてテストを開始します。
...
共有アクセス署名 URI を作成する」の指示を行うことができる [共有アクセス署名、第 1 部: SAS を理解するモデルの ][link-azure-1] と [共有アクセス署名、第 2 部: を作成し、SAS を使用して、Azure Blob サービスの ][link-azure-2]します。
...
1. 移動して、 [公開ポータルの ][link-pubportal], 、販売者のアカウントを使用してサインインします。
2. **[VM イメージ]** タブを選択します。
...
前方に移動することができます認定資格の取得、仮想マシン イメージの Sku を送信した後、 [Azure Marketplace マーケティング コンテンツ ガイド ][link-pushstaging]します。 発行プロセスのそのステップでは、**ステップ 3: ステージングでの VM プランのテスト**の前に必要なマーケティング コンテンツ、価格、その他の情報を提供します。ステップ 3 では、プランを Azure Marketplace にデプロイして一般に公開して購入できるようにする前にさまざまなユース ケース シナリオをテストします。
