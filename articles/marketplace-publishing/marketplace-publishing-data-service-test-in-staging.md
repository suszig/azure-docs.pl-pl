...
最初の 2 つの手順が完了したら [マイクロソフトの開発者アカウントを作成する](marketplace-publishing-accounts-creation-registration.md) と [公開ポータルで提供してデータ サービスを作成する](marketplace-publishing-data-service-creation.md) Azure Marketplace で利用可能な提案を行うための準備ができたらします。 このトピックは第一ステップ、中間ステップといった「ステージング」について説明します。
...
1.  ログイン、 [ポータル発行](https://publish.windowsazure.com)
2.  選択 **Data Services** 左側のナビゲーション ウィンドウで
3.  ステージング環境にプッシュするプランを選択します。 上記の画面が表示されます。
4.  クリックして **ステージングにプッシュ** ] ボタンをクリックします。  
5.  プランにおいて、ステージングにプッシュする前に完了すべき問題がある場合には、一覧が表示されます。  一覧の各項目をクリックしてそれらの項目を修正してください。 すべての修正が加えのをクリックして **Push to Staging** をもう一度クリックします。
...
> [AZURE.NOTE] これらの Azure サブスクリプション Id はテスト環境とステージングのために必要な [Azure 管理ポータル](https://manage.windowsazure.com)します。 Azure Marketplace でのテストには必要ではありません。
...
## 次のステップ
問題が発生してヘルプを必要としている場合にご連絡をくださいそれらを解決する [Azure パブリッシャー サポート]( http://go.microsoft.com/fwlink/?LinkId=272975)します。

満足と提案をお読みくださいを発行する準備がかどうかは、 [を運用環境にプッシュ要求の承認](marketplace-publishing-push-to-production.md) ドキュメントです。
