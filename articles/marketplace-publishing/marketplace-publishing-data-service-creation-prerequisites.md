...
発行者は、Azure Marketplace でデータ サービスを発行する際に、複数のテクノロジの中から決定できます。 サポートされている主要なテクノロジは以下で説明します。 エンドユーザーどのようなテクノロジを使用すると、データ サービスを発行すると、データを消費するかに関係なく、 **OData フィード** Azure Marketplace のサービスによって公開されます。 OData サービスで確認できますに関する詳細情報 [http://www.odata.org/](http://www.odata.org/)

## SQL Azure データベース

SQL Azure でデータセットを準備することは発行者の責任です。 Azure にサブスクライブし、適切なサイズのデータベースをプロビジョニングし、データを SQL Azure DB にアップロードする必要があります。 また、発行者はデータを常に最新に保つ責任もあります。 詳細については、[あります Azure サービスにサブスクライブする [https://azure.microsoft.com/services/sql-database/](https://azure.microsoft.com/services/sql-database/)
