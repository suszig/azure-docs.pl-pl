...
| 必須| 説明
| :----------------------: | :------: | :----------
| apiVersion| はい| リソースの作成に使用する REST API バージョン。特定のリソースの種類の利用可能なバージョン番号を確認するのを参照してください。 [API のサポートされているバージョン](../resource-manager-supported-services/#supported-api-versions)します。
| type| はい| リソースの種類。
...
**properties** 要素の値は、リソースを作成するために REST API 操作 (PUT メソッド) の要求本文に指定した値とまったく同じです。 参照してください [Azure リファレンス](https://msdn.microsoft.com/library/azure/mt420159.aspx) 配置するリソースの REST API 操作のです。
...
指定した回数だけ反復処理するリソースの種類を作成する場合は、次を参照してください。 [Azure リソース マネージャーでリソースの複数のインスタンスを作成する](resource-group-create-multiple.md)します。
...
- 作成したテンプレートをデプロイする方法を参照してください [Azure リソース マネージャー テンプレートを使用してアプリケーションを配置](resource-group-template-deploy.md)
...
- 利用可能なスキーマを表示するには、を参照してください [Azure リソース マネージャーのスキーマ](https://github.com/Azure/azure-resource-manager-schemas)。
