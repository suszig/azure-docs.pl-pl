---
title: "Dokumentacja zabezpieczeń platformy Azure — samouczki | Microsoft Docs"
description: "Dowiedz się, jak dostosować opcje zabezpieczeń w usługach platformy Azure w celu spełnienia wymagań dotyczących wdrożeń usług i danych w chmurze."
services: security
author: carolz
manager: carolz
layout: LandingPage
ms.assetid: 
ms.service: security
ms.tgt_pltfrm: na
ms.devlang: na
ms.topic: landing-page
ms.date: 01/23/2017
ms.author: carolz
translationtype: Human Translation
ms.sourcegitcommit: 92b37ea86e4744a46efd81e4731c7949413011e3
ms.openlocfilehash: f85d46d513dc4600c09112e1a52e207681da6f38

---
# <a name="security-documentation"></a>Dokumentacja zabezpieczeń

Dowiedz się, jak używać usługi Azure Security w celu zapewnienia bezpieczeństwa usług i danych w chmurze. Samouczki i inne dokumenty ułatwiające dostosowanie opcji zabezpieczeń w usługach platformy Azure w celu spełnienia wymagań dotyczących wdrożeń.

<ul class="panelContent cardsFTitle">
    <li>
        <a href="/azure/security/azure-security-services-technologies">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/security.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Dowiedz się więcej o zabezpieczeniach platformy Azure</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/security/azure-security-getting-started">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/get-started.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Rozpocznij pracę z zabezpieczeniami platformy Azure</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/security/security-network-overview">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/get-started.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Rozpocznij pracę z zabezpieczeniami usługi Networks</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/security/security-storage-overview">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/get-started.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Rozpocznij pracę z zabezpieczeniami usługi Storage</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/security/security-virtual-machines-overview">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/get-started.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Rozpocznij pracę z zabezpieczeniami usługi Virtual Machines</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
</ul>

---





<!--HONumber=Feb17_HO2-->


