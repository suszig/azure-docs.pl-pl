...
Azure AD テナントでは、(既存の Microsoft アカウントを持つユーザー) を除くテナントのすべてのユーザーが、フォームの電子メール アドレスを使ってサインイン `< xyz > @< テナント ドメイン >` 場所 `< テナント ドメイン >` は、テナントまたは、最初に確認済みドメインの 1 つ `<>.... onmicrosoft.com` ドメイン。このタイプのアカウントは "職場または学校のアカウント" であり、"組織アカウント" とも呼ばれます。
