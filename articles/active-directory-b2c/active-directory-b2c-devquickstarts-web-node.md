<properties
    pageTitle="Dodawanie funkcji logowania się do aplikacji sieci Web w środowisku Node.js w usłudze Azure B2C w wersji zapoznawczej | Microsoft Azure"
...
# Wersja zapoznawcza usługi B2C: dodawanie funkcji logowania się do aplikacji sieci Web w środowisku Node.js
...
> [AZURE.NOTE] W tym artykule nie opisano sposobu wdrażania logowania, rejestracji i zarządzania profilami za pomocą usługi Azure Active Directory B2C. Skupiono się tu w szczególności na wywoływaniu interfejsów API sieci Web po uwierzytelnieniu użytkownika. Jeśli nie zostało to jeszcze zrobione, zalecamy na początek zapoznać się z [wprowadzeniem do aplikacji sieci Web w środowisku .NET](active-directory-b2c-devquickstarts-web-dotnet.md) i poznać podstawy usługi Azure Active Directory B2C.
...
## Dodawanie wymagań wstępnych do katalogu
...
## Konfigurowanie aplikacji do korzystania ze strategii Passport-Node.js
...
## Korzystanie z programu Passport do wysyłania żądań zalogowania się i wylogowania do usługi Azure AD

Aplikacja jest teraz prawidłowo skonfigurowana do komunikowania się z punktem końcowym v2.0 za pomocą protokołu uwierzytelniania OpenID Connect. `passport-azure-ad` obsługuje tworzenie komunikatów uwierzytelniania, sprawdzanie poprawności tokenów z usługi Azure AD i podtrzymywanie sesji użytkowników. Pozostaje tylko zapewnić użytkownikom możliwość logowania się i wylogowywania oraz zebrać dodatkowe informacje o użytkownikach, którzy się zalogowali.
...
- `/logout` po prostu wywołuje `logout.ejs` (i trasę). Powoduje wyczyszczenie plików cookie, a następnie przekierowuje użytkownika z powrotem do `index.ejs`.
...
## Tworzenie widoków i tras w aparacie Express w celu wywołania zasad
...
Uruchom plik `node app.js` i przejdź do `http://localhost:3000`
