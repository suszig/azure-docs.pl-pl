<properties
    ...
    description="B2C テナントからのトークンを受け付ける NodeJS Web API を作成する方法"
...
> [AZURE.NOTE] この記事では、Azure AD B2C でサインイン、サインアップ、プロファイルの管理を実装する方法については説明しません。 ユーザーが既に認証された後での Web API の呼び出しに焦点を合わせています。 まだインストールしていない場合、これを最初にください、 [.NET Web アプリケーション チュートリアル入門](active-directory-b2c-devquickstarts-web-dotnet.md) に Azure AD B2C の基本について説明します。


> [AZURE.NOTE]  このサンプルは、とへの接続を書き込んだ [iOS B2C サンプル アプリケーション](active-directory-b2c-devquickstarts-ios.md)。先にこのチュートリアルを行った後、そのサンプルに従ってください。
...
このチュートリアルのコードは保持 [github](https://github.com/AzureADQuickStarts/B2C-WebAPI-nodejs)します。 理解するには [.zip としてアプリのスケルトンをダウンロードする](https://github.com/AzureADQuickStarts/B2C-WebAPI-nodejs/archive/skeleton.zip) またはスケルトンを複製。

`git クローン--ブランチ スケルトン https://github.com/AzureADQuickStarts/B2C-WebAPI-nodejs.git`
...
## 7: Passport.js を Web API にインストールする
...
## 8: Passport-Azure-AD を Web API に追加する
...
``
passport-azure-ad@1.0.0 node_modules/passport-azure の ad
├── xtend@4.0.0
├── xmldom@0.1.19
...
## 9: MongoDB モジュールを Web API に追加する
...
## 12: Azure AD の設定を保存する構成ファイルを作成する
...
*audience*: サービスを識別するポータルの URI。 このサンプルを使用して: `http://localhost/TodoListService`
...
手順 11. で作成した config.js ファイルから再度呼び出しを行うと、mongoose_auth_local 接続 URL の末尾に配置されているため、データベース *tasklist* が呼び出されます。 このデータベースを MongoDB で事前に作成する必要はありません。存在しない場合、サーバー アプリケーションの初回実行時に作成されます。
...
`$ curl - isS http://127.0.0.1:8080 と指定 |json`
...
`$ curl - isS-x POST http://127.0.0.1:8080 と指定/タスク/brandon/こんにちは`
...
`$ curl - isS http://127.0.0.1:8080 と指定/タスク/brandon/`
...
## 20: サーバー アプリケーションを再度実行して自分が拒否されることを確認する
...
リファレンスについては、完全なサンプル (構成値) を除く [.zip をここでは現状](https://github.com/AzureADQuickStarts/B2C-WebAPI-nodejs/archive/complete.zip), 、または GitHub から複製することができます。

`git クローン--ブランチの完全な https://github.com/AzureADQuickStarts/B2C-WebAPI-nodejs.git`
...
[Web API への接続 B2C で iOS を使用して >>](active-directory-b2c-devquickstarts-ios.md)
