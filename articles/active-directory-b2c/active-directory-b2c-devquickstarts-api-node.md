...
> [AZURE.NOTE] この記事は、サインイン、サインアップを実装する方法と Azure AD B2C とプロファイルの管理には適用されません。  ユーザーが既に認証された後での Web API の呼び出しに焦点を合わせています。  まだインストールしていない場合、これを最初にください、 [.NET Web アプリケーション チュートリアル入門](active-directory-b2c-devquickstarts-web-dotnet.md) Azure AD B2C の基本について説明します。


> [AZURE.NOTE]  このサンプルとに接続されているに書き込まれた、 [iOS B2C サンプル アプリケーションです。](active-directory-b2c-devquickstarts-ios.md) 先にこのチュートリアルを行った後、そのサンプルに従ってください。
...
*対象ユーザー*: サービスを識別するポータルからの URI。 このサンプルでは `http://localhost/TodoListService` を使用します。

*tenantName*: テナントの名前 (contoso.onmicrosoft.com など)

*policyName*: サーバー送信されたトークンを検証するポリシーです。 クライアント アプリケーションでサインインするために使用したのと同じポリシーを使用する必要があります。

> [AZURE.NOTE] B2C プレビューは、クライアントとサーバーの両方の設定にわたって、同じポリシーを使用します。 既にチュートリアルを行っていて、これらのポリシーを作成してある場合は、再度作成する必要はありません。 このチュートリアルを完了しているため、このサイトで任意のクライアントのチュートリアルを行うときに新しいポリシーを設定する必要はありません。
