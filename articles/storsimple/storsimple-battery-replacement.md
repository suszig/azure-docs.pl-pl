...
2. バッテリが故障している PCM を特定します。 図 1 は、StorSimple デバイスの背面を示します。
...
- システムで最近停電が発生したか、またはバッテリのメンテナンスが定期的に行われている可能性があります。 続行する前に、12 時間はシステムを観察します。
    ...
    - コントローラーと PCM を実行したまま AC 電源に 12 時間連続して接続した後、状態がまだ **[低下]** の場合は、バッテリを交換する必要があります。 ください [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) 交換用バックアップ バッテリ モジュールについてです。

    - 12 時間後に状態が [OK] になった場合は、バッテリは動作しており、保守充電のみが必要です。

- 関連する AC 電源の停電がなく、PCM をオンにして AC 電源に接続されている場合は、バッテリを交換する必要があります。 [Microsoft サポートに問い合わせる](storsimple-contact-microsoft-support.md) して交換用バックアップ バッテリ モジュールを注文します。
