...
4. ボリュームの追加ウィザードの **[基本設定]** で、次の操作を行います。
  ...
  3. **[使用法の種類]** ボックスの一覧で、ボリュームの使用法の種類を選択します。 アーカイブ データにこのボリュームを使用する場合は、**[アクセス頻度の低いアーカイブ データにこのボリュームを使用します]** チェック ボックスをオンにします。 その他のユースケースの場合は、**[階層化ボリューム]** を選択します。 (「階層化されたボリューム」は、以前は「プライマリ ボリューム」と呼ばれていました)。
...
5. **[追加設定]** で、新しいアクセス制御レコード (ACR) を追加します。
  ...
  2. **[このボリュームの既定のバックアップの有効化]** チェックボックスをオンにして、既定のバックアップを有効にすることをお勧めします。
   3. チェック アイコンをクリックして ![チェック マーク アイコン](./media/storsimple-manage-volumes/HCS_CheckIcon.png) 指定した設定でボリュームを作成します。
...
6. チェック マーク アイコンをクリックして変更を保存 ![チェック マーク アイコン](./media/storsimple-manage-volumes/HCS_CheckIcon.png)します。 ボリュームを更新中であることを示すメッセージが Azure ポータルに表示されます。 ボリュームが正常に更新されると、成功メッセージが表示されます。
