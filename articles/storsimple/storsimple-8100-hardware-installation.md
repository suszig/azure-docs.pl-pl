...
![警告アイコン](./media/storsimple-safety/IC740879.png)![ヘビー ウェイト アイコン](./media/storsimple-8100-hardware-installation/HCS_HeavyWeight_Icon.png) **警告!**
...
1. 箱と梱包材を調べて、つぶれ、切断、水害などの明らかな損傷がないか確認します。 箱または梱包材がひどく破損している場合は、箱を開けないでください。 ください [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) デバイスが正常な状態であるかどうかを評価するためです。 
...
3. 開梱後、次の内容物を確認してください。
   ...
   - 使用の開始に関するドキュメント
    ...
    場合は、上記のアイテムのいずれかで足りないものが [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md)します。 
...
2. 内部レールがデバイスに取り付けられていない場合にのみ、この手順を実行します。 通常、内部レールは出荷時に取り付けられています。 レールが取り付けられていない場合は、エンクロージャ シャーシの両側に左右のレール スライドを取り付けます。 これらは 6 つのメートルねじを使用して両側に取り付けます。 印刷の向きを支援するため、レール スライドをマーク **LH-Front** と **RH – Front**, 、端は細くなっており、エンクロージャの背面方向わかりします。<br/>
...
3. ラック キャビネットの垂直材に左右の外部レール アセンブリを取り付けます。 角かっこがマークされている **LH**, 、**RH**, 、および **この面を上** に正しい方向を指示します。 
...
2. 慎重にデバイスをレールに挿入し、デバイスに完全に押し込みますラック キャビネットします。<br/>
...
4. 位置に取り付けますインプレースのスナップ フランジ キャップをインストールします。<br/>
...
詳細についてを参照してください、 **ネットワーク インターフェイス** 下にある、 [、StorSimple デバイスの高可用性要件](storsimple-system-requirements/#high-availability-requirements-for-storsimple.md)します。

>[AZURE.NOTE] SFP + トランシーバーを 10 GbE ネットワーク インターフェイスに使用すると場合、は、指定された QSFP を使用して-SFP + アダプター。 詳細について [StorSimple デバイス上の 10 GbE ネットワーク インターフェイスのサポートされているハードウェア](storsimple-supported-hardware-for-10-gbe-network-interfaces.md)
...
1. デバイスの各コントローラーにはシリアル ポートがあり、レンチのアイコンで識別されます。 図を参照してください、 [ネットワークのケーブル配線](#network-cabling) セクションで、デバイスのバック プレーンにあるシリアル ポートを検索します。 
...
>[AZURE.NOTE] **ライトアウト管理**: デバイスにインストールしているリモート データ センターまたはアクセスが制限されたコンピューター ルームには、両方のコント ローラーへのシリアル接続がシリアル コンソール スイッチまたは同様の機器に常に接続されていることを確認します。 これにより、帯域外リモート制御が可能になり、ネットワーク切断や予期しない障害が発生した場合に操作がサポートされます。

これでデバイスに電源、ネットワーク アクセス、およびシリアル接続用のケーブルが接続されました。 次のステップでは、ソフトウェアを構成してデバイスをデプロイします。

## 次のステップ

学習方法 [展開して、オンプレミスの StorSimple デバイスを構成します。](storsimple-deployment-walkthrough.md) 
