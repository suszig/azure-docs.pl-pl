...
- **保有期間** – 保持するバックアップ コピーの数。 オンにした場合、 **すべて** ボックスで、すべてのバックアップ コピーを保持する 1 つのボリュームのバックアップ コピーの最大数に到達するまで、この時点で、ポリシーは失敗し、エラー メッセージが生成されます。 保持するバックアップの数 (1 ～ 64) を指定することもできます。
