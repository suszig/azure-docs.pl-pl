...
Microsoft Azure StorSimple デバイスを通常のシステム運用の一環としてシャットダウンする必要はありません。 ただし、新しいデバイスまたはシャット ダウンする必要があったデバイスをオンにする必要があります。 一般にシャットダウンは、故障したハードウェアを交換するときや、物理的に装置を移動するとき、デバイスを廃棄するときに必要となります。 このチュートリアルでは、さまざまな状況で StorSimple デバイスの電源をオンにしたりシャットダウンしたりするために必要な手順を説明します。
...
>[AZURE.NOTE]デバイス セットアップを完了および指示のケーブル配線 [StorSimple 8100 デバイスの取り付け](storsimple-8100-hardware-installation.md)します。 説明されている手順に厳密に従ってください。
...
>[AZURE.NOTE]デバイス セットアップを完了および指示のケーブル配線 [StorSimple 8600 デバイスの取り付け](storsimple-8600-hardware-installation.md)します。 説明されている手順に厳密に従ってください。
...
3. 次の点をチェックして、デバイスの電源が完全に入っていることを確認します。
    ...
    これらの条件のいずれか 1 つでも満たされていない場合、デバイス は正常ではありません。 ください [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md)します。
...
1. EBOD エンクロージャが主エンクロージャに接続されていることを確認します。 詳細については、次を参照してください。 [StorSimple 8600 デバイスの取り付け](storsimple-8600-hardware-installation.md)します。
...
>[AZURE.IMPORTANT] SAS ケーブルに欠陥が、または EBOD エンクロージャと主エンクロージャの間の接続は適していませんが、システムを有効にすると、それが回復モードに変わります。 ください [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) このような場合です。
...
>[AZURE.IMPORTANT] によってシャット ダウンされない実行中のデバイス、デバイスの背面に電源ボタンを使用します。
>
>デバイスをシャットダウンする前に、すべてのデバイス コンポーネントが正常であることを確認してください。 Azure クラシック ポータルに移動 **デバイス** > **メンテナンス** > **ハードウェアの状態**, 、すべてのコンポーネントのステータスが緑であることを確認します。 この表示になるのは、システムが正常な状態にある場合だけです。 正常に機能しないコンポーネントを交換して、システムをシャット ダウンする場合が障害 (赤) または機能低下 (黄) の状態で、該当するコンポーネントの **ハードウェアの状態**します。
...
2. 表示されたメニューであることを確認、コント ローラーに接続している、 **スタンバイ** コント ローラーです。 スタンバイ コントローラーでない場合は、接続を解除し、別のコントローラーに接続してください。

3. シリアル コンソール メニューで選択して **オプション 1** フル アクセスを持つスタンバイ コント ローラーにログオンします。
...
4. プロンプトで、次のように入力します。 
    ...
    >[AZURE.IMPORTANT] 次の手順に進む前に完全に停止するコント ローラーを待ちます。
...
>[AZURE.IMPORTANT] 主エンクロージャと EBOD エンクロージャをシャット ダウンする前に、すべてのデバイス コンポーネントが正常であることを確認します。 Azure クラシック ポータルに移動 **デバイス** > **メンテナンス** > **ハードウェアの状態**, 、すべてのコンポーネントが正常であることを確認します。

#### EBOD エンクロージャを備えた動作中のデバイスをシャットダウンするには

1. 示されているすべての手順に従います [主エンクロージャのみを持つデバイス](#8100a)します。
...
>[AZURE.NOTE] 主エンクロージャに EBOD エンクロージャを接続するために使用している SAS ケーブルは、システムがシャット ダウン後まで削除されません。
