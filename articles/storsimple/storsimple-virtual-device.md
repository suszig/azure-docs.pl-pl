...
- 仮想デバイスに [Azure の仮想ネットワークの構成](virtual-networks-create-vnet.md)します。
...
3.  **仮想デバイスの作成** ] ダイアログ ボックスで、次の詳細を指定します。
    ...
    3. **サブネット** – 仮想デバイスで使用するための仮想ネットワークにサブネットです。

    4. **仮想デバイスの作成、ストレージ アカウント** – プロビジョニング中に、仮想デバイスのイメージを保持するために使用されるストレージ アカウント。 このストレージ アカウントは、仮想デバイスおよび仮想ネットワークと同じリージョンに存在する必要があります。 物理デバイスまたは仮想デバイスのデータ保管にこのストレージ アカウントを使用することは避けてください。 既定では、この目的に使用する新しいストレージ アカウントが作成されます。 ただし、この用途に適したストレージ アカウントが既にあることがわかっている場合は、一覧からそのアカウントを選択してください。

4. 仮想デバイスに格納するデータがマイクロソフトのデータセンターでホストされることに同意する旨のチェック ボックスをオンにします。 これで仮想デバイスが作成されます。 仮想デバイスを作成する 1 時間には、最大 45 分をかかる場合があります。
    ![StorSimple 仮想デバイスの作成ステージ](./media/storsimple-virtual-device/StorSimple_VirtualDeviceCreating1M.png)

物理デバイスのみを使用する場合は、暗号化キーがご利用のデバイスに保管されるため、マイクロソフトが暗号化を解除することはできません。 仮想デバイスを使用する場合、暗号化キーと復号化キーの両方が Microsoft Azure に保管されます。 詳細については、次を参照してください。 [仮想デバイスを使用してセキュリティの考慮事項](#security-considerations-for-using-a-virtual-device)します。

### 仮想デバイスの構成と登録

この手順を開始する前に、サービス データ暗号化キーのコピーがあることを確認してください。 サービス データ暗号化キーは、最初の StorSimple 物理デバイスの構成時に作成され、安全な場所に保存するように指示されます。 必要な場合は、サービス データ暗号化キーのコピーがない、 [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) サポートが必要です。

StorSimple 仮想デバイスを構成して登録するには、次の手順を実行します。


1. 選択、 **StorSimple 仮想デバイス** 上で作成した、 **デバイス** ページです。

2. クリックして **デバイス構成を完了**します。 デバイスの構成ウィザードが起動します。
...
1. デバイスの構成ウィザードで、次のように実行します。
    ...
    1. 入力、 **サービス データ暗号化キー** 所定の場所にします。
    2. 入力、 **Snapshot Manager のパスワード**します。  **Snapshot Manager のパスワード** 14 または 15 文字の長さにする必要があり、小文字、大文字、数字、および特殊文字の組み合わせを含める必要があります。
    3. 入力、 **デバイス管理者のパスワード**します。  **デバイス管理者のパスワード** 8 ~ 15 文字の範囲である必要があり、小文字、大文字、数字、および特殊文字の組み合わせを含める必要があります。
...
- 下へスクロールして、 **Snapshot Manager** セクションです。 14 または 15 文字のパスワードを入力します。 大文字、小文字、数字、および特殊文字を組み合わせたパスワードを使用してください。

- パスワードを確認入力します。

- クリックして **保存** ページの下部にあります。
...
1. 下へスクロールして、 **デバイス管理者パスワード** セクションです。 管理者のパスワードは、8 ～ 15 文字で指定してください。 大文字、小文字、数字、および特殊文字を組み合わせたパスワードを使用する必要があります。

1. パスワードを確認入力します。

1. クリックして **保存** ページの下部にあります。
...
![使用可能なビデオ](./media/storsimple-virtual-device/Video_icon.png) **ビデオ**

クラウドで仮想の StorSimple デバイスを作成する方法を説明するビデオを見る] をクリックして [ここ](http://azure.microsoft.com/documentation/videos/create-a-storsimple-virtual-device/)します。
...
- **ソフトウェアの更新** – ソフトウェアが最後に更新された日付を表示とステータス メッセージを更新することができます。 新しい更新プログラムがないかどうかを確認する場合は、ページの下部にある [更新プログラムのスキャン] ボタンを使って、手動スキャンを実行できます。
...
- 開いている、 **デバイス** ] ページで、クリックして、 **ボリューム コンテナー** ] タブをクリックします。

- 仮想デバイスにフェールオーバーするボリューム コンテナーを選択します。 ボリューム コンテナーをクリックし、そのコンテナー内のボリュームを一覧表示します。 ボリュームを選択し、クリックして **オフライン** ボリュームをオフラインにします。 ボリューム コンテナーのすべてのボリュームでこのプロセスを繰り返します。

- 仮想デバイスにフェールオーバーするすべてのボリューム コンテナーに前の手順を繰り返します。

-  **デバイス** フェールオーバーし、をクリックする必要があるデバイスを選択] ページで、 **フェールオーバー** デバイスのフェールオーバー ウィザードを開きます。

-  **フェールオーバーするボリューム コンテナーを選択してください**, 、フェールオーバーするボリューム コンテナーを選択します。 ボリューム コンテナーがこの一覧に表示されるには、ボリューム コンテナーにクラウド スナップショットが含まれていて、オフラインである必要があります。 表示されるはずのボリューム コンテナーが表示されない場合、ウィザードをキャンセルし、ボリューム コンテナーがオフラインになっていることを確認します。

- 次のページで **のボリュームのターゲット デバイスを選択して** 選択したコンテナーには、利用できるデバイスのドロップダウン リストから仮想デバイスを選択します。 利用できる容量があるデバイスのみが一覧に表示されます。

- すべてのフェールオーバー設定を確認、 **フェールオーバーの確認** ページです。 すべての設定が正しければ、チェック マーク アイコンをクリックします。

フェールオーバー プロセスが開始されます。 フェールオーバーが完了したらに移動して、 **デバイス** ページし、フェールオーバー プロセスのターゲットとして使用された仮想デバイスを選択します。 [ボリューム コンテナー] ページに移動します。 すべてのボリューム コンテナーと古いデバイスのボリュームが表示されます。
...
仮想デバイスを管理するためのワークフローの詳細な一覧を参照してください [管理の StorSimple デバイスが StorSimple Manager サービスを使用して](storsimple-manager-service-administration.md#administer-storsimple-device-using-storsimple-manager-service)します。
