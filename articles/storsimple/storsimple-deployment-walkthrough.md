...
|---------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **IP**</br>**Subnet**</br>**Gateway**| これを有効な IPv4 または IPv6 アドレスですか。</br>は有効なサブネット?</br>これを有効なゲートウェイですか。</br>は、ネットワーク上の IP が重複してでしょうか。| `ip に ping を実行`</br>`arp-a`</br>、 `ping` と `arp` コマンドでは、この IP を使用して、データ センター ネットワークでデバイスがないことを示す失敗する必要があります。
| | | |
| **DNS**| 有効な DNS で、Azure の URL を解決できるかどうか。| `Resolve-dnsname-名前 www.bing.com-サーバー < DNS サーバーの IP アドレス >` </br>使用できる代替のコマンドは、:</br>`nslookup - dns ip = < DNS サーバーの IP アドレス > www.bing.com`|
| | ポート 53 が開いているかどうかを確認します。これは、デバイスに外部 DNS を使用している場合にのみ適用できます。内部 DNS の場合、外部 URL は自動的に解決されます。| `テスト ポート – comp dc1 の udp ポート 53 UDPtimeout 10000`  </br>[の詳細については、このコマンドレットでは](http://learn-powershell.net/2011/02/21/querying-udp-ports-with-powershell/)|
| | | |
| **NTP**| NTP サーバーを入力するとすぐに時刻の同期が開始されます。入力するときに、確認の UDP ポート 123 が開いている `time.windows.com` またはサーバーのパブリック時間) です。| [をダウンロードしてこのスクリプトを使用して](https://gallery.technet.microsoft.com/scriptcenter/Get-Network-NTP-Time-with-07b216ca)します。|
| | | |
| **プロキシ (省略可能)**| 有効なプロキシ URI とポートかどうか。</br> 認証モードは正しいですか。| <code>wget http://bing.com & #124 文字です。% {$_.StatusCode}</code></br>web プロキシを構成した直後にこのコマンドを実行する必要があります。状態コード 200 が返された場合は、接続が成功したことを示します。|
...
| | | |
| **登録**| 送信 TCP ポート 443、80、9354 が開いているかどうかを確認します。| `テスト NetConnection のポート 443 InformationLevel Detailed`</br>[テスト NetConnection コマンドレットの詳細については](https://technet.microsoft.com/library/dn372891.aspx)|
