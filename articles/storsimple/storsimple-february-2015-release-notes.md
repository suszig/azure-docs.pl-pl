...
>[AZURE.IMPORTANT]   
>
> -年 2 月の更新プログラムのインストール、StorSimple 用 Windows PowerShell ではなく、StorSimple Manager サービスを使用します。   
> -は、この更新プログラムをインストールする、1 時間程度です。 ただし、累積的更新プログラムをインストールする場合は、プロセスの完了に約 3 時間を要します。  
> -StorSimple の 2 月のリリースでは、StorSimple 仮想デバイスに加えた変更は含まれません。 最新のセキュリティ修正プログラムを含む、仮想デバイスの使用可能な Windows 更新プログラムを適用できますが、仮想デバイスのバージョン変更は表示されません。  
