---
title: Wymagania systemowe StorSimple | Dokumentacja firmy Microsoft
...
| TCP 9354 |limit |WAN |Tak |Wychodzący port jest używany przez urządzenia StorSimple do komunikowania się z usługą Menedżer StorSimple. |
...
Algorytmy metryki routingu są różne w zależności od wersji oprogramowania uruchomione na urządzeniu StorSimple.

**Wersje przed Update 1**

Dotyczy to również wersje oprogramowania przed Update 1, takich jak GA, 0.1, 0,2 lub 0,3 wersji. Kolejność, w oparciu metryki routingu jest następujący:
...
**Wersje uruchamianie z Update 1 i Update 2**

W tym wersje oprogramowania, takich jak 1, 1.1 lub 1.2. Kolejność, w oparciu metryki routingu decyzji w następujący sposób:
   ...
   *DANE 0 > ostatnio skonfigurowany interfejs sieciowy Ethernet 10 > Interfejs sieciowy inne 10 GbE > ostatnio skonfigurowany interfejs sieciowy Ethernet 1 > inne 1 GbE interfejsu sieciowego*
   ...
   W pakiecie Update 1, staje się metryki routingu danych 0 najniższy; w związku z tym wszystkie chmury — ruch jest kierowany przez dane 0. Zwróć uwagę to, jeśli istnieje więcej niż jeden interfejs sieciowy z obsługą chmury na urządzeniu StorSimple.

**Wersje, począwszy od aktualizacji 2**

Aktualizacja 2 ma kilka ulepszeń dotyczących sieci i routingu metryki została zmieniona. To zachowanie można wyjaśnić w następujący sposób.
...
* Kolejność, w której będą kierowane ruchu w chmurze za pośrednictwem interfejsów sieciowych jest:
    ...
    *gdzie liczby w nawiasach wskazują odpowiednich metryk routingu.*
...
* Alert jest zgłaszany również na urządzeniu StorSimple, po awarii adresu VIP. Aby uzyskać więcej informacji, przejdź do [alertów krótkimi opisami](storsimple-manage-alerts.md).
...
* Izolowanie ruchu iSCSI i chmury przez używania dedykowanych interfejsów sieciowych na twoim urządzeniu dostęp iSCSI i chmury. Aby uzyskać więcej informacji, zobacz temat jak [zmodyfikować interfejsów sieciowych](storsimple-modify-device-config.md#modify-network-interfaces) na urządzeniu StorSimple.
...
* Jeśli moduł kontrolera awarią lub koniecznością wymiany, upewnij się, że moduł kontroler jest w stanie aktywnym przed wymianą moduł kontrolera nie powiodło się. Aby sprawdzić, czy kontroler jest aktywny, przejdź do [Zidentyfikuj active kontroler na urządzeniu](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device).
...
* Podczas wymiany, monitorować **stan sprzętu** w **konserwacji** strony dla dysków SSD i dysków twardych. Stan zielony wyboru wskazuje, że dyski są w dobrej kondycji lub OK punktu czerwony wykrzyknik wskazuje, nie powiodło się dysków SSD i HDD.
...
* Jeśli moduł EBOD obudowa kontrolera nie powiedzie się, upewnij się, moduł kontrolera jest aktywna, zanim zastąpienie modułu nie powiodło się. Aby sprawdzić, czy kontroler jest aktywny, przejdź do [Zidentyfikuj active kontroler na urządzeniu](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device).
* Podczas wymiany modułu EBOD kontroler na stałe monitorowanie stanu składnika w usłudze Menedżer StorSimple uzyskując dostęp do **konserwacji** > **stan sprzętu**.
...
* [Dowiedz się, jak wdrażanie rozwiązania StorSimple](storsimple-deployment-walkthrough-u2.md).
