...
>[AZURE.NOTE] * * メンテナンス モードは正しく機能しているデバイスでのみサポートされます。 デバイスでコント ローラーの一方または両方がにない functioning.* * ではサポートされていません
</br>
...
>[AZURE.NOTE] * * 回復モードでデバイスを配置できません。 回復モードが、Microsoft サポート担当者が調べることができます。 * * 状態にデバイスを取得しようとした、デバイスが正常な状態である場合は、
...
> [AZURE.IMPORTANT] メンテナンス モードに入る前に、Azure クラシック ポータルの **[メンテナンス]** ページの **[ハードウェアの状態]** にアクセスして、両方のデバイス コントローラーが正常な状態であることを確かめます。 コントローラーが正常な状態でない場合は次の手順を Microsoft サポートにお問い合わせください。 詳細についてを参照してください [Microsoft サポートに問い合わせる](storsimple-contact-microsoft-support.md)します。

#### メンテナンス モードを開始するには

1. 次の手順に従って、デバイスのシリアル コンソールにログオン [デバイスのシリアル コンソールに接続するための PuTTY を使用して](storsimple-deployment-walkthrough.md#use-putty-to-connect-to-the-device-serial-console)します。

2. シリアル コンソール メニューで、オプション 1 を選択し、**フル アクセスでログイン**します。 画面の指示に従って、**デバイス管理者のパスワード**を入力します。 既定のパスワードは: `Password1`します。
