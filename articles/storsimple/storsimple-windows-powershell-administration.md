...
- [StorSimple デバイスのシリアル コンソールへの接続します。](#connect-to-windows-powershell-for-storsimple-via-the-device-serial-console)
- [Windows PowerShell を使用して StorSimple にリモート接続します。](#connect-remotely-to-storsimple-using-windows-powershell-for-storsimple)
...
>[AZURE.IMPORTANT] バナー メッセージを使用して、接続中のコントローラーの状態 (アクティブまたはパッシブ) を識別できます。
...
1. [PuTTY Reconfiguration] ダイアログ ボックスで、**[Category]** パネルの **[Keyboard]** を選択します。
...
1. **[Category]** パネルで、**[Translation]** を選択します。

1. **[Remote character set]** ボックスの一覧から **[UTF-8]** を選択します。

1. **[Handling of line drawing characters]** の **[Use Unicode line drawing code points]** を選択します。 次の図に、正しい PuTTY の選択を示します。
...
- [Http を使用してリモートで接続します。](storsimple-remote-connect.md#connect-through-http)
- [Https を使用してリモートで接続します。](storsimple-remote-connect.md#connect-through-https)
...
| 目的の操作| 実行する手順|
|---|---|
| デバイスを登録する| [構成し、StorSimple 用 Windows PowerShell を使用してデバイスの登録](storsimple-deployment-walkthrough.md#step-3-configure-and-register-the-device-through-windows-powershell-for-storsimple)|
| Web プロキシの構成</br>web プロキシ設定を表示します。| [StorSimple デバイスの web プロキシを構成します。](storsimple-configure-web-proxy.md)|
| デバイスの DATA 0 ネットワーク インターフェイス設定を変更する| [StorSimple デバイスの DATA 0 ネットワーク インターフェイスを変更します。](storsimple-modify-data-0.md)|
| コント ローラーの停止 </br> 再起動またはシャット ダウンのコント ローラー </br> デバイスのシャット ダウン</br>デバイスを出荷時設定にリセットします。| [デバイス コント ローラーを管理します。](storsimple-manage-device-controller.md)|
| メンテナンス モードで更新プログラムと修正プログラムをインストールする| [デバイスを更新します。](storsimple-update-device.md)|
| メンテナンス モードに入る </br>メンテナンス モードを終了します。| [StorSimple デバイスのモード](storsimple-device-modes.md)|
| サポート パッケージの作成</br>暗号化を解除してサポート パッケージの編集| [作成し、サポート パッケージの管理](storsimple-create-manage-support-package.md)|
| サポート セッションを開始します。</br>| [StorSimple 用 Windows PowerShell でサポート セッションを開始します。](/storsimple-contact-microsoft-support.md#start-a-support-session-in-windows-powershell-for-storsimple)
...
- 任意のコマンドレッドまたは関数のヘルプを取得するには、次のコマンドを使用します `Get-help < コマンドレット名 >`。

- 任意のコマンドレットのオンライン ヘルプを取得するには、前のコマンドレットでを使用して、 `-オンライン` パラメーター: `Get-help < コマンドレット名 > - オンライン`

- 完全なヘルプを使用することができます、– Full パラメーターと、および例についてを使用して、 `– 例` パラメーター。
...
1. ヘルプ ファイルがインストールされた後の入力: `Get-help Get-command`します。 これにより、ヘルプを利用できるコマンドレットの一覧が表示されます。
