...
セットアップ ウィザードを使用する ([Invoke-hcssetupwizard][1]) をデバイスの構成を確認し、必要に応じて是正措置を実行します。
...
|:--- |:-------------------------- |:--------------- |:------------------ |
|  1  | エラー 350032: このデバイスは既に非アクティブ化されています。 | 非アクティブ化されたデバイスでセットアップ ウィザードを実行すると、このエラーが表示されます。 | [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) に対処します。 非アクティブ化されているデバイスを利用することはできません。 デバイスを再アクティブ化する前に、出荷時の設定に戻す必要があります。 |
...
|  3  | StorSimple Manager サービスのポータルではデバイスがオンラインであると表示されますが、最小のセットアップを完了して構成を保存しようとすると、操作に失敗します。 | 実際のプロキシ サーバーがあるにもかかわらず、初期セットアップ時に、Web プロキシが構成されませんでした。 | 使用して、 [Test-hcsmconnection コマンドレット][2] 、エラーを特定します。 [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) 、問題を解決できない場合。 |
|  4  | Invoke-HcsSetupWizard: 値が期待される範囲内にありません。 | 不適切なサブネット マスクがこのエラーの原因です。 考えられる原因は次のとおりです。 <ul><li> サブネット マスクが見つからないか空です。</li><li>Ipv6 プレフィックスの形式が正しくありません。</li><li>インターフェイスはクラウドに対応しているが、ゲートウェイが見つからないか、正しくないです。</li></ul>DATA 0 は、自動的にクラウド対応のセットアップ ウィザードで構成されている場合は注意してください。 | 問題を特定するには、サブネット 0.0.0.0 または 256.256.256.256 を使用して出力を確認します。 必要に応じて、サブネット マスク、ゲートウェイ、および IPv6 プレフィックスの正しい値を入力します。 |
...
|40018|バックアップ セット内のディスクのいずれも解決できません。|引き続きエラーが発生する場合は、Microsoft サポートに対処法をお問い合わせください。|
...
問題が解決されない場合 [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) サポートが必要です。 
