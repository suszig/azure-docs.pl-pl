...
StorSimple Update 2 では、適切なボリュームを*ローカル固定*として識別し、プライマリ データをデバイスのローカルに保持されたままにして、クラウドに階層化しないようにすることができます。 これにより、ローカル固定ボリューム上で、SQL および仮想マシンのワークロードなど、クラウドの待機時間に影響を受けやすいワークロードを実行しながら、引き続きクラウドをバックアップに使用できます。 ローカルで固定されたボリュームに関する詳細については、次を参照してください。 [StorSimple Manager サービスを使用してボリュームを管理する](storsimple-manage-volumes-u2.md)します。

また、Update 2 では、Azure Premium Storage で提供される短い待機時間と高いパフォーマンスを活用した StorSimple 仮想デバイスを作成することもできます。 Premium の StorSimple 仮想デバイスの詳細については、次を参照してください。 [展開 Azure StorSimple 仮想デバイスを管理および](storsimple-virtual-device-u1.md)します。 Azure premium storage についての詳細についてを参照してください [Premium Storage: Azure 仮想マシン ワークロード向けの高パフォーマンス ストレージ](storage-premium-storage-preview-portal.md)します。
...
![ビデオ アイコン](./media/storsimple-overview/video_icon.png) Microsoft Azure StorSimple に簡単な概要についてのビデオを見る。
...
| 他のクラウド サービス プロバイダーのサポート| ソフトウェア Update 1 以降の StorSimple 8000 シリーズでは、Amazon S3 with RRS、HP、OpenStack クラウド サービス、Microsoft Azure をサポートします(デバイスの管理が目的の場合は Microsoft Azure ストレージ アカウントが必要になります)。詳細についてを参照してください [更新 1.2 の新機能](storsimple-update1-release-notes.md#whats-new-in-update-12)します。|
...
| クリティカル アプリケーションのサポート| StorSimple Update 2 では、適切なボリュームをローカル固定として識別できます。この機能により、クリティカル アプリケーションで必要なデータをクラウドに階層化しないようにすることができます。ローカル固定ボリュームは、クラウドの待機時間や接続の問題によって影響を受けることはありません。ローカルで固定されたボリュームに関する詳細については、次を参照してください。 [StorSimple Manager サービスを使用してボリュームを管理する](storsimple-manage-volumes-u2.md)します。|
| 短い待機時間と高いパフォーマンス| StorSimple Update 2 では、Azure Premium Storage の高いパフォーマンスと短い待機時間の特徴を利用する仮想デバイスを作成することができます。Premium の StorSimple 仮想デバイスの詳細については、次を参照してください。 [展開 Azure StorSimple 仮想デバイスを管理および](storsimple-virtual-device-u2.md)します。|

![ビデオ アイコン](./media/storsimple-overview/video_icon.png) ウォッチ [このビデオ](https://www.youtube.com/watch?v=4MhJT5xrvQw&feature=youtu.be) StorSimple 8000 シリーズの機能と利点の概要についてです。
...
- **StorSimple 用 Azure PowerShell コマンドレット** – コマンドラインからのサービス レベル タスクと移行タスクを自動化することができる Windows PowerShell コマンドレットのコレクションです。 StorSimple 用 Azure PowerShell コマンドレットの詳細についてを参照してください、 [コマンドレット リファレンス](https://msdn.microsoft.com/library/dn920427.aspx)します。
...
StorSimple 仮想デバイスの詳細についてを参照してください [展開 Azure StorSimple 仮想デバイスを管理および](storsimple-virtual-device-u1)します。
...
詳細についてを参照してください [StorSimple Manager サービスを使用して、StorSimple デバイスを管理する](storsimple-manager-service-administration.md)します。
...
詳細については、を参照してください [StorSimple Snapshot Manager は何ですか?](storsimple-what-is-snapshot-manager.md)
