...
- **機密性** – 許可されているエンティティがデータを表示できるのみです。 
- **整合性** – 承認されたエンティティは変更したり、データを削除します。

Microsoft Azure StorSimple ソリューションは、互いに連携し合う 4 つのメイン コンポーネントで構成されています。

- **Microsoft Azure でホストされる StorSimple Manager サービス** – 管理サービスの構成し、StorSimple デバイスをプロビジョニングするために使用します。
- **StorSimple デバイス** – データ センターに設置される物理デバイス。 データを生成するすべてのホストとクライアントは StorSimple デバイスに接続します。データは StorSimple デバイスが管理し、適宜 Azure クラウドに移動します。
- **デバイスに接続されたクライアント/ホスト** – StorSimple デバイスへの接続を保護する必要があるデータが生成されるインフラストラクチャ内のクライアントです。
- **クラウドの記憶域** – データが格納されている Azure クラウド内の場所。
...
StorSimple Manager サービスにアクセスするには、StorSimple を含む Azure サブスクリプションを会社が保有している必要があります。 Azure クラシック ポータルから利用できる機能は、保有するサブスクリプションによって決まります。 組織が Azure サブスクリプションを持っていない場合の詳細については、それらを参照してください [組織として Azure にサインアップ](../sign-up-organization.md)します。 

StorSimple Manager サービスは Azure でホストされるため、Azure のセキュリティ機能によって保護されます。 Microsoft Azure によって提供されるセキュリティ機能の詳細についてを参照してください、 [Microsoft Azure トラスト センター](http://azure.microsoft.com/support/trust-center/security/)します。
...
については、どのサービス登録キーの取得に移動して [手順 2: サービス登録キーの取得](storsimple-deployment-walkthrough.md#step-2-get-the-service-registration-key)します。
...
> [AZURE.NOTE] すべての使用後に、サービス登録キーを再生成することをお勧めします。
...
PowerShell リモート処理機能は、HTTPS または HTTP で利用することができます。 HTTPS でのリモート管理が有効になっている場合、対象デバイスからリモート管理証明書をダウンロードしてリモート クライアントにインストールする必要があります。 PowerShell リモート処理の詳細についてを参照してください [、StorSimple デバイスにリモートで接続する](storsimple-remote-connect)です。
...
CHAP と StorSimple ソリューションを構成する方法の詳細についてを参照してください [StorSimple デバイスの CHAP を構成する](storsimple-configure-chap.md)です。
...
StorSimple Snapshot Manager の詳細についてを参照してください [StorSimple Snapshot Manager は何ですか。](storsimple-what-is-snapshot-manager.md)
...
- 強力なパスワードを使用します。 詳細についてを参照してください [より強力なパスワードを作成し、保護する](http://blogs.microsoft.com/cybertrust/2014/08/25/create-stronger-passwords-and-protect-them/)です。
...
> [AZURE.NOTE] 重複除去では、StorSimple デバイス上や Microsoft Azure storage 内に格納されているデータに追加の保護を提供します。 データ重複除去が有効である場合、データ オブジェクトは、そのマッピングとアクセスに使用されるメタデータとは別に保管されます。ボリューム構造やファイル システム、ファイル名などに基づいてデータを再構築するためのストレージ レベルのコンテキストにアクセスすることはできません。
...
>
> サービス データ暗号化キーを紛失した場合、マイクロソフトのサポート担当者の支援を得ることでキーを読み出すことができます。そのためには、少なくとも 1 つのデバイスがオンライン状態であることが必要です。 サービス データ暗号化キーは、その取得後に変更するようお勧めします。 手順についてを参照してください [サービス データ暗号化キーを変更する](storsimple-service-dashboard.md#change-the-service-data-encryption-key)です。

サービス データ暗号化キーと対応するデータの暗号化証明書を選択して変更することができます、 **サービス データ暗号化キーの変更** サービス ダッシュ ボードでオプションです。 データのセキュリティ侵害を確実に防ぐために、サービス データ暗号化キーは、物理 StorSimple デバイスを使用して変更する必要があります。 暗号化キーを変更する場合、すべてのデバイスを新しいキーで更新する必要があります。 そのため、キーの変更は、すべてのデバイスがオンライン状態のときに行うようお勧めします。 デバイスがオフラインである場合、それらのキーが異なるタイミングで変更される可能性があります。 デバイスのキーの有効期限が切れていても、バックアップは実行できますが、キーが更新されるまでデータは復元できません。 詳細についてを参照してください [StorSimple Manager サービス ダッシュ ボードを使用して](storsimple-service-dashboard.md)します。
...
**Q:** サービスが侵害します。 次に何をすべきですか。

**A:** サービス データ暗号化キーとデータの階層化されているストレージ アカウントのストレージ アカウント キーを直ちに変更する必要があります。 手順については、次をご覧ください。 
...
**Q:** サービス登録キーを依頼する新しい StorSimple デバイスがあります。 どうすれば入手できますか。

**A:** StorSimple Manager サービスを最初に作成したときにこのキーは作成します。 StorSimple Manager サービスを使用してデバイスに接続する場合、サービスのクイック スタート ページを使用してサービス登録キーを表示したり再生成したりできます。 新しいサービス登録キーを生成しても、既存の登録済みデバイスには影響ありません。 手順については、次をご覧ください。
...
**A:** Microsoft サポートに問い合わせてください。 デバイスのサポート セッションにログインし、キーを取得できるようにサポートします (少なくとも 1 台のデバイスがオンラインである必要があります)。 サービス データ暗号化キーを取得したらすぐに変更し、お客様だけが新しいキーを認識するようにする必要があります。 手順については、次をご覧ください。

- [サービス データ暗号化キーの変更](storsimple-service-dashboard.md#change-the-service-data-encryption-key)

**Q:**  サービス データ暗号化キーの変更、デバイスを承認しましたが、キー変更プロセスは開始されませんでした。 どうすればよいですか。

**A:** タイムアウト期間が過ぎている場合は、サービス データ暗号化キーの変更については、デバイスを再度承認、もう一度プロセスを開始する必要があります。

**Q:**  サービス データ暗号化キーを変更しましたが、4 時間以内に他のデバイスを更新できませんでした。 再度開始する必要はありますか。

**A:** 、4 時間という期間が、変更の開始についてのみです。 承認された StorSimple デバイスに対する更新プロセスが開始された後は、すべてのデバイスが更新されるまで承認は有効です。

**Q:** StorSimple の管理者が退職します。 どうすればよいですか。

**A:** 未承認担当者の変更とリセットの StorSimple デバイスへのアクセスを許可して、サービス データ暗号化キーを変更したパスワードはキーが新しい情報が認知されないことを確認します。 手順については、次をご覧ください。
...
**A:** パスワードを忘れた場合は、新しいを作成する必要があります。 その後、既存のすべてのユーザーに対して、パスワードが変更されたので新しいパスワードを使用するようにクライアントを更新する必要があることを通知します。 手順については、次をご覧ください。
...
**Q:** StorSimple 用 Windows PowerShell へのリモート アクセス用の証明書がデバイス上で変更されました。 リモート アクセス クライアントを更新するにはどうすればよいですか。

**A:** StorSimple Manager サービスから新しい証明書をダウンロードして、リモート アクセス クライアントの証明書ストアにインストールすることを指定します。 手順については、次をご覧ください。

- [Import-Certificate コマンドレット](https://technet.microsoft.com/library/hh848630.aspx)

**Q:** データは、StorSimple Manager サービスが侵害された場合の保護ですか?

**A:** サービス構成データは、web ブラウザーで表示すると、常に、公開キーで暗号化します。 サービスには秘密キーへのアクセスがないため、データを表示することはできません。 StorSimple Manager サービスが侵害された場合、StorSimple Manager サービスに格納されたキーが存在しないため、影響はありません、

**Q:** データ暗号化証明書へのアクセスをだれかが成功した場合にデータが侵害されるでしょうか。

**A:** Microsoft Azure は、暗号化された形式で、お客様のデータ暗号化キー (.pfx ファイル) を格納します。 .pfx ファイルは暗号化されており、StorSimple サービスには .pfx ファイルの暗号化を解除するためのサービス データ暗号化キーがないため、.pfx ファイルにアクセスできても、機密データが漏えいすることはありません。

**Q:** 政府機関は、データの Microsoft を要求した場合ですか?

**A:** サービス上のすべてのデータは暗号化されており、秘密キーが、デバイスに保管されて、いるため、政府機関では顧客をデータの要求する必要があります。 
