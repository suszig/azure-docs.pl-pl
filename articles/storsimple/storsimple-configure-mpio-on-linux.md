...
### multipath.conf 構成ファイルについて

構成ファイル `/etc/multipath.conf` マルチパス機能の多くはユーザー構成可能なします。  `マルチパス` コマンドとカーネル デーモン `multipathd` このファイルに記載された情報を使用します。 ファイルが参照されるのは、マルチパス デバイスの構成中のみです。 実行する前に、すべての変更が加えられたことを確認、 `マルチパス` コマンドです。 ファイルを後で変更した場合は、その変更が有効になるように、multipathd を停止してからもう一度開始する必要があります。
...
1. **ブラックリストの例外** *(blacklist_exceptions)*: ブラックリストに載っている場合でもマルチパス デバイスとして扱う特定のデバイスを指定することができます。
...
1. CentOS ホストに、有効な 2 つのネットワーク インターフェイスがあることを確認します。 次のコマンドを入力します。
 ...
 2 つのネットワーク インターフェイスと、次の例は、出力を示します (`eth0` と `eth1`) がホスト上に存在します。
...
1. CentOS サーバーに *iSCSI-initiator-utils* をインストールします。 次の手順を実行して、*iSCSI-initiator-utils* をインストールします。
    ...
    1. としてログオン `ルート` CentOS ホストにします。

    1. *iSCSI-initiator-utils* をインストールします。 次のコマンドを入力します。

        `yum インストール iscsi イニシエーターのバージョン`

    1. *iSCSI-Initiator-utils* が正常にインストールされた後で、iSCSI サービスを開始します。 次のコマンドを入力します。

        `iscsid のサービスの開始`

        `Iscsid` が実際に起動しないと、 `-強制的に` オプションが必要になる可能性

    1. ブート時に、iSCSI イニシエーターが有効になっていることを確認するを使用して、 `chkconfig` コマンドをサービスを有効にします。

        `chkconfig iscsi`

    1. 正しく設定されていることを確認するには、次のコマンドを実行します。

        `chkconfig--リスト |正規表現の iscsi`
...
>[AZURE.IMPORTANT] 
>
>1 GbE とマルチパスの 10 GbE ネットワーク インターフェイスを混合しないことをお勧めします。 2 つのネットワーク インターフェイスを使用しているときは、どちらも同じ種類である必要があります。
>StorSimple デバイスで DATA0、DATA1、DATA4、および DATA5 は 1 GbE インターフェイス DATA2 および DATA3 は 10 GbE ネットワーク インターフェイスです |。
...
1. 初期化 `/etc/multipath.conf` ファイルです。 次のコマンドを入力します。

     `Mpathconf--のコピーを有効にします。`

    上記のコマンドは、作成、 `sample/etc/multipath.conf` ファイルです。

1. マルチパス サービスを開始します。 次のコマンドを入力します。

    `Multipathd のサービス開始をコピーします。`

    次の出力が表示されます。

    `Multipathd デーモンを開始します。`

1. マルチパスの自動検出を有効にします。 次のコマンドを入力します。

    `mpathconf--find_multipaths y`

    既定値] セクションが修正されます、 `multipath.conf` 次のようにします。
...
1. 編集、 `/etc/mulitpath.conf` ファイルです。 次のコマンドを入力します。
...
1. 編集、 `/etc/multipath.conf` ファイルです。 次のコマンドを入力します。

    `vi/etc/multipath.conf`

1. 下にある、 `の既定値` セクションで、設定、 `path_grouping_policy` に `マルチバス`します。  `Path_grouping_policy` できるように、指定されていないマルチパスに適用するポリシーをグループ化の既定のパスを指定します。 既定値のセクションは次のようになります。
...
> [AZURE.NOTE] 
> 最も一般的な値 `path_grouping_policy` が含まれます。
...
1. 再起動、 `multipathd` デーモンです。 次のコマンドを入力します。

    `サービスを再起動する multipathd`
...
1. まず、iSCSI 接続が、StorSimple デバイスで次のように確立されていることを確認します。
    ...
    1. StorSimple デバイスを検出します。 次のコマンドを入力します。
        ...
        `iscsiadm-m 検出-t sendtargets-p < デバイスのネットワーク インターフェイスの IP アドレス >: < iSCSI ポートの StorSimple デバイスで >`
        ...
        StorSimple デバイスの IQN をコピー `iqn.1991 05.com.microsoft:storsimple8100 shx0991003g00dv ターゲット`, 、上記の出力からです。

    1. ターゲット IQN を使用してデバイスに接続します。 ここでは StorSimple デバイスが iSCSI ターゲットです。 次のコマンドを入力します。

        `iscsiadm-m ノード - ログイン-t < iSCSI ターゲットの IQN >`

        次の例では、ターゲット IQN と出力の `iqn.1991 05.com.microsoft:storsimple8100 shx0991003g00dv ターゲット`します。 この出力は、デバイスの 2 つの iSCSI 対応ネットワーク インターフェイスに正常に接続されていることを示しています。
        ...
        ここに表示されているのが 1 つのホスト インターフェイスと 2 つのパスのみの場合は、iSCSI 用にホストの両方のインターフェイスを有効にする必要があります。 行うことができる、 [Linux のドキュメントの手順の詳細な](https://access.redhat.com/documentation/Red_Hat_Enterprise_Linux/5/html/Online_Storage_Reconfiguration_Guide/iscsioffloadmain.html)します。

    1. ボリュームは、StorSimple デバイスから CentOS サーバーに公開されます。 詳細については、次を参照してください。 [手順 6: ボリュームを作成する](storsimple-deployment-walkthrough.md#step-6-create-a-volume) 、StorSimple デバイスで Azure クラシック ポータルを使用します。
...
Q. 変更が表示されない `multipath.conf` ファイルが有効になっています。

A. 変更を加えた場合、 `multipath.conf` ファイル、マルチパス化サービスを再起動する必要があります。 次のコマンドを入力します。
...
A. 2 つのパスが同じサブネットにあり、ルーティング可能であることを確認してください。 ネットワーク インターフェイスが別の vLAN にあり、ルーティングできない場合は、2 つのパスのみが表示されます。 これを確認する方法の 1 つに、StorSimple デバイスのネットワーク インターフェイスから、両方のホスト インターフェイスにアクセスできること確認する方法があります。 必要があります [Microsoft サポートにお問い合わせ](storsimple-contact-microsoft-support.md) ようにこの確認は、サポート セッション経由でのみ実行できます。

Q. 使用可能なパスの一覧を表示しても、出力に何も表示されません。

A. 通常、マルチパス デーモンと問題と思われる任意のマルチパス パスが表示されない、ここでの問題があることはほとんどの場合、 `multipath.conf` ファイルです。

マルチパスの一覧から応答を得られないのは、ディスクがないことを意味している可能性もあるため、ターゲットに接続した後で一部のディスクが実際に表示されることを確認する必要もあります。

- 次のコマンドを使用して SCSI バスを再スキャンします。

    `$ 再スキャン scsi bus.sh `(sg3_utils パッケージの一部)
...
詳細についてを参照してください [マルチパスの対話型コマンドのトラブルシューティングを使用して](http://www.centos.org/docs/5/html/5.1/DM_Multipath/multipath_config_confirm.html)します。

## 便利なコマンドの一覧

| 型| コマンド| 説明|
|---|---|---|
| **iSCSI**| `iscsid のサービスの開始`| iSCSI サービスを開始する|
| | `サービス iscsid の停止`| ISCSI サービスを停止します|
| | `サービスを再起動する iscsid`| ISCSI サービスを再起動します。|
| | `iscsiadm-m 検出-t sendtargets-p < TARGET_IP >`| 指定されたアドレスで利用可能なターゲットを検出します。|
| | `iscsiadm-m ノード - ログイン-t < TARGET_IQN >`| ISCSI ターゲットにログインします。|
| | `iscsiadm-m ノード--ログアウト-p < Target_IP >`| ISCSI ターゲットからログアウトします。|
| | `cat/etc/iscsi/initiatorname.iscsi`| ISCSI イニシエーターの名前を印刷します。|
| | `iscsiadm – m セッション – s < sessionid >-p 3`| ISCSI セッションと、ホスト上で検出されたボリュームの状態を確認します。|
| | `iscsi – m セッション`| ホストと StorSimple デバイスの間で確立されているすべての iSCSI セッションを示しています。|
| | | |
| **マルチパス**| `multipathd のサービスの開始`| マルチパス デーモンを起動します。|
| | `サービス multipathd の停止`| マルチパス デーモンを停止します。|
| | `サービスを再起動する multipathd`| マルチパス デーモンを再起動します。|
| | `に chkconfig multipathd` </br> または </br> `mpathconf – with_chkconfig y`| 起動時に開始するためのマルチパスのデーモンを有効にします。|
| | `multipathd – k`| トラブルシューティングのための対話型コンソールを起動します。|
| | `マルチパス – l`| マルチパス接続のリストを表示およびデバイス|
| | `mpathconf--を有効にします。`| サンプル mulitpath.conf ファイルを作成 `/etc/mulitpath.conf`|
| | | |
...
- [CentOS に MPIO を設定します。](http://www.centos.org/docs/5/html/5.1/DM_Multipath/setup_procedure.html)
