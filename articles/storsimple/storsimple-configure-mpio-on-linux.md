...
>[AZURE.NOTE] このプロシージャは、StorSimple 仮想デバイスで使用できません。 詳細については、仮想デバイスのホスト サーバーを構成する方法を参照してください。
...
- **高可用性**: (ケーブル、スイッチ、ネットワーク インターフェイス コント ローラーなど)、I/O パスの任意の要素が失敗した場合は、代替パスを提供します。

- **負荷分散**: これストレージ デバイスの構成によっては、I/O パスの負荷を検出し、負荷を動的に再調整して、パフォーマンスを向上します。
...
- **カーネル**: 主なコンポーネントは、 *デバイス マッパー* を I/O の経路を変更し、パスとパスのグループのフェールオーバーをサポートしています。

1. **ユーザー領域**: これらは *マルチパス ツール* を行うにはどのようなデバイス マッパー マルチパス モジュールに指示してマルチパス デバイスを管理します。 このツールは次の要素で構成されます。

    - **マルチパス**: 示し、multipathed のデバイスを構成します。
        
    - **Multipathd**: デーモンをマルチパスを実行して、パスを監視します。
    
    - **Devmap 名前**: devmaps の udev へ意味のあるデバイス名を提供します。
 
    - **Kpartx**: 線形 devmaps をマルチパス マップに分割するデバイスのパーティションにマップします。
    
    - **Multipath.conf**: 組み込みの構成テーブルを上書きするために使用マルチパス デーモンの構成ファイル。
...
- **システム レベルの既定値** *(既定)*: システム レベルの既定値をオーバーライドすることができます。

1. **デバイスをブラック** *(ブラック リスト)*: デバイス マッパーによって制御する必要がないデバイスの一覧を指定することができます。

1. **例外のブラック_リスト** *(blacklist_exceptions)*: 特定のデバイスをブラック リストに示されている場合でも、マルチパス デバイスとして扱うことがわかります。

1. **記憶域コント ローラーの特定の設定** *(デバイス)*: 仕入先および製品情報を持つデバイスに適用される構成設定を指定できます。

1. **デバイス固有の設定** *(できるように、マルチパス)*: このセクションを使用して個々 の Lun の構成設定を微調整することができます。
...
1. インストール *iSCSI イニシエーター バージョン* CentOS サーバーにします。 次の手順に従って、インストール *iSCSI イニシエーター バージョン*です。

    1. CentOS ホストに `root` としてログオンします。

    1. インストール、 *iSCSI イニシエーター バージョン*です。 次のコマンドを入力します。
...
1. インストール *デバイス mapper-マルチパス*します。 次のコマンドを入力します。

    `yum install device-mapper-multipath`

    インストールが開始されます。 型 **Y** 確認を求め時に続行します。
...
- iSCSI 用に有効になっている 2 つ以上のインターフェイス。 StorSimple デバイス上の 2 つのインターフェイスが iSCSI に対応していることを確認するには、StorSimple デバイスの Azure クラシック ポータルで次の手順を実行します。
    ...
    1. StorSimple Manager サービスを選択して [ **デバイス** 特定の StorSimple デバイスを選択します。 クリックして **構成** 、ネットワーク インターフェイス設定を確認します。 2 つの iSCSI 対応ネットワーク インターフェイスを示したスクリーン ショットは次のとおりです。 DATA 2 と DATA 3 の両方で、iSCSI の 10 GbE インターフェイスが有効になっています。 
        ...
        1. 両方のネットワーク インターフェイスが確実に iSCSI に対応するようにします。  **ISCSI 対応** フィールドを設定する **はい**します。
