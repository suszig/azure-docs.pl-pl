...
前面パネルとも呼ばれる、 *操作パネル* または *ops パネル*, 、システムのすべてのモジュールの情報を集約、ステータスを表示します。 StorSimple デバイスと EBOD エンクロージャの前面パネルはまったく同じです。次の図に前面パネルを示します。  
...
EBOD エンクロージャにおけるデバイスの前面パネル Led の大きな違いは、 **システム ユニット識別番号** LED ディスプレイに表示されます。 既定のユニット ID、デバイスに表示されるが **00**, のに対し、EBOD エンクロージャに表示される既定のユニット ID は、 **01**します。 これにより、デバイスの電源をオンにしたときに、デバイスと EBOD エンクロージャをすぐに見分けることができます。 デバイスがになっている場合で提供される情報を使用して  [新しいデバイスをオンに](storsimple-turn-device-on-or-off.md#turn-on-a-new-device) デバイスに EBOD エンクロージャを区別するためにします。  
...
| LED | 説明                                                                            
|---- | ----------- |
| ID LED (青) | モジュールが識別されていることを示します。 青色の LED が動作中のコントローラーで点滅している場合、そのコントローラーがアクティブ コントローラーで、もう一方がスタンバイ コントローラーです。 詳細については、次を参照してください。 [デバイスでアクティブなコント ローラーの識別](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device)します。 |
...
|-------|----------------------|-------------------------------|----------------------------|
| コント ローラー モジュール OK | ON | OFF | - |
| コント ローラー モジュール障害 | OFF | ON | - |
| 外部ホスト ポート接続なし | - | - | OFF |
...
詳細について [StorSimple ハードウェア コンポーネントと状態](storsimple-monitor-hardware-status.md)します。
