...
| Update 2 ～ Update 1 (1、1.1、1.2)| [はい] <br></br>固定またはボリュームまたはその両方の 2 つの階層型を使用してローカルでを使用して、ボリュームは常が失敗した場合の階層型どおりにフェールオーバーします。| [はい]<br></br>ボリュームが固定されているローカルを使用する場合は、これらが失敗している階層型どおりにフェールオーバーします。|
| Update 2 ～ Update 2 (以降のバージョン)| [はい]<br></br>またはローカル固定またはボリュームの階層型を使用して、2 のボリュームの混在が経由で開始したボリュームの型として常に失敗した場合の階層型階層型およびローカルで固定されたとしてローカルの固定します。| [はい]<br></br>ボリュームが固定されているローカルを使用する場合は、これらが失敗している階層型どおりにフェールオーバーします。|
...
1. 表示されるウィザードの **[フェールオーバーするボリューム コンテナーを選択する]** で、次の手順を実行します。
    ...
    1. 最後に、**[フェールオーバーの確認]** ですべてのフェールオーバー設定を見直します。 チェック アイコンをクリックして ![チェック マーク アイコン](./media/storsimple-device-failover-disaster-recovery/IC740895.png)します。
...
1. 新しいデバイスの場合は、最初にデバイスの最小構成を完了します。
    >[AZURE.IMPORTANT] * * 最低限の構成が最初に完了しない場合、現在の実装にバグがあるため DR は失敗します。 この動作は、それ以降の release.* * で修正される予定

1. 古いデバイス (オフラインの状態) を選択し、**[フェールオーバー]** をクリックします。 表示されるウィザードでこのデバイスをフェールオーバーし、ターゲット デバイスを新しく登録されたデバイスに指定します。 詳細についてを参照してください [別の物理デバイスにフェールオーバーする](#fail-over-to-another-physical-device)します。
...
- [StorSimple デバイスを非アクティブ化します。](storsimple-deactivate-and-delete-device.md#deactivate-a-device)
- [StorSimple デバイスを削除します。](storsimple-deactivate-and-delete-device.md#delete-a-device)

StorSimple Manager サービスを使用する方法についてを参照してください [StorSimple Manager サービスを使用して、StorSimple デバイスを管理する](storsimple-manager-service-administration.md)します。
