...
>[AZURE.NOTE] 
>
>を正しく動作させるために、インターフェイスの速度と二重化する各デバイスのインターフェイスが接続されているスイッチを確認します。 スイッチ インターフェイスは、ギガビット イーサネット (1000 Mbps) でネゴシエートするかまたはこれに合わせて構成されて、全二重である必要があります。 インターフェイスが低速または半二重で動作している場合、パフォーマンスの問題が発生します。
>
>中断やダウンタイムを最小限に抑えるには、各デバイスの iSCSI ネットワーク インターフェイスが接続するスイッチのポートで、portfast を有効にすることお勧めします。 これにより、フェールオーバーの発生時にネットワーク接続を素早く確立できるようになります。
...
- 学習方法 [StorSimple デバイスの MPIO を構成する](storsimple-configure-mpio-windows-server.md)します。
