...
## はじめに
次の論理チャートでは、Microsoft Azure Platform で利用可能な多数のセキュリティ手法の具体例を示しています。 これを使用して必要な検索を高速で、または使用可能なオプションの多くの説明を表示する最初からドキュメントを参照します。
![セキュリティ オプションのフローチャート][0]

[例 1 – Nsg にアプリケーションを保護する境界ネットワークを構築します。](#example-1-build-a-simple-dmz-with-nsgs)</br>
[例 2: ファイアウォールと Nsg のアプリケーションを保護する境界ネットワークを作成します。](#example-2-build-a-dmz-to-protect-applications-with-a-firewall-and-nsgs)</br>
[例 3: ファイアウォール、UDR、NSG とのネットワークを保護する境界ネットワークを作成します。](#example-3-build-a-dmz-to-protect-networks-with-a-firewall-udr-and-nsg)</br>
[例 4 –、サイト対サイト、仮想アプライアンスの VPN とのハイブリッド接続を追加します。](#example-4-adding-a-hybrid-connection-with-a-site-to-site-virtual-appliance-vpn)</br>
[例 5:、サイト対サイト、Azure ゲートウェイの VPN とのハイブリッド接続を追加します。](#example-5-adding-a-hybrid-connection-with-a-site-to-site-azure-gateway-vpn)</br>
[例 6-ExpressRoute とハイブリッド接続を追加します。](#example-6-adding-a-hybrid-connection-with-expressroute)</br>
VNet の接続、高可用性、およびサービスのチェーンに VNet を追加するための例は、今後数か月間、このドキュメントに追加されます。 

## Microsoft のコンプライアンスとインフラストラクチャの保護
Microsoft は、企業顧客が必要とするコンプライアンス イニシアチブのサポートにおいて指導的立場を獲得しました。 Azure のコンプライアンスの証明書の一部を次に示します。
![Azure 対応のバッジ][1]

詳細については掲載されて: [http://azure.microsoft.com/support/trust-center/compliance/](http://azure.microsoft.com/support/trust-center/compliance/)
...
1.  **Ddos に対する防御**: サービス保護の分散拒否 (DDoS) は、Azure プラットフォーム自体を大規模なインターネット ベースの攻撃から保護する Azure の物理的なネットワークの層、攻撃者が複数の"bot"ノードで使用しようとインターネット サービスが過負荷にします。 Azure には、すべての着信インターネット接続に対する堅牢な DDoS の防御網が用意されています。 この DDoS 保護層にはユーザーが構成できる属性がないため、顧客はこの層にアクセスできません。 これにより、Azure はプラットフォームとして大規模な攻撃から保護されますが、個々の顧客アプリケーションは直接保護されません。 局地的な攻撃に対しては、顧客が復元層を追加で構成できます。 たとえば、顧客 A がパブリック エンドポイントで大規模な DDoS 攻撃を受けた場合、Azure はそのサービスへの接続をブロックします。 顧客 A は、その攻撃に関係のない別の仮想ネットワークまたはサービス エンドポイントにフェールオーバーして、サービスを復元できます。 注目すべきは、そのエンドポイントで顧客 A が影響を受ける可能性はあっても、そのエンドポイント以外の他のサービスには影響がないことです。 さらに、他の顧客やサービスからは、その攻撃による影響がわかりません。
2.  **サービス エンドポイント**: (インターネット) のパブリック IP アドレスとポートを公開しておくには、クラウド サービスまたはリソース グループに許可するエンドポイント、エンドポイントは、内部アドレスと Azure の仮想ネットワーク上のポートへの NAT トラフィック。 これは、外部トラフィックが Azure 仮想ネットワークに到達するための主な経路です。 サービス エンドポイントは、仮想ネットワークに渡されるトラフィックと、そのトラフィックが仮想ネットワークのどこでどのように変換されるかを決定するためにユーザーが構成できます。 
...
1.  **トラフィックの分離**: 仮想ネットワークは、Azure platform 上のトラフィックの分離の境界です。 ある仮想ネットワーク内の VM と別の仮想ネットワーク内の VM は、両方の仮想ネットワークを同じ顧客が作成した場合でも、直接通信することはできません。 これは、仮想ネットワーク内では顧客の VM と通信がプライベートであることを保証する重要な特性です。 
2.  **多層トポロジ**: 仮想ネットワークのサブネットの割り当てとさまざまな要素またはその作業負荷の「階層」の別のアドレス空間を指定することによって多層トポロジの定義に顧客できるようにします。 このような論理的なグループとトポロジを使用すると、顧客は、ワークロードの種類に基づいて異なるアクセス ポリシーを定義するだけでなく、各層の間のトラフィック フローを制御することもできます。 
3.  **クロス プレミス接続**: お客様は、仮想ネットワークと複数の間のクロスプレミス接続のオンプレミス サイトまたは他の仮想ネットワークから Azure Azure VPN ゲートウェイまたは 3 日を [パーティのネットワーク仮想アプライアンスを確立できます。 Azure では、標準的な IPsec/IKE プロトコルと ExpressRoute プライベート接続を使用したサイト間 (S2S) VPN をサポートしています。 
4.  **ネットワーク セキュリティ グループ** (NSG) により、必要な粒度のレベルにあるお客様は、ルール (Acl) を作成する: ネットワーク インターフェイス、個別の仮想マシンまたは仮想サブネットです。 顧客は、仮想ネットワーク内でのワークロード間の通信を許可または拒否することで、クロスプレミス接続を介した顧客のネットワーク上のシステムからのアクセス、または直接インターネット通信を制御できます。 
5.  **ユーザー定義のルート** (UDR) と **IP 転送** により、仮想ネットワーク内の階層間で通信パスを定義します。 顧客は、ファイアウォール、IDS/IPS、その他の仮想アプライアンスをデプロイし、これらのセキュリティ アプライアンスを介してネットワーク トラフィックをルーティングすることで、セキュリティ境界ポリシーを適用、監査、検査することができます。
6.  **仮想アプライアンスをネットワーク** Azure Marketplace で: セキュリティ アプライアンスなど、ファイアウォール、ロード バランサー、および IDS/IPS (侵入検出/防止サービス) が、Azure Marketplace および VM イメージ ギャラリーで規定されています。 顧客は、これらのアプライアンスを自身の仮想ネットワーク、特にセキュリティ境界 (DMZ サブネットを含む) にデプロイすることで、多層のセキュリティで保護されたネットワーク環境を実現できます。

このような機能を使用した場合に Azure で DMZ アーキテクチャがどのように構築されるかについて一例を次に示します。

![Azure の仮想ネットワークで DMZ][5]
...
>[AZURE.TIP] 維持歯車の形の境界ネットワークのセキュリティのアクセスを承認するユーザー アプリケーションの開発または配置/運用管理者として承認された個人とまったく異なります。 これらのグループを区別すると、義務の分離も可能になり、1 人でアプリケーション セキュリティとネットワーク セキュリティの両方の制御をバイパスできなくなります。
...
>[AZURE.TIP] 指定された状態のセキュリティ要件を満たしている境界の最小の数値を使用します。 境界の数が増えると、運用とトラブルシューティングが困難になり、複数の境界ポリシーの管理に伴うオーバーヘッドも時間と共に増大します。 ただし、境界の数が不足するとリスクが高まります。 適切なバランスを見つけることが重要です。

![ハイブリッド ネットワークの 3 つのセキュリティ境界を使用][6]
...
上記の質問に対する回答が覚えている、 [ファスト スタート](#fast-start) 前のセクションは、どの例が、特定のシナリオに最も適したを識別できます。

## Azure 仮想ネットワークでセキュリティ境界を構築する
### 例 1 - NSG を使用して単純な DMZ を構築する
[簡単に戻る](#fast-start) |[ビルドの詳細な手順についてはこの例の]例 [1]
...
さらに (スクリプトと ARM テンプレートの両方が提供します)、この例をビルドする詳細が [ビルドの詳細な手順] の [Example1] ページを配置します。
...
ご覧 [ビルドの詳細な手順] [Example1] ページ。

### 例 2 - ファイアウォールと NSG から成る DMZ を構築してアプリケーションを保護する
[簡単に戻る](#fast-start) |[ビルドの詳細な手順についてはこの例の][Example2]

![受信 DMZ NVA と NSG を使用][8]
...
さらに (スクリプトと ARM テンプレートの両方が提供します)、この例をビルドする詳細が [ビルドの詳細な手順] の [Example2] ページを配置します。
...
ご覧 [ビルドの詳細な手順] [Example2] ページ。

### 例 3 - ファイアウォール、UDR、NSG から成る DMZ を構築してネットワークを保護する
[簡単に戻る](#fast-start) |[ビルドの詳細な手順についてはこの例の][Example3]

![NVA、NSG、UDR と双方向 DMZ][9]
...
さらに (スクリプトと ARM テンプレートの両方が提供します)、この例をビルドする詳細が [ビルドの詳細な手順] の [Example3] ページを配置します。
...
>[AZURE.NOTE] UDR とハイブリッドのネットワークでの現在の制限があります。 これは、今後のリリースで解決される予定です。ExpressRoute またはサイト間ネットワークを使用した DMZ を有効にする方法の例については、この後の例 3 と 4 で説明します。
...
>[AZURE.NOTE] 使用されるネットワーク仮想アプライアンスに基づき、管理ポートは異なります。 ここでは、ポート 22、801、807 が使用される Barracuda NG Firewall を例に説明しています。 ご使用のデバイスの管理に使用される実際のポートについては、アプライアンス製造元のマニュアルを参照してください。
...
>[AZURE.TIP] 2 番目のアプリケーションのトラフィックのルールで任意のポートが許可の実際のシナリオで、この例の簡単な特定のポートされアドレスの範囲は、このルールの攻撃対象領域を削減するために使用する必要があります。
...
ご覧 [ビルドの詳細な手順] [Example3] ページ。

### 例 4 - サイト間の仮想アプライアンス VPN を使用してハイブリッド接続を追加する
[簡単に戻る](#fast-start) |詳細なビルド手順については、使用可能な間もなくできます。

![NVA と境界ネットワークには、ハイブリッド ネットワークが接続されている][11]
...
>[AZURE.NOTE] 静的ルートを作成する必要が有効になっている Azure のパブリック ピアリング オプションを使用して ExpressRoute を使用する場合、企業のインターネット エッジを ExpressRoute WAN のエッジのではなく NVA VPN の IP アドレスにルーティングします。 これは、VPN セッションを中断する可能性の高い、ExpressRoute の Azure パブリック ピアリング オプションでは NAT が必要になるためです (一般的に、IPSec では NAT が好まれません)。
...
![NVA と境界ネットワークは、サイト間 VPN を使用して接続されている][12]
...
![ゲートウェイと境界ネットワークには、ハイブリッド ネットワークが接続されている][14]
...
>[AZURE.NOTE] 静的ルートを作成する必要が有効になっている Azure のパブリック ピアリング オプションを使用して ExpressRoute を使用する場合、企業のインターネット エッジを ExpressRoute WAN のエッジのではなく、Azure VPN ゲートウェイの IP アドレスにルーティングします。 これは、VPN セッションを中断する可能性の高い、ExpressRoute の Azure パブリック ピアリング オプションでは NAT が必要になるためです (一般的に、IPSec では NAT が好まれません)。
...
![ゲートウェイと境界ネットワークは、ExpressRoute 接続を使用して接続されている][15]
...
![ゲートウェイと境界ネットワークには、ハイブリッド ネットワークが接続されている][16]
...
>[AZURE.NOTE] これには、Azure 仮想ゲートウェイで使用される動的ルーティングが複雑なのため、ユーザー定義のルーティング (UDR) と ExpressRoute を使用して制限があります。 ExpressRoute 接続の Azure ゲートウェイと通信するサブネットには、UDR を適用しないでください。 また、Azure ゲートウェイを、UDR が関連付けられている他のサブネットの NextHop デバイスとすることはできません。 UDR と ExpressRoute を完全に連携させる機能は、将来の Azure リリースで提供予定となっています。

</br>
>[AZURE.TIP] ExpressRoute を使用してセキュリティを向上させるためにインターネットから企業のネットワーク トラフィックを維持、大幅に、パフォーマンスが向上でき、Sla の ExpressRoute プロバイダーから提供できます。 ExpressRoute のパフォーマンスに関しては、Azure ゲートウェイでは最大 2 Gbps のスループットを実現できるのに対し、サイト間 VPN を使用する場合、Azure ゲートウェイの最大スループットは 200 Mbps になります。
...
![ゲートウェイと境界ネットワークは、ExpressRoute 接続を使用して接続されている][17]
...
- 仮想ネットワークに関するドキュメント: [https://azure.microsoft.com/documentation/services/virtual-network/](https://azure.microsoft.com/documentation/services/virtual-network/)
- ネットワーク セキュリティ グループに関するドキュメント: [https://azure.microsoft.com/documentation/articles/virtual-networks-nsg/](./virtual-network/virtual-networks-nsg.md)
- ユーザーには、ルーティングのドキュメントが定義されています [https://azure.microsoft.com/documentation/articles/virtual-networks-udr-overview/。](./virtual-network/virtual-networks-udr-overview.md)
