<properties 
   pageTitle="Azure Mobile Engagement ユーザー インターフェイス - 分析" 
...
この記事で説明、 **ANALYTICS** のタブ、 **モバイル エンゲージメント** ポータルです。 使用する、 **モバイル エンゲージメント** を監視し、モバイル アプリの管理ポータル。 ポータルの使用を開始する最初必要を作成する、 **Azure モバイル エンゲージメント** アカウントです。 詳細については、次を参照してください。 [Azure モバイル エンゲージメント アカウントを作成する](mobile-engagement-create-account.md)です。
...
>[AZURE.NOTE] 多くのセクションで、 **モバイル エンゲージメント** ポータル UI を含む、 **ヘルプを表示する** ] ボタンをクリックします。 このボタンを押すと、セクションに関する文脈上の情報が表示されます。

## 標準分析とカスタム分析

Azure Mobile Engagement は、SDK とアプリを統合するとすぐにグラフ化できる、アプリケーションに関する基本的かつ標準的な分析情報のセットを提供します。 Azure Mobile Engagement では、エンドユーザーの動作について入手したい場合に追加のカスタム分析情報を収集する機能も備えています。 カスタム"Tags (アプリ情報)"にから作成されたのタグ プランを作成することでこれを行う **設定** Azure モバイル エンゲージメントをこの追加のデータを収集できるようにします。
...
- ユーザー パス: では、ユーザーがアプリケーションのアクティビティ (画面) を移動する方法を示します。 スライダーを動かして詳細のレベルを調整できます。 青のノードは、アプリケーションのアクティビティを表します。 その大きさは、ユーザーが費やした時間に比例します。 白のノードは、セッションの開始と停止を表します。 赤のノードは、クラッシュを表します。 リンクは、アプリケーションのアクティビティ間 (またはアクティビティとクラッシュ間) の移行を表します。 ノードまたはリンクをクリックすると、ツールヒントにデータの詳細情報 (特定の画面で費やされた時間、移行の数、アクティビティからアクティビティへの移行の比率) が表示されます。 (A ---60% ---> B は、ユーザーは、60% の確率でアクティビティ A からアクティビティ B に移行した、という意味です)。グラフを分かりやすいように再編成できます。グラフの位置は変更を加えるたびに保存されます。 クラッシュを表示または非表示にしてグラフの負荷を減らすことができます。
