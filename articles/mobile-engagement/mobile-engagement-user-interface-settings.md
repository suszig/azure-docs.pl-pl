 ...
 **設定** 、アプリケーションによって異なりますが与えられているアプリケーションのアクセス許可はアプリケーションのプラットフォームで使用できるメニュー オプション。 設定には、詳細、プロジェクト、ネイティブのプッシュ、プッシュの速度、タグ (アプリ情報)、コマーシャル プレッシャーの項目があります。 [設定] セクションの [タグ (アプリ情報)] メニュー オプションはアプリケーション (SDK 使用) またはバックエンド (デバイス API 使用) で管理できます。 
...
分析構成では週の開始日および日の期間表示または変更を行うことができます。
...
GCM を使用してネイティブのプッシュを有効にするには、Google の指示に従う必要があります。 その後、IP の制限なしで構成された、サーバーの SimpleAPI キーを貼り付ける必要があります。 
...
ADM を使ってネイティブ プッシュ通知を有効にするには、Amazon を提供する必要があります。 <OAuth credentials> クライアント ID とクライアント シークレット (Android v2.1.0 SDK との統合が必要) で構成されます。
