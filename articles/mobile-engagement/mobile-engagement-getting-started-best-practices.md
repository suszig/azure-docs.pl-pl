...
**携帯電話の画面は非常に多くなりスペース:** 2013 で、調査によると、平均のモバイル デバイスが 27 のアプリケーションがインストールされている必要があります。 ユーザーが日頃アプリに費やす月間の利用時間は 30 時間となっています。 その時間の大半約 20 時間は、ソーシャル ネットワークやゲームに費やされていました。 2014 年には、Android マーケットでユーザーに提供されているアプリケーション数は約 150 万個を突破しました。 Apple ストアで提供されているアプリの数も 120 万個に及びます。 今もなおモバイル アプリは増え続け、この急成長する市場で開発者は競争を繰り広げています。 
...
KPI を正しく定義することは簡単ではありません。 異なる業界を対象に作成されたアプリは、仕様や目的もそれぞれ異なります。 どう作業を進めればよいかがわからず、つまずいてしまうこともあります。 これを回避するために、目標と Kpi を 3 つのカテゴリに分類する必要があります: **ビジネス**, 、**エンゲージメント**, 、および **技術**します。 これと呼ばれる、 **おすすめコンテンツ モデル**します。
...
使用することができます、 **KPI ビルダー** 内のワークシート、 [メディア戦略テンプレート][Media Playbook link] 例とガイダンスです。
...
*"オンボードご利用いただきございます。 ご、1 か月間無料の取得にログインしてください」*
...
*"John 深刻なフットボールのファンが! NFL のセクションにログインし、SuperBowl に自由にアクセスを獲得!"*
...
*"Hi Wes、コーヒーのブランドのいずれかになります発売 2015 年 9 月の最初の週から 25% です。 お願いいたしますお客様は、確実に実行するに注意してくださかった。"*
...
1. **お知らせ** : 異なる時点でユーザーに広告メッセージを送信することができます (out of app、アプリケーションでいつでもまたは)。
2. **投票** : 質問を投げかけることでエンドユーザーからの情報を収集することができました。 ターゲットのエンドユーザーに条件を作成する場合は、これらの回答を使用しです。
3. **データ プッシュ** : アプリケーションを更新して、バイナリ シリアル化または base 64 のデータ ファイルを送信することができます。 必要な情報をデータ プッシュに格納してアプリケーションに送信することで、アプリの機能をパーソナライズすることができます。 データ プッシュに格納されたデータを処理する機能はアプリケーション側で実装する必要があります。
4. **タイル (Windows Phone のみ)** : を Microsoft Push Notification Service (MPNS) を使用して XML データ (サポートされる SDK バージョン 0.9.0 以降を含むネイティブのプッシュ通知の送信を有効になっています。 タイルの最終的なペイロードは 32 キロバイト以下である必要があります)。 メッセージは、ボードのタイルに直接表示されます。
5. **Webview** : web ビューが含まれる web コンテンツをポップアップします。 このポップアップは、エンドユーザーがプッシュ通知をクリックしたときに表示されます。 Web ビューでは、エンドユーザーとの対話を増やすことができます。
 
>[AZURE.NOTE] 各プラットフォーム (iOS、Android、Windows) に準拠してプッシュ通知として送信するコンテンツが、アプリを開発し、プッシュ通知を送信するためのガイドラインを使用することを確認します。
...
使用して、 **エンゲージメント プログラム** 内のワークシート、 [メディア戦略テンプレート][Media Playbook link] 例とガイダンスです。
...
アプリケーションに対し、やみくもにタグ付けすることは避けてください。 対象とするタグ データは、モバイル エンゲージメント戦略に不可欠な要素に限定します。 通常この点はアプリケーションによって異なります。  [メディア戦略テンプレート][Media Playbook link] 指定されたメソッドを使用してタグ計画でビルドする Azure モバイル エンゲージメントによりによって提供されます。 使用して、 **タグ計画** タグ計画を作成するためのガイドとしてワークシートです。

ワークシートのタグ セクションは、具体的に定義してください。 混乱を避けるためには、具体的に記述することがきわめて重要となります。. それぞれのタグをどのような状況で送信するかを細かく記述してください。 各タグの埋め込み先となるアクティビティの名前も記述します。 これは、すべてに含めるか、 **情報** ワークシートの一部です。 Tag plan ワークシートは、テスト検証の主要な参考資料としてご利用ください。 

 **データを収集する** セクションは、開発チームが検索するタグの種類、名前、値、および各に必要な追加情報のキー/値ペアに埋め込まれますアプリケーションです。

プロジェクトに関わるすべてのチームと連携して、Tag plan シートをレビューすることをお勧めします。 必要な修正を行ったうえで、すべての内容がマーケティング チームおよび開発チームに対して明確になっていることを確認します。

 **作業記述書** ワークシートを使用して、プロジェクトにかかわるすべてのユーザー ガイドを支援することです。
...
Azure モバイル エンゲージメント SDK をアプリに統合する際の手順の詳細については、以下、  [Engagement SDK 統合](mobile-engagement-windows-store-integrate-engagement.md) Azure の web サイトに関するドキュメントです。 ページ上部のリンクからターゲット プラットフォームを選択できます。
...
Azure Mobile Engagement のセットアップはきわめて単純です。 ユーザー インターフェイスに関連するすべてのドキュメントは、Azure モバイル エンゲージメントの web サイトで利用できる [ユーザー インターフェイスを移動する方法](mobile-engagement-user-interface-navigation.md)します。
...
その後。
- 独自のデバイスでテスト、deviceID を登録します。
- アカウントの設定] に移動し、グラフと通知の配信時間のタイム ゾーン設定を持つように、タイム ゾーンを設定します。
- アプリケーションの設定に移動し、"アプリ情報を登録の"ターゲットのエンドユーザーを現実のものにする必要があります。

最初のプッシュ通知キャンペーンを実行する方法の詳細については、確認 [の使用とエンドユーザーに通知するためのプッシュの管理を始める方法](mobile-engagement-how-tos.md)します。
...
- 追跡: ユーザーを取得して、データ コレクションのソースを定義する可能性があります。 Azure Mobile Engagement をそのデータ収集ソースにリンクさせます。 パフォーマンスは、ソースごとに監視することができます。 その情報を活かして、マーケティングへの投資を最大限に高めることが可能です。 
...
- 参照してください [Mobile Engagement 戦略を定義する](mobile-engagement-define-your-mobile-engagement-strategy.md) を Mobile Engagement 戦略の定義の詳細を参照してください。
