...
John は読み取りが最近、 [Azure モバイル エンゲージメント - ファースト ステップ ガイドのベスト プラクティスで](mobile-engagement-getting-started-best-practices.md) し、番組ガイドからの推奨事項を実装することにしました。
...
主要関係者とのミーティングに基づいて、ビジネス KPI を定義します。 彼は次の第 1 部、 [Azure モバイル エンゲージメント - ファースト ステップ ガイドのベスト プラクティスで](mobile-engagement-getting-started-best-practices.md)します。 
...
その John は、彼の Kpi の定義が完了したら、これで 4 のエンゲージメント プログラムとそれぞれの目標を定義することで、Engagement 戦略のフェーズを開始します。
    ![][1]
...
詳細についてを参照してください、 [プレイブック](https://github.com/Azure/azure-mobile-engagement-samples/tree/master/Playbooks)します。
 ...
 **非アクティブなプログラム** が John ユーザーの傾向を理解できるようにします。 非アクティブなユーザーの 15% が、またアプリを使用するようになっていることがわかります。 ただし、そのほとんどは、アクティブな状態が 1 か月以上続きません。 このような推移は、通知を追加し、コンテンツの選択肢を増やすことで、最適化できると予測しました。

 **検出プログラム** うまく動作しません。 抱き合わせ販売は増えていますが、目標の達成には不十分です。 適切なターゲティングを行い、妥当なコンテンツを提示するためのデータが足りないと判断しました。 このプログラムは停止し、Azure Mobile Engagement による "読み物プッシュ通知" の送信に集中することにしました。 記者たちは既にプッシュ通知を送信するための CMS ソリューションを持っており、変更は望んでいません。
...
1. **オペレーティング システム** : すべてがあるため、John は、すべてのケースを一覧表示することに決めます、かどうか、Api 処理を確認、プッシュ通知を管理する、独自のルール。
例: Android のプッシュ システムでは大きな画像を使用できますが、iOS ではできません。

2. **時間枠**: John が API タイム フレームを設定して、キャンペーンに終了を設定します。 ユーザーをわずらわせるような大量の通知は送信しないようにします。

3. **カテゴリ**: マーケティング チームは、アラートの種類ごとにテンプレートを準備します。 IT チームに、API 内にカテゴリを設定するように依頼します。
