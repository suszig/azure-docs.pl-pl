...
- 証明書が有効でない場合、または本番用と開発用の証明書を正しく使用していない場合、プッシュは機能しません (iOS のみ)  (**注:** 「アプリ外」プッシュ通知配信されない iOS に両方の開発 (DEV) があり、セキュリティ トークンは、証明書に関連付けられているために、同じデバイスにインストールされているアプリケーションのバージョンを実稼働 (PROD) は、Apple によって無効にすることがあります。 この問題を解決するには、本番用と開発用のアプリケーションのバージョンをアンインストールして、デバイスに 1 つのバージョンのみを再インストールします)。
...
- プッシュ通知をクリックした後に、別の場所にユーザーをリダイレクトするディープ リンクのスキームは、Mobile Engagement から直接ではなく、アプリケーションとデバイス OS内で作成し、管理する必要があります  (**注:** out of app 通知にはリンクできません ios アプリの場所で Android とします)。
...
- Android と iOS のアプリケーション通知の処理方法が異なるので、アプリケーションの Android や iOS のバージョン間でのプッシュの統計情報を直接比較するのが困難になっています。 Android は iOS より多くの OS レベル通知情報を提供します。 されていない限り、iOS でこの情報がレポートされませんが、ネイティブ通知の受信、クリックすると、または通知センターで削除された場合に報告を android 
通知をクリックします。 
