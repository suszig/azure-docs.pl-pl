...
- [「UI ドキュメント - Reach - 新しいプッシュ キャンペーン」][Link 27]

## 対象ユーザーの条件には次のようなものを使用できます:
- **テクニカル：** [分析] セクションおよび [監視] セクションに表示されるのと同一の技術情報に基づいて対象を絞り込むことができます。 **関連項目:** [UI ドキュメント - Analytics][Link 15],  、[UI ドキュメント - Monitor][Link 16]
- **場所:** 地理フェンス操作用いた"Real time location reporting"を使用するアプリケーションは GPS の位置から対象を基準として地理的な場所を使用することができます。 "大まかなエリアの位置報告" の呼び出しを使用して携帯電話の位置から対象ユーザーを決定することもできます ("Real time location reporting" と "大まかなエリアの位置報告" は、SDK からアクティブ化する必要があります)。 **関連項目:** [SDK ドキュメント - iOS - 統合][Link 5], 、[SDK ドキュメント - Android - 統合][Link 5]
...
- **App Info:** ユーザーの行動を追跡するには、[設定] からカスタムのアプリ情報タグを作成できます。 **関連項目:** [UI ドキュメント - 設定][Link 20]
