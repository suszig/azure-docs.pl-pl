...
---|---|---|---
 ...
 SUSE Linux Enterprise| SLES 11 sp 3 +、SLES 12 + と  <p><p> SAP 11.3 + 向け SLES| カーネル内| : パッケージ [Cloud:tools](https://build.opensuse.org/project/show/Cloud:Tools) リポジトリ"walinuxagent"<p><p>ソース コード: [GitHub](http://go.microsoft.com/fwlink/p/?LinkID=250998)
...
* CoreOS はセキュリティ、一貫性、および信頼性に設計されています。 yum や apt でパッケージをインストールする代わりに、Linux コンテナーを使用して、より抽象的なレベルでサービスを管理します。 1 つのサービスのコードとすべての依存関係が 1 つまたは複数の CoreOS machines.* で実行できるコンテナー内にパッケージ化します。
