...
インストールが完了したら、 」と入力して、リソース マネージャー モードになっている確認 `azure config モード arm`します。

さらに、 今すぐ [、職場または学校の id ログイン](../xplat-cli-connect.md#use-the-log-in-method) 」と入力して `azure ログイン` 、Azure アカウントに対話型のログイン エクスペリエンスを画面の指示に従います。
...
型 `azure グループを作成 < マイ グループ名 > westus` 交換 _< マイ グループ名 >_ をユーザーに固有のグループ名 (できる領域を使用するさまざまな場合)。次のように表示されます。
...
」と入力して VM を作成 `azure vm の簡易作成`, と残りのパラメーターを入力するプロンプトが表示されます。 、先ほど作成したと、リソース グループの名前を使用して、 **ImageURN** 値を使用して `正規: ubuntuserver:14.04.2-LTS:latest`, れるため、次のようになります。 なお、 `azure vm の簡易作成` コマンド プロンプトを作成するに必要な基本的な情報については、ホスト、およびなど、Linux 仮想マシンに接続します。
...
> [AZURE.NOTE] このトピックは、ユーザー名とパスワードを使用して VM に接続します。パブリックおよびプライベート キーのペアを使用する VM と通信して、次を参照してください。 [ssh Azure 上の Linux の使用方法](virtual-machines-linux-use-ssh-key.md)します。 変更することができます、 **SSH** で作成された仮想マシンの接続、 `azure vm の簡易作成` コマンドを使用する、 `azure vm のリセット アクセス` をリセットするコマンド **SSH** または、完全にアクセス、追加または削除、ユーザーのアクセスをセキュリティで保護する公開キー ファイルを追加します。 この記事では、わかりやすいように、ユーザー名とパスワードに **SSH** を使用しています。

使用した接続に慣れていない場合 **ssh**, 、コマンドの形式 `ssh < ユーザー名 > @< publicdnsaddress >-p <、ssh ポート >`します。ここでは、前の手順のユーザー名およびパスワードと、既定の **ssh** ポートであるポート 22 を使用します。
...
新しいディスクには簡単に接続できます。入力するだけ `azure vm ディスクを接続-新しい < myuniquegroupname >< myuniquevmname >< サイズで GB >` を作成して、VM に新しい GB のディスクをアタッチします。これは、次のようになります。
...
これで、ディスクを探してみましょうを使用して `dmesg | grep SCSI` (新しいディスクの探索に使用する方法が異なる場合があります)。 この場合、次のようになります。
...
このトピックで、 `sdc` ディスクである必要があります。 ディスクをパーティション分割を使用してディスク `sudo fdisk/デベロッパー/sdc` いると仮定すると、ディスクがあった場合は、 `sdc`, と、プライマリ ディスク パーティション 1 上と他の既定値をそのまま使用します。
...
**mkfs** コマンドを使用し、ファイル システムの種類とデバイス名を指定して、パーティションにファイル システムを作成します。 このトピックで使用している `ext4` と `/dev/sdc1` 上から。
...
使用して、ファイル システムをマウントするディレクトリを作成して、ここで `mkdir`:

    sudo mkdir /datadrive

使用してディレクトリをマウントして `マウント`:
...
> [AZURE.NOTE] 識別用の SSH キーを使用して Linux 仮想マシンに接続することもできます。 詳細については、「 [ssh Azure 上の Linux の使用方法](virtual-machines-linux-use-ssh-key.md)します。
...
- [Azure コマンド ライン インターフェイスを使用する方法](../virtual-machines-command-line-tools.md)
