...
この記事では、Azure リソース マネージャー テンプレートと Azure CLI を使用し、Azure 仮想マシンのデプロイと管理に関する以下の一般的なタスクを実行する方法について説明します。 他のテンプレートを使用する、次を参照してください。 [Azure クイック スタート テンプレート](http://azure.microsoft.com/documentation/templates/) と [テンプレートを使用したアプリケーション フレームワーク](virtual-machines-app-frameworks.md)します。
...
Windows ベースの仮想マシンでは、次を参照してください。 [の作成とアップロード Azure への Windows Server VHD](virtual-machines-create-upload-vhd-windows-server.md)します。

Linux ベースの仮想マシンでは、次を参照してください。 [の作成とアップロード Linux オペレーティング システムを含むバーチャル ハード ディスク](virtual-machines-linux-create-upload-vhd.md)します。
