...
かどうかは内部設置型 SQL Server を実行するマシンの場合がありますが [、内部設置型 SQL Server マシンから Azure Key Vault にアクセスするための手順](https://msdn.microsoft.com/library/dn198405.aspx)します。 Azure Vm で SQL server を使用して時間を節約できますが、 *Azure Key Vault 統合* 機能します。 いくつかの Azure PowerShell コマンドレットでこの機能を有効にし、SQL VM が Key Vault にアクセスするために必要な構成を自動化できます。
...
- アプリケーションを追加すると、検索、 **クライアント ID**  上、 **構成** ] タブをクリックします。 
    ...
    クライアント ID が後に割り当てられている、 **$spName** Azure Key Vault 統合を有効にする PowerShell スクリプトのパラメーターを (サービス プリンシパル名)。 
- また、これらの手順で、鍵を作成するとき、次のスクリーンショットのように、鍵のシークレットをコピーします。 このキーのシークレットが後に割り当てられている、 **$spSecret** PowerShell スクリプトで (サービス プリンシパル シークレット) パラメーター。  
    ![Azure Active Directory のシークレット](./media/virtual-machines-sql-server-azure-key-vault-integration/aad-sp-secret.png)
- この新しいクライアント ID を次のアクセス権限を承認する必要があります: **暗号化**, 、**を復号化**, 、**wrapKey**, 、**unwrapKey**, 、**記号**, 、および **ことを確認**します。 これは、 [Set-azurekeyvaultaccesspolicy](https://msdn.microsoft.com/library/azure/dn903607%28v=azure.98%29.aspx) コマンドレットです。 詳細については、次を参照してください。 [キーやシークレットを使用してアプリケーションを承認](../key-vault/key-vault-get-started.md#authorize)します。
...
資格情報コンテナーの手順の作成を始めるときに注意して、返された **vaultUri** プロパティで、資格情報コンテナーの URL です。 例では、資格情報コンテナー名が ContosoKeyVault で、次に示す手順で提供されるため、資格情報コンテナー URL になります https://contosokeyvault.vault.azure.net/します。

![Azure Active Directory シークレット](./media/virtual-machines-sql-server-azure-key-vault-integration/new-azurekeyvault.png)
 
資格情報コンテナーの URL が後に割り当てられている、 **$akvURL** Azure Key Vault 統合を有効にする PowerShell スクリプトのパラメーターです。
...
|**$credName**|**資格情報名**: 正常統合は、VM が key vault にアクセスできるように、SQL Server の資格情報を作成します。 この資格情報の名前を選択します。|"mycred1"|
|**$vmName**|**仮想マシン名**: 以前に作成した SQL VM の名前。|"myvmname"|
|**$serviceName**|**サービス名**: SQL VM に関連付けられているクラウド サービス名です。|"mycloudservicename"|

### PowerShell で AKV 統合を有効にする
 **新規 AzureVMSqlServerKeyVaultCredentialConfig** コマンドレットは、Azure Key Vault 統合機能の構成オブジェクトを作成します。  **Set-azurevmsqlserverextension** との統合を構成、 **KeyVaultCredentialSettings** パラメーター。 次の手順では、これらのコマンドを使用する方法を示します。
...
>[AZURE.NOTE] 各例は、2 つの前提条件に基づいて: key vault からの非対称キーと呼ばれる **いる CONTOSO_KEY** 正常統合機能によって作成された資格情報と呼ばれる、 **Azure_EKM_TDE_cred**します。
