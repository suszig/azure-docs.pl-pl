...
> [AZURE.NOTE] このチュートリアルを実行するには、Linux を実行する Microsoft Azure Virtual Machine が既に存在している必要があります。 参照してください、
[Azure Linux VM チュートリアル](virtual-machines-linux-tutorial.md) で Linux VM を作成および設定を `mysqlnode` VM 名として、 `azureuser` 続行する前にユーザーとして。
...
- 手順 2. MySQL サーバー にログインする
    ...
    MySQL サーバー のインストールが完了したら、MySQL サービスが自動的に開始されます。 MySQL サーバーにログインして `ルート` ユーザーです。
