...
> [AZURE.NOTE]ログに記録された情報のほとんどについては、http://blogs.msdn.com/b/kwill/archive/2013/08/09/windows-azure-paas-compute-diagnostics-data.asp にあります。

コレクションの 2 つのモードが収集するファイルの種類によって異なります。
- Azure ゲスト エージェントのログのみ (GA)。 この収集モードには、Azure のゲスト エージェントなど他の Azure コンポーネントに関連するすべてのログが含まれています。
...
- Azure PowerShell コマンドレット V0.8.0 以降を使用している必要があります。 詳細については、次を参照してください。 [Azure のダウンロード](http://azure.microsoft.com/downloads/)します。
