...
読者は既に Azure サブスクリプションを持っていることを前提にしています。  無料試用版にサインアップしない場合 [http://azure.microsoft.com](http://azure.microsoft.com)します。 MSDN サブスクリプションがある場合は、次を参照してください。 [Microsoft Azure 特別料金: MSDN、MPN、Bizspark の特典](http://azure.microsoft.com/pricing/member-offers/msdn-benefits/?c=14-39)します。 Azure の詳細については、次を参照してください。 [Azure は何ですか。](http://azure.microsoft.com/overview/what-is-azure/)

このトピックに加え、既に仮想マシンあり、別の Linux ディストリビューションへの LAMP スタックのインストールの基本について知りたいを参照してください [Azure で Linux 仮想マシンの LAMP スタックのインストール](virtual-machines-linux-install-lamp-stack.md)します。

Azure Marketplace から事前構成済みの LAMP イメージをデプロイすることもできます。 次の 10 分がビデオでは、Azure Marketplace から構築済みの LAMP イメージを展開が導入されています。 (Azure Vm 上の LAMP スタック] (https://channel9.msdn.com/Shows/Azure-Friday/LAMP-stack-on-Azure-VMs-with-Guy-Bowerman)。
...
-   ダウンロードして、次の場所から puttygen をインストールします [http://www.chiark.greenend.org.uk/~sgtatham/](http://www.chiark.greenend.org.uk/~sgtatham/)putty/download.html。
-   puttygen.exe を実行します。
-   クリックして **生成** キーを生成します。 処理中にウィンドウの空白の領域にマウスを移動すると、ランダム性を高めることができます。  
![][1]
-   生成処理が終わると、生成されたキーが表示されます。 次に例を示します。  
![][2]
-   選択し、その公開キーのコピー **キー** という名前のファイルに保存および **publicKey.pem**します。 クリックしてしないでください **公開キーの保存**, 、保存された公開キーのファイル形式がする公開キーと異なるためです。
-   クリックして **秘密キーの保存** という名前のファイルに保存し、 **privateKey.ppk**します。

###手順 2. Azure ポータルでイメージを作成する
 [Azure ポータル](https://portal.azure.com/), 、] をクリックして **新規** タスク バーし、次の手順に従って、お客様のニーズに合った Linux イメージを選択してイメージを作成します。 この例では Ubuntu 14.04 イメージを使用します。

![][3]

 **ホスト名**, 、このバーチャル マシンにアクセスするユーザーとインターネット クライアントが使用する URL の名前を指定します。 DNS 名の最後の部分を定義すると (LAMPDemo など)、Lampdemo.cloudapp.net のように Azure が URL を生成します。

 **ユーザー名**, 、後で使用する名前を選択、バーチャル マシンにログインします。

 **SSH 認証キー**, からキーの値をコピー、 **publicKey.pem** ファイルで、puttygen によって生成された公開キーが含まれています。  

![][4]

必要に応じて、その他の設定を構成し、 **作成**します。
...
Azure ポータルでをクリックして **参照に仮想マシン]-> [**, 、作成した仮想マシンをクリックします。

![][5]

仮想マシンにエンドポイントを追加するには、クリックして、 **エンドポイント** ボックス。

![][6]

クリックして **追加**します。 新しい仮想マシンをプロビジョニングするときには、必要に応じてエンドポイントを有効または無効にできます。

エンドポイントを構成します。  

1.  [エンドポイントの名前を入力 **エンドポイント**します。
2.  80 と入力 **パブリック ポート**します。 Apache の既定のリッスン ポートを変更している場合は、Apache のリッスン ポートと同じになるように [プライベート ポート] を更新する必要があります。
3.  80 と入力 **パブリック ポート**します。 既定では、HTTP トラフィックはポート 80 を使用します。
80 に設定している場合は、Apache Web サービスに接続するための URL にポート番号を含める必要はありません。 たとえば、http://lampdemo.cloudapp.net のようになります。
Apache のリッスン ポートを 81 などの別の値に設定している場合は、Apache Web サービスにアクセスするための URL にポート番号を追加する必要があります。 たとえば、http://lampdemo.cloudapp.net:81/です。

![][7]

をクリックして **OK** 仮想マシンにエンドポイントを追加します。
...
最初に、Azure ポータルから仮想マシンの DNS 名を取得します。 をクリックして **-> 仮想マシン] の [参照]** 仮想マシンの名前 **のプロパティ]-> [**, を検索し、 **ドメイン名** のフィールド、 **プロパティ** を並べて表示します。

SSH 接続のポート番号を取得、 **SSH** フィールドです。   たとえば次のようになります。  

![][8]

Putty をダウンロード [ここ](http://www.putty.org/) します。  

ダウンロード後、実行可能ファイル PUTTY.EXE をクリックします。 仮想マシンのプロパティから取得したホスト名とポート番号を使用して、基本オプションを構成します。 たとえば次のようになります。

![][9]

左のウィンドウで  **接続 SSH]-> [認証]-> [** 順にクリック **参照** の場所を指定、 **privateKey.ppk** ファイルで、フェーズ 1 で puttygen によって生成された秘密キーが含まれています: イメージを作成します。 たとえば次のようになります。  

![][10]

クリックして **開く**します。 メッセージ ボックスによるアラートが表示される場合があります。 DNS 名を構成してポート番号を正しくクリックして **はい**します。
...
Tasksel は、複数の関連パッケージを一連のタスクとしてシステムにインストールする、Debian/Ubuntu ツールです。 詳細については、次を参照してください。 [Tasksel のコミュニティ ヘルプ Wiki](https://help.ubuntu.com/community/Tasksel)します。
...
次に、ウィザードを使用し、選択、 **MySQL ルート パスワード**します。
...
-   Web サーバーの root は **/var/www/html**します。 このフォルダーにファイルをアップロードするユーザーに権限を付与する必要があります。 次の例は、書き込み権限を lampappgroup という名前のグループに追加し、このグループに azureuser のユーザー名を追加する方法を示しています。  
    ...
    >[AZURE.NOTE] Var/www/html/ファイルを変更する場合、再度ログインする必要があります。
-   SFTP クライアント (FileZilla など) を使用して、DNS、仮想マシンの名前 (例: lampdemo.cloudapp.net) に接続しに移動/**var/www/html** してサイトを公開します。  
![][18]
...
-   **考えられる根本原因**
    ...
    パブリック ポートとプライベート ポートのエンドポイント設定をチェックし、プライベート ポートが Apache のリッスン ポートと同じになっていることを確認します。 仮想マシンのエンドポイントを構成する手順については、「フェーズ 1: イメージを作成する」を参照してください。</br>
...
root アカウントからアクセス許可を取得する必要があります。 フォルダーの所有権を、root からマシンをプロビジョニングしたときに使用したユーザー名に変更できます。 次は、azureuser のアカウント名の例です。  
    ...
    このコマンドは、ディレクトリとそれ自体のディレクトリ内のすべてのファイルとディレクトリの所有権 (ユーザーとグループの両方) を変更します。  
