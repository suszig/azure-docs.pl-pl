...
SQL Server のセキュリティ運用方法についての詳細については、次を参照してください [SQL Server 2008 R2 のセキュリティ ベスト プラクティス - 運用および管理タスク。](http://download.microsoft.com/download/1/2/A/12ABE102-4427-4335-B989-5DA579A4D29D/SQL_Server_2008_R2_Security_Best_Practice_Whitepaper.docx)
...
- 名前が付いていない一意のローカル管理者アカウントを作成する **管理者**します。
...
- 既定で、Azure は SQL Server 仮想マシンのセットアップ中に Windows 認証を選択します。 したがって、 **SA** ログインが無効になっており、セットアップによって、パスワードが割り当てられます。 お勧めします **SA** ログインにする必要があります使用せず、有効にします。 次に示すのは、SQL ログインが必要な場合の代替方法です。
    - SQL アカウントを作成する **CONTROL SERVER** アクセス許可。
    - 使用する場合、 **SA** ログイン、ログインを有効にして名前を変更し、新しいパスワードを割り当てます。
...
- 上記のどちらのオプション、認証モードを変更する必要 **SQL Server と Windows 認証モード**します。 詳細については、次を参照してください。 [サーバー認証モードの変更](https://msdn.microsoft.com/library/ms188670.aspx)します。
...
- 仮想マシンが特定のネットワークからのみアクセスされる場合は、Windows ファイアウォールを使用して、特定の IP アドレスまたはネットワーク サブネットへのアクセスを制限します。 また、エンドポイントに ACL を追加して、許可されたクライアントのみにトラフィックを制限することを検討してください。 エンドポイントと Acl を使用する方法の詳細については、次を参照してください [エンドポイントの ACL の管理。](virtual-machines-set-up-endpoints.md#manage-the-acl-on-an-endpoint)
