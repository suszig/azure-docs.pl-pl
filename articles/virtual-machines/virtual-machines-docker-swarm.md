...
ここでは 4 つの VM を作成しますが、作成する VM の数は任意に決められます。 は、次を呼び出す *& lt; password & gt;* で指定したパスワードを交換してください。
...
終わったときに使用できる必要があります **azure vm の一覧** に Azure Vm を参照してください。
...
このトピックでは、 [docker のインストールのコンテナーのモデルの swarm のドキュメント](https://github.com/docker/swarm#1---docker-image) --に ssh で通信こともできますが、 **swarm マスター**します。 このモデルで **swarm** は swarm を実行する docker コンテナーとしてダウンロードします。 この手順を実行して次に、 *docker を使ってノート パソコンからリモートで* への接続に、 **swarm マスター** VM クラスター id 生成コマンドを使用するよう指示 **swarm 作成**します。 クラスター id はどのように **swarm** swarm グループのメンバーを検出します。 (リポジトリを複製して自身で構築することも可能です。そうすると完全に制御でき、デバッグもできます)。
...
最後の行がクラスター ID です。ノード VM を Swarm マスターに結合して「Swarm」を作成する際に再度使いますので、どこかに保存しておいてください）。 この例ではクラスター id は **36731c17189fd8f450c395db8437befd**します。

> [AZURE.NOTE] 明確にするのにはインストールを使用する、ローカルの docker への接続に、 **swarm マスター** Azure および命令で VM **swarm マスター** をダウンロードするには、インストール、および実行、 **作成** コマンドで、後で調査目的のために使用するクラスター id が返されます。
<!-- -->
> これを確認するには実行 `docker -H tcp://`*& lt; ホスト名 & gt;* ` images` で処理されるコンテナーを一覧表示する、 **swarm マスター** コンピューターと比較のための別のノードで (で前に swarm コマンドを実行しているため、 **--rm** 終了した後、これを使用して、スイッチ、コンテナーが削除されました **docker ps-a** も返されません).。
        ...
        $
<P />
> 慣れている場合は、 **docker**, 、他のノードがあるないエントリ イメージがダウンロードされ、まだ実行されているされるためことがわかります。

## ノード VM の Docker クラスターへの結合

各ノードで Azure CLI を使ってエンドポイント情報を一覧表示します。 次のようなこと、 **swarm ノード 1** ノードの docker ポートを取得するために docker ホストします。
...
問題ないようです。 いることを確認する **swarm** で実行している **swarm ノード 1** を入力します。

    $ docker --tls -H tcp://swarm-node-1.cloudapp.net:2376 ps -a
        CONTAINER ID        IMAGE               COMMAND                CREATED             STATUS              PORTS               NAMES
        bbf88f61300b        swarm:latest        "/swarm join --addr=   13 seconds ago      Up 12 seconds       2375/tcp            angry_mclean

クラスターのすべてのノードに対して繰り返します。 ここでは、ようなこと **swarm ノード 2** と **swarm ノード 3**します。
