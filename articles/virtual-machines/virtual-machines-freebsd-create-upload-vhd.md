...
- **Azure サブスクリプション** -があるない場合は、ほんの数分でアカウントを作成することができます。 MSDN サブスクリプションがある場合は、次を参照してください。 [MSDN サブスクライバー向けの Azure 特典](http://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/)します。 そうしないと、 [無料の試用アカウントを作成する](http://azure.microsoft.com/pricing/free-trial/)です。  

- **Azure PowerShell ツール** -Microsoft Azure PowerShell モジュールをインストールし、サブスクリプションを使用するように構成があります。 モジュールをダウンロードするを参照してください。 [Azure のダウンロード](http://azure.microsoft.com/downloads/)します。 モジュールのインストールと構成のチュートリアルはこちらで入手できます。 使用して、 [Azure のダウンロード](http://azure.microsoft.com/downloads/) コマンドレットは VHD をアップロードします。
...
6. **Azure エージェントをインストールする**
    ...
    **重要な**: インストール後に、ください念のため確認が実行されています。

        # service –e | grep waagent
        /etc/rc.d/waagent
        # cat /var/log/waagent.log

    方法があります。 これで **シャット ダウン** VM です。 手順 7. を実行してからシャットダウンすることもできますが、これは任意です。
...
4. 次のようにフィールドを指定します。
    ...
    -  **URL**, 、ストレージ アカウントの URL で使用するサブドメイン名を入力します。 文字数は 3 ～ 24 文字で、アルファベット小文字と数字を使用できます。 この名前は、対応するサブスクリプションの BLOB リソース、キュー リソース、またはテーブル リソースのアドレス指定に使用される URL のホスト名になります。
...
8. 型 a **名前** コンテナーと選択、 **アクセス** ポリシーです。
    ...
    > [AZURE.NOTE] 既定では、コンテナーはプライベートなのアカウント所有者のみがアクセスできます。 コンテナー内の BLOB にはパブリック読み取りアクセスを許可し、コンテナーのプロパティやメタデータにはアクセスを許可しない場合は、[パブリック BLOB] オプションを使用します。 コンテナーと BLOB に完全パブリック読み取りアクセスを許可するには、[パブリック コンテナー] オプションを使用します。
...
4. 次のコマンドを入力します。
   ...
   詳細については、次を参照してください [Microsoft Azure コマンドレットの概要。](http://msdn.microsoft.com/library/windowsazure/jj554332.aspx)
...
.vhd ファイルをアップロードするときは、BLOB ストレージ内であればどこにでも .vhd ファイルを置くことができます。 次のコマンド例で **BlobStorageURL** 手順 2. で作成したストレージ アカウントの URL は、 **YourImagesFolder** blob ストレージ内のコンテナーは、イメージを格納します。 **VHDName** は、バーチャル ハード_ディスクを識別するために Azure 旧ポータルに表示されるラベルです。 **PathToVHDFile** は完全なパスと、.vhd ファイルの名前。
...
1. 前の手順で使用した Azure PowerShell ウィンドウで、次のように入力します。
    ...
    **重要な**: OS として Linux を使用してください。 ここでは現在の Azure PowerShell バージョンでは"Linux"または"Windows"をパラメーターとしてのみからの型。

2. 前の手順を完了すると、新しいイメージが一覧に表示を選択すると、 **イメージ** Azure クラシック ポータルでタブをクリックします。  

    ![イメージの追加](./media/virtual-machines-freebsd-create-upload-vhd/addfreebsdimage.png)

3. ギャラリーから仮想マシンを作成します。 この新しいイメージは、「 **マイ イメージ**します。 新しいイメージを選択し、表示される画面でホスト名、パスワード、SSH キーなどを設定します。
