...
5.  **[構成データベースの設定]** ページで、次の操作を行います。
 ...
 - **ユーザー名**, 、[ドメイン] を入力**\sp_farm_db** (で作成した [フェーズ 2: ドメイン コント ローラー構成](virtual-machines-workload-intranet-sharepoint-phase2.md))します。 sp_farm_db アカウントには、データベース サーバーに対する sysadmin 権限があります。
