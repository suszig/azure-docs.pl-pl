...
- [Azure の CentOS 6.x 仮想マシンを準備します。](#centos6)
- [Azure の CentOS 7.0 以上の仮想マシンを準備します。](#centos7)
...
12. ファイルを編集して、yum モジュール"fastestmirror"を無効にする"/etc/yum/pluginconf.d/fastestmirror.conf"、および [ `[メイン]` 次の入力
...
10. GRUB 構成でカーネルのブート行を変更して Azure の追加のカーネル パラメーターを含めます。 これを行うに開いている「//etc/default/grub」のテキスト エディターで編集、 `GRUB_CMDLINE_LINUX` 例については、パラメーター。
