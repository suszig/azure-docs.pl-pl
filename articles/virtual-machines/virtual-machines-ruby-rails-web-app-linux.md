...
1. サインイン、 [Azure クラシック ポータル](http://manage.windowsazure.com)
2. クリックして **新しい** > **コンピューティング** > **仮想マシン** > **簡易作成**します。 Linux イメージを選択します。
3. パスワードを入力します。

VM がプロビジョニングされた後、VM 名をクリックし、クリックして **ダッシュ ボード**します。 下に一覧表示、SSH エンドポイント見つけます **SSH の詳細**します。

### Azure CLI

手順に従います [を実行している Linux の仮想マシンを作成する][vm-instructions]です。
...
1. 移動して、 [Azure クラシック ポータル][management-portal] し、VM を選択します。

    ![仮想マシンの一覧][vmlist]

2. 選択 **エンドポイント** クリックして、ページの上部にある **+ エンドポイントの追加** ページの下部にあります。

    ![エンドポイント ページ][endpoints]

3.  **エンドポイントの追加** ダイアログで [「[スタンドアロン エンドポイントの追加」およびをクリックして、 **次** 矢印です。
...
5. メッセージが表示されることを示す必要があります **進行中の更新プログラム**します。 このメッセージが消えると、エンドポイントがアクティブになります。 この時点で仮想マシンの DNS 名に移動すると、アプリケーションをテストできます。 Web サイトは次のようになります。
