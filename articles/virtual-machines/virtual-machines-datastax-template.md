...
内容は、Azure Marketplace で既に使用可能なだけでなくようになりましたことも簡単にデプロイする Ubuntu Vm 上に新しい DataStax クラスターを使用して展開リソース マネージャー テンプレートを使用して [Azure PowerShell](../powershell-install-configure.md) または [Azure CLI](../xplat-cli-install.md)します。
...
デプロイが完了すると、構成済みの DNS アドレスを使用して DataStax オペレーション センターの VM インスタンスにアクセスできます。 OpsCenter VM には、有効な SSH ポート 22 と、HTTPS 用のポート 8443 があります。 オペレーション センターでは、DNS アドレスに *dnsName* と *地域* 形式でパラメーターとして入力される `{dnsName}.{region}.cloudapp.azure.com`します。 使用した展開を作成した場合、 *dnsName* "West US"リージョンの"datastax"に設定するパラメーター、デプロイメントの DataStax オペレーション センターの VM にアクセスする可能性があります `https://datastax.westus.cloudapp.azure.com:8443`します。

> [AZURE.NOTE] 展開で使用される証明書は、ブラウザー警告を作成する自己署名証明書です。 処理を行うことができる、 [DataStax](http://www.datastax.com/) を独自の SSL 証明書、証明書を置換するための web サイトです。
...
複製を完了すると、検索、 **ubuntu 上の datastax** C:\Azure\Templates ディレクトリ内のフォルダーです。
...
- 棒グラフ [イベント] をクリックして、 **監視** 一部リソース グループ] ブレードは、デプロイのイベントを表示できます。
...
この例を詳しく調べると、2 つの異なる手法を確認できます。 この最初のフラグメントで、 **osSettings** 変数は入れ子になった JSON 要素であり、4 つのキー/値ペアが含まれています。
    ...
     
この 2 つ目のフラグメントで、 **スクリプト** 変数は JSON 配列であり、各要素がテンプレート言語関数 (concat) との別の変数に文字列定数を加えた値を介して実行時に計算されます。
...
"resources" セクションではアクションの大部分が発生します。 このセクションを注意深く調べると、2 つの異なるケースがあることがすぐにわかります。 1 つ目は、型 `Microsoft.Resources/deployments`の定義されている要素です。基本的に、メインのデプロイ内に入れ子になったデプロイの呼び出しを意味します。 を介して、 **templateLink** 要素 (および関連するバージョン プロパティ)、このフラグメントに示すように、入力として一連のパラメーターを渡すことによって呼び出されるリンクされたテンプレート ファイルを指定することは。
...
-   **opscenter-resources.json**: OpsCenter VM とネットワーク インターフェイスやパブリック IP アドレスなど、関連するすべてのリソースをデプロイします。
-   **opscenter-resources.json インストール**: VM 内での OpsCenter サービスのセットアップに必要な bash スクリプト ファイル (opscenter.sh) を呼び出す OpsCenter VM 拡張機能 (Linux 用のカスタム スクリプト) を展開します。
-   **一時的なノードの resources.json**: すべてのクラスター ノードの Vm と接続されているリソースをデプロイ (ネットワーク カード、プライベート Ip などです。)。 このテンプレートは、VM 拡張機能 (Linux 用のカスタム スクリプト) も同様にデプロイし、bash スクリプト (dsenode.sh) を呼び出して各ノードに Apache Cassandra の構成要素を物理的にインストールします。

この最後のテンプレートは、テンプレート開発の観点から見て最も興味深いテンプレートの 1 つです。このテンプレートの使用方法について、詳しく見てみましょう。 注意が必要な重要な概念は、どのようにすれば 1 つのテンプレート ファイルによって単一の種類のリソースの複数のコピーをデプロイできるか、また、各インスタンスが必要な設定に対して一意の値を設定できるかということです。 この概念と呼ばれる **リソース ループ**します。

メインの azuredeploy.json ファイル内で一時的なノードの resources.json がから呼び出されると、パラメーターと呼ばれる *nodeCount* がパラメーター リストの一部として提供されます。 子テンプレート内で *nodeCount* 内 (クラスターにデプロイするノードの数) で使用される、 **コピー** の下のフラグメントで表されているように、複数のコピーにデプロイする必要のある各リソースの要素。 デプロイ済みのリソースの異なるインスタンスの一意の値が必要なすべての設定に対して、 **copyindex()** 関数を使用して、特定のリソース ループ作成という点では、現在のインデックスを示す数値を取得します。 次のフラグメントでは、DataStax クラスター ノードに対して作成される複数 VM に、この概念が適用されていることがわかります。
...
わかるように、リソース作成におけるもう 1 つの重要な概念は依存関係と、リソース間の優先指定する機能、 **dependsOn** JSON 配列。 このテンプレートでは、各ノードに Apache Cassandra インスタンスのバックアップとスナップショットをホストするために使用できる 1 TB のディスク ("dataDisks" を参照) も接続されます。

接続されたディスクは、dsenode.sh スクリプト ファイルの実行によってトリガーされるノード準備アクティビティの一部としてフォーマットされます。 そのスクリプトの最初の行は、別のスクリプトを呼び出しています。

    bash vm-disk-utils-0.1.sh

Vm ディスク ユーティリティ 0.1.sh ファイルの一部では、 **shared_scripts** azure クイック スタート テンプレート GitHub のリポジトリ内のフォルダーとディスクのマウント、書式設定、ストライピングのための便利な機能が含まれています。 これらの機能は、リポジトリ内のすべてのテンプレートで使用できます。
