...
- 考えられる移行方法。
- 完全エンド ツー エンド例 Azure、Windows、および SQL Server の移行、既存の AlwaysOn 実装の手順を実行します。
...
Premium Storage を使用するには、DS シリーズ仮想マシン (VM) を使用する必要があります。 前に、クラウド サービスに DS シリーズ マシンを使用しない場合、既存の VM を削除、アタッチされたディスクを保持、DS * ロール サイズとして VM を再作成する前に、新しいクラウド サービスを作成してください。 仮想マシンのサイズの詳細については、次を参照してください。 [仮想マシンと Azure のクラウド サービスのサイズ](virtual-machines-size-specs.md)します。
...
Premium Storage アカウントの一部であるディスクの作成の主な違いは、ディスク キャッシュの設定です。 SQL Server データ ボリューム ディスクの場合に使用することを推奨は、'**読み取りキャッシュ**' です。 トランザクション ログの量がディスク キャッシュの設定に設定 '**None**' です。 これは、Standard Storage アカウントに対する推奨事項とは異なります。
...
> [AZURE.NOTE] 既存のデプロイ先を参照してください、 [の前提条件](#prerequisites-for-premium-storage) このトピックのセクションです。
...
- 1 対 1 の移行です。
- 複雑さが軽減されます。
- Premium Storage SKU による IOPS の増加に対応します。 ディスクを VM からデタッチして新しいクラウド サービスにコピーするとき、サード パーティのツールを使用して VHD のサイズを増やし、スループットを上げることができます。 VHD のサイズを増やすと、情報を参照して [フォーラム ディスカッション](https://social.msdn.microsoft.com/Forums/azure/4a9bcc9e-e5bf-4125-9994-7c154c9b0d52/resizing-azure-data-disk?forum=WAVirtualMachinesforWindows)します。
...
ハイブリッド AlwaysOn 構成の次のような例について考えます。
...
AlwaysOn は、インストールされるときに Windows クラスター リソース グループを作成します。フェールオーバー クラスター マネージャーを開くと、少なくとも 3 つのリソースがあることがわかり、次の 2 つがドキュメントで参照されています。
...
[IP アドレスを追加するを参照してください。、 [付録](#appendix-migrating-a-multisite-alwayson-cluster-to-premium-storage), 、手順 14 です。

1. 現在の IP アドレス リソースについて、実行可能な所有者を「既存のプライマリ SQL Server」に変更します。下の例では "dansqlams4" です。

    ![Appendix13][23]

1. 新しい IP アドレス リソースの '移行完了セカンダリ SQL server' に次の例"dansqlams5 です"実行可能な所有者を変更します。
