...
1.  プライマリ データベース サーバーとしてのログオン **[domain] \sqladmin**します。 
2.  F:\disk に移動します。 
3.  右クリックし、 **バックアップ** フォルダーとクリック **と共有** ] をクリック **特定のユーザー**します。
...
Sqlservice アカウントに与えます点を除き、セカンダリ データベース サーバーで上記の手順を実行 **読み取り** 手順 5 で [F:\Backup] フォルダーのアクセスを許可します。
...
1.  プライマリ データベース サーバーのスタート画面で「 **SQL Studio**, 、クリックして **SQL Server Management Studio**します。
2.  クリックして **接続**します。
...
6.  クリックして **追加**します。  **ファイル名**, 、型 **\\[machineName]\backup\[databaseName].bak**, ここで、 **machineName** プライマリ SQL の名前を指定 **サーバー コンピューター** と **databaseName** 、データベースの名前を指定します。 をクリックして **[ok]**, 、順にクリック **OK** バックアップの成功に関するメッセージの後にもう一度です。
...
5.   **ソース** ] セクションの [ **デバイス**, 、省略記号 (...) ボタンをクリック
6.   **バックアップ デバイスの選択**, 、クリックして **追加**します。
7.   **バックアップ ファイルの場所**, 、型 **\\[machineName]\backup**, 、キーを押して **Enter**, を選択 **[databaseName] .bak**, 、順にクリック **[ok]** 2 回クリックします。 確認、完全バックアップとログ バックアップ、 **復元するバックアップ セット** セクションです。
...
5.   **データベースの選択** ] ページで、バックアップされたアプリケーションのデータベースを選択してクリックして **次**します。 対象とするプライマリ レプリカで完全バックアップを少なくとも 1 つは作成しているので、これらのデータベースは可用性グループの前提条件を満たしています。
6.   **レプリカの指定** ] ページで [ **のレプリカ追加**します。
7.   **サーバーへの接続**, をセカンダリ データベース サーバーの名前を入力して、クリックして **接続**します。 
8.   **レプリカの指定** セカンダリ データベース サーバーが表示されているページで、 **可用性レプリカ**します。 両方のインスタンスについて、次のオプション値を設定します。 
