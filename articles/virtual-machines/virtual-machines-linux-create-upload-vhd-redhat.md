...
9.  WALinuxAgent パッケージ `WALinuxAgent - < バージョン >` は Fedora EPEL 6 リポジトリにプッシュされました。次のコマンドを実行して epel リポジトリを有効にします。

        # wget http://download.fedoraproject.org/pub/epel/6/x86_64/epel-release-6-8.noarch.rpm
        # rpm -ivh epel-release-6-8.noarch.rpm

10. GRUB 構成でカーネルのブート行を変更して Azure の追加のカーネル パラメーターを含めます。 これを行う `/boot/grub/menu.lst` テキスト エディターでし、既定のカーネルに次のパラメーターが含まれていることを確認します。
...
8.  完了した時点編集 `/etc/default/grub` あたり、次のコマンドを実行 grub 構成をリビルドします。

        # sudo grub2-mkconfig -o /boot/grub2/grub.cfg

9.  SSH サーバーがインストールされており、起動時に開始するように構成されていることを確認します。 通常これが既定です。 変更、 `/etc/ssh/sshd_config` を次の行を含めます。

        ClientAliveInterval 180

10. WALinuxAgent パッケージ `WALinuxAgent - < バージョン >` は Fedora EPEL 6 リポジトリにプッシュされました。次のコマンドを実行して epel リポジトリを有効にします。
...
12. OS ディスクにスワップ領域を作成しないでください。 Azure Linux エージェントは、Azure でプロビジョニングされた後に VM に接続されたローカルのリソース ディスクを使用してスワップ領域を自動的に構成します。 ローカル リソース ディスクは一時ディスクであるため、VM のプロビジョニングが解除されると空になることに注意してください。 Azure Linux エージェントのインストール後に (前の手順を参照)、`/etc/waagent.conf` にある次のパラメーターを適切に変更します。
...
9.  GRUB 構成でカーネルのブート行を変更して Azure の追加のカーネル パラメーターを含めます。 これを行う `/boot/grub/menu.lst` テキスト エディターでし、既定のカーネルに次のパラメーターが含まれていることを確認します。
...
12. WALinuxAgent パッケージ `WALinuxAgent - < バージョン >` は Fedora EPEL 6 リポジトリにプッシュされました。次のコマンドを実行して epel リポジトリを有効にします。
...
9.  完了した時点編集 `/etc/default/grub` あたり、次のコマンドを実行 grub 構成をリビルドします。
...
14. OS ディスクにスワップ領域を作成しないでください。 Azure Linux エージェントは、Azure でプロビジョニングされた後に VM に接続されたローカルのリソース ディスクを使用してスワップ領域を自動的に構成します。 ローカル リソース ディスクは一時ディスクであるため、VM のプロビジョニングが解除されると空になることに注意してください。 Azure Linux エージェントのインストール後に (前の手順を参照)、`/etc/waagent.conf` にある次のパラメーターを適切に変更します。
...
7.  WALinuxAgent パッケージ `WALinuxAgent - < バージョン >` は Fedora EPEL 6 リポジトリにプッシュされました。次のコマンドを実行して epel リポジトリを有効にします。
...
6.  完了した時点編集 `/etc/default/grub` あたり、次のコマンドを実行 grub 構成をリビルドします。

         # sudo grub2-mkconfig -o /boot/grub2/grub.cfg

7.  Hyper-V モジュールを initramfs に追加します。

    編集 `/etc/dracut.conf`, 、コンテンツを追加します。
...
8.  SSH サーバーがインストールされており、起動時に開始するように構成されていることを確認します。 通常これが既定です。 変更、 `/etc/ssh/sshd_config` を次の行を含めます。
...
11. OS ディスクにスワップ領域を作成しないでください。 Azure Linux エージェントは、Azure でプロビジョニングされた後に VM に接続されたローカルのリソース ディスクを使用してスワップ領域を自動的に構成します。 ローカル リソース ディスクは一時ディスクであるため、VM のプロビジョニングが解除されると空になることに注意してください。 Azure Linux エージェントのインストール後に (前の手順を参照)、`/etc/waagent.conf` にある次のパラメーターを適切に変更します。
...
6.  Enter `inst.ks= < 完備ファイルの場所 >` ブート オプションとそのキーを押しての最後に **Enter**します。
...
[AZURE です。Red Hat で注意してください] この既知の問題がアドレス指定された既に。 関連付けられている修正プログラムをインストールするには、次のコマンドを実行します。
