...
このテンプレートで、Ubuntu 仮想マシンに Spark クラスターがデプロイされます。 このテンプレートでは、インストールに必要なストレージ アカウント、仮想ネットワーク、可用性セット、パブリック IP アドレス、およびネットワーク インターフェイスも準備されます。 Spark クラスターはサブネットの背後に作成されるので、パブリック IP で Spark クラスターにアクセスすることはできません。  デプロイの一部として、オプションの “jump box” をデプロイできます。 この"jump box"は、デプロイされる Ubuntu VM サブネットにもその *は* に接続できる開かれた SSH ポートでパブリック IP アドレスを公開します。 “jump box” からは、サブネット内にあるすべての Spark VM に SSH で接続できます。

このテンプレートでは、「小」、「中」、「大」の Spark クラスター セットアップを指定するために、「T シャツのサイズ」の概念を使用します。  テンプレートの言語がさらに動的なテンプレートのサイズ指定をサポートするようになったら、この概念は Spark クラスターのマスター ノード数、スレーブ ノード数、VM サイズなどの指定に変更される可能性があります。ここでは、VM のサイズおよびマスターとスレーブの変数、azuredeploy.json で定義されている数を表示できる **tshirtSizeS**, 、**tshirtSizeM**, 、および **tshirtSizeL**:
...
複製を完了すると、検索、 **ubuntu 上の spark** C:\Azure\Templates ディレクトリ内のフォルダーです。
...
次に、azuredeploy-parameters.json ファイルのパラメーター セットの例を示します。 パラメーターの有効な値を指定する必要 **storageAccountName**, 、**adminUsername**, 、および **adminPassword**, 、他のパラメーターのすべてのカスタマイズ。
...
```
> [AZURE.NOTE] **$RGName** 、サブスクリプション内で一意である必要があります。

実行すると、 **New-azureresourcegroupdeployment** コマンドを JSON parameters ファイルからパラメーター値が抽出され、テンプレートを適宜実行が開始されます。 さまざまな環境 (テスト環境、運用環境など) によって複数のパラメーター ファイルを定義し、使用することは、テンプレートの再利用を推進し、複雑な多環境ソリューションを単純化します。
...
- クリックして **参照** 左側のナビゲーション バーと、下にスクロールしてクリックして **リソース グループ**します。
- 作成したリソース グループをクリックして、[リソース グループ] ブレードを表示します。
- クリックして、 **イベント** でを示す棒グラフ、 **監視** 一部リソース グループ] ブレードは、デプロイのイベントを表示できます。
...
> [AZURE.NOTE] 注意して **defaultValue** 指定するだけでなく **allowedValues**します。
 ...
 **VmStorageAccountContainerName** 変数は、単純な名前/値変数の例を示します。  **vnetID** 関数を使用して実行時に計算される変数の例は、 **resourceId** と **パラメーター**します。  値、 **numberOfMasterInstances** と **vmSize** 変数が使用して実行時に計算される、 **concat**, 、**変数**, 、および **パラメーター** 関数です。  

Spark クラスターのデプロイメントのサイズをカスタマイズするかどうかは、変数のプロパティを変更する **tshirtSizeS**, 、**tshirtSizeM**, 、および **tshirtSizeL** azuredeploy.json テンプレート内です。  

MSDN の「テンプレートの言語に関する詳細情報が見つかりません [Azure リソース マネージャー テンプレートの言語](../resource-group-authoring-templates.md)です。


### "resources" セクション

"resources" セクションではアクションの大部分が発生します。 このセクションを注意深く調べると、2 つの異なるケースがあることがすぐにわかります。 1 つ目は、基本的にはメインのデプロイに入れ子になったデプロイを呼び出す `Microsoft.Resources/deployments` 型で定義された要素です。 2 番目は、 **templateLink** プロパティ (および関連する **contentVersion** プロパティ) 呼び出される、一連のパラメーターを入力として渡し、リンクされたテンプレート ファイルを指定できるようにします。 次のテンプレート フラグメントはこれらを示したものです。
...
-   **shared-resource.json**: デプロイメント全体で共有されるすべてのリソースの定義が含まれています。 たとえば、VM の OS ディスクおよび仮想ネットワークの格納に使用されるストレージ アカウントです。
-   **jumpbox リソース-enabled.json**:"jump box"VM およびネットワーク インターフェイス、パブリック IP アドレス、入力エンドポイントなどのすべての関連リソースを環境に SSH を使用します。
...
詳しく見てみましょう *方法* この最後のテンプレートは、azuredeploy.json、使用、テンプレート開発の観点から最も興味深いの 1 つです。 注意が必要な重要な概念は、どのようにすれば 1 つのテンプレート ファイルによって単一の種類のリソースの複数のコピーをデプロイできるか、また、各インスタンスが必要な設定に対して一意の値を設定できるかということです。 この概念と呼ばれる **リソース ループ**します。

使用するリソース、 **コピー** 要素を「コピー」自体で指定された回数、 **カウント** のパラメーター、 **コピー** 要素。  デプロイ済みリソースの異なるインスタンス間で一意の値を指定する必要があるすべての設定に対して、 **copyindex()** 関数を使用して、特定のリソース ループ作成という点では、現在のインデックスを示す数値を取得します。 azuredeploy.json の次のフラグメントでは、この概念が複数のネットワーク アダプター、VM、および Spark クラスター用に作成された VM 拡張機能に適用されていることを確認できます。
...
わかるように、リソース作成におけるもう 1 つの重要な概念は依存関係と、リソース間の優先指定する機能、 **dependsOn** JSON 配列。 このテンプレートでは、Spark クラスター ノードが共有リソースに依存していることを確認でき、 **networkInterfaces** 最初に作成されるリソースです。

もう 1 つの興味深いフラグメントは、1 つに関連する **CustomScriptForLinux** VM 拡張機能です。 これらは、各クラスター ノードに依存関係を持つ別の種類のリソースとしてインストールされます。  この場合、これは各 VM ノードに Spark をインストールしてセットアップするために使われます。  これらを使用する azuredeploy.json テンプレートのスニペットを見てみましょう。
...
マスターとスレーブ ノードのリソース拡張機能がで定義されている、別のコマンドを実行することに注意してください、 **commandToExecute** 、プロビジョニング プロセスの一部としてのプロパティです。  

最新の仮想マシンの拡張機能の JSON スニペットを見ると、このリソースが、仮想マシン リソースとそのネットワーク インターフェイスに依存していることがわかります。 これは、この VM の拡張機能をプロビジョニングし、実行する前に、これらの 2 つのリソースをデプロイしておく必要があることを示しています。 使用にも注意してください、 **copyindex()** 関数を各スレーブ仮想マシンに対してこの手順を繰り返します。
