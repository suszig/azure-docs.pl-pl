...
別の興味深いフラグメントは、1 つに関連する **CustomScriptForLinux** VM 拡張機能です。 これらは、各クラスター ノードに依存関係を持つ別の種類のリソースとしてインストールされます。  この場合、これは各 VM ノードに Spark をインストールしてセットアップするために使われます。  これらを使用する azuredeploy.json テンプレートのスニペットを見てみましょう。
