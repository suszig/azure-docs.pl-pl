...
> [AZURE.NOTE] この定義済みのアーキテクチャは、わかりやすくするための単一のサブネットを使用します。 一連のサブネットの分離をエミュレートするようにトラフィックのフィルターをオーバーレイする場合は、Azure を使用することができます [ネットワーク セキュリティ グループ](virtual-networks-nsg.md)します。
