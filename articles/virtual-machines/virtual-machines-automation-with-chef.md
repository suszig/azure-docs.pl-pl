...
開始する前に Chef の基本的な概念を確認しておくことをお勧めします。 優れた記事が <a href="http://www.chef.io/chef" target="_blank">ここ</a> このチュートリアルを行う先に読んでがあることをお勧めします。 基礎については開始する前に要約します。
...
Chef ワークステーションは管理者用のワークステーションで、ポリシーを作成したり、管理コマンドを実行します。 実行して、 **knife** インフラストラクチャを管理、Chef ワークステーションからコマンドです。
...
ダウンロード、発行の設定から [ここ](https://manage.windowsazure.com/publishsettings/)します。

発行設定ファイルを C:\chef に保存します。

##管理された Chef アカウントの作成

ホストされた Chef アカウントにサインアップ [ここ](https://manage.chef.io/signup)します。
...
> [AZURE.NOTE] キーがリセットされることを警告するプロンプトが表示された場合は、既存のインフラストラクチャがまだ構成があるしないまま進んでかまいません。
...
Chef-starter\chef-repo\.chef 下にあるすべてのファイルを c:\chef ディレクトリにコピーします。
...
次 [をダウンロードしてインストール](http://downloads.getchef.com/chef-dk/windows) ChefDK (Chef 開発キット) Chef Workstation を設定します。
...
*パスの順序は重要な注意してください。*opscode パスの順序が正しくない場合は、問題が生じます。
...
> [AZURE.NOTE] – Pre 引数は、最新の Api のセットへのアクセスを提供する Knife Azure プラグインの最新 RC バージョンを受け取っているを確認します。
...
Cookbookは管理するクライアント上で実行する一連のコマンドを定義するために Chef で使用します。 Cookbook の作成は簡単ですし、使用して、 **chef クックブックを生成する** Cookbook のテンプレートを生成するコマンドです。 ポリシーで IIS を自動的にデプロイさせるため、Cookbook webserver を呼び出します。
...
このステップでは、ローカル コンピューターに作成した Cookbook のコピーを、Chef がホストするサーバーにアップロードします。 クックブックが表示され、アップロードされた後、 **ポリシー** ] タブをクリックします。
...
これを行うために使用して、 **knife azure server 作成** コマンドです。
...
> [AZURE.NOTE] コマンド ラインでも自動化エンドポイント ネットワーク フィルター ルール – tcp エンドポイントのパラメーターを使用しています。 ここではポート 80 と 3389 を開き、Web ページと RDP セッションにアクセスできるようにしています。
...
お役に立てれば幸いです。 今すぐ Azure でコードを工夫としてのインフラストラクチャを始めてください。
