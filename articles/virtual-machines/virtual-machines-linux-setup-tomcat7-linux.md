...
3.  クリックして **生成** キーを生成します。 処理中にウィンドウの空白の領域にマウスを移動すると、ランダム性を高めることができます。  
![][1]
4.  生成処理が終わると、生成されたキーが表示されます。 次に例を示します。  
![][2]
5.  選択し、その公開キーのコピー **キー** し、publicKey.pem という名前のファイルに保存します。 クリックしてしないでください **公開キーの保存**, 、保存された公開キーのファイル形式がする公開キーと異なるためです。
6.  クリックして **秘密キーの保存** し、privateKey.ppk という名前のファイルに保存します。

###手順 2. Azure ポータルでイメージを作成する
 [Azure ポータル](https://portal.azure.com/), 、] をクリックして **新規** イメージを作成するタスク バーのニーズに基づく Linux イメージを選択します。 次の例では、Ubuntu 14.04 イメージを使用します。
![][3]

 **ホスト名** ユーザーとインターネット クライアントがこの仮想マシンへのアクセスに使用する URL の名前を指定します。 DNS 名の最後の部分 (tomcatdemo など) を定義すると、Azure は URL を tomcatdemo.cloudapp.net として生成します。  
...
1.  Azure ポータルでをクリックして **参照** ]-> [ **仮想マシン**, 、作成した仮想マシンをクリックします。  
![][5]
2.  仮想マシンにエンドポイントを追加するには、クリックして、 **エンドポイント** ボックス。
![][6]
3.  クリックして **追加**します。  
    1.   **エンドポイント**, 、エンドポイントにエンドポイントの名前を入力しに 80 と入力し、 **パブリック ポート**します。  

        80 に設定している場合は、tomcat に接続するための URL にポート番号を含める必要はありません。 たとえば、http://tomcatdemo.cloudapp.net のようになります。    

        81 などの別の値に設定した場合は、tomcat にアクセスするための URL にポート番号を追加する必要があります。 たとえば、http://tomcatdemo.cloudapp.net:81//です。
...
4.  をクリックして **OK** 仮想マシンにエンドポイントを追加します。
...
最初に、Azure ポータルから仮想マシンの DNS 名を取得します。 **参照] をクリックして** ]-> [ **仮想マシン** 仮想マシンの名前]->-> [ **プロパティ**, を検索し、 **ドメイン名** のフィールド、 **プロパティ** を並べて表示します。  
...
左のウィンドウで **接続** ]-> [ **SSH** ]-> [ **Auth** 順にクリック **参照** の場所を指定する、 **privateKey.ppk** ファイルで、フェーズ 1 で puttygen によって生成された秘密キーが含まれています: イメージを作成します。 たとえば次のようになります。  
![][10]
...
使用して、 **sudo apt キャッシュ search tomcat7** コマンドを使用可能なすべてのコンポーネントを参照してください。 次のコマンドは、役立ついくつかのパーツをインストールする例です。  
...
ブラウザーを開き、URL を入力 **http://<your tomcat server DNS name>/マネージャー/html**します。 たとえば、この記事では、URL は http://tomcatexample.cloudapp.net/manager/html. です。  
