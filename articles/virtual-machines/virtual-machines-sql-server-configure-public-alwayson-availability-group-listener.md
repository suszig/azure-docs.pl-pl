...
この記事では、**外部負荷分散**を使用するリスナーの作成を中心に説明します。 仮想ネットワークにプライベートのリスナーを設定する場合は、を設定するための手順は、この記事のバージョンを参照してください [リスナーと ILB](virtual-machines-sql-server-configure-ilb-alwayson-availability-group-listener.md)。
