<properties
    pageTitle="Wprowadzenie do programu SQL Server w usłudze Azure Virtual Machines | Microsoft Azure"
    description="Przenieś obciążenia lokalnej bazy danych SQL Server do chmury za pomocą usługi Azure Virtual Machines. Szybkie rozpoczynanie pracy dzięki wstępnie skonfigurowanym obrazom maszyn wirtualnych z programem SQL."
...
# Wprowadzenie do programu SQL Server w usłudze Azure Virtual Machines

W tym temacie opisano opcje uruchamiania programu SQL Server na maszynach wirtualnych Azure. Temat zawiera również wskazówki i zasoby, które ułatwią rozpoczęcie pracy.
...
> [AZURE.NOTE] Podczas oceniania programu SQL Server na maszynach wirtualnych Azure zapoznaj się również z innymi opcjami magazynu i rozwiązań SQL dostępnymi na platformie Azure, takimi jak [SQL Database](../sql-database/sql-database-technical-overview.md), [SQL Data Warehouse](../sql-data-warehouse/sql-data-warehouse-overview-what-is.md) i [baza danych SQL Server Stretch](../sql     -server-stretch-database/sql-server-stretch-database-overview.md). Aby uzyskać jedno szczegółowe porównanie, zobacz [Choose a cloud SQL Server option: Azure SQL (PaaS) Database or SQL Server on Azure VMs (IaaS)](../sql-database/data-management-azure-sql-database-and-sql-server-iaas.md) (Wybór opcji programu SQL Server w chmurze: Baza danych SQL Azure [PaaS] lub program SQL Server na maszynach wirtualnych Azure [IaaS]).
...
Wskazówki krok po kroku są dostępne w samouczku [Aprowizowanie maszyny wirtualnej programu SQL Server w Portalu Azure](virtual-machines-windows-portal-sql-server-provision.md). Sprawdź również temat [Performance best practices for SQL Server VMs](virtual-machines-windows-sql-performance.md) (Najlepsze rozwiązania umożliwiające zwiększenie wydajności maszyn wirtualnych z programem SQL Server), który wyjaśnia, w jaki sposób wybrać odpowiedni rozmiar maszyny i inne funkcje dostępne podczas aprowizacji.

|Wersja programu SQL Server|System operacyjny|Wydanie programu SQL Server|
|---|---|---|
...
Przechowuj kopie zapasowe bazy danych bezpośrednio na platformie Azure przy użyciu Magazynu obiektów Blob. Aby uzyskać więcej informacji, zobacz [Use Azure Storage for SQL Server Backup and Restore](../sql-database/storage-use-storage-sql-server-backup-restore.md) (Używanie magazynu Azure do tworzenia kopii zapasowych programu SQL Server i ich przywracania). Choć ta funkcja świetnie się sprawdza w przypadku maszyn wirtualnych z programem SQL, działa także dla lokalnych baz danych programu SQL Server. Omówienie opcji tworzenia i przywracania kopii zapasowych znajduje się w temacie [Backup and Restore for SQL Server in Azure Virtual Machines](virtual-machines-windows-sql-backup-recovery.md) (Tworzenie i przywracanie kopii zapasowych programu SQL Server w usłudze Azure Virtual Machines).

Maszyny wirtualne Azure mogą również korzystać z funkcji [Automatyczna usługa Backup](virtual-machines-windows-sql-automated-backup.md) i [Automatyczne stosowanie poprawek](virtual-machines-windows-sql-automated-patching.md) dla programu SQL Server.
