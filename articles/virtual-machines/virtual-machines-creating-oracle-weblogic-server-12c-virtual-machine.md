...
12. インターネットで実行されているショッピングカート アプリケーションを表示するには、ブラウザーを開いて、url の形式で `http://<<unique_domain_name>>/shoppingcart`します。(値を指定することができます <<*unique_domain_name*>> 内で、 [Azure ポータル](https://ms.portal.azure.com/) をクリックして **仮想マシン** し、Oracle WebLogic Server を実行している仮想マシンを選択) します。
