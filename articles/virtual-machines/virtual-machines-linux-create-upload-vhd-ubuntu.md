...
6. Grub のカーネルのブート行を変更して Azure の追加のカーネル パラメーターを含めます。 これを行うに開いている「//etc/default/grub」という名前の変数を検索、テキスト エディターで `GRUB_CMDLINE_LINUX_DEFAULT` (または、必要であれば追加) し、次のパラメーターを追加するための編集します。

        GRUB_CMDLINE_LINUX_DEFAULT="console=tty1 console=ttyS0 earlyprintk=ttyS0 rootdelay=300"

    保存し、このファイルを閉じると、実行 '`sudo 更新 grub`' です。 これにより、すべてのコンソール メッセージが最初のシリアル ポートに送信され、メッセージを Azure での問題のデバッグに利用できるようになります。
...
9.  Azure Linux エージェントをインストールします。
    ...
    インストールすることに注意してください、 `walinuxagent` パッケージが削除、 `NetworkManager` と `networkmanager-gnome` インストールされている場合に、パッケージ化します。
