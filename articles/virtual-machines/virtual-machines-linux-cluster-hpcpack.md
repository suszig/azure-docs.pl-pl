...
![Linux ノードと共に HPC クラスター][シナリオ]

## Linux コンピューティング ノードと共に HPC Pack クラスターをデプロイします。

Microsoft HPC Pack IaaS デプロイメント スクリプトを使う (**New-hpciaascluster.ps1**) を Azure インフラストラクチャ サービス (IaaS) でクラスター デプロイメントを自動化します。 この Azure PowerShell スクリプトは迅速なプロイメントのために Azure Marketplace の HPC Pack VM イメージを使用します。包括的な構成パラメーターを備え、簡単かつ柔軟なデプロイを可能にします。 スクリプトは、Azure の仮想ネットワークを展開、ストレージ アカウント、クラウド サービス、ドメイン コント ローラー、任意の個別の SQL Server データベース サーバー、クラスターのヘッド ノード、計算ノード、ブローカー ノード、Azure PaaS (「バースト」) ノード、および Linux 計算ノード (で導入された Linux サポート [HPC Pack 2012 R2 Update 2](https://technet.microsoft.com/library/mt269417.aspx))。

HPC Pack クラスター デプロイメント オプションの詳細については、「、 [HPC Pack 2012 R2 と HPC Pack 2012 ため方法の概要ガイド](https://technet.microsoft.com/library/jj884144.aspx) と [の Azure with Microsoft HPC Pack クラスターのオプションを作成し、高パフォーマンス コンピューティング (HPC) の管理](virtual-machines-hpcpack-cluster-options.md)します。

### 前提条件

* **クライアント コンピューター** -クラスター デプロイメント スクリプトを実行する Windows ベースのクライアント コンピューターが必要です。

* **Azure PowerShell** - [をインストールし、Azure PowerShell を構成](../powershell-install-configure.md) (バージョン 0.8.10 以降)、クライアント コンピューターにします。

* **HPC Pack IaaS デプロイメント スクリプト** - をダウンロードしてから、スクリプトの最新バージョンを開梱、 [Microsoft ダウンロード センター](https://www.microsoft.com/download/details.aspx?id=44949)します。 `New-HPCIaaSCluster.ps1 –Version` を実行すると、スクリプトのバージョンを確認できます。 この記事はバージョン 4.4.0 以降のスクリプトに基づきます。

* **Azure サブスクリプション** -Azure Global または Azure China service でサブスクリプションを使用することができます。 アカウントがない場合は、無料試用版のアカウントを数分で作成することができます。 詳細については、[Azure の無料試用版サイト](http://azure.microsoft.com/pricing/free-trial/)をご覧ください。

* **コア クォータ** 、特に、マルチコア VM サイズでいくつかのクラスター ノードを展開する場合、コアのクォータを増やす必要があります。 この記事の例では、使用可能なコアが少なくとも 12 個必要です。 クォータを増やすには [オンライン カスタマー サポートに申請](http://azure.microsoft.com/blog/2014/06/04/azure-limits-quotas-increase-requests/) 無償でします。
...
* **サブスクリプション** -Azure サブスクリプションの HPC Pack クラスターを展開するために使用します。 下のコマンドを使用し、Azure サブスクリプションの名前を設定し、それがクライアント コンピューターで一意になるようにします。 このサンプルでは、Azure サブスクリプションの「Subscription-1」を使用します。

    ```
    PS > Get-AzureSubscription –SubscriptionName <SubscriptionName>
    ```

    >[AZURE.NOTE]または、サブスクリプション ID を使用して、使用するサブスクリプションを指定します。 スクリプト フォルダーにある Manual.rtf ファイルを参照します。

* **StorageAccount** -HPC Pack クラスターのすべての永続的なデータは、指定されたストレージ アカウント (この例では allvhdsje) に格納されます。 ストレージ アカウントが存在しない場合、スクリプトによって作成されますで指定したリージョンで **場所**します。

* **場所** -Azure のリージョンに (この例では日本東部) HPC Pack クラスターを展開します。

* **VNet** -仮想ネットワークと、HPC クラスターを作成するサブネットの設定です。 このスクリプトを実行する前に仮想ネットワークとサブネットを自分で作成できます。あるいは、このスクリプトによりアドレス空間が「192.168.0.0/20」の仮想ネットワークと「192.168.0.0/23」のサブネットが作成されます。 この例では、このスクリプトにより、仮想ネットワーク centos7rdmavnetje とサブネット CentOS7RDMACluster が作成されます。

* **ドメイン** -HPC Pack クラスターの Active Directory ドメインの設定です。 このスクリプトで作成された Windows VM はすべてこのドメインに参加します。 現在のところ、このスクリプトでは、ExistingDC、NewDC、HeadNodeAsDC の 3 つのドメインを選択できます。 この例では、完全修飾ドメイン名が hpc.local のドメイン コントローラーとしてヘッド ノードを構成します。

* **データベース** -HPC Pack クラスターのデータベース設定します。 現在のところ、このスクリプトでは、ExistingDB、NewRemoteDB、LocalDB の 3 つのデータベースを選択できます。 この例では、ヘッド ノードでローカル データベースを作成します。

* **ヘッドノード** -HPC Pack ヘッド ノードに設定します。 この例では、クラウド サービス centos7rdma-je で「CentOS7RDMA-HN」という名前の A7 ヘッド ノードを作成します。 リモート (ドメイン不参加) クライアント コンピューターから HPC ジョブを送信するために、このスクリプトにより HPC ジョブ スケジューラ REST API と HPC Web ポータルが有効になります。

* **LinuxComputeNodes** -計算ノードの HPC Pack linux 設定します。 この例では、2 つのサイズの A7 CentOS 7 Linux コンピューティング ノード (CentOS7RDMA-LN1 と CentOS7RDMA-LN2) をクラウド サービス centos7rdma-je で作成します。
...
* この記事の例では Azure Marketplace で入手できる特定の CentOS バージョンを使用してクラスターを作成します。 使用可能なその他のイメージを使用する場合を使用して、 **get azurevmimage** Azure PowerShell コマンドレットを見つける必要があります。 たとえば、すべての CentOS 7.0 イメージを一覧表示するには、次のコマンドを実行します。
    ...
    必要があるし、置換のものを見つけ、 **ImageName** 構成ファイル内の値。
...
3. 下のコマンドを実行し、HPC Pack クラスターをデプロイします。 この例では、構成ファイルは E:\HPCDemoConfig.xml にあるものと想定しています。
    ...
    a.  **AdminPassword** で指定されていない上記のコマンドを促すメッセージ ユーザーのパスワードを入力する *MyAdminName*します。

    b. スクリプトにより構成ファイルの検証が開始されます。 ネットワーク接続の状態にもよりますが、数秒から数分かかります。

    ![検証][検証]

    c. 検証に合格すると、HPC クラスターに作成されるリソースが一覧表示されます。 入力 *Y* を続行します。
...
Linux ノードに Azure File 共有をマウントするには、実行、 **clusrun** ヘッド ノードでコマンドです。 **[Clusrun](https://technet.microsoft.com/library/cc947685.aspx)** 複数のノードで管理タスクを実行する便利な HPC Pack ツールです。 (「」も参照 [Linux ノードの CLusrun](#CLusrun-for-Linux-nodes) この記事の内容です)。
...
REST API を使用してジョブを送信するを参照してください [Creating and Submitting Jobs Microsoft HPC Pack で REST API を使用して、](http://social.technet.microsoft.com/wiki/contents/articles/7737.creating-and-submitting-jobs-by-using-the-rest-api-in-microsoft-hpc-pack-windows-hpc-server.aspx)です。 Python サンプルを参照しても、 [HPC Pack SDK](https://www.microsoft.com/download/details.aspx?id=47756) Linux クライアントからジョブを送信します。

## Linux ノードの clusrun

HPC Pack **clusrun** ツールをコマンド プロンプトまたは HPC クラスター マネージャーを介して Linux ノードでコマンドを実行できます。 基本的な例の一部を次に示します。
...
* インストール、 **gdb** デバッガー ツール **yum** すべて linuxnodes 内のノードがグループ化し、10 分後にノードを再起動します。
...
>[AZURE.NOTE] 内の特定のエスケープ文字を使用する必要があります **clusrun** コマンドです。 この例に示すように、コマンド ウィンドウで ^ を使用すると、">" 記号がエスケープされます。
