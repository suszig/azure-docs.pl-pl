...
[実行後 `apt get 更新` パッケージのローカル リストを更新することができますし、インストールするこれらのパッケージを 1 つの `apt get インストール` コマンド。
...
これらのパッケージをインストールするには、1 つの `yum インストール` コマンド。
...
これらのパッケージをインストールするには、1 つの `zypper インストール` コマンド。
...
1. **Apache** を設定します。
    ...
    - 次のコマンドを実行して Apache Web サーバーが起動されたことを確認します。
        ...
        - Ubuntu と SLES: `sudo service apache2 再起動`

        - CentOS と Oracle: `sudo service httpd 再起動`
...
2. **MySQL** を設定します。
    ...
    - この手順は MySQL の入力を求める Ubuntu のために必要なことに注意してください `ルート` mysql-server パッケージのインストール時にパスワードです。

    - 他のディストリビューションの場合、次のコマンドを実行して MySQL の root パスワードを設定します。

            # mysqladmin -u root -p password yourpassword

    - MySQL を使用してを管理することができますし、 `mysql` または `mysqladmin` ユーティリティです。
