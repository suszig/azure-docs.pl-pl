...
> [AZURE.NOTE] アフィニティ グループの使用は推奨されません。 これらの使用についてはここでは説明しません。 詳細については、次を参照してください。 [地域 Vnet およびアフィニティ グループについて](../virtual-network/virtual-networks-migrate-to-regional-vnet.md)します。
...
オペレーティング システム ディスクとデータ ディスク blob の最大サイズは 1024 GB とそれに VHD ファイルのメタデータ (フッター) が含まれるため最大サイズは 1023 ギガバイト (GB) がある (1 GB は 1024年<sup>3</sup> バイト)。 Windows にディスク ストライピングを実装して、この制限を超えることができます。
...
> [AZURE.NOTE] Azure リソース マネージャーには、クラウド サービスはありません。 概要については、長所リソース マネージャーの次を参照してください。 [Azure コンピューティング、ネットワークおよび記憶域プロバイダーの Azure リソース マネージャーにおける](../articles/virtual-machines/virtual-machines-azurerm-versus-azuresm.md)します。
...
> [AZURE.NOTE] 通常、内部設置型サブネットの場合は、n 個のホスト ビットのサブネットに対するホスト アドレスの最大数は 2<sup>n</sup> – 2 です。 Azure サブネットでは、n 個のホスト ビットのサブネットに対するホスト アドレスの最大数は 2<sup>n</sup> – 5 (2 に加えて、Azure が各サブネットに使用するアドレスの 3)。
...
[Microsoft Azure サブスクリプションとサービスの制限、クォータ、および制約](../azure-subscription-service-limits.md#storage-limits)
...
[データ センター拡張機能の参照アーキテクチャの図](https://gallery.technet.microsoft.com/Datacenter-extension-687b1d84)

[Azure コンピューティング、ネットワーク、および記憶域プロバイダーでは、Azure リソース マネージャー](../articles/virtual-machines/virtual-machines-azurerm-versus-azuresm.md)
