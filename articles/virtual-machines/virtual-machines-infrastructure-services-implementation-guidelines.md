...
この記事の内容を作成し直した、 [Azure 実装ガイドライン](http://blogs.msdn.com/b/thecolorofazure/archive/2014/05/13/azure-implementation-guidelines.aspx) ブログの投稿です。 基の資料を作成した Santiago Cánepa (Microsoft のアプリケーション開発マネージャー) と Hugo Salcedo (Microsoft のアプリケーション開発マネージャー) に感謝します。

> [AZURE.NOTE] アフィニティ グループは廃止されました。 これらの使用についてはここでは説明しません。 詳細については、次を参照してください。 [地域 Vnet およびアフィニティ グループについて](../virtual-network/virtual-networks-migrate-to-regional-vnet.md)します。
...
Azure Virtual Machine の名前と基になるオペレーティング システムのコンピューター名を同じにすることをお勧めします。 このため、以下の NetBIOS 名前付け規則に従って [Microsoft NetBIOS コンピューターの名前付け規則](https://support.microsoft.com/kb/188997/)します。

### ストレージ アカウント名

ストレージ アカウン名の管理には特別な規則があります。 小文字と数字のみを使用できます。 参照してください [ストレージ アカウントの作成](../storage/storage-create-storage-account.md#create-a-storage-account) の詳細。 また、ストレージ アカウント名は、core.windows.net と組み合わせ、グローバルで有効な一意の DNS 名にする必要があります。 たとえば、ストレージ アカウントの名前が「mystorageaccount」の場合、次の DNS 名が一意となります。
...
ストレージ アカウントはスケーラビリティ ターゲットと関連付けられています。 参照してください [Microsoft Azure サブスクリプションとサービスの制限、クォータ、および制約](../azure-subscription-service-limits.md#storage-limits) 現在 Azure ストレージの制限について理解しておく。 参照してください [Azure ストレージのスケーラビリティおよびパフォーマンスのターゲット](../storage-scalability-targets.md)します。
...
オペレーティング システム ディスクとデータ ディスク blob の最大サイズは 1024 GB (1 GB は 1024 VHD ファイルのメタデータ (フッター) が含まれている必要がありますので最大サイズは 1023 ギガバイト (GB) があります。<sup>3</sup> バイト単位)。 Windows にディスク ストライピングを実装して、この制限を超えることができます。
...
Azure では、仮想マシンのサイズにより、使用できるデータ ディスクの量と帯域幅が制限されます。 詳細については、「 [仮想マシンのサイズ](virtual-machines-size-specs.md)します。
...
詳細については、次を参照してください。 [記憶域スペース - パフォーマンス用の設計](http://social.technet.microsoft.com/wiki/contents/articles/15200.storage-spaces-designing-for-performance.aspx)します。
...
- 名前付け規則を使用してストレージ アカウントのセットを作成します。 Azure ポータル、Azure クラシック ポータルを使用する、または **New-azurestorageaccount** PowerShell コマンドレット。
...
> [AZURE.NOTE] クラウド サービスには、Azure リソース マネージャーではありません。 概要については、長所リソース マネージャーの次を参照してください。 [Azure コンピューティング、ネットワークおよび記憶域プロバイダーの Azure リソース マネージャーにおける](../articles/virtual-machines/virtual-machines-azurerm-versus-azuresm.md)します。
...
- 名前付け規則を使用してクラウド サービスのセットを作成します。 Azure クラシック ポータルを使用する、または **New-azureservice** PowerShell コマンドレット。
...
クロスプレミス仮想ネットワークでは、サブネットを設計する必要があります - 内部設置型リソースを使用すると同じ規則でことに注意している **Azure は常に、各サブネットのアドレス空間の最初の 3 つの IP アドレスを使用**します。 サブネットに必要なアドレスの数を決定するには、現在必要な仮想マシンの数を数え、将来の増加を予測した後、次の表を使用してサブネットのサイズを決定します。
...
> [AZURE.NOTE] 通常、内部設置型サブネットの場合は、n 個のホスト ビットのサブネットに対するホスト アドレスの最大数は 2<sup>n</sup> – 2 です。 Azure サブネットの場合、ホスト ビット数 n のサブネットに対するホスト アドレスの最大数は 2<sup>n</sup> – 5 (2 に加えて、Azure が各サブネットに使用するアドレス数の 3)。
...
- Azure ポータル、Azure クラシック ポータルでは、仮想マシンを作成、 **New-azurevm** PowerShell コマンドレット、Azure CLI、または Resource Manager テンプレートです。
...
- **contosoazfaeusesawebapp** は web サーバー、アプリケーション サーバーとドメイン コントおよびそれらの追加データ ディスクの standard storage
- **contosoazfaeusesasqlclust** SQL Server の premium storage はクラスターおよびそれらの追加データ ディスク
...
- **dc として azfae 使用** ドメイン コント ローラー用
- **azfae 使用として web** web サーバー用
- **アプリとして azfae 使用** アプリケーション サーバー用
- **azfae 使用として sql** SQL Server クラスター内のサーバー
...
- **azfae 使用 vm dc01** 最初のドメイン コント ローラー用
- **azfae 使用 vm dc02** 2 番目のドメイン コント ローラー用
- **azfae 使用 vm web01** 番目の web サーバー用
- **azfae 使用 vm web02** 2 番目の web サーバー用
- **azfae 使用 vm app01** 最初のアプリケーション サーバー用
- **azfae 使用 vm app02** 2 番目のアプリケーション サーバー用
- **azfae 使用 vm sql01** for SQL Server クラスター内の最初の SQL Server
- **azfae 使用 vm sql02** SQL Server クラスター内の 2 番目の SQL サーバー用
- **azfae 使用 vm sqlmn01** SQL Server クラスター内のマジョリティ ノード監視用
