...
9.   **ログイン名**, 、型 *ドメイン*で \sqladmin (を *ドメイン* で sqladmin アカウントが作成したドメインの名前を指定 [フェーズ 2](virtual-machines-workload-high-availability-LOB-application-phase2.md))。 
...
実行しているときにドメイン アカウントの資格情報を与える必要があります、 **Add-computer** コマンドです。
...
4.   **開始する前に** ] ページで [ **次**します。
5.   **サーバーの選択** ページで、プライマリ SQL Server コンピューターの名前を入力をクリックして **追加**, 、順にクリック **次**します。
6.   **検証の警告** ] ページで [ **残念ですが、このクラスターでは、マイクロソフトのサポートは必要ありませんしたがってしないように検証テストを実行します。次をクリックしたときは、クラスターの作成を続行**, 、クリックして **次**します。
7.   **クラスター管理用アクセス ポイント** ] ページで、 **クラスター名**e テキスト ボックスに、クラスターの名前を入力し、順にクリックして **次**します。
8.   **確認** ] ページで [ **次** クラスターの作成を開始します。 
9.   **概要** ] ページで [ **完了**します。
...
18.  **開始する前に** ] ページで [ **次**します。 
19.  **サーバーの選択** ] ページで、名前を入力してをクリックし、 **追加** 、セカンダリ SQL サーバーとクラスター マジョリティ ノードをクラスターに追加します。 2 台のコンピューターを追加したら、クリックして **次**します。
...
20.  **検証の警告** ] ページで [ **残念ですが、このクラスターでは、マイクロソフトのサポートは必要ありませんしたがってしないように検証テストを実行します。次をクリックしたときは、クラスターの作成を続行**, 、クリックして **次**します。 
21.  **確認** ] ページで [ **次**します。
22.  **概要** ] ページで [ **完了**します。
