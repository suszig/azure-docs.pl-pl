...
> [AZURE.IMPORTANT]
> アプリケーションの最大限の効率とパフォーマンスを確保するために、.NET API リスト呼び出しには**常に** ODATADetailLevel を指定することを強くお勧めします。 詳細レベルの指定は、Batch サービスの応答時間の短縮、ネットワーク使用率の改善、およびクライアント アプリケーションによるメモリ使用量の最小化に役立ちます。
...
- **filter** - *filter 文字列* は、返される項目の数を減らす式です。 たとえば、あるジョブの実行中のタスクのみ、またはタスクの実行準備が完了しているコンピューティング ノードのみをリストします。
  ...
  - 実行中のみを一覧表示するフィルター文字列の例は、タスクを表示します `startswith (id、'renderTask') と (状態 eq '実行中)`。
- **select** - 各項目に対して返されるプロパティの値は、*select 文字列*で制限されます。 項目のプロパティ リストを select 文字列に指定でき、リスト クエリの結果では、各項目に対してこれらのプロパティの値のみが返されます。
  ...
  - 各タスクに対して返される 3 つのプロパティを指定する select 文字列の例: `id、状態、stateTransitionTime`
...
- 日付/時刻文字列は、2 つの形式のいずれかし、前にする必要があります `DateTime`
  - W3CDTF 形式の使用例: `creationTime gt DateTime' 2011年-05-08T08:49:37Z'`
  - RFC1123 形式の使用例: `creationTime gt datetime ' Sun, 08 May 2011 08時 49分: 37 GMT'`
- ブール値の文字列がいずれかの `true` または `false`
- 無効なプロパティまたは演算子が指定されている場合、 `400 (Bad Request)` エラーが発生
...
- **REST リスト要求** - この列の各 REST API ページには、*filter* 文字列で許可されるプロパティと操作を指定する表が含まれます。 構築するときに、これらのプロパティ名と操作を使用するが、 [ODATADetailLevel.FilterClause][odata_filter] 文字列。

| .NET リスト メソッド| REST リスト要求|
|---|---|
| [CertificateOperations.ListCertificates][net_list_certs]| [アカウント ][rest_list_certs]
| [CloudTask.ListNodeFiles][net_list_task_files]| [タスク ][rest_list_task_files]
| [JobOperations.ListJobPreparationAndReleaseTaskStatus][net_list_jobprep_status]| [ジョブの準備と ][rest_list_jobprep_status]
| [JobOperations.ListJobs][net_list_jobs]| [アカウント ][rest_list_jobs]
| [JobOperations.ListNodeFiles][net_list_nodefiles]| [[Rest_list_nodefiles] ノードでファイルを一覧表示します。][rest_list_nodefiles]
| [JobOperations.ListTasks][net_list_tasks]| [ジョブ ][rest_list_tasks]
| [JobScheduleOperations.ListJobSchedules][net_list_job_schedules]| [アカウント ][rest_list_job_schedules]
| [JobScheduleOperations.ListJobs][net_list_schedule_jobs]| [ジョブのスケジュール ][rest_list_schedule_jobs]
| [PoolOperations.ListComputeNodes][net_list_compute_nodes]| [プール ][rest_list_compute_nodes]
| [PoolOperations.ListPools][net_list_pools]| [アカウント ][rest_list_pools]

### select 文字列のマッピング

- **BATCH .NET の種類** - Batch .NET API の種類
- **REST API のエンティティ** - この列の各ページには、各種類の REST API プロパティ名をリストする 1 つ以上の表が含まれます。 *select* 文字列を構築する際に、これらのプロパティ名が使用されます。 構築する場合、これらの同じプロパティ名を使用する [ODATADetailLevel.SelectClause][odata_select] 文字列。

| Batch .NET の種類| REST API のエンティティ|
|---|---|
| [証明書の ][net_cert]| [[Rest_get_cert] 証明書に関する情報を取得します。][rest_get_cert]|
| [CloudJob ][net_job]| [ジョブ ][rest_get_job]|
| [CloudJobSchedule ][net_schedule]| [ジョブのスケジュール ][rest_get_schedule]|
| [ComputeNode ][net_node]| [[Rest_get_node] のノードに関する情報を取得します。][rest_get_node]|
| [CloudPool ][net_pool]| [プール ][rest_get_pool]|
| [CloudTask ][net_task]| [タスク ][rest_get_task]|

### 例: filter 文字列の構築

フィルター文字列を構築するときに、 [ODATADetailLevel.FilterClause][odata_filter], 、上記の下の表を参照してください *フィルター文字列のマッピング* を実行する] ボックスの一覧操作に対応する REST API ドキュメント ページを検索します。 そのページの最初の複数行の表に、フィルター可能なプロパティとサポートされている演算子が表示されます。 終了コードが 0 以外のすべてのタスクを取得する場合は、たとえば、この行に [[rest_list_tasks] のジョブに関連するタスクを一覧表示][rest_list_tasks] 許容の演算子と適用可能なプロパティの文字列を指定します。
...
`(executionInfo/exitCode lt 0) または (executionInfo/exitCode gt 0)`

### 例: select 文字列の構築

構築する、 [ODATADetailLevel.SelectClause][odata_select], 、上記の下の表を参照してください *マッピング select 文字列を* し、一覧表示するエンティティの種類に対応する REST API のページに移動します。 そのページの最初の複数行の表に、選択可能なプロパティとサポートされている演算子が表示されます。 ID およびリスト内の各タスクのコマンドラインを取得する場合は、たとえば、これらの行で見つかります適用可能なテーブルで [タスク ][rest_get_task]:

| プロパティ| 型| メモ|
| :--- | :--- | :--- |
| `id`| `String`| `タスクの id です。`|
| `コマンドライン`| `String`| `タスクのコマンドラインです。`|
