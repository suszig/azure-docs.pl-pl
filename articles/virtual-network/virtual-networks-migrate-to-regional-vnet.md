...
また、通常はアフィニティ グループを使用しないことが推奨されます。 VNet の要件とは別に、以前はコンピューティングおよびストレージなどのリソースが互いに近くに配置されるよう、アフィニティ グループを使用することも重要でした。 しかし、現在の Azure のネットワーク アーキテクチャでは、このような配置要件は必要なくなりました。 参照してください [アフィニティ グループと Vm](#Affinity-groups-and-VMs) 、いくつかの固有な場合についてアフィニティ グループを使用することがあります。
...
1. ネットワーク構成ファイルをエクスポートします。 PowerShell または管理ポータルを使用できます。 手順については、管理ポータルを使用して、次を参照してください。 [VNet をネットワーク構成ファイルを使用して構成する](../virtual-networks-using-network-configuration-file/)します。
