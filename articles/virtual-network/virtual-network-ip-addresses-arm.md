...
IP アドレスがパブリック IP リソースに割り当てられている 2 つのメソッドもあります。 *動的* または *静的*します。 既定の割り当て方法は *動的*します。 場合に *動的*, 、IP アドレスが **いない** に割り当てられている、 *パブリック IP アドレス リソース* 、作成時にします。 (VM またはロード バランサー) のようなリソースに関連付けられている場合、 *パブリック IP アドレス リソース* が作成または開始されると、IP アドレスは IP アドレスのプールから割り当てられます。 この IP アドレスは、関連付けられたリソースを削除または停止すると、割り当てが解除され、使用可能なプールに解放されます。

使用する場合 *静的* IP アドレスが割り当てられている割り当て方法、 *パブリック IP アドレス リソース* 、作成時にします。 この場合、関連付けられたリソースの状態に関係なく、同じ IP アドレスが割り当てられたままになります。 使用可能なプールにリリースされた場合にのみ、 *パブリック IP アドレス リソース* は削除またはその割り当て方法を変更して *動的*します。

### DNS の解決
パブリック IP リソースに対して DNS ドメイン名のラベルを指定できます。これによって、対応する DNS エントリが Azure DNS サーバーに作成されます。 対応する FQDN *domainnamelabel*.*場所*. cloudapp.azure.com はパブリック IP リソースに割り当てられた IP アドレスにグローバルに解決されます。

以下で、パブリック IP アドレスに関連付けることができるさまざまな種類のリソースについて説明します。

### 仮想マシン
関連付けられているパブリック IP アドレス、 [仮想マシン](virtual-machines-about.md) (VM) ネットワーク インターフェイス カード (NIC) を使用します。 関連付けられている NIC の数に関係なく、各 VM に割り当てることができるパブリック IP アドレスは 1 つのみです。 のみ、 *動的に* 割り当て *パブリック IP アドレス リソース* VM の NIC に関連付けることができます 複数 NIC の VM が発生した場合、 *パブリック IP アドレス リソース* のみのプライマリ NIC に関連付けることができます。

### Azure Load Balancer
パブリック IP アドレスの関連付けできる、 [Azure ロード バランサー](load-balancer-overview.md) として機能する負荷分散された仮想 IP アドレス (VIP) 到達可能なインターネット経由で (lb による) フロント エンド構成します。 両方とも *動的* または *静的* のパブリック IP リソースは、LB を関連付けることができます。 複数の関連付けを行うことが *のパブリック IP アドレス リソース* 、LB フロント エンドの構成を有効にする複数の SSL ベースの web サイトでのマルチ テナント環境のようなシナリオです。

### Application Gateway
パブリック IP アドレスの関連付けできる、 [Azure Application Gateway](application-gateway-introduction.md) フロント エンド構成か、到達可能なインターネットを使用して構成を負荷分散仮想 IP (VIP)。 現在のところ、のみ、 *動的に* 割り当てられたパブリック IP リソースは、アプリケーション ゲートウェイを関連付けることができます。 ただし、複数のパブリック IP アドレスに関連付けることができます。

### VPN Gateway
関連付けられるパブリック IP アドレスが必要な [Azure VPN Gateway](vpn-gateway-about-vpngateways.md) IP 構成、Azure の仮想ネットワークと内部設置型ネットワークまたは別の Azure 仮想ネットワークの間の VPN 接続を作成するプロセスです。 現在のところ、のみ、 *動的に* 割り当てられたパブリック IP リソースは、VPN ゲートウェイを関連付けることができます。
...
|Load Balancer のフロント エンド|あり|はい|あり|
...
IP アドレスが割り当てられている 2 つのメソッドもあります。 *動的* または *静的*します。 既定の割り当て方法は *動的*, DHCP を使用して IP アドレスが割り当てられたであるという点です。 代わりに、割り当て方法を明示的に指定するとして *静的* と共に IP アドレスを指定します。 この場合、IP アドレスがサブネットのアドレスの範囲内にあり、自由である (他のリソースに割り当てられていない) 限り、リソースには指定した IP アドレスが割り当てられます。

さまざまな種類のリソースにプライベート IP アドレスを割り当てることができます。 両方の割り当て方法 (*静的* または *動的*) これらすべてのリソースの種類の作業です。
...
|リソース|静的割り当て|動的割り当て|複数の IP アドレス|
|---|---|---|---|
|仮想マシン (VM)/ネットワーク インターフェイス カード (NIC)|あり|はい|あり|
|Load Balancer のフロント エンド|あり|はい|あり|
