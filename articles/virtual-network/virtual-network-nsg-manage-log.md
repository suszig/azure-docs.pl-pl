...
- **イベント ログ:** Vm および MAC アドレスに基づいてインスタンスのロールにどのような NSG ルールが適用されるを表示するこのログを使用することができます。 これらのルールの状態は 60 秒ごとに収集されます。 
- **カウンター ログ:** このログを使用するを拒否またはトラフィックを許可する各 NSG ルールが適用された回数を表示します。

>[AZURE.WARNING] ログでは、リソース マネージャーの配置モデルでデプロイされているリソースの使用のみです。 クラシック デプロイメント モデルのリソースには使用できません。 詳細については、2 つのモデルの参照、 [理解リソース マネージャーの展開と従来のデプロイ](resource-manager-deployment-model.md) 記事です。
...
2.  プレビュー ポータルで、クリックして **参照** >> **ネットワーク セキュリティ グループ**します。
...
4.  **設定** ブレードで、をクリックして **診断**, 、し、[、 **診断** ウィンドウで、次へ] **ステータス**, 、] をクリックして **上**
5.  **設定** ブレードで、をクリックして **ストレージ アカウント**, 、および既存のストレージ アカウントを選択するか、新しいものを作成します。  

>[AZURE.INFORMATION] 監査ログには、別々 のストレージ アカウントは不要です。 イベントとルールのログ記録にストレージを使用すると、サービス料金が発生します。

6. ドロップ ダウン リストのすぐ下 **ストレージ アカウント**, イベント、カウンター、またはその両方にログインし、をクリックするかどうかを **保存**します。
...
- **Azure ツール:** Azure PowerShell、Azure コマンド ライン インターフェイス (CLI)、Azure の REST API または Azure プレビュー ポータルで監査ログから情報を取得します。  それぞれの方法の詳細な手順の詳細については、 [リソース マネージャーでの操作を監査](resource-group-audit.md) 記事です。
...
- [表示し、Power BI での Azure 監査ログを分析](https://azure.microsoft.com/blog/analyze-azure-audit-logs-in-powerbi-more/) ブログの投稿です。
