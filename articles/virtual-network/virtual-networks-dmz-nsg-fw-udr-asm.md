...
>[AZURE.IMPORTANT] ルーティングが定義されるユーザーと共に IP 転送を有効にする必要があります。
...
>[AZURE.IMPORTANT] すべての上記の規則が作成されるは、トラフィックを許可または必要に応じて、拒否することを確認するには、各ルールの優先度を確認する必要があります。 この例では、各ルールに優先順位が割り当てられています。 ルールの優先順位に間違いがあって、ファイアウォールが適用されないケースが多く見られます。 少なくともファイアウォールそのものを管理するためのルールには、無条件で最も高い優先度を設定してください。
...
はっきりと次に示すために、変数が、エンドポイントの使用は **のみ** セキュリティ クラウド サービスで開かれています。 こうすることによって、すべての受信トラフィックが確実にファイアウォールで処理 (ルーティング、NAT 変換、破棄) されます。
...
>[AZURE.NOTE] 2 つの理由で、ファイアウォールの場合、RDP サービスは必要ありません。1) 最初は RDP、および 2) のポート 22 ではなく VM 管理用のポート 22 で SSH が使用されるため、他の 2 つの管理ポートが管理との接続を許可するように次に示す最初の管理規則で許可される、VM のファイアウォールは、Linux ベースのイメージです。

### ファイアウォール ルールの作成
この例には 3 種類のファイアウォール ルールが使われており、それぞれ以下のアイコンで明確に区別されています。

アプリケーションのリダイレクト ルール:![アプリケーションのリダイレクト アイコン][7]
...
- **ファイアウォールの管理規則**: このアプリのリダイレクト ルールがこの例では、ネットワークの仮想アプライアンスの管理ポート Barracuda NG firewall に渡すへのトラフィックを許可します。 管理ポートは 801 と 807、および (必要に応じて) 22 です。 外部ポートと内部ポートは同じです (ポートの変換は使用しません)。 この SETUP-MGMT-ACCESS は既定のルールであり初期設定で有効になっています (Barracuda NG Firewall Version 6.1)。

    ![ファイアウォールの管理の規則][10]

>[AZURE.TIP] このルールで元のアドレス空間は、管理 IP アドレスの範囲がわからない場合はこのスコープを削減するさらに低くなります管理ポートへの攻撃対象領域。
...
このルールを作成するうえで重要となるフィールドは 4 つです。
  ...
  3.    [宛先]: 必要があります、 *ファイアウォール上のローカル ポート*, 、"DCHP 1 のローカル IP"または eth0 静的 ip アドレスを使用する場合。 ご使用のネットワーク アプライアンスが複数のローカル インターフェイスを備えている場合、序数 (eth0、eth1 など) は異なる場合があります。 これは、ファイアウォールからの送信ポートになります (受信ポートと同じ場合もあります)。実際のルーティング先は [Target List] フィールドで指定します。
...
>[AZURE.TIP] ソースおよびサービスの各フィールドの範囲を縮小すると、攻撃対象領域が減少します。 正常な機能を維持できる範囲で最も限定的なスコープを使用してください。

- **アプリケーション トラフィック規則**: は、フロント エンドの web トラフィック用の最初とバック エンド トラフィック (例: データ層に web サーバー) の 2 つ目、2 つのアプリケーションのトラフィック規則があります。 これらのルールは、(サーバーが置かれている) ネットワーク アーキテクチャとトラフィック フロー (トラフィック フローの方向と使用ポート) によって異なります。
    ...
    **注**: ことが、対象のリストのフィールドに割り当てられているポートはありません。 したがって、web サーバーのリダイレクトには、受信ポート 80 (または選択したサービスの場合は 443) を使用します。 それ以外の待機ポートが Web サーバーに使用されている場合 (たとえばポート 8080 など)、[Target List] フィールドに「10.0.1.4:8080」と指定することで、ポートのリダイレクトも行うことができます。

    次のアプリケーション トラフィック ルールは、バックエンド ルールです。Web サーバーから AppVM01 サーバー (ただし AppVM02 は不可) への、任意のサービスを介した通信を許可します。

    ![ファイアウォール AppVM01 規則][13]
    ...
    このスクリーン ショットでは、「\ < 明示的な dest\ >」は送信先として 10.0.2.5 を示すために、目的のフィールドで使用されます。 ここは、この例のように明示的に指定してもかまいませんが、(DNS サーバーの前提条件で作成した) 名前付きネットワーク オブジェクトを指定することもできます。 どちらの方法を使用するかは、ファイアウォールの管理者が決めます。 下にある最初の空の行をダブルクリックして、明示的な宛先 e として 10.0.2.5 を追加するに \ < 明示的な dest\ > がポップアップ ウィンドウで、アドレスを入力します。

    内部トラフィックが対象となるため、この Pass ルールに NAT は不要です。[Connection Method] は "No SNAT" に設定してください。

    **注**: このルールで基になるネットワークのリソースは、フロント エンド サブネット上にある場合のみ、または既知の特定の数、web サーバーのネットワーク オブジェクト リソースでした作成されますフロント エンド サブネット全体ではなく、これらの正確な IP アドレスを複数指定できます。

>[AZURE.TIP] このルールを使用して、サービス「、」サンプル アプリケーションを簡単にセットアップして使用する、ICMPv4 これにより、また 1 つのルールでは、(ping)。 しかし、このような設定は推奨されません。 ポートとプロトコル ("Services") は、アプリケーションが正常に動作する範囲で可能な限り狭くし、この領域への攻撃対象領域を小さくすることをお勧めします。

<br />

>[AZURE.TIP] このルールで使用されている追加先の明示的な参照を示しています、ファイアウォールの構成全体で一貫したアプローチを使用してください。 可読性と保守性の点から、名前付きネットワーク オブジェクトの使用をお勧めします。 ここで explicit-dest を使っているのは、あくまで代替の参照方法を示すことが目的であり、一般にはお勧めできません (特に複雑な構成の場合)。

- **インターネット ルールへのアウト バウンド**: このを渡すルールを選択した宛先ネットワークに渡す任意の発信元ネットワークからのトラフィックが許可されます。 通常、Barracuda NG ファイアウォールにはこのルールが、既定のルールとして (ただし無効状態で) あらかじめ設定されています。 このルールを右クリックすると、[Activate Rule] コマンドにアクセスできます。 ここに示したルールの Source 属性には、このドキュメントの前提条件セクションで参考用として作成した 2 つのローカル サブネットを追加しています。

    ![送信のファイアウォール規則][14]

- **DNS ルール**: このを渡すルールでは、DNS サーバーに渡す (ポート 53) の DNS トラフィックのみです。 この環境では、フロントエンドからバックエンドへのトラフィックは大部分がブロックされるので、このルールによって DNS を明示的に許可しています。

    ![ファイアウォールの DNS の規則][15]

    **注**: このスクリーン ショット、接続方法は含まれます。 このルールは内部 IP アドレス間のトラフィックを対象としているため、NAT 変換は不要です。そのため、この Pass ルールでは [Connection Method] を "No SNAT" に設定しています。

- **サブネット ルール**: このを渡すルールは、既定のルールがアクティブ化およびフロント エンド サブネット上の任意のサーバーに接続するためのバックエンド サブネット上の任意のサーバーを許可するように変更します。 すべて内部トラフィックが対象となるため、[Connection Method] は [No SNAT] に設定してください。

    ![ファイアウォール VNet 内の規則][16]

    **注**: これは、このルールの重要なルールの「1 つの方向」これは、双方向のチェック ボックスはチェックされません (でもなく、ほとんどの規則はチェックイン) は、フロント エンド ネットワークが、逆は不可能にバックエンド サブネットからの接続を開始できます。 このチェック ボックスがオンの場合、双方向のトラフィックにルールが適用されます。論理図の仕様の観点からそれは望ましくありません。

- **すべてのトラフィックのルールを拒否する**: これは常に (優先順位)、という点で最後のルールとそのため、トラフィック フローのいずれかの比較に失敗する場合上記のルールこのルールは削除されます。 これは既定のルールであり通常は有効になっているため、一般に変更は不要です。 

    ![ファイアウォールでは、ルールを拒否する][17]

>[AZURE.IMPORTANT] すべての上記の規則が作成されるは、トラフィックを許可または必要に応じて、拒否することを確認するには、各ルールの優先度を確認する必要があります。 この例では、Barracuda 管理クライアントにおける転送ルールの順序、つまり、そのメイン グリッドに表示させる順序でルールを記載しています。
...
## トラフィックのシナリオ
>[AZURE.IMPORTANT] キー takeway は覚えておくことは **すべて** ファイアウォール経由のトラフィックが開始します。 たとえば、リモート デスクトップで IIS01 サーバーにアクセスするとします。IIS01 サーバーは、実際にはフロント エンド クラウド サービスのフロント エンド サブネット上にありますが、このサーバーにアクセスするためには、RDP でファイアウォールのポート 8014 にアクセスし、ファイアウォールで RDP 要求を内部的に IIS01 RDP ポートにルーティングしてもらう必要があります。 RDP で直接 IIS01 に到達する経路は (ポータルから見える範囲では) 存在しないので、Azure ポータルの [接続] ボタンは機能しません。 つまり、インターネットからのすべての接続は secscv001.cloudapp.net:xxxx (セキュリティ サービスとポート) にリダイレクトされます。
