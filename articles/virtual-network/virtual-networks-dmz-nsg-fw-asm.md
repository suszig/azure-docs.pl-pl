...
インターネットへ向かうトラフィックは、既定の送信ルールによって許可されています。 この例では、送信トラフィックを許可していますが、送信ルールに対する変更は一切行っていません。 [主なセキュリティ境界のドキュメント] で検出された両方の手順については、ユーザー定義されているルーティングが必要なことができる別の例については、これでトラフィックをロックダウンする [ホーム] です。

上記の説明の NSG ルールは、[例 1 - ビルドを Nsg の単純な DMZ] で、NSG のルールとよく似ています [例 1] です。 個々の NSG ルールとその属性について詳しくは、そのドキュメントで NSG の説明を再確認してください。
