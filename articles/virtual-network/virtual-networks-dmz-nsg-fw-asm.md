...
インターネットへ向かうトラフィックは、既定の送信ルールによって許可されています。 この例では、送信トラフィックを許可していますが、送信ルールに対する変更は一切行っていません。 両方の手順については、ユーザー定義されているルーティングが必要なを参照して別の例については、これでトラフィックをロックダウンする、 [主なセキュリティ境界ドキュメントの ][home]します。

上記の説明の NSG ルールは、NSG のルールとよく似ています [例 1 - ビルド Nsg の ][example1]します。 個々の NSG ルールとその属性について詳しくは、そのドキュメントで NSG の説明を再確認してください。
...
次のような変換先の NAT 規則アイコン: ![変換先の NAT アイコン][2]
