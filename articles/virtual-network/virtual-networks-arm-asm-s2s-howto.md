...
>[AZURE.NOTE] この記事では、従来の Vnet が既にある ARM Vnet とを熟知している従来の Vnet の S2S VPN 接続を構成すると想定しています。 クラシック間の S2S VPN 接続に関する詳細なエンド ツー エンド ソリューションの ARM Vnet を参照してくださいと [ソリューション ガイド - S2S VPN を使用して、従来の VNet と ARM VNet を接続](../virtual-networks-arm-asm-s2s.md)します。

次に示すのは、従来の VNet と ARM VNet の間に Azure ゲートウェイを使用して S2S VPN 接続を作成するために、実行すべきタスクの概要です。

1- [従来の VNet 用の VPN ゲートウェイの作成](#Step-1:-Create-a-VPN-gateway-for-the-classic-VNet)

2- [ARM VNet 用の VPN ゲートウェイの作成](#Step-2:-Create-a-VPN-gateway-for-the-ARM-VNet)

3- [ゲートウェイ間の接続を作成します。](#Step-3:-Create-a-connection-between-the-gateways)
...
2. 画面の下部左側にある上隅には、をクリックして、 **新規** をクリックして、 **ネットワーク サービス**, 、] をクリックし、 **仮想ネットワーク**, 、順にクリック **ローカル ネットワークの追加**します。
3.  **ローカル ネットワークの詳細を指定** し、ウィンドウの右下隅に接続する ARM VNet の名前が、矢印ボタンをクリックしてウィンドウで、型です。
3. アドレス空間で **開始 IP** テキスト ボックスに接続する ARM VNet 用のネットワーク プレフィックスを入力します。 
4.  **CIDR (アドレス数)** ボックスの一覧で、ネットワークに接続する ARM VNet で使用される CIDR ブロックの部分では使用されるビット数を選択します。
5.  **VPN デバイス IP アドレス (オプション)**, 、有効なパブリック IP アドレスを入力します。 この IP アドレスは後で変更します。 次に、画面の右下にあるチェック マーク ボタンをクリックします。 次の図は、このページの設定例です。

    ![ローカル ネットワークの設定](..\virtual-network\media\virtual-networks-arm-asm-s2s-howto\figurex1.png)

5.  **ネットワーク** ] ページで、をクリックして **仮想ネットワーク**, では、従来の VNet をクリックし、をクリックして **構成**します。
6.  **サイト対サイト接続** を有効にする、 **ローカル ネットワークに接続** チェック ボックスをオンします。
7. 使用可能なネットワークの一覧から、手順 4 で作成したローカル ネットワークを選択して、 **ローカル ネットワーク** ドロップダウンをクリックして **保存**します。
8. 設定が保存さたら] をクリックして **ダッシュ ボード**, 、し、ページの下部にある次のようにクリックします。 **ゲートウェイの作成**, 、順にクリック **動的ルーティング**, 、順にクリック **はい**します。
...
4. 次のコマンドを実行して、ゲートウェイに使用されているサブネットを取得します。
    ...
    >[AZURE.IMPORTANT] ゲートウェイ サブネットが存在する必要があり、GatewaySubnet という必要があります。
...
5. 次のコマンドを実行して、ゲートウェイの IP 構成オブジェクトを作成します。 ゲートウェイ サブネットの ID が使用されていることに注意してください。 そのサブネットは、VNet に存在している必要があります。。
    ...
    >[AZURE.IMPORTANT]  *SubnetId* と *PublicIpAddressId* パラメーターは、サブネット、および IP アドレスのオブジェクトの repectively から id プロパティ渡す必要があります。 単純な文字列を使用することはできません。
...
1. Https://manage.windowsazure.com から従来のポータルを開き、必要に応じて、資格情報を入力します。
2. クラシック ポータルで下へスクロールし、をクリックして **ネットワーク**, 、] をクリックし、 **ローカル ネットワーク**, 、をクリックし、接続となる ARM VNet] をクリックして、 **編集** ] ボタンをクリックします。
3.  **VPN デバイス IP アドレス (オプション)**, 、上記の手順 2 では、ARM VNet ゲートウェイの取得の IP アドレスを入力および右下にある右矢印をクリックし、チェック マーク ボタンをクリックします。
