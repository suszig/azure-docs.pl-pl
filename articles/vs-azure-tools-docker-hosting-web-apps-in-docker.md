...
1. 環境変数に Docker ホストを割り当て、検証します。 そのためには、コマンド プロンプトに次のコマンドを入力します (*NameofAzureVM* を Docker ホストの名前に置き換えます)。
    ...
    これらのコマンドを起動することで、追加する `– H (ホスト) tcp://< NameofAzureVM >. cloudapp.net:2376` と `--TLSVERIFY` 実行するコマンドごとにします。
...
1. ここで、これらのようなコマンドを発行して、Docker ホストが存在し、機能しているかどうかをテストできます。
   ...
   | `docker ログ < Docker コンテナー名 >`| 指定されたコンテナーのログを取得します。|
   | `docker images`| イメージの一覧を取得します。|

    Docker コマンドの一覧については、コマンドを入力した `docker` コマンド プロンプトでします。 詳細については、次を参照してください。 [Docker コマンドライン](https://docs.docker.com/reference/commandline/cli/)します。
