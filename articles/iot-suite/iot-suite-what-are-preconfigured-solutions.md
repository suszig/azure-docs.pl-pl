...
- [リモートの監視 ][lnk-remote-monitoring]
- [予測のメンテナンス ][lnk-predictive-maintenance]
...
[EventPocessorHost ][lnk-event-processor] で実行されるインスタンス、 [web ジョブの ][lnk-web-job], 、ASA ジョブ 2 で識別されるコマンド応答し、デバイス ステータス メッセージを処理およびでは、この情報を格納、 [[lnk ドキュメント db]「Azure-documentdb][lnk-document-db] データベースです。
...
> [AZURE.NOTE] ソリューション portal も IoT Hub を維持する [デバイスの id のレジストリ (lnk identity レジストリ)][lnk-identity-registry] ソリューションの DocumentDB データベース内の豊富なデバイスの状態情報のストアと同期します。
...
- [Azure の IoT ソリューションの概要 ][lnk-suite-overview]
- [[Lnk-preconf--はじめに] 構成済みの IoT ソリューションを使ってみる][lnk-preconf-get-started]
