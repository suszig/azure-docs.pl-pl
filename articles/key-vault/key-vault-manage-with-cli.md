...
>[AZURE.NOTE]  このチュートリアルでは、Azure アプリケーションの記述手順のいずれかが含まれている、key vault でキーやシークレットを使用するアプリケーションの承認方法を説明する手順については含まれません。
>
...
- コマンドライン インターフェイス バージョン 0.9.1 以降。 最新のバージョンをインストールして、Azure サブスクリプションへの接続、次を参照してください。 [インストールして、Azure クロスプラット フォーム コマンド ライン インターフェイスを構成する](xplat-cli-install.md)です。
...
>[AZURE.NOTE]  ログイン方法は、組織のアカウントでのみ機能します。 組織のアカウントは、組織で管理されるユーザーであり、組織の Azure Active Directory テナントで定義されています。
...
- **名前**: 例では、これは ContosoKeyVault です。 この名前を他の Key Vault コマンドレットに使用できます。
...
2. 左側で、次のようにクリックします。 **Active Directory**, 、し、アプリケーションを登録するディレクトリを選択します。 <br> <br> 注: Key Vault を作成した Azure サブスクリプションが含まれている、同じディレクトリを選択する必要があります。 不明なディレクトリをこの場合は、クリックして **設定**, 、key vault を使用したサブスクリプションを見つけて、最後の列に表示されているディレクトリの名前に注意してください。

3. クリックして **アプリケーション**します。 このページはのみを表示する場合は、ディレクトリにアプリケーションが追加されていない、 **アプリを追加** リンクします。 リンクをクリックするか、また、クリックすることができます、 **追加** コマンド バーにします。
4.   **アプリケーションの追加** ウィザードの **実行する操作ですか?** ] ページで [ **[組織が開発中のアプリケーションを追加**します。
5.   **アプリケーションについてお聞かせ** ] ページで、アプリケーションの名前を指定して選択 **WEB アプリケーションや WEB API** (既定値)。 [次へ] アイコンをクリックします。
...
8.  クイック スタート] ページで、次のようにクリックします。 **構成**します。
