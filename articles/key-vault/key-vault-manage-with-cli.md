...
>
>現時点では、Azure ポータルで Azure Key Vault を構成できません。 代わりに、クロスプラット フォーム コマンド ライン インターフェイスの手順を使用します。 Azure PowerShell の手順については、次を参照してください。 または、 [対応するチュートリアル](key-vault-get-started.md)します。
...
- コマンドライン インターフェイス バージョン 0.9.1 以降。 最新のバージョンをインストールして、Azure サブスクリプションへの接続、次を参照してください。 [のインストールと Azure クロスプラット フォーム コマンド ライン インターフェイスを構成](xplat-cli-install.md)します。
- このチュートリアルで作成したキーやパスワードを使用して構成されるアプリケーション。 サンプル アプリケーションは、 [Microsoft ダウンロード センター](http://www.microsoft.com/download/details.aspx?id=45343)します。 手順については、付属の Readme ファイルをご覧ください。

## Azure クロスプラットフォーム コマンドライン インターフェイスでのヘルプの取得

このチュートリアルでは、コマンドライン インターフェイス (Bash、Terminal、Command プロンプト) に慣れていることを前提としています。

--help または -h パラメーターを使用すると特定のコマンドに対するヘルプを表示できます。 Azure のヘルプまたは、 [コマンド ][options] 形式は、同じ情報を返す場合にも使用できます。 たとえば、次のコマンドでは、すべて同じ情報が返されます。
...
- [インストールして、Azure クロスプラット フォーム コマンド ライン インターフェイスを構成する方法](xplat-cli-install.md)
- [Azure リソース マネージャーで Azure クロスプラット フォーム コマンド ライン インターフェイスを使用します。](xplat-cli-azure-resource-manager.md)
...
1.  ログインのログイン、 [Azure 管理ポータル](https://manage.windowsazure.com/), 、[Active Directory] をクリックします。
...
Microsoft Azure で組織のアカウントを使用する方法の詳細については、次を参照してください。 [組織としての Microsoft Azure へのサインアップ](sign-up-organization.md)します。
...
Azure クロスプラット フォーム コマンド ライン インターフェイスを構成する方法の詳細については、次を参照してください。 [インストールおよび Azure クロスプラット フォーム コマンド ライン インターフェイスの構成方法](xplat-cli-install.md)します。
...
最初のパラメーターはリソース グループ名で、2 番目のパラメーターは場所です。 場所に、コマンドを使用して `azure の場所のリスト` この例のように、別の場所を指定する方法を識別します。 詳細については、必要がある場合は、入力: `azure ヘルプ場所`



## Key Vault を作成します

使用して、 `azure key vault を作成` key vault を作成するコマンドです。 このスクリプトには、3 つの必須パラメーター (リソース グループ名、Key Vault 名、地理的な場所) が含まれています。
...
- **vaultUri**: 例では、これは https://contosokeyvault.vault.azure.net です。 その REST API から資格情報コンテナーを使用するアプリケーションは、この URI を使用する必要があります。
...
Azure Key Vault でソフトウェアで保護されたキーを作成する場合を使用して、 `azure キーを押す` コマンドを使用し、次を入力します。
...
- 表示するには、キーを入力: `azure key vault キー リスト - 資格情報コンテナー名 'ContosoKeyVault'`
- 表示するには、シークレットを入力: `azure key vault シークレットのリスト - 資格情報コンテナー名 'ContosoKeyVault'`
...
キーやシークレットの資格情報コンテナーにアクセスするアプリケーションを承認するために使用して、 `azure key vault の設定ポリシー` コマンドです。
...
さらに安心感を高めたい場合には、ハードウェア セキュリティ モジュール (HSM) でキーのインポートや生成を行うことができ、キーは HSM の境界内から出ることはありません。 HSM は、FIPS 140-2 レベル 2 で検証済みです。 このセクションをスキップし、移動する場合はこの要件が自分に当てはまらない、 [key vault と関連付けられているキーやシークレットを削除](#delete-the-key-vault-and-associated-keys-and-secrets)します。
...
この BYOK パッケージを生成する方法について詳細を参照してください。 [Azure Key Vault の hsm 保護キーを使用する方法](key-vault-hsm-protected-keys.md)します。
