...
3. W **nowe konto** wprowadź ustawienia dla nowego konta bazy danych Azure rozwiązania Cosmos. 
    ...
    Interfejs API|SQL (DocumentDB)|Interfejs API Określa typ konta, aby utworzyć. Azure DB rozwiązania Cosmos udostępnia cztery interfejsy API w celu odpowiada potrzebom aplikacji: Gremlin (wykres), bazy danych MongoDB, SQL (DocumentDB) i tabeli (wartość klucza), każdy wymagających obecnie oddzielne konto. <br><br>Wybierz **SQL (DocumentDB)** ponieważ w tym szybkiego startu tworzysz dokumentu bazy danych, która jest zapytań przy użyciu składni SQL.<br><br>[Więcej informacji na temat interfejsu API usługi DocumentDB](../articles/cosmos-db/documentdb-introduction.md)|
