...
>[AZURE.NOTE] ネットワーク管理者は、CIDR 表記に慣れています。 CIDR に慣れていない場合 [詳細情報を入手](http://whatismyipaddress.com/cidr)します。
...
| **サブネット**| VNet を構成するサブネットのコレクション| see [subnets](#Subnets) below.|
...
| **networkSecurityGroup**| サブネットに適用される NSG| see [NSGs](#Network-Security-Group)|
| **routeTable**| サブネットに適用されるルート テーブル| see [UDR](#Route-table)|
| **ipConfigurations**| サブネットに接続されている NIC で使用される IP 構成オブジェクトのコレクション| see [UDR](#Route-table)|
