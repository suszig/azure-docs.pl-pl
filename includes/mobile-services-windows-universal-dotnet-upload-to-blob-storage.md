...
2. 左ペインで選択、 **オンライン** カテゴリで、検索 `WindowsAzure.Storage`, 、] をクリックして **インストール** 上、 **Azure Storage** をパッケージ化し、使用許諾契約に同意します。 
...
1. Visual Studio で、Windows アプリ プロジェクト用の Package.appxmanifest ファイルを開き、 **機能** ] タブの有効化、 **web カメラ** と **マイク** 機能します。
...
3. Windows アプリ プロジェクトで、MainPage.xaml ファイルを開くし、置換、 **StackPanel** 要素の 1 つ目の直後後 **QuickStartTask** 要素を次のコード。
...
3. Windows Phone アプリ プロジェクトで MainPage.xaml ファイルを開くし、置換、 **ButtonSave** 要素を次のコード。
...
3. 共有 MainPage.cs プロジェクト ファイルを開き、次の追加 **を使用して** ステートメント。
...
3. をクリックするかをクリックし、写真を撮ってプレビューをタップして **アップロード** 新しい項目を挿入し、イメージをアップロードします。
...
4. 新しい項目がアップロード イメージと共にリスト ビューに表示されます。
    ...
    >[AZURE.NOTE]イメージは Blob ストレージから自動的にダウンロード時に、 *imageUri* 新しい項目のプロパティにバインドする、 **イメージ** コントロールです。

5. アプリケーションを停止し、Windows Phone プロジェクト バージョンのアプリケーションを再起動します。

    前にアップロードされた画像が表示されます。

6. 前回と同様に、テキスト ボックスに、いくつかのテキストを入力し、クリックして **フォト**します。

    ![](./media/mobile-services-windows-universal-dotnet-upload-to-blob-storage/mobile-upload-blob-app-view-wp8.png)

3. クリックし、写真を撮ってプレビューをタップして **アップロード** 新しい項目を挿入し、イメージをアップロードします。
