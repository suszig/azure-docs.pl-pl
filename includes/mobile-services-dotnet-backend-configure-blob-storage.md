...
この POST メソッドが今すぐあり有効期間が 5 分に生成された SAS の値を代入挿入された項目の新しい SAS を生成、 `sasQueryString` 返された項目のプロパティです。  `ImageUri` クライアント UI でのバインド中に画像の表示を有効にするプロパティが設定を新しい BLOB のリソース パスにもします。
>[AZURE.NOTE] このコードでは、個々の BLOB に対して SAS が作成されます。 同じ SAS を使用してコンテナーに複数の blob をアップロードする場合は、代わりに呼び出せる、 <a href="http://go.microsoft.com/fwlink/?LinkId=390455" target="_blank">generateSharedAccessSignature メソッド</a> で次のように、空の blob リソース名。 
