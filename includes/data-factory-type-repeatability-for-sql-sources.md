...
> [AZURE.NOTE] スライスを再実行する自動的に Azure Data Factory で再試行ポリシーを指定します。

### メカニズム 1

活用できる **sqlWriterCleanupScript** プロパティをスライスの実行時にまずクリーンアップ アクションを実行します。 
...
再現性を実現するために別のメカニズムは専用の列がありません (**sliceIdentifierColumnName**)、ターゲット テーブル。 この列は、Azure Data Factory がソースと対象の同期状態を保つために使用されます。 この手法は、対象の SQL テーブル スキーマを変更または定義する際に柔軟性がある場合に機能します。 
