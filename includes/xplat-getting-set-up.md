...
サービス管理モードで命令型コマンドに Azure CLI を使用するときは、できれば最新バージョンを使用します。 バージョンを確認するには、入力 `azure--バージョン`します。 次のような結果が表示されます。
...
使用するアカウントに Azure CLI を接続すると、複数のサブスクリプションが存在する場合があります。作成する場合は、する必要があります使用可能なサブスクリプション、アカウント確認」と入力して `azure アカウント] ボックスの一覧`, 、し」と入力して使用するサブスクリプションを選択 `azure アカウントを設定 < サブスクリプション id または名前 > true` 、_subscription id または名前 _ は、サブスクリプション id またはの現在のセッションで使用するには、サブスクリプション名。次のように表示されます。
