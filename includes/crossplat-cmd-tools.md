...
> [AZURE.NOTE] 発行の設定をインポートする場合は、内に、Azure サブスクリプションにアクセスするための資格情報が格納されて、 `user` フォルダーです。 `user` フォルダーはオペレーティング システムによって保護されていますが、 追加の作業を行って `user` フォルダーを暗号化することをお勧めします。 そのためには、次の操作を行います。    
> 
