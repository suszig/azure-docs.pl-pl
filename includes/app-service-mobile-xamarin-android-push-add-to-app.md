4. という名前のプロジェクトに新しいクラスを作成 `ToDoBroadcastReceiver`します。
...
5. **OnRegistered** イベント ハンドラーをオーバーライドする次のコードを **PushHandlerService** クラスに追加します。
 ...
 このメソッドでは、返された GCM 登録 ID を使用して、プッシュ通知のために Azure に登録します。 タグは、作成後にのみ登録に追加できます。 詳細については、次を参照してください。 [方法: プッシュのタグを有効にするデバイスのインストールにタグを追加](../articles/app-service-mobile/app-service-mobile-dotnet-backend-how-to-use-server-sdk.md#tags)します。
