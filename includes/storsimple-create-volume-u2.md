...
2. ボリュームの追加ウィザードの **[基本設定]** で、次の手順を実行します。
   ...
   4. 矢印アイコンをクリックして ![矢印アイコン](./media/storsimple-create-volume-u2/HCS_ArrowIcon-include.png) 次のページに移動します。
...
3. **[追加設定]** ダイアログ ボックスで、新しい Access Control レコード (ACR) を追加します。
    ...
    1. ACR の **[名前]** を入力します。
    2. **[ISCSI イニシエーターの名前]** に Windows ホストの iSCSI 修飾名 (IQN) を指定します。 IQN を取得していない場合は、「 [Windows Server ホストの IQN を取得する](#get-the-iqn-of-a-windows-server-host)します。
...
4. チェック アイコンをクリックして ![チェック マーク アイコン](./media/storsimple-create-volume-u2/HCS_CheckIcon-include.png)します。 指定された設定でボリュームが作成されます。
