...
このセクションからプロジェクトを更新する、 [Get started with Mobile Services] チュートリアル写真を撮影して Azure Blob ストレージにアップロードします。 このチュートリアルでは、イメージをキャプチャするには、使用、 [CameraCaptureTask] から、 `Microsoft.Phone.Tasks` 名前空間。 このクラスは Windows Phone デバイスのカメラ UI を起動し、写真を撮影して、自動的にそのイメージを Windows Phone デバイスのカメラ ロールに保存します。 イメージをカメラ ロールに保存したくない場合は使用して、 [PhotoCamera] クラス、 `Microsoft.Devices` 名前空間代わりにします。

1. ソリューション エクスプ ローラーで、プロジェクトの下で、Visual Studio の **プロパティ**します。 WMAppManifest.xml ファイルを開くと、 **機能** ] タブをクリックして、カメラを有効にする **id \_cap\_isv\_camera**します。 ファイルを閉じて変更内容を保存します。
...
2. MainPage.xaml ファイルを開き、置換、 **グリッド** という名前の要素 **ContentPanel** を次のコード。
    ...
    起動する新しいボタンが追加されます、 [CameraCaptureTask] にイメージを追加し、 **ItemTemplate** され、Blob ストレージ サービスにアップロードされたイメージの URI として、バインディング ソースを設定します。
...
5. MainPage.xaml.cs プロジェクト ファイルで、MainPage クラスを更新します。 宣言する次のコードを追加、 [CameraCaptureTask] し、キャプチャしたイメージを参照するストリーム オブジェクト。
...
7. MainPage.xaml.cs プロジェクト ファイルで、MainPage クラスを更新します。 カメラ イメージをキャプチャするユーザーを許可するように UI が表示され、次のコードを追加するときに、 **イメージのキャプチャ** ] ボタンをクリックします。
...
2. テキスト ボックスに、いくつかのテキストを入力し、クリックして **イメージのキャプチャ**します。
