...
最新の利用可能に [Python 2.7 の Microsoft Visual C コンパイラ][], 、Python 2.7 のネイティブ コードを含むパッケージの作成が容易になります。
...
Mac/linux ユーザー: Windows コンピューターへのアクセスを取得していない場合に表示 [仮想マシンを実行している Windows のを作成する][] Azure で VM を作成する方法についてです。 その方法に従って、wheel をビルドしてリポジトリに追加し、必要に応じて VM を破棄します。

Python 2.7 をインストールできます [Python 2.7 の Microsoft Visual C コンパイラ][]します。

Python 3.4 の場合は、インストールすることができます [の Microsoft の Visual C 2010 Express][]します。

wheel をビルドするには、wheel パッケージが必要です。

    env\scripts\pip install wheel

使う `pip ホイール` 、依存関係をコンパイルします。

    env\scripts\pip wheel azure==0.8.4

これによって、\wheelhouse フォルダーに .whl ファイルが作成されます。 \wheelhouse フォルダーと wheel ファイルをリポジトリに追加します。

追加する、requirements.txt を編集、 `--検索リンク` 上部にあるオプション。 このオプションは、python パッケージのインデックスにアクセスする前に、ローカル フォルダーで完全一致を検索するように pip に指示します。

    --find-links wheelhouse
    azure==0.8.4

\Wheelhouse フォルダーにすべての依存関係を含めるし、python package index を使用しない場合は、追加することで、パッケージのインデックスを無視する pip で強制的にすることができます `-インデックスが` 、requirements.txt の先頭にしません。
