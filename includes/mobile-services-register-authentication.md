...
> [AZURE.IMPORTANT] Id プロバイダーの開発者サイトで、正しいリダイレクト URI を設定したことを確認します。 上記リンクの各プロバイダーに関する手順で説明しているように、リダイレクト URI は .NET バックエンド サービス用と JavaScript バックエンド サービス用とで異なる場合があります。 リダイレクト URI が正しく構成されていないと、ログイン画面が正しく表示されず、アプリが予期しない異常を起こす場合があります。
