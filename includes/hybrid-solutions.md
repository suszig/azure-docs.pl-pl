...
この場合のプロセスは単純です。送信者がメッセージを Service Bus キューに送信すると、受信者が後でそのメッセージを取得します。 キューに指定できるは単一の受信者のみ [図 2](#Fig2) 番組、または複数のアプリケーションが同じキューから読み取ることができます。 後者のケースでは、各メッセージは 1 つの受信者によって読み取られます。つまり、マルチキャスト サービスではトピックを使用する必要があります。
