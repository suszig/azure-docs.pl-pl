...
- アプリケーションが Azure クラウド サービスで実行されている場合は、Azure サービス構成システム (`*.csdef` ファイルと `*.cscfg` ファイル) を使用して、接続文字列を保存します。 参照してください [を作成して、クラウド サービスをデプロイする方法](../articles/cloud-services/cloud-services-how-to-create-deploy.md) の詳細については Azure クラウド サービスの構成。
...
2.  をクリックして、 **設定** ] タブをクリックし、キーを押して、 **設定の追加** ] ボタンをクリックします。  
    ![Visual Studio で、クラウド サービスの設定を追加します。][connection-string2]

    新しい **Setting1** 設定のグリッドにエントリが表示されます。

3.   **型** 新しいドロップダウン **Setting1** エントリを選択
    **接続文字列**します。  
    ![接続文字列の種類の設定][connection-string3]

4.  クリックして、 **...** の右端にあるボタン、 **Setting1** エントリです。
...
5.  (Microsoft ストレージ エミュレーターをターゲットとするかどうかを選択します。
    ...
    > [AZURE.NOTE] Azure Storage に関連するコストの発生を回避するには、ストレージ エミュレーターを対象にすることができます。 ただし、クラウド内の Azure ストレージ アカウントをターゲットとしても、このチュートリアルを実行するための利用料金はごくわずかです。
...
6.  エントリの変更 **名前** から **Setting1** のわかりやすい名前を
