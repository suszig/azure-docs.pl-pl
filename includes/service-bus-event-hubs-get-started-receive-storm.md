...
このチュートリアルでは、 [HDInsight Storm:operator[]][] はまだ利用可能で、Event Hubs スパウトと共にインストールします。

1. 次の [HDInsight Storm の使用開始](../hdinsight/hdinsight-storm-overview.md) 手順を新しい HDInsight クラスターを作成し、リモート デスクトップを介して接続します。

2. コピー、 `%STORM_HOME%\examples\eventhubspout\eventhubs-storm-spout-0.9-jar-with-dependencies.jar` ファイルをローカル開発環境にします。 これには events-storm-spout が含まれています。
...
8. **Pom.xml**, で次の依存関係を追加、 `< 依存関係 >` ノードです。
