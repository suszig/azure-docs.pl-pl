...
1 つの例が、同じ Azure VNet (ただしレプリカをホストしていない) 内のいずれかの VM から、リスナーに接続する場合です。 このテストを完了する簡単な方法は、SSMS を可用性グループ リスナーに接続してみることです。 別の単純な方法は、実行する [SQLCMD.exe](https://technet.microsoft.com/library/ms162773.aspx) 次のようにします。
