...
Azure Virtual Machines では、幅広いクラウド ホスティング タスクを処理できます。 ただし、VM インフラストラクチャを作成して管理するには、特別なスキルと多くの労力が必要です。 簡単な (かつ安い) ソリューションがある場合は、web アプリやモバイル アプリ バックエンド、API アプリなどを実行する Vm に制御を完全する必要がある、: *サービスとしてのプラットフォーム* (PaaS)。 PaaS では、アプリケーションを実行する VM に対する管理作業のほとんどを Azure が処理します。 [Azure App Service](../article/app-service/app-service-value-prop-what-is.md) は完全に管理された PaaS 製品をビルドすることができるようにする展開、および秒単位でエンタープライズ レベルのアプリをスケールします。
...
**図: Azure App Service は、静的な web ページ、一般的な web アプリケーション、およびさまざまなテクノロジで構築されたカスタム web アプリケーションをサポートします。 モバイル バックエンド、API アプリ、および (web ジョブを使用して) web 以外のコンピューティング ワークロードを実行することもできます。** 

Azure App Service で、任意の種類のコンピューティング ワークロードを使用してを実行することも、 [WebJobs](../article/app-service-web/websites-webjobs-resources.md) 機能します。 
...
として、 [図](#appservice_diagram) に示すようにすることができますコードやその他の web コンテンツを Azure App Service に発行いくつかの方法です。 たとえば、FTP、FTPS、Microsoft の WebDeploy テクノロジを使用できます。 Azure App Service は、ソース管理システム (Git、GitHub、CodePlex、BitBucket、Dropbox、Mercurial、Team Foundation Server、クラウドベースの Team Foundation Service など) からのコードの発行もサポートしています。
