...
期限切れのトークンは、検出して更新できるようにする必要があります。 使用してこれを行うには、 [ServiceFilter](http://dl.windowsazure.com/androiddocs/com/microsoft/windowsazure/mobileservices/ServiceFilter.html) から、 [Android クライアント ライブラリ](http://dl.windowsazure.com/androiddocs/)します。
...
2. 次のメンバーを追加、 `ToDoActivity` クラスです。
...
5. ToDoActivity.java ファイルでは、更新、 `認証` メソッドをクラスのトークンとトークン キャッシュの更新を強制するブール型パラメーターを受け入れるようにします。 また、ブロックされたスレッドが新しいトークンを取得できるように、認証が完了したらこれらにスレッドに通知する必要があります。
...
6. ToDoActivity.java ファイルにコードを追加、新しい `RefreshTokenCacheFilter` 、ToDoActivity クラス内のクラス。
...
7. ToDoActivity.java ファイルにコードを追加、新しい `ProgressFilter` 、ToDoActivity クラス内のクラス。
...
8. ToDoActivity.java ファイルでは、更新、 `onCreate` メソッドを次のようにします。
